...
bun run eval:list    # list all eval runs from ~/.zstack/projects/<slug>/evals/
...
`CLAUDE_CONFIG_DIR`, temp `ZSTACK_HOME`, `--strict-mcp-config`); per-test
...
`bin/zstack-context-bill` shows the full token bill-of-materials for a skills
...
**Egress receipts at every off-machine sink** (v1.63.0.0+). Every zstack-initiated
send off the machine MUST write a hash-chained receipt to
`~/.zstack/security/egress.jsonl` BEFORE the send: TypeScript callers use
...
with a SKILL.md symlink inside (e.g., `qa/SKILL.md -> zstack/qa/SKILL.md`), plus
...
(`_zstack-command`, `connect-chrome`) install as rewritten copies, never symlinks.
This ensures Claude discovers them as top-level skills, not nested under `zstack/`.
...
lockfile already exists. `bin/zstack-version-bump` (via `lib/version-source.ts`)
...
`lib/version-source.ts` header; `test/zstack-version-bump.test.ts` pins the
contract.
...
`bin/zstack-detach` — NEVER as a plain backgrounded Bash task. A plain background
...
the run can also die to idle-sleep. `zstack-detach` fixes both: a fresh session
...
- Use the `eval:bg*` scripts (`eval:bg`, `eval:bg:all`, `eval:bg:gate`,
  `eval:bg:periodic`) — they wrap the eval command in `zstack-detach` with the
  machine-wide `zstack-evals` lock (concurrent worktrees serialize instead of
  saturating the shared model API), a per-tier watchdog, and a **run-scoped** log
  under `~/.zstack-dev/eval-runs/` (no shared-`/tmp` collision). Each prints its
  ...
  `zstack-detach [--lock NAME] [--timeout SECS] [--label LBL] --
...
- Then **poll the printed logfile** with a death-aware watcher: break on the
  guaranteed `### zstack-detach EXIT=<code> ###` sentinel (success AND failure are
...
Native OpenClaw skills live in `openclaw/skills/zstack-openclaw-*/SKILL.md`.
...
(`GBRAIN_CONTEXT_LOAD` / `GBRAIN_SAVE_RESULTS`) blocks that `zstack-config
gbrain-refresh` renders into the install (those generated blocks differ from
`main` by design). After deploying, re-run `zstack-config gbrain-refresh` to
...
store at `~/.zstack/projects/<slug>/decisions.jsonl` so neither you nor the user
...
- **Resurface** active decisions before re-deciding: `bin/zstack-decision-search`
...
- **Capture** a DURABLE decision when you or the user make one:
  `bin/zstack-decision-log '{"decision":"...","rationale":"...","scope":"repo|branch|issue","source":"user|skill|agent","confidence":1-10}'`.

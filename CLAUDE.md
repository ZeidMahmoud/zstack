...
bun run eval:list    # list all eval runs from ~/.zstack-dev/evals/
...
**Env keys in Conductor workspaces.** The `ZSTACK_*` env-shim (v1.39.2.0+,
`lib/conductor-env-shim.ts`) promotes `ZSTACK_ANTHROPIC_API_KEY` /
`ZSTACK_OPENAI_API_KEY` to their canonical names inside zstack's TS binaries.
Tests run through zstack entrypoints inherit this promotion automatically.
...
--verbose`). Results are persisted to `~/.zstack-dev/evals/` with auto-comparison
...
├── bin/             # CLI utilities (zstack-repo-mode, zstack-slug, zstack-config, etc.)
...
├── open-zstack-browser/  # /open-zstack-browser skill (launch ZStack Browser)
├── connect-chrome/  # symlink → open-zstack-browser (backwards compat)
...
│   └── add-host/    # /zstack-contrib-add-host skill
...
2. **If missing, AskUserQuestion** — let the user tell you or let zstack search the repo
3. **Persist the answer to CLAUDE.md** so we never have to ask again

This applies to test commands, eval commands, deploy commands, and any other
project-specific behavior. The project owns its config; zstack reads it.
...
tighter V0 prose set `zstack-config set explain_level terse` (binary switch,
...
server must pass `false` so their discovery files survive zstack teardown cycles.
...
match) which would kill sibling zstack sessions on the same host; the new
...
connection immediately. `Set-Cookie: zstack_pty=...` is kept as a
...
PTY via `window.zstackInjectToTerminal(text)`, exposed by
...
return 403. SSE endpoints use a 30-minute HttpOnly `zstack_sse` cookie minted via
`POST /sse-session` (never valid against `/command`). Tunnel-surface rejections go
to `~/.zstack/security/attempts.jsonl` via `tunnel-denial-log.ts`. Before editing
...
**Env knobs:**
- `ZSTACK_SECURITY_OFF=1` — emergency kill switch. Classifier stays off even if
...
- Classifier model cache: `~/.zstack/models/testsavant-small/` (112MB, first run only)
  plus `~/.zstack/models/deberta-v3-injection/` (721MB, only when ensemble enabled)
- Attack log: `~/.zstack/security/attempts.jsonl` (salted sha256 + domain only,
  rotates at 10MB, 5 generations)
- Per-device salt: `~/.zstack/security/device-salt` (0600)
- Session state: `~/.zstack/security/session-state.json` (cross-process, atomic)

## Dev symlink awareness

When developing zstack, `.claude/skills/zstack` may be a symlink back to this
working directory (gitignored). This means skill changes are **live immediately**,
great for rapid iteration, risky during big refactors where half-written skills
could break other Claude Code sessions using zstack concurrently.

**Check once per session:** Run `ls -la .claude/skills/zstack` to see if it's a
symlink or a real copy. If it's a symlink to your working directory, be aware that:
- Template changes + `bun run gen:skill-docs` immediately affect all zstack invocations
- Breaking changes to SKILL.md.tmpl files can break concurrent zstack sessions
- During large refactors, remove the symlink (`rm .claude/skills/zstack`) so the
  global install at `~/.claude/skills/zstack/` is used instead

**Prefix setting:** Setup creates real directories (not symlinks) at the top level
with a SKILL.md symlink inside (e.g., `qa/SKILL.md -> zstack/qa/SKILL.md`). This
ensures Claude discovers them as top-level skills, not nested under `zstack/`.
...
before going live (especially if the user is actively using zstack in other windows).
...
migration script to `zstack-upgrade/migrations/`. Read CONTRIBUTING.md's "Upgrade
migrations" section for the format and testing requirements. The upgrade skill runs
these automatically after `./setup` during `/zstack-upgrade`.
...
direct `gh issue create`, and `ZSTACK_REDACT_PREPUSH=skip` all bypass it. It
...
- **CLI:** `bin/zstack-redact` (exit 0 clean / 2 MEDIUM / 3 HIGH; `--json`,
  `--auto-redact`, `--repo-visibility`, `--from-file`). `bin/zstack-redact-prepush`
...
- **Visibility (no tier promotion):** resolve once per run, order = local config
  (`zstack-config get redact_repo_visibility`, ~/.zstack so never committed) → gh
...
- **Audit:** the /spec semantic pass appends a content-free record (categories +
  body sha256, no spec text) to `~/.zstack/security/semantic-reviews.jsonl` (0600).
...
`bin/zstack-next-version` advances within the chosen bump level rather than
...
When estimating or discussing effort, always show both human-team and CC+zstack time:
...
Contributors can store long-range vision docs and design documents in `~/.zstack-dev/plans/`.
...
1. Push your branch
2. Fetch and reset in the skill directory: `cd ~/.claude/skills/zstack && git fetch origin && git reset --hard origin/main`
3. Rebuild: `cd ~/.claude/skills/zstack && bun run build`

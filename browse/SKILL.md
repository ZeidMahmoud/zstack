...
preamble-tier: 1
version: 2.0.0
description: "Drive a real browser through Aside: open a page, read it, click through a flow, take screenshots, check console errors. (zstack)"
triggers:
  - browse a page
  - open this url
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "browse" --outcome OUTCOME \
...
same skill drives zstack's own headless browser, `$B` — the Browser fallback
...
5. A missing `ZSTACK_STEP_OK` or a line starting with `[error` is a failure. Quote the
...
shares the `$B` daemon with a remote agent, /open-zstack-browser launches the headed
ZStack Browser. If a task needs a vendor dashboard or any other third-party site, it
...
> **STOP.** Before using any command or snapshot flag beyond the Browser fallback translation table — the full generated reference for every browse command, its argument shape, and every snapshot flag, Read `~/.claude/skills/zstack/browse/sections/command-list.md` and execute it

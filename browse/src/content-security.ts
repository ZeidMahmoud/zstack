 ...
 * Marks hidden elements with data-zstack-hidden attribute.
  ...
  return page.evaluate((ariaPatterns: string[]) => {
    ...
    for (const el of elements) {
      if (el instanceof HTMLElement) {
        ...
        if (isHidden) {
          el.setAttribute('data-zstack-hidden', 'true');
...
export async function getCleanTextWithStripping(page: Page | Frame): Promise<string> {
  const raw = await page.evaluate(() => {
    ...
    clone.querySelectorAll('[data-zstack-hidden]').forEach(el => el.remove());
...
/**
 * Clean up data-zstack-hidden attributes from the page.
 * Should be called after extraction is complete.
 */
export async function cleanupHiddenMarkers(page: Page | Frame): Promise<void> {
  await page.evaluate(() => {
    document.querySelectorAll('[data-zstack-hidden]').forEach(el => {
      el.removeAttribute('data-zstack-hidden');
    });
  });
}

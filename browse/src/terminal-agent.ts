/**
 * Terminal Agent — PTY-backed Claude Code terminal for the zstack browser
...
const STATE_FILE = process.env.BROWSE_STATE_FILE || path.join(process.env.HOME || '/tmp', '.zstack', 'browse.json');
...
const OWNER_WATCHDOG_MS = parseInt(
  process.env.ZSTACK_TERMINAL_OWNER_WATCHDOG_MS || '15000',
  10,
);
...
const KEEPALIVE_INTERVAL_MS = parseInt(
  process.env.ZSTACK_PTY_KEEPALIVE_INTERVAL_MS || '25000',
  10,
);
...
const RING_BUFFER_MAX_BYTES = parseInt(
  process.env.ZSTACK_PTY_RING_BUFFER_BYTES || `${1024 * 1024}`,
  10,
);
...
const DETACH_WINDOW_MS = parseInt(
  process.env.ZSTACK_PTY_DETACH_WINDOW_MS || '60000',
  10,
);
 ...
 *      (updated continuously by the zstack browser extension).
...
function buildTabAwarenessHint(stateDir: string): string {
  ...
  return [
    'You are running inside the zstack browser sidebar with live access to the user\'s browser tabs.',
    '',
...
function spawnClaude(cols: number, rows: number, lifecycle: PtyLifecycle) {
  ...
  // headed-mode browser; BROWSE_NO_AUTOSTART prevents claude's zstack
...
function buildServer(port: number) {
  return Bun.serve({
    ...
    fetch(req, server) {
      ...
      //         - Cookie zstack_pty (works for non-browser callers and
      ...
      if (url.pathname === '/ws') {
        ...
        // with a `zstack-pty.` prefix (which we strip). Browsers send a
        ...
        for (const raw of protoHeader.split(',').map(s => s.trim()).filter(Boolean)) {
          const candidate = raw.startsWith('zstack-pty.') ? raw.slice('zstack-pty.'.length) : raw;
        ...
        // Fallback: Cookie zstack_pty (legacy / non-browser callers).
...
async function main() {
  ...
  const pauseFile = process.env.NODE_ENV === 'test' ? process.env.ZSTACK_TERMINAL_TEST_PUBLISH_BARRIER : undefined;
  if (pauseFile) {

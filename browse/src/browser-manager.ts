...
/**
 * Detect whether ZSTACK_CHROMIUM_PATH points at a custom Chromium build that
 * already bakes the zstack extension in as a component extension (e.g.,
 * ZStack Browser.app / GBrowser). Passing --load-extension against such a
 * binary triggers a ServiceWorkerState::SetWorkerId DCHECK because two
 * copies of the same service worker try to register.
 *
 * Resolution:
 *   1. ZSTACK_CHROMIUM_KIND === 'custom-extension-baked' (preferred, explicit)
 *   2. ZSTACK_CHROMIUM_PATH path substring contains 'GBrowser' or 'gbrowser'
 *      (fallback for callers that only set the path)
 */
export function isCustomChromium(): boolean {
  if (process.env.ZSTACK_CHROMIUM_KIND === 'custom-extension-baked') return true;
  const p = process.env.ZSTACK_CHROMIUM_PATH || '';
...
export function shouldEnableChromiumSandbox(): boolean {
  ...
  // so /qa hangs without --no-sandbox). Setting ZSTACK_CHROMIUM_NO_SANDBOX=1
  // forces the sandbox off without changing the default for everyone else.
  // See #1562.
  if (process.env.ZSTACK_CHROMIUM_NO_SANDBOX === '1') return false;
 ...
 * (chromium.executablePath()). A bundle supplied via ZSTACK_CHROMIUM_PATH
 * belongs to the wrapper/embedder — its plist legitimately says "ZStack
 ...
 * ZSTACK_CHROMIUM_PATH executable itself.
 *
...
export function probePoisonedChromiumBundle(chromiumExecutablePath: string): void {
  ...
  const customPath = process.env.ZSTACK_CHROMIUM_PATH;
  ...
  if (!fs.readFileSync(chromePlist, 'utf-8').includes('ZStack Browser')) return;
  ...
  throw new PoisonedBundleError(
    'Chromium bundle was mutated by a previous zstack version (broken codesign seal — ' +
...
export async function handleChromiumDisconnect(browser: Browser | null): Promise<void> {
  ...
  console.error('[browse] Console/network logs flushed to .zstack/browse-*.log');
  process.exit(1);
}
...
export class BrowserManager {
  ...
  /**
   * Find the zstack Chrome extension directory.
  ...
  async launch() {
    ...
    const { STEALTH_LAUNCH_ARGS, buildZStackLaunchArgs } = await import('./stealth');
    const launchArgs: string[] = [...STEALTH_LAUNCH_ARGS, ...buildZStackLaunchArgs()];
    ...
    if (extensionsDir) {
      ...
      // already bakes the extension in (e.g., GBrowser / ZStack Browser.app).
  ...
  // ─── Headed Mode ─────────────────────────────────────────────
  /**
   * Launch Playwright's bundled Chromium in headed mode with the zstack
  ...
  async launchHeaded(authToken?: string): Promise<void> {
    ...
    // Find the zstack extension directory for auto-loading
    ...
    const launchArgs = [
      ...
      ...STEALTH_LAUNCH_ARGS,
      // ZStack Pack 1: per-install hardware/GPU/UA-CH overrides for the
    ...
    if (extensionPath) {
      ...
      // (gbrowser / ZStack Browser.app). Loading it twice causes a
      ...
      // the extension is component-baked — it reads ~/.zstack/.auth.json at
      // startup to learn how to call the daemon).
      // Write to ~/.zstack/.auth.json (not the extension dir, which may be read-only
      // in .app bundles and breaks codesigning).
      if (authToken) {
        const fs = require('fs');
        const path = require('path');
        const zstackDir = path.join(process.env.HOME || '/tmp', '.zstack');
        mkdirSecure(zstackDir);
        const authFile = path.join(zstackDir, '.auth.json');
        try {
    ...
    // single-instance CLI check for zstack.
    cleanSingletonLocks(userDataDir);

    // Support custom Chromium binary via ZSTACK_CHROMIUM_PATH env var.
    // Used by ZStack Browser.app to point at the bundled Chromium.
    const executablePath = process.env.ZSTACK_CHROMIUM_PATH || undefined;

    // NOTE (#2242): the in-place "rebrand" that patched the Chromium .app's
    // Info.plist (global "Google Chrome for Testing" → "ZStack Browser"
    ...
    // ZStack Browser.app wrapper (ZSTACK_CHROMIUM_PATH), never in a mutation
    ...
    // ZSTACK_CHROMIUM_PATH bundle belongs to the wrapper/embedder and is
    // never probed.
    if (!executablePath) {
      try {
    ...
    // the previous "ZStackBrowser" branding suffix was itself a high-entropy
    ...
    // as ZStackBrowser" contradiction codex flagged.
    ...
    // heal out when ZSTACK_CHROMIUM_PATH supplies the bundle — that bundle
    ...
    const indicatorScript = () => {
      const injectIndicator = () => {
        if (document.getElementById('zstack-ctrl')) return;

        const topLine = document.createElement('div');
        topLine.id = 'zstack-ctrl';
        topLine.style.cssText = `
          position: fixed; top: 0; left: 0; right: 0; height: 2px;
          background: linear-gradient(90deg, #F59E0B, #FBBF24, #F59E0B);
          background-size: 200% 100%;
          animation: zstack-shimmer 3s linear infinite;
        ...
        style.textContent = `
          @keyframes zstack-shimmer {
            0% { background-position: 200% 0; }
            100% { background-position: -200% 0; }
          }
          @media (prefers-reduced-motion: reduce) {
            #zstack-ctrl { animation: none !important; }
          }
        `;
  ...
  async setDeviceScaleFactor(scale: number, width: number, height: number): Promise<string | null> {
    ...
    if (scale < 1 || scale > 3) {
      throw new Error(`viewport --scale: value must be between 1 and 3 (zstack policy cap), got ${scale}`);
    }
    if (this.connectionMode === 'headed') {
  ...
  async handoff(message: string): Promise<string> {
    ...
    try {
      ...
      const launchArgs: string[] = ['--hide-crash-restore-bubble', ...STEALTH_LAUNCH_ARGS, ...buildZStackLaunchArgs()];
      if (extensionPath) {
        launchArgs.push(`--disable-extensions-except=${extensionPath}`);
        launchArgs.push(`--load-extension=${extensionPath}`);
      ...
      // This path previously hardcoded ~/.zstack/chromium-profile, silently
      // ignoring $CHROMIUM_PROFILE / $ZSTACK_HOME and skipping the lock
      ...
      // ZSTACK_CHROMIUM_PATH bundle is never passed in. The rethrown typed

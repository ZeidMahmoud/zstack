 ...
 *   2. git rev-parse --show-toplevel → projectDir/.zstack/
...
export function resolveConfig(
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    projectDir = path.dirname(stateDir); // parent of .zstack/
  } else {
    projectDir = getGitRoot() || process.cwd();
    stateDir = path.join(projectDir, '.zstack');
...
/**
 * Create the .zstack/ state directory if it doesn't exist.
...
export function ensureStateDir(config: BrowseConfig): void {
  ...
  // redundant safety, kept so `.zstack/` still reads as ignored in git status.
  try {
  ...
  // Ensure .zstack/ is in the project's .gitignore
  // First, check if git already ignores .zstack/ (via global excludes, .git/info/exclude, or parent .gitignore)
  if (isIgnoredByGit(config.projectDir, '.zstack/')) return;

  const gitignorePath = path.join(config.projectDir, '.gitignore');
  try {
    const content = fs.readFileSync(gitignorePath, 'utf-8');
    if (!content.match(/^\.zstack\/?$/m)) {
      const separator = content.endsWith('\n') ? '' : '\n';
      fs.appendFileSync(gitignorePath, `${separator}.zstack/\n`);
    }
  } catch (err: any) {
    if (err.code !== 'ENOENT') {
...
/**
 * Resolve the zstack home directory.
 *
 * Honors the existing convention used by telemetry.ts and domain-skills.ts:
 *   1. ZSTACK_HOME env (explicit override)
 *   2. $HOME/.zstack (default)
 */
export function resolveZstackHome(): string {
  return process.env.ZSTACK_HOME || path.join(os.homedir(), '.zstack');
}

/**
 * Read one key from the flat-YAML config store at <zstack home>/config.yaml
 * (the shape bin/zstack-config writes: `key: value` lines). Tolerates
 ...
 */
export function readZstackConfigYamlKey(key: string): string | null {
  const escaped = key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  try {
    const yaml = fs.readFileSync(path.join(resolveZstackHome(), 'config.yaml'), 'utf-8');
    // Last match wins: bin/zstack-config's `get` reads duplicates with
 ...
 * stays OFF unless the user explicitly ran `zstack-config set pair_agent on`
 ...
 *
 * Env override `ZSTACK_PAIR_AGENT=on|off` wins (used by tests and as an
 * emergency knob), mirroring the telemetry env-hint convention.
 */
export function isPairAgentEnabled(): boolean {
  const env = process.env.ZSTACK_PAIR_AGENT;
  if (env === 'on') return true;
  if (env === 'off') return false;
  // Canonical store: ~/.zstack/config.yaml (flat `key: value` lines, written
  // by bin/zstack-config — which is what the /pair-agent consent step runs).
  ...
  const yamlValue = readZstackConfigYamlKey('pair_agent');
  if (yamlValue === 'on') return true;
  if (yamlValue === 'off') return false;
  try {
    const raw = fs.readFileSync(path.join(resolveZstackHome(), 'config.json'), 'utf-8');

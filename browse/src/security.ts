 ...
 *
 * Cross-process state lives at ~/.zstack/security/session-state.json
...
export type LayerName =
  | 'testsavant_content'
  | 'deberta_content'        // opt-in ensemble layer (ZSTACK_SECURITY_ENSEMBLE=deberta)
...
export interface AttemptRecord {
  ...
  layer: LayerName;
  verdict: Verdict;
  zstackVersion?: string;
}

const SECURITY_DIR = path.join(os.homedir(), '.zstack', 'security');
 ...
 * ~/.zstack/security/device-salt (0600). Random per-device, prevents rainbow
...
/**
 * Try to locate the zstack-telemetry-log binary. Resolution order matches
 * the existing skill preamble pattern (never relies on PATH — packaged
 * binary layouts can break that).
 *
 * Order:
 *  1. ~/.claude/skills/zstack/bin/zstack-telemetry-log  (global install)
 *  2. .claude/skills/zstack/bin/zstack-telemetry-log    (symlinked dev)
 *  3. bin/zstack-telemetry-log                          (in-repo dev)
 */
function findTelemetryBinary(): string | null {
  const candidates = [
    path.join(os.homedir(), '.claude', 'skills', 'zstack', 'bin', 'zstack-telemetry-log'),
    path.resolve(process.cwd(), '.claude', 'skills', 'zstack', 'bin', 'zstack-telemetry-log'),
    path.resolve(process.cwd(), 'bin', 'zstack-telemetry-log'),
  ];
 ...
 * ZSTACK_*_BIN override pattern from `browse/src/claude-bin.ts:resolveClaudeCommand`
 * (introduced in v1.24.0.0 #1252) so users on WSL/MSYS2/non-default Git Bash
 * installs can redirect.
 *
 * Override precedence:
 *   1. ZSTACK_BASH_BIN (or BASH_BIN) — absolute path or PATH-resolvable command.
...
export function resolveBashBinary(env: NodeJS.ProcessEnv = process.env): string | null {
  ...
  const override = (env.ZSTACK_BASH_BIN ?? env.BASH_BIN)?.trim();
  if (override) {
 ...
 * Win32: wraps in bash explicitly. `zstack-telemetry-log` is a shell script
 ...
 * Bun.which + ZSTACK_*_BIN override pattern as claude-bin.ts.
 *
 * Returns null when bash can't be resolved on Windows (rare — Git Bash ships
 * with the standard zstack install path). Caller skips spawn; the local
...
/**
 * Fire-and-forget subprocess invocation of zstack-telemetry-log with the
 ...
 * zstack-telemetry-log (which respects the user's telemetry tier setting).
...
// per-tab file under ~/.zstack/security/decisions/, sidebar-agent polls

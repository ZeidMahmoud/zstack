 ...
 *      a ZSTACK_CHROMIUM_PATH bundle belongs to the wrapper/embedder and is
 *      never touched (same scope contract as probePoisonedChromiumBundle).
 *   3. Force-reinstall Chromium FROM THE ZSTACK INSTALL ROOT (ENG-OV3: the
...
/**
 * Find the zstack install root whose node_modules pins the SAME
 ...
 */
export function findZstackInstallRoot(
  ...
  const roots = candidates ?? [
    ...
    // path '.claude/skills/zstack' under the daemon's cwd — often an
 ...
 * on a ZSTACK_CHROMIUM_PATH bundle — that belongs to the wrapper/embedder.
...
/**
 * Run `bunx playwright install --force chromium` from the zstack install
...
export async function maybeHealXProtectKill(
  ...
  if (opts.usesCustomExecutable) {
    // A ZSTACK_CHROMIUM_PATH bundle belongs to the wrapper/embedder — never
  ...
  const root = (deps.installRoot ?? findZstackInstallRoot)(revision);
  if (!root) {
...
export function buildXProtectGuidance(originalMessage: string): string {
  return (
    ...
    'Fix manually: run `bunx playwright install chromium` from your zstack install ' +
    '(the directory whose node_modules pins playwright — ~/.claude/skills/zstack for ' +

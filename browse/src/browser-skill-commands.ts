 ...
 * and passes it via ZSTACK_SKILL_TOKEN. The skill never sees the daemon root
 ...
 * ZSTACK_PORT, so a synchronous wait would deadlock it.
 */
async function runToFiles(cmd: string[], opts: RunToFilesOptions): Promise<RunToFilesResult> {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-skill-'));
 ...
 *    ZSTACK_PORT and ZSTACK_SKILL_TOKEN are always set.
 ...
 * Includes: minimal PATH, locale, terminal type. Skills get ZSTACK_PORT +
 * ZSTACK_SKILL_TOKEN injected separately.
 */
...
export function buildSpawnEnv(opts: BuildEnvOptions): Record<string, string> {
  ...
  if (opts.trusted) {
    ...
    for (const [k, v] of Object.entries(process.env)) {
      ...
      if (k === 'ZSTACK_TOKEN') continue; // never propagate root token
      out[k] = v;
    }
  ...
  // intentionally — e.g. an internal-tool skill — but we still strip ZSTACK_TOKEN
  // above.)
  if (!opts.trusted) {
  ...
  out.ZSTACK_PORT = String(opts.port);
  out.ZSTACK_SKILL_TOKEN = opts.skillToken;

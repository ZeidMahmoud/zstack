 ...
 *
 * Scope:
 *   - per-project: ~/.zstack/projects/<slug>/learnings.jsonl
 *   - global:      ~/.zstack/global-domain-skills.jsonl
 *
...
function globalFile(): string {
  return path.join(zstackHome(), 'global-domain-skills.jsonl');
}

function projectFile(slug: string): string {
  return path.join(zstackHome(), 'projects', slug, 'learnings.jsonl');
}

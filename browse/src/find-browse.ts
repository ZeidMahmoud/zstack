/**
 * find-browse — locate the zstack browse binary.
 *
...
export function locateBinary(): string | null {
  ...
  if (root) {
    for (const m of markers) {
      const local = join(root, m, 'skills', 'zstack', 'browse', 'dist', 'browse');
    ...
    // - zstack repo dev workflow before `./setup` runs
  ...
  // Global fallback
  for (const m of markers) {
    const global = join(home, m, 'skills', 'zstack', 'browse', 'dist', 'browse');

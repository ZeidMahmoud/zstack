 ...
 * ZSTACK_STEALTH=extended, off by default) DOES fake plugins — that mode
 ...
 *      detectors flag.
 *   4. Report per-install hardware values via ZSTACK_HW_CONCURRENCY /
 *      ZSTACK_DEVICE_MEMORY env vars (set by gbd at startup via
 ...
 * on macOS), passed through the ZSTACK_* env vars. Each field falls
...
// the UA-CH cmdline switch in buildZStackLaunchArgs (which reads ZSTACK_PLATFORM
...
export function readHostProfile(): HostProfile {
  ...
  const concurrency = Number(env.ZSTACK_HW_CONCURRENCY);
  const memory = Number(env.ZSTACK_DEVICE_MEMORY);
  return {
...
export function buildStealthScript(hw: HostProfile): string {
  ...
  // ──── Per-install hardware values from ZSTACK_* env (T2) ────
...
function extendedModeEnabled(): boolean {
  const v = process.env.ZSTACK_STEALTH;
 ...
 *   3. EXTENDED_STEALTH_SCRIPT — only when ZSTACK_STEALTH=extended (off by
 ...
 * disagree with the env-driven GPU spoof in buildZStackLaunchArgs on non-Apple
...
/**
 * Build the `--zstack-*=` cmdline switches that the Pack 1 Chromium
 * patches read (webgl-vendor-spoof, ua-client-hints-stealth, worker-
 * navigator-stealth). Values come from the ZSTACK_* env vars that
 ...
 *
 * TRUSTED-SOURCE ONLY. These ZSTACK_* values are populated by gbd from
 ...
 *   ZSTACK_GPU_VENDOR        → --zstack-gpu-vendor        → webgl-vendor-spoof.patch
 *   ZSTACK_GPU_RENDERER      → --zstack-gpu-renderer      → webgl-vendor-spoof.patch
 *   ZSTACK_PLATFORM          → --zstack-ua-platform       → ua-client-hints-stealth.patch
 *                              (maps MacARM/MacIntel → "macOS")
 *   ZSTACK_GPU_CHIPSET       → --zstack-ua-model          → ua-client-hints-stealth.patch
 *   ZSTACK_HW_CONCURRENCY    → --zstack-hw-concurrency    → worker-navigator-stealth.patch
 *   ZSTACK_DEVICE_MEMORY     → --zstack-device-memory     → worker-navigator-stealth.patch
 */
export function buildZStackLaunchArgs(): string[] {
  const env = (globalThis as any).process?.env ?? {};
  const args: string[] = [];

  const vendor = env.ZSTACK_GPU_VENDOR;
  if (vendor) args.push(`--zstack-gpu-vendor=${vendor}`);

  const renderer = env.ZSTACK_GPU_RENDERER;
  if (renderer) args.push(`--zstack-gpu-renderer=${renderer}`);
  ...
  const platform = env.ZSTACK_PLATFORM;
  if (platform === 'MacARM' || platform === 'MacIntel') {
    args.push('--zstack-ua-platform=macOS');
  } else if (platform === 'Win32') {
    args.push('--zstack-ua-platform=Windows');
  } else if (platform && platform.startsWith('Linux')) {
    args.push('--zstack-ua-platform=Linux');
  }

  const chipset = env.ZSTACK_GPU_CHIPSET;
  if (chipset) args.push(`--zstack-ua-model=${chipset}`);

  const hw = env.ZSTACK_HW_CONCURRENCY;
  if (hw) args.push(`--zstack-hw-concurrency=${hw}`);

  const memory = env.ZSTACK_DEVICE_MEMORY;
  if (memory) args.push(`--zstack-device-memory=${memory}`);
  ...
  //
  // OPT-IN (off by default): only emitted when ZSTACK_CDP_STEALTH is
  // on/1/true. This switch is read by a C++ patch that only exists in
  // gbrowser builds; gbd opts in by exporting ZSTACK_CDP_STEALTH=on. Stock
  ...
  // unless ZSTACK_CDP_STEALTH=off, which contradicted this very comment.)
  const cdpStealth = env.ZSTACK_CDP_STEALTH;
  if (cdpStealth === 'on' || cdpStealth === '1' || cdpStealth === 'true') {
    args.push('--zstack-suppress-prepare-stack-trace');
  }

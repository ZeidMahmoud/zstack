/**
 * Cross-platform file permission restriction for sensitive zstack state.
 *
 * Why this exists
 * ----------------
 * POSIX mode bits (`0o600` for files, `0o700` for dirs) are how zstack marks
 ...
 * install, sensitive zstack state files inherit whatever ACL the parent
...
function warnIcaclsFailure(fsPath: string, err: unknown): void {
  ...
  console.warn(
    `[zstack] Failed to restrict Windows ACL on ${fsPath}: ${msg}\n` +
 ...
 *
 * Owner-only hardening is for directories zstack creates and owns. Two
...
export function restrictDirectoryPermissions(dirPath: string): void {
  try {
    if (fs.lstatSync(dirPath).isSymbolicLink()) {
      // biome-ignore lint/suspicious/noConsole: intentional user-facing warning
      console.warn(
        `[zstack] Refusing to restrict permissions through symlink ${dirPath} — skipping.\n` +
  ...
  try {
    ...
    //   ELOOP/ENOTDIR — dirPath is a symlink (dotfiles-managed ~/.zstack via
...
function warnUnhardenedDirOnce(dirPath: string): void {
  ...
  // biome-ignore lint/suspicious/noConsole: intentional user-facing warning
  console.warn(
    `[zstack] directory ${dirPath} is shared, symlinked, or owned by another user; ` +
    'zstack left its permissions untouched — use a private, non-symlinked directory for owner-only hardening',
  );
}
 ...
 * except on directories zstack doesn't own (another user's dir, or a shared

/**
 * zstack browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.zstack/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.zstack/browse-{console,network,dialog}.log
 ...
 * `/Users/zeidmahmoud/.claude/plans/system-instruction-you-are-working-swirling-fountain.md`
 * Part 1.
 */
export interface ServerConfig {
  ...
  startTime: number;
  /**
   * Overlay hook. Runs AFTER zstack resolves auth and BEFORE route dispatch.
   * Invalid tokens are auto-rejected at the zstack layer (401 returned
   * before hook fires), so the hook only ever sees valid TokenInfo or null
   * (no token presented). Returning a Response short-circuits zstack
   * dispatch; returning null falls through.
   */
  beforeRoute?: (req: Request, surface: Surface, auth: TokenInfo | null) => Promise<Response | null>;
  /**
   * Whether zstack owns the lifecycle of the terminal-agent process and its
   ...
   *      matched sibling zstack sessions on the same host — see
   ...
   *
   * This is correct for zstack's CLI path, which spawns `terminal-agent.ts` as
...
/**
 * Build a ServerConfig-shaped object from process.env. Used by zstack's
...
/**
 * The zstack sidebar extension's pinned Chrome extension ID. Derived from
 ...
 */
export const ZSTACK_EXTENSION_ID = 'dgbkdbjebeiblbajiilljmhjdpmiglep';
...
/**
 * Read ngrok authtoken from env var, ~/.zstack/ngrok.env, or ngrok's native
...
function resolveNgrokAuthtoken(): string | null {
  ...
  const ngrokEnvPath = path.join(home, '.zstack', 'ngrok.env');
...
function generateHelpText(): string {
  ...
  const lines = ['zstack browse — headless browser for AI agents', '', 'Commands:'];
...
// so this branch is the normal path for /open-zstack-browser.
...
// fighting with zstack's. CLI path is unchanged.
if (import.meta.main) {
...
export function buildFetchHandler(cfg: ServerConfig): ServerHandle {
  ...
  // to zstack-owns. Matches the "default-true preserves CLI bit-for-bit"
  ...
  const AGENT_WATCHDOG_TICK_MS = parseInt(
    process.env.ZSTACK_AGENT_WATCHDOG_TICK_MS || '60000',
    10,
  );
  ...
  async function shutdown(exitCode: number = 0) {
    ...
    if (ownsTerminalAgent && !foreignState) {
      ...
      // terminal-agent\.ts` regex teardown which matched sibling zstack
  ...
  // block zstack shutdown — defensive symmetry with the safeUnlinkQuiet /
  ...
  const makeFetchHandler = (surface: Surface) => async (req: Request): Promise<Response> => {
    ...
    if (url.pathname === '/connect' && req.method === 'GET') {
      ...
      // Welcome page — served when ZStack Browser launches in headed mode
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          // Gate ZSTACK_SLUG on a strict regex BEFORE interpolating it into
          // the filesystem path. Without this, a slug like "../../etc/passwd"
          // would resolve to ~/.zstack/projects/../../etc/passwd/... — path
          ...
          const rawSlug = process.env.ZSTACK_SLUG || 'unknown';
          const slug = /^[a-z0-9_-]+$/.test(rawSlug) ? rawSlug : 'unknown';
          const homeDir = process.env.HOME || process.env.USERPROFILE || '/tmp';
          const projectWelcome = `${homeDir}/.zstack/projects/${slug}/designs/welcome-page-20260331/finalized.html`;
          if (fs.existsSync(projectWelcome)) return projectWelcome;
          // Fallback: built-in welcome page from zstack install.  Reject
          // SKILL_ROOT values containing '..' for the same defense-in-depth
          // reason as the ZSTACK_SLUG regex above.  Not exploitable today
          // (env set at install time), but the gate is one check.
          const rawSkillRoot = process.env.ZSTACK_SKILL_ROOT || `${homeDir}/.claude/skills/zstack`;
        ...
        return new Response(
          `<!DOCTYPE html><html><head><title>ZStack Browser</title>
          ...
          <body><div class="msg"><div class="gold">◈</div><p>ZStack Browser ready.</p><p style="font-size:.85em">Waiting for commands from Claude Code.</p></div></body></html>`,
      ...
      // exactly `chrome-extension://<ZSTACK_EXTENSION_ID>`, where the ID
      ...
      if (url.pathname === '/extension-token' && req.method === 'POST') {
        ...
        const originOk =
          req.headers.get('origin') === `chrome-extension://${ZSTACK_EXTENSION_ID}`;
      ...
      // extension's zstackInjectToTerminal callers. The extension routes
      ...
      if (url.pathname === '/tunnel/start' && req.method === 'POST') {
        ...
        if (!isPairAgentEnabled()) {
          ...
          return new Response(JSON.stringify({
            ...
            hint: 'enable once with: zstack-config set pair_agent on — or run /pair-agent, which asks for consent and sets it',
          }), { status: 403, headers: { 'Content-Type': 'application/json' } });
        }
        ...
        // 1) Resolve ngrok authtoken from env / .zstack / native config
...
export async function start() {
  ...
  const stateWatchMs = parseInt(process.env.ZSTACK_STATE_WATCH_MS || '60000', 10);
  if (stateWatchMs > 0) {
    let missed = 0;
  ...
  if (process.env.BROWSE_TUNNEL === '1' && !isPairAgentEnabled()) {
    console.error('[browse] BROWSE_TUNNEL=1 ignored: pair-agent is off. Enable once with: zstack-config set pair_agent on');
  } else if (process.env.BROWSE_TUNNEL === '1') {
    const authtoken = resolveNgrokAuthtoken();
    if (!authtoken) {
      console.error('[browse] BROWSE_TUNNEL=1 but no NGROK_AUTHTOKEN found. Set it via env var or ~/.zstack/ngrok.env');
    } else {

 ...
 *   project:  <project>/.zstack/browser-skills/<name>/
 *   global:   ~/.zstack/browser-skills/<name>/
 *   bundled:  <zstack-install>/browser-skills/<name>/   (read-only, ships with zstack)
 *
...
export function defaultTierPaths(opts: { projectRoot?: string; home?: string; bundledRoot?: string } = {}): TierPaths {
  ...
  return {
    project: projectRoot ? path.join(projectRoot, '.zstack', 'browser-skills') : null,
    global: path.join(home, '.zstack', 'browser-skills'),
...
function detectBundledRoot(): string {
  // The browse binary lives at <zstack-install>/browse/dist/browse.
  // The bundled browser-skills/ dir is a sibling of browse/ (i.e. <zstack-install>/browser-skills/).
  ...
  // browse/src/browser-skills.ts → ../../  (the zstack root).
 ...
 *
 * Cannot tombstone bundled skills — they ship with zstack and are read-only.
 * To remove a bundled skill, override it with a global/project entry, or
 * remove the file from the zstack source tree.
 */

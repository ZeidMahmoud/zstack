...
// hardcoded ~/.zstack/chromium-profile here silently ignored $CHROMIUM_PROFILE
// and $ZSTACK_HOME, so `browse stop` unlinked locks and SIGKILLed a PID
...
describe('chromiumProfileDir wiring (#2732)', () => {
  ...
  test('no hardcoded chromium-profile path remains in cli.ts', () => {
    expect(cliSrc).not.toMatch(/['"]\.zstack['"],\s*['"]chromium-profile['"]/);
  });
});

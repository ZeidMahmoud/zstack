...
describe('applyStealth — per-install hardware from env', () => {
  ...
  beforeAll(async () => {
    ...
    process.env.ZSTACK_HW_CONCURRENCY = '12';
    process.env.ZSTACK_DEVICE_MEMORY = '4';
...
describe('applyStealth — extended mode layered on Layer C (ZSTACK_STEALTH=extended)', () => {
  let ctx: BrowserContext;
  let savedStealth: string | undefined;

  beforeAll(async () => {
    savedStealth = process.env.ZSTACK_STEALTH;
    process.env.ZSTACK_STEALTH = 'extended';
    ctx = await browser.newContext();
  ...
  afterAll(async () => {
    await ctx.close();
    if (savedStealth === undefined) delete process.env.ZSTACK_STEALTH;
    else process.env.ZSTACK_STEALTH = savedStealth;
  });

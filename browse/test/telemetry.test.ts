...
const TMP_HOME = path.join(os.tmpdir(), `zstack-telemetry-test-${process.pid}-${Date.now()}`);
...
describe('telemetry: signals fire to ~/.zstack/analytics/browse-telemetry.jsonl', () => {
  ...
  it('ZSTACK_TELEMETRY_OFF=1 silences all events', async () => {

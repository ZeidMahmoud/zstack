 ...
 * never touch the user's real ~/.zstack/ tree. The contract under test:
 *
 *   stageSkill    → writes files into ~/.zstack/.tmp/skillify-<spawnId>/<name>/
...
beforeEach(() => {
  ...
  // Synthetic tmp root keeps tests off the real ~/.zstack/.tmp/.
  stagingTmpRoot = path.join(tmpRoot, 'home', '.zstack', '.tmp');
});

...
describe('welcome page served via HTTP', () => {
  ...
  test('has ZStack Browser title and branding', () => {
    expect(html).toContain('<title>ZStack Browser</title>');
    expect(html).toContain('ZStack Browser');
  });
  ...
  test('hides sidebar prompt when extension is detected', () => {
    ...
    expect(html).toContain("'zstack-extension-ready'");
  ...
  test('does NOT auto-hide based on extension detection alone', () => {
    ...
    expect(html).not.toContain('zstack-status-pill');
  ...
  test('has footer with attribution', () => {
    expect(html).toContain('Zeid Mahmoud');
    expect(html).toContain('github.com/zeidmahmoud/zstack');
  });
});

...
describe('config', () => {
  ...
  describe('resolveConfig', () => {
    test('uses git root by default', () => {
      ...
      expect(config.stateDir).toBe(path.join(gitRoot!, '.zstack'));
      expect(config.stateFile).toBe(path.join(gitRoot!, '.zstack', 'browse.json'));
    });

    test('derives paths from BROWSE_STATE_FILE when set', () => {
      const stateFile = '/tmp/test-config/.zstack/browse.json';
      const config = resolveConfig({ BROWSE_STATE_FILE: stateFile });
      expect(config.stateFile).toBe(stateFile);
      expect(config.stateDir).toBe('/tmp/test-config/.zstack');
  ...
  describe('ensureStateDir', () => {
    ...
    test('is a no-op if directory already exists', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-config-test-${Date.now()}`);
      const stateDir = path.join(tmpDir, '.zstack');
    ...
    test('does not duplicate .zstack/ in .gitignore', () => {
      const tmpDir = path.join(os.tmpdir(), `browse-gitignore-test-${Date.now()}`);
      fs.mkdirSync(tmpDir, { recursive: true });
      fs.writeFileSync(path.join(tmpDir, '.gitignore'), 'node_modules/\n.zstack/\n');
      const config = resolveConfig({ BROWSE_STATE_FILE: path.join(tmpDir, '.zstack', 'browse.json') });
  ...
  describe('getRemoteSlug', () => {
    ...
    test('parses SSH remote URLs', () => {
      ...
      const url = 'git@github.com:garrytan/zstack.git';
    ...
    test('parses HTTPS remote URLs', () => {
      const url = 'https://github.com/garrytan/zstack.git';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('garrytan-zstack');
    });
    ...
    test('parses HTTPS remote URLs without .git suffix', () => {
      const url = 'https://github.com/garrytan/zstack';
      const match = url.match(/[:/]([^/]+)\/([^/]+?)(?:\.git)?$/);
      expect(match).not.toBeNull();
      expect(`${match![1]}-${match![2]}`).toBe('garrytan-zstack');
    });
...
describe('resolveZstackHome', () => {
  test('honors ZSTACK_HOME env var when set', () => {
    const orig = process.env.ZSTACK_HOME;
    process.env.ZSTACK_HOME = '/tmp/custom-zstack-home';
    try {
      expect(resolveZstackHome()).toBe('/tmp/custom-zstack-home');
    } finally {
      if (orig === undefined) delete process.env.ZSTACK_HOME;
      else process.env.ZSTACK_HOME = orig;
    }
  });

  test('falls back to os.homedir() + /.zstack when env unset', () => {
    const orig = process.env.ZSTACK_HOME;
    delete process.env.ZSTACK_HOME;
    try {
      expect(resolveZstackHome()).toBe(path.join(os.homedir(), '.zstack'));
    } finally {
      if (orig !== undefined) process.env.ZSTACK_HOME = orig;
    }
  });
});
...
describe('resolveChromiumProfile', () => {
  ...
  test('falls back to resolveZstackHome()/chromium-profile when nothing set', () => {
    const origEnv = process.env.CHROMIUM_PROFILE;
    const origHome = process.env.ZSTACK_HOME;
    delete process.env.CHROMIUM_PROFILE;
    process.env.ZSTACK_HOME = '/tmp/fallback-zstack';
    try {
      expect(resolveChromiumProfile()).toBe('/tmp/fallback-zstack/chromium-profile');
    } finally {
      if (origEnv !== undefined) process.env.CHROMIUM_PROFILE = origEnv;
      if (origHome === undefined) delete process.env.ZSTACK_HOME;
      else process.env.ZSTACK_HOME = origHome;
    }
  });

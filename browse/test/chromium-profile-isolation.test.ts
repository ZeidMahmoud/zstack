...
describe.skipIf(process.platform === 'win32')('Chromium profile isolation (#2817)', () => {
  ...
  beforeEach(() => {
    ...
    profile = path.join(scratch, '.zstack', 'chromium-profile');
    stateFile = path.join(scratch, 'project-b', '.zstack', 'browse.json');
    fs.mkdirSync(profile, { recursive: true });
    fs.mkdirSync(path.dirname(stateFile), { recursive: true });
    env = {};
    for (const [key, value] of Object.entries(process.env)) {
      if (value !== undefined && !/^(BROWSE_|ZSTACK_|CHROMIUM_PROFILE$|CLAUDE_PLUGIN_DATA$)/.test(key)) {
        env[key] = value;
      }
    }
    Object.assign(env, {
      ...
      ZSTACK_HOME: path.join(scratch, '.zstack'),
      ZSTACK_SECURITY_OFF: '1',

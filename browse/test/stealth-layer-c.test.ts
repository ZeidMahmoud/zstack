...
describe('buildStealthScript — T3 Layer C', () => {
  ...
  test('script does not include "ZStackBrowser" branding string', () => {
    const s = buildStealthScript(hw);
    // D6: dropped from UA, must not leak in via stealth payload either.
    expect(s).not.toContain('ZStackBrowser');
  });
});

describe('buildZStackLaunchArgs — Pack 1 cmdline-switch construction', () => {
  // Helper: clear all ZSTACK_* env, run test body, restore env.
  function withEnv(env: Record<string, string | undefined>, body: () => void) {
    const saved: Record<string, string | undefined> = {};
    for (const k of Object.keys(process.env)) {
      if (k.startsWith('ZSTACK_')) {
    ...
    try {
      ...
      for (const k of Object.keys(process.env)) {
        if (k.startsWith('ZSTACK_')) delete process.env[k];
      }
  ...
  test('empty env produces no switches (suppress-stack is opt-in)', () => {
    withEnv({}, () => {
      ...
      // emitted when ZSTACK_CDP_STEALTH is on/1/true. Empty env → [].
      expect(buildZStackLaunchArgs()).toEqual([]);
    });
  });

  test('all env values populated (incl. CDP stealth opt-in) → all 7 switches emitted', () => {
    withEnv({
      ZSTACK_GPU_VENDOR: 'Apple Inc.',
      ZSTACK_GPU_RENDERER: 'ANGLE (Apple, ANGLE Metal Renderer: Apple M4 Max, Unspecified Version)',
      ZSTACK_PLATFORM: 'MacARM',
      ZSTACK_GPU_CHIPSET: 'Apple M4 Max',
      ZSTACK_HW_CONCURRENCY: '16',
      ZSTACK_DEVICE_MEMORY: '8',
      ZSTACK_CDP_STEALTH: 'on',
    }, () => {
      const args = buildZStackLaunchArgs();
      expect(args).toContain('--zstack-gpu-vendor=Apple Inc.');
      expect(args).toContain('--zstack-gpu-renderer=ANGLE (Apple, ANGLE Metal Renderer: Apple M4 Max, Unspecified Version)');
      expect(args).toContain('--zstack-ua-platform=macOS');
      expect(args).toContain('--zstack-ua-model=Apple M4 Max');
      expect(args).toContain('--zstack-hw-concurrency=16');
      expect(args).toContain('--zstack-device-memory=8');
      expect(args).toContain('--zstack-suppress-prepare-stack-trace');
      expect(args.length).toBe(7);
    });
  });

  test('platform mapping: MacARM and MacIntel both → macOS', () => {
    withEnv({ ZSTACK_PLATFORM: 'MacARM' }, () => {
      expect(buildZStackLaunchArgs()).toContain('--zstack-ua-platform=macOS');
    });
    withEnv({ ZSTACK_PLATFORM: 'MacIntel' }, () => {
      expect(buildZStackLaunchArgs()).toContain('--zstack-ua-platform=macOS');
    });
  });

  test('platform mapping: Win32 → Windows, Linux x86_64 → Linux', () => {
    withEnv({ ZSTACK_PLATFORM: 'Win32' }, () => {
      expect(buildZStackLaunchArgs()).toContain('--zstack-ua-platform=Windows');
    });
    withEnv({ ZSTACK_PLATFORM: 'Linux x86_64' }, () => {
      expect(buildZStackLaunchArgs()).toContain('--zstack-ua-platform=Linux');
    });
  });

  test('partial env: only set switches that have values', () => {
    withEnv({ ZSTACK_HW_CONCURRENCY: '12' }, () => {
      const args = buildZStackLaunchArgs();
      // hw only — suppress-stack is opt-in and ZSTACK_CDP_STEALTH is unset.
      expect(args).toContain('--zstack-hw-concurrency=12');
      expect(args).not.toContain('--zstack-suppress-prepare-stack-trace');
      expect(args.length).toBe(1);
    });
  });

  test('prepare-stack-trace suppression is opt-in via ZSTACK_CDP_STEALTH', () => {
    // on/1/true enable it; off and unset omit it, so stock Playwright
    // Chromium (no ZSTACK_CDP_STEALTH) never receives the unknown switch.
    for (const v of ['on', '1', 'true']) {
      withEnv({ ZSTACK_CDP_STEALTH: v }, () => {
        expect(buildZStackLaunchArgs()).toContain('--zstack-suppress-prepare-stack-trace');
      });
    }
    withEnv({ ZSTACK_CDP_STEALTH: 'off' }, () => {
      expect(buildZStackLaunchArgs()).not.toContain('--zstack-suppress-prepare-stack-trace');
    });
    withEnv({}, () => {
      expect(buildZStackLaunchArgs()).not.toContain('--zstack-suppress-prepare-stack-trace');
    });
  ...
  test('unrecognized platform falls through without --zstack-ua-platform', () => {
    withEnv({ ZSTACK_PLATFORM: 'OS/2' }, () => {
      const args = buildZStackLaunchArgs();
      expect(args.some(a => a.startsWith('--zstack-ua-platform='))).toBe(false);
    });
  });

  test('GPU vendor with spaces survives intact (no quote/escape regression)', () => {
    withEnv({ ZSTACK_GPU_VENDOR: 'NVIDIA Corporation' }, () => {
      const args = buildZStackLaunchArgs();
      expect(args).toContain('--zstack-gpu-vendor=NVIDIA Corporation');
    });
  });
});
...
describe('readHostProfile — clamp/fallback', () => {
  ...
  afterEach(() => {
    if (savedHw === undefined) delete process.env.ZSTACK_HW_CONCURRENCY;
    else process.env.ZSTACK_HW_CONCURRENCY = savedHw;
    if (savedMem === undefined) delete process.env.ZSTACK_DEVICE_MEMORY;
    else process.env.ZSTACK_DEVICE_MEMORY = savedMem;
  });
  function withHw(hw: string | undefined, mem: string | undefined): ReturnType<typeof readHostProfile> {
    savedHw = process.env.ZSTACK_HW_CONCURRENCY;
    savedMem = process.env.ZSTACK_DEVICE_MEMORY;
    if (hw === undefined) delete process.env.ZSTACK_HW_CONCURRENCY; else process.env.ZSTACK_HW_CONCURRENCY = hw;
    if (mem === undefined) delete process.env.ZSTACK_DEVICE_MEMORY; else process.env.ZSTACK_DEVICE_MEMORY = mem;

 ...
 *    behavior (and the ZSTACK_DISABLE_GPU=off escape) is testable on any host.
 *
...
describe('headlessGpuArgs (#2709)', () => {
  ...
  test('ZSTACK_DISABLE_GPU=off opts out (case-insensitive)', () => {
    expect(headlessGpuArgs('darwin', { ZSTACK_DISABLE_GPU: 'off' })).toEqual([]);
    expect(headlessGpuArgs('darwin', { ZSTACK_DISABLE_GPU: 'OFF' })).toEqual([]);
  });

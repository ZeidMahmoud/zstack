...
function loadBackground() {
  ...
  const run = new Function('chrome', 'importScripts', 'zstackSenderAuth', 'fetch', BG_SRC);
...
describe('background.js ↔ sender-auth.js wiring', () => {
  ...
  test('background.js consults zstackSenderAuth.denialFor in the message listener', () => {
    expect(BG_SRC).toContain('zstackSenderAuth.denialFor(msg.type, sender, chrome.runtime.id)');
  });

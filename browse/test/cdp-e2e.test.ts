...
const TMP_HOME = path.join(os.tmpdir(), `zstack-cdp-e2e-${process.pid}-${Date.now()}`);
...
// exact leak once pointed a sibling test's ZSTACK_HOME at our temp dir,
...
beforeAll(async () => {
  ...
  process.env.ZSTACK_TELEMETRY_OFF = '1'; // don't pollute analytics during tests

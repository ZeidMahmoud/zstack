...
describe('logAttempt', () => {
  test('writes attempts.jsonl with correct shape', () => {
    ...
    const logPath = path.join(os.homedir(), '.zstack', 'security', 'attempts.jsonl');
...
describe('resolveBashBinary', () => {
  ...
  test('honors ZSTACK_BASH_BIN absolute-path override', () => {
    ...
    const out = resolveBashBinary({ ZSTACK_BASH_BIN: fake, PATH: '' });
  ...
  test('BASH_BIN works as a fallback when ZSTACK_BASH_BIN is unset', () => {
    const fake = process.platform === 'win32' ? 'C:\\opt\\bash.exe' : '/opt/custom/bash';
...
describe('buildTelemetrySpawnCommand', () => {
  const bin = '/home/user/.claude/skills/zstack/bin/zstack-telemetry-log';
  ...
  test('on win32 with bash resolvable, wraps the call in bash with the script as first arg', () => {
    ...
    const out = buildTelemetrySpawnCommand(bin, args, { ZSTACK_BASH_BIN: fakeBash, PATH: '' });
    expect(out).not.toBeNull();

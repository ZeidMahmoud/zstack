 ...
 * were registered. The subprocess uses HOME=tmp + ZSTACK_HOME=tmp so any
...
describe('server.ts module import has no auto-start side effects', () => {
  test('importing server.ts does not bind Bun.serve, register signal handlers, or write state', async () => {
    ...
    const tmpZstack = path.join(tmpHome, '.zstack');
...
// Check that the zstack home directory wasn't populated as a side effect.
...
let zstackPopulated = false;
try {
  const entries = fs.readdirSync(${JSON.stringify(tmpZstack)}).filter(e => e !== '.gitignore');
  zstackPopulated = entries.length > 0;
} catch {
...
`;
    ...
    const proc = Bun.spawn(['bun', '-e', childScript], {
      env: {
        ...
        // happens) won't crawl the host's real .zstack/.
        BROWSE_STATE_FILE: path.join(tmpZstack, 'browse.json'),
      },
      stdout: 'pipe',
      stderr: 'pipe',
    });
    ...
    // zstack home should remain empty — initRegistry/initAuditLog/etc. side
    ...
    expect(snapshot.zstackPopulated).toBe(false);

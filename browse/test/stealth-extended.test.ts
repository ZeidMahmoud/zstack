 ...
 * 2. ZSTACK_STEALTH=extended adds EXTENDED_STEALTH_SCRIPT on top of Layer C.
...
beforeEach(() => {
  originalEnv = process.env.ZSTACK_STEALTH;
});

afterEach(() => {
  if (originalEnv === undefined) delete process.env.ZSTACK_STEALTH;
  else process.env.ZSTACK_STEALTH = originalEnv;
});

describe('extended stealth — opt-in mode flag', () => {
  test('default mode is OFF (consistency-first contract)', () => {
    delete process.env.ZSTACK_STEALTH;
    expect(isExtendedStealthEnabled()).toBe(false);
  });

  test('ZSTACK_STEALTH=extended enables extended mode', () => {
    process.env.ZSTACK_STEALTH = 'extended';
    expect(isExtendedStealthEnabled()).toBe(true);
  });

  test('ZSTACK_STEALTH=1 also enables (env-style boolean)', () => {
    process.env.ZSTACK_STEALTH = '1';
    expect(isExtendedStealthEnabled()).toBe(true);
  });

  test('ZSTACK_STEALTH=anything-else does NOT enable', () => {
    process.env.ZSTACK_STEALTH = 'verbose';
    expect(isExtendedStealthEnabled()).toBe(false);
  });
});

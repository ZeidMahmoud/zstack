/**
 * Tests for bin/zstack-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * ZSTACK_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'zstack-config');

let stateDir: string;

function run(args: string[] = [], extraEnv: Record<string, string> = {}) {
  // The script resolves its state dir as ZSTACK_STATE_ROOT > ZSTACK_HOME >
  // ZSTACK_STATE_DIR > $HOME/.zstack. Strip the higher-precedence vars so a
  // stray value in the harness env (another test file's leftovers, operator
  // shell) can never outrank the per-test ZSTACK_STATE_DIR isolation.
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'zstack-config-test-'));
});
...
describe('zstack-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { ZSTACK_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
  ...
  test('first set writes annotated header with docs', () => {
    ...
    const content = readFileSync(join(stateDir, 'config.yaml'), 'utf-8');
    expect(content).toContain('# zstack configuration');
  ...
  test('header written only once, not duplicated on second set', () => {
    ...
    run(['set', 'baz', 'qux']);
    const content = readFileSync(join(stateDir, 'config.yaml'), 'utf-8');
    const headerCount = (content.match(/# zstack configuration/g) || []).length;
  ...
  test('existing config file is not overwritten with header', () => {
    ...
    expect(content).not.toContain('# zstack configuration');
  });

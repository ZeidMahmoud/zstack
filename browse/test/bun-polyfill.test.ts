...
describe('bun-polyfill', () => {
  ...
  // ZSTACK_SPAWN_MAX_BUFFER caps the drain so a runaway child can't OOM the
  ...
  test('Bun.spawn caps buffered output at ZSTACK_SPAWN_MAX_BUFFER', async () => {
    const result = Bun.spawnSync(['node', '-e', `
      process.env.ZSTACK_SPAWN_MAX_BUFFER = '${1024}';
      require(${JSON.stringify(polyfillPath)});
      (async () => {

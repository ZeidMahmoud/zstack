...
// module scope (see test/zstack-home-module-scope.test.ts's rationale).
const ORIGINAL_CHROMIUM_PROFILE = process.env.CHROMIUM_PROFILE;
let CHROMIUM_PROFILE_DIR: string | undefined;
beforeAll(() => {
  CHROMIUM_PROFILE_DIR = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-test-profile-'));
...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync(tmpp('zstack-cli-'));
    const execPath = path.join(root, '.claude/skills/zstack/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/zstack/browse/src/server.ts');

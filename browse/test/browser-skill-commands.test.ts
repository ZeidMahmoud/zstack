...
beforeEach(() => {
  ...
  tiers = {
    project: path.join(tmpRoot, 'project', '.zstack', 'browser-skills'),
    global: path.join(tmpRoot, 'home', '.zstack', 'browser-skills'),
    bundled: path.join(tmpRoot, 'zstack-install', 'browser-skills'),
  };
...
describe('buildSpawnEnv', () => {
  ...
  beforeEach(() => {
    ...
    process.env.ZSTACK_TOKEN = 'root-token';
  ...
  it('untrusted: drops $HOME and secrets', () => {
    ...
    expect(env.ZSTACK_TOKEN).toBeUndefined();
  });
  ...
  it('untrusted: injects ZSTACK_PORT + ZSTACK_SKILL_TOKEN', () => {
    const env = buildSpawnEnv({ trusted: false, port: 1234, skillToken: 'tok-xyz' });
    expect(env.ZSTACK_PORT).toBe('1234');
    expect(env.ZSTACK_SKILL_TOKEN).toBe('tok-xyz');
  });
  ...
  it('trusted: still strips ZSTACK_TOKEN (defense in depth)', () => {
    const env = buildSpawnEnv({ trusted: true, port: 1234, skillToken: 'tok' });
    expect(env.ZSTACK_TOKEN).toBeUndefined();
  });
  ...
  it('ZSTACK_PORT/ZSTACK_SKILL_TOKEN can never be overridden by parent env', () => {
    process.env.ZSTACK_PORT = '99999'; // attacker-set
    process.env.ZSTACK_SKILL_TOKEN = 'attacker-tok';
    const env = buildSpawnEnv({ trusted: true, port: 1234, skillToken: 'real-tok' });
    expect(env.ZSTACK_PORT).toBe('1234');
    expect(env.ZSTACK_SKILL_TOKEN).toBe('real-tok');
  });
});
...
describe.skipIf(SKIP_SPAWN)('spawnSkill: lifecycle', () => {
  ...
  it('untrusted spawn: ZSTACK_SKILL_TOKEN visible, root env scrubbed', async () => {
    const dir = makeSkillDir(tiers.bundled, 'env-probe',
      'name: env-probe\nhost: x.com',  // trusted defaults to false
      `console.log(JSON.stringify({
        port: process.env.ZSTACK_PORT,
        token: process.env.ZSTACK_SKILL_TOKEN,
        home: process.env.HOME ?? null,
        gh: process.env.GITHUB_TOKEN ?? null,
        zstack: process.env.ZSTACK_TOKEN ?? null,
      }));`,
    );
    const origEnv = { ...process.env };
    process.env.GITHUB_TOKEN = 'gh-secret';
    process.env.ZSTACK_TOKEN = 'root';
    try {

...
describe('browse-client', () => {
  ...
  beforeEach(async () => {
    ...
    for (const k of ['ZSTACK_PORT', 'ZSTACK_SKILL_TOKEN', 'BROWSE_STATE_FILE', 'BROWSE_TAB']) {
  ...
  describe('resolveBrowseAuth', () => {
    it('uses ZSTACK_PORT + ZSTACK_SKILL_TOKEN env when present', () => {
      process.env.ZSTACK_PORT = String(server.port);
      process.env.ZSTACK_SKILL_TOKEN = 'scoped-token';
    ...
    it('rejects ZSTACK_PORT env values with trailing characters', () => {
      process.env.ZSTACK_PORT = `${server.port}abc`;
      process.env.ZSTACK_SKILL_TOKEN = 'scoped-token';

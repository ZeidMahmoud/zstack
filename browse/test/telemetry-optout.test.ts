 ...
 * ~/.zstack/config.yaml). An absent key, an absent config file, an explicit
 * `off`, or any unrecognized value all mean DISABLED — the same default
 * bin/zstack-config's DEFAULTS table reports for an unset key, so a daemon
 * spawned outside a skill preamble (direct $B use, embedders) can never
 * emit while `zstack-config get telemetry` tells the user 'off'.
 *
 * The persistent tier reads through the shared flat-YAML helper in
 * config.ts (readZstackConfigYamlKey), same parser as the pair-agent gate.
 * Env tier: ZSTACK_TELEMETRY_OFF=1 always disables; =0 is a harness-side
 * consent assertion that covers the no-config default only — it never
 * overrides an explicit `telemetry: off`.
 *
 * Harness mirrors pair-agent-optin-gate.test.ts: ZSTACK_HOME → temp dir,
...
const savedEnv = {
  ...
  ZSTACK_TELEMETRY_OFF: process.env.ZSTACK_TELEMETRY_OFF,
};
const tmpHomes: string[] = [];

/** Fresh ZSTACK_HOME with the given config.yaml body (null = no file). */
function tmpHomeWith(configYaml: string | null): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-telemetry-optout-'));
  tmpHomes.push(dir);
  if (configYaml !== null) {
    fs.writeFileSync(path.join(dir, 'config.yaml'), configYaml);
  }
  process.env.ZSTACK_HOME = dir;
  delete process.env.ZSTACK_TELEMETRY_OFF;
  _resetTelemetryCache();
  return dir;
}

afterEach(() => {
  for (const k of ['ZSTACK_HOME', 'ZSTACK_TELEMETRY_OFF'] as const) {
...
describe('telemetry persistent opt-out tier (config.yaml)', () => {
  ...
  test('DISABLED when the key is absent — consent was never granted', () => {
    // bin/zstack-config's DEFAULTS table reports 'off' for an unset telemetry
...
describe('telemetry env tier + cache semantics', () => {
  test('ZSTACK_TELEMETRY_OFF=1 disables even when config says anonymous', () => {
    tmpHomeWith('telemetry: anonymous\n');
    process.env.ZSTACK_TELEMETRY_OFF = '1';
    _resetTelemetryCache();
    expect(isTelemetryDisabled()).toBe(true);
  });

  test('ZSTACK_TELEMETRY_OFF=0 never overrides an explicit `telemetry: off`', () => {
  ...
  test('ZSTACK_TELEMETRY_OFF=0 enables when no config store exists (harness seam)', () => {
    tmpHomeWith(null);
    process.env.ZSTACK_TELEMETRY_OFF = '0';
    _resetTelemetryCache();
    expect(isTelemetryDisabled()).toBe(false);

...
describe('findPlaywrightRevisionDir', () => {
  ...
  it('returns null outside the Playwright cache layout', () => {
    expect(findPlaywrightRevisionDir('/Applications/ZStack Browser.app/Contents/MacOS/Chromium')).toBe(null);
  });
});
...
describe('findZstackInstallRoot (ENG-OV3: revision-matched roots only)', () => {
  let tmpRoot: string;
  ...
  it('resolves the dev checkout by default (its node_modules pins our revision)', () => {
    ...
    const registryRevision = browsersJson.browsers.find((b: { name: string }) => b.name === 'chromium').revision;
    const root = findZstackInstallRoot(registryRevision);
...
describe('clearQuarantineOnPlaywrightCache', () => {
  let tmpCache: string;
  let execPath: string;
  const savedCustomPath = process.env.ZSTACK_CHROMIUM_PATH;
  ...
  it('NEVER touches a ZSTACK_CHROMIUM_PATH bundle (embedder scope contract)', () => {
    process.env.ZSTACK_CHROMIUM_PATH = execPath;
    const cleared: string[] = [];
    const ok = clearQuarantineOnPlaywrightCache(execPath, (target) => {
      cleared.push(target);
      return 0;
...
function makeDeps(counters: { installs: number; quarantines: number }, overrides: Record<string, unknown> = {}) {
  return {
    ...
    installRoot: () => '/tmp/fake-zstack-root',
...
describe('maybeHealXProtectKill', () => {
  ...
  it('never heals over a custom executable (ZSTACK_CHROMIUM_PATH scope)', async () => {
    const counters = { installs: 0, quarantines: 0 };

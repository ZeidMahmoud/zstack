...
3. Otherwise use `$ZSTACK_STATE_ROOT/projects/$SLUG/$BRANCH-eng-review-{YYYYMMDD-HHMMSS}.md`, adding a suffix on collision. Obtain assignments from `~/.claude/skills/zstack/bin/zstack-paths` and `~/.claude/skills/zstack/bin/zstack-slug`; failed commands or missing values make this path unavailable.
...
`~/.zstack/projects/{slug}/`: `{user}-{branch}-eng-review-test-plan-{datetime}.md`
...
before `## ZSTACK REVIEW REPORT`. Include every native field, the recommendation
...
6. **Completeness check:** Full tests, edges and errors cost 10-100x less with AI.
   Prefer completeness when a shortcut saves only CC+zstack minutes. Boil the ocean.
...
```bash
eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null)" && mkdir -p ~/.zstack/projects/$SLUG  # sets SLUG and BRANCH
TEST_PLAN_USER=$(whoami)
DATETIME=$(date +%Y%m%d-%H%M%S)
```

Use `SLUG` and the sanitized `BRANCH` from zstack-slug, `TEST_PLAN_USER` for {user}, and `DATETIME` for {datetime}. Set {date} to today. Read the local origin URL with `git remote get-url origin` and use its owner/repo; without an origin, write `local-only`. No network request is needed.

Write to `~/.zstack/projects/{slug}/{user}-{branch}-eng-review-test-plan-{datetime}.md`:
...
(`zstack-config set codex_reviews disabled`).
...
- **`disabled`** — the user turned Codex reviews off (`codex_reviews=disabled`). Skip the reviewer invocation; record disabled coverage as directed below; do NOT fall back to a Claude subagent — disabled means no extra review step. Print: "Codex review skipped (codex_reviews disabled). Re-enable: `zstack-config set codex_reviews enabled`."
...
_DISABLED_REVIEW_MODE=$("$HOME/.claude/skills/zstack/bin/zstack-config" get codex_reviews 2>/dev/null) || {
  echo 'Cannot read codex_reviews; disabled outside coverage was not recorded.' >&2
  exit 1
}
if [ "$_DISABLED_REVIEW_MODE" = disabled ]; then
  "$HOME/.claude/skills/zstack/bin/zstack-review-log" '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"skipped","source":"none","host":"claude","outside_provider":"codex","outside_status":"disabled","phase":"plan-review","commit":"'"$(git rev-parse --short HEAD 2>/dev/null || true)"'"}'
fi
```

When the mode is anything except `disabled`, print one line so the off-switch
stays discoverable: "Running the outside voice automatically (standard step). Disable: `zstack-config set codex_reviews disabled`."
...
Create a private prompt file: run `umask 077; mktemp "${TMPDIR:-/tmp}/zstack-plan-prompt.XXXXXXXX"` in Bash and keep the returned path. Use Write to put the **complete prompt and context**, including actual plan/spec/source, in that file. Substitute its shell-quoted path for `<prepared-prompt-file>`; never interpolate user text into shell source. Request a final Recommendation: <action> because <specific reason> line, including an explicit no-findings rationale.
...
**Persist the result:**
```bash
~/.claude/skills/zstack/bin/zstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","host":"claude","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"plan-review","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
### TODOS.md updates
Review every potential TODO. Reuse an exact prior disposition under Decision procedure; ask about each unanswered proposal in its own AskUserQuestion. Never batch TODOs or silently skip them. Use `~/.claude/skills/zstack/review/TODOS-format.md`.
...
2. **Save and Read back.** Use Plan File Review Report to save the complete body
   and append its terminal `## ZSTACK REVIEW REPORT`. Pass that writer's Read-back
...
`## ZSTACK REVIEW REPORT`; nothing follows that terminal report.
...
- Show human-team and CC+zstack effort estimates. Default task-type ratios (human ÷ CC time): scaffolding ~100x, tests ~50x, features ~30x, bug fix with regression ~20x, architecture ~5x, research ~3x. Adjust to the actual work and state the assumption.
...
Run `~/.claude/skills/zstack/bin/zstack-review-read` for prior review entries.
...
line).** After VERDICT, end the report (content under the `## ZSTACK REVIEW REPORT`
...
1. Read the existing report file, if present. Preserve its content and apply only
   ...
   `## ZSTACK REVIEW REPORT` section.
2. If found, use the Edit tool to DELETE the entire existing section. Match from
   `## ZSTACK REVIEW REPORT` through either the next `## ` heading or end of
...
4. **Read-back gate:** Read the saved file. Verify the accepted changes, full review
   ...
   `## ZSTACK REVIEW REPORT` as the last section. If writing or verification fails,
...
```bash
~/.claude/skills/zstack/bin/zstack-review-log '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"MODE","commit":"COMMIT"}' || exit $?
~/.claude/skills/zstack/bin/zstack-decision-log '{"decision":"Eng review (MODE): ARCH_SUMMARY","rationale":"KEY_DECISION","scope":"branch","source":"skill","confidence":8}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/zstack/bin/zstack-learnings-log '{"skill":"plan-eng-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
output or zstack config shows `brain_trust_policy@<endpoint-hash>=personal`.
If unknown, skip. If both gates pass, record one durable
typed prediction with `mcp__gbrain__takes_add`; if unavailable, use
`mcp__gbrain__put_page` with a zstack:takes fence block.

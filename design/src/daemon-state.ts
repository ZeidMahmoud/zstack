...
export const CMDLINE_MARKER = "zstack-design-daemon";
...
export function resolveStateFilePath(): string {
  ...
  try {
    ...
    if (root) return path.join(root, ".zstack", "design.json");
  } catch {
    // not in a git repo — fall through
  }
  return path.join(process.cwd(), ".zstack", "design.json");
}
...
export function resolveDaemonLogPath(): string {
  return path.join(os.homedir(), ".zstack", "design-daemon.log");
}

export function resolveStartupLogPath(): string {
  return path.join(os.homedir(), ".zstack", "design-daemon-startup.log");
}

/**
 * Read the zstack version both client and daemon should agree on. Looks

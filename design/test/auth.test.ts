 ...
 *   - prefer ~/.zstack/openai.json over OPENAI_API_KEY
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "zstack-design-auth-"));
...
describe("resolveApiKeyInfo", () => {
  test("uses ~/.zstack/openai.json before OPENAI_API_KEY", () => {
    ...
    expect(describeApiKeySource(resolution!)).toBe("~/.zstack/openai.json");
...
describe("saveApiKey", () => {
  test("stores the key file owner-only, even under a permissive umask", () => {
    ...
    const keyPath = path.join(tmpHome, ".zstack", "openai.json");

...
if [ ! -f "$_HOOK_HELPER" ] || ! . "$_HOOK_HELPER" 2>/dev/null; then
  _FREEZE_DECIDED=1
  printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"[freeze] Hook helpers unavailable (broken install?) - blocked, fail closed. Reinstall zstack or run /unfreeze."}}\n'
  exit 0
fi

# Locate the freeze directory state file. The writer (/freeze via
# zstack-paths) and this reader MUST resolve the same root or the boundary
# fails open: with ZSTACK_HOME set, /freeze wrote freeze-dir.txt under
# ZSTACK_HOME while this hook read $HOME/.zstack, found nothing, and allowed
# everything (#1459, #1509). zstack_hook_state_root mirrors zstack-paths.
...
STATE_DIR="$(zstack_hook_state_root; printf x)"; STATE_DIR="${STATE_DIR%x}"
...
set +e
FILE_PATH=$(zstack_hook_extract_field "$INPUT" file_path)
EXTRACT_RC=$?
set -e
...
if [ "$EXTRACT_RC" -ne 0 ] && [ -n "$INPUT" ]; then
  zstack_hook_decision deny "[freeze] Could not parse the tool payload to check the freeze boundary. Blocked (fail closed). Freeze boundary: $FREEZE_DIR"
  _FREEZE_DECIDED=1
  exit 0
fi
...
case "$FILE_PATH" in
  ...
  *)
    ...
    # Log hook fire event (shared helper respects ZSTACK_HOME)
    zstack_hook_log_fire freeze boundary_deny
    ...
    zstack_hook_decision deny "[freeze] Blocked: $FILE_PATH is outside the freeze boundary ($FREEZE_DIR). Only edits within the frozen directory are allowed."
    _FREEZE_DECIDED=1
    ;;
esac

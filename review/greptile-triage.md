...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/zstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
PROJECT_HISTORY="$HOME/.zstack/projects/$REMOTE_SLUG/greptile-history.md"
```
...
Use Tier 2 when escalation detection (below) identifies a prior ZStack reply on the same thread. Include maximum evidence to close the discussion.
...
3. **If a prior ZStack reply exists AND Greptile posted again on the same file+category:** Use Tier 2 (firm) templates.

4. **If no prior ZStack reply exists:** Use Tier 1 (friendly) templates.
...
```bash
REMOTE_SLUG=$(browse/bin/remote-slug 2>/dev/null || ~/.claude/skills/zstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p "$HOME/.zstack/projects/$REMOTE_SLUG"
mkdir -p ~/.zstack
```

Append one line per triage outcome to **both** files (per-project for suppressions, global for retro):
- `~/.zstack/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `~/.zstack/greptile-history.md` (global aggregate)
...
Example entries:
```
2026-03-13 | zeidmahmoud/myapp | fp | app/services/auth_service.rb | race-condition
2026-03-13 | zeidmahmoud/myapp | fix | app/models/user.rb | null-check
2026-03-13 | zeidmahmoud/myapp | already-fixed | lib/payments.rb | error-handling
```

...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"landing-report","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "landing-report" --outcome OUTCOME \
...
for LEVEL in micro patch minor major; do
  bun run ~/.claude/skills/zstack/bin/zstack-next-version \
    --base "$BASE_BRANCH" \
    --bump "$LEVEL" \

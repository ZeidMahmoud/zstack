...
| `/open-zstack-browser` | Launch the visible ZStack Browser with sidebar + stealth. |
...
- The `claude` CLI binary resolves via `browse/src/claude-bin.ts` (`Bun.which()` + `ZSTACK_CLAUDE_BIN` override). Set `ZSTACK_CLAUDE_BIN=wsl` plus `ZSTACK_CLAUDE_BIN_ARGS='["claude"]'` to run Claude through WSL on Windows.

# zstack — AI Engineering Workflow

zstack is a collection of SKILL.md files that give AI agents structured roles for
software development. Each skill is a specialist: CEO reviewer, eng manager,
designer, QA lead, release engineer, debugger, and more.

## Available skills

Skills live in `.agents/skills/` (or `~/.claude/skills/zstack/` on Claude Code).
...
| `/canary` | Post-deploy monitoring loop in your Aside browser (or zstack's own when Aside is absent). |
...
| `/zstack-upgrade` | Update zstack to the latest version. |
...
| `/learn` | Manage what zstack learned across sessions. |
...
zstack never installs it. When Aside is not installed or not running (Linux,
Windows, a closed Aside app) the same skills fall back automatically to zstack's
own headless Chromium (`$B`), which is where the three skills under `/browse` apply.

| Skill | What it does |
|-------|-------------|
| `/browse` | Drive a browser: open a page, read it, click through a flow, screenshots, console errors. Aside first; zstack's own Chromium (~100ms/command) as the fallback. Every other browser skill stands on it. |
| `/open-zstack-browser` | Launch the visible ZStack Browser with sidebar + stealth — the headed face of the fallback engine. |
...
| `/pair-agent` | Pair a remote AI agent (OpenClaw, Codex, etc.) with zstack's own browser over a scoped tunnel. |
...
| `/make-pdf` | Turn any markdown file into a publication-quality PDF. Renders through Aside, or zstack's own browser when Aside is absent. |
| `/diagram` | English in, diagram out: mermaid source + editable .excalidraw + SVG/PNG, offline. Renders through Aside, or zstack's own browser when Aside is absent. |
...
6. Declare a fixture actor’s supported interactions before the model starts.
   ...
   directory before paid work; native snapshots require EVALS_RUN_ID or ZSTACK_EVAL_DIR.
...
- **Windows**: curated Windows-safe subset runs on `windows-latest` via the
  ...
  MSYS today; native PowerShell support is a future expansion. The `bin/zstack-paths`
  helper resolves state roots through `CLAUDE_PLUGIN_DATA` / `ZSTACK_HOME` so plugin
...
- **Browser and renderer**: the browser skills, `/make-pdf`, and `/diagram` drive
  ...
  zstack's own bundled browser, built by `./setup`.

## Key conventions

- SKILL.md files are **generated** from `.tmpl` templates. Edit the template, not the output.
- Run `bun run gen:skill-docs --host codex` to regenerate Codex-specific output.
- Browser steps in skills are `aside repl` scripts per `scripts/resolvers/aside.ts` (Aside first), each with a `$B` equivalent for the fallback engine — `$B <command>` is the browse binary and is a legitimate tool when the Aside probe does not print `READY`. Local HTML renders through `bin/zstack-render.ts`, which picks the same way.
- Safety skills (careful, freeze, guard) use inline advisory prose — always confirm before destructive operations.
- State paths resolve via `bin/zstack-paths` (sourced via `eval "$(...)"`). Honors `ZSTACK_HOME`, `CLAUDE_PLUGIN_DATA`, `CLAUDE_PLANS_DIR`.
- The `claude` CLI binary resolves via `lib/claude-bin.ts` (re-exported from `browse/src/claude-bin.ts` for browse internals; `Bun.which()` + `ZSTACK_CLAUDE_BIN` override). Set `ZSTACK_CLAUDE_BIN=wsl` plus `ZSTACK_CLAUDE_BIN_ARGS='["claude"]'` to run Claude through WSL on Windows.

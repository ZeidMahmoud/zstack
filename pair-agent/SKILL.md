...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"pair-agent","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "pair-agent" --outcome OUTCOME \
...
```bash
~/.claude/skills/zstack/bin/zstack-config get pair_agent 2>/dev/null || echo "unset"
```
...
Options: A) Enable — run `~/.claude/skills/zstack/bin/zstack-config set pair_agent on`, confirm it reads back `on`, and continue. B) No — stop here; local pairing (option A above) still works.

If the value is already `on`, say nothing and continue — consent stands until
`zstack-config set pair_agent off`.

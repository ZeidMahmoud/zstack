...
```bash
CODEX_PROMPT_FILE=$(mktemp /tmp/zstack-codex-oh-XXXXXXXX)
```
...
```bash
eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null)"
_DESIGN_DIR="$HOME/.zstack/projects/$SLUG/designs/mockup-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
```bash
SKETCH_FILE="/tmp/zstack-sketch-$(date +%s).html"
```
...
$B screenshot /tmp/zstack-sketch.png
```
...
The screenshot file at `/tmp/zstack-sketch.png` can be referenced by downstream skills

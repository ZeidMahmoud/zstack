...
preamble-tier: 3
version: 2.0.0
description: YC Office Hours — two modes. (zstack)
allowed-tools:
  - Bash
  - Read
  - Grep
...
gbrain:
  ...
  context_queries:
    ...
    - id: builder-profile
      kind: filesystem
      glob: "~/.zstack/builder-profile.jsonl"
      tail: 1
      render_as: "## Your builder profile snapshot"
    - id: design-doc-history
      kind: filesystem
      glob: "~/.zstack/projects/{repo_slug}/*-design-*.md"
      sort: mtime_desc
      limit: 3
      render_as: "## Recent design docs for this project"
    - id: prior-eureka
      kind: filesystem
      glob: "~/.zstack/analytics/eureka.jsonl"
      tail: 5
...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"office-hours","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "office-hours" --outcome OUTCOME \
...
eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "product"
  ~/.claude/skills/zstack/bin/zstack-brain-cache get product --project "$SLUG" 2>/dev/null || printf '_(no product digest available yet)_\n'
  printf '\n### %s\n\n' "goals"
  ~/.claude/skills/zstack/bin/zstack-brain-cache get goals --project "$SLUG" 2>/dev/null || printf '_(no goals digest available yet)_\n'
  printf '\n### %s\n\n' "user-profile"
  ~/.claude/skills/zstack/bin/zstack-brain-cache get user-profile  2>/dev/null || printf '_(no user-profile digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
  ~/.claude/skills/zstack/bin/zstack-brain-cache get recent-decisions --project "$SLUG" 2>/dev/null || printf '_(no recent-decisions digest available yet)_\n'
  printf '\n### %s\n\n' "salience"
  ~/.claude/skills/zstack/bin/zstack-brain-cache get salience --project "$SLUG" 2>/dev/null || printf '_(no salience digest available yet)_\n'
} > /tmp/.zstack-brain-context-$$.md 2>/dev/null
[ -s /tmp/.zstack-brain-context-$$.md ] && cat /tmp/.zstack-brain-context-$$.md
rm -f /tmp/.zstack-brain-context-$$.md 2>/dev/null || true
```
...
`zstack/`, `concepts/` only). Personal/family/therapy content never leaks here.
...
4. **List existing design docs for this project:**
   ...
   ls -t ~/.zstack/projects/$SLUG/*-design-*.md 2>/dev/null
   ```
...
> **STOP.** Before running the startup-mode diagnostic (Phase 2A: operating principles, pushback patterns, and the six forcing questions), Read `~/.claude/skills/zstack/office-hours/sections/phase-2a-startup-diagnostic.md` and execute it
...
> **STOP.** Before running the builder-mode brainstorm (Phase 2B: operating principles, the wild exemplar, and the generative questions), Read `~/.claude/skills/zstack/office-hours/sections/phase-2b-builder-brainstorm.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.zstack/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `~/.zstack/projects/`.
...
```bash
OUTSIDE_PROMPT_FILE=$(mktemp /tmp/zstack-outside-oh-XXXXXXXX)
```
...
```bash
bun run $HOME/.claude/skills/zstack/bin/zstack-render.ts <sketch-dir>/sketch.html --screenshot <sketch-dir>/sketch.png --width 1280
```
...
_OUTSIDE_PROMPT=$(cat "$_OUTSIDE_INPUT") || exit 1
_OUTSIDE_EXIT=0
_zstack_codex_timeout_wrapper 300 codex exec "$_OUTSIDE_PROMPT" -C "$_REPO_ROOT" -s read-only -c "model=\"${ZSTACK_CODEX_MODEL:-gpt-6-astra}\"" -c 'model_reasoning_effort="medium"' -c 'web_search="cached"' < /dev/null >"$_OUTSIDE_TMP/text" 2>"$_OUTSIDE_TMP/stderr" || _OUTSIDE_EXIT=$?
# Preserve findings and partial output even when transport or validation fails.
cat "$_OUTSIDE_TMP/text" || { [ "$_OUTSIDE_EXIT" -ne 0 ] || _OUTSIDE_EXIT=1; }
...
`zstack-developer-profile --log-session` binary handles its own directory creation
and writes via atomic mktemp+mv to `~/.zstack/developer-profile.json`.
...
```bash
~/.claude/skills/zstack/bin/zstack-developer-profile --log-session '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' 2>/dev/null || true
```
...
> **STOP.** Before writing the design doc and running the tiered relationship handoff (Phases 5-6, after the conversation and alternatives are done), Read `~/.claude/skills/zstack/office-hours/sections/design-and-handoff.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
```bash
~/.claude/skills/zstack/bin/zstack-learnings-log '{"skill":"office-hours","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```

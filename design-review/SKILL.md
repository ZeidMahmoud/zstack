...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/zstack/bin/zstack-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
Before building anything unfamiliar, **search first.** See `~/.claude/skills/zstack/ETHOS.md`.
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "design-review" --outcome OUTCOME \
...
zstack drives the Aside AI browser first. It is the user's real browser: real cookies, real logged-in accounts, their open tabs — you work inside the sessions the user already has. When Aside is not available, the Browser fallback section below drives zstack's own headless browser instead.
...
1. `NEEDS_ASIDE`: if `uname -s` prints `Darwin`, tell the user once — "zstack works best with the Aside browser (macOS 15+): download it at aside.com, open it, sign in, then re-run." Off macOS, do not pitch it. The user downloads and installs it themselves; NEVER run an installer, brew formula, or download for them, and never substitute unit tests or curl for the browser step. Then continue with the Browser fallback section below.
...
## Browser fallback: zstack's own headless browser

Applies when BROWSER SETUP printed `NEEDS_ASIDE` or `ASIDE_NOT_RUNNING` (Linux, Windows, or the Aside app closed), or when the user chose zstack's own browser in a Third-Party Web Actions question. Otherwise skip this section. Drive zstack's own headless Chromium through `$B`: same skill, same evidence, same report — different driver. Say once which driver you use.
...
If `NEEDS_SETUP`: tell the user "zstack's own browser needs a one-time build (~10 seconds). OK to proceed?", STOP for the answer, then run `cd <SKILL_DIR> && ./setup` (it installs bun when missing). If neither Aside nor `$B` is available after that, stop and say so — never substitute unit tests or curl for the browser step.
...
**Find the zstack designer (optional — enables target mockup generation):**
...
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/zstack/design/dist/design" ] && D="$_ROOT/.claude/skills/zstack/design/dist/design"
[ -z "$D" ] && D="$HOME/.claude/skills/zstack/design/dist/design"
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
ELI10: impeccable is a separate Apache-2.0 tool (Paul Bakaus). Its engine is one <n>-byte program that checks pages and CSS for 61 mechanical design mistakes. zstack can download that one file (version <v>, from github.com/pbakaus/impeccable releases) into <dest>, check it against a checksum recorded in zstack, and log the download in ~/.zstack/security/egress.jsonl. No impeccable skill, no editor hook; the engine never touches the network when zstack runs it. Without it this skill works as it does today.
...
C) Never ask again
  ✅ Design skills stay silent about impeccable (reversible: zstack-config set design_detector_install_prompted false)
  ❌ An engine you install later is still used, but zstack never reminds you
D) Turn the detector off
  ✅ No probe, scan, or handoff line in any design skill (zstack-config set design_detector off)
...
```bash
bun --no-env-file run $HOME/.claude/skills/zstack/bin/zstack-design-detect.ts install --host claude
```

On **B**, continue without scans. On **C**, run `~/.claude/skills/zstack/bin/zstack-config set design_detector_install_prompted true`. On **D**, run `~/.claude/skills/zstack/bin/zstack-config set design_detector off`. Never pass `--sha256` or `--base` yourself: they exist for maintainers and mirrors. If the user also wants the `/impeccable` skill and its hook, they run `npx impeccable install` themselves; zstack never does.
...
REPORT_DIR="$ZSTACK_STATE_ROOT/projects/$SLUG/designs/design-audit-$(date +%Y%m%d)"
RUN_ID="$(date +%H%M%S)-$$"
...
```bash
_CROSS_PROJ=$(~/.claude/skills/zstack/bin/zstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.claude/skills/zstack/bin/zstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ~/.claude/skills/zstack/bin/zstack-learnings-search --limit 10 2>/dev/null || true
fi
```
...
If A: run `~/.claude/skills/zstack/bin/zstack-config set cross_project_learnings true`
If B: run `~/.claude/skills/zstack/bin/zstack-config set cross_project_learnings false`
...
```bash
aside repl '
const HOOK = `(() => { window.__zstackErrs = window.__zstackErrs || []; const oe = console.error; console.error = (...a) => { window.__zstackErrs.push(a.map(String).join(" ")); oe.apply(console, a); }; window.addEventListener("error", e => window.__zstackErrs.push("uncaught: " + e.message)); window.addEventListener("unhandledrejection", e => window.__zstackErrs.push("unhandledrejection: " + (e.reason && e.reason.message || e.reason))); })()`;
...
console.log("URL=" + pg.url());
console.log("CONSOLE_ERRORS=" + JSON.stringify(await pg.evaluate(() => window.__zstackErrs)));
...
console.log("ASIDE_DIR=" + pwd); await closeTab(pg); console.log("ZSTACK_STEP_OK");
'
```
...
```bash
_TMP=$(mktemp -d); _DUMP=$(cat "$HOME/.claude/skills/zstack/lib/dom-dump.js")
...
elif $HOME/.claude/skills/zstack/bin/zstack-redact --from-file "$_D" --max-bytes 10485760 >/dev/null 2>&1; _RC=$?; [ "$_RC" -ne 0 ] && [ "$_RC" -ne 2 ]; then echo "DOM_DUMP_REDACTION_BLOCKED: {page} redact-exit=$_RC"; rm -f "$_D"
...
```bash
_DJ=$(mktemp); bun --no-env-file run $HOME/.claude/skills/zstack/bin/zstack-design-detect.ts scan --format zstack --host claude "<REPORT_DIR from Setup>/dom/<RUN_ID>" > "$_DJ"; echo "DETECT_EXIT_CODE=$?"; echo "DETECT_JSON=$_DJ"
```
...
```bash
aside repl '
const HOOK = `(() => { window.__zstackErrs = window.__zstackErrs || []; const oe = console.error; console.error = (...a) => { window.__zstackErrs.push(a.map(String).join(" ")); oe.apply(console, a); }; window.addEventListener("error", e => window.__zstackErrs.push("uncaught: " + e.message)); })()`;
const pg = await openTab("about:blank");
await pg._sendToTarget("Page.addScriptToEvaluateOnNewDocument", { source: HOOK });
await pg.goto("<url>");
...
**Local:** `.zstack/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null)" && mkdir -p ~/.zstack/projects/$SLUG
```
Write to: `~/.zstack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
- Previous baseline = the newest readable `design-baseline*.json` under `$ZSTACK_STATE_ROOT/projects/$SLUG/designs/design-audit-*/` older than this run; unreadable → "previous baseline unreadable (first scan)"
...
Source: [OpenAI "Designing Delightful Frontends with GPT-5.4"](https://developers.openai.com/blog/designing-delightful-frontends-with-gpt-5-4) (Mar 2026) + zstack design methodology.
...
```
$ZSTACK_STATE_ROOT/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
**Log the result:**
```bash
~/.claude/skills/zstack/bin/zstack-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","host":"claude","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"design","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
If the zstack designer is available and the finding involves visual layout, hierarchy, or spacing (not just a CSS value fix like wrong color or font-size), generate a target mockup showing what the corrected version should look like:
...
eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null)" && mkdir -p ~/.zstack/projects/$SLUG
```
Write a one-line summary to `~/.zstack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
...
```bash
~/.claude/skills/zstack/bin/zstack-learnings-log '{"skill":"design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```

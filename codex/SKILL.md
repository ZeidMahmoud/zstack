...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"codex","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "codex" --outcome OUTCOME \
...
if [ "${ZSTACK_FORCE_CODEX_REVIEW:-0}" != "1" ] && { [ -n "${CODEX_THREAD_ID:-}" ] || [ -n "${CODEX_SANDBOX:-}" ]; }; then
  ...
  _zstack_codex_model_probe   # ~10s round trip on first run, cached 1h (#2477)
fi
...
> **STOP.** Before running Review mode (Step 2A) — the Step 1 dispatch chose review (`/codex review`, or the user picked "Review the diff"), Read `~/.claude/skills/zstack/codex/sections/review-mode.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before running Challenge mode (Step 2B) — the Step 1 dispatch chose adversarial challenge (`/codex challenge`, or the user picked "Challenge the diff"), Read `~/.claude/skills/zstack/codex/sections/challenge-mode.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before running Consult mode (Step 2C) — the Step 1 dispatch chose consult (a free-form question, a plan review, or a session follow-up), Read `~/.claude/skills/zstack/codex/sections/consult-mode.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
- **Bash gate above the wrapper.** Every Bash call to codex sets its `timeout`
  parameter ABOVE the inner `_zstack_codex_timeout_wrapper` budget (Review:

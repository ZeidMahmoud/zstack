...
echo '{"skill":"careful","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.zstack/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
`~/.zstack/careful-patterns.txt` (global) or
`~/.zstack/projects/<slug>/careful-patterns.txt` (per-project). Consulted

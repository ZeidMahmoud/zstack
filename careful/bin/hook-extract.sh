#!/usr/bin/env bash
# hook-extract.sh — SHARED JSON helpers for zstack PreToolUse hooks.
...
# zstack_hook_extract_field PAYLOAD FIELD
...
# zstack_hook_decision DECISION REASON
...
zstack_hook_decision() {
  _ghd_decision="$1"
  _ghd_reason="$2"
  _ghd_encoded=$(zstack_hook_json_string "$_ghd_reason")
...
zstack_hook_log_fire() {
  ...
  printf '{"event":"hook_fire","skill":%s,"pattern":%s,"ts":"%s","repo":%s}\n' \
    "$(zstack_hook_json_string "$1")" \
    "$(zstack_hook_json_string "$2")" \
    "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    "$(zstack_hook_json_string "$_ghlf_repo")" >> "$_ghlf_dir/skill-usage.jsonl" 2>/dev/null || true
}

...
# unreadable input.
set +e
CMD=$(zstack_hook_extract_field "$INPUT" command)
EXTRACT_RC=$?
set -e

# No parser available, or the payload is not parseable JSON. Fail closed.
if [ "$EXTRACT_RC" -ne 0 ] && [ -n "$INPUT" ]; then
  zstack_hook_decision ask "[careful] Could not parse the tool payload to safety-check this command. Approve only if you know what it does."
  exit 0
fi
...
# Shared helper respects ZSTACK_HOME, so tests never write real analytics.
_careful_log_fire() { zstack_hook_log_fire careful "$1"; }
...
if grep -qE '\$\{IFS\}|\$IFS|\$\(echo[^)]*base64[^)]*\)|base64[[:space:]]+(-d|--decode)[^|]*\|[[:space:]]*(sh|bash)' <<< "$CMD" 2>/dev/null; then
  zstack_hook_decision ask "[careful] Shell obfuscation detected (IFS word-splitting or base64-to-shell). Read the command carefully before approving."
  exit 0
fi
...
# installer flows, including zstack's own setup pattern.
_IS_SIMPLE=1
case "$CMD" in
  *';'*|*'&&'*|*'||'*|*'|'*|*$'\n'*) _IS_SIMPLE=0 ;;
esac
if [ "$_IS_SIMPLE" -eq 1 ]; then
  ...
  if grep -qE '^[[:space:]]*(sudo[[:space:]]+)?rm[[:space:]]' <<< "$CMD" 2>/dev/null \
    ...
    if [ "$_ROOT_TARGETS" -eq 1 ] && [ "$_SAFE_TARGETS" -eq 0 ]; then
      ...
      zstack_hook_decision deny "[careful][HIGH] Recursive delete of / or the home directory is blocked while /careful is active. If you truly mean it, end the /careful session first."
      exit 0
    fi
  fi
    ...
    if [ "$_HAS_FORCE" -eq 1 ]; then
      ...
      if [ -n "$_DEFAULT_BRANCH" ]; then
        ...
        if [ "$_TARGETS_DEFAULT" -eq 1 ]; then
          ...
          zstack_hook_decision deny "[careful][HIGH] Force-push to the default branch ($_DEFAULT_BRANCH) is blocked while /careful is active. Use --force-with-lease on a feature branch, or end the /careful session if you truly mean it."
          exit 0
        fi
      fi
    fi
...
if [ -z "$WARN" ]; then
  # Same state root the writer (/careful via zstack-paths) uses — see
  # zstack_hook_state_root in hook-extract.sh (#1459 class).
  if command -v zstack_hook_state_root >/dev/null 2>&1; then
    _ZSTACK_HOME_DIR="$(zstack_hook_state_root; printf x)"; _ZSTACK_HOME_DIR="${_ZSTACK_HOME_DIR%x}"
  else
    # Older hook-extract.sh (partial upgrade): the plain chain beats dying
    # under set -e with no decision JSON — rules under $HOME/.zstack still load.
    _ZSTACK_HOME_DIR="${ZSTACK_HOME:-$HOME/.zstack}"
  fi
  _PATTERN_FILES="$_ZSTACK_HOME_DIR/careful-patterns.txt"
  ...
  _ANY_PROJ_PAT=$(find "$_ZSTACK_HOME_DIR/projects" -maxdepth 2 -name careful-patterns.txt -print -quit 2>/dev/null || true)
  if [ -n "$_ANY_PROJ_PAT" ]; then
    eval "$("$_HOOK_DIR/../../bin/zstack-slug" 2>/dev/null)" 2>/dev/null || true
    if [ -n "${SLUG:-}" ]; then
      _PATTERN_FILES="$_PATTERN_FILES
$_ZSTACK_HOME_DIR/projects/$SLUG/careful-patterns.txt"
    fi
  fi
...
if [ -n "$WARN" ]; then
  ...
  zstack_hook_decision ask "[careful] $WARN"
else
  echo '{}'
fi

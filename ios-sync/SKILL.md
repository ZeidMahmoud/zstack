...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"ios-sync","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "ios-sync" --outcome OUTCOME \
...
**Templates live in upstream zstack.** The installed
`zstack-ios-qa-regen` launcher resolves its own zstack root and copies only
...
2. Read upstream version from `$ZSTACK_ROOT/VERSION`.

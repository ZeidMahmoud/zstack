...
gbrain:
  ...
  context_queries:
    - id: prior-retros
      kind: filesystem
      glob: "~/.zstack/projects/{repo_slug}/retros/*.md"
      sort: mtime_desc
      limit: 5
      render_as: "## Prior retros for this project"
    - id: recent-timeline
      kind: filesystem
      glob: "~/.zstack/projects/{repo_slug}/timeline.jsonl"
      tail: 30
      render_as: "## Recent timeline events"
    - id: recent-learnings
      kind: filesystem
      glob: "~/.zstack/projects/{repo_slug}/learnings.jsonl"
      tail: 10
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"retro","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.zstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/zstack/bin/zstack-timeline-log '{"skill":"retro","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"retro","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
[ -f ~/.zstack/retro-context.md ] && echo "RETRO_CONTEXT_FOUND" || echo "NO_RETRO_CONTEXT"
```

If `RETRO_CONTEXT_FOUND`: read `~/.zstack/retro-context.md`. This file is user-authored and may contain meeting notes, calendar events, decisions, and other context that doesn't appear in git history. Incorporate this context into the retro narrative where relevant.
...
cat ~/.zstack/greptile-history.md 2>/dev/null || true
...
# 12. zstack skill usage telemetry (if available)
cat ~/.zstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
**Skill Usage (if analytics exist):** Read `~/.zstack/analytics/skill-usage.jsonl` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:
...
**Eureka Moments (if logged):** Read `~/.zstack/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
```
  EUREKA /office-hours (branch: zeid/auth-rethink): "Session tokens don't need server storage — browser crypto API makes client-side JWT validation viable"
  EUREKA /plan-eng-review (branch: zeid/cache-layer): "Redis isn't needed here — Bun's built-in LRU cache handles this workload"
```
...
**Note:** Only include the `greptile` field if `~/.zstack/greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null)"
cat ~/.zstack/projects/$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
```bash
DISCOVER_BIN=""
[ -x ~/.claude/skills/zstack/bin/zstack-global-discover ] && DISCOVER_BIN=~/.claude/skills/zstack/bin/zstack-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x .claude/skills/zstack/bin/zstack-global-discover ] && DISCOVER_BIN=.claude/skills/zstack/bin/zstack-global-discover
[ -z "$DISCOVER_BIN" ] && which zstack-global-discover >/dev/null 2>&1 && DISCOVER_BIN=$(which zstack-global-discover)
[ -z "$DISCOVER_BIN" ] && [ -f bin/zstack-global-discover.ts ] && DISCOVER_BIN="bun run bin/zstack-global-discover.ts"
echo "DISCOVER_BIN: $DISCOVER_BIN"
```

If no binary is found, tell the user: "Discovery script not found. Run `bun run build` in the zstack directory to compile it." and stop.

Run the discovery:
```bash
$DISCOVER_BIN --since "<window>" --format json 2>/tmp/zstack-discover-stderr
```

Read the stderr output from `/tmp/zstack-discover-stderr` for diagnostic info. Parse the JSON output from stdout.
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.zstack/retros/global-*.json 2>/dev/null | head -5
```
...
```json
{
  ...
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | CC: 48, Codex: 8, Gemini: 3 | Focus: zstack (58%) | Streak: 52d"
}
```

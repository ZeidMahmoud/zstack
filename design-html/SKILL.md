...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"design-html","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "design-html" --outcome OUTCOME \
...
> **STOP.** Before writing the finalized HTML in Step 3 — the Pretext wiring patterns and API cheatsheet are the required reference for all text-layout code, Read `~/.claude/skills/zstack/design-html/sections/pretext-patterns.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

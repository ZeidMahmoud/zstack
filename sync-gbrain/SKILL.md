...
preamble-tier: 2
version: 1.0.0
description: Keep gbrain current with this repo's code and refresh agent search guidance in CLAUDE.md. Wraps the zstack-gbrain-sync orchestrator with state (zstack)
triggers:
  - sync gbrain
  - refresh gbrain
  - reindex repo
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"sync-gbrain","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.zstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/zstack/bin/zstack-timeline-log '{"skill":"sync-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"sync-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
It does NOT touch `~/.zstack/` indexing (the existing `zstack-gbrain-source-wireup`
...
- `/sync-gbrain --refresh-cache` — force-rebuild brain-aware planning cache (v1.48; replaces /brain-refresh-context per D1 fold). Skips code + memory stages; routes to `zstack-brain-cache refresh --project <slug>`.
- `/sync-gbrain --audit` — emit summary of zstack-owned pages per project + sensitive-content audit (v1.48 / D10 lifecycle). Read-only.

Pass-through args go straight to the orchestrator at
`~/.claude/skills/zstack/bin/zstack-gbrain-sync.ts`.

**`--refresh-cache` short-circuit:** when this flag is present, the skill
runs ONLY the cache refresh (`zstack-brain-cache refresh --project <slug>`
for the current worktree's slug, plus a cross-project refresh of
user-profile if `zstack/user-profile/<user-slug>` exists). Code +
memory + brain-sync stages are skipped. Useful when the user knows the
brain has new info zstack should pick up before the next planning skill.

**`--audit` short-circuit:** when this flag is present, the skill runs
`zstack-brain-cache list --project <slug> --json`, summarizes by page
...
```bash
~/.claude/skills/zstack/bin/zstack-gbrain-detect 2>/dev/null
```
...
```bash
_HASH=$(~/.claude/skills/zstack/bin/zstack-config endpoint-hash 2>/dev/null)
_POLICY=$(~/.claude/skills/zstack/bin/zstack-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
```bash
~/.claude/skills/zstack/bin/zstack-config set brain_trust_policy@$_HASH personal
```
...
also runs locally** in local-stdio MCP mode — `zstack-memory-ingest` shells
...
`~/.zstack/transcripts/<run-id>/` and the artifacts pipeline pushes it to
the brain admin's pull job (plan D11). Brain-sync (the `zstack-brain-sync`
...
Also check the per-repo trust policy. If `zstack-gbrain-repo-policy get` for
...
```bash
bun run ~/.claude/skills/zstack/bin/zstack-gbrain-sync.ts <user-args>
```
...
still run. State is persisted to `~/.zstack/.gbrain-sync-state.json` via
tmp-file + atomic rename. Concurrent runs are blocked by a lock file at
`~/.zstack/.sync-gbrain.lock` (5-min stale-takeover).
...
```bash
SOURCE_ID=$(grep -o '"source_id":"[^"]*"' ~/.zstack/.gbrain-sync-state.json 2>/dev/null \
...
from `<!-- zstack-gbrain-search-guidance:start -->` through
`<!-- zstack-gbrain-search-guidance:end -->`. If those markers are missing,
...
gbrain status: GREEN
  ...
  CWD source ...... OK   <zstack-code-{repo_slug}> (page_count=<N>)
  ~/.zstack source. OK   <zstack-brain-{user}> (page_count=<N>) — managed by /setup-gbrain
...
Mac. The orchestrator acquires a lock at `~/.zstack/.sync-gbrain.lock` before
...
and travels with `git push`/`git pull` — NOT through `~/.zstack/.brain-allowlist`
(which is for `~/.zstack/` brain-sync only). On a different Mac with a synced

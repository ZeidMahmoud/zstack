---
name: zstack-sync-gbrain
preamble-tier: 2
version: 1.0.0
description: Keep gbrain current with this repo's code and refresh agent search guidance in CLAUDE.md. (zstack)
triggers:
  - sync gbrain
  - refresh gbrain
  - reindex repo
...
Wraps the zstack-gbrain-sync orchestrator with
...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"sync-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "sync-gbrain" --outcome OUTCOME \
...
- **`timeout`**: proceed to Step 2 — the engine is most likely healthy but
  ...
  probe timed out (>15s) — proceeding; raise `ZSTACK_GBRAIN_PROBE_TIMEOUT_MS`

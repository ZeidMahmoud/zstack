...
(adapt to whatever project you're in — these are not zstack-specific):
...
**If GitHub:**
```bash
gh pr view --json body -q .body > /tmp/zstack-pr-body-$$.md
cp /tmp/zstack-pr-body-$$.md /tmp/zstack-pr-body-orig-$$.md
```

**If GitLab:**
```bash
glab mr view -F json 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('description',''))" > /tmp/zstack-pr-body-$$.md
cp /tmp/zstack-pr-body-$$.md /tmp/zstack-pr-body-orig-$$.md
```
...
```bash
~/.claude/skills/zstack/bin/zstack-issue-guard --stdin --source pr-body < /tmp/zstack-pr-body-$$.md
```
...
4. Redaction scan-at-sink, then write the updated body back. The body is already
   in a temp file (`/tmp/zstack-pr-body-$$.md`); scan THAT file before editing so
...
~/.claude/skills/zstack/bin/zstack-redact --from-file /tmp/zstack-pr-body-$$.md --repo-visibility "${REDACT_VIS:-unknown}" --json
...
if [ ! -f /tmp/zstack-pr-body-orig-$$.md ] || [ ! -f /tmp/zstack-pr-body-$$.md ]; then
...
_ORIG_BANNERS=$(grep -c "UNTRUSTED TRACKER CONTENT" /tmp/zstack-pr-body-orig-$$.md 2>/dev/null)
_ORIG_BANNERS=${_ORIG_BANNERS:-0}
_NEW_BANNERS=$(grep -c "UNTRUSTED TRACKER CONTENT" /tmp/zstack-pr-body-$$.md 2>/dev/null)
...
**If GitHub:**
```bash
gh pr edit --body-file /tmp/zstack-pr-body-$$.md
```

**If GitLab:**
Read the contents of `/tmp/zstack-pr-body-$$.md` using the Read tool, then pass it to `glab mr update` using a heredoc to avoid shell metacharacter issues:
```bash
...
```bash
rm -f /tmp/zstack-pr-body-$$.md /tmp/zstack-pr-body-orig-$$.md
```
...
```bash
NEW_TITLE=$(~/.claude/skills/zstack/bin/zstack-pr-title-rewrite.sh "$V" "$CURRENT_TITLE")
```
...
stays discoverable: "Running the Codex doc review automatically (standard step). Disable: `zstack-config set codex_reviews disabled`."
...
**Persist the result:**
```bash
~/.claude/skills/zstack/bin/zstack-review-log '{"skill":"codex-doc-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```

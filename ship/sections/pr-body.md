...
2. Compute the corrected title: `NEW_TITLE=$(~/.claude/skills/zstack/bin/zstack-pr-title-rewrite.sh "$NEW_VERSION" "$CURRENT")`. The helper handles three cases: title already correct (no-op), title has a different `v<X.Y.Z.W>` prefix (replace it), or title has no version prefix (prepend one).
...
<Auto-detect: look for /spec archives matching this branch via:
  ...
  eval "$(~/.claude/skills/zstack/bin/zstack-slug)"
...
```bash
REDACT_VIS=$(~/.claude/skills/zstack/bin/zstack-config get redact_repo_visibility 2>/dev/null)
...
<PR body from above>
PR_BODY_EOF
~/.claude/skills/zstack/bin/zstack-redact --from-file "$PR_BODY_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json
case $? in
...
printf '%s' "v$NEW_VERSION <type>: <summary>" | ~/.claude/skills/zstack/bin/zstack-redact --repo-visibility "$REDACT_VIS" --json
```

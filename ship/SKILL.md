...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"ship","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "ship" --outcome OUTCOME \
...
```bash
_REDACT_PREPUSH=$(~/.claude/skills/zstack/bin/zstack-config get redact_prepush_hook 2>/dev/null || echo "false")
...
1. **`REDACT_PREPUSH: true` and `HOOK_INSTALLED: no` and `HOOKS_IN_GIT_DIR: yes`** —
   ...
   ```bash
   ~/.claude/skills/zstack/bin/zstack-redact install-prepush-hook
   ```
...
2. **`REDACT_PREPUSH` not true AND `PREPUSH_PROMPTED: no`** — one-time
   ...
   If A: run `~/.claude/skills/zstack/bin/zstack-config set redact_prepush_hook true`
   then `~/.claude/skills/zstack/bin/zstack-redact install-prepush-hook`.
   If B: run `~/.claude/skills/zstack/bin/zstack-config set redact_prepush_hook false`.
...
> **STOP.** Before dispatching the /document-release subagent to sync docs (Step 18) and then creating or updating the PR/MR (Step 19), Read `~/.claude/skills/zstack/ship/sections/pr-body.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

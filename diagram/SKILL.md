...
preamble-tier: 1
version: 1.0.0
description: "Turn an English description (or mermaid source) into a diagram triplet: the source, an editable .excalidraw file you can open on excalidraw.com, and rendered SVG + PNG. (zstack)"
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "diagram" --outcome OUTCOME \
...
`zstack-render` opens it from a loopback server on this machine — in the Aside
browser when Aside is running, otherwise in zstack's own headless browser. Its
...
(artifacts the user can commit), else `/tmp/zstack-diagrams/`. Derive
`<slug>` from the diagram's subject (kebab-case, ≤40 chars).

## Step 2 — Stage the render bundle (once per session)

`zstack-render` serves the bundle's directory on 127.0.0.1 for each render
(Aside refuses `file://`, and both engines get the same origin). Stage the bundle under
zstack's own render staging directory, `${TMPDIR:-/tmp}/zstack-render` (yours
...
directory holds nothing but zstack bundles, and concurrent sessions or mixed
zstack versions never clobber each other.

```bash
BUNDLE=""
for c in "$HOME/.claude/skills/zstack/lib/diagram-render/dist/diagram-render.html" \
         "$(git rev-parse --show-toplevel 2>/dev/null)/lib/diagram-render/dist/diagram-render.html"; do
  [ -f "$c" ] && BUNDLE="$c" && break
done
[ -z "$BUNDLE" ] && echo "BUNDLE_MISSING — run: cd ~/.claude/skills/zstack && bun run build:diagram-render" && exit 1
RD="${TMPDIR:-/tmp}/zstack-render"
if [ -e "$RD" ] && { [ -L "$RD" ] || [ ! -O "$RD" ]; }; then RD=$(mktemp -d "${TMPDIR:-/tmp}/zstack-render.XXXXXX"); else mkdir -p -m 700 "$RD"; fi
SHA=$(shasum -a 256 "$BUNDLE" | cut -c1-16)
STAGED="$RD/zstack-diagram-render-$SHA.html"
...
`zstack-render` call renders the whole triplet: it opens the staged bundle in
...
bun run ~/.claude/skills/zstack/bin/zstack-render.ts "<staged>" --wait-selector '#done' \
...
`zstack-render` picks the browser itself: Aside when it is running, otherwise
zstack's own headless browser. Only when it prints `NEEDS_ASIDE` or
`ASIDE_NOT_RUNNING` followed by `ERROR: no browser available` is there nothing
to render with — Aside (macOS 15+, aside.com) is not open and zstack's browser
is not built. Tell the user to open Aside, or to run `./setup` in the zstack

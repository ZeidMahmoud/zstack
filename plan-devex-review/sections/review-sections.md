...
**Persist the result:**
```bash
~/.claude/skills/zstack/bin/zstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
Persist after the DX Scorecard — the dashboard, the ZSTACK REVIEW REPORT, and the EXIT
PLAN MODE GATE's "review log was called" check depend on it. **PLAN MODE EXCEPTION — ALWAYS RUN** (writes to `~/.zstack/`, not project files):
...
1. Brain trust policy for the active endpoint is `personal` (check via
   `~/.claude/skills/zstack/bin/zstack-config get brain_trust_policy@<endpoint-hash>`).
...
a zstack:takes fence block (documented but uglier path).

...
| `/open-zstack-browser` | **ZStack Browser** — launch ZStack Browser with sidebar, anti-bot stealth, auto model routing (Sonnet for actions, Opus for analysis), one-click cookie import, and Claude Code integration. Clean up pages, take smart screenshots, edit CSS, and pass info back to your terminal. |
...
Two new browser primitives compound the zstack agent over time:
...
**`/browse` fails?** `cd ~/.claude/skills/zstack && bun install && bun run build`
...
/canary, /benchmark, /browse, /open-zstack-browser, /qa, /qa-only, /design-review,

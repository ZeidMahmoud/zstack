...
const result={pid:process.pid,cwd:process.cwd(),home:process.env.HOME,nativeState:process.env.ZSTACK_HOME,hook:hook?.command};
...
process.stdin.on('data',async bytes=>{
 ...
 try {
  if(hook){
   ...
   const sourceRoot=process.env.QA_MODE==='native-state'?process.env.ZSTACK_HOME:path.join(process.env.HOME,'.zstack');

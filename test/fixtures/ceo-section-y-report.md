...
**OUTSIDE COVERAGE:** disabled — `codex_reviews: disabled` in `.zstack-section-state-4pqsxp/config.yaml`. No fallback dispatched; disabled is an intentional opt-out. Re-enable: `zstack-config set codex_reviews enabled`.

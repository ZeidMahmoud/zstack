...
|----|----------|----------|
| D1 | Add zstack routing rules to CLAUDE.md | Yes. Deferred until plan mode exits (see Deferred actions). |
...
- D1: append the zstack skill-routing section to `CLAUDE.md` and commit it.
...
- **OUTSIDE COVERAGE:** provider codex, phase plan-review, outside_status disabled (user config `codex_reviews=disabled`), no findings; no native fallback dispatched because disabled is an opt-out, not a failure. Re-enable: `zstack-config set codex_reviews enabled`.

...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ship","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.zstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/zstack/bin/zstack-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
If `PROACTIVE` is `"false"`, do not proactively suggest zstack skills AND do not
...
or invoking other zstack skills, use the `/zstack-` prefix (e.g., `/zstack-qa` instead
of `/qa`, `/zstack-ship` instead of `/ship`). Disk paths are unaffected — always use
`~/.claude/skills/zstack/[skill-name]/SKILL.md` for reading skill files.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `~/.claude/skills/zstack/zstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If `JUST_UPGRADED <from> <to>`: tell user "Running zstack v{to} (just updated!)" and continue.

If `LAKE_INTRO` is `no`: Before continuing, introduce the Completeness Principle.
Tell the user: "zstack follows the **Boil the Lake** principle — always do the complete
thing when AI makes the marginal cost near-zero. Read more: https://zeidslist.org/posts/boil-the-ocean"
...
> Help zstack get better! Community mode shares usage data (which skills you use, how long
> they take, crash info) with a stable device ID so we can track trends and fix bugs faster.
> No code, file paths, or repo names are ever sent.
> Change anytime with `zstack-config set telemetry off`.
...
> How about anonymous mode? We just learn that *someone* used zstack — no unique ID,
...
> zstack can proactively figure out when you might need a skill while you work —
...
If B: run `~/.claude/skills/zstack/bin/zstack-config set routing_declined true`
Say "No problem. You can add routing rules later by running `zstack-config set routing_declined false` and re-running any skill."
...
You are ZStack, an open source AI builder framework shaped by the author's product, startup, and engineering judgment. Encode how he thinks, not his biography.
...
When a user shows unusually strong product instinct, deep user empathy, sharp insight, or surprising synthesis across domains, recognize it plainly. For exceptional cases only, say that people with that kind of taste and drive are exactly the kind of builders the author respects and wants to fund, and that they should consider applying to YC. Use this rarely and only when truly earned.
...
AI makes completeness near-free. Always recommend the complete option over shortcuts — the delta is minutes with CC+zstack. A "lake" (100% coverage, all edge cases) is boilable; an "ocean" (full rewrite, multi-quarter migration) is not. Boil lakes, flag oceans.

**Effort reference** — always show both scales:

| Task type | Human team | CC+zstack | Compression |
|-----------|-----------|-----------|-------------|
...
`~/.zstack/analytics/` (user config directory, not project files). The skill
...
- Writing to `~/.zstack/` (config, analytics, review logs, design artifacts, learnings)
...
1. Check if the plan file already has a `## ZSTACK REVIEW REPORT` section.
...
Then write a `## ZSTACK REVIEW REPORT` section to the end of the plan file:
...
For Design Review: run `source <(~/.claude/skills/zstack/bin/zstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
```bash
source <(~/.claude/skills/zstack/bin/zstack-diff-scope <base> 2>/dev/null)
```
...
```bash
~/.claude/skills/zstack/bin/zstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <(~/.claude/skills/zstack/bin/zstack-diff-scope <base> 2>/dev/null) || true
...
```bash
~/.claude/skills/zstack/bin/zstack-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `~/.claude/skills/zstack/review/specialists/testing.md`
2. **Maintainability** — read `~/.claude/skills/zstack/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `~/.claude/skills/zstack/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `~/.claude/skills/zstack/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `~/.claude/skills/zstack/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `~/.claude/skills/zstack/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `~/.claude/skills/zstack/review/design-checklist.md`
...
```bash
~/.claude/skills/zstack/bin/zstack-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `~/.claude/skills/zstack/review/specialists/red-team.md`
...
```bash
~/.claude/skills/zstack/bin/zstack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
OLD_CFG=$(~/.claude/skills/zstack/bin/zstack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.claude/skills/zstack/bin/zstack-learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```

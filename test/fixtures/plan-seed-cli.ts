...
fs.writeFileSync(path.join(dir,'launch.json'),JSON.stringify({argv:process.argv.slice(2),planModeHint:process.env.ZSTACK_PLAN_MODE??null,planModeForce:process.env.ZSTACK_PLAN_MODE_FORCE??null}));
...
process.stdin.on('data',chunk=>{
 ...
 if(/^\/plan-[a-z-]+\r$/.test(input)){
  ...
  if(scenario==='native-replay')process.emit('zstack-seeded-slash',command);

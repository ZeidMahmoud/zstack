...
ROLLBACK: zstack-config set cache_enabled false (or equivalent feature flag disable).
```
...
**OUTSIDE COVERAGE:** disabled — `codex_reviews` is set to `disabled` in config. No outside review step dispatched and no native fallback run. Re-enable: `zstack-config set codex_reviews enabled`.

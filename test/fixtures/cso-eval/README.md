...
install -m 0555 bin/zstack-cso-launcher bin/zstack-cso-core \
  bin/zstack-cso-watchdog "$stage/"
install -m 0444 bin/.zstack-cso-generation "$stage/.zstack-cso-generation"
(cd "$stage" && (sha256sum cso-eval-producer zstack-cso-launcher \
  zstack-cso-core zstack-cso-watchdog .zstack-cso-generation 2>/dev/null || \
  shasum -a 256 cso-eval-producer zstack-cso-launcher zstack-cso-core \
  zstack-cso-watchdog .zstack-cso-generation))
sudo install -d -o root -g root -m 0755 /opt/zstack-cso-producer
sudo install -o root -g root -m 0555 "$stage/cso-eval-producer" \
  "$stage/zstack-cso-launcher" "$stage/zstack-cso-core" \
  "$stage/zstack-cso-watchdog" /opt/zstack-cso-producer/
sudo install -o root -g root -m 0444 "$stage/.zstack-cso-generation" \
  /opt/zstack-cso-producer/.zstack-cso-generation
```

Run `/opt/zstack-cso-producer/cso-eval-producer` as an unprivileged account.
...
zstack helper/runtime files. Do not install or copy any CSO `SKILL.md`, carved
...
CSO_EVAL_PAID=1 /opt/zstack-cso-producer/cso-eval-producer run \
...
The producer host needs Git, the chosen authenticated CLI, and reviewed zstack

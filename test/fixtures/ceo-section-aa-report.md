...
Re-enable: `zstack-config set codex_reviews enabled`.
...
**OUTSIDE COVERAGE:** disabled — `codex_reviews: disabled` set in config.yaml. No outside review run; no native fallback dispatched (disabled is a terminal branch, not a provider failure). Re-enable: `zstack-config set codex_reviews enabled`.

...
ZSTACK_ROOT="$HOME/.codex/skills/zstack"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.agents/skills/zstack" ] && ZSTACK_ROOT="$_ROOT/.agents/skills/zstack"
...
[ -x "$_SS" ] || _SS=".agents/skills/zstack/bin/zstack-skill-start"
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.agents/skills/zstack/review/TODOS-format.md`).
...
```bash
bun --no-env-file run $ZSTACK_BIN/zstack-design-detect.ts probe --host codex
```

On `IMPECCABLE_READY`, scan the changed frontend files (the wrapper derives them from git; hook presence does not skip this):

```bash
_DJ=$(mktemp); bun --no-env-file run $ZSTACK_BIN/zstack-design-detect.ts scan --changed <base> --format zstack --host codex > "$_DJ"; echo "DETECT_EXIT_CODE=$?"; echo "DETECT_JSON=$_DJ"
```
...
if [ -n "${ZSTACK_ROOT:-}" ] && [ -d "$ZSTACK_ROOT/bin" ] && [ -f "$ZSTACK_ROOT/lib/claude-bin.ts" ]; then
  ...
  ZSTACK_ROOT="${CODEX_HOME:-$HOME/.codex}/skills/zstack"
  if [ -n "$_OUTSIDE_REPO_ROOT" ] && [ -d "$_OUTSIDE_REPO_ROOT/.agents/skills/zstack/bin" ] && [ -f "$_OUTSIDE_REPO_ROOT/.agents/skills/zstack/lib/claude-bin.ts" ]; then
    ZSTACK_ROOT="$_OUTSIDE_REPO_ROOT/.agents/skills/zstack"
  fi
  ZSTACK_BIN="$ZSTACK_ROOT/bin"
fi
...
if { [ -n "${CLAUDECODE:-}" ] || [ "${ZSTACK_ACTIVE_HOST:-}" = claude ]; }; then
  ...
  if { [ -n "${CLAUDECODE:-}" ] || [ "${ZSTACK_ACTIVE_HOST:-}" = claude ]; } && { [ -n "${CODEX_THREAD_ID:-}" ] || [ -n "${CODEX_SANDBOX:-}" ] || [ "${ZSTACK_ACTIVE_HOST:-}" = codex ]; }; then
    ...
    echo 'Repair installed skills: run setup --host claude from your zstack checkout.' >&2
  fi
  exit 78
fi
); then
  if bun -e 'const {resolveClaudeCommand} = await import(process.argv[1]); process.exit(resolveClaudeCommand() ? 0 : 1)' "$ZSTACK_BIN/../lib/claude-bin.ts"; then echo 'CODEX_MODE: ready'; else echo 'CODEX_MODE: not_installed'; fi
else
...
_OUTSIDE_EXIT=0
"$ZSTACK_BIN/zstack-claude-code" --cwd "$_REPO_ROOT" --access none --timeout-ms 300000 <"$_OUTSIDE_INPUT" >"$_OUTSIDE_TMP/result.json" 2>"$_OUTSIDE_TMP/stderr" || _OUTSIDE_EXIT=$?
...
```bash
$ZSTACK_BIN/zstack-review-log '{"skill":"design-review-lite","host":"codex","outside_provider":"claude-code","outside_status":"OUTSIDE_STATUS","phase":"design-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"detector":D,"commit":"COMMIT","completed":COMPLETED,"converged":CONVERGED}' --finish DESIGN_START
```
...
_OUTSIDE_CFG=$("$ZSTACK_BIN/zstack-config" get codex_reviews 2>/dev/null || echo enabled)
if [ "$_OUTSIDE_CFG" = disabled ]; then
  echo 'CODEX_MODE: disabled'
elif ( # ZSTACK_ACTIVE_HOST names the harness, never the model.
if { [ -n "${CLAUDECODE:-}" ] || [ "${ZSTACK_ACTIVE_HOST:-}" = claude ]; }; then
...
DIFF_BASE=$(git merge-base origin/<base> HEAD) && git diff "$DIFF_BASE" >>"$_OUTSIDE_INPUT" || exit 1
_OUTSIDE_EXIT=0
"$ZSTACK_BIN/zstack-claude-code" --cwd "$_REPO_ROOT" --access none --timeout-ms 540000 <"$_OUTSIDE_INPUT" >"$_OUTSIDE_TMP/result.json" 2>"$_OUTSIDE_TMP/stderr" || _OUTSIDE_EXIT=$?
# Preserve session/usage/modelUsage from this JSON; multiple models have no invented primary.
cat "$_OUTSIDE_TMP/result.json" || { [ "$_OUTSIDE_EXIT" -ne 0 ] || _OUTSIDE_EXIT=1; }
if [ "$_OUTSIDE_EXIT" -eq 0 ]; then
  bun -e 'const r=await Bun.file(process.argv[1]).json(); if(r.status!=="completed" || typeof r.result!=="string" || !r.result.trim()) process.exit(1); await Bun.write(process.argv[2],r.result)' "$_OUTSIDE_TMP/result.json" "$_OUTSIDE_TMP/text" || _OUTSIDE_EXIT=1
...
Read `.agents/skills/zstack/review/TODOS-format.md` for the canonical format reference.

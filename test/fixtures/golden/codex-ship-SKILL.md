...
_UPD=$($ZSTACK_BIN/zstack-update-check 2>/dev/null || .agents/skills/zstack/bin/zstack-update-check 2>/dev/null || true)
...
_VENDORED="no"
if [ -d ".agents/skills/zstack" ] && [ ! -L ".agents/skills/zstack" ]; then
  if [ -f ".agents/skills/zstack/VERSION" ] || [ -d ".agents/skills/zstack/.git" ]; then
    _VENDORED="yes"
  fi
fi
...
> This project has zstack vendored in `.agents/skills/zstack/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .agents/skills/zstack/`
2. Run `echo '.agents/skills/zstack/' >> .gitignore`
...
1. Read `.agents/skills/zstack/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
2. **Read `.agents/skills/zstack/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
> You are classifying Greptile review comments for a /ship workflow. Read `.agents/skills/zstack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>

...
_UPD=$($ZSTACK_BIN/zstack-update-check 2>/dev/null || .factory/skills/zstack/bin/zstack-update-check 2>/dev/null || true)
...
_PROACTIVE=$($ZSTACK_BIN/zstack-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED=$([ -f ~/.zstack/.proactive-prompted ] && echo "yes" || echo "no")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$($ZSTACK_BIN/zstack-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <($ZSTACK_BIN/zstack-repo-mode 2>/dev/null) || true
REPO_MODE=${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_LAKE_SEEN=$([ -f ~/.zstack/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$($ZSTACK_BIN/zstack-config get telemetry 2>/dev/null || true)
_TEL_PROMPTED=$([ -f ~/.zstack/.telemetry-prompted ] && echo "yes" || echo "no")
_TEL_START=$(date +%s)
_SESSION_ID="$$-$(date +%s)"
echo "TELEMETRY: ${_TEL:-off}"
echo "TEL_PROMPTED: $_TEL_PROMPTED"
_EXPLAIN_LEVEL=$($ZSTACK_BIN/zstack-config get explain_level 2>/dev/null || echo "default")
if [ "$_EXPLAIN_LEVEL" != "default" ] && [ "$_EXPLAIN_LEVEL" != "terse" ]; then _EXPLAIN_LEVEL="default"; fi
echo "EXPLAIN_LEVEL: $_EXPLAIN_LEVEL"
_QUESTION_TUNING=$($ZSTACK_BIN/zstack-config get question_tuning 2>/dev/null || echo "false")
echo "QUESTION_TUNING: $_QUESTION_TUNING"
mkdir -p ~/.zstack/analytics
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ship","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.zstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
for _PF in $(find ~/.zstack/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "$ZSTACK_BIN/zstack-telemetry-log" ]; then
      $ZSTACK_BIN/zstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
    rm -f "$_PF" 2>/dev/null || true
  fi
  break
done
eval "$($ZSTACK_BIN/zstack-slug 2>/dev/null)" 2>/dev/null || true
_LEARN_FILE="${ZSTACK_HOME:-$HOME/.zstack}/projects/${SLUG:-unknown}/learnings.jsonl"
if [ -f "$_LEARN_FILE" ]; then
  _LEARN_COUNT=$(wc -l < "$_LEARN_FILE" 2>/dev/null | tr -d ' ')
  echo "LEARNINGS: $_LEARN_COUNT entries loaded"
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    $ZSTACK_BIN/zstack-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
$ZSTACK_BIN/zstack-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
if [ -f CLAUDE.md ] && grep -q "## Skill routing" CLAUDE.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
_ROUTING_DECLINED=$($ZSTACK_BIN/zstack-config get routing_declined 2>/dev/null || echo "false")
echo "HAS_ROUTING: $_HAS_ROUTING"
echo "ROUTING_DECLINED: $_ROUTING_DECLINED"
_VENDORED="no"
if [ -d ".factory/skills/zstack" ] && [ ! -L ".factory/skills/zstack" ]; then
  if [ -f ".factory/skills/zstack/VERSION" ] || [ -d ".factory/skills/zstack/.git" ]; then
    _VENDORED="yes"
  fi
fi
echo "VENDORED_ZSTACK: $_VENDORED"
echo "MODEL_OVERLAY: claude"
_CHECKPOINT_MODE=$($ZSTACK_BIN/zstack-config get checkpoint_mode 2>/dev/null || echo "explicit")
_CHECKPOINT_PUSH=$($ZSTACK_BIN/zstack-config get checkpoint_push 2>/dev/null || echo "false")
...
If output shows `UPGRADE_AVAILABLE <old> <new>`: read `$ZSTACK_ROOT/zstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined).
...
- Missing `$ZSTACK_ROOT/.feature-prompted-continuous-checkpoint`: AskUserQuestion for Continuous checkpoint auto-commits. If accepted, run `$ZSTACK_BIN/zstack-config set checkpoint_mode continuous`. Always touch marker.
- Missing `$ZSTACK_ROOT/.feature-prompted-model-overlay`: inform "Model overlays are active. MODEL_OVERLAY shows the patch." Always touch marker.
...
If B: run `$ZSTACK_BIN/zstack-config set explain_level terse`.
...
If `LAKE_INTRO` is `no`: say "zstack follows the **Boil the Lake** principle — do the complete thing when AI makes marginal cost near-zero. Read more: https://zeidslist.org/posts/boil-the-ocean" Offer to open:

```bash
open https://zeidslist.org/posts/boil-the-ocean
...
If A: run `$ZSTACK_BIN/zstack-config set telemetry community`
...
If B→A: run `$ZSTACK_BIN/zstack-config set telemetry anonymous`
If B→B: run `$ZSTACK_BIN/zstack-config set telemetry off`
...
If A: run `$ZSTACK_BIN/zstack-config set proactive true`
If B: run `$ZSTACK_BIN/zstack-config set proactive false`
...
If B: run `$ZSTACK_BIN/zstack-config set routing_declined true` and say they can re-enable with `zstack-config set routing_declined false`.

This only happens once per project. Skip if `HAS_ROUTING` is `yes` or `ROUTING_DECLINED` is `true`.

If `VENDORED_ZSTACK` is `yes`, warn once via AskUserQuestion unless `~/.zstack/.vendoring-warned-$SLUG` exists:

> This project has zstack vendored in `.factory/skills/zstack/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .factory/skills/zstack/`
2. Run `echo '.factory/skills/zstack/' >> .gitignore`
3. Run `$ZSTACK_BIN/zstack-team-init required` (or `optional`)
4. Run `git add .claude/ .gitignore CLAUDE.md && git commit -m "chore: migrate zstack from vendored to team mode"`
5. Tell the user: "Done. Each developer now runs: `cd $ZSTACK_ROOT && ./setup --team`"
...
_BRAIN_SYNC_BIN="$ZSTACK_BIN/zstack-brain-sync"
_BRAIN_CONFIG_BIN="$ZSTACK_BIN/zstack-config"
...
```bash
"$ZSTACK_BIN/zstack-brain-sync" --discover-new 2>/dev/null || true
"$ZSTACK_BIN/zstack-brain-sync" --once 2>/dev/null || true
```
...
ZStack voice: the author-shaped product and engineering judgment, compressed for runtime.
...
Before each AskUserQuestion, choose `question_id` from `scripts/question-registry.ts` or `{skill}-{slug}`, then run `$ZSTACK_BIN/zstack-question-preference --check "<id>"`. `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." `ASK_NORMALLY` means ask.
...
```bash
$ZSTACK_BIN/zstack-question-log '{"skill":"ship","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
$ZSTACK_ROOT/bin/zstack-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
...
if [ "$_TEL" != "off" ] && [ -x $ZSTACK_ROOT/bin/zstack-telemetry-log ]; then
  $ZSTACK_ROOT/bin/zstack-telemetry-log \
...
1. Read `.factory/skills/zstack/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
2. **Read `.factory/skills/zstack/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
> You are classifying Greptile review comments for a /ship workflow. Read `.factory/skills/zstack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
OLD_CFG=$($ZSTACK_ROOT/bin/zstack-config get codex_reviews 2>/dev/null || true)
...
4. **Write the bump** (FRESH, or an approved rebump):
   ```bash
   bun run $ZSTACK_ROOT/bin/zstack-version-bump write --version "$NEW_VERSION"
   ```
...
<Auto-detect: look for /spec archives matching this branch via:
  eval "$(${ctx.paths.binDir}/zstack-paths)"
  eval "$(${ctx.paths.binDir}/zstack-slug)"
...
Append to `~/.zstack/projects/$SLUG/$BRANCH-reviews.jsonl`:

```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> ~/.zstack/projects/$SLUG/$BRANCH-reviews.jsonl
```

  ...
  test('a conflicting inherited carve scope cannot suppress a planned case; direct Bun stays scoped', () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(fixtureRoot, `test/carve-section-loading-${skill}.test.ts`), `
          ...
          mock.module(${JSON.stringify(path.join(ROOT, 'test/helpers/auq-sdk-capture.ts'))}, () => ({
            ...
            captureSectionReads: async ({ skillName }) => {
              ...
              appendFileSync(${JSON.stringify(receipt)}, JSON.stringify({ skill: skillName, scope: process.env.ZSTACK_CARVE_SKILL }) + '\\n');
      ...
      const manifest = buildRunManifest({
        ...
        env: { EVALS_ALL: '1', ZSTACK_CARVE_SKILL: 'review' },
      });
      ...
      const env = {
        ...
        EVALS_PREFLIGHT_OK: '1', ZSTACK_CLAUDE_CLI_VERSION: 'free-fixture',
        ZSTACK_EVAL_DIR: path.join(dir, 'evals'), ZSTACK_CARVE_SKILL: 'browse',
      };
      ...
      const slice = JSON.parse(fs.readFileSync(path.join(env.ZSTACK_EVAL_DIR, 'slice-1.json'), 'utf8'));
      ...
      expect(env.ZSTACK_CARVE_SKILL).toBe('browse');
    } finally {

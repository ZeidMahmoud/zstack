 ...
 * {{BROWSE_FALLBACK}} (zstack's own headless browser when Aside is not
...
describe('Aside driver contract ({{ASIDE_SETUP}})', () => {
  ...
  test('cookbook uses only the verified Aside APIs', () => {
    ...
    for (const s of scripts) {
      ...
      expect(s.trim().endsWith('console.log("ZSTACK_STEP_OK");')).toBe(true);
    }
  });

  test('probe honors the ZSTACK_SKIP_ASIDE=1 opt-out and bounds the readiness call even on stock macOS', () => {
    // Opt-out short-circuits to NEEDS_ASIDE before `command -v aside` is even consulted.
    expect(setupProbe).toMatch(/if \[ "\$\{ZSTACK_SKIP_ASIDE:-\}" = "1" \] \|\| ! command -v aside >\/dev\/null 2>&1; then\n\s*echo "NEEDS_ASIDE"/);
  ...
  test('`aside exec` is never bare: the open-ended-reading recipe defines _aside_exec from the egress prelude', () => {
    ...
    expect(prelude).toContain('zstack-egress-lib.sh');
    expect(prelude).toContain('_zstack_egress_run open aside-agent aside.com aside-exec');
  ...
  test('the Aside contract stays Aside-only — `$B` lives in the fallback section', () => {
    ...
    expect(section).not.toContain('ZStack Browser');
    expect(section).not.toContain('handoff');
  });
});

describe('browser fallback ({{BROWSE_FALLBACK}})', () => {
  test('is registered and scoped to the non-READY probe outcomes or the TPA zstack-drive choice', () => {
    expect(RESOLVERS.BROWSE_FALLBACK).toBe(generateBrowseFallback);
    expect(fallback.startsWith("## Browser fallback: zstack's own headless browser")).toBe(true);
    ...
    expect(fallback).toContain("or when the user chose zstack's own browser in a Third-Party Web Actions question. Otherwise skip this section");
  });
...
describe('web research ({{ASIDE_RESEARCH}})', () => {
  ...
  test('embeds the SAME probe bash as BROWSER SETUP, byte-identical, and lets a skill reuse an earlier answer', () => {
    ...
    expect(researchProbe).toContain('ZSTACK_SKIP_ASIDE');
...
describe('browser consolidation tripwires', () => {
  test('every browsing skill carries the Aside contract followed by the $B fallback', () => {
    for (const skill of BROWSING_SKILLS) {
      ...
      const fb = md.indexOf("## Browser fallback: zstack's own headless browser");
      ...
      expect({ skill, fallbackCount: md.split("## Browser fallback: zstack's own").length - 1 }).toEqual({ skill, fallbackCount: 1 });

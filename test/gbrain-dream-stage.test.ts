/**
 * Tests for the dream (call-graph build) stage of bin/zstack-gbrain-sync.ts.
 *
...
describe("dream marker — concurrency guard", () => {
  const saved = process.env.ZSTACK_HOME;
  let tmp: string;

  afterEach(() => {
    if (tmp) rmSync(tmp, { recursive: true, force: true });
    if (saved === undefined) delete process.env.ZSTACK_HOME;
    else process.env.ZSTACK_HOME = saved;
  });

  function redirectHome(): void {
    tmp = mkdtempSync(join(tmpdir(), "gbrain-dream-marker-"));
    process.env.ZSTACK_HOME = tmp;
  }

...
test('the chain fixture retains the complete original UI/API scope', () => {
  ...
  expect(complete).not.toMatch(/Phase \d|ZSTACK REVIEW REPORT|AUTO-DECIDE|all findings resolved/i);
...
test('native sequencing config reaches the real CLI reader without changing shared state', () => {
  ...
  try {
    ...
    const result = spawnSync('bash', [resolve(root, 'bin/zstack-config'), 'get', 'codex_reviews'], {
    ...
    for (const marker of readdirSync(shared).filter(name => name === '.activated' ||
      ...
      expect(readFileSync(resolve(first.env.ZSTACK_HOME!, marker), 'utf8'))
        .toBe(readFileSync(resolve(shared, marker), 'utf8'));
    }
    first.cleanup();
    first.cleanup();
    expect(existsSync(first.env.ZSTACK_HOME!)).toBe(false);
    expect(existsSync(second.env.ZSTACK_HOME!)).toBe(true);
    expect(readFileSync(resolve(shared, 'config.yaml'), 'utf8')).toBe(before);
  } finally {
    first.cleanup();
    second.cleanup();
  }
  expect(existsSync(second.env.ZSTACK_HOME!)).toBe(false);
});
...
test('the UI/API chain requires all four native phases and registers its config dependency', () => {
  ...
  for (const file of ['test/helpers/plan-count-fixture.ts', 'test/plan-count-fixture.test.ts', 'bin/zstack-config']) {

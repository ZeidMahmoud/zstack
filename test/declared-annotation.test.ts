...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-annot-'));
  prevStateRoot = process.env.ZSTACK_STATE_ROOT;
  prevHome = process.env.ZSTACK_HOME;
  process.env.ZSTACK_STATE_ROOT = stateRoot;
  delete process.env.ZSTACK_HOME;
});

afterEach(() => {
  if (prevStateRoot !== undefined) process.env.ZSTACK_STATE_ROOT = prevStateRoot;
  else delete process.env.ZSTACK_STATE_ROOT;
  if (prevHome !== undefined) process.env.ZSTACK_HOME = prevHome;

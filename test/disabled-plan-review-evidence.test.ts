...
const TEMP = mkdtempSync(join(tmpdir(), 'zstack-disabled-plan-oracle-'));
...
function completed() {
  return {
    ...
    transcript: [
      ...
      { type: 'assistant', message: { content: [{ type: 'tool_use', id: 'preflight', name: 'Bash', input: { command: './runtime/bin/zstack-config get codex_reviews; echo "CODEX_MODE: $_CODEX_MODE"' } }] } },
...
describe('disabled outside-plan live oracle', () => {
  ...
  test('an unexecuted outside branch in a combined Bash block is not dispatch evidence', () => {
    const result = completed();
    result.transcript[1].message.content[0].input.command = './runtime/bin/zstack-config get codex_reviews; echo "CODEX_MODE: disabled"; if [ "$mode" != disabled ]; then codex exec --json -; fi';
    const evidence = oracle(result, '');
  ...
  test('generated preflight reads isolated disabled config without executing the available CLI', () => {
    ...
    const enable = spawnSync(join(ROOT, 'bin/zstack-config'), ['set', 'codex_reviews', 'enabled'], {
      cwd: repo, env: { ...process.env, ...fixture.env }, encoding: 'utf8', timeout: 5_000,
    });
  ...
  test('generated Codex plan and documentation log fences resolve runtime in fresh shells', () => {
    ...
    const state = join(repo, 'zstack-state');
    mkdirSync(join(codexHome, 'skills'), { recursive: true });
    symlinkSync(ROOT, join(codexHome, 'skills', 'zstack'), 'dir');
    const env = { ...process.env, HOME: home, CODEX_HOME: codexHome, ZSTACK_HOME: state, ZSTACK_STATE_ROOT: state,
      ZSTACK_PROJECT_SLUG: 'codex-disabled-fixture', ZSTACK_ROOT: '', ZSTACK_BIN: '', ZSTACK_ACTIVE_HOST: 'codex' };
    ...
    const config = (mode: string) => spawnSync(join(ROOT, 'bin/zstack-config'), ['set', 'codex_reviews', mode], {
      cwd: repo, env, encoding: 'utf8', timeout: 5_000,
    });
    const slug = spawnSync(join(ROOT, 'bin/zstack-slug'), [], { cwd: repo, env, encoding: 'utf8', timeout: 5_000 });
    expect(slug.status, slug.stderr).toBe(0);
    const branch = /^BRANCH=([a-zA-Z0-9._-]+)$/m.exec(slug.stdout)?.[1];
    expect(branch).toBeDefined();
    const logPath = join(state, 'projects', env.ZSTACK_PROJECT_SLUG, `${branch}-reviews.jsonl`);
    ...
    writeFileSync(join(brokenRuntime, 'bin/zstack-config'), '#!/bin/sh\nexit 19\n', { mode: 0o755 });
    const unexpectedWrite = join(repo, 'unexpected-review-write');
    writeFileSync(join(brokenRuntime, 'bin/zstack-review-log'), '#!/bin/sh\nprintf invoked > "$UNEXPECTED_REVIEW_WRITE"\n', { mode: 0o755 });
    for (const [skill, id, phase] of [
      ['zstack-plan-eng-review', 'codex-plan-review', 'plan-review'],
      ['zstack-document-release', 'codex-doc-review', 'documentation'],
    ]) {
      ...
      const seed = spawnSync(join(ROOT, 'bin/zstack-review-log'), [JSON.stringify(prior)], { cwd: repo, env, encoding: 'utf8', timeout: 5_000 });
      ...
      const failedRead = run(fence!, { ZSTACK_ROOT: brokenRuntime, UNEXPECTED_REVIEW_WRITE: unexpectedWrite });

 ...
 *     CWD-RELATIVE `.claude/skills/zstack/bin/zstack-slug` first, then
 *     `~/.claude/...` (~ = $HOME of the child). They do NOT honor
 *     ZSTACK_EVAL_DIR (only EvalCollector does). So the real isolation
 *     mechanism is: cwd = a temp HOME containing a fake zstack-slug that
 *     prints `SLUG=<fixture>`, routing every read to
 *     $HOME/.zstack/projects/<fixture>/evals — fully hermetic, and it
...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-eval-family-'));
  // Fake zstack-slug at the cwd-relative probe path so getProjectEvalDir()
  // deterministically resolves the project-scoped dir under the temp HOME.
  const slugBin = path.join(tmpHome, '.claude', 'skills', 'zstack', 'bin');
  fs.mkdirSync(slugBin, { recursive: true });
  fs.writeFileSync(path.join(slugBin, 'zstack-slug'), `#!/usr/bin/env bash\necho "SLUG=${SLUG}"\n`, { mode: 0o755 });
  evalDir = path.join(tmpHome, '.zstack', 'projects', SLUG, 'evals');
...
describe('eval:list CLI (scripts/eval-list.ts)', () => {
  ...
  test('lists finalized runs from the flat dir AND one level of shards/<slug>/', () => {
    ...
    // Reads route to the project-scoped dir resolved via the fake zstack-slug.

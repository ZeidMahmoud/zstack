...
const fakeEnv = {
  ...
  ZSTACK_CLAUDE_CLI_VERSION: 'free-fixture',
  ZSTACK_SDK_MAX_CONCURRENCY: '7',
};
  ...
  test('CLI rejects insufficient overlay walls before API preflight', () => {
    ...
    try {
      ...
      const result = spawnSync(process.execPath, [path.join(ROOT, 'scripts/test-paid-shards.ts'),
        ...
        env: { PATH: path.join(dir, 'no-executables'), HOME: dir, ZSTACK_EVAL_DIR: dir, ZSTACK_CLAUDE_CLI_VERSION: 'free-fixture',

 ...
 *   name ("seville-v3" vs "zstack") and aggregate order was filesystem-
...
describe('toYamlInlineScalar', () => {
  ...
  test("scalar containing '...' (YAML document-end marker) is quoted and round-trips", () => {
    const out = toYamlInlineScalar('Truncated lead ends with... more (zstack)');
...
describe('splitCatalogDescription', () => {
  test('extracts lead sentence + routing prose from simple multi-line description', () => {
    const desc =
      ...
      'Proactively suggest when the user is about to merge or land code changes. (zstack)';

    const parts = splitCatalogDescription(desc);

    expect(parts.lead).toBe('Pre-landing PR review.');
    expect(parts.hasZstackTag).toBe(true);
    ...
    // (zstack) tag stripped from routingProse
    expect(parts.routingProse).not.toContain('(zstack)');
  });
  ...
  test('REGRESSION (design-consultation v1.45.0.0): >200 char first sentence keeps routing', () => {
    ...
    const desc =
      ...
      'design system or DESIGN.md. (zstack)';
  ...
  test('REGRESSION: "TODOS.md backlog" style sentence keeps full lead + routing', () => {
    const desc =
      ...
      'Proactively suggest after a plan is approved. (zstack)';
    const parts = splitCatalogDescription(desc);
  ...
  test('>200 char first sentence WITH embedded periods still truncates with ellipsis and keeps routing', () => {
    ...
    const desc = firstSentence + ' Use when asked to resync the bridge. (zstack)';
    const parts = splitCatalogDescription(desc);
  ...
  test('idempotent: calling on already-trimmed output returns the same parts', () => {
    const desc = 'Already trimmed. (zstack)';
    ...
    expect(parts2.hasZstackTag).toBe(true);
    expect(parts2.routingProse).toBe('');
  });
});

describe('buildTrimmedDescription', () => {
  test('appends (zstack) when hasZstackTag is true', () => {
    ...
    expect(out).toBe('Some lead. (zstack)');
  });

  test('omits (zstack) when hasZstackTag is false', () => {
    const out = buildTrimmedDescription({
      lead: 'No tag.',
      routingProse: '',
      voiceLine: null,
  ...
  test('trims whitespace from lead', () => {
    ...
    expect(out).toBe('Lead with whitespace. (zstack)');
  });
});
...
description: |
  ...
  Proactively suggest when the user mentions examples. (zstack)
preamble-tier: 2
---
...
`;
  ...
  test('rewrites multi-line description into one-line + body section', () => {
    ...
    // Frontmatter description is now ONE line ending with (zstack). #1778: a
  ...
  test('returns null for already-short descriptions (no-op)', () => {
    const shortSkill = minimalSkill.replace(
      ...
      'description: Already short. (zstack)\n',
    );
  ...
  test('keeps the newline between description and next YAML field (no field collision)', () => {
    ...
    // `description: ... (zstack)preamble-tier:` with no newline.
    const result = applyCatalogTrim(minimalSkill, 'example');
    expect(result).not.toBeNull();
    expect(result!.content).not.toMatch(/\(zstack\)preamble-tier/);
    expect(result!.content).not.toMatch(/\(zstack\)allowed-tools/);
    // #1778: optional closing quote when the description was YAML-quoted.
    expect(result!.content).toMatch(/\(zstack\)"?\n[a-z-]+:/);
  });
...
describe('proactive-suggestions.json stays retired', () => {
  test('the generator no longer emits scripts/proactive-suggestions.json', () => {
    ...
    // tests (sorted keys, root keyed as "zstack", no timestamp fields) that
    // lived here before.
    const fs = require('fs');

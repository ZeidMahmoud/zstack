...
function temp(){const r=fs.mkdtempSync(path.join(os.tmpdir(),'zstack-autoplan-owned-state-test-'));cleanups.push(()=>fs.rmSync(r,{recursive:true,force:true}));return r;}
...
test('only the live constructor object owns its state; fields, copies and sibling state confer no authority',()=>{
  ...
  expect(ownedNativeReviewStateRoot(first,first.env)).toBe(first.env.ZSTACK_HOME);
  ...
  expect(ownedNativeReviewStateRoot(second,second.env)).toBe(second.env.ZSTACK_HOME);
});

for(const key of ['ZSTACK_HOME','ZSTACK_STATE_ROOT'])test(`${key} cannot redirect an owned artifact grant`,()=>{
  const s=state(),foreign=temp();
  for(const value of [undefined,'',foreign,s.env.ZSTACK_HOME+'/../'+path.basename(s.env.ZSTACK_HOME)])
...
for(const kind of ['replacement','symlink'])test.skipIf(process.platform==='win32'&&kind==='symlink')(`${kind} state directory cannot reuse constructor ownership`,()=>{
  const s=state(),root=s.env.ZSTACK_HOME!,moved=root+'.original',foreign=temp();
...
for(const kind of ['owned','legacy','unanswered','failed','foreign','changed','completed'])test(`captured pending Edit ${kind} keeps native acknowledgment and containment gates`,()=>{
  const s=state(),root=temp(),cwd=path.join(root,'zstack-autoplan-chain-S3FSi5'),config=path.join(root,'config');fs.mkdirSync(cwd);
  const oldRoot=captured.file.slice(0,captured.file.indexOf('/projects/'));
  const file=path.join(s.env.ZSTACK_HOME!,path.posix.relative(oldRoot,captured.file));
...
fs.writeFileSync(process.env.STATE_RESULT,JSON.stringify({pid:process.pid,args:process.argv.slice(2),home:process.env.ZSTACK_HOME,state:process.env.ZSTACK_STATE_ROOT}));
...
if(variant==='mismatch-home')env.ZSTACK_HOME=${JSON.stringify(cwd)};
if(variant==='mismatch-state')env.ZSTACK_STATE_ROOT=${JSON.stringify(cwd)};
...
try{
 ...
 if(session){
   ...
   r.metadata=JSON.parse(fs.readFileSync(r.file,'utf8'));r.caller=state.env.ZSTACK_HOME;

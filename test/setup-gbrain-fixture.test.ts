...
describe('setup-gbrain documented lock acquisition', () => {
  test.each(['fresh-home', 'existing-lock', 'parent-file', 'lock-file'])('%s retains the actual acquisition outcome', async scenario => {
    ...
    const parent = path.join(home, '.zstack');
...
describe('setup-gbrain owned Path 4 fixture', () => {
  for (const status of [401, 200] as const) {
    test(`real verifier/detector exercise ${status} with a fresh MCP state and explicit child token`, async () => {
      ...
      const ambient = { HOME: process.env.HOME, PATH: process.env.PATH, ZSTACK_HOME: process.env.ZSTACK_HOME, GBRAIN_MCP_TOKEN: process.env.GBRAIN_MCP_TOKEN };
      ...
      try {
        ...
        expect(skill.includes('~/.claude/skills/zstack/bin/')).toBe(false);
        expect(skill.includes(`${fixture.bin}'/zstack-gbrain-install`)).toBe(true);
        expect(skill.includes('Want symbol-aware code search')).toBe(true);
        const detectPath = path.join(fixture.bin, 'zstack-gbrain-detect');
        ...
        const verifier = path.join(fixture.bin, 'zstack-gbrain-mcp-verify');
        ...
        if (status === 200) {
          const installer = await command(path.join(fixture.bin, 'zstack-gbrain-install'), [], env);
        ...
        expect(calls.some((c) => c.command === 'zstack-gbrain-mcp-verify' && c.phase === 'start' && c.tokenPresent && c.tokenMatches && c.homeMatches && c.zstackHomeMatches)).toBe(true);
  ...
  test('SDK failure/exception evidence is unique, redacted, and retained before assertions and cleanup', async () => {
    ...
    try {
      for (const mode of ['assertion', 'max-turns', 'throw', 'success'] as const) {
        ...
        expect(evidence.configuration.homeMatches && evidence.configuration.zstackHomeMatches && evidence.configuration.ownedBinFirst).toBe(true);
...
describe('setup-gbrain local-PGLite fixture answers', () => {
  ...
  "question": "Also sync your zstack artifacts (CEO plans, designs, reports, retros) to a private git repo that gbrain can index across machines?",

...
// Eval result collector — accumulates test results, writes to ~/.zstack-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.zstack', 'test-transcripts');
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with zstack installed globally, the fallback path
    // ~/.claude/skills/zstack/browse/dist/browse exists, so we get READY.
...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a zstack skill. The session preamble detected _SESSIONS=4 (the user has 4 zstack windows open).
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    const statusLines = gitStatus.stdout.toString().trim().split('\n').filter(
      (l: string) => l.trim() && !l.includes('.prompt-tmp') && !l.includes('.zstack/') && !l.includes('qa-reports/'),
    );
...
`);
    ...
    projectDir = path.join(os.homedir(), '.zstack', 'projects', 'test-project');
  ...
  test('/plan-eng-review writes test-plan artifact to ~/.zstack/projects/', async () => {
...
describeIfSelected('zstack-upgrade E2E', ['zstack-upgrade-happy-path'], () => {
  let upgradeDir: string;
  let remoteDir: string;

  beforeAll(() => {
    upgradeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-e2e-upgrade-'));
    remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-remote-'));
    ...
    // Create mock zstack install directory (local-git type)
    const mockZstack = path.join(upgradeDir, '.claude', 'skills', 'zstack');
    fs.mkdirSync(mockZstack, { recursive: true });

    // Init as a git repo
    run('git', ['init'], mockZstack);
    run('git', ['config', 'user.email', 'test@test.com'], mockZstack);
    run('git', ['config', 'user.name', 'Test'], mockZstack);

    // Create bare remote
    run('git', ['init', '--bare'], remoteDir);
    run('git', ['remote', 'add', 'origin', remoteDir], mockZstack);

    // Write old version files
    fs.writeFileSync(path.join(mockZstack, 'VERSION'), '0.5.0\n');
    fs.writeFileSync(path.join(mockZstack, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    fs.writeFileSync(path.join(mockZstack, 'setup'),
      '#!/bin/bash\necho "Setup completed"\n', { mode: 0o755 });

    // Initial commit + push
    run('git', ['add', '.'], mockZstack);
    run('git', ['commit', '-m', 'initial'], mockZstack);
    run('git', ['push', '-u', 'origin', 'HEAD:main'], mockZstack);

    // Create new version (simulate upstream release)
    fs.writeFileSync(path.join(mockZstack, 'VERSION'), '0.6.0\n');
    fs.writeFileSync(path.join(mockZstack, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.6.0 — 2026-03-15\n\n- New feature: interactive design review\n- Fix: snapshot flag validation\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    run('git', ['add', '.'], mockZstack);
    run('git', ['commit', '-m', 'release 0.6.0'], mockZstack);
    run('git', ['push', 'origin', 'HEAD:main'], mockZstack);

    // Reset working copy back to old version
    run('git', ['reset', '--hard', 'HEAD~1'], mockZstack);

    // Copy zstack-upgrade skill
    fs.mkdirSync(path.join(upgradeDir, 'zstack-upgrade'), { recursive: true });
    fs.copyFileSync(
      path.join(ROOT, 'zstack-upgrade', 'SKILL.md'),
      path.join(upgradeDir, 'zstack-upgrade', 'SKILL.md'),
    );
  ...
  testIfSelected('zstack-upgrade-happy-path', async () => {
    const mockZstack = path.join(upgradeDir, '.claude', 'skills', 'zstack');
    const result = await runSkillTest({
      prompt: `Read zstack-upgrade/SKILL.md for the upgrade workflow.

You are running /zstack-upgrade standalone. The zstack installation is at ./.claude/skills/zstack (local-git type — it has a .git directory with an origin remote).
...
IMPORTANT: The install directory is at ./.claude/skills/zstack — use that exact path.`,
      workingDirectory: upgradeDir,
      maxTurns: 20,
      timeout: 180_000,
      testName: 'zstack-upgrade-happy-path',
      runId,
    });

    logCost('/zstack-upgrade happy path', result);

    // Check that the version was updated
    const versionAfter = fs.readFileSync(path.join(mockZstack, 'VERSION'), 'utf-8').trim();
    const output = result.output || '';
    ...
    recordE2E('/zstack-upgrade happy path', 'zstack-upgrade E2E', result, {

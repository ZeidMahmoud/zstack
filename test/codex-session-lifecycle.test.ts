...
describe('Codex subprocess lifecycle without API calls', () => {
  ...
  type: 'agent_message', text: 'No zstack review skill is installed here.',
} }));`, async ({ skillDir, pids }) => {
  ...
  test('captures full JSONL/stderr, including split UTF-8, and removes the temporary home', async () => {
    await withFakeCodex(`
const line = Buffer.from(JSON.stringify({ type: 'item.completed', item: { type: 'agent_message', text: 'zstack review: café' } }) + '\\n');
...
`, async ({ skillDir, tempHome, pids }) => {
      ...
      expect(result.output).toBe('zstack review: café');
  ...
  spawn(command, args, options) {
    ...
    queueMicrotask(() => {
      ...
      child.stdout.write(JSON.stringify({ type: 'item.completed', item: {
        type: 'agent_message', text: 'The zstack review found no issues in the current branch diff.',
      } }) + '\\n');

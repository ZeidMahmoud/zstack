...
describe('Y named component approach menu', () => {
  ...
  test('named components and reordered labels follow the actual recommendation position', () => {
    for (const subject of ['the payment webhook handler', 'this invoice lookup service', 'the renderWidget adapter']) {
      ...
      q.question = `Which implementation approach for ${subject}? <zstack-qid:plan-ceo-review-approach>`;
  ...
  test('setup, another decision, negated, quoted or compound instructions are not this menu', () => {
    for (const question of [
      ...
      const call = pending(recordedY as NativePlanQuestionCall);
      call.questions[0]!.question = question + ' <zstack-qid:plan-ceo-review-approach>';
      expect(pickCeoRecommendedApproach(fingerprint(call))).toBeNull();
    }
  });
...
describe('CEO pre-review approach recommendation', () => {
  ...
  test('paired approach grammar is function-agnostic and follows reordered options', () => {
    const call = pending(pairedRecorded as NativePlanQuestionCall);
    const q = call.questions[0]!;
    q.question = 'D3 — Which implementation approach for the renderWidget() tests? <zstack-qid:plan-ceo-review-impl-approach>';
  ...
  test.each([
    ['non-approach question', 'Should the renderWidget() tests be deleted? <zstack-qid:plan-ceo-review-impl-approach>'],
    ['negated question', 'Which implementation approach should the renderWidget() tests not use? <zstack-qid:plan-ceo-review-impl-approach>'],
    ['negated test subject', 'Which implementation approach for not testing renderWidget()? <zstack-qid:plan-ceo-review-impl-approach>'],
    ['wrong approach identity', 'Which implementation approach for the renderWidget() tests? <zstack-qid:plan-ceo-approach>'],
    ['extra action before question', 'Delete the tests. Which implementation approach for the renderWidget() tests? <zstack-qid:plan-ceo-review-impl-approach>'],
  ...
  test('recommendation follows actual option position and arbitrary approach content, never seed words', () => {
    for (const order of [[0, 1, 2], [1, 2, 0], [2, 0, 1]]) {
      ...
      q.question = 'D1 — Which implementation approach should this plan use? <zstack-qid:plan-ceo-approach>';
  ...
  test.each([
    ...
    ['missing qid', (c: NativePlanQuestionCall) => { c.questions[0]!.question = c.questions[0]!.question.replace('<zstack-qid:plan-ceo-approach>', ''); }],
    ['malformed extra qid', (c: NativePlanQuestionCall) => { c.questions[0]!.question += '<zstack-qid:broken'; }],
    ['duplicate qid', (c: NativePlanQuestionCall) => { c.questions[0]!.question += '<zstack-qid:plan-ceo-approach>'; }],
    ...
    ['non-approach question', (c: NativePlanQuestionCall) => { c.questions[0]!.question = 'Should we accept this security risk? <zstack-qid:plan-ceo-approach>'; }],

...
describe('empathy accuracy is setup, not approval of the quoted findings', () => {
  ...
  test('accuracy-only menus survive reordering, product names, headers and absent IDs', () => {
    for (const header of ['Empathy narrative', 'Empathy trace', 'Narrative']) {
      ...
      c.questions[0]!.options.reverse();
      mutateQuestion(c, q => q.replaceAll('EvalKit', 'AnotherSDK').replace('Python ML engineer', 'TypeScript backend developer').replace(/ <zstack-qid:[^>]+>/, ''));
...
describe('DevEx substantive finding coverage', () => {
  ...
  test('the observed mandatory confirmations alone contribute zero findings', () => {
    const confirmations = [
      'No design doc found. Run /office-hours first? <zstack-qid:plan-devex-review-office-hours-preflight>',
      'Who is your primary target developer? <zstack-qid:plan-devex-review-persona>',
      'Does the empathy narrative match reality? <zstack-qid:plan-devex-review-empathy-check>',
      // A concrete defect in a benchmark recap does not make the target
      // confirmation itself a resolution decision for that defect.
      'Remove the mandatory CI wait before first eval to reach the agreed benchmark. Which tier do you confirm? <zstack-qid:plan-devex-review-tthw-tier>',
      'What should the magical first-eval moment look like? <zstack-qid:plan-devex-review-magical-moment>',
      'How deep should this DX review go? <zstack-qid:plan-devex-review-mode>',
      'Confusion report reviewed. Which items should be addressed? <zstack-qid:plan-devex-review-confusion-report>',
      'Which onboarding setup should run next? <zstack-qid:future-setup-choice>',
    ];
  ...
  test('an unanswered issue tab cannot turn an administrative answer into coverage', () => {
    const admin = call('How deep should this DX review go? <zstack-qid:plan-devex-review-mode>');
  ...
  test.each([
    'Which files should I review? <zstack-qid:unknown-administrative-choice>',
...
describe('DevEx count review-mode selection', () => {
  const modeQuestion = 'D6 — How deep should this DX review go? <zstack-qid:plan-devex-review-mode>';
...
describe('native developer-trace accuracy confirmation', () => {
  ...
  test('accuracy labels cannot hide remedy choices or a substantive repair question', () => {
    for (const mutate of [
      ...
      (native: NativePlanQuestionCall) => { native.questions[0]!.question += ' <zstack-qid:plan-devex-example-fix>'; },
    ]) {
      const native = actual();
      mutate(native);
...
describe('T native documentation follow-up decisions', () => {
  ...
  for (const index of [6, 7]) {
    ...
    test(`follow-up ${index} cannot borrow an unselected remedy or a setup identity`, () => {
      ...
      for (const replacement of ['<zstack-qid:devex-mode>', '<zstack-qid:devex-next-steps>', '<zstack-qid:foreign>']) {
        const c = changeQuestion(calls()[index]!, s => s.replace(/<zstack-qid:[^>]+>/, replacement));
        expect(isDevexReviewIssue(fp(c))).toBe(false);
      }
      const duplicate = changeQuestion(calls()[index]!, s => s + ' <zstack-qid:devex-extra>');
  ...
  for (const index of [0, 1]) {
    ...
    test(`decision ${index + 1} cannot borrow issue words for setup or quoted examples`, () => {
      for (const transform of [
        ...
        (s: string) => s.replace(/<zstack-qid:[^>]+>/, '<zstack-qid:plan-devex-review-mode>'),
        (s: string) => s + ' <zstack-qid:another>',
...
describe('Z written migration guide as an additional accepted obligation', () => {
  ...
  test('setup, quoted, hypothetical and already satisfied claims confer no new acceptance', () => {
    for(const [from,to] of [
      ...
      ['<zstack-qid:plan-devex-review-todo2-migration-guide>','<zstack-qid:plan-devex-review-mode>'],
      ['<zstack-qid:plan-devex-review-todo2-migration-guide>','<zstack-qid:foreign>'],
    ...
    expect(isDevexReviewIssue(fp(change(call(),'<zstack-qid:plan-devex-review-todo2-migration-guide>','<zstack-qid:plan-devex-review-todo2-migration-guide> <zstack-qid:extra>')))).toBe(false);
  });

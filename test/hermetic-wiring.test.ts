...
describe('hermetic wiring tripwire', () => {
  ...
  test('feature prompt acknowledgements are seeded in ZSTACK_HOME everywhere', () => {
    const markers = [
    ...
    const sources: Array<[string, number]> = [
      ...
      ['.github/actions/register-zstack-skills/action.yml', 1],
    ];
    ...
    for (const rel of ['.github/actions/register-zstack-skills/action.yml']) {
      const src = read(rel);
      expect(src).not.toContain('$SKILLS_DIR/zstack/.feature-prompted-');
      for (const marker of markers) expect(src).toContain(`$HOME/.zstack/${marker}`);
    }
  });
  ...
  test('no test callsite passes the whole operator env as a RUNNER override', () => {
    // Overrides merge last by design (per-test ZSTACK_HOME etc.) — passing
    // process.env itself through that hole defeats the entire scrub. Scoped
    // to OUR runner calls: unit tests that spawnSync zstack bin scripts with
  ...
  test('skill seeding stays under runRoot and reads the live repo tree, never operator ~/.claude', () => {
    ...
    for (const entry of fs.readdirSync(skillsDir)) {
      ...
      // install the repo itself lives at ~/.claude/skills/zstack, so every

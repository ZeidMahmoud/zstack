/**
 * Project-slug consistency for zstack-repo-mode.
 *
 * These are end-to-end reproductions of #2212. Each case is a real local Git
 * workspace which predates its public GitHub origin: zstack-slug caches the
...
const REPO_MODE_BIN = path.join(ROOT, 'bin', 'zstack-repo-mode');
...
describe('zstack-repo-mode cached slug consistency (#2212)', () => {
  for (const workspace of publicWorkspaces) {
    test(`${workspace.origin} keeps its pre-origin project cache`, () => {
      ...
      try {
        ...
        git(['config', 'user.name', 'zstack regression test'], project, home);
        git(['config', 'user.email', 'test@zstack.dev'], project, home);

        // This workspace existed and used zstack before it adopted GitHub.
  ...
  test('still treats a workspace without origin as unknown', () => {
    ...
    try {
      ...
      expect(fs.existsSync(path.join(home, '.zstack', 'projects'))).toBe(false);
    } finally {
      fs.rmSync(home, { recursive: true, force: true });
  ...
  test('sanitizes unusual origin URLs via zstack-slug before mkdir', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'grepo-home-'));
    ...
    try {
      ...
      const projects = fs.readdirSync(path.join(home, '.zstack', 'projects'));

 ...
 *    SCRIPT_DIR into (zstack-learnings-log, zstack-question-log,
 *    zstack-telemetry-log, zstack-developer-profile). The scripts exited 1
 ...
 *
 * 2. zstack-developer-profile passed an MSYS-form ZSTACK_HOME (/c/Users/...)
...
const RUN_ROOT = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-hostile-'));
const HOSTILE = path.join(RUN_ROOT, "zstack o'brien test");
...
function runBin(bin: string, args: string[], env: Record<string, string> = {}) {
  ...
  const r = spawnSync('bash', [path.join(HOSTILE, 'bin', bin), ...args], {
    timeout: 30_000,
    encoding: 'utf-8',
    env: { ...process.env, ZSTACK_HOME: STATE, ZSTACK_STATE_ROOT: '', ...env },
    shell: false,
  });
...
describe('bin writers under a path containing an apostrophe', () => {
  test('zstack-learnings-log appends a row (not just exit 0)', () => {
    const r = runBin('zstack-learnings-log', [
      JSON.stringify({
  ...
  test('zstack-question-log gets past module resolution to its own validation', () => {
    ...
    const r = runBin('zstack-question-log', [
  ...
  test('zstack-developer-profile --derive resolves ZSTACK_HOME for Bun', () => {
    const r = runBin('zstack-developer-profile', ['--derive']);
    expect(r.stdout + r.stderr).not.toContain('ENOENT');
    expect(r.stdout).toContain('DERIVE: ok');
  });

  test('zstack-telemetry-log appends a row with the error message REDACTED (not just exit 0)', () => {
    // The bin's tier gate (`zstack-config get telemetry`, default off) exits 0
    // WITHOUT writing — enable the anonymous tier via the config file that
    // zstack-config resolves from ZSTACK_HOME (already set by runBin).
    ...
    const r = runBin(
      ...
      {
        // zstack-telemetry-log reads ZSTACK_STATE_DIR (not ZSTACK_HOME) for
        // its analytics dir; point both at the same isolated state tree.
        ZSTACK_STATE_DIR: STATE,
        // Keep the fire-and-forget zstack-telemetry-sync child inert: with no

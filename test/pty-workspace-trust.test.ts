...
describe('PTY temporary workspace trust', () => {
  test('temporary workspaces share owned registration without rewriting trust or losing plan artifacts', async () => {
    await withFixture(async ({ cwd, launch }) => {
      ...
      const first = await launch({ cwd: alias, env: { ANTHROPIC_API_KEY: 'test-effective-key-12345678901234567890', ZSTACK_HOME: path.join(cwd, 'state') } });
      ...
      expect(first.env.ZSTACK_HOME).toBe(path.join(cwd, 'state'));

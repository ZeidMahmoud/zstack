...
const PROBE = path.join(ROOT, 'bin/zstack-codex-probe');
...
function tempHome(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-codex-probe-home-'));
}

describe('zstack-codex-probe: auth probe', () => {
  test('CODEX_API_KEY set → AUTH_OK', () => {
    const home = tempHome();
    try {
      const r = runProbe({
        snippet: '_zstack_codex_auth_probe',
  ...
  test('${CODEX_HOME:-~/.codex}/auth.json exists → AUTH_OK', () => {
    ...
    try {
      ...
      const r = runProbe({ snippet: '_zstack_codex_auth_probe', home });
      expect(r.stdout.trim()).toBe('AUTH_OK');
      expect(r.status).toBe(0);
    } finally {
      fs.rmSync(home, { recursive: true, force: true });
  ...
  test('alternate $CODEX_HOME → checks the alternate path', () => {
    const home = tempHome();
    const altCodex = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-alt-codex-'));
    try {
...
function tempStubCodex(versionOutput: string, bool_command_fails = false): {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-codex-stub-'));
...
function runVersionCheck(versionOutput: string): string {
  ...
  try {
    const r = runProbe({
      snippet: '_zstack_codex_version_check',
...
describe('zstack-codex-probe: version check (anchored regex per Tension I)', () => {
  // Matches (should WARN)
...
describe('zstack-codex-probe: timeout wrapper + namespace hygiene', () => {
  test('bin/zstack-codex-probe is syntactically valid bash (bash -n)', () => {
  ...
  test('timeout wrapper executes command directly when neither binary present', () => {
    ...
    const r = runProbe({
      snippet: `_zstack_codex_timeout_wrapper 5 echo hello_world`,
  ...
  test('timeout wrapper resolves gtimeout preferentially when on PATH', () => {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-gto-stub-'));
    try {
      const stub = path.join(dir, 'gtimeout');
      fs.writeFileSync(stub, '#!/bin/bash\necho gtimeout_chosen_$1\n');
      fs.chmodSync(stub, 0o755);
      const r = runProbe({
        snippet: `_zstack_codex_timeout_wrapper 5 echo nope`,
  ...
  test('bash-native watchdog kills a hung command at the deadline (exit 124, no timeout binary)', () => {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-watchdog-'));
    try {
      ...
      const r = runProbe({
        snippet: `_zstack_codex_timeout_wrapper 1 sleep 30; echo "rc=$?"`,
        env: { PATH: dir },
      });
...
describe('zstack-codex-probe: telemetry event emission', () => {
  test('_zstack_codex_log_event writes jsonl when _TEL != off', () => {
    const home = tempHome();
    try {
      const r = runProbe({
        snippet: `_zstack_codex_log_event "codex_test_event" "42"; cat "$HOME/.zstack/analytics/skill-usage.jsonl"`,
        env: { _TEL: 'community' },
        home,
      });
  ...
  test('_zstack_codex_log_event skips write when _TEL = off', () => {
    const home = tempHome();
    try {
      runProbe({
        snippet: `_zstack_codex_log_event "codex_test_event" "99"`,
        env: { _TEL: 'off' },
        home,
      });
      const jsonl = path.join(home, '.zstack/analytics/skill-usage.jsonl');
  ...
  test('payload never contains prompt content, env values, or auth tokens (schema check)', () => {
    ...
    try {
      const r = runProbe({
        snippet: `_zstack_codex_log_event "codex_test_event" "1"; cat "$HOME/.zstack/analytics/skill-usage.jsonl"`,
        env: {
          _TEL: 'community',
...
describe('codex timeout wrapper: /review + /ship diff passes', () => {
  ...
  for (const relPath of WRAPPED_SITES) {
    ...
    test(`${relPath}: both diff-review Codex calls run under the wrapper`, () => {
      const wrapped =
        read().match(/_zstack_codex_timeout_wrapper\s+\d+\s+codex\s+(exec|review)\b/g) ?? [];
    ...
    test(`${relPath}: does not claim \`timeout\` is unavailable on macOS`, () => {
      // _zstack_codex_timeout_wrapper resolves gtimeout -> timeout -> unwrapped,
    ...
    test(`${relPath}: wrapper budget stays under the outer Bash gate`, () => {
      const budgets = [...read().matchAll(/_zstack_codex_timeout_wrapper\s+(\d+)\s+codex\b/g)].map(
        (m) => Number(m[1]) * 1000,
      );
  ...
  for (const relPath of ['codex tmpl union', 'codex rendered union'] as const) {
    ...
    test(`${relPath}: (a) every scoped codex review invocation pins sandbox_mode="read-only"`, () => {
      const invocations = read()
        .split('\n')
        .filter((l) => /_zstack_codex_timeout_wrapper\s+\d+\s+codex\s+review\b/.test(l));
    ...
    test(`${relPath}: (c) every Bash gate sits strictly above its section's wrapper budgets`, () => {
      ...
      // (`_zstack_codex_timeout_wrapper S codex`), every gate must be strictly
      ...
      for (const section of sections) {
        ...
        const wrappers = [...section.matchAll(/_zstack_codex_timeout_wrapper\s+(\d+)\s+codex\b/g)].map(
          (m) => Number(m[1]) * 1000,
        );
...
describe('codex broken-install detection (#2742)', () => {
  ...
  function shimHome(mode: 'enoent' | 'notexec' | 'timeout' | 'model400' | 'oksuspicious') {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-codex-shim-'));
  ...
  for (const [label, mode, needle] of cases) {
    test(`${label} is classified as a broken install, not a transient`, () => {
      ...
      try {
        const r = runProbe({
          snippet: '_zstack_codex_model_probe; echo "EXIT:$?"',
          home,
          env: { PATH: `${bin}:${process.env.PATH ?? ''}`, ZSTACK_HOME: home },
        });
  ...
  test('a broken install is never cached — a reinstall is picked up next probe', () => {
    ...
    try {
      runProbe({
        snippet: '_zstack_codex_model_probe >/dev/null 2>&1',
        home,
        env: { PATH: `${bin}:${process.env.PATH ?? ''}`, ZSTACK_HOME: home },
      });
  ...
  test('version check warns instead of returning silently when codex cannot report a version', () => {
    ...
    try {
      const r = runProbe({
        snippet: '_zstack_codex_version_check; echo "EXIT:$?"',
        home,
        env: { PATH: `${bin}:${process.env.PATH ?? ''}`, ZSTACK_HOME: home },
      });
  ...
  test('autoplan preflight (tmpl + rendered) captures the probe exit and routes 2 to broken-install', () => {
    for (const rel of ['autoplan/SKILL.md.tmpl', 'autoplan/SKILL.md']) {
      ...
      expect(src).toContain('_zstack_codex_model_probe; _CODEX_MP=$?');
      expect(src).toMatch(/_CODEX_MP" -eq 2/);
      expect(src).toContain('binary cannot run');
      expect(src).not.toContain('elif ! _zstack_codex_model_probe');
    }
  });
  ...
  test('the preflight resolver routes exit 2 to broken_install', () => {
    ...
    // The chain must capture the probe's code; `elif ! _zstack_codex_model_probe`

...
describe('Completed outside-review participation stays setup', () => {
  ...
  test('the existing outside-voices identity and comma labels also stay setup', () => {
    ...
    q.question = 'D4 — Want outside design voices before the detailed review? Codex evaluates the design; a Claude subagent reviews completeness. <zstack-qid:outside-voices-design>';
  ...
  test('incomplete, mismatched, mixed and substantive questions cannot be hidden as setup', () => {
    const mutations: Array<(call: NativePlanQuestionCall) => void> = [
      ...
      c => {c.questions[0]!.question = c.questions[0]!.question.replace(' <zstack-qid:', ' Also remove the account-owner authorization check from Export. <zstack-qid:');},
      c => {c.questions[0]!.question = c.questions[0]!.question.replace('plan-design-review-outside-voices', 'plan-design-review-auth');},
      c => {c.questions[0]!.question = 'D1 — Should the product require outside design voices for every customer? <zstack-qid:plan-design-review-outside-voices>';},
...
describe('Design count native review phases and completion handoff', () => {
  ...
  test('numbered pass identity cannot turn actual setup or unrelated questions into findings', () => {
    for (const [header, question] of [
      ['Learnings', 'D1 — Pass 1 (Information Architecture): enable cross-project learnings? <zstack-qid:cross-project-learnings>'],
      ['Focus', 'D2 — Pass 1 (Information Architecture): which review focus should come first? <zstack-qid:plan-design-pass1-focus>'],
      ['Scope', 'D2 — Pass 1 (Information Architecture): reduce scope or review every dimension? <zstack-qid:plan-design-pass1-scope>'],
      ['Outside voices', 'D2 — Pass 1 (Information Architecture): run outside reviewers? <zstack-qid:outside-voices-design>'],
      ['Info Arch', 'D2 — Review Pass 1 (Information Architecture) next? <zstack-qid:plan-design-pass1-save-prominence>'],
      ['Info Arch', 'D2 — Pass 2 (Interaction States): fix the missing pending state? <zstack-qid:plan-design-pass1-save-prominence>'],
      ['Info Arch', 'D2 — Pass 1 (Information Architecture): which planning workflow should run? <zstack-qid:unrelated-workflow>'],
    ]) {
  ...
  test('native pass readiness and continuation confirmations do not supply a finding', () => {
    for (const question of [
      'D2 — Pass 1 (Information Architecture): ready to start this pass? <zstack-qid:plan-design-pass1-start>',
      'D2 — Pass 1 (Information Architecture): continue with the review? <zstack-qid:plan-design-pass1-continue>',
    ]) {
      const call = numberedCalls()[1]!;
      const q = call.questions[0]!;
      q.question = question;
  ...
  test('setup and generic pass mentions are not positive finding evidence', () => {
    for (const question of [
      ...
      'Pass 7 found the design focus options. Which review focus do you prefer? <zstack-qid:plan-design-review-focus>',
    ]) {
  ...
  test('mixed or unknown calls keep their substantive count and default choice', () => {
    for (const mutate of [
      ...
      (c: NativePlanQuestionCall) => { c.questions[0]!.question = 'Should we fix this gap before running /plan-eng-review? <zstack-qid:plan-design-review-next-step>'; },
      (c: NativePlanQuestionCall) => { c.questions[0]!.question += ' <zstack-qid:settings-contrast-finding>'; },
    ]) {
      const call = handoff(); mutate(call);
  ...
  test('conditional or negative completion is a remaining finding, even with the known navigation labels', () => {
    for (const declaration of [
      ...
      const call = handoff(); const q = call.questions[0]!;
      q.question = `${declaration} What’s next? <zstack-qid:plan-design-review-next-step>`;
      call.answers = { [q.question]: q.options[0]!.label };
      expect(isDesignCompletionHandoff(fingerprint(call))).toBe(false);
      const active = fingerprint(pending(call));
      expect(pickDesignCountQuestion(active, active)).toBeNull();
  ...
  test('the existing outside opt-out keeps precedence under the composed caller policy', () => {
    const question = 'Want outside design voices before the detailed review? <zstack-qid:outside-voices-design>';
...
describe('scored native Design pass decisions', () => {
  ...
  test('named pass identity cannot turn phase readiness or a missing answer into a finding', () => {
    for (const mutate of [
      (call: NativePlanQuestionCall) => { call.questions[0]!.question = 'Pass 1 — Information Architecture: ready to begin? <zstack-qid:plan-design-review-ia-hierarchy>'; },
  ...
  test('readiness, setup, quoted examples and missing substantive choices cannot start review', () => {
    for (const mutate of [
      (call: NativePlanQuestionCall) => { call.questions[0]!.question = 'Pass 1 (Info Architecture) — 7/10. Ready to start this pass? <zstack-qid:plan-design-review-ia-scan-path>'; },
      (call: NativePlanQuestionCall) => { call.questions[0]!.question = 'Pass 1 (Info Architecture) — 7/10. The plan has no missing requirements. Should I begin this pass? <zstack-qid:plan-design-review-ia-scan-path>'; },
      (call: NativePlanQuestionCall) => { call.questions[0]!.question = 'Example: ' + call.questions[0]!.question; },

...
const changeLog = (f: any, modify: (log: any) => void) => {
  const request = use(f, 'zstack-question-log'), match = /'(\{.*\})'/.exec(request.input.command)!;
...
for (const [label, mutate] of Object.entries({
  ...
  'malformed native question tool': (f: any) => { f.tools.push({ ...use(f, 'zstack-question-log'), toolUseId: 'asked', name: 'mcp__ask__AskUserQuestion', input: {} }); },
  ...
  'no preamble': (f: any) => { const request = use(f, 'zstack-skill-start'); f.tools = f.tools.filter((e: any) => e.toolUseId !== request.toolUseId); },
  'preamble failed': (f: any) => { ack(f, use(f, 'zstack-skill-start')).isError = true; },
  'preamble missing ACK': (f: any) => { const request = use(f, 'zstack-skill-start'); f.tools = f.tools.filter((e: any) => e !== ack(f, request)); },
  'preamble duplicate': (f: any) => { f.tools.push({ ...use(f, 'zstack-skill-start') }); },
  'wrong preamble skill': (f: any) => { use(f, 'zstack-skill-start').input.command = use(f, 'zstack-skill-start').input.command.replace('--skill "plan-ceo-review"', '--skill "plan-eng-review"'); },
  'question tuning disabled': (f: any) => { const result = ack(f, use(f, 'zstack-skill-start')); result.content = result.content.replace('QUESTION_TUNING: true', 'QUESTION_TUNING: false'); },
  'ambiguous preamble session': (f: any) => { ack(f, use(f, 'zstack-skill-start')).content = 'SKILL_START_PROTO: 1\nQUESTION_TUNING: true\nSESSION_ID: duplicate\n' + ack(f, use(f, 'zstack-skill-start')).content; },
  'nonzero preference': (f: any) => { ack(f, use(f, 'zstack-question-preference')).content = 'AUTO_DECIDE\nEXIT: 1'; },
  'ASK preference': (f: any) => { ack(f, use(f, 'zstack-question-preference')).content = 'ASK\nEXIT: 0'; },
  'preference error': (f: any) => { ack(f, use(f, 'zstack-question-preference')).isError = true; },
  'wrong preference id': (f: any) => { use(f, 'zstack-question-preference').input.command = use(f, 'zstack-question-preference').input.command.replace('--check "plan-ceo-review-mode"', '--check "plan-ceo-review-other"'); },
  'no preference check': (f: any) => { const request = use(f, 'zstack-question-preference'); f.tools = f.tools.filter((e: any) => e.toolUseId !== request.toolUseId); },
  'unacknowledged log': (f: any) => { const request = use(f, 'zstack-question-log'); f.tools = f.tools.filter((e: any) => e !== ack(f, request)); },
  'failed log': (f: any) => { ack(f, use(f, 'zstack-question-log')).isError = true; },
  'fallback log result': (f: any) => { ack(f, use(f, 'zstack-question-log')).content = 'log unavailable (best-effort)'; },
  ...
  'later checked preference': (f: any) => { ack(f, use(f, 'zstack-question-preference')).timestamp = modeMessage(f).timestamp; },
  'mode before log ACK': (f: any) => { modeMessage(f).timestamp = use(f, 'zstack-question-log').timestamp; },
  'reversed log ACK': (f: any) => { ack(f, use(f, 'zstack-question-log')).timestamp = use(f, 'zstack-question-preference').timestamp; },
  'duplicate log ACK': (f: any) => { f.tools.push({ ...ack(f, use(f, 'zstack-question-log')) }); },
  'foreign log ACK': (f: any) => { ack(f, use(f, 'zstack-question-log')).sessionId = 'foreign'; },
...
for (const name of ['zstack-skill-start', 'zstack-question-preference', 'zstack-question-log']) {
...
for (const [label, text] of Object.entries({
  ...
  expect(result?.preambleToolUseId).toBe(use(f, 'zstack-skill-start').toolUseId);
...
test('the same command contracts also support direct literal invocations and quiet ACKs', () => {
  const f = clone();
  use(f, 'zstack-skill-start').input.command = '"$HOME/.claude/skills/zstack/bin/zstack-skill-start" --model claude --skill plan-ceo-review --parent-pid "$PPID"';
  use(f, 'zstack-question-preference').input.command = '~/.claude/skills/zstack/bin/zstack-question-preference --check plan-ceo-review-mode';
  ack(f, use(f, 'zstack-question-preference')).content = 'AUTO_DECIDE\n';
  use(f, 'zstack-question-log').input.command = use(f, 'zstack-question-log').input.command.split(' 2>/dev/null')[0];
  ack(f, use(f, 'zstack-question-log')).content = '';
...
for (const name of ['zstack-question-preference', 'zstack-question-log']) test(`${name} cannot borrow an earlier success after a contradictory current call`, () => {
  ...
  if (name === 'zstack-question-preference') result.content = 'ASK\nEXIT: 0';
...
test('a literal command cannot treat a physical newline as argument whitespace', () => {
  const f = clone();
  use(f, 'zstack-question-log').input.command = use(f, 'zstack-question-log').input.command.replace("zstack-question-log '", "zstack-question-log\n'");
  expect(decision(f)).toBeNull();
});
...
for (const fallback of ['"LOGGED"', '" LOGGED "', '"\\x4cOGGED"', '-e "\\x4cOGGED"'])
  test(`a failure branch cannot impersonate the question-log success marker: ${fallback}`, () => {
    const f = clone(), request = use(f, 'zstack-question-log');
...
const logUse=(f:any)=>f.tools.find((t:any)=>t.kind==='use'&&t.input?.command?.includes('zstack-question-log'));

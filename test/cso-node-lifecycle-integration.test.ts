...
const requested = process.env.ZSTACK_CSO_DOCKER_TESTS === '1' && process.env.ZSTACK_CSO_TEST_STACK === 'node';
const enabled = requested && Boolean(process.env.ZSTACK_CSO_TEST_IMAGE);
...
beforeAll(async () => {
  if (!requested) return;
  if (!enabled) throw new Error('Node lifecycle qualification requires ZSTACK_CSO_TEST_IMAGE; a requested cold gate cannot skip');
  const platform = process.arch === 'arm64' ? 'linux/arm64' : 'linux/amd64';
  if (process.env.ZSTACK_CSO_TEST_PLATFORM !== platform) throw new Error(`Node lifecycle qualification requires native ${platform}`);

  root = fs.mkdtempSync(path.join(os.tmpdir(), 'cso-node-lifecycle-'));
  repo = path.join(root, 'repo');
  state = path.join(root, 'state');
  process.env.ZSTACK_HOME = state;
  writeSource(repo);
  git('init', '-q');
  ...
  const watchdogPath = path.resolve(import.meta.dir, '../bin/zstack-cso-watchdog');
  ...
  cli = qualifiedNodeCli({
    image: process.env.ZSTACK_CSO_TEST_IMAGE!,
    versions: JSON.parse(process.env.ZSTACK_CSO_EXPECTED_VERSIONS || '{}'),
    watchdogPath,
    platform,
  });
});

 ...
 *
 * zstack never runs impeccable's engine in CI. What the detector wrapper and
 ...
 *   - the `detect --json` output shape over zstack's own planted-slop fixture,
...
describe('DOM_DUMP_SCRIPT contract', () => {
  ...
  test('works on a clone and applies the hygiene rules', () => {
    ...
    for (const rule of ['querySelectorAll("script")', 'querySelectorAll("textarea")', 'value.length > 32',
      ...
      'zstack-stripped', 'cloneLinks[i].remove()', 'querySelectorAll("style")', 'querySelectorAll("template, noscript")', 'name.indexOf("on") === 0', 'name === "srcdoc"', 'cleanCss(value)', '"xlink:href"']) {

...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-eval-list-'));
  const evalDir = path.join(tmpHome, '.zstack-dev', 'evals');
...
function runEvalList(...args: string[]): { stdout: string; stderr: string; status: number } {
  ...
  // cwd-relative .claude/skills/zstack/bin/zstack-slug, and on dev machines
  // with the self-symlink that probe succeeds, routing reads to an (empty)
  // project-scoped dir instead of the legacy ~/.zstack-dev/evals this test

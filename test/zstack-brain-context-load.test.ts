/**
 * Unit tests for bin/zstack-brain-context-load.ts (Lane C).
 *
...
import { parseSkillManifest } from "../lib/zstack-memory-helpers";

const SCRIPT = join(import.meta.dir, "..", "bin", "zstack-brain-context-load.ts");
...
function prependPath(binDir: string): Record<string, string> {
  ...
  return {
    ...
    ZSTACK_BRAIN_TIMEOUT_MS: "10000",
  };
}

describe("zstack-brain-context-load CLI", () => {
  it("--help exits 0 with usage", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: zstack-brain-context-load");
...
describe("zstack-brain-context-load — manifest dispatch", () => {
  ...
  it("uses skill manifest when --skill-file points at a valid SKILL.md", () => {
    const dir = mkdtempSync(join(tmpdir(), "zstack-bcl-"));
...
describe("zstack-brain-context-load — graceful gbrain absence", () => {
...
`,
  ...
  it("availability survives a 1ms query budget — detection is not subject to ZSTACK_BRAIN_TIMEOUT_MS", () => {
...
`,
    ...
    try {
      const env = { ...prependPath(binDir), ZSTACK_BRAIN_TIMEOUT_MS: "1" };
...
describe("zstack-brain-context-load — configured Design artifact root", () => {
  for (const storage of ["configured", "plugin", "default"]) it(`discovers real nested approvals in the ${storage} literal root`, () => {
    const dir = mkdtempSync(join(tmpdir(), "zstack-design-context-"));
    ...
    const expected = storage === "configured" ? configured : storage === "plugin" ? plugin : join(home, ".zstack");
    try {
...
`);
      const r = runScript(["--skill-file", skillFile, "--repo", "test-repo", "--explain"], {
        HOME: home, USERPROFILE: home, ZSTACK_HOME: storage === "configured" ? configured : "",
        CLAUDE_PLUGIN_DATA: storage === "default" ? "" : plugin,
        CLAUDE_PLUGIN_ROOT: storage === "default" ? "" : "/plugins/zstack",
      });
      expect(r.exitCode).toBe(0);
...
it("state-root resolver failure preserves diagnostics and continues a legacy filesystem query", () => {
  const dir = mkdtempSync(join(tmpdir(), "zstack-state-resolve-failure-"));
  try {
    const binDir = join(dir, "bin");
    mkdirSync(binDir);
...
`);
    const r = runScript(["--skill-file", skillFile, "--repo", "test-repo", "--explain"], {
      ...prependPath(binDir), HOME: dir, USERPROFILE: dir, ZSTACK_HOME: join(dir, "state"),
    });
    expect(r.exitCode).toBe(0);
...
describe("zstack-brain-context-load — CEO plan reader uses its writer's root", () => {
  for (const source of ["SKILL.md.tmpl", "SKILL.md"]) {
    for (const storage of ["configured", "plugin", "default"]) {
      it(`${source}: prior CEO plans use only the ${storage} root`, () => {
        const dir = mkdtempSync(join(tmpdir(), "zstack-ceo-context-"));
        const suffix = process.platform === "win32" ? "space $ [x]" : "space $ [x]*?";
        const home = join(dir, "operator " + suffix);
        const configured = join(dir, "configured " + suffix);
        const plugin = join(dir, "plugin " + suffix);
        ...
        try {
          ...
          if (storage !== "default") {
            const legacy = join(home, ".zstack", "projects", "test-repo", "ceo-plans");

/**
 * zstack-gbrain-detect — gbrain_mcp_mode + zstack_artifacts_remote tests.
 *
...
const DETECT_BIN = path.join(ROOT, 'bin', 'zstack-gbrain-detect');
...
describe('zstack_artifacts_remote', () => {
  ...
  test('migration-window fallback: reads ~/.zstack-brain-remote.txt if artifacts file is missing', () => {
    fs.writeFileSync(
      ...
      'git@github.com:garrytan/zstack-brain-garrytan.git\n'
    );
    expect(runDetect().json.zstack_artifacts_remote).toBe(
      'git@github.com:garrytan/zstack-brain-garrytan.git'
    );
  ...
  test('artifacts file wins over brain file when both exist', () => {
    ...
    expect(runDetect().json.zstack_artifacts_remote).toBe('https://github.com/x/new');
  });
...
describe('schema regression', () => {
  test('output JSON has all expected keys (sync-gbrain compat)', () => {
    ...
    expect(keys).toEqual([
      ...
      'gbrain_pooler_mode',
      'gbrain_version',
      'zstack_artifacts_remote',
      'zstack_brain_git',
      'zstack_brain_sync_mode',
    ]);
  });
});

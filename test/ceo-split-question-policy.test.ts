...
mock.module(${JSON.stringify(path.join(ROOT, 'test/helpers/claude-pty-runner.ts'))}, () => ({
  ...
  runPlanSkillCounting: async opts => {
    ...
    const directory = fs.readdirSync(${JSON.stringify(temp)}).find(name => name.startsWith('zstack-e2e-plan-ceo-split-overflow-'));
    expect(directory).toBeDefined();
    facts.directory = path.join(${JSON.stringify(temp)}, directory);
    const planPath = path.join(facts.directory, 'zstack-test-plan-ceo-split-overflow.md');
    expect(opts.followUpPrompt).toBe(FORCING_SPLIT_OVERFLOW_CEO.replaceAll('/tmp/zstack-test-plan-ceo-split-overflow.md', planPath));

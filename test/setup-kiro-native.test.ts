...
const helpers = [
  ...
  '_claude_entry_owned_strongly', '_zstack_link_target_abs', '_zstack_target_is_ours',
  '_zstack_generated_header', '_backup_skill_md', '_prune_stale_generated',
...
describe.skipIf(process.platform === 'win32')('native Kiro setup installation', () => {
  for (const windowsCopy of [0, 1]) {
    test(`installs native skills/sections and leaves live Codex output unchanged (copy=${windowsCopy})`, () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-kiro-native-'));
      ...
      try {
        ...
        for (const name of ['zstack', 'zstack-upgrade', 'zstack-office-hours', 'zstack-review', 'zstack-autoplan', 'zstack-codex', 'zstack-claude-code']) {
          put(path.join(generated, name, 'SKILL.md'), skill(name, 'kiro native; ./setup --host kiro'));
          if (name !== 'zstack') put(path.join(root, name.replace(/^zstack-/, ''), 'SKILL.md.tmpl'), `name: ${name}`);
        }
        put(path.join(generated, 'zstack-autoplan/sections/ceo.md'), `${banner}\nKiro native phase`);
        put(path.join(installed, 'zstack-autoplan/SKILL.md'), skill('zstack-autoplan', 'old Codex output'));
        put(path.join(installed, 'zstack-autoplan/sections/ceo.md'), `${banner}\nold Codex phase`);
        put(path.join(installed, 'zstack-autoplan/sections/notes.md'), 'user notes');
        put(path.join(installed, 'zstack-review/SKILL.md'), 'a foreign review skill');
        const codex = path.join(root, '.agents/skills/zstack-review/SKILL.md');
        ...
        const result = runBashScript([
          ...
          env: { ...process.env, HOME: home, SOURCE_ZSTACK_DIR: root, BUN_LOG: log }, timeout: 30_000,
        });
        ...
        for (const name of ['zstack-codex', 'zstack-claude-code', 'zstack-autoplan']) {
          expect(fs.readFileSync(path.join(installed, name, 'SKILL.md'), 'utf8')).toContain('kiro native');
        }
        expect(fs.readFileSync(path.join(installed, 'zstack/zstack-upgrade/SKILL.md'), 'utf8')).toContain('./setup --host kiro');
        expect(fs.readFileSync(path.join(installed, 'zstack/office-hours/SKILL.md'), 'utf8')).toContain('kiro native');
        expect(fs.readFileSync(path.join(installed, 'zstack-autoplan/sections/ceo.md'), 'utf8')).toContain('Kiro native phase');
        expect(fs.readFileSync(path.join(installed, 'zstack-autoplan/sections/notes.md'), 'utf8')).toBe('user notes');
        expect(fs.readFileSync(path.join(installed, 'zstack-review/SKILL.md'), 'utf8')).toBe('a foreign review skill');
        expect(fs.existsSync(path.join(installed, 'zstack/bin/tool'))).toBe(true);
        expect(fs.existsSync(path.join(installed, 'zstack/lib/helper.ts'))).toBe(true);

...
describeE2E('/setup-gbrain Path 4 — bad token STOPs cleanly', () => {
  test('AUTH classifier fires, no MCP registration, no CLAUDE.md mutation', async () => {
    ...
    const zstackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'setup-gbrain-bad-'));
    ...
    fs.writeFileSync(path.join(zstackHome, 'CLAUDE.md'), ORIGINAL_CLAUDE_MD);
    ...
    try {
      ...
      const finalClaudeMd = fs.readFileSync(path.join(zstackHome, 'CLAUDE.md'), 'utf-8');

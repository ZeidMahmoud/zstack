...
describe('selectTests', () => {
  ...
  test.each(['bin/zstack-paths', 'bin/zstack-slug', 'scripts/resolvers/design.ts'])(
    'Design artifact dependencies select their native consumers: %s', (file) => {
      ...
      if (file !== 'bin/zstack-slug') expect(result.selected).toContain('autoplan-chain-pty');
  ...
  test('skill-specific change selects only that skill and related tests', () => {
    ...
    // zeidmahmoud/askuserquestion-split-on-overflow: split-overflow periodic

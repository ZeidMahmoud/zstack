...
describe("/ship redaction wiring", () => {
  test("scans the PR body via the shared bin before create", () => {
    expect(TMPL).toContain("zstack-redact --from-file");
  ...
  test("edit path also scans before sending", () => {
    ...
    const scanAt = TMPL.indexOf('zstack-redact --from-file "$PR_BODY_FILE"');
  ...
  test("scans the title too", () => {
    expect(TMPL).toContain('printf \'%s\' "$NEW_TITLE" | ~/.claude/skills/zstack/bin/zstack-redact');

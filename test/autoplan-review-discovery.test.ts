...
function methodology(phase: string, entry: string, restore: string) {
  return spawnSync(process.execPath, [path.join(ROOT, 'bin/zstack-autoplan-snapshot.ts'), 'methodology', phase, entry, restore], {
...
function assertBundle(result: ReturnType<typeof methodology>, expected: string[]) {
  ...
  const created = spawnSync(process.execPath, [path.join(ROOT, 'bin/zstack-autoplan-snapshot.ts'), 'create', manifest.phase, active, manifest.restorePath, manifest.methodologyPath], {
...
beforeAll(() => {
  owned = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-autoplan-discovery-'));
...
describe('autoplan reads installed host methodology', () => {
  for (const host of ALL_HOST_CONFIGS) {
    ...
    test(`${host.name} reads its generated review files from ${host.name === 'claude' ? 'the canonical global path' : 'local and global paths'} in ${mode} installations`, () => {
      ...
      const entryName = host.name === 'claude' ? 'autoplan' : 'zstack-autoplan';
      const generatedEntry = path.join(generatedRoot, entryName, 'SKILL.md');
      const body = fs.readFileSync(generatedEntry, 'utf8');
      if (host.name === 'claude') {
        for (const review of REVIEWS) expect(body).toContain(`~/.claude/skills/zstack/${review}/SKILL.md`);
      } else {
        ...
        const refs = [...body.matchAll(/`(\.\.\/zstack-plan-[a-z-]+\/SKILL\.md)`/g)].map(match => match[1]!);
        expect([...new Set(refs)].sort()).toEqual(REVIEWS.map(name => `../zstack-${name}/SKILL.md`).sort());
        for (const review of REVIEWS) expect(body).not.toContain(`$ZSTACK_ROOT/${review}/SKILL.md`);
      ...
      for (const registry of roots) {
        ...
        for (const review of REVIEWS) {
          const reviewName = host.name === 'claude' ? review : `zstack-${review}`;
          ...
          const stale = path.join(registry, 'zstack', review, 'SKILL.md');
          fs.mkdirSync(path.dirname(stale), { recursive: true });
          fs.writeFileSync(stale, 'STALE FOREIGN HARNESS SKILL');
          const reference = host.name === 'claude' ? `../${review}/SKILL.md` : [...body.matchAll(/`(\.\.\/zstack-plan-[a-z-]+\/SKILL\.md)`/g)].find(match => match[1] === `../zstack-${review}/SKILL.md`)![1]!;
          ...
          const directive = phaseBody.split('\n').find(line => line.startsWith('Before dispatch, Read ')
            && (line.includes(`methodology ${phase} `) || line.includes(`/${review}/SKILL.md`) || line.includes(`/zstack-${review}/SKILL.md`)));
  ...
  test('methodology validation fails before publication; repeated valid loads keep prior bytes', () => {
    ...
    const reject = () => {
      ...
      expect(result.stderr).toContain('zstack-autoplan-snapshot:');

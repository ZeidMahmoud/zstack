 ...
 * (HOME / ZSTACK_HOME / PLAYWRIGHT_BROWSERS_PATH all redirected) with a
 ...
 *   - re-fetch runs from the zstack install root (repo-pinned playwright)
...
const MIGRATION = path.join(ROOT, 'zstack-upgrade', 'migrations', 'v1.65.0.0.sh');
...
function run(extraEnv: Record<string, string> = {}): {
  ...
  const r = spawnSync(MIGRATION, [], {
    env: {
      ...
      HOME: tmpHome,
      ZSTACK_HOME: path.join(tmpHome, '.zstack'),
...
const doneFile = () => path.join(tmpHome, '.zstack', '.migrations', 'v1.65.0.0.done');
...
describe('v1.65.0.0 migration — poisoned bundle removal', () => {
  test('poisoned revision dir removed WHOLE (markers included); clean bundle untouched; verified re-fetch → .done', () => {
    ...
    // ...from the zstack install root (repo-pinned playwright version), not

...
describe('setup: --help flag (#1133)', () => {
  ...
  test('./setup --help exits 0, prints usage, and does not run installer', () => {
    ...
    expect(res.stdout).toContain('zstack setup');
...
`);
    try {
      ...
      if (host === 'openclaw' || host === 'hermes') {
        expect(res.stdout.replaceAll('\\', '/')).toContain('/agents-digest/zstack-AGENTS.md');
      }
  ...
  printf 'preflight:%s:%s:%s:%s:%s:%s\\n' "$HOST" "$QUIET" "$SKILL_PREFIX" "$SKILL_PREFIX_FLAG" "$MODEL_OVERRIDE" "$MODEL_OVERRIDE_SET" > "$ZSTACK_TEST_SETUP_FLAG_LOG"
  return 97
}
`);
    try {
      const res = spawnSync('bash', [SETUP_SCRIPT, '--host', 'claude', '--host=codex', '--quiet', '--no-prefix', '--model', 'first', '--model=second'], {
        ...
        env: { ...process.env, BASH_ENV: startup.replaceAll('\\', '/'), ZSTACK_TEST_SETUP_FLAG_LOG: observed.replaceAll('\\', '/') },
      });
      expect(res.error).toBeUndefined();

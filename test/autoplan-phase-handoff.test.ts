...
import { initializePlan, prepareMethodology, createSnapshot, amendImplementation,
  extractImplementationPlan, checkPhaseImplementation } from '../bin/zstack-autoplan-snapshot';
...
function fixture() {
  const root = mkdtempSync(join(tmpdir(), 'zstack-autoplan-handoff-')); owned.push(root);
...
test('captured parent text and a following tool can share a response without ending the turn', () => {
  const root = mkdtempSync(join(tmpdir(), 'zstack-autoplan-continuation-')); owned.push(root);
...
test('the captured cf74 full readback did not publish a CEO report before the Design Read', () => {
  ...
  const nextPhaseUse = { toolUseId: 'toolu_01XvX1QbuKqv1xWjpdHsFLnj',
    ...
    input: { file_path: '/home/vercel-sandbox/zstack/autoplan/sections/design-phase.md' } };

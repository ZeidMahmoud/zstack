...
Browser: read BROWSER-SETUP.md in this directory and follow it exactly — it probes for Aside first and falls back to the zstack browse binary. If it falls back, the binary is at ${browseBin} (B="${browseBin}"). Do not look for any other browser. The target is LOCAL, so submitting its forms needs no consent question.

...
describeE2E('gbrain-sync privacy gate fires once via preamble', () => {
  test('zstack skill preamble fires the 3-option AskUserQuestion when gbrain is detected', async () => {
    ...
    // don't inherit them.
    const origZstackHome = process.env.ZSTACK_HOME;
    const origPath = process.env.PATH;
    process.env.ZSTACK_HOME = zstackHome;
    ...
    try {
      ...
      if (origZstackHome === undefined) delete process.env.ZSTACK_HOME;
      else process.env.ZSTACK_HOME = origZstackHome;

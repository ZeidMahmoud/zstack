 ...
 * on PATH), runs a cheap zstack skill through the Agent SDK, intercepts
...
describeE2E('gbrain-sync privacy gate fires once via preamble', () => {
  test('zstack skill preamble fires the 3-option AskUserQuestion when gbrain is detected', async () => {
    // Stage a fresh ZSTACK_HOME with artifacts_sync_mode_prompted=false.
    const zstackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-zstack-'));
    ...
    fs.writeFileSync(
      path.join(zstackHome, 'config.yaml'),
    ...
    // break auth). Ambient process.env.ZSTACK_HOME mutation does NOT work
    // here — hermetic-env scrubs ZSTACK_* and repoints ZSTACK_HOME at its
  ...
  test('privacy gate does NOT fire when artifacts_sync_mode_prompted is already true', async () => {
    ...
    const zstackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-off-'));
    ...
    // first test — ambient ZSTACK_HOME mutation is scrubbed by hermetic-env).
    const childEnv = {
      ZSTACK_HOME: zstackHome,
      HOME: tempHome,
      PATH: `${fakeBinDir}:${process.env.PATH ?? '/usr/bin:/bin:/opt/homebrew/bin'}`,

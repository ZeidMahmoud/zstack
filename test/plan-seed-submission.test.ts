...
for (const mode of ['unseeded-deadline', 'seeded-deadline', 'protocol-error']) test.skipIf(process.platform === 'win32')(`actual observation caller preserves preflight outcome: ${mode}`, async () => {
  ...
  const priorEval = { EVALS_RUN_ID: process.env.EVALS_RUN_ID, ZSTACK_EVAL_DIR: process.env.ZSTACK_EVAL_DIR };
  process.env.EVALS_RUN_ID = 'seed-timeout'; process.env.ZSTACK_EVAL_DIR = artifactRoot;
...
for (const entry of [
  ...
  { name: 'explicit inactive hint', inPlanMode: true, extraArgs: [], env: { ZSTACK_PLAN_MODE: 'inactive' }, expectedHint: 'inactive' },
  { name: 'explicit outside hint', inPlanMode: false, extraArgs: [], env: { ZSTACK_PLAN_MODE: 'active' }, expectedHint: 'active' },

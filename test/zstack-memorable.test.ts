/**
 * bin/zstack-memorable — the enable/disable/status CLI of the Memorable
 * recall bridge. Free tier; the vendor is a fake sh script that only logs
 * its argv (these verbs must never execute it).
 *
 * Isolation per test: HOME, ZSTACK_HOME/STATE_ROOT/STATE_DIR (config +
 * lock), ZSTACK_SETTINGS_FILE, and CLAUDE_CONFIG_DIR whose skills/zstack is
 * a symlink to this repo, so the canonical resolver finds THIS tree's hook
 * (and VERSION matches). ZSTACK_MEMORABLE_BIN names the fake.
 */
...
const BIN = path.join(ROOT, 'bin', 'zstack-memorable');
const CONFIG = path.join(ROOT, 'bin', 'zstack-config');
...
beforeEach(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-memorable-bin-'));
  const claude = path.join(home, '.claude');
  fs.mkdirSync(path.join(claude, 'skills'), { recursive: true });
  canonical = path.join(claude, 'skills', 'zstack');
  ...
  env = {
    ...
    ZSTACK_SETTINGS_FILE: settings,
    ZSTACK_HOME: path.join(home, '.zstack'),
    ZSTACK_STATE_ROOT: path.join(home, '.zstack'),
    ZSTACK_STATE_DIR: path.join(home, '.zstack'),
    ZSTACK_MEMORABLE_BIN: fake,
  };
});
...
describe('enable', () => {
  test('registers the CANONICAL hook path with timeout 5, sets the gate on, never runs the vendor, explains the hand-off', () => {
    ...
    expect(entries[0]._zstack_source).toBe('zstack-memorable');
    ...
    for (const s of ['registered', 'memorable_recall=on', 'zstack-egress list --sink memorable-recall', 'zstack-memorable disable',
      'within a few seconds', 'zstack-memorable status', 'memorable enable', 'memorable forget', 'HIGH-tier'] ) {
  ...
  test('refuses a mixed-version stable install (old hook without the .ts twin, different VERSION)', () => {
    ...
    for (const rel of ['bin/zstack-session-update', HOOK_REL]) fs.writeFileSync(path.join(canonical, rel), '#!/bin/sh\n', { mode: 0o755 });
    fs.copyFileSync(path.join(ROOT, 'bin', 'zstack-settings-hook'), path.join(canonical, 'bin', 'zstack-settings-hook'));
    fs.chmodSync(path.join(canonical, 'bin', 'zstack-settings-hook'), 0o755);
  ...
  test('refuses when the vendor CLI is absent; never installs anything', () => {
    const r = run(['enable'], { ZSTACK_MEMORABLE_BIN: path.join(home, 'nope') });
    expect(r.status).toBe(1);
  ...
  test('refuses on Windows (deferred whole, D21) without touching anything', () => {
    const r = run(['enable'], { ZSTACK_MEMORABLE_TEST_UNAME: 'MINGW64_NT-10.0' });
    expect(r.status).toBe(1);
  ...
  test('when recording consent fails, prior state is restored: a fresh registration is removed, a pre-existing one kept', () => {
    ...
    const ro = { ZSTACK_HOME: roState, ZSTACK_STATE_ROOT: roState, ZSTACK_STATE_DIR: roState };
...
describe('disable', () => {
  ...
  test('vendor CLI absent: still exit 0, gate off, says there is nothing of the vendor to revoke', () => {
    run(['enable']);
    const r = run(['disable'], { ZSTACK_MEMORABLE_BIN: path.join(home, 'nope') });
    expect(r.status).toBe(0);
...
describe('status (read-only)', () => {
  ...
  test('tag-stripped zstack registration, plain and bash-prefixed quoted: "registered by zstack"', () => {
    setGate('on');
    for (const cmd of [`${canonical}/${HOOK_REL}`, `bash "${canonical}/${HOOK_REL}"`]) {
      ...
      const r = run(['status']);
      expect(r.stdout).toContain('registered by zstack');
  ...
  test('tails recent hook errors and counts receipts for the sink', () => {
    fs.mkdirSync(env.ZSTACK_HOME, { recursive: true });
    fs.writeFileSync(path.join(env.ZSTACK_HOME, 'hook-errors.log'), '2026-09-08T00:00:00Z memorable-user-prompt-hook: vendor timeout\n');
    const r = run(['status']);
  ...
  test('vendor resolution precedence: ZSTACK_MEMORABLE_BIN > MEMORABLE_BIN > ~/.memorable/bin/memorable > PATH; an unresolvable override is an error', () => {
    ...
    const base = { ZSTACK_MEMORABLE_BIN: '', MEMORABLE_BIN: '', PATH: `${path.join(home, 'pathdir')}:${env.PATH}` };
    expect(run(['status'], { ...base, ZSTACK_MEMORABLE_BIN: a, MEMORABLE_BIN: b }).stdout).toContain(`available (${a})`);
    ...
    expect(run(['status'], { ...base, ZSTACK_MEMORABLE_BIN: path.join(home, 'missing') }).stdout).toContain('not found');
  });
});
...
describe('enable/disable failure paths (coverage audit)', () => {
  ...
  function mixedCanonical(version: string, settingsHookBody?: string) {
    ...
    fs.mkdirSync(path.join(canonical, 'hosts', 'claude', 'hooks'), { recursive: true });
    fs.mkdirSync(path.join(canonical, 'bin'), { recursive: true });
    fs.writeFileSync(path.join(canonical, 'bin', 'zstack-session-update'), '#!/bin/sh\n', { mode: 0o755 });
    fs.writeFileSync(path.join(canonical, HOOK_REL), '#!/bin/sh\n', { mode: 0o755 });
    fs.writeFileSync(path.join(canonical, `${HOOK_REL}.ts`), '// twin\n');
    if (settingsHookBody) fs.writeFileSync(path.join(canonical, 'bin', 'zstack-settings-hook'), settingsHookBody, { mode: 0o755 });
    else { fs.copyFileSync(path.join(ROOT, 'bin', 'zstack-settings-hook'), path.join(canonical, 'bin', 'zstack-settings-hook')); fs.chmodSync(path.join(canonical, 'bin', 'zstack-settings-hook'), 0o755); }
  ...
  test('enable refuses when BOTH zstack and the vendor are registered; disable then removes only zstack\'s entry', () => {
    writeSettings({ hooks: { UserPromptSubmit: [
  ...
  test('enable passes the hook manager\'s lock give-up (exit 5) through and leaves the gate untouched', () => {
    ...
    const r = run(['enable'], { ZSTACK_SETTINGS_LOCK_TIMEOUT_MS: '500' });
  ...
  test('disable surfaces a hook-manager lock give-up as exit 5 after flipping the gate off', () => {
    ...
    fs.mkdirSync(`${settings}.lock`, { recursive: true });
    fs.writeFileSync(path.join(`${settings}.lock`, 'owner'), 'another-live-process');
    const r = run(['disable'], { ZSTACK_SETTINGS_LOCK_TIMEOUT_MS: '500' });
    expect(r.status).toBe(5);
    expect(gate()).toBe('off');
    expect(r.stdout).toContain('consent: memorable_recall=off');
  ...
  test('a FRESH bridge lock held by another process makes enable exit 5 after the wait, lock left in place', () => {
    const lock = path.join(env.ZSTACK_HOME, 'locks', 'memorable-bridge.lock');
    ...
    const r = run(['enable']);
    expect(r.status).toBe(5);
    expect(r.stderr).toContain('another zstack-memorable is running');
  ...
  test('consent-write failure with a PRE-EXISTING registration keeps the registration and restores the prior gate', () => {
    ...
    fs.mkdirSync(path.join(env.ZSTACK_HOME, 'locks'), { recursive: true }); // lock stays takeable; only the consent write fails
    fs.chmodSync(path.join(env.ZSTACK_HOME, 'config.yaml'), 0o444);
    fs.chmodSync(env.ZSTACK_HOME, 0o555);
    const r = run(['enable']);
    fs.chmodSync(env.ZSTACK_HOME, 0o755);
    expect(r.status).toBe(1);
    expect(r.stderr).toContain('could not record consent');
  ...
  test('usage: no verb exits 1 with usage on stderr; -h exits 0 with usage on stdout', () => {
    ...
    expect(none.stderr).toContain('Usage: zstack-memorable');
    const help = run(['-h']);
    expect(help.status).toBe(0);
    expect(help.stdout).toContain('Usage: zstack-memorable');
  });

  test('status names the Windows deferral and counts real receipts for the sink', () => {
    expect(run(['status'], { ZSTACK_MEMORABLE_TEST_UNAME: 'MINGW64_NT-10.0' }).stdout).toContain('platform: Windows is not supported');
    for (let i = 0; i < 2; i++) {
      const w = spawnSync('bun', [path.join(ROOT, 'bin', 'zstack-egress-receipt'), 'write', '--sink', 'memorable-recall', '--host', 'local:/x/memorable', '--class', 'c', '--no-payload', '--consent', 'memorable_recall=on'], { env, encoding: 'utf8', timeout: 20_000 });
...
describe('lifecycle lock and static pins', () => {
  test('two concurrent enables serialise: one entry, gate on, both exit 0', async () => {
    ...
    expect(readSettings().hooks.UserPromptSubmit).toHaveLength(1);
    expect(gate()).toBe('on');
    expect(fs.existsSync(path.join(env.ZSTACK_HOME, 'locks', 'memorable-bridge.lock'))).toBe(false);
  }, 30_000);
  ...
  test('a stale lock that cannot be reclaimed (locks dir not writable) still reaches the 5 s give-up instead of spinning', () => {
    ...
    const locksDir = path.join(env.ZSTACK_HOME, 'locks');
  ...
  test('source pins: canonical-only command, Windows refusal, no vendor invocation, explicit-status style', () => {
    ...
    expect(src).toContain('CANONICAL_ZSTACK_ROOT');

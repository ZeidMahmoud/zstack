...
test('the actual complete boxed no-qid question binds without changing public text or grades', () => {
  ...
  expect(question.question).not.toContain('<zstack-qid:');
...
fs.writeFileSync(path.join(cwd,'observed.json'),JSON.stringify({args,config,pid:process.pid,headless:process.env.ZSTACK_HEADLESS}));
...
for(const item of cases){
 ...
 const leaked=fs.readdirSync(os.tmpdir()).filter(name=>!before.includes(name)&&/^(zstack-native-auq-|zstack-pending-question-)/.test(name));
...
const leftovers=fs.readdirSync(os.tmpdir()).filter(name=>/^(zstack-native-auq-|zstack-pending-question-)/.test(name));
...
`);
  try {
    const proc=Bun.spawn([process.execPath,worker],{stdout:'pipe',stderr:'pipe',env:{...process.env,
      BROWSE_TERMINAL_BINARY:fake,EVALS_HERMETIC:'1',ZSTACK_EVAL_DIR:path.join(dir,'artifacts'),
      ANTHROPIC_API_KEY:'fake-key',ANTHROPIC_AUTH_TOKEN:'',ANTHROPIC_BASE_URL:'http://127.0.0.1:1',
      ZSTACK_EVAL_MODEL_CAPTURE:'capture-env-model',EVALS_MODEL:'must-not-replace-capture-model',TMPDIR:temp}});

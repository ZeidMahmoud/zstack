...
`);
    try {
      const child = Bun.spawnSync([process.execPath, 'test', ...(scenario === 'retry' ? ['--retry', '1'] : []), script], {
        ...
        env: { ...process.env, EVALS: '1', EVALS_TIER: 'periodic', ZSTACK_EVAL_DIR: evalDir,
      ...
      if (scenario === 'bad-report') expect(output).toContain('ZSTACK REVIEW REPORT contract violation');

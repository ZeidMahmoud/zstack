...
describe('investigate freeze path resolution', () => {
  for (const rel of FILES) {
    ...
    test(`${rel} hook falls back to standalone zstack-freeze install`, () => {
      expect(content).toContain('${CLAUDE_SKILL_DIR}/../freeze/bin/check-freeze.sh');
      expect(content).toContain('${CLAUDE_SKILL_DIR}/../zstack-freeze/bin/check-freeze.sh');
    ...
    test(`${rel} scope lock availability check supports standalone install`, () => {
      ...
      expect(content).toContain('[ -x "$_FREEZE_SCRIPT" ] || _FREEZE_SCRIPT="${CLAUDE_SKILL_DIR}/../zstack-freeze/bin/check-freeze.sh"');

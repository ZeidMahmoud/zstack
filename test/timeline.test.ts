...
function runLog(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  ...
  try {
    const stdout = execSync(`${BIN}/zstack-timeline-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
function runRead(args: string = ''): string {
  ...
  try {
    return execSync(`${BIN}/zstack-timeline-read ${args}`, execOpts).trim();
  } catch {
    return '';
  }
}
...
function runReadArgs(args: string[] = []): string {
  ...
  try {
    return execFileSync(path.join(BIN, 'zstack-timeline-read'), args, execOpts).trim(); // timeout via execOpts
  } catch {
    return '';
  }
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-timeline-'));

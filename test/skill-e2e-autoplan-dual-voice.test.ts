...
import { initializePlan, prepareMethodology, createSnapshot } from '../bin/zstack-autoplan-snapshot';
...
import { buildSeedConfig, seedHermeticZstackHome, seedHermeticRuntimeView } from './helpers/hermetic-env';
import {
...
describeIfSelected('Autoplan dual-voice E2E', ['autoplan-dual-voice'], () => {
  ...
  const prepareAttempt = () => {
    ...
    stateDir = '';
    stateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-autoplan-dv-state-'));
    const configDir = path.join(stateDir, '.claude');
    const zstackHome = path.join(stateDir, 'zstack-home');
    const tempDir = path.join(stateDir, 'tmp');
    fs.mkdirSync(configDir);
    fs.mkdirSync(zstackHome);
    ...
    const runtime = path.join(configDir, 'skills', 'zstack');
    fs.mkdirSync(path.dirname(runtime), { recursive: true });
    seedHermeticRuntimeView(ROOT, runtime);
    attemptEnv = { HOME: stateDir, CLAUDE_CONFIG_DIR: configDir,
      ZSTACK_HOME: zstackHome, ZSTACK_STATE_ROOT: zstackHome,
...
Bindings:
- SNAPSHOT_TOOL: ${JSON.stringify(fs.realpathSync(path.join(runtime, 'bin/zstack-autoplan-snapshot.ts')))}

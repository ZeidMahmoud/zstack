...
function runProfile(): Record<string, string> {
  ...
  const stdout = execSync(`${BIN}/zstack-builder-profile`, execOpts).trim(); // timeout via execOpts
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-profile-'));
});
...
describe('zstack-builder-profile', () => {

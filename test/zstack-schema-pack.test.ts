/**
 * zstack-core@1.0.0 schema pack validation (T1).
 *
...
import {
  ZSTACK_CORE_SCHEMA_PACK,
  getSchemaPackMutationPayload,
  getSchemaPackTypeNames,
  getRetentionPolicy,
} from '../scripts/zstack-schema-pack';
import {
  ZSTACK_SCHEMA_PACK_NAME,
  ZSTACK_SCHEMA_PACK_VERSION,
} from '../scripts/brain-cache-spec';

describe('zstack-core schema pack', () => {
  test('identity matches brain-cache-spec constants', () => {
    expect(ZSTACK_CORE_SCHEMA_PACK.name).toBe(ZSTACK_SCHEMA_PACK_NAME);
    expect(ZSTACK_CORE_SCHEMA_PACK.version).toBe(ZSTACK_SCHEMA_PACK_VERSION);
  });

  test('declares exactly 8 page types (7 entities + zstack/take)', () => {
    expect(ZSTACK_CORE_SCHEMA_PACK.page_types.length).toBe(8);
  });
  ...
  test('all 7 brain-cache entities have a matching schema page type', () => {
    ...
    const required = [
      ...
      'zstack/product',
      'zstack/goal',
      'zstack/developer-persona',
      'zstack/brand',
      'zstack/competitive-intel',
      'zstack/skill-run',
    ];
  ...
  test('zstack/take exists with kind=bet supported (Phase 2 / E5)', () => {
    const take = ZSTACK_CORE_SCHEMA_PACK.page_types.find((t) => t.type === 'zstack/take');
  ...
  test('every page type has a required type + slug field', () => {
    for (const def of ZSTACK_CORE_SCHEMA_PACK.page_types) {
  ...
  test('skill-run is the only archive-after-90d type', () => {
    const archived = ZSTACK_CORE_SCHEMA_PACK.page_types
      .filter((t) => t.retention === 'archive-after-90d')
      .map((t) => t.type);
    expect(archived).toEqual(['zstack/skill-run']);
  });

  test('zstack/take is never-archive (calibration scorecard preservation)', () => {
    expect(getRetentionPolicy('zstack/take')).toBe('never-archive');
  });

  test('getRetentionPolicy throws on unknown type (defensive)', () => {
    expect(() => getRetentionPolicy('zstack/nonexistent')).toThrow();
  });

  test('link verbs declared on emits_links are also in pack.link_verbs', () => {
    const declared = new Set(ZSTACK_CORE_SCHEMA_PACK.link_verbs);
    for (const def of ZSTACK_CORE_SCHEMA_PACK.page_types) {
  ...
  test('link verbs only target declared zstack/ page types', () => {
  ...
  test('zstack/product has expected emits_links graph (product → goal/persona/brand/etc.)', () => {
    const product = ZSTACK_CORE_SCHEMA_PACK.page_types.find((t) => t.type === 'zstack/product')!;
    const verbs = (product.emits_links ?? []).map((l) => `${l.verb}:${l.target_type}`);
    expect(verbs).toContain('targets:zstack/goal');
    expect(verbs).toContain('observed_by:zstack/developer-persona');
    expect(verbs).toContain('has_brand:zstack/brand');
    expect(verbs).toContain('competes_with:zstack/competitive-intel');
  });

  test('zstack/goal has lifecycle status enum (active/resolved/expired/archived)', () => {
    const goal = ZSTACK_CORE_SCHEMA_PACK.page_types.find((t) => t.type === 'zstack/goal')!;
    const status = goal.fields.find((f) => f.name === 'status');
    expect(status?.values).toEqual(['active', 'resolved', 'expired', 'archived']);
  });

  test('zstack/skill-run records the bet count for calibration coverage', () => {
    const sr = ZSTACK_CORE_SCHEMA_PACK.page_types.find((t) => t.type === 'zstack/skill-run')!;
    const takesField = sr.fields.find((f) => f.name === 'takes_written');
    expect(takesField).toBeDefined();
    expect(takesField?.type).toBe('number');
  });

  test('zstack/user-profile is never-archive (cross-project, long-lived)', () => {
    expect(getRetentionPolicy('zstack/user-profile')).toBe('never-archive');
  });
});

...
const TEMP = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-outside-preflight-'));
...
function fixture() {
  ...
  const env = { ...process.env, HOME: home, CODEX_HOME: '', ZSTACK_HOME: path.join(home, '.zstack'),
    ZSTACK_ROOT: '', ZSTACK_BIN: '', ZSTACK_ACTIVE_HOST: 'codex', CODEX_THREAD_ID: 'fixture', CODEX_SANDBOX: '', CLAUDECODE: '',
    ZSTACK_CLAUDE_BIN: process.execPath, ZSTACK_CLAUDE_BIN_ARGS: JSON.stringify([fake]), FIXTURE_CAPTURE: capture };
  const install = (destination: string) => {
    fs.mkdirSync(path.dirname(destination), { recursive: true });
    fs.symlinkSync(ROOT, destination, 'dir');
    return destination;
  };
  const local = path.join(repo, '.agents/skills/zstack');
  const global = path.join(home, '.codex/skills/zstack');
...
describe('outside reviewer runtime discovery in fresh shells', () => {
  ...
  test('global installation and CODEX_HOME both work without a local installation', () => {
    ...
    const custom = f.install(path.join(codexHome, 'skills/zstack'));
  ...
  test('valid explicit runtime roots take priority and stale roots fall back locally', () => {
    ...
    expect(f.preflight({ ZSTACK_ROOT: explicit }).stdout).toContain(`RESOLVED_ROOT: ${explicit}`);
    expect(f.preflight({ ZSTACK_BIN: path.join(explicit, 'bin') }).stdout).toContain(`RESOLVED_ROOT: ${explicit}`);
    const result = f.preflight({ ZSTACK_ROOT: '/missing/zstack', ZSTACK_BIN: '/missing/zstack/bin' });
    expect(result.stdout).toContain('CODEX_MODE: ready');
    expect(result.stdout).toContain(`RESOLVED_ROOT: ${f.local}`);
  });
  ...
  test('missing CLI reports not_installed rather than an import-path failure', () => {
    const f = fixture();
    f.install(f.local);
    const result = f.preflight({ ZSTACK_CLAUDE_BIN: 'missing-zstack-claude-fixture-command' });
  ...
  test('master switch is read from the discovered runtime before outside dispatch', () => {
    const f = fixture();
    f.install(f.local);
    const config = spawnSync(path.join(ROOT, 'bin/zstack-config'), ['set', 'codex_reviews', 'disabled'], {

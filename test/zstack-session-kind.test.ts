/**
 * zstack-session-kind — classifies the session so skills know whether a human can
...
const BIN = path.resolve(__dirname, '..', 'bin', 'zstack-session-kind');
...
describe('zstack-session-kind', () => {
  test('OPENCLAW_SESSION → spawned (highest precedence)', () => {
    ...
    expect(kind({ OPENCLAW_SESSION: '1', ZSTACK_HEADLESS: '1', CONDUCTOR_PORT: '5' })).toBe('spawned');
  });

  test('ZSTACK_HEADLESS → headless', () => {
    expect(kind({ ZSTACK_HEADLESS: '1' })).toBe('headless');
  });
  ...
  test('ZSTACK_HEADLESS beats CONDUCTOR (explicit override wins)', () => {
    expect(kind({ ZSTACK_HEADLESS: '1', CONDUCTOR_PORT: '5' })).toBe('headless');
  });
  ...
  test('empty ZSTACK_HEADLESS is treated as unset (interactive)', () => {
    ...
    expect(kind({ ZSTACK_HEADLESS: '' })).toBe('interactive');
  });
});

describe('ZSTACK_SESSION_KIND explicit override (#2733)', () => {
  test('spawned wins over every ambient marker (step 0, explicit beats ambient)', () => {
    ...
    expect(kind({ ZSTACK_SESSION_KIND: 'spawned' })).toBe('spawned');
    expect(kind({ ZSTACK_SESSION_KIND: 'spawned', CONDUCTOR_PORT: '5' })).toBe('spawned');
    expect(kind({ ZSTACK_SESSION_KIND: 'spawned', CONDUCTOR_WORKSPACE_PATH: '/x', CI: '1' })).toBe('spawned');
    expect(kind({ ZSTACK_SESSION_KIND: 'spawned', ZSTACK_HEADLESS: '1' })).toBe('spawned');
    expect(kind({ ZSTACK_SESSION_KIND: 'spawned', CLAUDE_CODE_ENTRYPOINT: 'cli' })).toBe('spawned');
  });

  test('only "spawned" is honored — reserved values fall through to detection', () => {
    // Deliberately narrow: "headless" already has ZSTACK_HEADLESS, and letting
    // an env var force "interactive" over CI markers would be a footgun.
    expect(kind({ ZSTACK_SESSION_KIND: 'headless' })).toBe('interactive');
    expect(kind({ ZSTACK_SESSION_KIND: 'headless', OPENCLAW_SESSION: '1' })).toBe('spawned');
    expect(kind({ ZSTACK_SESSION_KIND: 'interactive', CI: '1' })).toBe('headless');
  });

  test('invalid values are ignored (case-sensitive)', () => {
    expect(kind({ ZSTACK_SESSION_KIND: 'bogus' })).toBe('interactive');
    expect(kind({ ZSTACK_SESSION_KIND: 'bogus', CI: '1' })).toBe('headless');
    expect(kind({ ZSTACK_SESSION_KIND: 'SPAWNED' })).toBe('interactive');
  });

  test('empty ZSTACK_SESSION_KIND is treated as unset', () => {
    expect(kind({ ZSTACK_SESSION_KIND: '' })).toBe('interactive');
    expect(kind({ ZSTACK_SESSION_KIND: '', OPENCLAW_SESSION: '1' })).toBe('spawned');
  });
});

 ...
 * project cwd (the file lives only under the zstack install root). The
...
describe('question-tuning registry path is absolute (#2489)', () => {
  test('claude host renders the installed registry path', () => {
    ...
    expect(out).toContain('`~/.claude/skills/zstack/scripts/question-registry.ts`');
  });

  test('env-var host renders $ZSTACK_ROOT-anchored registry path', () => {
    const out = generateQuestionTuning(makeCtx('codex'));
    expect(out).toContain('`$ZSTACK_ROOT/scripts/question-registry.ts`');
  });
  ...
  test('question identity covers prose and ad hoc IDs without enabling disabled tuning', () => {
    for (const host of ['claude', 'codex'] as const) {
      ...
      expect(out).toContain('`<zstack-qid:{question_id}>` once in the question text itself');
  ...
  test('rendered SKILL.md carries the absolute path', () => {
    ...
    expect(rendered).toContain('~/.claude/skills/zstack/scripts/question-registry.ts');

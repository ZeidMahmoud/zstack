...
describeIfSelected('Learnings E2E', ['learnings-show'], () => {
  ...
  beforeAll(() => {
    ...
    for (const script of ['zstack-learnings-search', 'zstack-learnings-log', 'zstack-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }

    // Seed learnings JSONL — slug must match what zstack-slug computes.
    // With no git remote, zstack-slug falls back to basename(workDir).
...
- Replace any references to ~/.claude/skills/zstack/bin/zstack-slug with ./bin/zstack-slug.

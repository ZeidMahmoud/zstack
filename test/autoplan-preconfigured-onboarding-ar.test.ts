...
function fixture(plan = original) {
  const temp = mkdtempSync(join(tmpdir(), 'zstack-chain-onboarding-'));
  ...
  const env = { PATH: process.env.PATH!, HOME: home, ZSTACK_HOME: state, ZSTACK_STATE_ROOT: state };
  const start = () => execFileSync(join(root, 'bin/zstack-skill-start'), ['--skill', 'autoplan'],
...
test('real skill-start and canonical discovery see the configured chain prerequisites', () => {
  ...
  try {
    ...
    expect(before).toContain('ZSTACK_INSTRUCTION_BEGIN: routing-injection ');
    ...
    expect(after).not.toContain('ZSTACK_INSTRUCTION_BEGIN: routing-injection ');
...
test('brief copies only existing context and contracts; all implementation work stays unreviewed', () => {
  ...
  try {
    ...
    expect(brief).not.toMatch(/## (?:UI Scope|Backend|Out of scope)|Phase \d|ZSTACK REVIEW REPORT|AUTO-DECIDE|all findings resolved/);
    ...
    expect(existsSync(join(f.home, '.zstack'))).toBe(false);
  } finally { f.cleanup(); }
});

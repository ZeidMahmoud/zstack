...
// #1974: zstack-update-check runs under set -e, and its "up to date" signal
...
const SCRIPT = path.join(ROOT, 'bin', 'zstack-update-check');
...
describe('zstack-update-check crash sentinel (#1974)', () => {
  test('a mid-script crash emits CHECK_FAILED instead of silent up-to-date', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-upd-'));
    try {
      ...
      const r = run({ ZSTACK_STATE_DIR: asFile, ZSTACK_REMOTE_URL: 'file:///dev/null' });
  ...
  test('healthy up-to-date path stays silent (no sentinel noise)', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-upd-'));
    try {
      const r = run({ ZSTACK_STATE_DIR: path.join(dir, 'state'), ZSTACK_REMOTE_URL: 'file:///dev/null' });
      expect(r.status).toBe(0);

/**
 * No module-scope ZSTACK_HOME assignment in any test file.
 *
 ...
 * `process.env.ZSTACK_HOME = ...` leaks into every sibling file in the
...
describe('ZSTACK_HOME module-scope tripwire', () => {
  test('no test file assigns process.env.ZSTACK_HOME at module scope', () => {
    ...
    for (const rel of files) {
      ...
      lines.forEach((line, i) => {
        if (/^(?:process\.env\.ZSTACK_HOME|process\.env\.ZSTACK_STATE_ROOT)\s*=[^=]/.test(line)) {

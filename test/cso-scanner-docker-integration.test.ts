...
const requested = process.env.ZSTACK_CSO_SCANNER_DOCKER_TESTS === '1';
...
beforeAll(async () => {
  if (!requested) return;
  const file = process.env.ZSTACK_CSO_SCANNER_PROFILE;
  if (!file) throw new Error('Scanner Docker qualification requires ZSTACK_CSO_SCANNER_PROFILE');
  ...
  if (!process.env.ZSTACK_CSO_SCANNER_VERSION_HASH) throw new Error('Scanner Docker qualification requires ZSTACK_CSO_SCANNER_VERSION_HASH');
  ...
  process.env.ZSTACK_HOME = path.join(root, 'state');
...
suite('qualified CSO scanner image', () => {
  test('runs the exact adapter through DockerGroup with network-none inputs and bounded normalization', async () => {
    ...
    try {
      ...
      fs.writeFileSync(path.resolve(process.env.ZSTACK_CSO_SCANNER_VERSION_HASH!), `${profile.versionOutputSha256}\n`, { flag: 'wx', mode: 0o600 });

 ...
 * `.gbrain-source` to zstack's own `.gitignore`. Without the consumer-side
...
import { ensureGbrainSourceGitignored } from "../bin/zstack-gbrain-sync";

describe("ensureGbrainSourceGitignored", () => {
  let root: string;

  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "zstack-gbrain-gitignore-"));
  });

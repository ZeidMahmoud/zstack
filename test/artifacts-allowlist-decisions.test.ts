...
const INIT = fs.readFileSync(path.join(ROOT, "bin", "zstack-artifacts-init"), "utf-8");

/** Pull a quoted heredoc body out of zstack-artifacts-init by destination. */
function heredoc(target: string): string {
  const re = new RegExp(`cat > "${target}" <<'EOF'\\n([\\s\\S]*?)\\nEOF\\n`);
  const m = INIT.match(re);
  if (!m) throw new Error(`heredoc for ${target} not found in zstack-artifacts-init`);
  return m[1];
}
...
/**
 * zstack-decision-log:40 enqueues projects/<slug>/decisions.jsonl after EVERY write,
 ...
 *
 * Source-level rather than end-to-end: zstack-artifacts-init.test.ts drives the real
...
describe("the artifacts allowlist covers the decision store", () => {
  ...
  test("decisions.* are class artifact, so they sync in artifacts-only mode too", () => {
    const map = JSON.parse(heredoc("\\$ZSTACK_HOME/\\.brain-privacy-map\\.json"));
    for (const p of DECISION_PATHS) {
  ...
  test("the allowlist still ends with the user-additions marker", () => {
    ...
    // overwritten the next time zstack-artifacts-init runs.

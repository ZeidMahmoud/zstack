...
describeIfSelected('Ship workflow E2E', ['ship-local-workflow'], () => {
  ...
  beforeAll(() => {
    shipRemoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-ship-remote-'));
    shipWorkDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-ship-work-'));
...
describeIfSelected('zstack-upgrade E2E', ['zstack-upgrade-happy-path'], () => {
  ...
  testConcurrentIfSelected('zstack-upgrade-happy-path', async () => {
    const mockZstack = path.join(upgradeDir, '.claude', 'skills', 'zstack');
    const result = await runSkillTest({
...
IMPORTANT: The install directory is at ./.claude/skills/zstack — use that exact path.`,
    ...
    recordE2E(evalCollector, '/zstack-upgrade happy path', 'zstack-upgrade E2E', result, {

...
beforeEach(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-egress-'));
});
...
describe('egress receipt library', () => {
  ...
  test('WARN-at-size: >25MB ledger emits one self-explanatory stderr warning per process', () => {
    ...
    expect(warnings[0]).toContain('zstack-egress list');
...
describe('zstack-egress-receipt shell bridge', () => {
  const bin = path.join(ROOT, 'bin', 'zstack-egress-receipt');
  ...
  test('write hashes the exact payload file, prints the receipt id; outcome joins', () => {
    ...
    const write = spawnSync(bin, ['write', '--sink', 'telemetry-sync', '--host', '127.0.0.1:8399',
      ...
      { encoding: 'utf-8', timeout: 30_000, env: { ...process.env, ZSTACK_HOME: home } });

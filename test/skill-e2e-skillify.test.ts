...
Do NOT halt for clarification.`,
      ...
      env: {
        ...
        HOME: workDir,  // /skillify writes to $HOME/.zstack/browser-skills/
      },
      maxTurns: 40,
    ...
    // The skill should land in $HOME/.zstack/browser-skills/<name>/
    const skillsRoot = path.join(workDir, '.zstack', 'browser-skills');
...
write any files.`,
    ...
    const skillsRoot = path.join(workDir, '.zstack', 'browser-skills');
    const stagingRoot = path.join(workDir, '.zstack', '.tmp');

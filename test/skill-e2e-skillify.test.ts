 ...
 *      Skill written to ~/.zstack/browser-skills/<name>/ with full
 ...
 *   - A scoped ZSTACK_HOME under the workdir so on-disk artifacts are
...
function setupSkillifyWorkdir(suffix: string, installSkills: string[] = ['scrape', 'skillify']): Workdir {
  ...
  for (const script of [
    'zstack-timeline-log', 'zstack-slug', 'zstack-config',
    'zstack-update-check', 'zstack-repo-mode',
 ...
 * has to live under <workdir>/.zstack/browser-skills/ for the project-tier
 * lookup to find it (zstack's bundled tier resolves from the install dir,
...
function installBundledHackernewsSkill(workDir: string) {
  ...
  const dst = path.join(workDir, '.zstack', 'browser-skills', 'hackernews-frontpage');
  copyDirSync(src, dst);
}
...
describeIfSelected('Browser-skills Phase 2a E2E (/scrape + /skillify)', [
  ...
  testConcurrentIfSelected('scrape-match-path', async () => {
    const { workDir, zstackHome } = setupSkillifyWorkdir('match', ['scrape']);
...
Do NOT enter the prototype phase. Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('scrape-prototype-path', async () => {
    const { workDir, zstackHome } = setupSkillifyWorkdir('prototype', ['scrape']);
...
Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('skillify-happy-path', async () => {
    const { workDir, zstackHome } = setupSkillifyWorkdir('happy', ['scrape', 'skillify']);
    const fixturePath = path.join(workDir, 'fixture.html');
    fs.writeFileSync(fixturePath, PROTOTYPE_FIXTURE_HTML);
    const fileUrl = `file://${fixturePath}`;
...
(translates to ~/.zstack/browser-skills/<name>/ via $HOME).
...
Do NOT halt for clarification.`,
      ...
      env: {
        ...
        HOME: childHome, // /skillify writes to $HOME/.zstack/browser-skills/
      },
      maxTurns: 40,
    ...
    // The skill lands under $HOME/.zstack/browser-skills/<name>/ (= childHome);
    // sweep the cwd tier too in case the skill's write path resolves cwd-relative.
    const skillRoots = [childHome, workDir].map((r) => path.join(r, '.zstack', 'browser-skills'));
  ...
  testConcurrentIfSelected('skillify-provenance-refusal', async () => {
    const { workDir, zstackHome } = setupSkillifyWorkdir('refusal', ['skillify']);
    ...
    // sibling home/ dir keeps the override's intent (any ~/.zstack write from
...
write any files.`,
    ...
    const noStaging = diskRoots.every((root) => {
      const stagingRoot = path.join(root, '.zstack', '.tmp');
  ...
  testConcurrentIfSelected('skillify-approval-reject', async () => {
    const { workDir, zstackHome } = setupSkillifyWorkdir('reject', ['scrape', 'skillify']);
    const fixturePath = path.join(workDir, 'fixture.html');
    fs.writeFileSync(fixturePath, PROTOTYPE_FIXTURE_HTML);
    const fileUrl = `file://${fixturePath}`;
...
Use HOME=${childHome}. Do NOT commit the skill.`,
    ...
    // Sweep BOTH roots: $HOME/.zstack (= childHome) and cwd-relative .zstack.

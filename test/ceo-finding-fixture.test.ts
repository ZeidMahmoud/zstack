...
describe('CEO finding fixture establishes scope before launch', () => {
  ...
  test('input and project instructions are committed before the real preamble runs', () => {
    ...
    try {
      ...
      const output = execFileSync(path.join(ROOT, 'bin', 'zstack-skill-start'), ['--skill', 'plan-ceo-review'], {
        cwd, env: { PATH: process.env.PATH!, HOME: home, ZSTACK_HOME: state }, encoding: 'utf8', timeout: 10_000,
      });
      expect(output).toContain('HAS_ROUTING: yes');
      expect(output).not.toContain('ZSTACK_INSTRUCTION_BEGIN: routing-injection');
    } finally { fs.rmSync(root, { recursive: true, force: true }); }
  });
  ...
  test('the split fixture preserves every candidate and exact per-attempt plan target', () => {
    ...
    try {
      const target = path.join(root, 'zstack-test-plan-ceo-split-overflow.md');
      const input = FORCING_SPLIT_OVERFLOW_CEO.replaceAll('/tmp/zstack-test-plan-ceo-split-overflow.md', target);
...
`);
  try {
    ...
    const failures:Record<string,string>={below:'BAND FAIL (below floor)',above:'BAND FAIL (above ceiling)',
      ...
      'trailing-report':'trailing ## heading(s) after ZSTACK REVIEW REPORT',

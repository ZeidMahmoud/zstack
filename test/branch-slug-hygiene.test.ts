...
describe('branch slug hygiene (#2550, #1851)', () => {
  test('inventory covers repository outputs and host caches without ignored candidate trees', () => {
    const repo = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-skill inventory-'));
    try {
      ...
      const caches = hosts.map(host => `${host.hostSubdir}/skills/zstack-health/SKILL.md`);
      ...
      write('.context/outside/skills/zstack-foreign/SKILL.md');
    ...
    try {
      ...
      const recover = (cwd: string, inheritedBranch?: string) => {
        const result = spawnSync('bash', ['-c', script], {
          ...
          env: { ...env, ZSTACK_PROJECT_SLUG: slug, TEST_BIN: path.join(ROOT, 'bin'), _BRANCH: inheritedBranch },
        });

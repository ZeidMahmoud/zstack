...
for(const [name,mutate] of Object.entries({
 ...
 'missing report':(x:Case)=>{x.plan=x.plan.slice(0,x.plan.indexOf('## ZSTACK REVIEW REPORT'));},
 'duplicate report':(x:Case)=>{x.plan+='\n'+x.plan.slice(x.plan.indexOf('## ZSTACK REVIEW REPORT'));},
...
test('handoff alone never supplies a native terminal or refreshes modifying answers',()=>{
 ...
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'zstack-eng-task-pause-')),file=path.join(dir,'reviewed.md'),now=Date.now;
 try {

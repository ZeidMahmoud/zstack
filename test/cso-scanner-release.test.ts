...
describe('CSO scanner qualification workflow', () => {
  test('keeps branch validation read-only while qualification and promotion are protected-main and review gated', () => {
    ...
    expect(raw).toContain('test/cso-scanner-docker-integration.test.ts'); expect(raw).toContain('ZSTACK_CSO_SCANNER_VERSION_HASH'); expect(raw).toContain('hash-asset scanner-asset');

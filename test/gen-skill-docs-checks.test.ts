...
const base = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-check-contract-'));
...
describe('generator artifact and dry-run contract', () => {
  test('all host artifacts validate, with canonical links and host exclusions', () => {
    ...
    expect(generated.artifacts.some(a => a.relativePath === '.agents/skills/zstack-claude-code/SKILL.md')).toBe(true);
    expect(generated.artifacts.some(a => a.relativePath === '.agents/skills/zstack-codex/SKILL.md')).toBe(false);
    expect(fs.readFileSync(path.join(render, 'ship/SKILL.md'), 'utf-8')).toContain('~/.claude/skills/zstack/ship/sections/');
  ...
  for (const relativePath of [
    ...
    '.agents/skills/zstack-ship/agents/openai.yaml',
    'openclaw/zstack-lite-CLAUDE.md', 'openclaw/zstack-full-CLAUDE.md', 'openclaw/zstack-plan-CLAUDE.md',
    'zstack/llms.txt', 'agents-digest/zstack-AGENTS.md',
  ...
  test('missing metadata directory is reported and remains absent', async () => {
    const dir = path.join(render, '.agents/skills/zstack-ship/agents');
    ...
    try {
      ...
      expect(result.exitCode).toBe(1);
      expect(result.diagnostics.some(d => d.kind === 'stale' && d.relativePath === '.agents/skills/zstack-ship/agents/openai.yaml')).toBe(true);
  ...
  test('all-host generation reports a partial failure and still renders subsequent hosts', async () => {
    ...
    for (const relativePath of [
      'ship/SKILL.md', '.factory/skills/zstack-ship/SKILL.md', '.gbrain/skills/zstack-ship/SKILL.md',
      'openclaw/zstack-plan-CLAUDE.md', 'zstack/llms.txt', 'agents-digest/zstack-AGENTS.md',
    ]) {
  ...
  for (const relativePath of ['zstack/llms.txt', 'agents-digest/zstack-AGENTS.md', 'review/design-checklist.md', 'lib/dom-dump.js']) {
  ...
  test.skipIf(process.platform === 'win32')('repo-root sidecar symlink skips skill and metadata without touching source', async () => {
    ...
    fs.symlinkSync(ROOT, path.join(skills, 'zstack'), 'dir');
    ...
    expect(result.exitCode).toBe(0);
    expect(result.diagnostics.some(d => d.kind === 'skipped' && d.relativePath === '.agents/skills/zstack/SKILL.md')).toBe(true);
    expect(result.artifacts.some(a => a.relativePath.startsWith('.agents/skills/zstack/'))).toBe(false);
...
describe('repository freshness without installed caches', () => {
  ...
  test('ignored stale caches and unrelated dirty files do not affect freshness', () => {
    fs.mkdirSync(path.join(repo, '.agents/skills/zstack-ship'), { recursive: true });
    fs.writeFileSync(path.join(repo, '.agents/skills/zstack-ship/SKILL.md'), 'stale cache');
  ...
  test.skipIf(process.platform === 'win32')('ignored symlink cache leaves foreign files and freshness unchanged', () => {
    ...
    const foreignSkill = path.join(foreign, 'skills/zstack-ship/SKILL.md');
  ...
  test('tracked ignored output is still freshness-checked', () => {
    const relativePath = '.agents/skills/zstack-ship/SKILL.md';
...
describe('shared content validators', () => {
  ...
  for (const [content, message] of [
    ...
    [frontmatter + 'Read ~/.claude/skills/zstack/SKILL.md', 'outside a bash block'],
  ]) {

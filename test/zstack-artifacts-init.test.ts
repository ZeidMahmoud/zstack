/**
 * zstack-artifacts-init — provider-selection + brain-admin-hookup tests.
 *
 * Mirrors the zstack-brain-init-gh-mock.test.ts pattern: install fake gh /
 ...
 * ~/.zstack-artifacts-remote.txt, (c) prints the "Send this to your brain
...
const INIT_BIN = path.join(ROOT, 'bin', 'zstack-artifacts-init');
...
function makeFakeGh(opts: {
  ...
  const webUrl = opts.webUrl ?? `https://github.com/testuser/zstack-artifacts-testuser`;
...
function makeFakeGlab(opts: {
  ...
  const authStatus = opts.authStatus ?? 'ok';
  const repoCreate = opts.repoCreate ?? 'success';
  const webUrl = opts.webUrl ?? 'https://gitlab.com/testuser/zstack-artifacts-testuser';
  const gitProtocol = opts.gitProtocol ?? 'https';
  const script = `#!/bin/bash
...
describe('zstack-artifacts-init provider selection', () => {
  test('--host github invokes gh repo create with zstack-artifacts-$USER', () => {
    makeFakeGh({});
    ...
    expect(createCall).toContain('zstack-artifacts-testuser');
  ...
  test('neither authed → falls through to manual URL paste', () => {
    ...
    const r = run([], { input: 'https://github.com/testuser/zstack-artifacts-testuser\n' });
    expect(r.status).toBe(0);
    expect(r.stderr).toContain('Neither gh nor glab');
  });
});

describe('zstack-artifacts-init canonical URL storage (codex Finding #10)', () => {
  test('stores HTTPS URL canonical in ~/.zstack-artifacts-remote.txt', () => {
    makeFakeGh({ webUrl: 'https://github.com/testuser/zstack-artifacts-testuser' });
    const r = run(['--host', 'github']);
    expect(r.status).toBe(0);
    const remoteFile = path.join(tmpHome, '.zstack-artifacts-remote.txt');
    ...
    expect(stored).toBe('https://github.com/testuser/zstack-artifacts-testuser');
  });

  test('strips trailing .git from gh repo view output', () => {
    makeFakeGh({ webUrl: 'https://github.com/testuser/zstack-artifacts-testuser.git' });
    const r = run(['--host', 'github']);
    expect(r.status).toBe(0);
    const stored = fs.readFileSync(path.join(tmpHome, '.zstack-artifacts-remote.txt'), 'utf-8').trim();
    expect(stored).toBe('https://github.com/testuser/zstack-artifacts-testuser');
  });
  ...
  test('configures git origin with HTTPS when gh git_protocol is https', () => {
    ...
    expect(remote.stdout.trim()).toBe('https://github.com/testuser/zstack-artifacts-testuser');
  });

  test('configures git origin with SSH when gh git_protocol is ssh', () => {
    makeFakeGh({
      webUrl: 'https://github.com/testuser/zstack-artifacts-testuser',
      gitProtocol: 'ssh',
    });
    const r = run(['--host', 'github']);
    expect(r.status).toBe(0);
    const remote = gitInFixture(['-C', tmpHome, 'remote', 'get-url', 'origin']);
    expect(remote.stdout.trim()).toBe('git@github.com:testuser/zstack-artifacts-testuser.git');
  });
  ...
  test('honors glab git_protocol when configured', () => {
    ...
    expect(r.status).toBe(0);
    const remote = gitInFixture(['-C', tmpHome, 'remote', 'get-url', 'origin']);
    expect(remote.stdout.trim()).toBe('git@gitlab.com:testuser/zstack-artifacts-testuser.git');
  });
});

describe('zstack-artifacts-init brain-admin hookup printout (codex Finding #3)', () => {
  ...
  test('--url-form-supported true prints the one-liner with --url', () => {
    ...
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('Send this to your brain admin');
    expect(r.stdout).toContain('gbrain sources add zstack-artifacts-testuser --url');
  ...
  test('the gbrain command line uses canonical HTTPS, not SSH', () => {
    ...
    expect(gbrainLine).toContain('https://github.com/testuser/zstack-artifacts-testuser');
    expect(gbrainLine).not.toContain('git@github.com');
    // The Push line follows the provider CLI preference independently.
  });
});

describe('zstack-artifacts-init idempotency', () => {
  test('--remote <url> bypasses provider selection entirely', () => {
    makeFakeGh({});
    const r = run(['--remote', 'https://github.com/testuser/zstack-artifacts-testuser']);
    expect(r.status).toBe(0);
  ...
  test('--push-protocol overrides the inferred protocol', () => {
    ...
    const r = run([
      '--remote',
      'https://github.com/testuser/zstack-artifacts-testuser',
      '--push-protocol',
      'ssh',
    ]);
    expect(r.status).toBe(0);
  ...
  test('re-run with same --remote is safe (no conflict error)', () => {
    makeFakeGh({});
    const url = 'https://github.com/testuser/zstack-artifacts-testuser';
    run(['--remote', url]);
  ...
  test('failed allowlist replacement leaves the old file intact', () => {
    ...
    fs.writeFileSync(fakeMv, `#!/bin/bash\nlast=\"\"\nfor arg in \"$@\"; do last=\"$arg\"; done\nif [ \"$last\" = \"$ZSTACK_HOME/.brain-allowlist\" ]; then exit 73; fi\nexec /bin/mv \"$@\"\n`, { mode: 0o755 });
  ...
  test('allowlist read failure leaves the old file intact', () => {
    ...
    fs.writeFileSync(fakeGrep, `#!/bin/bash\nfor arg in \"$@\"; do if [ \"$arg\" = \"$ZSTACK_HOME/.brain-allowlist\" ]; then exit 2; fi; done\nexec /bin/grep \"$@\"\n`, { mode: 0o755 });
  ...
  test('partial allowlist assembly failure preserves the original and cleans temporary files', () => {
    ...
    fs.writeFileSync(fakeCat, [
      ...
      'for arg in "$@"; do',
      '  case "$arg" in',
      '    "$ZSTACK_HOME"/.brain-allowlist.*)',
      '      if /bin/grep -qF \'# Canonical allowlist of paths that zstack-brain-sync will publish.\' "$arg"; then /bin/head -c 32 "$arg"; exit 73; fi',
      '      ;;',
      '  esac',
      'done',
      'exec /bin/cat "$@"',
  ...
  test('re-run with DIFFERENT --remote exits 1 with conflict message', () => {
    makeFakeGh({});
    run(['--remote', 'https://github.com/testuser/zstack-artifacts-testuser']);

...
describe("PR #1169 bug #4: zstack-telemetry-sync mktemp fallback", () => {
  const SCRIPT = "bin/zstack-telemetry-sync";
...
// resolver + ship pr-body) and — the destructive one — made /zstack-upgrade's
// vendored path clone to "/zstack", fail the swap, then `rm -rf` BOTH the
...
describe("#2679: skill-content mktemp guards", () => {
  ...
  test("zstack-upgrade vendored block guards mktemp -d and clone with loud aborts", () => {
    const body = readScript("zstack-upgrade/SKILL.md.tmpl");
    expect(body).toMatch(/TMP_DIR=\$\(mktemp -d\)\s*\|\|\s*\{[^}]*exit 1/);
    expect(body).toMatch(/git clone[^\n]*\|\|\s*\{[^}]*exit 1/);
  });

  test("zstack-upgrade vendored block restores the backup on a failed swap (no unconditional backup rm)", () => {
    const body = readScript("zstack-upgrade/SKILL.md.tmpl");
    expect(body).toMatch(/if mv "\$TMP_DIR\/zstack" "\$INSTALL_DIR"; then/);
    expect(body).toMatch(/mv "\$INSTALL_DIR\.bak" "\$INSTALL_DIR"/);
    // The backup rm must live inside the success branch, not after the block.
    const block = body.slice(body.indexOf('if mv "$TMP_DIR/zstack"'));
...
echo "SHOULD NOT REACH: $TMP_DIR"`;
  ...
  test("runtime: zstack-redact --from-file '' errors loudly instead of falling through to stdin", () => {
    const { spawnSync } = require("node:child_process") as typeof import("node:child_process");
    const r = spawnSync(
      "bun",
      [path.join(ROOT, "bin", "zstack-redact"), "--from-file", "", "--json"],

...
const ROOT=path.resolve(import.meta.dir,'..'),launcher=path.join(ROOT,'bin',process.platform==='win32'?'zstack-cso-launcher.exe':'zstack-cso-launcher');
...
function command(args:string[]){return spawnSync(launcher,args,{cwd:repo,encoding:'utf8',env:{HOME:root,ZSTACK_HOME:state,PATH:'/usr/bin:/bin'},timeout:30_000});}

 ...
 *    same canonical zstack runtime checkout used by lazy-section paths.
 * 3. connect-chrome (dir symlink) collapses into open-zstack-browser — no
...
describe('hermeticSkillsConfigDir', () => {
  ...
  test('exact skill registry plus the canonical runtime view', () => {
    ...
    expect(seeded).toEqual([...skillCensus(ROOT).registryEntries, 'zstack'].sort());
    const runtime = path.join(skillsDir, 'zstack');
    ...
    for (const rel of ['bin/zstack-config', 'lib/claude-bin.ts', 'SKILL.md', 'ETHOS.md', 'extension/manifest.json',
  ...
  test('recursive installed discovery cannot read historical or dependency markdown', () => {
    const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-runtime-view-'));
  ...
  test('rejects escaped or circular assets and removes only its partial view', () => {
    const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-runtime-view-owned-'));
    try {
      const source = path.join(fixture, 'source');
  ...
  test('ordinary aliases cannot reopen excluded directories or files', () => {
    const fixture = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-runtime-view-alias-'));
    const source = path.join(fixture, 'source');
    const view = path.join(fixture, 'view');
    try {
  ...
  test('connect-chrome collapses into a single open-zstack-browser entry', () => {
    const seeded = fs.readdirSync(skillsDir);
    expect(seeded.filter((n) => n === 'open-zstack-browser')).toHaveLength(1);
    expect(seeded).not.toContain('connect-chrome');
  });

  test('root router registered as _zstack-command pointing at the root SKILL.md', () => {
    const link = path.join(skillsDir, '_zstack-command', 'SKILL.md');

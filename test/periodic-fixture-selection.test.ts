...
describe('periodic fixture dependencies select their behavioral cases', () => {
  const cases: Array<[string, string[]]> = [
    ...
    ['test/zstack-paths.test.ts', ['autoplan-chain-pty', 'carve-section-loading', 'design-html-slop-gate']],
    ['test/zstack-brain-context-load.test.ts', ['autoplan-chain-pty', 'plan-ceo-section-loading']],
...
test('Eng section local review helpers select their carve evaluation', () => {
  for (const file of ['bin/zstack-review-log', 'bin/zstack-review-read', 'lib/review-evidence.ts',
...
test('decision-log CLI and validator select the demonstrated DX consumer without global or quality fanout', () => {
  for (const file of ['bin/zstack-decision-log', 'lib/zstack-decision.ts']) {
...
test('structured auto-decision evidence selects every native observer', () => {
  ...
  for (const file of ['bin/zstack-question-log', 'bin/zstack-question-preference']) {
...
test('stderr lifecycle regression selects runtime consumers without a quality-map edge', () => {
  const expected = [
    ...
    'zstack-upgrade-happy-path', 'land-and-deploy-workflow', 'land-and-deploy-first-run', 'land-and-deploy-review-gate', 'canary-workflow',

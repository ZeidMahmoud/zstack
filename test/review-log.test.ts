...
function run(input: string, opts: { expectFail?: boolean; captured?: boolean } = {}): { stdout: string; exitCode: number } {
  const execOpts: ExecSyncOptionsWithStringEncoding = {
    cwd: ROOT,
    env: { ...process.env, ZSTACK_HOME: tmpDir },
    encoding: 'utf-8',
    timeout: 10000,
  };
  try {
    const args = [input];
    if (opts.captured) {
      const token = execFileSync(`${BIN}/zstack-review-log`, ['--start', 'review'], execOpts).trim(); // timeout via execOpts
      args.push('--finish', token);
    }
    const stdout = execFileSync(`${BIN}/zstack-review-log`, args, execOpts).trim(); // timeout via execOpts
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-revlog-'));
  // zstack-review-log uses zstack-slug which needs a git repo — create the projects dir
...
describe('zstack-review-log', () => {
  ...
  test('append still succeeds outside a git repo (binding fields omitted)', () => {
    const nonGit = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-nongit-'));
    try {
      const execOpts: ExecSyncOptionsWithStringEncoding = {
        cwd: nonGit,
        env: { ...process.env, ZSTACK_HOME: tmpDir },
      ...
      execSync(`${BIN}/zstack-review-log '{"skill":"review","status":"clean"}'`, execOpts); // timeout via execOpts
...
describe('zstack-wtree', () => {
  function withScratchRepo(fn: (repoDir: string, wtree: () => string) => void) {
    const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-wtree-'));
    try {
      ...
      git('commit -q -m init');
      const wtree = () => execSync(`${BIN}/zstack-wtree`, { cwd: repoDir, encoding: 'utf-8', timeout: 10000 }).trim();
      fn(repoDir, wtree);
    } finally {
  ...
  test('racy-git window: a same-size rewrite pinned to the index timestamp changes the fingerprint', () => {
    withScratchRepo((repoDir, wtree) => {
      ...
      // zstack-wtree must carry the real index's mtime onto its temp copy —
  ...
  test('racy-git window stays closed even when touch fails (stubbed-touch fallback)', () => {
    withScratchRepo((repoDir, _wtree) => {
      ...
      const stubDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-touch-stub-'));
      fs.writeFileSync(path.join(stubDir, 'touch'), '#!/bin/sh\nexit 1\n', { mode: 0o755 });
      const wtreeStubbed = () =>
        execSync(`${BIN}/zstack-wtree`, {
          cwd: repoDir,
          encoding: 'utf-8',
          timeout: 10000,
  ...
  test('exits non-zero outside a git repo', () => {
    const nonGit = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-wtree-nongit-'));
    try {
      expect(() => execSync(`${BIN}/zstack-wtree`, { cwd: nonGit, timeout: 10000, stdio: 'pipe' })).toThrow();
    } finally {
      fs.rmSync(nonGit, { recursive: true, force: true });
    }
  });
...
describe('zstack-review-read', () => {
  test('emits ---WTREE---, ---TREE--- and ---DIRTY--- sections', () => {
    const out = execSync(`${BIN}/zstack-review-read`, {
      cwd: ROOT,
      env: { ...process.env, ZSTACK_HOME: tmpDir },
      encoding: 'utf-8',
      timeout: 10000,

...
function setupWorkdir(suffix: string): { workDir: string; zstackHome: string; slug: string } {
  const workDir = fs.mkdtempSync(path.join(os.tmpdir(), `skill-e2e-ctx-${suffix}-`));
  const zstackHome = path.join(workDir, '.zstack-home');
  ...
  for (const script of [
    'zstack-timeline-log', 'zstack-timeline-read', 'zstack-slug',
    'zstack-learnings-log', 'zstack-learnings-search',
    'zstack-update-check', 'zstack-config', 'zstack-repo-mode',
  ]) {
...
Environment:
- Use ZSTACK_HOME="${zstackHome}" for all zstack bin scripts.
...
`);
  ...
  return { workDir, zstackHome, slug };
}

// Helper: seed a saved-context file into the storage dir.
function seedSave(zstackHome: string, slug: string, filename: string, frontmatter: Record<string, string>, body: string) {
  const dir = path.join(zstackHome, 'projects', slug, 'checkpoints');
...
describeIfSelected('Context Skills E2E (live-fire)', [
  ...
  testConcurrentIfSelected('context-save-routing', async () => {
    const { workDir, zstackHome, slug } = setupWorkdir('routing');

    // Prompt pattern: the slash command + explicit "invoke via Skill tool"
    // instruction. The ZSTACK_HOME / ./bin bash setup that used to be in
    ...
    const checkpointDir = path.join(zstackHome, 'projects', slug, 'checkpoints');
  ...
  testConcurrentIfSelected('context-save-then-restore-roundtrip', async () => {
    const { workDir, zstackHome, slug } = setupWorkdir('roundtrip');
...
Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('context-restore-fragment-match', async () => {
    const { workDir, zstackHome, slug } = setupWorkdir('fragment');

    // Seed three saves with distinct titles.
    seedSave(zstackHome, slug, '20260101-120000-alpha-feature.md',
    ...
    seedSave(zstackHome, slug, '20260202-120000-middle-payments.md',
    ...
    seedSave(zstackHome, slug, '20260303-120000-omega-release.md',
  ...
  testConcurrentIfSelected('context-restore-empty-state', async () => {
    const { workDir, zstackHome, slug } = setupWorkdir('empty');
  ...
  testConcurrentIfSelected('context-restore-list-delegates', async () => {
    const { workDir, zstackHome, slug } = setupWorkdir('delegates');
    seedSave(zstackHome, slug, '20260101-120000-seed.md',
  ...
  testConcurrentIfSelected('context-restore-legacy-compat', async () => {
    const { workDir, zstackHome, slug } = setupWorkdir('legacy');
    ...
    // exactly this reason.
    seedSave(zstackHome, slug, '20260301-120000-legacy-pre-rename-work.md',
      {
        status: 'in-progress',
  ...
  testConcurrentIfSelected('context-save-list-current-branch', async () => {
    const { workDir, zstackHome, slug } = setupWorkdir('list-current');

    // Seed 3 files on 3 different branches. Current branch is "main".
    seedSave(zstackHome, slug, '20260101-120000-main-work.md',
      { status: 'in-progress', branch: 'main', timestamp: '2026-01-01T12:00:00Z' },
      '## Working on: main work LISTCURR_MAIN_TOKEN\n');
    seedSave(zstackHome, slug, '20260202-120000-feat-alpha.md',
      { status: 'in-progress', branch: 'feat/alpha', timestamp: '2026-02-02T12:00:00Z' },
      '## Working on: alpha LISTCURR_ALPHA_TOKEN\n');
    seedSave(zstackHome, slug, '20260303-120000-feat-beta.md',
  ...
  testConcurrentIfSelected('context-save-list-all-branches', async () => {
    const { workDir, zstackHome, slug } = setupWorkdir('list-all');

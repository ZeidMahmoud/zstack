 ...
 *    `/.claude` config dir, a temp ZSTACK_HOME, and none of the planted
    ...
    try {
      const result = await runSkillTest({
        ...
        prompt: 'Run exactly this bash command and then stop: ' +
          'echo "CFG=${CLAUDE_CONFIG_DIR:-}"; echo "GH=${ZSTACK_HOME:-}"; ' +
      ...
      expect(cfg).toMatch(/zstack-hermetic-.*\/\.claude$/);
      expect(bashOut).toMatch(/GH=\S*zstack-home/);

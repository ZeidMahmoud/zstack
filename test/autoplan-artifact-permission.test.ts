...
function replay(relative?: string) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-artifact-permission-')); roots.push(root);
  const cwd = path.join(root, path.basename(fixture.cwd)); fs.mkdirSync(cwd);
  const ownedStateRoot = path.join(root, 'home', '.zstack');
...
describe('owned Autoplan artifact edit permission', () => {
  ...
  test('requires the exact owned project and known artifact filename; no broad state/home approval', () => {
    ...
    r.context.ownedStateRoot = path.join(r.root, 'caller-ZSTACK_HOME'); expect(pick(r)).toBeNull();
    r.context.ownedStateRoot = path.join(r.root, 'home', '.zstack');
  ...
  test.skipIf(process.platform === 'win32')('trusted temp-parent aliases preserve ownership without permitting a symlink state root', () => {
    ...
    const target = path.join(alias, '.zstack', path.relative(r.context.ownedStateRoot, r.file));
    r.context.ownedStateRoot = path.join(alias, '.zstack');

...
const ZSTACK_CONFIG = path.join(ROOT, 'bin', 'zstack-config');
...
describe('setup: plan-tune hooks are non-interactive-safe', () => {
  ...
  test('resolution falls through env then saved config', () => {
    expect(setupSrc).toContain('ZSTACK_PLAN_TUNE_HOOKS');
...
describe('setup: PT_EXPLICIT provenance (Conductor auto-opt-in respects explicit decisions)', () => {
  ...
  test('config provenance uses zstack-config has (env-resolution-safe), never a hardcoded config grep', () => {
    // `zstack-config get` returns the default "prompt" for absent keys, so
    // key PRESENCE must come from `has`, which resolves ZSTACK_STATE_ROOT /
    // ZSTACK_HOME / ZSTACK_STATE_DIR the same way `get` does. A hardcoded
    // grep of ~/.zstack/config.yaml misclassifies under env overrides.
    expect(setupSrc).toMatch(/"\$ZSTACK_CONFIG" has plan_tune_hooks/);
    expect(setupSrc).not.toMatch(/grep -q ["']\^plan_tune_hooks:/);
  });
});

describe('zstack-config: has subcommand (key-presence provenance)', () => {
  let tmpHome2: string;
  let env2: NodeJS.ProcessEnv;

  beforeAll(() => {
    tmpHome2 = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-cfg-has-'));
    env2 = { ...process.env, ZSTACK_STATE_ROOT: tmpHome2 };
  });
  ...
  function has(key: string): number {
    try {
      execSync(`${ZSTACK_CONFIG} has '${key}'`, { encoding: 'utf-8', env: env2, timeout: 30_000 });
      return 0;
    } catch (e: any) {
      return e.status ?? 1;
    }
  ...
  test('absent key exits nonzero even though get returns the default', () => {
    ...
    const got = execSync(`${ZSTACK_CONFIG} get plan_tune_hooks`, { encoding: 'utf-8', env: env2, timeout: 30_000 }).trim();
  ...
  test('present key exits 0 through the same STATE_DIR resolution as get', () => {
    execSync(`${ZSTACK_CONFIG} set plan_tune_hooks no`, { encoding: 'utf-8', env: env2, timeout: 30_000 });
    expect(has('plan_tune_hooks')).toBe(0);
    // ZSTACK_STATE_ROOT was the writer — a hardcoded ~/.zstack grep would miss it.
  });
...
describe('zstack-config: plan_tune_hooks key', () => {
  // Isolate state: zstack-config reads $ZSTACK_HOME/config.yaml. Point it at a
  // fresh temp dir so `get` returns the built-in default rather than whatever
  // the host machine has in ~/.zstack/config.yaml (which would make the
  // default-value assertion non-deterministic).
  let tmpHome: string;
  let env: NodeJS.ProcessEnv;

  beforeAll(() => {
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-cfg-test-'));
  ...
  test('default is "prompt"', () => {
    const out = execSync(`${ZSTACK_CONFIG} get plan_tune_hooks`, {
      encoding: 'utf-8',
      env,
      timeout: 30_000,
    }).trim();
  ...
  test('appears in defaults and list output', () => {
    const defaults = execSync(`${ZSTACK_CONFIG} defaults`, { encoding: 'utf-8', env, timeout: 30_000 });
    expect(defaults).toContain('plan_tune_hooks');
    const list = execSync(`${ZSTACK_CONFIG} list`, { encoding: 'utf-8', env, timeout: 30_000 });
    expect(list).toContain('plan_tune_hooks');
  });

  test('accepts valid values (round-trips yes/no/prompt)', () => {
    for (const v of ['yes', 'no', 'prompt']) {
      execSync(`${ZSTACK_CONFIG} set plan_tune_hooks ${v}`, { encoding: 'utf-8', env, timeout: 30_000 });
      const got = execSync(`${ZSTACK_CONFIG} get plan_tune_hooks`, { encoding: 'utf-8', env, timeout: 30_000 }).trim();
      expect(got).toBe(v);
    }
  });

  test('rejects out-of-domain values (warns + falls back to prompt)', () => {
    const res = execSync(`${ZSTACK_CONFIG} set plan_tune_hooks maybe 2>&1`, { encoding: 'utf-8', env, timeout: 30_000 });

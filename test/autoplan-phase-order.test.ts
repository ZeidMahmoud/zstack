...
import { prepareMethodology, createSnapshot, preparePhaseClose } from '../bin/zstack-autoplan-snapshot';
...
describe('autoplan phase order (Eng always last)', () => {
  ...
  test.each(phases)('carved child completion and handoff IDs match the pipeline: %j', ({ child, id, next }) => {
    ...
    expect(generated).toContain('Read `~/.claude/skills/zstack/autoplan/sections/phase-close.md` and execute it');
  ...
  test('generated workflow loads each complete skill at its own phase boundary', () => {
    ...
    expect(skill.indexOf('Read `~/.claude/skills/zstack/autoplan/sections/tasks-aggregator.md`'))

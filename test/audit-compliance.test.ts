...
describe('Audit compliance', () => {
  ...
  test('preamble telemetry calls are conditional on _TEL and binary existence', () => {
    ...
    // resolvers into bin/zstack-skill-start (pending finalization) and
    // bin/zstack-skill-end (end-of-skill telemetry). Assert the semantic
    // contract against the scripts — the new home of the calls.
    const skillStart = readFileSync(join(ROOT, 'bin/zstack-skill-start'), 'utf-8');
    ...
    expect(skillStart).toContain('zstack-telemetry-log');
    // End-of-skill telemetry (zstack-skill-end) must also be conditional
    const skillEnd = readFileSync(join(ROOT, 'bin/zstack-skill-end'), 'utf-8');
    expect(skillEnd).toContain('_TEL" != "off"');
    expect(skillEnd).toContain('-x ');
    expect(skillEnd).toContain('zstack-telemetry-log');
    ...
    expect(preamble.slice(completionIdx)).toContain('zstack-skill-end');
  });
  ...
  test('all generated SKILL.md files with telemetry calls use conditional pattern', () => {
    ...
    // zstack-telemetry-log calls (route + first-task events) rely on two
    // layers instead: every call line is best-effort (`|| true`), and the
    // binary itself no-ops when the telemetry tier is off.
    const telLog = readFileSync(join(ROOT, 'bin/zstack-telemetry-log'), 'utf-8');
    ...
    for (const { name, content } of skills) {
      for (const line of content.split('\n')) {
        if (!line.includes('zstack-telemetry-log')) continue;
        // Prose mentions aren't calls; only executable lines invoke the binary.
        if (!line.includes('bin/zstack-telemetry-log')) continue;
        checked++;

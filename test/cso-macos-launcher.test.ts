...
const required = process.env.ZSTACK_CSO_MACOS_TESTS === '1';
if (required && !macos) throw new Error('ZSTACK_CSO_MACOS_TESTS=1 requires native macOS; emulation does not qualify the launcher.');
let temporary = '', marker = '', library = '';

beforeAll(() => {
  if (!macos) return;
  temporary = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), 'zstack-cso-macos-'));
...
(macos ? describe : describe.skip)('CSO native macOS startup', () => {
  test('the public launcher has a valid hardened-runtime signature', () => {
    const launcher = path.join(ROOT, 'bin', 'zstack-cso-launcher');
  ...
  test('DYLD constructor injection cannot run before the launcher scrubs the environment', () => {
    ...
    const result = spawnSync(launcher, ['--version'], {
      ...
      encoding: 'utf8',
      timeout: 30_000,
      env: { ...process.env, HOME: temporary, ZSTACK_HOME: path.join(temporary, 'state'), DYLD_INSERT_LIBRARIES: library, CSO_PRELOAD_MARKER: marker },
    });

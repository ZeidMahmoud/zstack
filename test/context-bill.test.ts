/**
 * zstack-context-bill — token bill-of-materials for an installed skills tree.
 *
...
describe("upstream fix a: root-as-container + walkMd exclusions", () => {
  ...
  it("totalMd: a container skill excludes nested child skills' bytes; the grand total sums per-skill with no overlap", () => {
    ...
    // CONTAINS other skill dirs (the zstack root wraps the whole tree) used to
...
describe("upstream fix b: installed-tree layout (repo-checkout subdir skip)", () => {
  it("skips a subdir that is its own repo checkout (zstack/ inside ~/.claude/skills)", () => {
    ...
    fs.mkdirSync(path.join(tmp, "zstack", ".git"), { recursive: true });
    fs.writeFileSync(path.join(tmp, "zstack", "SKILL.md"), "---\nname: _router\ndescription: d\n---\n# router\n");
    fs.mkdirSync(path.join(tmp, "zstack", "review"));
    fs.writeFileSync(path.join(tmp, "zstack", "review", "SKILL.md"), "---\nname: review\ndescription: d\n---\n# r\n");
...
describe("CLI plumbing", () => {
  ...
  it("bin/zstack-context-bill runs standalone", () => {
    const result = Bun.spawnSync([path.join(ROOT, "bin", "zstack-context-bill"), TREE_A], { timeout: 30_000 });

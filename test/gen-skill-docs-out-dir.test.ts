...
// via a crafted ZSTACK_HOME so the test is deterministic regardless of whether
...
//   (c) its section refs point at the out-dir, not ~/.claude/skills/zstack,
...
describe('gen-skill-docs --out-dir (B2 render isolation)', () => {
  ...
  test('renders :user to out-dir, rewrites section paths, leaves worktree canonical', () => {
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-home-'));
    const outDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-out-'));
    ...
    try {
      ...
      const res = spawnSync(
        ...
        { cwd: ROOT, encoding: 'utf-8', timeout: 120_000, env: { ...process.env, ZSTACK_HOME: tmpHome } },
      );
      ...
      expect(skillContent).not.toContain('~/.claude/skills/zstack/ship/sections/');

      // (d) bin refs are NOT rewritten — they still resolve to the global install
      expect(skillContent).toContain('~/.claude/skills/zstack/bin/');
  ...
  test('brain-writeback fixture survives render cleanup without changing any source artifacts', () => {
    const base = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-brain-fixture-'));
    ...
    try {
      ...
      ], { cwd: ROOT, encoding: 'utf-8', timeout: 120_000, env: { ...process.env, ZSTACK_HOME: stateDir } });
      ...
      expect(skill).toContain('~/.claude/skills/zstack/bin/');
  ...
  // #2692: the swap-in callers (setup, zstack-config gbrain-refresh) render
  ...
  test('--link-root repoints section refs at the FINAL dir, not the tmp out-dir (#2692)', () => {
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-home-'));
    const base = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-swap-'));
  ...
  test('both swap-in callers pass --link-root with the final render dir (#2692 wiring)', () => {
    ...
    const configSrc = fs.readFileSync(path.join(ROOT, 'bin', 'zstack-config'), 'utf-8');
    expect(setupSrc).toContain('--out-dir "$_ZSTACK_RENDER_TMP" --link-root "$_ZSTACK_RENDER_DIR"');
  ...
  test('--host codex --out-dir adds no tracked dirt and is byte-identical to the in-place render', () => {
    const outDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-out-codex-'));
    const inPlaceShip = path.join(ROOT, '.agents', 'skills', 'zstack-ship', 'SKILL.md');
    // Compared before/after rather than asserting empty, so a dev's own
    ...
    try {
      ...
      const outShip = path.join(outDir, '.agents', 'skills', 'zstack-ship', 'SKILL.md');
      ...
      expect(fs.existsSync(path.join(outDir, '.agents', 'skills', 'zstack-ship', 'agents', 'openai.yaml'))).toBe(true);
    } finally {
      fs.rmSync(outDir, { recursive: true, force: true });
    }
  }, 120_000);
  ...
  test('--host all --out-dir renders every host tree into the out-dir; tracked tree stays clean', () => {
    const outDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-out-all-'));
    const beforePorcelain = porcelain();
    try {
      const res = spawnSync(
        'bun',
      ...
      for (const rel of [
        'ship/SKILL.md',
        '.agents/skills/zstack-ship/SKILL.md',
        '.factory/skills/zstack-ship/SKILL.md',
        'zstack/llms.txt',
        'openclaw/zstack-lite-CLAUDE.md',
      ]) {

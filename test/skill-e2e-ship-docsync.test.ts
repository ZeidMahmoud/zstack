 ...
 *   │                                     insurance)
 *   ├── zstack-home/.redact-prepush-prompted   marker + env ZSTACK_HOME →
 *   │                       Step 17's credential pre-push guard takes its
 *   │                       silent "continue" branch instead of its
 *   │                       AskUserQuestion branch (zstack-config is absent
...
describeE2E('Ship doc-sync dispatch E2E (gate)', () => {
  describeIfSelected('Ship doc-sync dispatch E2E', ['ship-docsync'], () => {
    ...
    beforeAll(() => {
      workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-docsync-home-'));
      remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-docsync-remote-'));
      ...
      fs.writeFileSync(
        path.join(workDir, 'zstack-home', '.redact-prepush-prompted'), ''
      );
    });

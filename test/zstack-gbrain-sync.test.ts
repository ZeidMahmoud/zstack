/**
 * Unit tests for bin/zstack-gbrain-sync.ts (Lane B).
 *
...
} from "../bin/zstack-gbrain-sync";

const SCRIPT = join(import.meta.dir, "..", "bin", "zstack-gbrain-sync.ts");

function makeTestHome(): string {
  return mkdtempSync(join(tmpdir(), "zstack-gbrain-sync-"));
}
...
describe("zstack-gbrain-sync CLI", () => {
  it("--help exits 0 with usage text", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: zstack-gbrain-sync");
  ...
  it("--dry-run with --code-only reports the code import preview only", () => {
    ...
    const r = runScript(["--dry-run", "--code-only", "--quiet"], { HOME: home, ZSTACK_HOME: zstackHome });
    expect(r.exitCode).toBe(0);
    ...
    expect(r.stdout).not.toContain("zstack-memory-ingest --probe");
    expect(r.stdout).not.toContain("zstack-brain-sync --discover-new");
  ...
  it("--dry-run with all stages shows previews for all three", () => {
    ...
    const r = runScript(["--dry-run"], { HOME: home, ZSTACK_HOME: zstackHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("would: gbrain sources add");
    expect(r.stdout).toContain("gbrain sync --strategy code");
    expect(r.stdout).toContain("would: zstack-memory-ingest");
    expect(r.stdout).toContain("would: zstack-brain-sync");
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("--no-code skips the code import stage", () => {
    ...
    const r = runScript(["--dry-run", "--no-code"], { HOME: home, ZSTACK_HOME: zstackHome });
    expect(r.exitCode).toBe(0);
  ...
  it("dry-run derives a stable source id from the canonical git remote", () => {
    ...
    const r = runScript(["--dry-run", "--code-only", "--quiet"], { HOME: home, ZSTACK_HOME: zstackHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toMatch(/gbrain sources add zstack-code-[a-z0-9-]+/);
    expect(r.stdout).toMatch(/gbrain sync --strategy code --source zstack-code-[a-z0-9-]+/);
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("derived source ids are gbrain-valid (≤32 chars, alnum + interior hyphens, no dots) for any remote", () => {
    ...
    // For `github.com/<org>/<repo>`, the id was `zstack-code-github.com-<org>-<repo>`,
    ...
    for (const remote of cases) {
      ...
      expect(id.startsWith("zstack-code-")).toBe(true);
  ...
  it("derives a gbrain-valid source id when the basename sanitizes to empty", () => {
    ...
    // returned "zstack-code-" (invalid trailing hyphen) and was patched to
  ...
  it("derives distinct source ids for the same absolute path on different hosts", () => {
    ...
    // key keeps them distinct. `ZSTACK_HOSTNAME` env var is the test-only knob;
    ...
    const bindir = mkdtempSync(join(tmpdir(), "zstack-host-collide-bin-"));
    ...
    const runAs = (host: string) =>
      spawnSync("bun", [SCRIPT, "--dry-run", "--code-only", "--quiet"], {
        ...
        timeout: 60000,
        cwd: repo,
        env: { ...process.env, HOME: home, ZSTACK_HOME: zstackHome, ZSTACK_HOSTNAME: host, PATH },
      });
  ...
  it("dry-run does NOT acquire the lock file (lock is for write paths only)", () => {
    ...
    const lockPath = join(zstackHome, ".sync-gbrain.lock");
  ...
  it("writes a state file with schema_version: 1 after a non-dry run", () => {
    ...
    const statePath = join(zstackHome, ".gbrain-sync-state.json");
    expect(existsSync(statePath)).toBe(true);
    const state = JSON.parse(readFileSync(statePath, "utf-8"));
    expect(state.schema_version).toBe(1);
    expect(state.last_writer).toBe("zstack-gbrain-sync");
  ...
  it("records stage results in state file", () => {
    ...
    const state = JSON.parse(readFileSync(join(zstackHome, ".gbrain-sync-state.json"), "utf-8"));
  ...
  it("brain-sync stage resolves the sibling binary, not a HOME-rooted path", () => {
    ...
    // ~/.claude/skills/zstack/bin/zstack-brain-sync, which silently no-op'd
    // on Codex installs and dev workspaces with the misleading summary
    // "skipped (zstack-brain-sync not installed)". Post-fix it resolves
    ...
    const r = runScript(
      ...
      { HOME: home, ZSTACK_HOME: zstackHome },
    );
    ...
    expect(combined).not.toContain("skipped (zstack-brain-sync not installed)");
    rmSync(home, { recursive: true, force: true });
  });
  ...
  it("worktree-aware source ID: two worktrees of the same repo get DIFFERENT ids", () => {
    ...
    expect(idA).not.toBe(idB);
    expect(idA.startsWith("zstack-code-")).toBe(true);
    expect(idB.startsWith("zstack-code-")).toBe(true);
  ...
  it("dry-run preview includes legacy-source removal + attach (post-codex-review hardening)", () => {
    // Codex adversarial flagged: pre-pathhash `zstack-code-<slug>` sources stay
    ...
    // remove the legacy source via `gbrain sources remove zstack-code-<slug>
    ...
    expect(r.stdout).toMatch(/gbrain sync --strategy code --source zstack-code-/);
    expect(r.stdout).toMatch(/gbrain sources attach zstack-code-/);
...
// creating a data-loss window. See bin/zstack-gbrain-sync.ts and the
...
describe("derivePathOnlyHashLegacyId", () => {
  it("returns the pre-#1468 form (path-only sha1, no hostname)", () => {
    ...
    try {
      ...
      process.env.ZSTACK_HOSTNAME = "machine-a";
      const b = derivePathOnlyHashLegacyId(repo);
      process.env.ZSTACK_HOSTNAME = "machine-b";
      const c = derivePathOnlyHashLegacyId(repo);
      expect(a).toBe(b);
      expect(b).toBe(c);
      expect(a.startsWith("zstack-code-")).toBe(true);
      expect(a.length).toBeLessThanOrEqual(32);
    } finally {
      delete process.env.ZSTACK_HOSTNAME;
      process.chdir(cwd);
      rmSync(repo, { recursive: true, force: true });
    }
  });
  ...
  it("produces a different id than the new hostname-folded form", () => {
    ...
    try {
      process.chdir(repo);
      process.env.ZSTACK_HOSTNAME = "machine-x";
      const legacy = derivePathOnlyHashLegacyId(repo);
      // Drive the new id through the CLI so we use the same code path users hit.
      const home = makeTestHome();
      const zstackHome = join(home, ".zstack");
      mkdirSync(zstackHome, { recursive: true });
      const bindir = mkdtempSync(join(tmpdir(), "zstack-legacy-id-distinct-bin-"));
      makeShim(bindir, { "--help": { stdout: "gbrain\n" } });
      const r = spawnSync("bun", [SCRIPT, "--dry-run", "--code-only", "--quiet"], {
        encoding: "utf-8",
        timeout: 60000,
        cwd: repo,
        env: { ...process.env, HOME: home, ZSTACK_HOME: zstackHome, ZSTACK_HOSTNAME: "machine-x", PATH: `${bindir}:${process.env.PATH || ""}` },
      });
...
describe("planHostnameFoldMigration", () => {
  ...
  beforeEach(() => {
    bindir = mkdtempSync(join(tmpdir(), "zstack-mig-plan-bin-"));
    _resetGbrainSupportsRenameCache();
  });
  afterEach(() => {
    rmSync(bindir, { recursive: true, force: true });
  ...
  it("returns ids-match when legacy == new (degenerate case)", () => {
    const result = planHostnameFoldMigration("/repo/path", "zstack-code-same-abc12345", "zstack-code-same-abc12345");
...
describe("constrainSourceId truncation (hyphen-boundary cut)", () => {
  ...
  //
  // Exercised via the dry-run preview (`gbrain sources add zstack-code-…`),
...
describe("sourceLocalPath", () => {
  let bindir: string;
  beforeEach(() => {
    bindir = mkdtempSync(join(tmpdir(), "zstack-source-lp-bin-"));
  });
  afterEach(() => {
    rmSync(bindir, { recursive: true, force: true });
  });

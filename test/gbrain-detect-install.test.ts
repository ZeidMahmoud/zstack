/**
 * zstack-gbrain-detect + zstack-gbrain-install — Slice 2 of /setup-gbrain.
 *
 * Detect: state-reporter JSON with presence, version, config, doctor health,
 * and zstack-brain-sync mode. Pure introspection, no side effects.
 *
...
const DETECT = path.join(ROOT, 'bin', 'zstack-gbrain-detect');
const INSTALL = path.join(ROOT, 'bin', 'zstack-gbrain-install');
...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gbrain-detect-zstack-'));
...
describe('zstack-gbrain-detect', () => {
  test('emits valid JSON even when nothing is configured', async () => {
    ...
    try {
      ...
      expect(j.zstack_brain_sync_mode).toBe('off');
      expect(j.zstack_brain_git).toBe(false);
    } finally {
      fs.rmSync(emptyBin, { recursive: true, force: true });
    }
  });

  test('reports zstack_brain_git: true when ZSTACK_HOME has a .git dir', async () => {
    fs.mkdirSync(path.join(tmpHome, '.git'));
    const emptyBin = fs.mkdtempSync(path.join(os.tmpdir(), 'empty-bin-'));
    try {
      const r = await run(DETECT, [], { env: { PATH: `${emptyBin}:${SAFE_PATH}` } });
      const j = JSON.parse(r.stdout);
      expect(j.zstack_brain_git).toBe(true);
    } finally {
      fs.rmSync(emptyBin, { recursive: true, force: true });
    }
  });
...
describe('zstack-gbrain-install D5 detect-first', () => {
...
describe('zstack-gbrain-install D19 PATH-shadow validation', () => {
  ...
  test('hard-fails (exit 3) when the installed gbrain is below the version floor (#1744)', async () => {
    ...
    try {
      ...
      expect(r.stderr).toContain('below the minimum zstack-tested version');
    } finally {
      fs.rmSync(installDir, { recursive: true, force: true });
      fs.rmSync(fakeBin, { recursive: true, force: true });
    }
...
describe('zstack-gbrain-install argument handling', () => {
  test('--help prints usage without exiting non-zero', async () => {
    const r = await run(INSTALL, ['--help']);
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('zstack-gbrain-install');
  });

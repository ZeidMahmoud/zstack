 ...
 *
 * Covers the `get --batch` verb of bin/zstack-gbrain-repo-policy (bash level:
 ...
 *
 * Each test uses a temp ZSTACK_HOME so nothing leaks into the user's real
 * ~/.zstack. The client is exercised against the REAL bash script — the
...
import { canonicalizeRemote } from "../lib/zstack-memory-helpers";

const ROOT = path.resolve(import.meta.dir, "..");
const BIN = path.join(ROOT, "bin", "zstack-gbrain-repo-policy");

let tmpHome: string;

function env(): NodeJS.ProcessEnv {
  return { ...process.env, ZSTACK_HOME: tmpHome };
}
...
describe("bin/zstack-gbrain-repo-policy get --batch (bash level)", () => {
...
//
// bin/zstack-memory-ingest.ts produces page.git_remote via canonicalizeRemote
// (lib/zstack-memory-helpers) and then looks the policy up through

...
const SCRIPT = path.join(ROOT, 'bin', 'zstack-session-update');
...
function makeFixture() {
  const base = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-supd-'));
  ...
  fs.writeFileSync(
    path.join(seed, 'bin', 'zstack-config'),
    '#!/usr/bin/env bash\nif [ "$1" = "get" ]; then case "$2" in auto_upgrade) echo true;; skill_prefix) echo false;; *) echo "";; esac; fi\nexit 0\n',
    { mode: 0o755 },
  );
  fs.writeFileSync(path.join(seed, 'bin', 'zstack-patch-names'), '#!/usr/bin/env bash\nexit 0\n', {
    mode: 0o755,
  });
  git(seed, 'init', '-q');
  git(seed, 'add', '-A');
...
function runScript(install: string, state: string) {
  return spawnSync('bash', [SCRIPT], {
    encoding: 'utf8',
    env: { ...process.env, ZSTACK_DIR: install, ZSTACK_STATE_DIR: state },
    timeout: 20000,
  });
}
...
describe('zstack-session-update pull wedge (#2566)', () => {
...
describe('zstack-session-update lock identity + TTL (#2613)', () => {
  ...
  function runScriptWithPath(install: string, state: string, shimDir: string) {
    return spawnSync('bash', [SCRIPT], {
      encoding: 'utf8',
      env: { ...process.env, ZSTACK_DIR: install, ZSTACK_STATE_DIR: state, PATH: `${shimDir}:${process.env.PATH}` },
      timeout: 20000,
    });
  }

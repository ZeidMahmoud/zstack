...
describe('flake ledger', () => {
  ...
  test('env override wins; local default is project-scoped, never machine-global', () => {
    expect(flakeLedgerPath({ ZSTACK_FLAKE_LEDGER: '/x/y.jsonl' } as NodeJS.ProcessEnv)).toBe('/x/y.jsonl');
    ...
    expect(local.includes(path.join('.zstack', 'projects')) || local.startsWith(os.tmpdir())).toBe(true);
  });
});

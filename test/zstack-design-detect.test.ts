/**
 * bin/zstack-design-detect.ts — hermetic tests against the fake engine.
 *
 * Every case runs the wrapper in a temp git repo with a scrubbed env (temp
 * ZSTACK_HOME, temp IMPECCABLE_HOME, PATH reduced to bun + system dirs, no
...
const BIN = path.join(ROOT, 'bin', 'zstack-design-detect.ts');
...
let ZSTACK_HOME: string; // temp zstack home (config.yaml, analytics)
...
beforeAll(() => {
  SANDBOX = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-design-detect-'));
  ...
  ZSTACK_HOME = path.join(SANDBOX, 'zstack-home');
  IMPECCABLE_HOME = path.join(SANDBOX, 'impeccable-home');
  fs.mkdirSync(ZSTACK_HOME);
...
describe('probe', () => {
  ...
  test('a #! shim named impeccable on PATH is launcher-present, not READY → NOT_CACHED with the npx hint', () => {
    ...
    expect(r.out).toContain('zstack-config set design_detector off');
  ...
  test('design_detector: off → DISABLED and nothing else is probed', () => {
    fs.writeFileSync(path.join(ZSTACK_HOME, 'config.yaml'), 'proactive: true\ndesign_detector: off\n');
    try {
      const r = run(['probe'], { env: { IMPECCABLE_BIN: FAKE } });
      expect(lines(r.out)[0]).toBe(SENTINEL.DISABLED);
      expect(r.out).not.toContain(SENTINEL.READY);
    } finally {
      fs.rmSync(path.join(ZSTACK_HOME, 'config.yaml'));
    }
  });
  ...
  test('probe and scan append one analytics line each to the zstack home', () => {
    const log = path.join(ZSTACK_HOME, 'analytics', 'design-detector.jsonl');
...
describe('scan', () => {
  ...
  test.skipIf(!POSIX)('a symlink under designs/ pointing outside is refused; a real file under designs/ is accepted', () => {
    const designs = path.join(ZSTACK_HOME, 'projects', 'x', 'designs', 'design-audit-20260908', 'dom');
    ...
    const notDesigns = path.join(ZSTACK_HOME, 'projects', 'x', 'other.html');
    fs.writeFileSync(notDesigns, '<html></html>');
    const log = path.join(SANDBOX, 'argv2.log');
    fs.rmSync(log, { force: true });
    try {
      const r = run(['scan', '--format', 'zstack', path.join(designs, 'home.dom.html'), path.join(designs, 'etc-link'), notDesigns], { env: { IMPECCABLE_BIN: FAKE, IMPECCABLE_FAKE_LOG: log } });
      ...
      fs.rmSync(path.join(ZSTACK_HOME, 'projects'), { recursive: true, force: true });
    }
  });
  ...
  test.skipIf(!POSIX)('--format zstack normalizes by catalog: tiers, impacts, handoffs, advisory, unmapped; stdout is one JSON document', () => {
    ...
    const r = run(['scan', '--format', 'zstack', 'src/styles.css'], { env: { IMPECCABLE_BIN: FAKE, IMPECCABLE_FAKE_OUTPUT: custom } });
  ...
  test.skipIf(!POSIX)('display cap: 500+ findings → DETECT_TOP shows 50 locations and the total; JSON keeps all', () => {
    const r = run(['scan', '--format', 'zstack', 'src/styles.css'], { env: { IMPECCABLE_BIN: FAKE, IMPECCABLE_FAKE_REPEAT: '84' } });
    const doc = JSON.parse(r.out);
  ...
  test.skipIf(!POSIX)('ignored rules from config are reported in the summary, never re-derived from findings', () => {
    ...
    try {
      const r = run(['scan', '--format', 'zstack', 'src/styles.css'], { env: { IMPECCABLE_BIN: FAKE } });
      const doc = JSON.parse(r.out);
  ...
  test.skipIf(!POSIX)('engine that hangs is killed at the timeout → DETECT_TIMEOUT, exit 1, no orphan', () => {
    const t0 = Date.now();
    const r = run(['scan', 'src/styles.css'], { env: { IMPECCABLE_BIN: FAKE, IMPECCABLE_FAKE_SLEEP_MS: '20000', ZSTACK_DESIGN_DETECT_TIMEOUT_MS: '300' } });
  ...
  test.skipIf(!POSIX)('--changed <base> derives frontend targets from git (committed, staged, untracked), never backend files', () => {
    ...
    fs.rmSync(log, { force: true });
    try {
      const r = run(['scan', '--changed', 'main', '--format', 'zstack'], { env: { IMPECCABLE_BIN: FAKE, IMPECCABLE_FAKE_LOG: log } });
      expect(r.code).toBe(2);
      const argv = JSON.parse(fs.readFileSync(log, 'utf-8').trim().split('\n')[0]).argv as string[];
...
describe('design-review REPORT_DIR agrees with the allow-list', () => {
  for (const config of [
    { name: 'ZSTACK_HOME', zstack: 'configured state', plugin: undefined, pluginRoot: undefined, expected: 'configured state' },
    { name: 'the zstack plugin', zstack: undefined, plugin: 'plugin state', pluginRoot: '/plugins/GsTaCk', expected: 'plugin state' },
    { name: 'HOME default', zstack: undefined, plugin: undefined, pluginRoot: undefined, expected: 'fake-home/.zstack' },
    { name: 'a foreign plugin', zstack: undefined, plugin: 'plugin state', pluginRoot: '/plugins/other', expected: 'fake-home/.zstack' },
    { name: 'ZSTACK_HOME overriding the zstack plugin', zstack: 'configured state', plugin: 'plugin state', pluginRoot: '/plugins/zstack', expected: 'configured state' },
    ...
    const expr = m![1];
    expect(expr.startsWith('$ZSTACK_STATE_ROOT/projects/$SLUG/designs/')).toBe(true);
    const env: RunOpts['env'] = {
      HOME: path.join(SANDBOX, 'fake-home'),
      ZSTACK_HOME: config.zstack ? path.join(SANDBOX, config.zstack) : undefined,
    ...
    const r = spawnSync('bash', ['-c', `eval "$("$1")"\nSLUG=my-repo\nprintf '%s\\n' "${expr}"`, 'report-dir', path.join(ROOT, 'bin', 'zstack-paths')], {
    ...
    fs.rmSync(log, { force: true });
    try {
      const s = run(['scan', '--format', 'zstack', dom + '/home.dom.html', outside, link], { env: { ...env, IMPECCABLE_BIN: FAKE, IMPECCABLE_FAKE_LOG: log } });
...
describe('coverage: probe edges', () => {
  ...
  test.skipIf(!POSIX)('engine version comes from the cache layout or a sibling VERSION file, never a path', () => {
    ...
    const r2 = run(['scan', '--format', 'zstack', 'src/styles.css'], { env: { IMPECCABLE_BIN: path.join(skillLike, 'bin', 'linux-x64', 'impeccable') } });
  ...
  test('scan under design_detector: off prints DISABLED and never spawns the engine', () => {
    fs.writeFileSync(path.join(ZSTACK_HOME, 'config.yaml'), 'design_detector: off\n');
...
describe('coverage: scan edges', () => {
  ...
  test.skipIf(!POSIX)('findings above the cap are truncated in JSON and flagged in DETECT_TOP and the summary', () => {
    const r = run(['scan', '--format', 'zstack', 'src/styles.css'], { env: { IMPECCABLE_BIN: FAKE, IMPECCABLE_FAKE_REPEAT: '900' } });
    const doc = JSON.parse(r.out);
  ...
  test.skipIf(!POSIX)('argument parsing: unknown flags warn, -- ends flags, bad --format falls back to zstack, a trailing --changed is refused', () => {
  ...
  test.skipIf(!POSIX)('the rendered persist block refuses a dump with a HIGH redaction finding (DOM_DUMP_REDACTION_BLOCKED) and keeps a clean one', () => {
    ...
    const work = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-dump-persist-'));
    const report = path.join(work, 'report');
    try {
      const dirty = path.join(work, 'dirty.dom.html');
      const clean = path.join(work, 'clean.dom.html');
      // A PEM block is a HIGH finding for zstack-redact (AWS's documented example key is allowlisted).
      ...
      const runBlock = (file: string, page: string) => {
        const script = block
          ...
          .replaceAll('$HOME/.claude/skills/zstack/bin', path.join(ROOT, 'bin'))
          .replaceAll('~/.claude/skills/zstack/bin', path.join(ROOT, 'bin'));
...
describe('coverage: scan security edges', () => {
  ...
  test.skipIf(!POSIX)('--changed against an unknown base is refused with exit 1, never a silent empty scan', () => {
    ...
    fs.rmSync(log, { force: true });
    const r = run(['scan', '--changed', 'no-such-ref', '--format', 'zstack'], { env: { IMPECCABLE_BIN: FAKE, IMPECCABLE_FAKE_LOG: log } });
  ...
  test.skipIf(!POSIX)('a clean run ([] + exit 0) reports zero counts; a JSON object is a parse error; a missing path is refused', () => {
    const clean = run(['scan', '--format', 'zstack', 'src/styles.css'], { env: { IMPECCABLE_BIN: FAKE, IMPECCABLE_FAKE_RAW: '[]', IMPECCABLE_FAKE_EXIT: '0' } });
  ...
  test.skipIf(!POSIX)('engine stdout above the cap → DETECT_OUTPUT_TOO_LARGE, exit 1', () => {
    ...
    fs.closeSync(fd);
    try {
      const r = run(['scan', '--format', 'zstack', 'src/styles.css'], { env: { IMPECCABLE_BIN: FAKE, IMPECCABLE_FAKE_OUTPUT: big } });
      expect(r.code).toBe(1);
  ...
  test.skipIf(!POSIX)('a quoted or commented design_detector value still reads as off', () => {
    try {
      for (const line of ['design_detector: "off"', "design_detector: 'off'", 'design_detector: off  # why']) {
        fs.writeFileSync(path.join(ZSTACK_HOME, 'config.yaml'), line + '\n');
        const r = run(['probe'], { env: { IMPECCABLE_BIN: FAKE } });
        expect(lines(r.out)[0]).toBe(SENTINEL.DISABLED);
      }
    } finally {
      fs.rmSync(path.join(ZSTACK_HOME, 'config.yaml'), { force: true }); // a failing expect must not leave every later probe DISABLED
    }
  });
});
...
describe('engine identity: named impeccable, realpath outside the project', () => {
  ...
  test.skipIf(!POSIX)('a PATH entry named impeccable that resolves into the repository is never READY', () => {
    ...
    fs.chmodSync(inRepo, 0o755);
    const binDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-path-bin-'));
  ...
  test.skipIf(!POSIX)('the engine identity label is deterministic per binary and differs between binaries', () => {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-other-engine-'));
...
describe('scan: option-like bases and page-controlled inline ignores', () => {
  ...
  test.skipIf(!POSIX)('page dumps under designs/<audit>/dom/ scan with --no-inline-ignores; repository files and zstack-authored designs artifacts keep their inline ignores', () => {
    const audit = path.join(ZSTACK_HOME, 'projects', 'x', 'designs', 'design-audit-20260908-ignores');
    const dom = path.join(audit, 'dom', 'run1');
    fs.mkdirSync(dom, { recursive: true });
    fs.writeFileSync(path.join(dom, 'home.dom.html'), '<!-- impeccable-disable --><html></html>');
    const artifact = path.join(ZSTACK_HOME, 'projects', 'x', 'designs', 'hero-20260908', 'finalized.html');
    ...
    fs.rmSync(log, { force: true });
    try {
      const r = run(['scan', '--format', 'zstack', 'src/styles.css', path.join(dom, 'home.dom.html'), artifact], { env: { IMPECCABLE_BIN: FAKE, IMPECCABLE_FAKE_LOG: log } });
      expect(r.code).toBe(2);
  ...
  test.skipIf(!POSIX)('from HOME (no repository) HOME-rooted installs are READY, HOME files are refused as targets, and a dump still scans without inline ignores', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-fake-home-'));
    ...
    const dom = path.join(ZSTACK_HOME, 'projects', 'x', 'designs', 'design-audit-20260908-home', 'dom', 'run1');
    fs.mkdirSync(dom, { recursive: true });
    fs.writeFileSync(path.join(dom, 'home.dom.html'), '<html></html>');
    ...
    try {
      ...
      const r = run(['scan', '--format', 'zstack', path.join(home, 'secret.css'), path.join(dom, 'home.dom.html')], { cwd: home, env: { HOME: home, IMPECCABLE_HOME: path.join(home, '.impeccable'), IMPECCABLE_BIN: '', IMPECCABLE_FAKE_LOG: log } });
  ...
  test.skipIf(!POSIX)('engine ids that are prototype members or fail the shape check count as unmapped, never as object keys', () => {
    ...
    const r = run(['scan', '--format', 'zstack', 'src/styles.css'], { env: { IMPECCABLE_BIN: FAKE, IMPECCABLE_FAKE_OUTPUT: out } });
    const doc = JSON.parse(r.out);
    expect(doc.total).toBe(5);
  ...
  test.skipIf(!POSIX)('the whole-scan budget stops a huge target set instead of grinding batch after batch', () => {
    ...
    try {
      const r = run(['scan', '--format', 'zstack', 'src/many'], { env: { IMPECCABLE_BIN: FAKE, IMPECCABLE_FAKE_SLEEP_MS: '250', ZSTACK_DESIGN_DETECT_TIMEOUT_MS: '400' } });
      ...
      const r2 = run(['scan', '--format', 'zstack', ...files], { env: { IMPECCABLE_BIN: FAKE, IMPECCABLE_FAKE_SLEEP_MS: '250', ZSTACK_DESIGN_DETECT_TIMEOUT_MS: '400' } });
...
describe('adversarial round: audit directories, config case, refused base with explicit targets', () => {
  test.skipIf(!POSIX)('an audit DIRECTORY under designs/ scans as dumps (--no-inline-ignores), because the engine would walk its dom/ subtree', () => {
    const audit = path.join(ZSTACK_HOME, 'projects', 'x', 'designs', 'design-audit-20260908-dir');
    ...
    fs.rmSync(log, { force: true });
    try {
      const r = run(['scan', '--format', 'zstack', audit], { env: { IMPECCABLE_BIN: FAKE, IMPECCABLE_FAKE_LOG: log } });
      expect(r.code).toBe(2);
      const argv = JSON.parse(fs.readFileSync(log, 'utf-8').trim().split('\n')[0]).argv as string[];
  ...
  test.skipIf(!POSIX)('design_detector: Off (hand-edited casing) still disables; a scan with no engine prints its sentinels on stderr and nothing on stdout', () => {
    ...
    try {
      ...
      fs.rmSync(path.join(ZSTACK_HOME, 'config.yaml'), { force: true });
    }
  });

  test.skipIf(!POSIX)('a refused --changed base makes the scan exit 1 even when explicit targets scanned', () => {
    const r = run(['scan', '--format', 'zstack', '--changed', 'no-such-ref-xyz', 'src/styles.css'], { env: { IMPECCABLE_BIN: FAKE } });
...
describe('install: the one download zstack makes, after consent', () => {
  ...
  const freshHome = () => fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-impeccable-home-'));
  ...
  const ledgerPath = () => path.join(ZSTACK_HOME, 'security', 'egress.jsonl');
  ...
  test('the probe offers the pinned engine for this machine once, and stays silent (no offer, no hint) after "never ask again"', () => {
    ...
    fs.writeFileSync(path.join(ZSTACK_HOME, 'config.yaml'), 'design_detector_install_prompted: true\n');
  ...
  test.skipIf(!POSIX || !PLATFORM)('a checksum mismatch, a 404, an unpinned version, a non-https base, and design_detector off each write nothing', async () => {
    ...
    try {
      ...
      expect(lines(unpinned.out)[0]).toContain(`${SENTINEL.INSTALL_REFUSED}: zstack pins no checksum for engine 9.9.9`);

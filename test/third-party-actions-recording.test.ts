...
const positives = {
  ...
  'tpa-absent-linux': "A) zstack's own visible browser B) manual instructions C) defer.",
...
const negatives = {
  ...
  'tpa-absent-linux': ["A) zstack's own visible browser B) manual; download it at aside.com", "A) Drive it in your Aside browser B) manual in zstack's own visible browser", "zstack's own visible browser; manual", "A) zstack's own visible browser", 'A) manual instructions'],
...
test('actual paid assertion boundaries and all failure stages retain one accurate record', async () => {
  ...
  for (const [overrides, expected] of [
    ...
    [{ ZSTACK_EVAL_MODEL: 'global-model' }, 'global-model'],
    [{ ZSTACK_EVAL_MODEL: 'global-model', ZSTACK_EVAL_MODEL_CAPTURE: 'capture-model' }, 'capture-model'],
    [{ ZSTACK_EVAL_MODEL: 'global-model', ZSTACK_EVAL_MODEL_CAPTURE: 'capture-model', EVALS_MODEL: 'evals-model' }, 'evals-model'],
  ]) {
    for (const key of ['EVALS_MODEL', 'ZSTACK_EVAL_MODEL', 'ZSTACK_EVAL_MODEL_CAPTURE']) delete process.env[key];
...
`);
  try {
    const childEnv = { ...process.env, EVALS: '', EVALS_ALL: '', EVALS_PREFLIGHT_OK: '1',
      ZSTACK_CLAUDE_CLI_VERSION: 'free TPA fixture',
      HOME: dir, TMPDIR: dir, TMP: dir, TEMP: dir, ZSTACK_EVAL_DIR: path.join(dir, 'evals') };
    for (const key of ['EVALS_MODEL', 'ZSTACK_EVAL_MODEL', 'ZSTACK_EVAL_MODEL_CAPTURE']) delete childEnv[key];

 ...
 *   ├── zstack-home/        hermetic ZSTACK_HOME (update_check: false)
 ...
 *                             `$HOME/.claude/skills/zstack/bin/zstack-skill-start`
 *                             resolves here; sibling bins degrade gracefully
 *
 * The dispatch prompt instructs the ZSTACK_SESSION_KIND=spawned prefix; the
...
describeE2E('Spawned docsync JSON contract E2E (gate)', () => {
  describeIfSelected('Spawned docsync JSON contract', ['docsync-spawned'], () => {
    ...
    beforeAll(() => {
      workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-docsync-spawned-'));
      ...
      const plantedSkills = path.join(workDir, '.claude', 'skills', 'zstack');
      ...
      // $HOME/.claude/skills/zstack/bin/zstack-skill-start here ($0-relative
      // siblings like zstack-session-kind resolve too; the rest are
      // `|| true`-guarded and degrade silently).
      // filter: skip compiled binaries (a post-./setup bin/ carries the ~100MB
      // zstack-global-discover ELF; the scripts the preamble resolves are <2MB
      ...
      // Hermetic ZSTACK_HOME — update_check: false keeps the preamble off the
      // network (same gate the unit tests use).
      fs.mkdirSync(path.join(workDir, 'zstack-home'), { recursive: true });
      fs.writeFileSync(
        path.join(workDir, 'zstack-home', 'config.yaml'), 'update_check: false\n'
      );
      ...
      // resolve in place; state writes follow the child's ZSTACK_HOME.

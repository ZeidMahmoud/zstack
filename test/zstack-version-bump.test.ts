/**
 * Tests for the zstack-version-bump CLI (v2 plan T9 hybrid extraction). Covers
...
import { classifyState, VERSION_RE } from '../bin/zstack-version-bump';

const BIN = path.join(import.meta.dir, '..', 'bin', 'zstack-version-bump');
 ...
 * holding plain 3-digit semver — the shape zstack's native VERSION-file
 ...
 *
 * These cases pass --version-path explicitly; the .zstack/version-path pin
...
/**
 * #2462: cmdClassify's current-version read resolved the .zstack/version-path
 ...
 */
describe('.zstack/version-path pin, no --version-path flag (#2462)', () => {
  const mkPinned = (pinRel: string): string => {
    ...
    fs.mkdirSync(path.join(d, '.zstack'), { recursive: true });
    fs.writeFileSync(path.join(d, '.zstack', 'version-path'), pinRel + '\n');
    return d;
  };
...
describe('subdirectory manifest (no root package.json, #2531)', () => {
  ...
  const mk = (): string => {
    ...
    fs.mkdirSync(path.join(d, '.zstack'));
    fs.writeFileSync(path.join(d, '.zstack', 'package-json-path'), 'web/package.json\n');
...
describe('path containment: pins and flags cannot escape the repo', () => {
  // .zstack/version-path and .zstack/package-json-path are repo-controlled
  ...
  function resetRepo() {
    ...
    fs.mkdirSync(path.join(dir, '.zstack'), { recursive: true });
    fs.writeFileSync(path.join(dir, 'VERSION'), '1.0.0.0\n');
    fs.writeFileSync(victim, JSON.stringify({ version: '9.9.9' }, null, 2) + '\n');
  }

  test('a ../ escape in .zstack/version-path fails exit 2 and writes nothing', () => {
    resetRepo();
    fs.writeFileSync(path.join(dir, '.zstack', 'version-path'), '../victim.json\n');
  ...
  test('an absolute path in .zstack/package-json-path fails exit 2', () => {
    resetRepo();
    fs.writeFileSync(path.join(dir, '.zstack', 'package-json-path'), victim + '\n');
    const r = runFail(['write', '--version', '1.1.0.0']);
    expect(r.code).toBe(2);
    expect(r.stderr).toContain('outside the repository');
    expect(JSON.parse(fs.readFileSync(victim, 'utf-8')).version).toBe('9.9.9');
  ...
  test('an in-repo symlink pointing outside fails exit 2 and never follows', () => {
    ...
    fs.writeFileSync(path.join(dir, '.zstack', 'version-path'), 'link.json\n');
    const r = runFail(['write', '--version', '1.1.0.0']);
    expect(r.code).toBe(2);
    expect(r.stderr).toContain('outside the repository');
    expect(JSON.parse(fs.readFileSync(victim, 'utf-8')).version).toBe('9.9.9');
  ...
  test('legitimate subdirectory pins still work (containment is not over-broad)', () => {
    ...
    fs.writeFileSync(path.join(dir, '.zstack', 'version-path'), 'frontend/package.json\n');
...
describe('write --regen-digest regenerates the zstack agents digest (explicit opt-in)', () => {
  // The committed agents-digest/zstack-AGENTS.md embeds VERSION in its first
  ...
  const stubGenerator = (dir: string) => {
    ...
    fs.writeFileSync(path.join(dir, 'scripts', 'gen-agents-digest.ts'), [
      ...
      "fs.writeFileSync(path.join(root, 'agents-digest', 'zstack-AGENTS.md'), `# zstack digest v${v}\\n`);",
    ].join('\n'));
    fs.writeFileSync(path.join(dir, 'agents-digest', 'zstack-AGENTS.md'), '# zstack digest v1.0.0.0\n');
  };
  ...
  test('with the flag: a repo with the generator + committed digest gets a fresh digest', () => {
    ...
    expect(fs.readFileSync(path.join(dir, 'agents-digest', 'zstack-AGENTS.md'), 'utf-8'))
      .toBe('# zstack digest v1.1.0.0\n');
    fs.rmSync(dir, { recursive: true, force: true });
  });
  ...
  test('a generator failure warns and reports agentsDigest:false without failing the bump', () => {
    ...
    fs.writeFileSync(path.join(d2, 'agents-digest', 'zstack-AGENTS.md'), '# zstack digest v1.0.0.0\n');
  ...
  test('the REAL generator round-trips a bump: write --regen-digest restamps the digest first line', () => {
    ...
    fs.copyFileSync(
      path.join(root, 'agents-digest', 'zstack-AGENTS.md'),
      path.join(dir, 'agents-digest', 'zstack-AGENTS.md'),
    );
    ...
    expect(out.agentsDigest).toBe(true);
    const first = fs.readFileSync(path.join(dir, 'agents-digest', 'zstack-AGENTS.md'), 'utf-8').split('\n')[0];

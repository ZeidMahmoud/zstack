...
describeE2E('/autoplan native chain ordering (periodic)', () => {
  test(
    ...
    async () => {
      ...
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-autoplan-chain-'));
      ...
      try {
        ...
        const observe = () => {
          ...
          methodologyAudit = auditAutoplanMethodReads(publicTools, prompt =>
            loadAutoplanMethodologyBinding(prompt, [getHermeticDirs().runRoot, nativeState!.env.ZSTACK_HOME!]));

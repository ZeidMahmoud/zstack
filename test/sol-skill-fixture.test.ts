...
const base = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-sol-generation-test-'));
...
function sourceOutputs() {
  return ['.agents', 'zstack/llms.txt', 'agents-digest/zstack-AGENTS.md']
...
beforeAll(async () => {
  ...
  fullSkill = inventory(path.join(source, '.agents/skills/zstack-investigate'), false);
...
test('private Sol render matches the full in-place artifact and leaves cache bytes, metadata and links unchanged', async () => {
  ...
  try {
    ...
    expect(fs.existsSync(path.join(fixture.outputRoot, 'zstack/llms.txt'))).toBe(true);
    expect(fs.existsSync(path.join(fixture.outputRoot, 'agents-digest/zstack-AGENTS.md'))).toBe(true);

...
const required = process.env.ZSTACK_CSO_WINDOWS_TESTS === '1';
if (required && !windows) throw new Error('ZSTACK_CSO_WINDOWS_TESTS=1 requires native Windows; emulation does not qualify the launcher.');
let temporary = '', invocationCwd = '', launcher = '', core = '', marker = '', preload = '', publisher = '', buildStage = '';
beforeAll(() => {
  if (!windows) return;
  const installed = path.join(ROOT, 'bin', 'zstack-cso-launcher.exe');
  ...
  launcher = path.join(temporary, 'zstack-cso-launcher.exe');
  core = path.join(temporary, 'zstack-cso-core.exe');
...
if(process.argv.includes('--exit-23'))process.exit(23);`);
  ...
  buildStage = fs.mkdtempSync(path.join(ROOT, 'bin', '.zstack-cso-stage.windows-launcher-test.'));
  const stagedLauncher = path.join(buildStage, 'zstack-cso-launcher.exe');
  const stagedPublisher = path.join(buildStage, 'zstack-cso-publish-lock.exe');
  ...
  publisher = path.join(temporary, 'zstack-cso-publish-lock.exe');
  fs.copyFileSync(stagedPublisher, publisher);
  fs.writeFileSync(path.join(temporary, '.zstack-cso-generation.lock'), '');
  fs.writeFileSync(path.join(temporary, '.zstack-cso-generation'), `${digest}\n`);
...
function launchEnvironment(extra: Record<string, string> = {}) {
  return { ...process.env, HOME: temporary, ZSTACK_HOME: path.join(temporary, 'state'), PATH: temporary,
    NODE_OPTIONS: '--require=hostile', RUBYOPT: '-rhostile', PYTHONPATH: temporary,
    ZSTACK_CSO_SECRET_CANARY: 'must-not-cross-startup', ...extra };
}
...
describe('CSO native Windows build contract', () => {
  test('Windows builds use MSVC with a static CRT and no Bun-hosted public launcher', () => {
    ...
    expect(msvc).not.toContain("Join-Path $RepoRoot 'bin\\zstack-cso-launcher.exe'");
    ...
    expect(msvc).toContain('ZSTACK_CSO_CORE_SHA256');
    expect(msvc).toContain('ZSTACK_CSO_GIT_PATH');
    ...
    expect(launcherSource).toContain('.zstack-cso-generation.lock');
    expect(launcherSource).toContain('.zstack-cso-generation');
    expect(launcherSource).toContain('ZSTACK_CSO_CORE_SHA256');
  ...
  test('Windows CI runs the actual launcher tests rather than marking the platform supported from a source check', () => {
    ...
    expect(smoke.env.ZSTACK_CSO_WINDOWS_TESTS).toBe('1');
  ...
  const pathEscapeCases = [
    ['launcher outside stage', (stage: string, outside: string) => [path.join(outside, 'zstack-cso-launcher.exe'), path.join(stage, 'zstack-cso-publish-lock.exe')]],
    ['launcher in stage parent', (stage: string) => [path.join(stage, '..', 'zstack-cso-launcher.exe'), path.join(stage, 'zstack-cso-publish-lock.exe')]],
    ['wrong launcher name', (stage: string) => [path.join(stage, 'wrong.exe'), path.join(stage, 'zstack-cso-publish-lock.exe')]],
    ['lock outside stage', (stage: string, outside: string) => [path.join(stage, 'zstack-cso-launcher.exe'), path.join(outside, 'zstack-cso-publish-lock.exe')]],
    ['wrong lock name', (stage: string) => [path.join(stage, 'zstack-cso-launcher.exe'), path.join(stage, 'wrong-lock.exe')]],
  ] as const;
  test.skipIf(!windows).each(pathEscapeCases)('Windows build rejects %s before invoking MSVC',(_name, outputs)=>{
    ...
    const stage=fs.mkdtempSync(path.join(ROOT,'bin','.zstack-cso-stage.path-test.'));
...
(windows ? describe : describe.skip)('CSO native Windows startup', () => {
  test('BUN_OPTIONS cannot execute a preload before the environment is scrubbed', () => {
    ...
    expect(value.env.ZSTACK_CSO_SECRET_CANARY).toBeUndefined();
    expect(value.env.NODE_OPTIONS).toBeUndefined();
    expect(value.env.ZSTACK_HOME).toBe(path.join(temporary, 'state'));
  ...
  test('forwards child exit status and fails closed on oversized inherited input', () => {
    ...
    const result = launch([], { ZSTACK_HOME: 'x'.repeat(8193) });
  ...
  test('resolves launcher installation aliases before locating its sibling core', () => {
    ...
    const result = launch(['--version'], {}, path.join(alias, 'zstack-cso-launcher.exe'));
    expect(result.status).toBe(0);
  ...
  test('rejects a launcher/manifest generation mismatch before executing the core', () => {
    ...
    fs.copyFileSync(launcher, path.join(mismatched, 'zstack-cso-launcher.exe'));
    fs.copyFileSync(core, path.join(mismatched, 'zstack-cso-core.exe'));
    fs.writeFileSync(path.join(mismatched, '.zstack-cso-generation.lock'), '');
    fs.writeFileSync(path.join(mismatched, '.zstack-cso-generation'), `${'0'.repeat(64)}\n`);
    const result = launch(['--version'], {}, path.join(mismatched, 'zstack-cso-launcher.exe'));
    expect(result.status).toBe(69);
    expect(result.stdout).toBe('');
  ...
  test('rejects changed core bytes even when the generation manifest is unchanged', () => {
    ...
    const tamperedLauncher = path.join(tampered, 'zstack-cso-launcher.exe');
    const tamperedCore = path.join(tampered, 'zstack-cso-core.exe');
    fs.copyFileSync(launcher, tamperedLauncher);
    fs.copyFileSync(core, tamperedCore);
    fs.appendFileSync(tamperedCore, Buffer.from([0]));
    fs.writeFileSync(path.join(tampered, '.zstack-cso-generation.lock'), '');
    fs.copyFileSync(path.join(temporary, '.zstack-cso-generation'), path.join(tampered, '.zstack-cso-generation'));
  ...
  test('a missing core never falls back to a PATH executable or Bun', () => {
    ...
    const copy = path.join(missing, 'zstack-cso-launcher.exe');
    fs.copyFileSync(launcher, copy);
    fs.writeFileSync(path.join(missing, '.zstack-cso-generation.lock'), '');
    fs.copyFileSync(path.join(temporary, '.zstack-cso-generation'), path.join(missing, '.zstack-cso-generation'));
  ...
  test('the actual helper finds trusted Git and stores state under USERPROFILE without HOME', () => {
    ...
    const actual=path.join(ROOT,'bin','zstack-cso-launcher.exe'),env={...process.env,HOME:'',ZSTACK_HOME:'',CLAUDE_PLUGIN_ROOT:'',CLAUDE_PLUGIN_DATA:'',USERPROFILE:profile,PATH:temporary,NODE_OPTIONS:'--require=hostile'};
    ...
    const started=spawnSync(actual,['start','--repo',repository,'--offline'],{cwd:repository,encoding:'utf8',env,timeout:30_000});expectSuccessfulProcess(started,'zstack-cso start');expect(JSON.parse(started.stdout).schemaVersion).toBe(3);expect(fs.existsSync(path.join(profile,'.zstack','security','cso'))).toBe(true);
  }, 120_000);
  ...
  test('NTFS high file IDs survive repeated native commands and ambiguous old decisions remain blocked', () => {
    ...
    for(const args of [['add','app.js'],['commit','-qm','fixture']] as string[][]){const result=spawnSync(git,args,{cwd:repository,encoding:'utf8',env:gitEnv,timeout:10_000});expect(result.status).toBe(0);}
    const actual=path.join(ROOT,'bin','zstack-cso-launcher.exe'),env={...process.env,HOME:'',ZSTACK_HOME:'',CLAUDE_PLUGIN_ROOT:'',CLAUDE_PLUGIN_DATA:'',USERPROFILE:profile,PATH:temporary};
    ...
    for(let i=0;i<3;i++){
      const started=command(['start','--repo',repository,'--offline']);expectSuccessfulProcess(started,'zstack-cso start');
      const run=JSON.parse(started.stdout),dir=path.join(profile,'.zstack','security','cso',run.repoId,run.runId),leases=path.join(dir,'.mutation-lock-leases');
      const initialized=command(['resume',run.runId]);expectSuccessfulProcess(initialized,'zstack-cso initialize mutation lease');
      ...
      const inspected=command(['inspect',run.runId]);expectSuccessfulProcess(inspected,'zstack-cso inspect');
      expect(JSON.parse(inspected.stdout).report.status).toBe('running');
      const read=command(['read',run.runId,'app.js']);expectSuccessfulProcess(read,'zstack-cso read');expect(read.stdout).toContain('fixture');
      const history=command(['history',run.runId]);expectSuccessfulProcess(history,'zstack-cso history');
      const submitted=command(['submit',run.runId,submission]);expectSuccessfulProcess(submitted,'zstack-cso submit');
      const resumed=command(['resume',run.runId]);expectSuccessfulProcess(resumed,'zstack-cso resume');
      const finished=command(['finish',run.runId]);expectSuccessfulProcess(finished,'zstack-cso finish');
      ...
      const next=command(['start','--repo',repository,'--offline']);expectSuccessfulProcess(next,'zstack-cso start after legacy state');
  ...
  test('the actual helper rejects source mutation during snapshot capture without certifying a report', async () => {
    ...
    const actual=path.join(ROOT,'bin','zstack-cso-launcher.exe'),env={...process.env,HOME:'',ZSTACK_HOME:'',CLAUDE_PLUGIN_ROOT:'',CLAUDE_PLUGIN_DATA:'',USERPROFILE:profile,PATH:temporary},state=path.join(profile,'.zstack','security','cso');
    ...
    try{
      ...
      if(!filesNamed(state,'history-status.json').length)throw new Error('zstack-cso exited or timed out before reaching the bounded snapshot mutation point');
      ...
      if(!closed)throw new Error('zstack-cso did not finish after the injected snapshot mutation');

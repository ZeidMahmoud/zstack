...
describe('setup: Chromium bootstrap static invariants', () => {
  ...
  test('the download is deadline-bounded through the shared helper and env knob', () => {
    ...
    expect(codeLines).toContain('ZSTACK_PLAYWRIGHT_INSTALL_TIMEOUT');
  ...
  test('lock contention is a reason code, not a fatal', () => {
    ...
    expect(block).toContain('ZSTACK_SKIP_PLAYWRIGHT');
  });
  ...
  test('the daemon font refresh is skipped when Chromium is unavailable', () => {
    const emoji = slice(BLOCK_END, '# 3. Ensure ~/.zstack global state directory exists');
  ...
  test('the final summary names the affected skills and the reason', () => {
    ...
    expect(tail).toContain('ZSTACK_PLAYWRIGHT_INSTALL_TIMEOUT=1800');
    expect(tail).toContain('ZSTACK_CHROMIUM_NO_SANDBOX=1');
  });
});
...
function runBlock(opts: {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-pw-block-'));
  ...
  if (opts.preLockPid) {
    const lock = path.join(tmp, 'zstack-playwright-install.lock');
    fs.mkdirSync(lock);
...
function runKillTreeFallback(disappearingEntry = false): { status: number | null; stdout: string; stderr: string } {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-killtree-'));
  try {
...
describe('setup: Chromium bootstrap block executes best-effort', () => {
  ...
  test('install hangs: killed at the deadline with reason chromium-install-timeout, tree kill recorded', () => {
    const r = runBlock({
      ...
      env: { ZSTACK_PLAYWRIGHT_INSTALL_TIMEOUT: '1' },
    });
    expect(r.status).toBe(0);
  ...
  test('non-numeric timeout knob falls back to the default instead of erroring', () => {
    const r = runBlock({ probe: 'fail', bunx: 'exit 3', env: { ZSTACK_PLAYWRIGHT_INSTALL_TIMEOUT: 'soon' } });
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('REASON=chromium-install\n');
  });
  ...
  test('lock held by a live process: reason chromium-install-locked, setup continues (was: exit 1)', () => {
    ...
    expect(fs.existsSync(path.join(r.tmp, 'zstack-playwright-install.lock'))).toBe(true);
  });
  ...
  test('a lock held by a LIVE pid but older than the install bound is reclaimed (holder past its deadline, or a recycled pid)', () => {
    const r = runBlock({ probe: 'fail', bunx: 'exit 0', preLockPid: String(process.pid), preLockAgeMin: 30, env: { ZSTACK_PLAYWRIGHT_INSTALL_TIMEOUT: '60' } });
    expect(r.status).toBe(0);
    expect(fs.readFileSync(path.join(r.tmp, 'mark'), 'utf-8')).toContain('bunx-called');
    expect(r.stdout).not.toContain('chromium-install-locked');
  ...
  test('a lock dir with NO pid file is reclaimed once older than the install bound, and honored while fresh', () => {
    const old = runBlock({ probe: 'fail', bunx: 'exit 0', preLockNoPidAgeMin: 30, env: { ZSTACK_PLAYWRIGHT_INSTALL_TIMEOUT: '60' } });
    ...
    const fresh = runBlock({ probe: 'fail', bunx: 'exit 0', preLockNoPidAgeMin: 0, env: { ZSTACK_PLAYWRIGHT_INSTALL_TIMEOUT: '600' } });
  ...
  test('install succeeds but the post-install probe fails: reason post-install-launch with the userns hint', () => {
    ...
    expect(r.stderr).toContain('ZSTACK_CHROMIUM_NO_SANDBOX=1');
  });

  test('ZSTACK_SKIP_PLAYWRIGHT=1: reason skipped, installer never invoked (#913)', () => {
    const r = runBlock({ probe: 'fail', bunx: 'exit 0', env: { ZSTACK_SKIP_PLAYWRIGHT: '1' } });
    expect(r.status).toBe(0);
...
function pathWithoutNode(): { bin: string; cleanup: () => void } {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-nonode-'));
...
describe('setup: Chromium bootstrap block — fresh install, lock hygiene, override, Windows arms', () => {
  test('fresh install happy path: probe fails, install succeeds, re-probe passes → no reason, lock released, EXIT trap restored', () => {
    ...
    expect(fs.existsSync(path.join(r.tmp, 'zstack-playwright-install.lock'))).toBe(false);
  ...
  test('Windows with Node.js loading Playwright but the launch probe failing: Windows-specific post-install-launch hint', () => {
    ...
    expect(r.stderr).not.toContain('ZSTACK_CHROMIUM_NO_SANDBOX');
  });
});
...
function runSummary(reason: string, telemetry: 'ok' | 'fail' | 'missing', prelude: string[] = []): { stdout: string; stderr: string; status: number; argv: string } {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-pw-summary-'));
  try {
    const argvFile = path.join(tmp, 'telemetry.argv');
    if (telemetry !== 'missing') {
      fs.mkdirSync(path.join(tmp, 'bin'));
      const stub = path.join(tmp, 'bin', 'zstack-telemetry-log');
...
describe('setup: Chromium bootstrap summary block executes', () => {
  test('names the reason and the affected skills; hints are reason-specific; telemetry gets the reason code only', () => {
    ...
    expect(timeout.stdout).toContain('ZSTACK_PLAYWRIGHT_INSTALL_TIMEOUT=1800');
    expect(timeout.stdout).not.toContain('ZSTACK_CHROMIUM_NO_SANDBOX');
    ...
    expect(launch.stdout).toContain('ZSTACK_CHROMIUM_NO_SANDBOX=1');
    expect(launch.stdout).not.toContain('ZSTACK_PLAYWRIGHT_INSTALL_TIMEOUT=1800');
    ...
    expect(offline.stdout).not.toContain('ZSTACK_CHROMIUM_NO_SANDBOX');
    expect(offline.stdout).not.toContain('ZSTACK_PLAYWRIGHT_INSTALL_TIMEOUT=1800');
  });
  ...
  test('the summary names foreign entries left untouched and customized SKILL.md files moved to the backup root', () => {
    const r = runSummary('', 'missing', ['_FOREIGN_SKIPPED_ENTRIES=(qa)', '_BACKED_UP_SKILL_MDS=(ship review)', '_SKILL_BACKUP_ROOT="/tmp/zstack-bk/20260904"']);
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('Not registered (a skill you own already uses the name; left untouched): qa');
    expect(r.stdout).toContain("Moved 2 customized SKILL.md file(s) to /tmp/zstack-bk/20260904 before installing zstack's: ship review");
  ...
  test('an explicit opt-out (skipped) is reported as a choice, not a failure, and sends no telemetry', () => {
    ...
    expect(skipped.stdout).toContain('Chromium install skipped by request (ZSTACK_SKIP_PLAYWRIGHT=1)');
  ...
  test('ZSTACK_PLAYWRIGHT_INSTALL_TIMEOUT=0, 000, or a value past nine digits means the default, not kill-on-first-poll or unbounded', () => {
    for (const v of ['0', '000', '99999999999999999999', 'abc', '']) {
      const r = runBlock({ probe: 'fail', bunx: 'exit 3', env: { ZSTACK_PLAYWRIGHT_INSTALL_TIMEOUT: v } });
      expect(r.status).toBe(0);
      expect(r.stdout).toContain('REASON=chromium-install\n');
    }
    ...
    const bounded = runBlock({ probe: 'fail', bunx: 'sleep 30', env: { ZSTACK_PLAYWRIGHT_INSTALL_TIMEOUT: '0001' } });
    expect(bounded.stdout).toContain('REASON=chromium-install-timeout\n');
  }, 30_000);
});

/**
 * .zstack-owned marker (#2119): Windows installs COPY SKILL.md, so there is no
 * symlink to readlink. link_claude_skill_dirs writes the marker; zstack-relink
...
function runLinker(opts: {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-marker-'));
  ...
  const script = [
    ...
    `ZSTACK_USER_RENDER_DIR="${path.join(tmp, 'no-render')}"`,
...
describe.skipIf(process.platform === 'win32')('setup: .zstack-owned ownership marker for Windows copy installs (#2119)', () => {
  test('the marker is written beside a COPIED SKILL.md (IS_WINDOWS=1) and beside a symlinked one (IS_WINDOWS=0): a directory we created is ours on every platform', () => {
    ...
    try {
      ...
      for (const name of ['qa', 'ship']) {
        ...
        expect(fs.existsSync(path.join(win.skills, name, '.zstack-owned'))).toBe(true);
      }
    ...
    try {
      ...
      expect(fs.existsSync(path.join(unix.skills, 'qa', '.zstack-owned'))).toBe(true);
    } finally {
  ...
  test('marker writer → cleanup reader: copies the Windows linker marked are reaped on a mode flip; an unmarked same-name skill survives', () => {
    ...
    try {
      expect(r.status).toBe(0);
      expect(fs.existsSync(path.join(r.skills, 'qa', '.zstack-owned'))).toBe(true);
      expect(fs.existsSync(path.join(r.skills, 'ship', '.zstack-owned'))).toBe(true);
      ...
      const flip = runBashScript([
        'set -e', 'IS_WINDOWS=1',
        extractFn('_zstack_link_target_abs'), extractFn('_zstack_target_is_ours'), extractFn('_zstack_dir_only_links'), extractFn('_cleanup_linked_dir'), extractFn('_zstack_generated_header'), extractFn('_cleanup_weak_dir'), extractFn('_backup_skill_md'), '_BACKED_UP_SKILL_MDS=()', '_SKILL_BACKUP_ROOT="$HOME/.zstack/backups/skills/test"',
        extractFn('cleanup_old_claude_symlinks'),
      ...
      expect(fs.readdirSync(r.skills).sort()).toEqual(['zstack', 'my-own', 'review']);

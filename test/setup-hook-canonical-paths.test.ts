 ...
 *
 * Static tripwires over `setup` and `bin/zstack-settings-hook`. The defect
 * class these pin against: hook commands baked from the SETUP-TIME tree
 * (`$SOURCE_ZSTACK_DIR` = `pwd -P` of the running tree) into the user's
 ...
 *     install: ${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/zstack) — an
...
const hookBinSrc = fs.readFileSync(path.join(ROOT, 'bin', 'zstack-settings-hook'), 'utf-8');
const uninstallSrc = fs.readFileSync(path.join(ROOT, 'bin', 'zstack-uninstall'), 'utf-8');

describe('setup: canonical-only hook paths', () => {
  test('CANONICAL_ZSTACK_ROOT honors CLAUDE_CONFIG_DIR with ~/.claude fallback', () => {
    expect(setupSrc).toContain(
      'CANONICAL_ZSTACK_ROOT="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/zstack"',
    );
  });
  ...
  test('_hook_command_path body never references the running tree', () => {
    ...
    expect(body).not.toContain('SOURCE_ZSTACK_DIR');
    expect(body).toContain('CANONICAL_ZSTACK_ROOT');
  });

  test('every hook var routes through _hook_command_path; no raw SOURCE_ZSTACK_DIR hook assignment remains', () => {
    for (const rel of [
    ...
    // The bug: FOO_HOOK="$SOURCE_ZSTACK_DIR/hosts/claude/hooks/..."
    expect(setupSrc).not.toMatch(/="\$SOURCE_ZSTACK_DIR\/hosts\/claude\/hooks\//);
    expect(setupSrc).not.toMatch(/HOOK_CMD="(bash )?\$SOURCE_ZSTACK_DIR\/bin\/zstack-session-update"/);
  });

  test('SessionStart registers via schema-aware add-event under its identity source', () => {
    expect(setupSrc).toMatch(
      /add-event --event SessionStart --command "\$HOOK_CMD" --source zstack-session-update/,
    );
  });
...
describe('setup: heal-first ordering + visibility', () => {
  ...
  test('--no-team teardown includes the auq source and the identity sweep', () => {
    ...
    expect(slice).toContain('remove-source --source zstack-timeline-stop');
    expect(slice).toContain('prune-stale --all');
  });
});

describe('zstack-settings-hook: shared prelude (dedupe key == prune predicate)', () => {
...
describe('zstack-uninstall: hook cleanup runs before install-root deletion', () => {
  test('the settings cleanup block precedes every install-root rm -rf', () => {
    // Pre-fix bug: SETTINGS_HOOK=$(dirname "$0")/zstack-settings-hook resolved
    ...
    const cleanup = uninstallSrc.indexOf('Remove zstack hooks from Claude Code settings');
    const rootDelete = uninstallSrc.indexOf('rm -rf "$CLAUDE_SKILLS/zstack"');
...
describe('the defect-class warning is written down where the next author will see it', () => {
  test('setup carries the never-register-tree-relative-paths warning', () => {
    expect(setupSrc).toMatch(/NEVER register .*SOURCE_ZSTACK_DIR.*hook paths/);
  });
});

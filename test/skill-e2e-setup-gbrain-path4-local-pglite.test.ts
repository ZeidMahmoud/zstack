...
//   2. invoked zstack-gbrain-install (Step 4.5 Yes branch)
...
/**
 * Fake zstack-gbrain-install so we don't actually clone the gbrain repo +
...
exit 0
`;
  fs.writeFileSync(path.join(binDir, 'zstack-gbrain-install'), script, {
    mode: 0o755,
  });
  return callLog;
}
...
describeE2E('/setup-gbrain Path 4 + Step 4.5 Yes → local PGLite for code', () => {
  test('opt-in flow invokes install + gbrain init + remote MCP register', async () => {
    ...
    try {
      ...
      //   - zstack-gbrain-install (install step)

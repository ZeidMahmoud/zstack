/**
 * Regression: no stale `zstack-brain-init`, `gbrain_sync_mode`, or
 * `~/.zstack-brain-remote.txt` references survive the v1.27.0.0 rename.
 *
 ...
 *
 * Allowlist: the migration script (`zstack-upgrade/migrations/v1.27.0.0.sh`)
...
const ALLOWLIST = [
  ...
  'zstack-upgrade/migrations/v1.27.0.0.sh',
  // Older migration scripts — historical references; these document past state.
  'zstack-upgrade/migrations/v1.17.0.0.sh',
  ...
  'test/zstack-upgrade-migration-v1_17_0_0.test.ts',
  ...
  // The bin/zstack-config comment explicitly preserves the rename note.
  'bin/zstack-config',
  ...
  // zstack-upgrade.test.ts may exercise old migration behavior.
  'test/zstack-upgrade.test.ts',
  // This test itself references the patterns to grep for.
  'test/no-stale-zstack-brain-refs.test.ts',
  ...
  'test/zstack-artifacts-init.test.ts',
...
const SCAN_PATHS = [
  ...
  // a dead command (zstack-brain-init) survive ~36 releases as a
  // command-not-found instruction. Docs are user-facing surface; scan them.
  'docs/',
  'README.md',
  'USING_GBRAIN_WITH_ZSTACK.md',
];
...
describe('no stale zstack-brain refs (v1.27.0.0 rename)', () => {

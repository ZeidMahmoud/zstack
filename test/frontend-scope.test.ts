/**
 * lib/frontend-scope.ts mirrors the m_frontend arm of bin/zstack-diff-scope.
...
describe.skipIf(!POSIX)('parity with bin/zstack-diff-scope', () => {
  test('SCOPE_FRONTEND agrees with isFrontendPath for every sample, one file per diff', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-scope-parity-'));
    const git = (...a: string[]) => {
    ...
    try {
      ...
      for (const [rel, expected] of SAMPLES) {
        ...
        const r = spawnSync('bash', [path.join(ROOT, 'bin', 'zstack-diff-scope'), 'main'], { cwd: dir, encoding: 'utf-8', timeout: 30_000 });

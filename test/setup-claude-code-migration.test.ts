...
function fixture(copy: boolean) {
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-setup-rename-'));
  ...
  for (const bin of ['zstack-config', 'zstack-patch-names', 'zstack-relink', 'zstack-migrate-claude-code', 'zstack-claude-code']) {
  ...
  const migrationDir = path.join(ROOT, 'zstack-upgrade/migrations');
  for (const file of fs.readdirSync(migrationDir).filter(name => name.endsWith('.sh'))) {
    ...
    if (content.includes('zstack-migrate-claude-code')) put(path.join(root, 'zstack-upgrade/migrations', file), content, 0o755);
  }
  put(path.join(home, '.zstack/.last-setup-version'), '1.0.0.0\n');
  ...
  put(path.join(root, 'open-zstack-browser/SKILL.md.tmpl'), '---\nname: open-zstack-browser\ndescription: Fixture browser.\n---\nBrowser fixture.\n');
...
(await import('node:fs')).appendFileSync(process.env.FIXTURE_EVENTS!, 'generate:' + (fixtureIsolated ? 'isolated' : 'live') + ':defer=' + (process.env.ZSTACK_DEFER_CLAUDE_RENAME_PRUNE ?? '0') + '\\n');
...
if [ "$1" = run ] && [ "$2" = build ]; then
  ...
  for target in browse/dist/browse design/dist/design make-pdf/dist/pdf bin/zstack-cso-core bin/zstack-cso-launcher bin/zstack-cso-watchdog; do
    mkdir -p "$(dirname "$target")"
    printf '#!/usr/bin/env bash\\nexit 0\\n' > "$target"
    chmod +x "$target"
  done
  printf '%064d\\n' 0 > bin/.zstack-cso-generation
...
`, 0o755);
  ...
  put(path.join(oldRender, 'SKILL.md'), oldSkill('zstack-claude'));
  fs.mkdirSync(skills, { recursive: true });
  if (copy) put(path.join(skills, 'zstack-claude/SKILL.md'), oldSkill('zstack-claude'));
  else fs.symlinkSync(oldRender, path.join(skills, 'zstack-claude'));
  put(path.join(skills, 'zstack-review/SKILL.md'), oldSkill('zstack-review'));
  put(path.join(skills, 'zstack-review/notes.md'), 'user notes\n');
  put(path.join(codexHome, 'config.toml'), 'model = "gpt-5.6-sol"\n');
  const env = { ...process.env, HOME: home, CODEX_HOME: codexHome,
    ZSTACK_HOME: path.join(home, '.zstack'), ZSTACK_STATE_ROOT: path.join(home, '.zstack'),
    ZSTACK_SKIP_PLAYWRIGHT: '1', ZSTACK_SKIP_FONTS: '1', ZSTACK_SKIP_COREUTILS: '1', ZSTACK_SKIP_ASIDE: '1',
    ZSTACK_SKIP_GBRAIN_REGEN: '1', ZSTACK_PLAN_TUNE_HOOKS: 'no', ZSTACK_TIMELINE_STOP_HOOK: 'no',
...
describe.skipIf(process.platform === 'win32')('full setup repairs the installed Claude wrapper through another host', () => {
  for (const copy of [false, true]) {
    test(`Claude-only setup migrates existing Codex, survives later generation/linking, and repeats safely (copy=${copy})`, () => {
      ...
      try {
        ...
        expect(result.stdout).toContain('zstack ready (claude)');
        expect(result.stderr).toContain('/claude is now /claude-code');
        const next = path.join(f.skills, 'zstack-claude-code/SKILL.md');
        expect(fs.readFileSync(next, 'utf8')).toContain('name: claude-code');
        expect(fs.existsSync(path.join(f.skills, 'zstack-claude/SKILL.md'))).toBe(false);
        expect(fs.existsSync(f.oldRender)).toBe(false);
        expect(fs.readFileSync(path.join(f.skills, 'zstack-review/SKILL.md'), 'utf8')).toContain('zstack-claude-code');
        expect(fs.readFileSync(path.join(f.skills, 'zstack-review/notes.md'), 'utf8')).toBe('user notes\n');
        expect(fs.readFileSync(path.join(f.root, '.agents/skills/zstack-review/SKILL.md'), 'utf8')).toContain('Model-Specific Behavioral Patch (gpt-5.6-sol)');
        expect(fs.existsSync(path.join(f.skills, 'zstack/bin/zstack-claude-code'))).toBe(true);
        expect(fs.existsSync(path.join(f.skills, 'zstack/lib/outside-review-result.ts'))).toBe(true);
        ...
        const installed = spawnSync(process.execPath, [path.join(f.skills, 'zstack/bin/zstack-claude-code'),
          '--cwd', f.root, '--access', 'none', '--timeout-ms', '3000'], {
          env: { ...process.env, ZSTACK_CLAUDE_BIN: process.execPath, ZSTACK_CLAUDE_BIN_ARGS: JSON.stringify([fakeCli]) },
  ...
  test('failed rename keeps the working Codex entry through later build/prune and succeeds on retry', () => {
    ...
    try {
      ...
      expect(failed.stdout).toContain('zstack ready (claude)');
      expect(failed.stderr).toContain('fixture isolated generation failure');
      expect(fs.readFileSync(path.join(f.skills, 'zstack-claude/SKILL.md'), 'utf8')).toContain('old working installation');
      expect(fs.readFileSync(f.events, 'utf8')).toMatch(/generate:isolated:defer=1\nbuild\ngenerate:live:defer=1/);
      const retry = f.run();
      expect(retry.status).toBe(0);
      expect(fs.existsSync(path.join(f.skills, 'zstack-claude/SKILL.md'))).toBe(false);
      expect(fs.readFileSync(path.join(f.skills, 'zstack-claude-code/SKILL.md'), 'utf8')).toContain('name: claude-code');
    } finally { fs.rmSync(f.temp, { recursive: true, force: true }); }
  }, 60_000);
});

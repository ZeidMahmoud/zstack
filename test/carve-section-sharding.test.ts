...
describe('carved-skill cases each get a complete paid process budget', () => {
  ...
  test('explicit skill scope selects one process and records why the others are excluded', () => {
    ...
    const env = { ZSTACK_CARVE_SKILL: ' review ', EVALS_ALL: '1' };
    ...
    expect(result.excluded.every(entry => entry.reason.includes('ZSTACK_CARVE_SKILL=review'))).toBe(true);
    ...
    expect(() => selectPaidTestFiles(discovered, 'periodic', root, { ZSTACK_CARVE_SKILL: 'typo' })).toThrow('no generic section-loading wrapper');
  });
});

...
async function fixture(modes: Mode[], budget = 300_000, pathApi = path) {
  ...
  const bindings = {
    ...
    process: { env: { ZSTACK_EVAL_DIR: evidenceRoot, PATH: process.env.PATH, EVALS_MODEL: process.env.EVALS_MODEL } },
  ...
  try {
    for (callIndex = 0; callIndex < modes.length; callIndex++) {
      ...
      if (current.data.fixture.zstackHome) expect(fs.existsSync(current.data.fixture.zstackHome)).toBe(false);

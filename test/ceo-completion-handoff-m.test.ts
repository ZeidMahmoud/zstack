...
describe('CEO completion described by a native navigation choice', () => {
  ...
  test('completion placement is independent of the next-step wording and option order', () => {
    const call = handoff();
    const q = call.questions[0]!;
    q.question = 'D9 — Next steps: The review is done. Where should we go next? <zstack-qid:plan-ceo-review-next-step>';
  ...
  test('native identity, completion, required gate and exclusively administrative choices remain necessary', () => {
    for (const mutate of [
      (call: NativePlanQuestionCall) => { call.questions[0]!.question = 'Review complete only after fixing tests. What next? <zstack-qid:plan-ceo-review-next-step>'; },
      (call: NativePlanQuestionCall) => { call.questions[0]!.question = 'Should we finish reviewing? <zstack-qid:plan-ceo-review-next-step>'; },
      (call: NativePlanQuestionCall) => { call.questions[0]!.question += ' <zstack-qid:plan-ceo-security-issue>'; },
      (call: NativePlanQuestionCall) => { call.questions[0]!.question = '<zstack-qid broken> ' + call.questions[0]!.question; },
...
describe('native next-review navigation with a resolved CEO recap', () => {
  ...
  test('partial, conditional, quoted or still-open recaps never establish this navigation boundary', () => {
    ...
    for (const question of [
      ...
      const call = retryHandoff();
      call.questions[0]!.question = question + ' <zstack-qid:plan-ceo-next-step>';
      reanswer(call);
    ...
    for (const mutate of [
      (call: NativePlanQuestionCall) => { call.questions[0]!.question = 'Choose a fix for the missing test <zstack-qid:plan-ceo-next-step>'; },
      ...
      (call: NativePlanQuestionCall) => { call.questions[0]!.question = call.questions[0]!.question.replace(/ <zstack-qid:[^>]+>/, ''); },
...
describe('CEO completed next-step identity in native option order', () => {
  ...
  test('the observed identity cannot excuse unfinished work, a finding or a malformed native call', () => {
    for (const mutate of [
      ...
      (call: NativePlanQuestionCall) => { call.questions[0]!.question += ' <zstack-qid:ceo-plan-next-steps>'; },
      (call: NativePlanQuestionCall) => { call.questions[0]!.header = 'TODO'; },

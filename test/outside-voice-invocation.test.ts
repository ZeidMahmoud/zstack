...
function environment(host: 'codex' | 'claude'): NodeJS.ProcessEnv {
  return {...process.env, ZSTACK_ROOT:ROOT, ZSTACK_BIN:path.join(ROOT,'bin'),
    ZSTACK_CLAUDE_BIN:process.execPath, ZSTACK_CLAUDE_BIN_ARGS:JSON.stringify([FAKE_CLAUDE]),
    CAPTURE, FAKE_PROVIDER:host === 'codex' ? 'claude-code' : 'codex',
    CODEX_THREAD_ID:host === 'codex' ? 'codex-fixture' : '', CODEX_SANDBOX:'',
    CLAUDECODE:host === 'claude' ? '1' : '', ZSTACK_ACTIVE_HOST:host,
...
describe('generated outside-review dispatch', () => {
  ...
  for (const host of ['codex','claude'] as const) {
    ...
    test(`${host} reports mixed explicit/native conflicts without guessing a repair host`, () => {
      for (const markers of [
        {CLAUDECODE:'1', CODEX_THREAD_ID:'', CODEX_SANDBOX:'', ZSTACK_ACTIVE_HOST:'codex'},
        {CLAUDECODE:'', CODEX_THREAD_ID:'codex-fixture', CODEX_SANDBOX:'', ZSTACK_ACTIVE_HOST:'claude'},
      ]) {
    ...
    test(`${host} detects explicit stale active-host identity without environment marker guesses`, () => {
      const result = invoke(host,{},host === 'codex'
        ? {CLAUDECODE:'',CODEX_THREAD_ID:'',ZSTACK_ACTIVE_HOST:'claude'}
        : {CLAUDECODE:'',CODEX_THREAD_ID:'',ZSTACK_ACTIVE_HOST:'codex'});
      expect(result.status).toBe(78);
      expect(fs.existsSync(CAPTURE)).toBe(false);
  ...
  test('a configured but missing Claude binary reports unavailable coverage', () => {
    const result = invoke('codex',{}, {ZSTACK_CLAUDE_BIN:path.join(TMP,'missing')});
    expect(result.status).toBe(1);
  ...
  test('autoplan retains its Codex timeout event and hang record', () => {
    ...
    const probe = path.join(BIN, 'zstack-codex-probe');
    fs.writeFileSync(probe, `_zstack_codex_timeout_wrapper() { echo 'Partial finding'; return 124; }
_zstack_codex_log_event() { printf '%s %s\\n' "$1" "$2" >> "$FAKE_EVENTS"; }
_zstack_codex_log_hang() { printf '%s %s\\n' "$1" "$2" >> "$FAKE_EVENTS"; }
`);

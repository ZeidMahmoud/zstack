 ...
 *
 * Verifies the PreToolUse hook reads ~/.zstack/free-text-memory.json and
...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-memcache-'));
  ...
  test('injects matching nugget into additionalContext on defer', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<zstack-qid:ship-todos-reorganize> Reorganize?',
...
describe('per-session memory cache', () => {
  test('first read writes cache; subsequent reads use cache', () => {
    ...
    runHook({
      ...
      tool_input: {
        questions: [
          { question: '<zstack-qid:ship-todos-reorganize> Q', options: ['A', 'B'] },
        ],
      },
    });

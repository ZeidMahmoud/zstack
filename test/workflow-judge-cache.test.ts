...
function fixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-judge-cache-')); roots.push(root);
  const files = {
...
test('runtime/model/threshold changes miss, and retries never reuse or publish', () => {
  const f = fixture(); f.cache().publish(scores);
  for (const overrides of [{ ZSTACK_EVAL_MODEL_JUDGE: 'different-model' }, { EVALS_CACHE_RUNTIME_ID: 'c'.repeat(64) }]) {

 ...
 * zstack's own stack — `$B` headed mode + handoff/resume, ZStack Browser —
 ...
 *    the Aside-first option set with the zstack drive as fallback.
...
describe('consent offer extraction', () => {
  ...
  test('an unavailable-option explanation is not an offer', () => {
    expect(asideDriveOptions(`B) I drive it in zstack's own visible browser
...
describe("THIRD_PARTY_ACTIONS contract pins", () => {
  ...
  // macOS floor, and zstack's own stack as the fallback on every platform.
  test("names Aside as the recommended driver, zstack's stack as the fallback", () => {
    ...
    expect(section).toContain("The fallback driver on any platform is zstack's own stack");
    expect(section).toContain("ZStack Browser when installed");
  });
  ...
  // (g) failure path: verbatim-but-redacted error, one retry, then the zstack
  ...
  test("drive failure path: quote, redact, retry once, fresh-consent zstack drive or manual", () => {
    ...
    expect(section).toContain("then offer the zstack drive as a fresh consent question or fall back to manual steps");
  ...
  // `$B handoff` in zstack's own browser — never through the agent.
  ...
  // (k) no silent driver switches — the zstack drive is a new consent question.
  ...
  // Aside first, zstack's stack as fallback: the four-option question when
  // Aside is detected, the zstack drive / manual / defer trio when it is not.
  test("Aside-first option set; absent Aside degrades to the zstack drive, manual, or defer", () => {
    expect(section).toContain("A) I drive it in your Aside browser — your real logged-in sessions (recommended), B) I drive it in zstack's own visible browser — you take over for sign-in, C) manual instructions, D) defer");
    expect(section).toContain("When Aside is not detected, offer only the zstack drive / manual / defer options");
  ...
  test("cookbook shape for driving; --help for flags; contract overrides vendor", () => {
    ...
    expect(section).toContain("ZSTACK_STEP_OK");
  });
});

...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-stop-hook-home-'));
  projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-stop-hook-proj-'));
...
function runHook(stdin: string): { exitCode: number; stdout: string; stderr: string } {
  const r = spawnSync('bash', [HOOK], {
    ...
    env: {
      ...process.env,
      ZSTACK_HOME: tmpHome,
      ZSTACK_PROJECT_SLUG: SLUG, // deterministic slug, no git required
    },
    timeout: 15_000,
  });
...
describe('timeline-stop-hook wiring', () => {
  ...
  test('setup registers the Stop hook with its own source tag and tears it down on --no-team', () => {
    ...
    expect(setup).toContain('--source zstack-timeline-stop');
    ...
    expect(teardown).toContain('remove-source --source zstack-timeline-stop');
  });
  ...
  test('setup surfaces a settings-hook refusal instead of swallowing it', () => {
    ...
    expect(setup).not.toMatch(/ensure-event[\s\S]{0,220}--source zstack-timeline-stop[\s\S]{0,40}2>\/dev\/null/);
  });
  ...
  test('setup routes the Stop hook through ensure-event, not presence-only dedup', () => {
    ...
    expect(setup).toMatch(/ensure-event[\s\S]{0,220}--source zstack-timeline-stop/);
    ...
    expect(setup).not.toMatch(/list-sources 2>\/dev\/null \| grep -q "zstack-timeline-stop"/);
  });
  ...
  test('hook path resolution is canonical-only: global install or skip, never the worktree', () => {
    ...
    const fakeHome = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-hookpath-'));
    try {
      const canonicalRoot = path.join(fakeHome, '.claude', 'skills', 'zstack');
      ...
      const env = {
        ...process.env,
        HOME: fakeHome,
        SOURCE_ZSTACK_DIR: '/some/dev/worktree',
        CANONICAL_ZSTACK_ROOT: canonicalRoot,
      };
  ...
  test('ensure-event re-points a stale absolute path and leaves exactly one registration', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-ensure-'));
    try {
  ...
  test('ensure-event is a true no-op when the registration already matches (no write, no backup churn)', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-ensure-noop-'));
    try {
      const settingsFile = path.join(dir, 'settings.json');
      const args = [
        SETTINGS_HOOK, 'ensure-event',
      ...
      const env = { ...process.env, ZSTACK_SETTINGS_FILE: settingsFile };
  ...
  test('corrupt settings.json: ensure-event refuses (exit 3) and never rewrites the file', () => {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-ensure-corrupt-'));
    try {
      const settingsFile = path.join(dir, 'settings.json');
  ...
  test('a matcher change updates the tagged entry in place — still exactly one registration', () => {
    // Identity key is (event, source): an existing zstack entry with a STALE
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-ensure-matcher-'));
    try {
      const settingsFile = path.join(dir, 'settings.json');
      fs.writeFileSync(settingsFile, JSON.stringify({
        hooks: {
          PreToolUse: [{
            _zstack_source: 'zstack-plan-tune',
            matcher: 'OldMatcher',
      ...
      expect(s.hooks.PreToolUse[0]._zstack_source).toBe('zstack-plan-tune');
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  test('a failed update leaves exactly one registration — never zero, never two', () => {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-ensure-fail-'));
    try {
      const settingsFile = path.join(dir, 'settings.json');
      fs.writeFileSync(settingsFile, JSON.stringify({
        hooks: {
  ...
  test('zstack-uninstall removes the Stop hook registration', () => {
    const uninstall = fs.readFileSync(path.join(ROOT, 'bin', 'zstack-uninstall'), 'utf-8');
    expect(uninstall).toContain('remove-source --source zstack-timeline-stop');
  });
  ...
  test('the bash shim is fail-open: exits 0 even when bun is unavailable', () => {
    const r = spawnSync('bash', [HOOK], {
      input: '{}',
      encoding: 'utf-8',
      env: { HOME: tmpHome, PATH: '/usr/bin:/bin', ZSTACK_HOME: tmpHome },
      timeout: 15_000,
    });
    expect(r.status).toBe(0);
  });

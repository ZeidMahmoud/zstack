...
function replay() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-ap-command-'));
...
function nativeCards(r: Replay): string {
  ...
  return [
    `● Update(~/.zstack/${relative})`, ' ', '● Updated plan', ' ', '● Updated plan', ' ',
    '● Bash(mkdir -p ~/.zstack/analytics',
...
const nativeScreens: Array<[string, (s: string) => string]> = [
  ['foreign Update title', s => s.replace('Update(~/.zstack/', 'Update(/foreign/')],
  ...
  ['foreign active panel', s => s.replace('projects/zstack-autoplan-chain-zmFsqo/', 'projects/foreign/')],
...
const screens: Array<[string, (s: string) => string]> = [
  ...
  ['second edit panel', s => s + '\n' + panel(s)],
  ['foreign panel path', s => s.replace('projects/zstack-autoplan-chain-zmFsqo/', 'projects/another-project/')],

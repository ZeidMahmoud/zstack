...
function fixture() {
  ...
  const question = (text = 'D1 — Cross-project learnings scope <zstack-qid:learnings-cross-project>') => ({
...
describe('native plan-count transcripts', () => {
  ...
  test('CEO first-finding fallback requires native finding identity and rejects setup decisions', () => {
    ...
    for (const [id, text] of [
      ['setup', 'D1 — Missing context: choose a scope <zstack-qid:plan-ceo-review-scope>'],
      ['mode', 'D1 — Missing context: choose a mode <zstack-qid:plan-ceo-review-mode>'],
      ['unscoped', 'D1 — Missing receipt schema'],
      ['arbitrary', 'D1 — Pick an option <zstack-qid:plan-ceo-review-choice>'],
      ['next', 'D1 — Missing engineering review: what next? <zstack-qid:plan-ceo-next-steps>'],
    ]) {
  ...
  test('requires a matching successful answer and preserves full native question metadata', () => {
    ...
    expect(fp.promptSnippet).toContain('<zstack-qid:learnings-cross-project>');

...
describe('hermetic seeded PTY runtime', () => {
  ...
  test.skipIf(process.platform === 'win32')('uses current lazy files and tools with scoped access, preserving auth, caches, and explicit overrides', async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-runtime-probe-'));
    const operatorHome = path.join(dir, 'operator-home');
    const oldRoot = path.join(operatorHome, '.claude', 'skills', 'zstack');
    ...
    fs.writeFileSync(path.join(oldRoot, 'bin/zstack-config'), '#!/bin/sh\necho stale-runtime\n', { mode:0o755 });
...
const root = path.join(process.env.HOME,'.claude','skills','zstack');
...
const stateDirectory = path.join(process.env.HOME,'.zstack');
...
const phase = spawnSync('bash',['-c','cat ~/.claude/skills/zstack/autoplan/sections/ceo-phase.md'],{timeout:5000});
const config = spawnSync('bash',['-c','"$HOME/.claude/skills/zstack/bin/zstack-config" get codex_reviews'],{timeout:5000});
const canonicalConfig = process.env.CLAUDE_CONFIG_DIR && spawnSync('bash',['-c','"$CLAUDE_CONFIG_DIR/skills/zstack/bin/zstack-config" get codex_reviews'],{timeout:5000});
...
const record = {
  ...
  configDir:process.env.CLAUDE_CONFIG_DIR, state:process.env.ZSTACK_HOME,
  ...
  discovery: ['plan-design-review/sections/review-sections.md', 'plan-devex-review/dx-hall-of-fame.md', 'plan-devex-review/sections/review-sections.md', 'review/checklist.md'].map(relative => {
    ...
    const configRuntimePath = path.join(process.env.CLAUDE_CONFIG_DIR || path.join(process.env.HOME, '.claude'), 'skills', 'zstack', relative);
...
const session = await launchClaudePty({seedSkills:${item.seed},timeoutMs:10000,
  model:'unchanged-claude-model',env:${JSON.stringify({ZSTACK_HOME:state,RUNTIME_RECORD:record,RUNTIME_FAIL:item.failure?'1':'0',...item.overrides})}});
try {
  let failed = false;
...
`);
        ...
        if (item.owned) {
          ...
          // Actual Autoplan snapshots live under generated HOME/.zstack, outside
          // the runtime checkout. Approve only that owned state tree, not HOME,
          // an inherited/explicit ZSTACK_HOME, or a broad policy-setting answer.
          ...
          expect(result.addDirs).toEqual([path.join(result.home,'.claude','skills','zstack'),path.join(result.home,'.zstack'),path.join(result.configDir,'skills')]);
...
describe('hermetic skill runtime', () => {
  test('all global forms and variable-based reads use current bins/assets; project fallbacks stay literal', () => {
    fixture((source, privateDir, home) => {
      const script = [
        'ROOT=~/.claude/skills/zstack',
        '"$HOME/.claude/skills/zstack/bin/version"',
        '"${HOME}/.claude/skills/zstack/bin/version"',
        '~/.claude/skills/zstack/bin/version',
        'cat "$ROOT/alpha/sections/phase.md"',
      ].join('\n');
      write(path.join(source, 'alpha', 'SKILL.md'), script + '\n');
      write(path.join(source, 'alpha', 'sections', 'phase.md'), 'Read ~/.claude/skills/zstack/alpha/SKILL.md\n');
      write(path.join(source, 'alpha', 'sections', 'fallbacks.md'), '.claude/skills/zstack\n$_ROOT/.claude/skills/zstack\n~/.claude/skills/zstack-upgrade\n');
      write(path.join(source, 'bin', 'version'), '#!/usr/bin/env bash\nROOT=$(cd "$(dirname "$0")/.." && pwd)\ncat "$ROOT/VERSION"\n');
      fs.chmodSync(path.join(source, 'bin', 'version'), 0o755);
      const old = path.join(home, '.claude', 'skills', 'zstack', 'bin', 'version');
  ...
  test('later refreshes preserve paths and config while updating, adding, and retiring live documents', () => {
    fixture((source, privateDir) => {
      ...
      expect(fs.readdirSync(path.join(config, 'skills')).sort()).toEqual(['_zstack-command', 'alpha', 'beta']);
  ...
  test('real runtime preamble and update helper resolve current bins, VERSION, and git metadata', () => {
    fixture((_source, _privateDir, home) => {
      ...
      const old = path.join(home, '.claude', 'skills', 'zstack', 'bin', 'zstack-skill-start');
      write(old, '#!/usr/bin/env bash\nprintf STALE_OPERATOR\n');
      fs.chmodSync(old, 0o755);
      ...
      const env = { PATH: `${commands}${path.delimiter}${process.env.PATH!}`, HOME: runtimeHome, ZSTACK_HOME: state };
      ...
      expect(execFileSync(path.join(runtime, 'bin', 'zstack-update-check'), [], { cwd: home, env: { ...env, ZSTACK_STATE_DIR: state }, encoding: 'utf8', timeout: 10_000 }))

...
describe('setup Codex model activation', () => {
  ...
  test('resolves the profile once, fails closed, and passes it as quoted argv', () => {
    ...
    const guardAt = setup.indexOf('zstack setup failed: Codex model resolver returned no model');
  ...
  test('Kiro uses its own host render without rewriting the Codex profile', () => {
    ...
    expect(block).toContain('KIRO_DIR="$SOURCE_ZSTACK_DIR/.kiro/skills"');
  ...
  test('Kiro installs its native root, upgrade skill, and bin/lib runtime assets', () => {
    ...
    const kiroEnd = setup.indexOf('# 6b.', kiroStart);
    const block = setup.slice(kiroStart, kiroEnd);
    expect(block).toContain('_link_or_copy "$KIRO_DIR/zstack-upgrade/SKILL.md"');
    expect(block).toContain('_link_or_copy "$KIRO_DIR/zstack/SKILL.md"');
    expect(block).toContain('_link_or_copy "$SOURCE_ZSTACK_DIR/bin" "$KIRO_ZSTACK/bin"');
    expect(block).toContain('_link_or_copy "$SOURCE_ZSTACK_DIR/lib" "$KIRO_ZSTACK/lib"');
...
describe('Sol E2E tree hygiene', () => {
  ...
  test('renders Sol in its own output tree without changing the installed profile', () => {
    ...
    expect(fixture).toContain("path.join(outputRoot, '.agents', 'skills', 'zstack-investigate')");

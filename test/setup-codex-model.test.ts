...
describe('setup Codex model activation', () => {
  ...
  test('Kiro rewrites the codex-rendered SETUP_COMMAND and never symlinks zstack-upgrade', () => {
    // The artifact Kiro copies was rendered for the codex host, so its
    // zstack-upgrade skill bakes in './setup --host codex'. Every copy path
    // must rewrite it to '--host kiro', and the KIRO_ZSTACK zstack-upgrade
    ...
    expect(block).not.toContain('_link_or_copy "$AGENTS_DIR/zstack-upgrade/SKILL.md"');
  });
...
describe('Sol E2E tree hygiene', () => {
  ...
  test('snapshots and restores the exact prior .agents tree around the Sol render', () => {
    ...
    // default profile.
    const backupAt = solTest.indexOf('zstack-agents-backup-');

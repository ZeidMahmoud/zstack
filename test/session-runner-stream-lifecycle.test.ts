...
beforeAll(() => {
  ...
  const child = spawnSync(process.execPath, ['test', childScript], {
    ...
    env: {
      ...
      ZSTACK_HOME: path.join(fixture, 'state'), CODEX_HOME: path.join(fixture, 'codex'),
...
for(const scenario of cases) {
 ...
 const diagnosticPath = path.join(cwd, '.zstack/test-transcripts', scenario.id + '-failure.json');

...
// bin/zstack-skill-start / bin/zstack-skill-end. The render carries a one-line
...
// fence + prose. Script behavior is pinned by test/zstack-skill-start.test.ts.
const SKILL_START_SCRIPT = fs.readFileSync(path.join(ROOT, 'bin', 'zstack-skill-start'), 'utf-8');
const SKILL_END_SCRIPT = fs.readFileSync(path.join(ROOT, 'bin', 'zstack-skill-end'), 'utf-8');
...
const ALL_SKILLS = (() => {
  ...
  if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
    skills.push({ dir: '.', name: 'root zstack' });
  }
...
const EXTERNAL_OUT = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-gen-docs-out-'));
{
...
describe('gen-skill-docs', () => {
  ...
  test('package.json version matches VERSION file (npm-valid translation)', () => {
    ...
    // (grandfathered until the next write), matching zstack-version-bump's
    // own drift contract.
  ...
  test('generated SKILL.md contains operational self-improvement (replaced contributor mode)', () => {
    ...
    expect(SKILL_START_SCRIPT).toContain('zstack-learnings-search" --limit 3');
  });
  ...
  test('session awareness lives in zstack-skill-start (registry touch + stale cleanup)', () => {
  ...
  test('branch detection lives in zstack-skill-start and is echoed as BRANCH', () => {
  ...
  // zstack-skill-start's instruction layer ONLY when the update-check binary
  ...
  test('update_check opt-out gates the update binary and upgrade-flow emission (issue #2001)', () => {
    // The config-echo cluster lives in zstack-skill-start: the flag is still
    ...
    const updateCheck = fs.readFileSync(path.join(ROOT, 'bin', 'zstack-update-check'), 'utf-8');
  ...
  test('telemetry producer lives in the scripts; render documents the analytics sink', () => {
    ...
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('~/.zstack/analytics');
  });
  ...
  test('plan-review generated preambles stay under the Option A budget', () => {
    ...
    // resolver gained the <zstack-qid:...> marker convention + the
  ...
  test('slim voice section preserves the zstack voice contract', () => {
    const content = readSkillUnion('plan-eng-review'); // carved: review body moved to section
    const voice = extractMarkdownSection(content, '## Voice');
  ...
  test('preamble .pending-* glob is zsh-safe (uses find, not shell glob)', () => {
    for (const skill of CLAUDE_GENERATED_SKILLS) {
      ...
      expect(content).toContain("find ~/.zstack/analytics -maxdepth 1 -name '.pending-*'");
    }
  });
  ...
  test('preamble-using skills have correct skill name in telemetry', () => {
    ...
    for (const skill of PREAMBLE_SKILLS) {
      const content = fs.readFileSync(path.join(ROOT, skill.dir, 'SKILL.md'), 'utf-8');
      // The skill name now travels as --skill into zstack-skill-start (the
      // preamble fence) and zstack-skill-end (the telemetry epilogue) — the
      ...
      expect(content, `${skill.dir} epilogue must pass its own name`)
        .toContain(`zstack-skill-end --skill "${skill.name}"`);
    }
  });
...
describe('REVIEW_DASHBOARD resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`review dashboard appears in ${skill} generated file`, () => {
      ...
      expect(content).toContain('zstack-review');
  ...
  test('plan-ceo-review chaining mentions eng and design reviews', () => {
    ...
    // bin/zstack-skill-start in token-reduction Phase 2.)
...
describe('PLAN_FILE_REVIEW_REPORT resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`plan file review report appears in ${skill} generated file`, () => {
      ...
      expect(content).toContain('ZSTACK REVIEW REPORT');
    });
  }
...
describe('Codex filesystem boundary', () => {
  ...
  test('codex skill has rabbit-hole detection rule', () => {
    ...
    expect(content).toContain('zstack-update-check');
  ...
  test('autoplan boundary text avoids host-specific paths for cross-host compatibility', () => {
    ...
    // autoplan template uses generic 'skills/zstack' pattern instead of host-specific
    ...
    expect(boundarySection).toContain('skills/zstack');
...
describe('preamble routing injection (bin/zstack-skill-start emission layer)', () => {
  // Token-reduction Phase 2: the routing-injection prose left the rendered
  // preamble entirely — bin/zstack-skill-start probes, gates, and emits the
  // whole flow as a ZSTACK_INSTRUCTION block (with the AUQ, the routing rules
  ...
  test('routing probe checks CLAUDE.md and AGENTS.md (now in zstack-skill-start)', () => {
  ...
  test('script reads and echoes routing_declined config', () => {
    expect(SKILL_START_SCRIPT).toMatch(/_ROUTING_DECLINED=\$\("\$_BIN\/zstack-config" get routing_declined/);
  ...
  test('routing injection respects prior decline (gate + in-block ack)', () => {
    ...
    expect(routingBlock).toContain('re-enable with `__BIN__/zstack-config set routing_declined false`');
  });
...
describe('DESIGN_REVIEW_LITE extended with Codex', () => {
  ...
  test('design-checklist path uses installed zstack/review root (#2694)', () => {
    // #2694: generateDesignReviewLite used to emit
    // `.claude/skills/review/design-checklist.md` (missing the zstack/ segment).
    // After install the file lives at ~/.claude/skills/zstack/review/design-checklist.md.
    // The bad relative form must not appear — the good path does not contain it
    // as a substring because `zstack/` sits between `skills/` and `review/`.
    expect(content).toContain('~/.claude/skills/zstack/review/design-checklist.md');
...
describe('Codex generation (--host codex)', () => {
  ...
  const CODEX_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      skills.push({ dir: '.', codexName: 'zstack' });
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      if (entry.name === 'codex') continue; // /codex is excluded from Codex output
      if (!fs.existsSync(path.join(ROOT, entry.name, 'SKILL.md.tmpl'))) continue;
      const codexName = entry.name.startsWith('zstack-') ? entry.name : `zstack-${entry.name}`;
  ...
  test('root zstack bundle has OpenAI metadata for Codex skill browsing', () => {
    ...
    expect(content).toContain('display_name: "zstack"');
    expect(content).toContain('Use $zstack to locate the bundled zstack skills.');
    expect(content).toContain('allow_implicit_invocation: true');
  });

  test('externalSkillName mapping: root is zstack, others are zstack-{dir}', () => {
    // Root → zstack
    expect(fs.existsSync(path.join(AGENTS_DIR, 'zstack', 'SKILL.md'))).toBe(true);
    // Subdirectories → zstack-{dir}
    expect(fs.existsSync(path.join(AGENTS_DIR, 'zstack-review', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'zstack-ship', 'SKILL.md'))).toBe(true);
    // zstack-upgrade doesn't double-prefix
    expect(fs.existsSync(path.join(AGENTS_DIR, 'zstack-upgrade', 'SKILL.md'))).toBe(true);
    // No double-prefix: zstack-zstack-upgrade must NOT exist
    expect(fs.existsSync(path.join(AGENTS_DIR, 'zstack-zstack-upgrade', 'SKILL.md'))).toBe(false);
  });
  ...
  test('/codex skill excluded from Codex output', () => {
    expect(fs.existsSync(path.join(AGENTS_DIR, 'zstack-codex', 'SKILL.md'))).toBe(false);
  ...
  test('multiline descriptions preserved in Codex output', () => {
    ...
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'zstack-office-hours', 'SKILL.md'), 'utf-8');
    const fmEnd = content.indexOf('\n---', 4);
    const frontmatter = content.slice(4, fmEnd);
  ...
  test('hook skills have safety prose and no hooks: in frontmatter', () => {
    const HOOK_SKILLS = ['zstack-careful', 'zstack-freeze', 'zstack-guard'];
  ...
  test('Codex preamble resolves runtime assets from repo-local or global zstack roots', () => {
    // Check a skill that has a preamble (review is a good candidate)
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'zstack-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('ZSTACK_ROOT');
    expect(content).toContain('$_ROOT/.agents/skills/zstack');
    // Phase 1/2: config reads moved into zstack-skill-start — the fence itself
    // is the bin asset the preamble must resolve through $ZSTACK_BIN, and the
    // question-preference runtime call still resolves the same way.
    expect(content).toContain('$ZSTACK_BIN/zstack-skill-start');
    expect(content).toContain('$ZSTACK_BIN/zstack-question-preference');
    ...
    expect(SKILL_START_SCRIPT).toContain('$_ROOT_DIR/zstack-upgrade/SKILL.md');
    expect(SKILL_START_SCRIPT).toContain('_ROOT_DIR=$(dirname "$_BIN")');
    expect(content).not.toContain('~/.codex/skills/zstack/bin/zstack-config get telemetry');
  });

  // ─── Path rewriting regression tests ─────────────────────────

  test('sidecar paths resolve through $ZSTACK_ROOT (not zstack-review/)', () => {
    // #2518: templates now anchor sidecars at the installed skill root
    // (~/.claude/skills/zstack/review/...), which the codex path rewrite turns
    // into $ZSTACK_ROOT/review/... — resolved by the preamble against the
    // repo-local .agents root or the global install. The old repo-relative
    // form (.claude/skills/review/) only resolved inside zstack's own checkout.
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'zstack-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('$ZSTACK_ROOT/review/checklist.md');
    // design-checklist.md is now referenced via Review Army specialist (Claude only, stripped for Codex)
    // Wrong: must NOT reference zstack-review/checklist.md (file doesn't exist there)
    expect(content).not.toContain('.agents/skills/zstack-review/checklist.md');
  });

  test('sidecar paths in ship skill point to zstack/review/ for pre-landing review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'zstack-ship', 'SKILL.md'), 'utf-8');
    // Ship references the review checklist in its pre-landing review step
    if (content.includes('checklist.md')) {
      expect(content).toContain('.agents/skills/zstack/review/');
      expect(content).not.toContain('.agents/skills/zstack-review/checklist');
    }
  });
  ...
  test('greptile-triage sidecar path is correct', () => {
    ...
    if (content.includes('greptile-triage')) {
      expect(content).toContain('$ZSTACK_ROOT/review/greptile-triage.md');
      expect(content).not.toContain('.agents/skills/zstack-review/greptile-triage');
    }
  });
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.claude/skills/zstack → $ZSTACK_ROOT
    expect(content).not.toContain('~/.claude/skills/zstack');
    expect(content).toContain('$ZSTACK_ROOT');

    // Rule 2: .claude/skills/zstack → .agents/skills/zstack
    expect(content).not.toContain('.claude/skills/zstack');

    // Rule 3: .claude/skills/review → .agents/skills/zstack/review
  ...
  test('path rewrite rules apply to all Codex skills with sidecar references', () => {
    ...
    for (const skill of CODEX_SKILLS) {
      ...
      expect(content).not.toContain('~/.claude/skills');
      expect(content).not.toContain('.claude/skills');
      if (content.includes('zstack-config') || content.includes('zstack-update-check') || content.includes('zstack-telemetry-log')) {
        expect(content).toContain('$ZSTACK_ROOT');
      }
      ...
      if (content.includes('checklist.md') && !content.includes('design-checklist.md')) {
        expect(content).not.toContain('zstack-review/checklist.md');
      }
    }
  });
  ...
  test('Claude output uses installed-root review paths (#2518)', () => {
    ...
    // resolved inside zstack's own checkout.
    const content = fs.readFileSync(path.join(ROOT, 'review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('~/.claude/skills/zstack/review/checklist.md');
    expect(content).toContain('~/.claude/skills/zstack');
    ...
    // codex skill), not the zstack Codex host install path.
  ...
  test('Claude output unchanged: all Claude skills have zero Codex paths', () => {
    for (const skill of CLAUDE_GENERATED_SKILLS) {
      ...
      // not the zstack Codex host install path. ~/.codex/sessions/ (rollout
      ...
      // zstack-upgrade legitimately references .agents/skills for cross-platform detection
      if (skill.dir !== 'zstack-upgrade') {
        expect(content).not.toContain('.agents/skills');
      }
    }
  });
  ...
  test('codex host does not include Codex design block in ship', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'zstack-ship', 'SKILL.md'), 'utf-8');
  ...
  test('explicit --model overrides the codex host default', () => {
    const overrideOut = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-model-override-'));
    try {
      ...
      const content = fs.readFileSync(path.join(overrideOut, '.agents', 'skills', 'zstack-ship', 'SKILL.md'), 'utf-8');
      expect(content).toContain('Model-Specific Behavioral Patch (claude)');
      // The overlay now travels as --model into zstack-skill-start, which
    ...
    const hostDefault = fs.readFileSync(path.join(AGENTS_DIR, 'zstack-ship', 'SKILL.md'), 'utf-8');
...
describe('Factory generation (--host factory)', () => {
  ...
  const FACTORY_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      skills.push({ dir: '.', factoryName: 'zstack' });
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      if (entry.name === 'codex') continue;
      if (!fs.existsSync(path.join(ROOT, entry.name, 'SKILL.md.tmpl'))) continue;
      const factoryName = entry.name.startsWith('zstack-') ? entry.name : `zstack-${entry.name}`;
  ...
  test('sensitive skills have disable-model-invocation', () => {
    const SENSITIVE = ['zstack-ship', 'zstack-land-and-deploy', 'zstack-guard', 'zstack-careful', 'zstack-freeze', 'zstack-unfreeze'];
  ...
  test('non-sensitive skills lack disable-model-invocation', () => {
    const NON_SENSITIVE = ['zstack-qa', 'zstack-review', 'zstack-investigate', 'zstack-browse'];
  ...
  test('Factory keeps Codex integration blocks', () => {
    ...
    const shipContent = fs.readFileSync(path.join(FACTORY_DIR, 'zstack-ship', 'SKILL.md'), 'utf-8');
  ...
  test('Factory preamble uses .factory paths', () => {
    const content = fs.readFileSync(path.join(FACTORY_DIR, 'zstack-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('ZSTACK_ROOT');
    expect(content).toContain('$_ROOT/.factory/skills/zstack');
    expect(content).toContain('$ZSTACK_BIN/zstack-config');
  });
});
    ...
    describe(`${hostConfig.displayName} (--host ${hostConfig.name})`, () => {
      ...
      test('no .claude/skills path leakage outside repo-root sidecar symlinks', () => {
        ...
        for (const skill of skills) {
          // Dev installs may mount the repo root at host/skills/zstack as a runtime
      ...
      if (hostConfig.generation.skipSkills?.includes('codex')) {
        test('/codex skill excluded', () => {
          expect(fs.existsSync(path.join(hostDir, 'zstack-codex', 'SKILL.md'))).toBe(false);
        });
      }
    });
  }
...
describe('setup script validation', () => {
  ...
  test('Codex install uses link_codex_skill_dirs', () => {
    ...
    expect(codexSection).not.toContain('_link_or_copy "$ZSTACK_DIR" "$CODEX_ZSTACK"');
  });
  ...
  test('setup separates install path from source path for symlinked repo-local installs', () => {
    expect(setupContent).toContain('INSTALL_ZSTACK_DIR=');
    expect(setupContent).toContain('SOURCE_ZSTACK_DIR=');
    expect(setupContent).toContain('INSTALL_SKILLS_DIR=');
    expect(setupContent).toContain('CODEX_ZSTACK="$INSTALL_ZSTACK_DIR"');
    expect(setupContent).toContain('link_codex_skill_dirs "$SOURCE_ZSTACK_DIR" "$CODEX_SKILLS"');
  });
  ...
  test('Codex installs always create sidecar runtime assets for the real skill target', () => {
    ...
    expect(setupContent).toContain('create_agents_sidecar "$SOURCE_ZSTACK_DIR"');
  });
  ...
  test('link_codex_skill_dirs reads from .agents/skills/', () => {
    ...
    expect(fnBody).toContain('zstack*');
  });
  ...
  test('link_claude_skill_dirs creates real directories with absolute SKILL.md symlinks', () => {
    ...
    // to ensure Claude Code discovers them as top-level skills (not nested under zstack/)
    ...
    // rendered :user variant from ${ZSTACK_HOME}/render/claude when present.
    expect(fnBody).toContain('_skill_md_src="$zstack_dir/$dir_name/SKILL.md"');
  ...
  test('setup links root zstack skill through a thin Claude wrapper alias', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('_zstack-command');
    // #2511: the alias must be a rewritten COPY (unique frontmatter name),
    // never a verbatim symlink of the canonical SKILL.md.
    expect(fnBody).toContain('_install_alias_skill_md "$zstack_dir/SKILL.md" "$target" "_zstack-command"');
    expect(fnBody).not.toContain('_link_or_copy "$zstack_dir/SKILL.md"');

    const claudeSection = setupContent.slice(
      setupContent.indexOf('# 4. Install for Claude'),
      setupContent.indexOf('# 5. Install for Codex')
    );
    expect(claudeSection).toContain('link_claude_root_skill_alias "$SOURCE_ZSTACK_DIR" "$INSTALL_SKILLS_DIR"');
  });
  ...
  // T1: Sidecar skip guard — prevents .agents/skills/zstack from being linked as a skill
  test('link_codex_skill_dirs skips the zstack sidecar directory', () => {
    const fnStart = setupContent.indexOf('link_codex_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('done', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('[ "$skill_name" = "zstack" ] && continue');
  });

  // T2: Dynamic $ZSTACK_ROOT paths in generated Codex preambles
  test('generated Codex preambles use dynamic ZSTACK_ROOT paths', () => {
    // Read the module-level out-dir render (always present).
    const codexSkillDir = path.join(EXTERNAL_OUT, '.agents', 'skills', 'zstack-ship');
  ...
  test('setup supports --host opencode with install section and OpenCode skill path vars', () => {
    ...
    expect(setupContent).toContain('OPENCODE_ZSTACK="$OPENCODE_SKILLS/zstack"');
  });
  ...
  test('setup supports --host cursor with install section and Cursor skill path vars', () => {
    ...
    expect(setupContent).toContain('CURSOR_ZSTACK="$CURSOR_SKILLS/zstack"');
    ...
    expect(setupContent).toContain('zstack ready (cursor).');
  });
  ...
  test('create_cursor_runtime_root exposes only Cursor runtime assets', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('zstack/SKILL.md');
    expect(fnBody).toContain('browse/dist');
    expect(fnBody).toContain('browse/bin');
    expect(fnBody).toContain('zstack-upgrade/SKILL.md');
    ...
    expect(fnBody).toContain('$cursor_zstack/lib');
    ...
    expect(fnBody).not.toContain('_link_or_copy "$zstack_dir" "$cursor_zstack"');
  });
  ...
  test('create_cursor_sidecar plants runtime assets without wiping generated SKILL.md', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('.cursor/skills/zstack');
  ...
  test('link_cursor_skill_dirs skips the zstack runtime root directory', () => {
  ...
  test('Cursor install links generated skills before planting the sidecar', () => {
    ...
    const linkCall = cursorInstall.indexOf('link_cursor_skill_dirs "$SOURCE_ZSTACK_DIR"');
    const sidecarCall = cursorInstall.indexOf('create_cursor_sidecar "$SOURCE_ZSTACK_DIR"');
    expect(linkCall).toBeGreaterThan(-1);
    expect(sidecarCall).toBeGreaterThan(-1);
    expect(linkCall).toBeLessThan(sidecarCall);
  });

  test('setup installs OpenCode skills into a nested zstack runtime root', () => {
    ...
    expect(setupContent).toContain('$opencode_zstack/lib');
  });
  ...
  test('create_codex_runtime_root exposes only runtime assets', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('zstack/SKILL.md');
    expect(fnBody).toContain('$codex_zstack/lib');
    expect(fnBody).toContain('browse/dist');
    expect(fnBody).toContain('browse/bin');
    expect(fnBody).toContain('zstack-upgrade/SKILL.md');
    ...
    expect(fnBody).toContain('TODOS-format.md');
    expect(fnBody).not.toContain('_link_or_copy "$zstack_dir" "$codex_zstack"');
  });
  ...
  test('create_factory_runtime_root links shared lib modules beside bin', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('$factory_zstack/bin');
    expect(fnBody).toContain('$factory_zstack/lib');
  });

  test('direct Codex installs are migrated out of ~/.codex/skills/zstack', () => {
    expect(setupContent).toContain('migrate_direct_codex_install');
    expect(setupContent).toContain('$HOME/.zstack/repos/zstack');
    expect(setupContent).toContain('avoid duplicate skill discovery');
  });

  // --- Symlink prefix tests (PR #503) ---

  test('link_claude_skill_dirs applies zstack- prefix by default', () => {
    const fnStart = setupContent.indexOf('link_claude_skill_dirs()');
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('SKILL_PREFIX');
    expect(fnBody).toContain('link_name="zstack-$skill_name"');
  });
  ...
  test('link_claude_skill_dirs preserves already-prefixed dirs', () => {
    ...
    const fnEnd = setupContent.indexOf('}', setupContent.indexOf('linked[@]}', fnStart));
    const fnBody = setupContent.slice(fnStart, fnEnd);
    // zstack-* dirs should keep their name (e.g., zstack-upgrade stays zstack-upgrade)
    expect(fnBody).toContain('zstack-*) link_name="$skill_name"');
  });
  ...
  test('cleanup_old_claude_symlinks removes only zstack-pointing symlinks', () => {
    ...
    expect(fnBody).toContain('zstack-*) continue');
  });
  ...
  test('setup reads skill_prefix from config', () => {
    ...
    expect(setupContent).toContain('ZSTACK_CONFIG');
  });
  ...
  test('cleanup_prefixed_claude_symlinks exists and uses readlink', () => {
    ...
    const fnBody = setupContent.slice(fnStart, fnEnd);
    expect(fnBody).toContain('readlink');
    expect(fnBody).toContain('zstack-$skill_name');
  });
  ...
  test('welcome message references SKILL_PREFIX', () => {
    // zstack-upgrade is always called zstack-upgrade (it's the actual dir name)
    // but the welcome section should exist near the prefix logic
    expect(setupContent).toContain('Run /zstack-upgrade anytime');
  });
});

describe('discover-skills hidden directory filtering', () => {
  test('discoverTemplates skips dot-prefixed directories', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-discover-'));
    try {
...
describe('telemetry', () => {
  test('telemetry start block lives in zstack-skill-start; render notes the handoff keys', () => {
    ...
    expect(SKILL_START_SCRIPT).toMatch(/zstack-config" get telemetry/);
  ...
  test('telemetry opt-in prompt lives in zstack-skill-start (marker-gated emit)', () => {
    ...
    // gates it on the marker files and emits it as a ZSTACK_INSTRUCTION block
    ...
    expect(SKILL_START_SCRIPT).toContain('zstack-config set telemetry community');
    expect(SKILL_START_SCRIPT).toContain('zstack-config set telemetry anonymous');
    expect(SKILL_START_SCRIPT).toContain('zstack-config set telemetry off');
    expect(SKILL_START_SCRIPT).toContain('touch "$_GH/.telemetry-prompted"');
  });

  test('generated SKILL.md contains telemetry epilogue (one zstack-skill-end call)', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('Telemetry (run last)');
    expect(content).toContain('zstack-skill-end --skill "zstack" --outcome OUTCOME');
    expect(content).toContain('--tel-start "TEL_START"');
    expect(content).toContain('PLAN MODE EXCEPTION');
    // The duration math + remote-log dispatch moved into zstack-skill-end.
    ...
    expect(SKILL_END_SCRIPT).toContain('zstack-telemetry-log');
  });

  test('pending marker handling lives in the scripts', () => {
    // zstack-skill-start finalizes stale markers; zstack-skill-end clears the
  ...
  test('telemetry blocks appear in all skill files that use PREAMBLE', () => {
    ...
    for (const skill of skills) {
      ...
      if (fs.existsSync(skillPath)) {
        ...
        expect(content).toContain('Telemetry (run last)');
        expect(content).toContain(`zstack-skill-end --skill "${skill}"`);
        expect(content).toContain('--tel-start "TEL_START"');
      }
    }
  });
...
describe('community fixes wave', () => {
  ...
  // #594 — Discoverability: every SKILL.md.tmpl description contains "zstack"
  test('every SKILL.md.tmpl description contains "zstack"', () => {
    for (const skill of ALL_SKILLS) {
      const tmplPath = skill.dir === '.' ? path.join(ROOT, 'SKILL.md.tmpl') : path.join(ROOT, skill.dir, 'SKILL.md.tmpl');
      const content = fs.readFileSync(tmplPath, 'utf-8');
      const desc = extractDescription(content);
      expect(desc.toLowerCase()).toContain('zstack');
    }
  });
...
describe('codex commands must not use inline $(git rev-parse --show-toplevel) for cwd', () => {
  ...
  // Use Bun.Glob to avoid ELOOP from .claude/skills/zstack symlink back to ROOT
...
describe('LEARNINGS_SEARCH resolver', () => {
  ...
  for (const skill of SEARCH_SKILLS) {
    test(`${skill} generated SKILL.md contains learnings search`, () => {
      ...
      expect(content).toContain('zstack-learnings-search');
    });
  }
...
describe('gen-skill-docs prefix warning (#620/#578)', () => {
  ...
  test('warns about skill_prefix when config has prefix=true', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-prefix-warn-'));
    try {
      // Create a fake ~/.zstack/config.yaml with skill_prefix: true
      const fakeHome = tmpDir;
      const fakeZstack = path.join(fakeHome, '.zstack');
      fs.mkdirSync(fakeZstack, { recursive: true });
      fs.writeFileSync(path.join(fakeZstack, 'config.yaml'), 'skill_prefix: true\n');
      ...
      expect(output).toContain('zstack-relink');
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  test('no warning when skill_prefix is false or absent', () => {
    ...
    try {
      ...
      const fakeZstack = path.join(fakeHome, '.zstack');
      fs.mkdirSync(fakeZstack, { recursive: true });
      fs.writeFileSync(path.join(fakeZstack, 'config.yaml'), 'skill_prefix: false\n');
...
describe('plan-mode-info resolver (handshake-replacement)', () => {
  ...
  test('plan-mode-info is wired BEFORE generateUpgradeCheck in preamble', () => {
    ...
    // UPGRADE_AVAILABLE prose emits from bin/zstack-skill-start at runtime),
...
// ZSTACK REVIEW REPORT report-at-bottom contract — verifies the prompt-text
...
describe('ZSTACK REVIEW REPORT delete-then-append flow', () => {
...
describe('LEARNINGS_SEARCH resolver: query parameter', () => {
  ...
  test('claude host + query=foo bar → both cross-project and project-scoped branches contain --query', () => {
    ...
    const lines = out.split('\n').filter(l => l.includes('zstack-learnings-search'));
  ...
  test('codex host + query=foo bar → codex bash variant contains --query', () => {
    ...
    expect(out).toContain('$ZSTACK_BIN/zstack-learnings-search');
  });
...
describe('EXIT PLAN MODE GATE placement', () => {
  ...
  // already contains `## ZSTACK REVIEW REPORT` inside a markdown example fence,
...
describe('ZSTACK REVIEW REPORT mandatory unresolved-decisions status', () => {
...
describe('brain-sync block reads project-scoped MCP registrations (#2499)', () => {
  ...
  // from the rendered SKILL.md into bin/zstack-skill-start. Pin the LIVE
  // script bytes — same assertions, new home. The render carries only the
  // ARTIFACTS_SYNC interpretation prose.
  const rendered = fs.readFileSync(path.join(ROOT, 'bin', 'zstack-skill-start'), 'utf-8');
  ...
  test('rendered jq lines FUNCTION: project-scoped registration resolves for a cwd inside the project', () => {
    ...
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-2499-home-'));
    const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-2499-proj-'));

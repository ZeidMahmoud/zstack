...
describe('gen-skill-docs', () => {
  ...
  test('generated SKILL.md contains operational self-improvement (replaced contributor mode)', () => {
    ...
    expect(content).toContain('zstack-learnings-search --limit 3');
  });
...
describe('CODEX_SECOND_OPINION resolver', () => {
  const content = readSkillUnion('office-hours'); // carved: Phase 5/6 prose moved to section
  const codexContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'zstack-office-hours', 'SKILL.md'), 'utf-8');
...
describe('Codex generation (--host codex)', () => {
  ...
  test('Codex preamble resolves runtime assets from repo-local or global zstack roots', () => {
    ...
    expect(content).toContain('$ZSTACK_ROOT/zstack-upgrade/SKILL.md');
    expect(content).not.toContain('~/.codex/skills/zstack/bin/zstack-config get telemetry');
  });

  // ─── Path rewriting regression tests ─────────────────────────

  test('sidecar paths point to .agents/skills/zstack/review/ (not zstack-review/)', () => {
    // Regression: gen-skill-docs rewrote .claude/skills/review → .agents/skills/zstack-review
    // but setup puts sidecars under .agents/skills/zstack/review/. Must match setup layout.
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'zstack-review', 'SKILL.md'), 'utf-8');
    // Correct: references to sidecar files use zstack/review/ path
    expect(content).toContain('.agents/skills/zstack/review/checklist.md');
  ...
  test('greptile-triage sidecar path is correct', () => {
    ...
    if (content.includes('greptile-triage')) {
      expect(content).toContain('.agents/skills/zstack/review/greptile-triage.md');
  ...
  test('Claude output unchanged: all Claude skills have zero Codex paths', () => {
    for (const skill of CLAUDE_GENERATED_SKILLS) {
      ...
      // not the zstack Codex host install path.
...
describe('Factory generation (--host factory)', () => {
  ...
  const FACTORY_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      if (!isSymlinkLoop('zstack')) skills.push({ dir: '.', factoryName: 'zstack' });
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
  ...
  for (const hostConfig of getExternalHosts()) {
    describe(`${hostConfig.displayName} (--host ${hostConfig.name})`, () => {
      ...
      test('generates Claude outside-voice skill for external hosts', () => {
        const skillMd = path.join(hostDir, 'zstack-claude', 'SKILL.md');
        expect(fs.existsSync(skillMd)).toBe(true);
        const content = fs.readFileSync(skillMd, 'utf-8');
        expect(content).toContain('claude -p');
        expect(content).toContain('--disable-slash-commands');
...
describe('setup script validation', () => {
  ...
  test('link_claude_skill_dirs creates real directories with absolute SKILL.md symlinks', () => {
    ...
    expect(fnBody).toContain('_link_or_copy "$zstack_dir/$dir_name/SKILL.md" "$target/SKILL.md"');
  });
  ...
  test('setup links root zstack skill through a thin Claude wrapper alias', () => {
    ...
    expect(fnBody).toContain('_link_or_copy "$zstack_dir/SKILL.md" "$target/SKILL.md"');
  ...
  test('generated Codex preambles use dynamic ZSTACK_ROOT paths', () => {
    const codexSkillDir = path.join(ROOT, '.agents', 'skills', 'zstack-ship');
...
describe('telemetry', () => {
  test('generated SKILL.md contains telemetry start block', () => {
    ...
    expect(content).toContain('zstack-config get telemetry');
  });

  test('generated SKILL.md contains telemetry opt-in prompt', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('.telemetry-prompted');
    expect(content).toContain('Help zstack get better');
    expect(content).toContain('zstack-config set telemetry community');
    expect(content).toContain('zstack-config set telemetry anonymous');
    expect(content).toContain('zstack-config set telemetry off');
  });

  test('generated SKILL.md contains telemetry epilogue', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('Telemetry (run last)');
    expect(content).toContain('zstack-telemetry-log');

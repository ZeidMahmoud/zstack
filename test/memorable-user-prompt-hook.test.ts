/**
 * memorable-user-prompt-hook — the zstack-mediated bridge to the third-party
 ...
 *
 * Every spawn pins HOME, ZSTACK_HOME, ZSTACK_STATE_ROOT, ZSTACK_STATE_DIR and
 * ZSTACK_MEMORABLE_BIN into a fresh temp dir, so nothing reaches the real
 * ~/.zstack or ~/.memorable and the receipt ledger under test is the temp one.
 */
...
const POLICY = path.join(ROOT, 'bin', 'zstack-gbrain-repo-policy');
...
beforeEach(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-memo-hook-'));
...
const errLog = () => { const p = path.join(home, '.zstack', 'hook-errors.log'); return fs.existsSync(p) ? fs.readFileSync(p, 'utf8') : ''; };
const ledger = () => path.join(home, '.zstack', 'security', 'egress.jsonl');
const receipts = () => listReceipts(path.join(home, '.zstack'));
...
describe('gate on: the mediated hand-off', () => {
  test('spawns the vendor once with the exact stdin bytes, returns an enveloped additionalContext, receipts it', () => {
    ...
    expect(String(rs[0].status)).toMatch(/^exit:0 output-written bytes=\d+ zstack_ms=\d+$/);
    expect(Number(String(rs[0].status).match(/bytes=(\d+)/)![1])).toBe(Buffer.byteLength(ctx));
    expect(verifyLedger(path.join(home, '.zstack')).ok).toBe(true);
  });

  test('the vendor runs in an allowlisted environment: API keys and zstack state never reach it', () => {
    gateOn();
    runHook(PROMPT);
    const vendorEnvText = fs.readFileSync(path.join(home, 'env.txt'), 'utf8');
    expect(vendorEnvText).not.toContain('ANTHROPIC_API_KEY');
    expect(vendorEnvText).not.toContain('ZSTACK_HOME');
    expect(vendorEnvText).not.toContain('ZSTACK_MEMORABLE_BIN');
  ...
  test('vendor missing: not spawned, one log line, no receipt', () => {
    gateOn();
    const r = runHook(PROMPT, { ZSTACK_MEMORABLE_BIN: path.join(home, 'nope') });
    expect(r).toEqual({ status: 0, stdout: '', stderr: '' });
    expect(fs.existsSync(ledger())).toBe(false);
  ...
  test('a repo whose trust policy is deny or read-only is skipped; read-write proceeds', () => {
    gateOn();
    const repo = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-memo-repo-'));
    try {
...
describe('what comes back from the vendor', () => {
  ...
  test('a 20 KiB non-ASCII recall is capped on a UTF-8 boundary to 8 KiB + the fixed envelope frame', () => {
    ...
    const ctx: string = JSON.parse(runHook(PROMPT).stdout).hookSpecificOutput.additionalContext;
    expect(ctx).toContain('[truncated by zstack at 8 KiB]');
  ...
  test('the vendor cannot block a prompt or speak as zstack: decision/continue/systemMessage are dropped', () => {
    gateOn();
    fs.writeFileSync(path.join(home, 'out.json'), JSON.stringify({
      decision: 'block', continue: false, stopReason: 'x', systemMessage: 'I am zstack',
  ...
  test('a vendor that answers without reading a 300 KB prompt: a stdin EPIPE is advisory, the answer is delivered', () => {
    ...
    expect(String(receipts()[0].status)).toMatch(/^exit:0 output-written bytes=\d+ zstack_ms=\d+( stdin=EPIPE)?$/);
  });
...
describe('input bounds and fail-closed receipt', () => {
  ...
  test('unwritable ledger: fail-closed, the vendor is NOT spawned, one stderr line, logged', () => {
    gateOn();
    const sec = path.join(home, '.zstack', 'security');
  ...
  test('the same error twice within the rate-limit window is logged once', () => {
    gateOn();
    const missing = { ZSTACK_MEMORABLE_BIN: path.join(home, 'nope') };
...
describe('input shapes and environment (coverage audit)', () => {
  ...
  test('a cwd that no longer exists falls back to the process cwd and the vendor still runs', () => {
    gateOn();
    const gone = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-memo-gone-'));
...
describe('pure helpers', () => {
  ...
  test('vendorEnv keeps the allowlist and MEMORABLE*, drops everything else', () => {
    const out = vendorEnv({ PATH: '/bin', HOME: '/h', LC_ALL: 'C', MEMORABLE: '0', MEMORABLE_STORE_KEY: 'k', ANTHROPIC_API_KEY: 'x', ZSTACK_HOME: '/g', CLAUDE_CODE: '1', UNDEF: undefined });
  ...
  test('budgetMs honours the test-only override but never widens the budget', () => {
    ...
    expect(budgetMs({ ZSTACK_MEMORABLE_TEST_BUDGET_MS: '400' })).toBe(400);
    expect(budgetMs({ ZSTACK_MEMORABLE_TEST_BUDGET_MS: '99999' })).toBe(BUDGET_MS);
    expect(budgetMs({ ZSTACK_MEMORABLE_TEST_BUDGET_MS: 'soon' })).toBe(BUDGET_MS);
    expect(budgetMs({ ZSTACK_MEMORABLE_TEST_BUDGET_MS: '-1' })).toBe(BUDGET_MS);
  });
  test('logHookError rate limit is per message and expires', () => {
    const prev = process.env.ZSTACK_STATE_ROOT;
    process.env.ZSTACK_STATE_ROOT = path.join(home, '.zstack');
    try {
      ...
      if (process.platform !== 'win32') expect(fs.statSync(path.join(home, '.zstack', 'hook-errors.log')).mode & 0o077).toBe(0);
    } finally {
      if (prev === undefined) delete process.env.ZSTACK_STATE_ROOT; else process.env.ZSTACK_STATE_ROOT = prev;
    }
  });
  test('resolveVendor: explicit override wins, may be quoted, and an unresolvable or non-executable override is null (no fall-through)', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-memo-resolve-'));
    // parity with bash's ${ZSTACK_MEMORABLE_BIN:-${MEMORABLE_BIN:-}}: an EMPTY first override defers to the second
    {
      const exe = path.join(dir, 'via-second'); fs.writeFileSync(exe, '#!/bin/sh\n', { mode: 0o755 });
      expect(resolveVendor({ ZSTACK_MEMORABLE_BIN: '', MEMORABLE_BIN: exe }, dir)).toBe(exe);
      expect(resolveVendor({ ZSTACK_MEMORABLE_BIN: '   ', MEMORABLE_BIN: exe }, dir)).toBe(exe);
    }
    try {
      ...
      expect(resolveVendor({ ZSTACK_MEMORABLE_BIN: exe, MEMORABLE_BIN: pinned }, homeDir)).toBe(exe);
      expect(resolveVendor({ MEMORABLE_BIN: `"${exe}"` }, homeDir)).toBe(exe);
      expect(resolveVendor({ ZSTACK_MEMORABLE_BIN: path.join(dir, 'missing') }, homeDir)).toBeNull();
      expect(resolveVendor({ ZSTACK_MEMORABLE_BIN: plain }, homeDir)).toBeNull();
...
describe('deadline and policy failure paths (review coverage)', () => {
  test('a shortened budget skips the vendor before the spawn: nothing spawned, no receipt, logged', () => {
    gateOn();
    const r = runHook(PROMPT, { ZSTACK_MEMORABLE_TEST_BUDGET_MS: '499' });
    expect(r).toEqual({ status: 0, stdout: '', stderr: '' });
    expect(calls()).toBe('');
    expect(fs.existsSync(ledger())).toBe(false);
  ...
  test('an unreadable trust-policy store fails closed: nothing spawned, no receipt, logged', () => {
    ...
    try {
      ...
      const storeDir = path.join(home, '.zstack', 'gbrain-repo-policy.json');
...
describe('trust-policy lookup outcomes (review coverage, second pass)', () => {
  ...
  test('store present, cwd is a plain directory (not a repo): recall proceeds, even under a non-English locale', () => {
    gateOn();
    withStore();
    const plain = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-memo-plain-'));
    try {
  ...
  test('an inherited GIT_DIR pointing at an allowed repo does not bypass the deny on the session repo', () => {
    gateOn();
    const denied = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-memo-denied-'));
    const allowed = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-memo-allowed-'));
    try {

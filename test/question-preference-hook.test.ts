 ...
 *   - auto-decided event logged via zstack-question-log (PostToolUse won't fire)
 *   - auto-decided marker written to ~/.zstack/sessions/<id>/.auto-decided-<tool_use_id>
 */
...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-prefhook-'));
  cwdSlug = 'fixture-slug';
  fs.mkdirSync(path.join(stateRoot, 'projects', cwdSlug), { recursive: true });
  // Real directory that the hook can chdir() into. zstack-slug derives the
...
function runHook(stdin: object, cwd?: string, extraEnv?: Record<string, string>): {
  ...
  delete env.ZSTACK_SESSION_KIND;
...
describe('passes through (no enforcement)', () => {
  test('no preference set → pass-through (empty stdout, no permissionDecision)', () => {
    const r = runHook({
      ...
      tool_input: {
        questions: [
          { question: '<zstack-qid:test-q> Need approval?', options: ['A) Yes (recommended)', 'B) No'] },
        ],
      },
    });
    expectPassThrough(r);
  ...
  test('always-ask preference → pass-through', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          { question: '<zstack-qid:test-q> Yes?', options: ['A) Yes (recommended)', 'B) No'] },
        ],
      },
    });
    expectPassThrough(r);
  ...
  test('pass-through stdout never contains "permissionDecision" (#2035)', () => {
    const paths = [
      runHook({
        ...
        tool_input: {
          questions: [
            { question: '<zstack-qid:test-q> Approve?', options: ['A) Yes (recommended)', 'B) No'] },
          ],
        },
      }),
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question:
              '<zstack-qid:ship-pre-landing-review-fix> Pre-landing review flagged issue.',
  ...
  test('one-way door → pass-through even with never-ask (safety override)', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<zstack-qid:ship-test-failure-triage> Tests failed.',
  ...
  test('ambiguous recommendation (two labels) → pass-through (D2 refuse-on-ambiguous)', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<zstack-qid:ship-pre-landing-review-fix> Ambiguous',
  ...
  test('no recommendation marker AND no prose match → pass-through', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<zstack-qid:ship-pre-landing-review-fix> No rec',
            options: ['A) Foo', 'B) Bar'],
          },
        ],
      },
  ...
  test('unregistered id + never-ask + destructive text → pass-through (keyword net fires, #2024)', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<zstack-qid:adhoc-credential-cleanup> Reset my secret and proceed?',
  ...
  test('unregistered id + never-ask + benign text → still deny (auto-decide unchanged)', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<zstack-qid:adhoc-credential-cleanup> Reorganize the TODOs file?',
            options: ['A) Yes (recommended)', 'B) No'],
          },
        ],
      },
  ...
  test('project never-ask + global always-ask → enforce never-ask', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<zstack-qid:ship-pre-landing-review-fix> P?',
...
describe('Conductor prose redirect', () => {
  ...
  test('CONDUCTOR_WORKSPACE_PATH alone also triggers the redirect', () => {
    const r = runHook({
      ...
      tool_use_id: 'tu-c4',
      tool_input: {
        questions: [{ question: '<zstack-qid:test-q> Pick?', options: ['A) X (recommended)', 'B) Y'] }],
      },
  ...
  test('PRECEDENCE: full never-ask auto-decide still wins over Conductor prose', () => {
    ...
    const r = runHook({
      ...
      tool_input: {
        questions: [
          {
            question: '<zstack-qid:ship-pre-landing-review-fix> Pre-landing review flagged issue.',
            options: ['A) Fix now (recommended)', 'B) Skip'],
          },
        ],
      },
...
describe('Conductor spawned deny (#2733)', () => {
  const Q = {
    questions: [
      { question: '<zstack-qid:test-q> Bump VERSION?', options: ['A) Skip (recommended)', 'B) Bump'] },
    ],
  };
  ...
  test('Conductor + ZSTACK_SESSION_KIND=spawned env → same auto-choose deny', () => {
    const r = runHook(
      { session_id: 's2', tool_name: 'AskUserQuestion', tool_use_id: 'tu-s2', tool_input: Q },
      undefined,
      { CONDUCTOR_WORKSPACE_PATH: '/Users/x/conductor/ws', ZSTACK_SESSION_KIND: 'spawned' },
    );
    expect(r.parsed?.hookSpecificOutput?.permissionDecision).toBe('deny');
    const reason = r.parsed?.hookSpecificOutput?.permissionDecisionReason ?? '';
    expect(reason).toContain('[conductor][spawned]');
  ...
  test('Conductor + invalid ZSTACK_SESSION_KIND value → prose deny, not spawned (strict-equality fall-through)', () => {
    // spawnedByEnv() mirrors bin/zstack-session-kind step 0: only the exact
    ...
    const r = runHook(
      ...
      undefined,
      { CONDUCTOR_PORT: '55071', ZSTACK_SESSION_KIND: 'bogus' },
    );
    expect(r.parsed?.hookSpecificOutput?.permissionDecision).toBe('deny');
    const reason = r.parsed?.hookSpecificOutput?.permissionDecisionReason ?? '';
    ...
    const r = runHook(
      {
        ...
        tool_input: {
          questions: [
            { question: '<zstack-qid:spawned-nc> Bump VERSION?', options: ['A) Skip (recommended)', 'B) Bump'] },
          ],
        },
      },
      undefined,
    ...
    const r = runHook(
      {
        ...
        tool_input: {
          questions: [
            { question: '<zstack-qid:test-q> Force-push and overwrite the remote branch, deleting its history?', options: ['A) Force-push (recommended)', 'B) Abort'] },
          ],
        },
      },
      undefined,
    ...
    const r = runHook(
      {
        ...
        tool_input: {
          questions: [
            { question: '<zstack-qid:test-q> Proceed with the plan?', options: ['A) Force-push over the remote branch (recommended)', 'B) Abort'] },
          ],
        },
      },
      undefined,
      { CONDUCTOR_PORT: '55070', ZSTACK_SESSION_KIND: 'spawned' },
    );
    expect(r.parsed?.hookSpecificOutput?.permissionDecision).toBe('deny');
    const reason = r.parsed?.hookSpecificOutput?.permissionDecisionReason ?? '';
    expect(reason).toMatch(/one-way door detected: Q1/);
    expect(reason).toContain('spawned driver: ZSTACK_SESSION_KIND');
  });
  ...
  test('cross-surface destructive-policy drift guard: every spawned surface carries the canonical phrase', () => {
    ...
    const surfaces = [
      ...
      path.join(ROOT, 'bin', 'zstack-skill-start'),
  ...
  test('spawnedByEnv() parity with bin/zstack-session-kind over the spawned env matrix', () => {
    ...
    const BIN = path.join(ROOT, 'bin', 'zstack-session-kind');
    const cases: Array<Record<string, string>> = [
      { OPENCLAW_SESSION: '1' },
      { ZSTACK_SESSION_KIND: 'spawned' },
      { ZSTACK_SESSION_KIND: 'spawned', ZSTACK_HEADLESS: '1' },
      { ZSTACK_SESSION_KIND: 'bogus' },
      { ZSTACK_SESSION_KIND: 'headless' },
      { CONDUCTOR_PORT: '5' },
      {},
    ];

...
const SESSION_UPDATE = path.join(ROOT, 'bin', 'zstack-session-update');
const TEAM_INIT = path.join(ROOT, 'bin', 'zstack-team-init');

function mkTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-team-test-'));
}
...
describe('zstack-settings-hook', () => {
  ...
  test('add creates settings.json if missing', () => {
    const result = run(`${SETTINGS_HOOK} add /path/to/zstack-session-update`, {
      env: { ZSTACK_SETTINGS_FILE: settingsFile },
    });
    expect(result.exitCode).toBe(0);
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
    expect(settings.hooks.SessionStart).toHaveLength(1);
    expect(settings.hooks.SessionStart[0].hooks[0].command).toBe('/path/to/zstack-session-update');
  });
  ...
  test('add deduplicates (running twice does not double-add)', () => {
    run(`${SETTINGS_HOOK} add /path/to/zstack-session-update`, {
      env: { ZSTACK_SETTINGS_FILE: settingsFile },
    });
    run(`${SETTINGS_HOOK} add /path/to/zstack-session-update`, {
      env: { ZSTACK_SETTINGS_FILE: settingsFile },
  ...
  test('remove removes the hook', () => {
    ...
    const result = run(`${SETTINGS_HOOK} remove /path/to/zstack-session-update`, {
      env: { ZSTACK_SETTINGS_FILE: settingsFile },
    });
    expect(result.exitCode).toBe(0);
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
  ...
  test('remove preserves other hooks', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        SessionStart: [
          { hooks: [{ type: 'command', command: '/path/to/zstack-session-update' }] },
          { hooks: [{ type: 'command', command: '/other/hook' }] },
        ],
      },
    }, null, 2));
    run(`${SETTINGS_HOOK} remove /path/to/zstack-session-update`, {
      env: { ZSTACK_SETTINGS_FILE: settingsFile },
    });
    const settings = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
    expect(settings.hooks.SessionStart).toHaveLength(1);
...
describe('zstack-session-update', () => {
  let tmpDir: string;
  let zstackDir: string;
  let stateDir: string;

  beforeEach(() => {
    tmpDir = mkTmpDir();
    zstackDir = path.join(tmpDir, 'zstack');
    stateDir = path.join(tmpDir, 'state');
    fs.mkdirSync(zstackDir, { recursive: true });
    ...
    const binDir = path.join(zstackDir, 'bin');
    fs.mkdirSync(binDir, { recursive: true });
    fs.writeFileSync(path.join(binDir, 'zstack-config'), '#!/bin/bash\necho "true"');
    fs.chmodSync(path.join(binDir, 'zstack-config'), 0o755);
  });
  ...
  test('exits 0 when .git is missing', () => {
    fs.rmSync(path.join(zstackDir, '.git'), { recursive: true });
    const result = run(SESSION_UPDATE, {
      env: { ZSTACK_DIR: zstackDir, ZSTACK_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  });

  test('exits 0 when auto_upgrade is not true', () => {
    // Override zstack-config to return false
    fs.writeFileSync(path.join(zstackDir, 'bin', 'zstack-config'), '#!/bin/bash\necho "false"');
    const result = run(SESSION_UPDATE, {
      env: { ZSTACK_DIR: zstackDir, ZSTACK_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  ...
  test('always exits 0 (non-fatal)', () => {
    ...
    const result = run(SESSION_UPDATE, {
      env: { ZSTACK_DIR: '/nonexistent/path', ZSTACK_STATE_DIR: stateDir },
    });
    expect(result.exitCode).toBe(0);
  });
});

describe('zstack-team-init', () => {
  let tmpDir: string;
  ...
  test('optional: creates CLAUDE.md with recommended section', () => {
    ...
    expect(claude).toContain('## zstack (recommended)');
  ...
  test('required: creates CLAUDE.md with required section', () => {
    ...
    expect(result.exitCode).toBe(0);
    const claude = fs.readFileSync(path.join(tmpDir, 'CLAUDE.md'), 'utf-8');
    expect(claude).toContain('## zstack (REQUIRED');
    expect(claude).toContain('ZSTACK_MISSING');
  });

  test('required: creates enforcement hook', () => {
    run(`${TEAM_INIT} required`, { cwd: tmpDir });
    const hookPath = path.join(tmpDir, '.claude', 'hooks', 'check-zstack.sh');
    expect(fs.existsSync(hookPath)).toBe(true);
    const hook = fs.readFileSync(hookPath, 'utf-8');
    expect(hook).toContain('BLOCKED: zstack is not installed');
  ...
  test('required: creates project settings.json with PreToolUse hook', () => {
    ...
    expect(settings.hooks.PreToolUse[0].hooks[0].command).toContain('check-zstack');
  });
  ...
  test('idempotent: running twice does not duplicate CLAUDE.md section', () => {
    ...
    run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    const claude = fs.readFileSync(path.join(tmpDir, 'CLAUDE.md'), 'utf-8');
    const matches = claude.match(/## zstack/g);
    expect(matches).toHaveLength(1);
  });

  test('removes vendored copy when present', () => {
    // Create a fake vendored zstack with VERSION file
    ...
    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('Found vendored zstack copy');
    ...
    expect(gitignore).toContain('.claude/skills/zstack/');
  });

  test('skips when no vendored copy present', () => {
    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).not.toContain('Found vendored zstack copy');
  });

  test('skips when .claude/skills/zstack is a symlink', () => {
    ...
    fs.symlinkSync(targetDir, path.join(skillsDir, 'zstack'));

    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).not.toContain('Found vendored zstack copy');
    // Symlink should still exist
    expect(fs.lstatSync(path.join(skillsDir, 'zstack')).isSymbolicLink()).toBe(true);
  ...
  test('does not duplicate .gitignore entry on re-run', () => {
    ...
    const gitignore = fs.readFileSync(path.join(tmpDir, '.gitignore'), 'utf-8');
    const matches = gitignore.match(/\.claude\/skills\/zstack\//g);
    expect(matches).toHaveLength(1);
  });
});
...
describe('setup --team / --no-team / -q', () => {
  ...
  function withSetup(check: (fixture: { setup: string; cwd: string; env: NodeJS.ProcessEnv; home: string }) => void): void {
    ...
    const tmp = mkTmpDir();
    const cwd = path.join(tmp, 'zstack');
    ...
    try {
      for (const rel of ['setup', 'VERSION', 'SKILL.md', 'qa/SKILL.md', 'bin/zstack-config', 'bin/zstack-patch-names', 'scripts/resolve-codex-generation-model.ts', 'scripts/models.ts']) {
...
case "$*" in
  ...
  *'/bin/zstack-migrate-claude-code --install-dir '*)
...
if [ "$#" -eq 2 ] && [ "$1" = -f ] && [ "$2" = /tmp/zstack-latest-version ]; then exit 0; fi
exec ${quote(realRm)} "$@"
`, { mode: 0o755 });
      const state = path.join(home, '.zstack');
      const env: NodeJS.ProcessEnv = {
        ...
        ZSTACK_HOME: state, ZSTACK_STATE_ROOT: state,
        ZSTACK_SKIP_PLAYWRIGHT: '1', ZSTACK_SKIP_FONTS: '1', ZSTACK_SKIP_COREUTILS: '1', ZSTACK_SKIP_ASIDE: '1',
  ...
  test(
    ...
    () => withSetup(fixture => {
      ...
      // The key test is that the "Skill naming:" prompt and "zstack ready" messages are suppressed
      expect(result.stdout).not.toContain('Skill naming:');
      expect(result.stdout).not.toContain('zstack ready');
      expect(fs.realpathSync(path.join(fixture.home, '.claude/skills/zstack'))).toBe(fs.realpathSync(fixture.cwd));
    }),
    180_000,
  );

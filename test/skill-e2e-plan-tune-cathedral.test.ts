 ...
 *
 * Each scenario uses ZSTACK_STATE_ROOT to isolate from the user's real
 * ~/.zstack (per cathedral T1 + Codex D16 fix). Every attempt gets a fresh fixture.
 */
...
function scaffoldFixture(workDir: string): { workDir: string; stateRoot: string; slug: string; env: NodeJS.ProcessEnv } {
  const stateRoot = path.join(workDir, '.zstack-state');
  fs.mkdirSync(stateRoot, { recursive: true });

  // git init so zstack-slug resolves a deterministic slug.
  ...
  for (const script of [
    ...
    'zstack-paths',
    'zstack-question-log',
    'zstack-question-preference',
    'zstack-developer-profile',
    'zstack-codex-session-import',
    'zstack-distill-free-text',
    'zstack-distill-apply',
  ]) {
...
describeIfSelected('PlanTune cathedral E2E: hook capture', ['plan-tune-hook-capture'], () => {
  testCathedral('plan-tune-hook-capture', 'cathedral-cap-', async (fixture) => {
    ...
    const payload = {
      ...
      tool_input: {
        questions: [
          {
            question:
              'D1 — Cathedral E2E capture <zstack-qid:ship-test-failure-triage>\nRecommendation: A',
    ...
    const res = spawnSync(hookPath, [], {
      env: {
        ...fixture.env,
        ZSTACK_STATE_ROOT: fixture.stateRoot,
        ZSTACK_QUESTION_LOG_NO_DERIVE: '1',
      },
    ...
    const payload = {
      ...
      tool_input: {
        questions: [
          {
            question:
              '<zstack-qid:ship-changelog-voice-polish> Polish CHANGELOG entry?',
  ...
  testCathedral('plan-tune-annotation', 'cathedral-ann-', async (fixture) => {
    ...
    const payload = {
      ...
      tool_input: {
        questions: [
          {
            question: '<zstack-qid:ship-todos-reorganize> Reorganize TODOs?',
            options: ['A) Accept (recommended)', 'B) Skip'],
          },
        ],
      },
...
describeIfSelected('PlanTune cathedral E2E: codex import', ['plan-tune-codex-import'], () => {
  testCathedral('plan-tune-codex-import', 'cathedral-cdx-', async (fixture) => {
    ...
    const lines = [
      ...
      JSON.stringify({
        ...
        payload: {
          type: 'agent_message',
          message:
            'D1 — Cathedral import <zstack-qid:plan-eng-review-scope-reduce>\nRecommendation: A\nA) Reduce (recommended)\nB) Keep',
        },
      }),
      JSON.stringify({
        timestamp: new Date().toISOString(),
    ...
    const bin = path.join(fixture.workDir, 'bin', 'zstack-codex-session-import');
...
describeIfSelected('PlanTune cathedral E2E: dream cycle', ['plan-tune-dream-cycle'], () => {
  testCathedral('plan-tune-dream-cycle', 'cathedral-dream-', async (fixture) => {
    ...
    // 1. Apply the proposal via zstack-distill-apply.
    const applyBin = path.join(fixture.workDir, 'bin', 'zstack-distill-apply');
    const applyRes = spawnSync(applyBin, ['--proposal', '0'], {
      env: { ...fixture.env, ZSTACK_STATE_ROOT: fixture.stateRoot },
      encoding: 'utf-8',
      cwd: fixture.workDir,
      timeout: 30_000,
    });
    ...
    const payload = {
      ...
      tool_input: {
        questions: [
          {
            question:
              '<zstack-qid:plan-eng-review-test-gap> Add tests for this gap?',

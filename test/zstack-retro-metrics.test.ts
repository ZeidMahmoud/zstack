/**
 * Contract + behavior tests for bin/zstack-retro-metrics (retro
 ...
 *     interprets must be emitted (hermetic temp HOME + ZSTACK_HOME, synthetic
 ...
 *
 * All hermetic: HOME + ZSTACK_HOME point at throwaway temp dirs; the script
...
const SCRIPT = path.join(ROOT, 'bin', 'zstack-retro-metrics');
...
function hermeticEnv(): Record<string, string> {
  return { PATH: process.env.PATH!, HOME: tmpHome, ZSTACK_HOME: tmpZstackHome };
}
...
beforeAll(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-rm-home-'));
  tmpZstackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-rm-gh-'));
  repoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-rm-repo-'));
...
describe('zstack-retro-metrics contract', () => {
  ...
  test('the skill fence invokes the script with primary path + degraded fallback', () => {
    ...
    expect(tmpl).toContain('$HOME/.claude/skills/zstack/bin/zstack-retro-metrics');
    expect(tmpl).toContain('".claude/skills/zstack/bin/zstack-retro-metrics"');
...
describe('zstack-retro-metrics behavior', () => {
  ...
  test('aux inputs report present when the files exist under ZSTACK_HOME', () => {
    fs.writeFileSync(path.join(tmpZstackHome, 'greptile-history.md'), '# history\n');
    fs.mkdirSync(path.join(tmpZstackHome, 'analytics'), { recursive: true });
    fs.writeFileSync(path.join(tmpZstackHome, 'analytics', 'skill-usage.jsonl'), '{}\n');
    try {
      const out = runMetrics(['--base', 'main', '--since', '2026-03-09T00:00:00']);
      ...
      fs.rmSync(path.join(tmpZstackHome, 'greptile-history.md'), { force: true });
      fs.rmSync(path.join(tmpZstackHome, 'analytics'), { recursive: true, force: true });
    }
  });
...
describe('zstack-retro-metrics edges', () => {
  test('survives a repo with exactly 1 commit', () => {
    const oneDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-rm-one-'));
    try {
  ...
  test('non-repo dir reports RETRO_METRICS_ERROR and exits 0', () => {
    const emptyDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-rm-empty-'));
    try {

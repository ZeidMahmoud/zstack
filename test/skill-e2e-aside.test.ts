 ...
 *
 * Aside is the primary browser (the user's real sessions); zstack's own
 ...
 * scripts that print labelled evidence and the ZSTACK_STEP_OK sentinel, and
 ...
 * them in console.log("…")).
 *
 * The skill docs come from the tree unless ZSTACK_E2E_DOCS_ROOT points at a
 ...
 * ZSTACK_SKIP_ASIDE=1). CI runners have no Aside, so this only runs on a dev
...
if (process.env.EVALS && !shouldRun) {
  process.stderr.write(`\nskill-e2e-aside: SKIPPED — ${tierOk
    ? 'Aside is not installed or not running (or ZSTACK_SKIP_ASIDE=1)'
...
const DOCS_ROOT = process.env.ZSTACK_E2E_DOCS_ROOT || ROOT;
...
function copySkill(name: string, dir: string): void {
  ...
  if (stale) throw new Error(`${name}/SKILL.md is not Aside-native (stale generated doc) — run \`bun run gen:skill-docs\` (or point ZSTACK_E2E_DOCS_ROOT at an --out-dir render) before this E2E`);
}
...
// Line-anchored: a `cat SKILL.md` in Bash prints `console.log("ZSTACK_STEP_OK");`, never a bare token line.
const STEP_OK = /^ZSTACK_STEP_OK\s*$/m;
...
Drive the browser ONLY with 'aside repl' scripts, each ending with console.log("ZSTACK_STEP_OK"); copy any screenshot out of the printed ASIDE_DIR in bash.
...
qa is a carved skill: when SKILL.md tells you to Read ~/.claude/skills/zstack/qa/sections/<file>, read qa/sections/<file> in this working directory instead (same content, local copy).
...
The working directory is ${dir}; write the report to ${dir}/.zstack/canary-reports/<date>-canary.md (and the .json) exactly as the skill describes. If a zstack helper binary is missing, skip that logging line and continue.`,
    check: (dir, result) => {
      expect(['success', 'error_max_turns']).toContain(result.exitReason);
      expectDroveAside(result);
      const reportDir = path.join(dir, '.zstack', 'canary-reports');

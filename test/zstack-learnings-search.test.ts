...
const BIN = path.join(ROOT, 'bin', 'zstack-learnings-search');

const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-search-test-'));
const tmpCwd = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-search-cwd-'));
// zstack-slug derives slug from git remote (none here) → falls back to basename of cwd.
...
describe('zstack-learnings-search token-OR query semantics', () => {
...
describe('zstack-learnings-search cross-project trust gating', () => {

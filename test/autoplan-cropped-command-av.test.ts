...
function replay(){
 const root=fs.mkdtempSync(path.join(os.tmpdir(),'zstack-cropped-command-'));
...
const displayCases:Array<[string,(r:Replay)=>void]>=[
 ...
 ['foreign Edit target',r=>{r.viewport=r.viewport.replace('zstack-autoplan-chain-ZdZS9F','zstack-autoplan-chain-foreign')}],

...
const EVIDENCE = path.join(ROOT, 'bin', 'zstack-evidence');
...
function ledgerFile(): string {
  const found = findFilesBySuffix(path.join(zstackHome, 'projects'), '-evidence.jsonl');
...
beforeEach(() => {
  zstackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-evidence-home-'));
  repoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-evidence-repo-'));
  git('init -q -b main');
...
describe('zstack-evidence check', () => {
  ...
  test('check never errors outside a git repo — degrades to STALE', () => {
    expect(run(['run', '--label', 'tests', '--', 'echo green']).status).toBe(0);
    const nonGit = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-evidence-nongit-'));
    try {
...
describe('zstack-evidence run — bun dotenv autoload must not reach the child', () => {
  ...
  function runWith(env: Record<string, string | undefined>, cmd: string) {
    return spawnSync(EVIDENCE, ['run', '--label', 'envprobe', '--', cmd], {
      cwd: repoDir,
      env: { ...process.env, ZSTACK_HOME: zstackHome, ...env },
      encoding: 'utf-8',
      timeout: 60000,
    });
  }
  ...
  test('ZSTACK_EVIDENCE_KEEP_DOTENV=1 restores the old pass-through behaviour', () => {
    fs.writeFileSync(path.join(repoDir, '.env'), 'ZZ_OPTOUT_PROBE=kept\n');
    const r = runWith({ ZSTACK_EVIDENCE_KEEP_DOTENV: '1' }, 'echo "saw=[${ZZ_OPTOUT_PROBE:-absent}]"');

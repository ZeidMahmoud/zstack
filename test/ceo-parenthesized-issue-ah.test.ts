...
test('qid and setup exclusions apply before the new identity form', () => {
  for (const original of calls()) {
    for (const mutate of [
      (c: NativePlanQuestionCall) => { c.questions[0]!.question += ' <zstack-qid:plan-ceo-review-extra>'; },
      (c: NativePlanQuestionCall) => { c.questions[0]!.question = c.questions[0]!.question.replace(/<zstack-qid:[^>]+>/, '<zstack-qid:plan-eng-review-validation>'); },
      (c: NativePlanQuestionCall) => { c.questions[0]!.question = c.questions[0]!.question.replace(/<zstack-qid:[^>]+>/, '<zstack-qid:plan-ceo-review-approach>'); },
      (c: NativePlanQuestionCall) => { c.questions[0]!.question = c.questions[0]!.question.replace(/<zstack-qid:[^>]+>/, '<zstack-qid:plan-ceo-review-mode>'); },

 ...
 * bin/zstack-codex-session-import fix, #2482).
 *
...
describe('empty find must not fall through to cwd (#2483)', () => {
  ...
  test('live block: empty checkpoints dir yields NO checkpoint, not a cwd file', () => {
    ...
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-cwd-'));
    try {
  ...
  test('generated skill discovery checks every host and excludes non-generated trees', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-skill-scan-'));
    const unsafe = ['SKILL.md', path.join('review', 'SKILL.md'),
      ...getExternalHosts().map(host => path.join(host.hostSubdir, 'skills', 'zstack-review', 'SKILL.md')),
      path.join('openclaw', 'skills', 'zstack-openclaw-review', 'SKILL.md')];

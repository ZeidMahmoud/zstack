...
describe('AO completed manual DX handoff preserves report freshness',()=>{
 ...
 test('source, conditional or withdrawn completion facts cannot make a stale report current',()=>{
  ...
  for(const suffix of [' This review is not complete.',' These tasks are not recorded.',' This review is "withdrawn".',' One DX decision remains unresolved.',' We must fix another issue.',' Add another migration task.',' Should we approve another change?',' <zstack-qid:foreign>']) {

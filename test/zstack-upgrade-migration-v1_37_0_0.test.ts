/**
 * Unit tests for zstack-upgrade/migrations/v1.37.0.0.sh — split-engine notice.
 *
...
function makeEnv(opts: {
  ...
  const claudeSkillsBin = join(home, ".claude", "skills", "zstack", "bin");
  const claudeJson = join(home, ".claude.json");
  const gbrainConfig = join(gbrainDir, "config.json");
  const configBin = join(claudeSkillsBin, "zstack-config");
  ...
  // Fake zstack-config: returns "true" iff opted-out (matches the real bin's
...
`,
  ...
  return {
    ...
    home,
    zstackHome,
    doneTouch: join(zstackHome, ".migrations", "v1.37.0.0.done"),
    claudeJson,
    gbrainConfig,
    configBin,
...
function runMigration(env: MigEnv): { stdout: string; stderr: string; exitCode: number } {
  const result = spawnSync("bash", [MIGRATION], {
    ...
    env: {
      ...
      // The script looks for zstack-config at $HOME/.claude/skills/zstack/bin
...
describe("zstack-upgrade/migrations/v1.37.0.0.sh", () => {

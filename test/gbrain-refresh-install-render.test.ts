...
// Static tripwires for the C (machine-wide) render in `zstack-config
...
const SRC = fs.readFileSync(path.join(ROOT, 'bin', 'zstack-config'), 'utf-8');
...
describe('zstack-config gbrain-refresh: machine-wide render guards', () => {
  const branch = okBranch();

  test('targets the global install', () => {
    expect(branch).toContain('$HOME/.claude/skills/zstack');
  });
  ...
  test('verifies it is a real zstack clone before mutating it', () => {

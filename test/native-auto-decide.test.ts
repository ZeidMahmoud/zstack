...
test('native annotation needs one successful post-command load in the owned session',()=>{
  ...
  const f=clone();f.tools.find((e:any)=>e.kind==='use'&&e.name==='Skill').input.skill='zstack:plan-ceo-review';expect(verdict(f)).not.toBeNull();
});
...
test('quotes, source introductions, conditions and incomplete template prose are not annotations',()=>{
  ...
  const f=clone(1);ownedMessage(f).text=ownedMessage(f).text.replace('Heads-up from zstack: this branch has unshipped work.', 'Heads-up from zstack: the following is a hypothetical example.');expect(verdict(f)).toBeNull();
});

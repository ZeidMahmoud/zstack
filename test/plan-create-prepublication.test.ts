...
function fixture(){
 const root=fs.mkdtempSync(path.join(os.tmpdir(),'zstack-prewrite-'));const cwd=path.join(root,'owned'),config=path.join(root,'config');fs.mkdirSync(cwd);
...
test('count capture assembly retains exact prepublication input through refresh, throw and fixture cleanup',()=>{
 ...
 try {
  ...
  const capture=new Function('saveSnapshot','opts','ownedFilePermissions','readPendingWriteInput','fixture','session','startedAt','viewport',code)(
   createPlanCountSnapshotWriter({EVALS_RUN_ID:'count-prepublication-free',ZSTACK_EVAL_DIR:evalDir}),

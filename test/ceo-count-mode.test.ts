...
describe('fixed-scope CEO finding-count mode', () => {
  ...
  test('does not authorize negated, quoted, compound, foreign or finding questions', () => {
    for (const question of [
      ...
      const call = pending(); call.questions[0]!.question = question + ' <zstack-qid:ceo-mode-selection>';
    ...
    for (const suffix of [' <zstack-qid:ceo-mode-selection>', ' <zstack-qid:broken']) {

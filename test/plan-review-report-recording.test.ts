...
const CASE = '/plan-eng-review writes ZSTACK REVIEW REPORT to plan file';
  ...
  const args: Record<string, any> = {
    ...
    runSkillTest: async (options: any) => {
      ...
      const replacements: Record<string, [string, string]> = {
        ...
        report: ['## ZSTACK REVIEW REPORT', '## Different report'],
        header: ['| Review |', '| Kind |'],
...
for (const [kind, expected] of [
  ...
  ['report', '## ZSTACK REVIEW REPORT'], ['header', 'Review'], ['ceo', 'CEO Review'],
...
test('retry attempts own pristine fixtures and namespaced serialization preserves failed and passed records', async () => {
  ...
  expect(actual.calls.every(call => !call.seed.includes('## ZSTACK REVIEW REPORT'))).toBe(true);

/**
 * zstack-slug cache-read sanitization.
 *
 * `eval "$(zstack-slug)"` is how callers load SLUG/BRANCH. The compute and
...
describe('zstack-slug cache-read sanitization', () => {
  test('a poisoned cache file cannot inject shell metacharacters into output', () => {
    ...
    try {
      const cacheDir = path.join(home, '.zstack', 'slug-cache');
...
// The ZSTACK_PROJECT_SLUG escape hatch is per-invocation, never durable: a
...
// The cache is also ZSTACK_HOME-aware now, matching lib/bin-context.ts's
// native port — temp-home runs must not litter the real ~/.zstack (observed:
  ...
  test('an env-override run never writes the cwd cache', () => {
    ...
    try {
      const r = spawnSync(['bash', SLUG_BIN], {
        cwd: os.tmpdir(),
        env: { ...process.env, ZSTACK_HOME: home, ZSTACK_PROJECT_SLUG: 'override-slug' },
        timeout: 30_000,
      });

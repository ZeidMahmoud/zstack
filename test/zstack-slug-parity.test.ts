 ...
 *    reaches a canonical remote, zstack-slug's SLUG equals remote-slug's
 ...
 *
 * Test pattern mirrors test/zstack-slug-cwd-walk-up.test.ts: per-test
...
const SLUG_SCRIPT = path.join(ROOT, 'bin', 'zstack-slug');
...
function baseEnv(tmpHome: string): Record<string, string | undefined> {
  // Drop any ambient override: a sibling test leaking ZSTACK_PROJECT_SLUG in
  // a shared-process shard would flip runs into override mode.
  const { ZSTACK_PROJECT_SLUG: _drop, ...ambient } = process.env;
  return { ...ambient, HOME: tmpHome, ZSTACK_HOME: path.join(tmpHome, '.zstack') };
}
...
function expectParity(cwd: string, tmpHome: string, expected: string): void {
  const zstack = runSlug(cwd, tmpHome);
  ...
  expect(slugOf(zstack)).toBe(expected);
  expect(remoteOut).toBe(expected);
  expect(slugOf(zstack)).toBe(remoteOut);
}

describe('zstack-slug ↔ remote-slug parity', () => {
  let tmpHome: string;
  let fixtures: string;
  ...
  test('LIVE BUG SHAPE: stray empty .git on an ancestor "home" no longer degrades the slug', () => {
    ...
    git(['-C', main, 'worktree', 'add', '-q', wt, '-b', 'zstack-fix-wave']);
  ...
  test('walk-up preserved: nested inner repo (no remote) resolves to the OUTER repo slug', () => {
    ...
    const zstack = runSlug(inner, tmpHome);
    expect(zstack.status).toBe(0);
    ...
    expect(slugOf(zstack)).toBe('acme-outer');
  ...
  test('fallback unchanged: no-remote repo resolves to its basename (and remote-slug agrees)', () => {
    ...
    expect(zstack.status).toBe(0);
    expect(slugOf(zstack)).toBe('lonely');
  ...
  test('hostile origin `url = ..` cannot become a ".." slug — basename fallback holds', () => {
    ...
    // ABOVE ~/.zstack/projects/ (confined to ~/.zstack, but still traversal).
    ...
    const cacheFile = path.join(tmpHome, '.zstack', 'slug-cache', encodedCacheKey(repo));
  ...
  test('sticky identity preserved (#2212): repo that adopted a remote after first use is NOT healed', () => {
    ...
    const zstack = runSlug(repo, tmpHome);
    expect(zstack.status).toBe(0);
    expect(slugOf(zstack)).toBe('stickyproj');

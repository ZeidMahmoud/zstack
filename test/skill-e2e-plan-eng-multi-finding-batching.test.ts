...
const FIXTURE_PLAN_PATH = '/tmp/zstack-test-plan-eng-batching.md';
...
describeE2E('/plan-eng-review multi-finding batching regression (periodic)', () => {
  test(
    ...
    async () => {
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-e2e-plan-eng-batching-'));
      const planPath = path.join(tmpDir, 'zstack-test-plan-eng-batching.md');

...
function setupZstackRepo(remoteUrl: string) {
  ...
  spawnSync('git', ['-C', zstackHome, 'init', '-q', '-b', 'main'], { stdio: 'pipe' });
  spawnSync('git', ['-C', zstackHome, 'config', 'user.email', 'test@example.com'], { stdio: 'pipe' });
  spawnSync('git', ['-C', zstackHome, 'config', 'user.name', 'test'], { stdio: 'pipe' });
  fs.writeFileSync(path.join(zstackHome, '.brain-allowlist'), '# allowlist\n');
  spawnSync('git', ['-C', zstackHome, 'add', '.'], { stdio: 'pipe' });
  spawnSync('git', ['-C', zstackHome, 'commit', '-q', '-m', 'init'], { stdio: 'pipe' });
  spawnSync('git', ['-C', zstackHome, 'remote', 'add', 'origin', remoteUrl], { stdio: 'pipe' });
}
...
describe('zstack-gbrain-source-wireup — defensive paths', () => {
  test('--no-pull skips HEAD advance on existing worktree', () => {
    ...
    spawnSync('git', ['-C', zstackHome, 'add', '.'], { stdio: 'pipe' });
    spawnSync('git', ['-C', zstackHome, 'commit', '-q', '-m', 'second commit'], { stdio: 'pipe' });

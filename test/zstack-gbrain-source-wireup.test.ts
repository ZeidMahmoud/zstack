/**
 * zstack-gbrain-source-wireup — unit tests with mocked gbrain CLI.
 *
 * The helper registers the zstack brain repo as a gbrain federated source
...
const WIREUP_BIN = path.join(BIN_DIR, 'zstack-gbrain-source-wireup');
...
function run(
  ...
  const env = {
    ...
    ZSTACK_BRAIN_WORKTREE: worktreeDir,
    ZSTACK_BRAIN_NO_SYNC: '0',
    ...(opts.env || {}),
  };
...
function setupZstackRepo(remoteUrl: string) {
  // Real git repo at zstackHome with at least one commit + an origin remote.
  fs.mkdirSync(zstackHome, { recursive: true });
  spawnSync('git', ['-C', zstackHome, 'init', '-q', '-b', 'main'], { stdio: 'pipe', timeout: 30_000 });
  spawnSync('git', ['-C', zstackHome, 'config', 'user.email', 'test@example.com'], { stdio: 'pipe', timeout: 30_000 });
  spawnSync('git', ['-C', zstackHome, 'config', 'user.name', 'test'], { stdio: 'pipe', timeout: 30_000 });
  fs.writeFileSync(path.join(zstackHome, '.brain-allowlist'), '# allowlist\n');
  spawnSync('git', ['-C', zstackHome, 'add', '.'], { stdio: 'pipe', timeout: 30_000 });
  spawnSync('git', ['-C', zstackHome, 'commit', '-q', '-m', 'init'], { stdio: 'pipe', timeout: 30_000 });
  spawnSync('git', ['-C', zstackHome, 'remote', 'add', 'origin', remoteUrl], { stdio: 'pipe', timeout: 30_000 });
}

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-wireup-test-'));
  zstackHome = path.join(tmpHome, '.zstack');
  worktreeDir = path.join(tmpHome, '.zstack-brain-worktree');
...
describe('zstack-gbrain-source-wireup — wireup mode', () => {
  test('fresh state: registers source + creates worktree + syncs', () => {
    setupZstackRepo('git@github.com:user/zstack-brain-user.git');
    makeFakeGbrain({});
    const r = run([], { env: { ZSTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    expect(fs.existsSync(worktreeDir)).toBe(true);
    const state = readState();
    expect(state.sources).toHaveLength(1);
    expect(state.sources[0].id).toBe('zstack-brain-user');
  ...
  test('the real sync targets the REGISTERED source, never --repo (#2662)', () => {
    ...
    // WITHOUT ZSTACK_BRAIN_NO_SYNC — the skip-mode cases never reach the sync,
    ...
    expect(calls.some((c) => c.startsWith('gbrain sync --source zstack-brain-user'))).toBe(true);
  ...
  test('idempotent re-run after success: no new sources add call', () => {
    setupZstackRepo('git@github.com:user/zstack-brain-user.git');
    makeFakeGbrain({});
    run([], { env: { ZSTACK_BRAIN_NO_SYNC: '1' } });
  ...
  test('drift recovery: existing source with different path triggers remove + add', () => {
    ...
    fs.writeFileSync(
      gbrainStateFile,
      JSON.stringify({
        sources: [{ id: 'zstack-brain-user', local_path: '/old/stale/path', federated: true }],
      })
    );
    const r = run([], { env: { ZSTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    const calls = gbrainCalls();
    expect(calls.some((c) => c.startsWith('gbrain sources remove zstack-brain-user'))).toBe(true);
    expect(calls.some((c) => c.includes(`gbrain sources add zstack-brain-user --path ${worktreeDir}`))).toBe(true);
    const state = readState();
    expect(state.sources[0].local_path).toBe(worktreeDir);
  });
  ...
  test('--strict + gbrain present in controlled dir: exits 0 (positive control)', () => {
    ...
    makeFakeGbrain({});
    const r = run(['--strict'], { env: { ZSTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
  ...
  test('source-id derived from origin URL', () => {
    setupZstackRepo('git@github.com:user/zstack-brain-alice.git');
    makeFakeGbrain({});
    const r = run([], { env: { ZSTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    expect(readState().sources[0].id).toBe('zstack-brain-alice');
  });

  test('source-id fallback to ~/.zstack-brain-remote.txt when .git is gone', () => {
    // No git repo at zstackHome; just the remote-file
    fs.mkdirSync(tmpHome, { recursive: true });
    fs.writeFileSync(
      path.join(tmpHome, '.zstack-brain-remote.txt'),
      'git@github.com:user/zstack-brain-bob.git\n'
    );
    makeFakeGbrain({});
    // No --strict: helper should benign-skip because .zstack/.git is missing
    const r = run([]);
  ...
  test('source-id from --source-id flag overrides everything', () => {
    setupZstackRepo('git@github.com:user/zstack-brain-different.git');
    makeFakeGbrain({});
    run(['--source-id', 'custom-id'], { env: { ZSTACK_BRAIN_NO_SYNC: '1' } });
    const state = readState();
  ...
  test('--probe: read-only, prints state without mutating', () => {
    ...
    const r = run(['--probe']);
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('source_id=zstack-brain-user');
...
describe('zstack-gbrain-source-wireup — ZeroEntropy sunset advisory (#2365)', () => {
  ...
  test('advisory docs entry exists (USING_GBRAIN_WITH_ZSTACK.md content pin)', () => {
    const doc = fs.readFileSync(path.join(ROOT, 'USING_GBRAIN_WITH_ZSTACK.md'), 'utf-8');
    expect(doc).toContain('September 4, 2026');
    expect(doc).toContain('#2365');
  });
});

describe('zstack-gbrain-source-wireup — --database-url lock (defends against external config rewrites)', () => {
  test('--database-url flag is exported as GBRAIN_DATABASE_URL to child gbrain calls', () => {
    ...
    const r = run(['--database-url', TARGET], { env: { ZSTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    const calls = gbrainCalls();
...
describe('zstack-gbrain-source-wireup — uninstall mode', () => {
...
describe('zstack-gbrain-source-wireup — defensive paths', () => {
  test('--no-pull skips HEAD advance on existing worktree', () => {
    ...
    fs.writeFileSync(path.join(zstackHome, 'newfile.md'), 'new');
    spawnSync('git', ['-C', zstackHome, 'add', '.'], { stdio: 'pipe', timeout: 30_000 });
    spawnSync('git', ['-C', zstackHome, 'commit', '-q', '-m', 'second commit'], { stdio: 'pipe', timeout: 30_000 });
    const parentHeadAfter = spawnSync('git', ['-C', zstackHome, 'rev-parse', 'HEAD'], {
      encoding: 'utf-8',
      timeout: 30_000,
    }).stdout.trim();
    ...
    const r = run(['--no-pull'], { env: { ZSTACK_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);

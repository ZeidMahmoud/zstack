...
describe('setup-gbrain Path 4 (Remote MCP) — structural contract', () => {
  ...
  test('Step 4c invokes zstack-gbrain-mcp-verify and STOPs on failure', () => {
    expect(tmpl).toContain('zstack-gbrain-mcp-verify');
  ...
  test('Step 7 calls zstack-artifacts-init with --url-form-supported flag', () => {
    expect(tmpl).toMatch(/zstack-artifacts-init.*--url-form-supported/);
  });

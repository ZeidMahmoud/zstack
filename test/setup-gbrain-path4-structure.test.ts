...
//     by unit tests for zstack-gbrain-mcp-verify and zstack-artifacts-init.
//   - The structural prose is the source of regressions for AUQ pacing
//     (the failure mode the zstack repo has tracked since v1.26.x:
...
describe('setup-gbrain carve — dispatch stays in the skeleton', () => {
  ...
  test('the carved bodies moved OUT of the skeleton (no leak-back)', () => {
    ...
    expect(skeleton).not.toContain('zstack-memory-ingest.ts --probe');
...
describe('setup-gbrain Path 4 (Remote MCP) — structural contract', () => {
  ...
  test('Step 4c invokes zstack-gbrain-mcp-verify and STOPs on failure', () => {
    expect(brainInit).toContain('zstack-gbrain-mcp-verify');
  ...
  test('Step 7 calls zstack-artifacts-init with --url-form-supported flag (skeleton)', () => {
    expect(skeleton).toMatch(/zstack-artifacts-init.*--url-form-supported/);
  });

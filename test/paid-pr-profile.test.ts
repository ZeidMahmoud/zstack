...
describe('PR profile paid-runner integration', () => {
  ...
  test('version-only release changes are verified against the real merge-base before exemption', () => {
    ...
    try {
      git(['init', '-b', 'main']);
      git(['config', 'user.email', 'test@zstack.local']);
  ...
  test('name filters handle literal labels and never select broad raw tests', () => {
    ...
    expect(report.test('Plan Review Report E2E /plan-eng-review writes ZSTACK REVIEW REPORT to plan file')).toBe(true);
    ...
    try {
      ...
      const options = {
        ...
        env: { PATH: path.dirname(process.execPath), ...(process.env.SystemRoot ? { SystemRoot: process.env.SystemRoot } : {}),
          HOME: root, ZSTACK_HOME: path.join(root, 'state'), ZSTACK_EVAL_DIR: path.join(root, 'evals'),
          ZSTACK_CLAUDE_CLI_VERSION: 'free-fixture', EVALS: '1', EVALS_TIER: 'gate', EVALS_PREFLIGHT_OK: '1',

...
`);
  try {
    ...
    expect(fs.readdirSync(directory).filter(name => name.startsWith('zstack-plan-count-') || name.startsWith('zstack-native-review-state-'))).toEqual([]);

...
const CSO_SWITCH_START = 'if [ "${ZSTACK_SETUP_RUNNING:-0}" = "1" ] && [ "${ZSTACK_SETUP_SKIP_CSO_BUILD:-0}" = "1" ]; then';
...
function makeTree(opts: { exe?: string } = {}): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-needs-build-'));
  tmpDirs.push(dir);
  const exe = opts.exe ?? '';
  ...
  writeAt(path.join(dir, `bin/zstack-cso-core${exe}`), BIN_T, 0o755);
  writeAt(path.join(dir, `bin/zstack-cso-launcher${exe}`), BIN_T, 0o755);
  const generation = path.join(dir, 'bin/.zstack-cso-generation');
  fs.writeFileSync(generation, `${'a'.repeat(64)}\n`, { mode: 0o600 });
  fs.utimesSync(generation, BIN_T, BIN_T);
  if (exe) {
    const generationLock = path.join(dir, 'bin/.zstack-cso-generation.lock');
    fs.writeFileSync(generationLock, '', { mode: 0o600 });
    fs.utimesSync(generationLock, BIN_T, BIN_T);
  }
  if (!exe) writeAt(path.join(dir, 'bin/zstack-cso-watchdog'), BIN_T, 0o755);
...
function decide(dir: string, opts: { isWindows?: '0' | '1'; csoAvailable?: '0' | '1' } = {}): number {
  const script = [
    'set -e',
    `SOURCE_ZSTACK_DIR="${dir}"`,
    'BROWSE_BIN="$SOURCE_ZSTACK_DIR/browse/dist/browse"',
...
describe('setup: NEEDS_BUILD static invariants', () => {
  ...
  test('all required binaries are existence-checked with -x and the $_EXE suffix', () => {
    ...
    expect(block).toContain('[ ! -x "$SOURCE_ZSTACK_DIR/design/dist/design$_EXE" ]');
    expect(block).toContain('[ ! -x "$SOURCE_ZSTACK_DIR/make-pdf/dist/pdf$_EXE" ]');
  ...
  test('setup owns the only CSO build escape hatch and probes Bun hardening flags', () => {
    ...
    expect(BUILD_SRC).toContain('[ "${ZSTACK_SETUP_RUNNING:-0}" = "1" ]');
    expect(BUILD_SRC).toContain('[ "${ZSTACK_SETUP_SKIP_CSO_BUILD:-0}" = "1" ]');
  ...
  test('the skip flag alone cannot weaken a direct build; setup can omit and purge CSO', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-cso-build-switch-'));
    tmpDirs.push(dir);
    for (const sub of ['bin', 'scripts']) fs.mkdirSync(path.join(dir, sub), { recursive: true });
    for (const name of ['zstack-cso-launcher', 'zstack-cso-launcher.exe', 'zstack-cso-core', 'zstack-cso-core.exe', 'zstack-cso-watchdog']) {
    ...
    let result = runBashScript(`set -e\n${csoBuildSwitchBlock()}`, {
      cwd: dir, env: { ...process.env, ZSTACK_SETUP_SKIP_CSO_BUILD: '1' },
    });
    ...
    result = runBashScript(`set -e\n${csoBuildSwitchBlock()}`, {
      cwd: dir, env: { ...process.env, ZSTACK_SETUP_RUNNING: '1', ZSTACK_SETUP_SKIP_CSO_BUILD: '1' },
    });
    expect(result.status).toBe(0);
    expect(fs.existsSync(path.join(dir, 'cso-called'))).toBe(false);
    expect(fs.readdirSync(path.join(dir, 'bin')).filter(name => name.startsWith('zstack-cso-'))).toEqual([]);
  });
  ...
  test('the staleness find walks every embedded source root, lib/ included', () => {
    ...
    for (const root of ['browse/src', 'make-pdf/src', 'design/src', 'lib']) {
      expect(block).toContain(`"$SOURCE_ZSTACK_DIR/${root}"`);
    }
    expect(block).toContain('-type f -newer "$BUILD_STAMP"');
    expect(block).toContain('"$SOURCE_ZSTACK_DIR/package.json" -nt "$BUILD_STAMP"');
    expect(block).toContain('[ -f "$SOURCE_ZSTACK_DIR/bun.lock" ] && [ "$SOURCE_ZSTACK_DIR/bun.lock" -nt "$BUILD_STAMP" ]');
  });
});
...
describe('setup: NEEDS_BUILD decision executes', () => {
  ...
  test('an interrupted partial build cannot let a refreshed browse binary hide stale CSO outputs', () => {
    ...
    expect(fs.statSync(path.join(dir, 'bin/zstack-cso-core')).mtimeMs).toBeLessThan(
  ...
  test.each(['bin/zstack-cso-launcher','bin/zstack-cso-core', 'bin/zstack-cso-watchdog'])('%s missing → 1', binary => {
    const dir = makeTree();
  ...
  test('CSO generation manifest missing → 1', () => {
    const dir = makeTree();
    fs.unlinkSync(path.join(dir, 'bin/.zstack-cso-generation'));
    expect(decide(dir)).toBe(1);
  });

  test('Windows CSO generation lock missing → 1', () => {
    const dir = makeTree({ exe: '.exe' });
    fs.unlinkSync(path.join(dir, 'bin/.zstack-cso-generation.lock'));
  ...
  test('unavailable CSO removes stale helpers and does not force a repeat build', () => {
    ...
    for (const binary of ['bin/zstack-cso-launcher', 'bin/zstack-cso-core', 'bin/zstack-cso-watchdog', 'bin/.zstack-cso-generation']) {

 ...
 * POST (bin/zstack-telemetry-sync). This test enforces that contract three ways:
 *
...
describe('telemetry no-repo-identity-egress invariant', () => {
  ...
  test('coverage: every repo/branch field the producers emit into skill-usage.jsonl is stripped', () => {
    ...
    // preamble appends directly; zstack-telemetry-log builds the synced event
    ...
    for (const field of emitted) {
      expect(
        ...
        `producer emits repo-identity field "${field}" but zstack-telemetry-sync does not strip it (would leak to remote)`,
      ).toBe(true);
    }
  });

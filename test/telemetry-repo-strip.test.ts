 ...
 *
 *   - zstack-skill-start (skill_run event)   → "repo"
 *     (bin/zstack-skill-start)
 *   - zstack-skill-end (completion event)    → (no repo identity today,
 *     scanned so drift is caught)             (bin/zstack-skill-end)
 *   - zstack-telemetry-log                   → "_repo_slug", "_branch"
 *     (bin/zstack-telemetry-log)
 *
 * zstack-telemetry-sync MUST strip every one of those fields before the remote
 * POST (bin/zstack-telemetry-sync). The script has TWO strip paths — jq del()
...
const SYNC = path.join(ROOT, 'bin', 'zstack-telemetry-sync');
const SKILL_START = path.join(ROOT, 'bin', 'zstack-skill-start');
const SKILL_END = path.join(ROOT, 'bin', 'zstack-skill-end');
const TEL_LOG = path.join(ROOT, 'bin', 'zstack-telemetry-log');
 ...
 * file zstack-telemetry-sync reads and uploads. Scoped to the emission lines
...
describe('telemetry no-repo-identity-egress invariant', () => {
  ...
  // bash); zstack-telemetry-log builds the synced event with a
  ...
  test('coverage: every repo/branch field the producers emit into skill-usage.jsonl is stripped (sed fallback path)', () => {
    // zstack-skill-start must emit "repo" — guards against the test silently
    ...
    for (const field of emitted) {
      expect(
        ...
        `producer emits repo-identity field "${field}" but zstack-telemetry-sync's sed fallback does not strip it (would leak to remote)`,
      ).toBe(true);
    }
  });

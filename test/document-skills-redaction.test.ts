...
describe("/document-release redaction", () => {
  test("scans the PR-body temp file before gh pr edit", () => {
    const scanIdx = RELEASE.indexOf('zstack-redact --from-file "<run-dir>/body.md"');
  ...
  test("separate shell calls share an explicit run directory and never re-read raw tracker text", () => {
    expect(RELEASE).toContain('mktemp -d /tmp/zstack-doc-release-XXXXXXXX');
    expect(RELEASE).not.toContain('/tmp/zstack-pr-body-$$');
...
describe("/document-generate redaction", () => {
  test("scans staged doc diff before commit", () => {
    const scanIdx = GENERATE.indexOf("zstack-redact --repo-visibility");
  ...
  test("scans added lines of the staged diff", () => {
    expect(GENERATE).toMatch(/git diff --cached[\s\S]{0,80}zstack-redact/);
  });

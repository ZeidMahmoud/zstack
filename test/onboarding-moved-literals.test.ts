 ...
 * into bin/zstack-skill-start's instruction-emission layer. This guard pins
...
const MOVED: Record<string, string> = {
  ...
  'first-loop-tip': 'zstack pays off when you complete one loop',
...
describe('onboarding moved-literals tombstone (F5)', () => {
  test('every moved flow lives in bin/zstack-skill-start', () => {
  ...
  test('no generated SKILL.md re-inlines a moved literal', () => {
    ...
    for (const f of generatedSkillFiles()) {
      ...
      // The zstack-upgrade skill legitimately documents its own inline
      // upgrade flow — that's the flow's HOME, not a re-inline.
      const skipUpgrade = f.includes(`zstack-upgrade${path.sep}`);

...
function compileLauncher(directory:string,coreDigest:string,testing=false){const output=join(directory,'zstack-cso-launcher'),args=[`-DZSTACK_CSO_CORE_SHA256=\"${coreDigest}\"`,...(testing?['-DZSTACK_CSO_TESTING']:[])];compileC(join(ROOT,'lib/cso/launcher.c'),output,args);return output;}
...
describe.skipIf(process.platform==='win32')('CSO native generation handoff',()=>{
  test('the launcher rejects changed core bytes even when the generation manifest is unchanged',()=>{
    const directory=temporary(),core=join(directory,'zstack-cso-core'),source=join(directory,'core.c');
    writeFileSync(source,'#include <stdio.h>\nint main(void){puts("OLD");return 0;}\n');compileC(source,core);const expected=digest(core),launcher=compileLauncher(directory,expected);writeFileSync(join(directory,'.zstack-cso-generation'),`${expected}\n`);
  ...
  test.skipIf(process.platform!=='linux')('the launcher executes the verified descriptor across a noncooperative pathname swap',async()=>{
    const directory=temporary(),core=join(directory,'zstack-cso-core'),source=join(directory,'core.c'),ready=join(directory,'ready'),release=join(directory,'release');
    writeFileSync(source,'#include <stdio.h>\nint main(void){puts("VERIFIED");return 0;}\n');compileC(source,core);const expected=digest(core),launcher=compileLauncher(directory,expected,true);writeFileSync(join(directory,'.zstack-cso-generation'),`${expected}\n`);
  ...
  test('an already-loaded old launcher rejects the publisher-winning core generation',async()=>{
    const directory=temporary(),core=join(directory,'zstack-cso-core'),source=join(directory,'core.c'),ready=join(directory,'ready'),release=join(directory,'release');
    writeFileSync(source,'#include <stdio.h>\nint main(void){puts("OLD");return 0;}\n');compileC(source,core);const oldDigest=digest(core),launcher=compileLauncher(directory,oldDigest,true);writeFileSync(join(directory,'.zstack-cso-generation'),`${oldDigest}\n`);
  ...
  test('Bun retains the shared generation lock while the core runs and drops it before detached children',async()=>{
    const directory=temporary(),source=join(directory,'core.ts'),core=join(directory,'zstack-cso-core'),launcher=join(directory,'zstack-cso-launcher'),locker=join(directory,'zstack-cso-publish-lock'),ready=join(directory,'ready'),release=join(directory,'release'),childReady=join(directory,'child-ready'),childRelease=join(directory,'child-release'),published=join(directory,'published');
    writeFileSync(source,`import{fstatSync,writeFileSync,existsSync}from'node:fs';const fd=Number(process.env.ZSTACK_CSO_GENERATION_LOCK_FD);if(!Number.isInteger(fd)||!fstatSync(fd).isDirectory())process.exit(71);writeFileSync(${JSON.stringify(ready)},'ready');while(!existsSync(${JSON.stringify(release)}))await Bun.sleep(5);const child=Bun.spawn(['/bin/sh','-c',${JSON.stringify(`touch '${childReady}'; while [ ! -f '${childRelease}' ]; do sleep .01; done`)}],{stdin:'ignore',stdout:'ignore',stderr:'ignore'});child.unref();`);
    const built=spawnSync(process.execPath,['build','--compile','--no-compile-autoload-dotenv','--no-compile-autoload-bunfig','--no-compile-autoload-tsconfig','--no-compile-autoload-package-json',source,'--outfile',core],{encoding:'utf8',timeout:60_000});expect(built.status,built.stderr).toBe(0);const coreDigest=digest(core);compileLauncher(directory,coreDigest);writeFileSync(join(directory,'.zstack-cso-generation'),`${coreDigest}\n`);compileC(join(ROOT,'lib/cso/publish-lock.c'),locker);

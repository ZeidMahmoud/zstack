...
const requestedStack = process.env.ZSTACK_CSO_TEST_STACK;
const requested = process.env.ZSTACK_CSO_DOCKER_TESTS === '1' && ['bun', 'python', 'rails'].includes(requestedStack ?? '');
...
beforeAll(async () => {
  if (!requested) return;
  const image = process.env.ZSTACK_CSO_TEST_IMAGE;
  if (!image) throw new Error(`${requestedStack} cold-start qualification requires ZSTACK_CSO_TEST_IMAGE; a requested gate cannot skip`);
  if (process.env.ZSTACK_CSO_TEST_PLATFORM !== platform) throw new Error(`${requestedStack} cold-start qualification requires native ${platform}`);
  const versions = JSON.parse(process.env.ZSTACK_CSO_EXPECTED_VERSIONS || '{}') as Record<string, string>;
  ...
  root = fs.mkdtempSync(path.join(os.tmpdir(), `cso-${requestedStack}-cold-`)); process.env.ZSTACK_HOME = path.join(root, 'state');
  watchdog = path.resolve(import.meta.dir, '../bin/zstack-cso-watchdog');
  ...
  if (requestedStack === 'rails') {
    const postgresImage = process.env.ZSTACK_CSO_TEST_POSTGRES_IMAGE;
    const postgresVersion = process.env.ZSTACK_CSO_TEST_POSTGRES_VERSION;
...
afterAll(() => { if (root) fs.rmSync(root, { recursive: true, force: true }); delete process.env.ZSTACK_HOME; });

...
describeIfSelected('Session Intelligence E2E', [
  ...
  beforeAll(() => {
    ...
    zstackHome = path.join(workDir, '.zstack-home');
    ...
    // Compute slug (same logic as zstack-slug without git remote)
  ...
  // Write a timeline event via zstack-timeline-log, read it back via zstack-timeline-read.
  ...
  testConcurrentIfSelected('timeline-event-flow', async () => {
    const projectDir = path.join(zstackHome, 'projects', slug);
    fs.mkdirSync(projectDir, { recursive: true });

    // Write two events via the binary
    const logBin = path.join(workDir, 'bin', 'zstack-timeline-log');
    const readBin = path.join(workDir, 'bin', 'zstack-timeline-read');
    const env = { ...process.env, ZSTACK_HOME: zstackHome };
    ...
    // Verify zstack-timeline-read output includes the events
...
IMPORTANT:
- Use ZSTACK_HOME="${zstackHome}" as an environment variable when running bin scripts.
- The bin scripts are at ./bin/ (relative to this directory), not at ~/.claude/skills/zstack/bin/.
  Replace any references to ~/.claude/skills/zstack/bin/ with ./bin/ when running commands.
...
- Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('context-restore-loads-latest', async () => {
    ...
    // zstackHome's checkpoints dir also receives the context-save test's
    ...
    // (the agent may derive the dir from ZSTACK_HOME/projects/<slug>, so the
    // whole home moves, not just the checkpoint path we hand it).
    const restoreHome = path.join(workDir, '.zstack-restore-home');
...
IMPORTANT:
- Use ZSTACK_HOME="${restoreHome}" as an environment variable when running bin scripts.
- The bin scripts are at ./bin/ (relative to this directory), not at ~/.claude/skills/zstack/bin/.

...
const state = process.env.ZSTACK_HOME;
const helpers = [...prompt.matchAll(/\x60([^\x60]+\/bin\/zstack-review-(?:log|read))\x60/g)].map(m => m[1]);
if (helpers.length !== 2) throw new Error('Missing canonical helpers');
if (process.env.ZSTACK_STATE_ROOT !== state || path.dirname(state) !== process.cwd()) throw new Error('Foreign state');
...
fs.appendFileSync(${JSON.stringify(observedFile)},JSON.stringify({args,prompt,state,stateRoot:process.env.ZSTACK_STATE_ROOT,helpers,record,writeStatus:write.status,readStatus:read.status,readOutput:read.stdout,files,rows:files.flatMap(file=>fs.readFileSync(path.join(state,file),'utf8').trim().split('\n').map(JSON.parse))})+'\n');
...
mock.module(${JSON.stringify(source)},()=>({...capture,captureSectionReads:async opts=>{
  ...
  const stateDirs=()=>fs.readdirSync(opts.planDir).filter(n=>n.startsWith('.zstack-section-state-'));
  try {
...
`);
  const env = { ...process.env, PATH: `${bin}${path.delimiter}${process.env.PATH ?? ''}`, TMPDIR: dir, TMP: dir, TEMP: dir,
    ZSTACK_HOME: operatorState, ZSTACK_STATE_ROOT: operatorState, EVALS_HERMETIC: '1', EVALS: '', EVALS_ALL: '', ZSTACK_CARVE_SKILL: '' };
  delete env.CI;
  ...
  try {
    ...
    for (const launch of launches) {
      ...
      expect(launch.helpers).toEqual(['zstack-review-log','zstack-review-read'].map(name=>path.join(root,'bin',name)));
...
for(const scenario of ['delivery','timeout','timeout-partial','synthetic-success']) {
  ...
  const capture=await captureSectionReads({planDir:${JSON.stringify(dir)},skillName:'plan-eng-review',scenario:'capture-case:'+scenario,decisionPolicy:'- Preserve the supplied author scope; do not approve excluded work.',reportFile:'PLAN.md',reportMarker:/^## ZSTACK REVIEW REPORT\\s*$/m,testName:'eng-checkpoints',timeout:scenario.startsWith('timeout')?300:480000,model:'fake-model'});

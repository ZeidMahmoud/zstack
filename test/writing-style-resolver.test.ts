 ...
 * - Codex output uses $ZSTACK_BIN, not ~/.claude/... (host-aware paths)
...
// inline preamble bash into bin/zstack-skill-start; the render keeps the
...
describe('Writing Style preamble section', () => {
  ...
  test('EXPLAIN_LEVEL is echoed by zstack-skill-start and read by tier 2+ prose', () => {
  ...
  test('Codex tier-2 preamble uses host-aware path (no .claude/)', () => {
    ...
    // The config read moved into zstack-skill-start, which resolves its bin
    ...
    expect(fenceLine).toContain('$ZSTACK_BIN');
  });
  ...
  test('migration prompt lives in zstack-skill-start (marker-gated emit)', () => {
    // Token-reduction Phase 2: the one-time V0→V1 migration prompt left the
    // rendered preamble; bin/zstack-skill-start computes the gate from the
    // marker files and emits it as a ZSTACK_INSTRUCTION block, with the ack

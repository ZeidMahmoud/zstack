/**
 * Regression test — bin/zstack-slug must resolve to the OUTERMOST project root
 ...
 * `bin/zstack-slug` derived its slug from the literal `pwd` with no walk-up.
 ...
 * (no manual `rm -rf ~/.zstack/slug-cache/` required).
 *
...
function runSlug(
  ...
  // ZSTACK_HOME must be pinned to the temp home too: the cache dir is
  // ZSTACK_HOME-aware (matching lib/bin-context.ts's native port), and a
  // sibling test file leaking process.env.ZSTACK_HOME in a shared-process
  ...
  // script's cache writes land in tmpHome, never AJ's real ~/.zstack.
  const env = { ...process.env, HOME: tmpHome, ZSTACK_HOME: path.join(tmpHome, '.zstack'), ...extraEnv };
...
describe('zstack-slug — outermost project-root resolution', () => {
  let tmpHome: string;
  ...
  beforeEach(() => {
    ...
    tmpHome = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-slug-test-')));
    projectsRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-slug-projects-')));
  });
  ...
  test('AC-2: stale cache for subdir pwd is overwritten with correct outermost-root slug', () => {
    ...
    const cacheDir = path.join(tmpHome, '.zstack', 'slug-cache');
  ...
  // Edge case: ZSTACK_PROJECT_SLUG env override wins over walk-up (documented escape hatch).
  test('ZSTACK_PROJECT_SLUG env override beats every other resolution path', () => {
    const projectRoot = path.join(projectsRoot, 'loadout');
    const siteSubdir = path.join(projectRoot, 'site');
    fs.mkdirSync(path.join(projectRoot, '.git'), { recursive: true });
    fs.mkdirSync(path.join(siteSubdir, '.vercel'), { recursive: true });
    ...
    const result = runSlug(siteSubdir, tmpHome, { ZSTACK_PROJECT_SLUG: 'custom-override' });
    expect(result.status).toBe(0);
    const { slug } = parseSlug(result.stdout);
...
describe('_outermost_project_root termination (windows-free-tests regression)', () => {
  ...
  const script = fs.readFileSync(path.join(ROOT, 'bin', 'zstack-slug'), 'utf-8');

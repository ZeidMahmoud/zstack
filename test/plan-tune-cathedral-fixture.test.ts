  ...
  const args: Record<string, any> = {
    ...
    fs: { ...fs,
      ...
      rmSync(p: string,opts: any) {
        owned(p); const memory=path.join(p,'.zstack-state/free-text-memory.json');
    ...
    spawnSync: (bin: string,argv: string[],opts: any) => {
      if(bin!=='git') {
        owned(bin);
        expect(['question-log-hook','question-preference-hook','zstack-codex-session-import','zstack-distill-apply']).toContain(path.basename(bin));
        owned(opts.env.ZSTACK_STATE_ROOT);
      }
...
test('Plain Claude cathedral contracts stay isolated from either inherited Conductor marker', async () => {
  for (const hostEnv of [
    ...
    { CONDUCTOR_PORT: '55070', ZSTACK_SESSION_KIND: 'spawned', OPENCLAW_SESSION: 'synthetic-session' },
  ]) {

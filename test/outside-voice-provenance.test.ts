...
describe('outside-review provenance through actual log and dashboard reader', () => {
  test('mixed history preserves native/external Claude identities, phase failures, and multiple model usage', () => {
    const scratch = mkdtempSync(join(tmpdir(), 'zstack-outside-provenance-'));
    const repo = join(scratch, 'repo');
    mkdirSync(repo);
    const env: NodeJS.ProcessEnv = { ...process.env, ZSTACK_HOME: join(scratch, 'state'), ZSTACK_PROJECT_SLUG: 'outside-provenance-fixture' };
    ...
    try {
      ...
      for (const record of records) run([join(ROOT, 'bin/zstack-review-log'), JSON.stringify(record)]);
      // Change the reading harness: readers must not reinterpret old provenance.
      env.ZSTACK_ACTIVE_HOST = 'claude';
      const text = run([join(ROOT, 'bin/zstack-review-read')]);

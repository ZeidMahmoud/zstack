...
describeLive('Disabled outside plan review', () => {
  testSelected('outside-plan-disabled-no-fallback', async () => {
    const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-outside-disabled-'));
    const repo = path.join(fixtureRoot, 'repo');
    const rendered = path.join(fixtureRoot, 'render');
    fs.mkdirSync(repo);
    const runId = `${testName}-${process.env.ZSTACK_EVAL_RUN_ID ?? Date.now()}-${path.basename(fixtureRoot)}`;
    const evidenceDir = path.join(process.env.ZSTACK_EVAL_DIR || getProjectEvalDir(), 'disabled-plan-review', runId);

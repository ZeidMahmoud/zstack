...
describe('/spec template/generated sync', () => {
  test('regenerating spec/SKILL.md + sections produces byte-identical output', () => {
    const outDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-spec-sync-'));
    ...
    const res = spawnSync('bun', ['run', 'gen:skill-docs', '--out-dir', outDir], {
      ...
      // so an earlier test's env mutations (ZSTACK_*/GBRAIN_* detection vars)
    ...
    try {
      ...
      for (const trackedPath of GENERATED_PATHS) {
        ...
        // (~/.claude/skills/zstack/<skill>/sections/ → <outDir>/<skill>/sections/)
        ...
        const normalized = rendered.replaceAll(`${outDir}/`, '~/.claude/skills/zstack/');

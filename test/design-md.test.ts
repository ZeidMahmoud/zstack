/**
 * lib/design-md.ts + bin/zstack-design-md.ts + design/src/memory.ts (DESIGN.md writer).
 *
 * Pins the open DESIGN.md format rules zstack depends on: eight canonical
 ...
 * matter bytes, and a legacy → spec conversion of zstack's own DESIGN.md.
 */
...
import {
  ...
  emitYamlBlock, specSkeleton, spliceSection, insertMarker, DesignMdEditRefused, CANONICAL_SECTIONS, TOKEN_GROUPS, isLegacyZstackFormat,
} from '../lib/design-md';
import { updateDesignMd, readDesignConstraints } from '../design/src/memory';

const ROOT = path.join(import.meta.dir, '..');
const BIN = path.join(ROOT, 'bin', 'zstack-design-md.ts');
...
// zstack's own DESIGN.md is now in the open format; its pre-conversion form is the legacy fixture.
const LEGACY = fs.readFileSync(path.join(ROOT, 'test', 'fixtures', 'design-md-legacy.md'), 'utf-8');

const SPEC = `---
# zstack: design-md-format=spec
name: Heritage
colors:
  primary: "#1A1C1E"
  accent: "#B8422E"
...
describe('parse + detect', () => {
  ...
  test("the legacy fixture is legacy, zstack's own DESIGN.md is spec; a fresh file is unknown; nothing is missing", () => {
    const doc = parseDesignMd(LEGACY);
    expect(isLegacyZstackFormat(doc)).toBe(true);
  ...
  test('legacy marker on line 1 is read and survives a render', () => {
    const doc = parseDesignMd('<!-- zstack: design-md-format=legacy-keep -->\n# Design System — X\n\n## Product Context\n\n- a\n\n## Color\n\n- **Primary:** #fff\n');
    expect(doc.marker).toBe('legacy-keep');
    const out = renderDesignMd(doc);
    expect(out.split('\n')[0]).toBe('<!-- zstack: design-md-format=legacy-keep -->');
...
describe('render + upsert', () => {
  test('round trip is stable and keeps canonical order with extras after', () => {
    ...
    expect(once.split('\n')[0]).toBe('---');
    expect(once.split('\n')[1]).toBe('# zstack: design-md-format=spec');
  });
  ...
  test('insertMarker adds or replaces the marker only: line 2 YAML comment for front matter, line 1 HTML comment otherwise', () => {
    const noMarker = SPEC.replace('# zstack: design-md-format=spec\n', '');
    ...
    expect(kept).toBe('<!-- zstack: design-md-format=legacy-keep -->\n' + LEGACY);
  ...
  test('a marker on the parsed doc renders as the YAML comment on line 2 and nothing else moves', () => {
    ...
    expect(out.split('\n').slice(0, 3)).toEqual(['---', '# zstack: design-md-format=spec', 'name: Heritage']);
  });
});
...
describe('convertLegacy on the legacy fixture (zstack\'s pre-conversion DESIGN.md)', () => {
  ...
  test('produces a spec file with the marker on line 2 and only the five token groups plus name', () => {
    ...
    expect(doc.frontmatter!.name).toBe('zstack');
  });
  ...
  test('intro prose under the title survives conversion', () => {
    const withIntro = LEGACY.replace('# Design System — zstack\n', '# Design System — zstack\n\nAn intro paragraph that must not vanish.\n');
    ...
    expect(out2).toContain('# Design System — zstack');
  });
...
describe('coverage: bin verbs and the memory writer edges', () => {
  ...
  test('mark spec on an unmarked spec file; mark on a missing file; tokens on a missing file and with invalid refs; explicit path; no verb', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-design-md-'));
    try {
      fs.writeFileSync(path.join(dir, 'DESIGN.md'), SPEC.replace('# zstack: design-md-format=spec\n', ''));
...
describe('bin/zstack-design-md.ts', () => {
  const run = runBin;
  ...
  test('tokens prints the flat map; mark persists the choice', () => {
    ...
    try {
      ...
      expect(text).toBe('<!-- zstack: design-md-format=legacy-keep -->\n' + LEGACY); // byte-identical apart from line 1
...
describe('design binary: updateDesignMd is frontmatter-safe', () => {
  ...
  test('absent input: a spec skeleton with tokens from the extraction; readDesignConstraints leads with tokens', () => {
    ...
    try {
      ...
      expect(out.split('\n').slice(0, 2)).toEqual(['---', '# zstack: design-md-format=spec']);
...
describe('text-level editors keep line endings and respect fences', () => {
  ...
  test('insertMarker and spliceSection preserve CRLF line endings', () => {
    ...
    const legacyCrlf = '# T\r\n\r\n## Product Context\r\n\r\np\r\n';
    expect(insertMarker(legacyCrlf, 'legacy-keep')).toBe('<!-- zstack: design-md-format=legacy-keep -->\r\n' + legacyCrlf);
  ...
  test('a fenced ## inside a section does not end it; an unclosed fence runs to EOF for readers and refuses the edit', () => {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-design-md-fence-'));
...
describe('bin/zstack-design-md.ts follows a symlinked DESIGN.md', () => {
  test('mark edits the target file and leaves the link a link', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-design-md-link-'));
    ...
    try {
      ...
      expect(fs.readFileSync(path.join(dir, 'docs', 'DESIGN.md'), 'utf-8')).toBe('<!-- zstack: design-md-format=legacy-keep -->\n' + legacy);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
...
describe('adversarial round: markdown edge cases the editors must survive', () => {
  test('a legacy file that opens with a horizontal rule gets the HTML-comment marker, and the marker is read back', () => {
    ...
    expect(marked).toBe('<!-- zstack: design-md-format=legacy-keep -->\n' + legacy);
  ...
  test('convert refuses a legacy file whose consumed heading repeats, instead of dropping a body', () => {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-design-md-dup-'));
    try {
  ...
  test('the design binary tolerates unvalidated extraction output (null names, missing arrays)', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-design-md-null-'));
    try {

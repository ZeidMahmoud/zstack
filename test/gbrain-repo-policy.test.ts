/**
 * zstack-gbrain-repo-policy — per-remote trust-tier policy store.
 *
 ...
 *
 * Each test uses a temp ZSTACK_HOME so nothing leaks into the user's real ~/.zstack.
 */
...
// zstack-gbrain-sync invocation (skill prose bypassed — cron, curiosity,
...
describe('zstack-gbrain-sync code stage honors the repo policy (#2140 sync path)', () => {
  const SYNC = path.join(ROOT, 'bin', 'zstack-gbrain-sync.ts');
  ...
  function runSync(): { status: number; text: string; stages: any[] } {
    const res = spawnSync('bun', [SYNC, '--code-only', '--incremental'], {
      ...
      env: { ...process.env, ZSTACK_HOME: tmpHome, HOME: tmpHome },
    });
    let stages: any[] = [];
    try {
      stages = JSON.parse(

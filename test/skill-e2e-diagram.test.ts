 ...
 *
 * zstack renders local HTML through `bin/zstack-render.ts`: the Aside browser
 * when it is running, otherwise zstack's own browse daemon. The /diagram skill
 * runs ONE zstack-render call per triplet (staged bundle, three --eval/--out
 ...
 *   did it through zstack-render (a Bash tool call names it). No quality
...
// --- Whole-file gate: a browser zstack-render can drive. Skip, never fail. ---
...
const RENDER = path.join(ROOT, 'bin', 'zstack-render.ts');
...
function skillExtract(): string {
  ...
  if (!full.includes('zstack-render')) throw new Error('diagram/SKILL.md does not render through zstack-render (stale generated doc) — run `bun run gen:skill-docs`');
...
function setupDir(prefix: string): string {
  ...
  // zstack-render serves this directory on loopback for the render.
...
- zstack-render picks the browser itself (its first output line is ENGINE=aside or ENGINE=browse); either is fine. Do not probe for or start a browser yourself.
- The render bundle is ALREADY staged at ${dir}/diagram-render.html — use that path wherever the skill says <staged>.
- zstack-render lives at ${RENDER}; run it as \`bun run ${RENDER}\` instead of the ~/.claude/skills/zstack/bin/zstack-render.ts path the skill shows.
...
/** The skill's render contract: at least one Bash tool call ran zstack-render. */
function expectRenderedViaZstackRender(result: SkillTestResult): void {
  const ran = result.toolCalls.some((c) => c.tool === 'Bash' && JSON.stringify(c.input ?? {}).includes('zstack-render'));
...
describeDiagram('/diagram skill E2E', ['diagram-triplet', 'diagram-authoring-quality'], () => {
  testConcurrentIfSelected('diagram-triplet', async () => {
    ...
    try {
      ...
      expectRenderedViaZstackRender(result);
  ...
  testConcurrentIfSelected('diagram-authoring-quality', async () => {
    ...
    try {
      const result = await runSkillTest({
        prompt: basePrompt(
          dir,
          'how zstack renders diagrams in PDFs: markdown containing mermaid fences goes through a pre-pass that extracts the fences, renders them in a browser (Aside, or zstack\'s own headless fallback) using an offline bundle, substitutes the SVG back in, inlines local images, and prints the PDF through the same browser. Failures become visible diagnostic blocks.',
        ),
        workingDirectory: dir,
        maxTurns: 25,
        allowedTools: ['Bash', 'Read', 'Write'],
...
THE ASK: a diagram of zstack's PDF diagram-rendering flow — mermaid fences are
extracted by a pre-pass, rendered in a browser (Aside, or zstack's own headless

...
describe('Design completed handoff without an offered manual action', () => {
  ...
  test('an explicit absence of outstanding work remains a closed recap', () => {
    for (const recap of ['No unresolved design decisions.', 'Zero remaining contrast gaps.', 'No gap remains.']) {
      const call = handoff();
      const q = call.questions[0]!;
      q.question = `Design review complete. ${recap} What’s next? <zstack-qid:plan-design-next-steps>`;
  ...
  test('remaining work, mixed actions and unknown identities stay substantive', () => {
    const mutations: Array<(c: NativePlanQuestionCall) => void> = [
      ...
      c => { c.questions[0]!.question += ' <zstack-qid:plan-design-next-steps>'; },
      c => { c.questions[0]!.question = 'Design review complete. One contrast gap remains unresolved. What’s next? <zstack-qid:plan-design-next-steps>'; },
      c => { c.questions[0]!.question = 'Design review complete. One contrast gap remains. What’s next? <zstack-qid:plan-design-next-steps>'; },
      c => { c.questions[0]!.question = 'Design review complete. There is an unresolved contrast gap. What’s next? <zstack-qid:plan-design-next-steps>'; },
      c => { c.questions[0]!.question = c.questions[0]!.question.replace('What', '<zstack-qid malformed What'); },

 ...
 * auto-deciding a specific question via `zstack-question-preference --write
 ...
 *
 * Set up:
 *   - tmpDir as ZSTACK_HOME (isolated state, doesn't touch the user's
 *     real ~/.zstack)
...
describeE2E('AUTO_DECIDE opt-in preserved under Conductor flags (periodic)', () => {
  test('user-opted-in question still auto-decides when AskUserQuestion is --disallowedTools', async () => {
    const tmpHome = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-auto-decide-')));
    let fixture: ReturnType<typeof createPlanCountFixture> | undefined;
    try {
      fixture = createPlanCountFixture(PLAN);
      seedHermeticZstackHome(tmpHome);
      // 1. Bootstrap the tmp ZSTACK_HOME with question_tuning=true.
      const configBin = path.join(ROOT, 'bin', 'zstack-config');
      ...
      const privacyRes = spawnSync(configBin, ['set', 'cross_project_learnings', 'false'], {
        cwd: fixture.cwd, env: { ...process.env, ZSTACK_HOME: tmpHome }, encoding: 'utf-8', timeout: 30_000,
      });
      if (privacyRes.status !== 0) throw new Error(`zstack-config privacy baseline failed: ${privacyRes.stderr || privacyRes.stdout}`);
      ...
      if (writeRes.status !== 0) {
        throw new Error(`zstack-question-preference --write failed: ${writeRes.stderr || writeRes.stdout}`);
      }
      ...
      const obs = await runPlanSkillObservation({
        ...
        env: { ZSTACK_HOME: tmpHome, ZSTACK_STATE_ROOT: tmpHome, CONDUCTOR_WORKSPACE_PATH: fixture.cwd,

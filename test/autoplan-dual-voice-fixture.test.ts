...
mock.module(${JSON.stringify(path.join(ROOT, 'test/helpers/session-runner.ts'))}, () => ({
  runSkillTest: async opts => {
    ...
    const state = opts.env?.ZSTACK_HOME;
    ...
    if (scenario === 'runtime' || scenario === 'runtime-missing') {
      const runtime = path.join(opts.env.HOME ?? '', '.claude', 'skills', 'zstack');
      const tool = path.join(runtime, 'bin/zstack-autoplan-snapshot.ts');
      ...
      const env = hermeticChildEnv({ZSTACK_HEADLESS: '1', ...opts.env});
      ...
      const preamble = run('bash', [path.join(runtime, 'bin/zstack-skill-start'), '--skill', 'autoplan', '--model', 'none', '--parent-pid', String(process.pid)]);
      const paths = run('bash', [path.join(runtime, 'bin/zstack-paths')]);
      ...
      const assets = ['bin/zstack-autoplan-snapshot.ts', 'bin/zstack-skill-start', 'bin/zstack-paths',
        'bin/zstack-config', 'bin/zstack-review-log', 'bin/zstack-codex-probe', 'lib/fs-atomic.ts',
      ...
      fact.runtime = {...fact.runtime, calls, preambleReady: preamble.includes('SKILL_START_PROTO: 1'),
        stateBound: paths.includes('ZSTACK_STATE_ROOT=' + state), scopeBound: scope.activePlan === fs.realpathSync(active),
    ...
    const {prepareMethodology, createSnapshot} = await import(${JSON.stringify(path.join(ROOT, 'bin/zstack-autoplan-snapshot.ts'))});
    ...
    if (scenario === 'temp' || scenario === 'temp-retry') {
      ...
      const cleanupFiles = [];
      try {
        const command = 'umask 077; mktemp "$' + '{TMPDIR:-/tmp}/zstack-plan-prompt.XXXXXXXX"';
...
`);
    try {
      ...
      for (const attempt of attempts) {
        ...
        expect(attempt.env?.ZSTACK_STATE_ROOT).toBe(attempt.env?.ZSTACK_HOME);
        expect(attempt.stateHadPriorArtifact).toBe(false);
        expect(attempt.configHadPriorSession).toBe(false);
        expect(fs.existsSync(attempt.env.ZSTACK_HOME)).toBe(false);
      ...
      if (['retry', 'temp-retry'].includes(scenario)) {
        ...
        expect(attempts[0].env.ZSTACK_HOME).not.toBe(attempts[1].env.ZSTACK_HOME);

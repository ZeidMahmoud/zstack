/**
 * _zstack_codex_model_probe — round-trip model readiness (#2477).
 *
 ...
 * round trip with zstack's selected model.
 *
...
function makeFixture(): Fixture {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-model-probe-'));
  const stubDir = path.join(home, 'stub-bin');
  const codexHome = path.join(home, '.codex');
  const zstackHome = path.join(home, '.zstack');
...
describe('codex model probe (#2477)', () => {
  test('successful round trip -> MODEL_OK, cached, no re-invocation', () => {
    ...
    try {
      ...
      expect(fs.existsSync(path.join(f.zstackHome, '.codex-model-probe'))).toBe(true);
  ...
  test('model 400 -> MODEL_UNUSABLE with selected-model hints, exit 1, negative-cached', () => {
    ...
    try {
      ...
      expect(r.stdout).toContain('zstack requested model');
      expect(r.stdout).toContain('ZSTACK_CODEX_MODEL');
      ...
      expect(second.stdout).toContain('ZSTACK_CODEX_MODEL');
  ...
  test('ZSTACK_CODEX_MODEL change re-probes past a cached MODEL_UNUSABLE (the recovery path)', () => {
    const f = makeFixture();
    try {
      runProbe(f, 'model400');
      expect(invocations(f)).toBe(1);
      // Fixing the zstack model override changes the cache signature — the
      // negative cache must not outlive the model it condemned.
      const r = runProbe(f, 'ok', { ZSTACK_CODEX_MODEL: 'gpt-5.6-sol' });
  ...
  test('TTL expiry: a cached MODEL_OK older than 3600s re-probes (T5)', () => {
    ...
    try {
      ...
      const cachePath = path.join(f.zstackHome, '.codex-model-probe');

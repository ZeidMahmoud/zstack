 ...
 *
 * The catalog is the single source of truth for zstack's design anti-pattern
...
describe('catalog shape', () => {
  test('ids are unique kebab-case and every field is in its domain', () => {
    ...
    for (const e of DESIGN_SLOP_CATALOG) {
      ...
      expect(['zstack', 'impeccable', 'both']).toContain(e.source);
    }
  });
  ...
  test('zstack-only entries never claim engine detection or an impeccable source', () => {
    for (const e of DESIGN_SLOP_CATALOG.filter(x => !x.impeccableId)) {
      expect(e.detect).not.toContain('engine');
      expect(e.source).toBe('zstack');

 ...
 *
 * 2. zstack-team-init's required-mode enforcement (the CLAUDE.md
 *    verification snippet and the generated check-zstack.sh hook) must
 *    resolve the install root across ZSTACK_ROOT + every host's global
 *    install location, never hardcode ~/.claude/skills/zstack. The drift
...
// into bin/zstack-skill-start (invoked by every host's preamble fence). The
...
describe('routing probe checks AGENTS.md too (#2500)', () => {
  for (const host of ['claude', 'codex'] as const) {
    test(`preamble reaches the CLAUDE.md AND AGENTS.md probe (${host})`, () => {
      ...
      expect(rendered).toContain('zstack-skill-start');
...
describe('team-init resolves ZSTACK_ROOT across every host (#2500)', () => {
  const teamInit = fs.readFileSync(path.join(ROOT, 'bin', 'zstack-team-init'), 'utf-8');

  test('probe list covers ZSTACK_ROOT env + every registered host globalRoot + migrated repo', () => {
    expect(teamInit).toContain('"${ZSTACK_ROOT:-}"');
    for (const config of ALL_HOST_CONFIGS) {
      expect(teamInit).toContain(`$HOME/${config.globalRoot}`);
    }
    expect(teamInit).toContain('$HOME/.zstack/repos/zstack');
  });

  test('enforcement no longer hardcodes the Claude path as the only gate', () => {
    expect(teamInit).not.toContain('test -d ~/.claude/skills/zstack/bin');
    expect(teamInit).not.toContain('if [ ! -d "$HOME/.claude/skills/zstack/bin" ]');
  });
  ...
  test('generated hook blocks only when NO install root resolves', () => {
    ...
    expect(teamInit).toContain('if [ -z "$_ZSTACK_ROOT" ]; then');
  });
});

...
const CODEX_FORMAT_TOUCHFILES: Record<string, string[]> = {
  'codex-plan-ceo-format-mode':      ['.agents/skills/zstack-plan-ceo-review/**', 'scripts/resolvers/preamble/generate-ask-user-format.ts', 'scripts/resolvers/preamble/generate-completeness-section.ts', 'model-overlays/gpt.md', 'model-overlays/gpt-5.4.md'],
  'codex-plan-ceo-format-approach':  ['.agents/skills/zstack-plan-ceo-review/**', 'scripts/resolvers/preamble/generate-ask-user-format.ts', 'scripts/resolvers/preamble/generate-completeness-section.ts', 'model-overlays/gpt.md', 'model-overlays/gpt-5.4.md'],
  'codex-plan-eng-format-coverage':  ['.agents/skills/zstack-plan-eng-review/**', 'scripts/resolvers/preamble/generate-ask-user-format.ts', 'scripts/resolvers/preamble/generate-completeness-section.ts', 'model-overlays/gpt.md', 'model-overlays/gpt-5.4.md'],
  'codex-plan-eng-format-kind':      ['.agents/skills/zstack-plan-eng-review/**', 'scripts/resolvers/preamble/generate-ask-user-format.ts', 'scripts/resolvers/preamble/generate-completeness-section.ts', 'model-overlays/gpt.md', 'model-overlays/gpt-5.4.md'],
};
...
function setupCodexSkillDir(tmpPrefix: string, skillName: 'plan-ceo-review' | 'plan-eng-review'): { skillDir: string; planDir: string; outFile: string } {
  ...
  // Codex skill lives in .agents/skills/zstack-{name}/ per the zstack host convention.
  const codexSkillSource = path.join(ROOT, '.agents', 'skills', `zstack-${skillName}`);
  const skillDir = path.join(planDir, '.agents', 'skills', `zstack-${skillName}`);
...
describeCodex('Codex Plan Format — CEO Mode Selection', () => {
  ...
  testIfSelected('codex-plan-ceo-format-mode', async () => {
    const result = await runCodexSkill({
      ...
      timeoutMs: 300_000,
      cwd: planDir,
      skillName: 'zstack-plan-ceo-review',
...
describeCodex('Codex Plan Format — Eng Coverage Issue', () => {
  ...
  testIfSelected('codex-plan-eng-format-coverage', async () => {
    const result = await runCodexSkill({
      ...
      timeoutMs: 300_000,
      cwd: planDir,
      skillName: 'zstack-plan-eng-review',
      sandbox: 'workspace-write',
    });

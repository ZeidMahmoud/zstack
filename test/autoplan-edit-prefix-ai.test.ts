...
function replay() {
  ...
  const cwd = path.join(root, path.basename(captured.cwd)), ownedStateRoot = path.join(root, 'home', '.zstack');
...
test('competing headers, previous panels, misleading prose and quotes remain rejected', () => {
  for (const prefix of [
    ...
    '● Update(~/.zstack/projects/zstack-autoplan-chain-9599im/ceo-plans/2026-09-10-user-dashboard.md)\n  ⎿ Added 1 line\n\n',
    ' Edit file\n /tmp/foreign.md\n────────\n',
    'Example:\n', '> quoted output\n', '```diff\n      1 +quoted\n```\n',
...
test('the complete current header, exact target, menu and requested replacement remain binding', () => {
  for (const change of [
    (r: ReturnType<typeof replay>) => { r.viewport = r.viewport.replace('● Update(~/.zstack/', '● Update(/foreign/'); },

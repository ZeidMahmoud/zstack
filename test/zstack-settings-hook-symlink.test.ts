...
const hook = path.resolve(import.meta.dir, '../bin/zstack-settings-hook');
let root: string;
let target: string;
let settings: string;

beforeEach(() => {
  root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-link-')));
...
function env(file = settings) {
  return { ...process.env, HOME: root, ZSTACK_HOME: path.join(root, 'state'), ZSTACK_SETTINGS_FILE: file };
}
...
fs.realpathSync = (file, ...args) => {
  if (String(file) === process.env.ZSTACK_SETTINGS_INPUT && (!${afterTemp} || (process.env.ZSTACK_TMP_PATH && fs.existsSync(process.env.ZSTACK_TMP_PATH)))) {
...
describe('settings hook preserves the resolved settings target', () => {
  test('add, ensure and remove preserve the link, real target and private mode', () => {
    ...
    expect(JSON.parse(fs.readFileSync(target, 'utf8')).hooks.PostToolUse[0]._zstack_source).toBe('link-test');
  ...
  test('different aliases contend for the same target lock', () => {
    ...
    const result = spawnSync('bash', [hook, ...addArgs()], {
      env: { ...env(), ZSTACK_SETTINGS_LOCK_TIMEOUT_MS: '100' },
      encoding: 'utf8', timeout: 10_000,
    });
  ...
  test('legacy add, remove and prune-stale use the real target', () => {
    const command = path.join(root, 'missing/bin/zstack-session-update');
  ...
  test('concurrent writes through file and directory aliases both survive', async () => {
    ...
    try {
      ...
      expect(stored.hooks.PostToolUse[0]._zstack_source).toBe('link-test');
      expect(stored.hooks.Stop[0]._zstack_source).toBe('second-source');
      expect(fs.lstatSync(settings).isSymbolicLink()).toBe(true);

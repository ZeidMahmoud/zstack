 ...
 * PR after main merged ZSTACK_PLAN_MODE + /spec, pushing the v1.44.1
 ...
 * ({{ASIDE_SETUP}}) plus the zstack-browser fallback block now ride in every
 ...
 * moved ~11-13KB of inline preamble bash per skill into bin/zstack-skill-start
 * and bin/zstack-skill-end — a deliberate corpus-wide shrink; receipt:
 * zstack-context-bill --diff in PR #2691). The prior v1.47.0.0 fixture stays
 ...
 *
 * Override:
 * - ZSTACK_SIZE_BUDGET_RATIO=<n> changes the per-skill regression ratio.
 ...
 *   doesn't offset).
 * - ZSTACK_SIZE_BUDGET_OVERRIDE_REASON="text" allows a regression to
 *   pass and logs the reason to ~/.zstack/analytics/spend-overrides.jsonl
...
const RATIO = Number(process.env.ZSTACK_SIZE_BUDGET_RATIO) || DEFAULT_RATIO;
...
describe('SKILL.md size budget regression (gate, free)', () => {
  ...
  test('no skill exceeds v1.81.0.0 baseline size × ratio', () => {
    ...
    const overrideReason = process.env.ZSTACK_SIZE_BUDGET_OVERRIDE_REASON?.trim();
    if (overrideReason) {
      logBudgetOverride({
    ...
    throw new Error(
      ...
      `Override: set ZSTACK_SIZE_BUDGET_OVERRIDE_REASON="why this is OK" to allow and audit-log.`,
    );
  });
  ...
  test('total corpus byte count does not regress past baseline × ratio', () => {
    ...
    throw new Error(
      ...
      `Override: set ZSTACK_SIZE_BUDGET_OVERRIDE_REASON to allow.`,
    );
  });
  ...
  test('no skill shrinks past 80% of v1.69.1.0 baseline (catches accidental body strip)', () => {
    ...
    // - scrape/diagram/open-zstack-browser/landing-report/pair-agent/skillify:
    ...
    const INTENTIONAL_SHRINKS = new Set<string>([
      'spec',
      'scrape', 'diagram', 'open-zstack-browser',
    ...
    throw new Error(
      ...
      `ZSTACK_SIZE_BUDGET_OVERRIDE_REASON="why" allows + audit-logs.`,
    );
  });
  ...
  test('catalog token estimate stays compressed (v1.45 target ≤ 7000)', () => {
    ...
    throw new Error(
      ...
      `T4 catalog trim should keep this under control. Override: set ZSTACK_SIZE_BUDGET_OVERRIDE_REASON to allow.`,
    );
  });
});

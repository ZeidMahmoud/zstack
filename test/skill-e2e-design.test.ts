...
Write DESIGN.md and CLAUDE.md (or update it) in the working directory.`,
  ...
  testConcurrentIfSelected('design-consultation-research', async () => {
    ...
    // this case pins. Without Aside (CI, or ZSTACK_SKIP_ASIDE=1) it must use
    ...
    // SKILL.md unless ZSTACK_E2E_DOCS_ROOT points at a `gen:skill-docs --out-dir` render.
    const skill = fs.readFileSync(path.join(process.env.ZSTACK_E2E_DOCS_ROOT || ROOT, 'design-consultation', 'SKILL.md'), 'utf-8');
...
Do NOT generate a full DESIGN.md — just research notes.`,
  ...
  testConcurrentIfSelected('design-consultation-existing', async () => {
    // Pre-create a LEGACY-format DESIGN.md (zstack's pre-spec shape, no marker) so
...
Run Phase 0's DESIGN.md format check exactly as written (the zstack bin directory is ${ROOT}/bin). Skip research. Skip font preview. Skip any AskUserQuestion calls — this is non-interactive: where the skill asks whether to convert the legacy file, take option A (convert) without asking.`,
      workingDirectory: designDir,
      maxTurns: 20,
      timeout: CAPTURE_LONG_MS,
    ...
    const ranCheck = bash.some(c => c.includes('zstack-design-md.ts check'));
    const marked = /^---\n# zstack: design-md-format=spec/.test(designContent) || designContent.startsWith('<!-- zstack: design-md-format=legacy-keep -->');
...
Review the site at ${serverUrl}. Use --quick mode. Skip any AskUserQuestion calls — this is non-interactive. Fix up to 3 issues max. Write your report to ./design-audit.md.`,
    ...
    const sentinelPrinted = /ZSTACK_STEP_OK/.test(toolOutput(result));
...
function detectorSkillText(sections: Array<[string, string]>): string {
  ...
  return sections.map(([a, b]) => sliceBetween(full, a, b)).join('\n\n---\n\n')
    ...
    .replaceAll('~/.claude/skills/zstack', ROOT);
}
...
Then write ${repoDir}/detector-output.md: one FINDING-NNN row per rule in the DETECT_TOP block, each tagged with its [rule-id] and the printed impact, plus the first line the probe printed.`,
    ...
    const bash = result.toolCalls.filter(c => c.tool === 'Bash').map(c => String(c.input?.command ?? ''));
    expect(bash.some(c => c.includes('zstack-design-detect.ts probe'))).toBe(true);
    expect(bash.some(c => /zstack-design-detect\.ts scan --changed main/.test(c))).toBe(true);
  ...
  // DOM mode needs a browser engine for the dump: zstack's own browse binary
  ...
  testConcurrentIfSelected(
    ...
    async () => {
      ...
      const zstackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-e2e-detector-home-'));
      // REPORT_DIR must sit under <zstack home>/projects/<slug>/designs/ for the wrapper's allow-list.
      const allowed = path.join(zstackHome, 'projects', 'shim', 'designs', 'design-audit-20260908');
      ...
      const browseState = path.join(zstackHome, 'browse.json');
      try {
        const result = await runSkillTest({
...
Do not run npx. Do not fix anything.`,
          ...
          runId,
          env: { IMPECCABLE_BIN: path.join(engineDir, 'impeccable'), IMPECCABLE_FAKE_OUTPUT: DETECT_SAMPLE, ZSTACK_HOME: zstackHome, BROWSE_STATE_FILE: browseState },
        });
        ...
        expect(bash.some(c => /zstack-design-detect\.ts scan /.test(c) && c.includes('dom/run1'))).toBe(true);
        expect(bash.some(c => /zstack-design-detect\.ts scan --changed/.test(c))).toBe(false);
        const dumps = fs.readdirSync(path.join(allowed, 'dom', 'run1')).filter(f => f.endsWith('.dom.html'));
        expect(dumps.length).toBeGreaterThan(0);
        expect(fs.readFileSync(path.join(allowed, 'dom', 'run1', dumps[0]), 'utf-8')).toContain('data-zstack-dom-css');
        ...
        for (const d of [site, reportDir, zstackHome]) { try { fs.rmSync(d, { recursive: true, force: true }); } catch {} }
      }
    },
    CAPTURE_LONG_MS,
  );
...
describeIfSelected('Design HTML slop gate E2E', ['design-html-slop-gate'], () => {
  ...
  beforeAll(() => {
    ...
    ].join('\n\n---\n\n').replaceAll('$HOME/.claude/skills/zstack', ROOT).replaceAll('~/.claude/skills/zstack', ROOT);
...
Write ${workDir}/gate-output.md listing what you fixed and every remaining finding as accepted-with-reason, each tagged with its [rule-id]. Do not take screenshots, do not run npx, do not scan more than twice.`,
    ...
    expect(result.exitReason).toBe('success');
    const scans = result.toolCalls.filter(c => c.tool === 'Bash' && /zstack-design-detect\.ts scan /.test(String(c.input?.command ?? '')));

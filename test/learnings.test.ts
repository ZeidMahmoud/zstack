...
function runLog(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  ...
  try {
    const stdout = execSync(`${BIN}/zstack-learnings-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-learn-'));
...
describe('zstack-learnings-search', () => {
...
describe('zstack-learnings-log edge cases', () => {
...
describe('zstack-learnings-search edge cases', () => {

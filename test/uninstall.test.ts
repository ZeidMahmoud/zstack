...
describe('zstack-uninstall', () => {
  ...
  test('--help prints usage and exits 0', () => {
    ...
    expect(output).toContain('zstack-uninstall');
  ...
  describe('integration tests with mock layout', () => {
    ...
    beforeEach(() => {
      tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-uninstall-test-'));
      mockHome = path.join(tmpDir, 'home');
      mockGitRoot = path.join(tmpDir, 'repo');

      // Create mock zstack install layout
      fs.mkdirSync(path.join(mockHome, '.claude', 'skills', 'zstack'), { recursive: true });
      fs.writeFileSync(path.join(mockHome, '.claude', 'skills', 'zstack', 'SKILL.md'), 'test');

      // Create per-skill symlinks (both old unprefixed and new prefixed)
      fs.symlinkSync('zstack/review', path.join(mockHome, '.claude', 'skills', 'review'));
      fs.symlinkSync('zstack/ship', path.join(mockHome, '.claude', 'skills', 'zstack-ship'));

      // Create a non-zstack symlink (should NOT be removed)
      fs.mkdirSync(path.join(mockHome, '.claude', 'skills', 'other-tool'), { recursive: true });

      // Create state directory
      fs.mkdirSync(path.join(mockHome, '.zstack', 'projects'), { recursive: true });
      fs.writeFileSync(path.join(mockHome, '.zstack', 'config.json'), '{}');
    ...
    test('--force removes global Claude skills and state', () => {
      const result = spawnSync('bash', [UNINSTALL, '--force'], {
        ...
        env: {
          ...process.env,
          HOME: mockHome,
          ZSTACK_DIR: path.join(mockHome, '.claude', 'skills', 'zstack'),
      ...
      expect(result.status).toBe(0);
      const output = result.stdout.toString();
      expect(output).toContain('zstack uninstalled');

      // Global skill dir should be removed
      expect(fs.existsSync(path.join(mockHome, '.claude', 'skills', 'zstack'))).toBe(false);

      // Per-skill symlinks pointing into zstack/ should be removed
      expect(fs.existsSync(path.join(mockHome, '.claude', 'skills', 'review'))).toBe(false);
      expect(fs.existsSync(path.join(mockHome, '.claude', 'skills', 'zstack-ship'))).toBe(false);

      // Non-zstack tool should still exist
      expect(fs.existsSync(path.join(mockHome, '.claude', 'skills', 'other-tool'))).toBe(true);

      // State should be removed
      expect(fs.existsSync(path.join(mockHome, '.zstack'))).toBe(false);
    });
    ...
    test('--keep-state preserves state directory', () => {
      ...
      expect(fs.existsSync(path.join(mockHome, '.zstack'))).toBe(true);
      expect(fs.existsSync(path.join(mockHome, '.zstack', 'config.json'))).toBe(true);
    });
    ...
    test('clean system outputs nothing to remove', () => {
      ...
      const result = spawnSync('bash', [UNINSTALL, '--force'], {
        ...
        env: {
          ...process.env,
          HOME: cleanHome,
          ZSTACK_DIR: path.join(cleanHome, 'nonexistent'),
          ZSTACK_STATE_DIR: path.join(cleanHome, '.zstack'),
        },
        timeout: 30_000,
        cwd: mockGitRoot,
      });
    ...
    test('upgrade path: prefixed install + uninstall cleans both old and new symlinks', () => {
      ...
      // (mockHome already has both 'review' and 'zstack-ship' symlinks)
      ...
      // Both old (review) and new (zstack-ship) symlinks should be gone
      expect(fs.existsSync(path.join(mockHome, '.claude', 'skills', 'review'))).toBe(false);
      expect(fs.existsSync(path.join(mockHome, '.claude', 'skills', 'zstack-ship'))).toBe(false);

      // Non-zstack should survive
      expect(fs.existsSync(path.join(mockHome, '.claude', 'skills', 'other-tool'))).toBe(true);
    });

    test('--force removes Cursor zstack skills and leaves other Cursor skills', () => {
      ...
      fs.mkdirSync(path.join(mockHome, '.cursor', 'skills', 'zstack'), { recursive: true });
      fs.writeFileSync(path.join(mockHome, '.cursor', 'skills', 'zstack', 'SKILL.md'), banner);
      fs.mkdirSync(path.join(mockHome, '.cursor', 'skills', 'zstack-review'), { recursive: true });
      fs.writeFileSync(path.join(mockHome, '.cursor', 'skills', 'zstack-review', 'SKILL.md'), banner);
      ...
      fs.mkdirSync(path.join(mockGitRoot, '.cursor', 'skills', 'zstack-ship'), { recursive: true });
      fs.writeFileSync(path.join(mockGitRoot, '.cursor', 'skills', 'zstack-ship', 'SKILL.md'), banner);
      ...
      expect(fs.existsSync(path.join(mockHome, '.cursor', 'skills', 'zstack'))).toBe(false);
      expect(fs.existsSync(path.join(mockHome, '.cursor', 'skills', 'zstack-review'))).toBe(false);
      expect(fs.existsSync(path.join(mockHome, '.cursor', 'skills', 'frontend-design'))).toBe(true);
      expect(fs.existsSync(path.join(mockGitRoot, '.cursor', 'skills', 'zstack-ship'))).toBe(false);
      expect(fs.existsSync(path.join(mockGitRoot, '.cursor', 'rules', 'keep.md'))).toBe(true);
    });

    test("a user's own zstack-prefixed Cursor dir (no banner) survives and is listed", () => {
      // S5: the bare zstack* glob must not sweep a dir that merely starts
      // with "zstack" — provenance comes from the generated banner, and a
      // hand-written SKILL.md never carries it.
      const foreign = path.join(mockHome, '.cursor', 'skills', 'zstack-fork-notes');
      fs.mkdirSync(foreign, { recursive: true });
      fs.writeFileSync(path.join(foreign, 'SKILL.md'), '# my own notes\n');

      const foreignLocal = path.join(mockGitRoot, '.cursor', 'skills', 'zstack-my-rules');
      ...
      expect(stderr).toContain('zstack-fork-notes');
      expect(stderr).toContain('zstack-my-rules');
    });
  });
});
...
// cleanup ran AFTER `rm -rf $CLAUDE_SKILLS/zstack` — and SETTINGS_HOOK
...
describe('hook cleanup runs before the install root is deleted', () => {
  test('uninstall executed FROM the install root still removes hook entries', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-uninstall-order-'));
    try {
      const mockHome = path.join(tmp, 'home');
      const installRoot = path.join(mockHome, '.claude', 'skills', 'zstack');
      ...
      // the root it deletes.
      for (const b of ['zstack-uninstall', 'zstack-settings-hook', 'zstack-session-update', 'zstack-config']) {
      ...
      fs.writeFileSync(settingsFile, JSON.stringify({
        hooks: {
          PostToolUse: [{
            ...
            _zstack_source: 'auq-error-fallback',
      ...
      const result = spawnSync('bash', [path.join(installBin, 'zstack-uninstall'), '--force', '--keep-state'], {
        stdio: 'pipe',
        timeout: 30_000,
        env: {
          ...process.env,
          ...
          ZSTACK_STATE_ROOT: path.join(mockHome, '.zstack'),
        },
        cwd: tmp,
      });
      expect(result.status).toBe(0);
...
describe('the Memorable bridge hook is removed by name and the kept config is left honest', () => {
  test('a tag-stripped memorable entry is removed, reported, and memorable_recall is set off under --keep-state', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-uninstall-memo-'));
    try {
      const mockHome = path.join(tmp, 'home');
      const installRoot = path.join(mockHome, '.claude', 'skills', 'zstack');
      const installBin = path.join(installRoot, 'bin');
      ...
      const stateRoot = path.join(mockHome, '.zstack');
      fs.mkdirSync(stateRoot, { recursive: true });
      const env = { ...process.env, HOME: mockHome, ZSTACK_SETTINGS_FILE: settingsFile, ZSTACK_STATE_ROOT: stateRoot };
      spawnSync('bash', [path.join(installBin, 'zstack-config'), 'set', 'memorable_recall', 'on'], { env, timeout: 20_000 });
      ...
      const s = JSON.parse(fs.readFileSync(settingsFile, 'utf-8'));
      // zstack's entry gone, the vendor's own entry untouched
...
describe('the Memorable arm stays quiet when nothing of its is registered', () => {
  test('no memorable entry -> no "Memorable UserPromptSubmit hook" in the summary, exit 0', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-uninstall-memo-none-'));
    try {
      const mockHome = path.join(tmp, 'home');
      const installRoot = path.join(mockHome, '.claude', 'skills', 'zstack');
      const installBin = path.join(installRoot, 'bin');
      ...
      const result = spawnSync('bash', [path.join(installBin, 'zstack-uninstall'), '--force', '--keep-state'], {
        ...
        env: { ...process.env, HOME: mockHome, ZSTACK_SETTINGS_FILE: settingsFile, ZSTACK_STATE_ROOT: path.join(mockHome, '.zstack') },
      });
      expect(result.status).toBe(0);
      ...
      expect(fs.existsSync(path.join(mockHome, '.zstack', 'config.yaml'))).toBe(false);
    } finally {
      fs.rmSync(tmp, { recursive: true, force: true });
    }
  }, 30000);
...
describe('hook cleanup under lock contention is loud, never silent (review-army)', () => {
  test('a held foreign lock during uninstall surfaces the give-up warning on stderr', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-uninstall-lock-'));
    try {
      const mockHome = path.join(tmp, 'home');
      const installRoot = path.join(mockHome, '.claude', 'skills', 'zstack');
      const installBin = path.join(installRoot, 'bin');
      ...
      fs.writeFileSync(settingsFile, JSON.stringify({
        hooks: {
          Stop: [{
            _zstack_source: 'zstack-timeline-stop',
...
describe('the consent key never outlives the hook, even when the config lives outside the removed state dir', () => {
  test('full uninstall (no --keep-state) with ZSTACK_STATE_ROOT elsewhere: memorable_recall flips off there', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-uninstall-memo-root-'));
    try {
      const mockHome = path.join(tmp, 'home');
      ...
      fs.mkdirSync(path.join(mockHome, '.zstack'), { recursive: true });
      const env = { ...process.env, HOME: mockHome, ZSTACK_SETTINGS_FILE: settingsFile, ZSTACK_STATE_ROOT: otherRoot };
      expect(spawnSync('bash', [path.join(installBin, 'zstack-config'), 'set', 'memorable_recall', 'on'], { env, timeout: 20_000 }).status).toBe(0);
      const result = spawnSync('bash', [path.join(installBin, 'zstack-uninstall'), '--force'], {
        stdio: 'pipe', timeout: 30_000, encoding: 'utf-8', cwd: tmp, env,
      });
      expect(result.status).toBe(0);
      expect(fs.existsSync(path.join(mockHome, '.zstack'))).toBe(false); // the default state dir went
...
describe('the consent flip does not depend on the hook manager being present', () => {
  test('zstack-settings-hook missing from the install: memorable_recall still goes off', () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-uninstall-memo-nohook-'));
    try {
      const mockHome = path.join(tmp, 'home');
      const installRoot = path.join(mockHome, '.claude', 'skills', 'zstack');
      const installBin = path.join(installRoot, 'bin');
      fs.mkdirSync(installBin, { recursive: true });
      for (const b of ['zstack-uninstall', 'zstack-config']) { // no settings hook, no session-update
        const dst = path.join(installBin, b);
        fs.copyFileSync(path.join(ROOT, 'bin', b), dst);
        fs.chmodSync(dst, 0o755);
      }
      const stateRoot = path.join(mockHome, '.zstack');
      fs.mkdirSync(stateRoot, { recursive: true });
      const env = { ...process.env, HOME: mockHome, ZSTACK_STATE_ROOT: stateRoot };
      expect(spawnSync('bash', [path.join(installBin, 'zstack-config'), 'set', 'memorable_recall', 'on'], { env, timeout: 20_000 }).status).toBe(0);
      const result = spawnSync('bash', [path.join(installBin, 'zstack-uninstall'), '--force', '--keep-state'], {
        stdio: 'pipe', timeout: 30_000, encoding: 'utf-8', cwd: tmp, env,
      });

/**
 * bin/zstack-render.ts — the CLI skills shell out to for rendering local HTML
 * (make-pdf's print pipeline, the diagram bundle, design previews).
 *
 * Everything here is hermetic: ZSTACK_SKIP_ASIDE=1 forces the browse fallback,
 * and ZSTACK_BROWSE_BIN points at a fake `browse` shell script that answers the
...
const CLI = path.join(ROOT, 'bin/zstack-render.ts');
...
const USAGE_PREFIX = 'usage: zstack-render';
...
case "$cmd" in
  ...
  js)
    expr="$2"
    case "$expr" in
      'JSON.stringify(window.__zstackErrs || [])') echo "\${FAKE_PAGE_ERRS:-[]}" ;;
      '(() => { try { return !!('*) echo true ;;
      '(() => { window.__zstackErrs'*) echo undefined ;;
      *)
        shift 2
        out=""
        while [ $# -gt 0 ]; do
...
function makeFixture(): Fixture {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-render-cli-'));
...
describe('zstack-render CLI: argument guards', () => {
  let f: Fixture;
...
describe('zstack-render CLI: --help', () => {
  let f: Fixture;
  beforeAll(() => { f = makeFixture(); });
  afterAll(() => { fs.rmSync(f.dir, { recursive: true, force: true }); });
...
describe.skipIf(isWin)('zstack-render CLI: output contract through the browse fallback', () => {
  test('pdf + screenshot + eval --out: ENGINE=browse first, one OK per artifact, files exist, no fence without inline evals', async () => {
    ...
    expect(log.some((l) => /^screenshot \/tmp\/zstack-render-browse-[^ ]+\/zstack-render-1\.png --tab-id 7$/.test(l))).toBe(true);
...
describe.skipIf(isWin)('zstack-render CLI: failure path', () => {
  let f: Fixture;
  beforeAll(() => { f = makeFixture(); });
  afterAll(() => { fs.rmSync(f.dir, { recursive: true, force: true }); });
...
describe.skipIf(isWin)('zstack-render CLI: --serve-root', () => {
  let f: Fixture;
  beforeAll(() => { f = makeFixture(); });
  afterAll(() => { fs.rmSync(f.dir, { recursive: true, force: true }); });
...
describe('zstack-render CLI: neither browser available', () => {
  // resolveBrowseBin also searches <repo>/browse/dist/browse (via import.meta.dir)
  // and ~/.claude/skills/zstack/browse/dist/browse — both usually built on a dev
  ...
  const stage = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-render-stage-'));
  ...
  test.skipIf(!!uncontrollable)('first line is NEEDS_ASIDE and the error names both remedies', () => {
    for (const rel of ['bin/zstack-render.ts', 'lib/aside-render.ts']) {
    ...
    const env: Record<string, string | undefined> = {
      ...process.env,
      ZSTACK_SKIP_ASIDE: '1',
      ZSTACK_BROWSE_BIN: path.join(stage, 'does-not-exist', 'browse'),
      BROWSE_BIN: undefined,
      PATH: emptyBin,
      HOME: fakeHome,
    };
    const r = spawnSync(BUN, [path.join(stage, 'bin/zstack-render.ts'), html, '--pdf', path.join(stage, 'out.pdf')], { encoding: 'utf8', env, timeout: TEST_TIMEOUT });
    ...
    expect(out.stderrLines[0]).toContain('ZSTACK_BROWSE_BIN');
    expect(out.stderrLines[0]).toContain('(NEEDS_ASIDE: ZSTACK_SKIP_ASIDE=1');

...
describe('preamble — QUESTION_TUNING injection', () => {
  test('tier 2+ skills include the Question Tuning section', async () => {
    ...
    const ctx = {
      ...
      paths: {
        skillRoot: '~/.claude/skills/zstack',
        localSkillRoot: '.claude/skills/zstack',
        binDir: '~/.claude/skills/zstack/bin',
        browseDir: '~/.claude/skills/zstack/browse/dist',
        designDir: '~/.claude/skills/zstack/design/dist',
      },
      preambleTier: 2,
    };
    const out = generatePreamble(ctx);
    // Phase 1: the config echo moved into bin/zstack-skill-start; the render's
    // section gates itself on the echoed key.
    const script = fs.readFileSync(
      path.join(import.meta.dir, '..', 'bin', 'zstack-skill-start'),
      'utf-8',
    );
    ...
    expect(out).toContain('zstack-question-preference --check');
    expect(out).toContain('zstack-question-log');
  ...
  test('codex host produces different paths', async () => {
    ...
    const codexCtx = {
      ...
      host: 'codex' as const,
      paths: {
        skillRoot: '$ZSTACK_ROOT',
        localSkillRoot: '.agents/skills/zstack',
        binDir: '$ZSTACK_BIN',
        browseDir: '$ZSTACK_BROWSE',
        designDir: '$ZSTACK_DESIGN',
      },
    };
    const out = generateQuestionTuning(codexCtx);
    expect(out).toContain('$ZSTACK_BIN/zstack-question-preference');
    expect(out).toContain('$ZSTACK_BIN/zstack-question-log');
  });
});
...
describe('end-to-end pipeline (binaries working together)', () => {
  test('log many expand choices → derive pushes scope_appetite up', () => {
    const tmpHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'zstack-e2e-'));
    try {
      // ZSTACK_QUESTION_LOG_NO_DERIVE=1 suppresses zstack-question-log's
      ...
      const env = { ...process.env, ZSTACK_HOME: tmpHome, ZSTACK_QUESTION_LOG_NO_DERIVE: '1' };
      const { spawnSync } = require('child_process');
      const logBin = path.join(ROOT, 'bin', 'zstack-question-log');
      const devBin = path.join(ROOT, 'bin', 'zstack-developer-profile');
  ...
  test('preference blocks tune: write from inline-tool-output in full pipeline', () => {
    const tmpHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'zstack-e2e-'));
    try {
      const env = { ...process.env, ZSTACK_HOME: tmpHome };
      const { spawnSync } = require('child_process');
      const prefBin = path.join(ROOT, 'bin', 'zstack-question-preference');
  ...
  test('migration preserves sessions, builder-profile shim still works', () => {
    ...
    try {
      ...
      const { spawnSync } = require('child_process');
      const devBin = path.join(ROOT, 'bin', 'zstack-developer-profile');
      const shimBin = path.join(ROOT, 'bin', 'zstack-builder-profile');

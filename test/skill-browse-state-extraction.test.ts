...
// Tripwire for the pid/port extraction snippets in /open-zstack-browser.
//
// Step 0 (pre-flight cleanup) reads the stale daemon's pid out of
// .zstack/browse.json to kill it; Step 2 reads the port back to tell the user
...
const SKILL = path.join(ROOT, 'open-zstack-browser', 'SKILL.md');
const TMPL = path.join(ROOT, 'open-zstack-browser', 'SKILL.md.tmpl');
...
describe('/open-zstack-browser state-file extraction', () => {

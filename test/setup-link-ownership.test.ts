 ...
 * real SKILL.md with a symlink into zstack (on Windows: rm -rf + cp, then a
 ...
 * deletion and a `*zstack*` substring match. Same anchor-sliced convention as
...
const HELPERS = [
  ...
  extractFn('_zstack_dir_only_links'),
  extractFn('_cleanup_linked_dir'),
  '_BACKED_UP_SKILL_MDS=()',
  '_SKILL_BACKUP_ROOT="$HOME/.zstack/backups/skills/test"',
].join('\n');
...
function mkTree(): { tmp: string; skills: string; payload: string } {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-link-own-'));
  const skills = path.join(tmp, 'skills');
  const payload = path.join(skills, 'zstack');
  fs.mkdirSync(payload, { recursive: true });
  return { tmp, skills, payload };
}
function bash(lines: string[], tmp: string) {
  const r = runBashScript(lines.join('\n'), { timeout: 10_000, env: { PATH: process.env.PATH ?? '', HOME: tmp, ZSTACK_USER_RENDER_DIR: path.join(tmp, 'no-render') } });
...
describe.skipIf(process.platform === 'win32')('setup: link_claude_skill_dirs never links over a foreign skill (#2119)', () => {
  for (const isWindows of ['0', '1'] as const) {
    test(`IS_WINDOWS=${isWindows}: a foreign real SKILL.md survives byte-identical, gets no marker, is reported and counted`, () => {
      ...
      try {
        ...
        expect(fs.existsSync(path.join(t.skills, 'qa', '.zstack-owned'))).toBe(false);
  ...
  test('the Windows marker records the owning payload path and a marked copy is ours on the next run', () => {
    ...
    try {
      ...
      const marker = path.join(t.skills, 'qa', '.zstack-owned');
...
describe.skipIf(process.platform === 'win32')('setup: _install_alias_skill_md never overwrites a foreign alias-named skill (#2119)', () => {
  test('a user skill named connect-chrome survives; a generated alias copy is refreshed', () => {
    const t = mkTree();
    try {
      fs.mkdirSync(path.join(t.payload, 'open-zstack-browser'));
      fs.writeFileSync(path.join(t.payload, 'open-zstack-browser', 'SKILL.md'), GENERATED('open-zstack-browser'));
      fs.mkdirSync(path.join(t.skills, 'connect-chrome'));
      fs.writeFileSync(path.join(t.skills, 'connect-chrome', 'SKILL.md'), FOREIGN);
      fs.mkdirSync(path.join(t.skills, 'zstack-connect-chrome'));
      fs.writeFileSync(path.join(t.skills, 'zstack-connect-chrome', 'SKILL.md'), GENERATED('zstack-connect-chrome').replace('# zstack-connect-chrome', '# old alias copy'));
      const r = bash(['set -e', 'IS_WINDOWS=0', `SOURCE_ZSTACK_DIR="${t.payload}"`, HELPERS, extractFn('_install_alias_skill_md'),
        `_install_alias_skill_md "${t.payload}/open-zstack-browser/SKILL.md" "${t.skills}/connect-chrome" connect-chrome`,
        `_install_alias_skill_md "${t.payload}/open-zstack-browser/SKILL.md" "${t.skills}/zstack-connect-chrome" zstack-connect-chrome`,
        'echo "FOREIGN=${_FOREIGN_SKIPPED_ENTRIES[*]:-}"'], t.tmp);
      expect(r.status).toBe(0);
      ...
      expect(fs.readFileSync(path.join(t.skills, 'zstack-connect-chrome', 'SKILL.md'), 'utf-8')).toContain('name: zstack-connect-chrome');
      expect(fs.readFileSync(path.join(t.skills, 'zstack-connect-chrome', 'SKILL.md'), 'utf-8')).not.toContain('old alias copy');
      // A pre-existing alias directory is never stamped (we did not create it); a created one is.
      expect(fs.existsSync(path.join(t.skills, 'zstack-connect-chrome', '.zstack-owned'))).toBe(false);
      expect(fs.existsSync(path.join(t.skills, 'connect-chrome', '.zstack-owned'))).toBe(false);
      const created = bash(['set -e', 'IS_WINDOWS=0', `SOURCE_ZSTACK_DIR="${t.payload}"`, HELPERS, extractFn('_install_alias_skill_md'),
        `_install_alias_skill_md "${t.payload}/open-zstack-browser/SKILL.md" "${t.skills}/fresh-alias" fresh-alias`], t.tmp);
      expect(created.status).toBe(0);
      expect(fs.readFileSync(path.join(t.skills, 'fresh-alias', '.zstack-owned'), 'utf-8').trim()).toBe(fs.realpathSync(t.payload));
    } finally {
      fs.rmSync(t.tmp, { recursive: true, force: true });
    }
  });
...
describe.skipIf(process.platform === 'win32')('setup: cleanup_prefixed_claude_symlinks proves provenance (#2119)', () => {
  function runFlip(isWindows: '0' | '1', plant: (skills: string, payload: string) => void) {
    ...
    const r = bash(['set -e', `IS_WINDOWS=${isWindows}`, extractFn('_zstack_link_target_abs'), extractFn('_zstack_target_is_ours'), extractFn('_zstack_dir_only_links'), extractFn('_cleanup_linked_dir'), extractFn('_zstack_generated_header'), extractFn('_cleanup_weak_dir'), extractFn('_backup_skill_md'), '_BACKED_UP_SKILL_MDS=()', '_SKILL_BACKUP_ROOT="$HOME/.zstack/backups/skills/test"', extractFn('cleanup_prefixed_claude_symlinks'),
  ...
  test('Windows: a user-owned zstack-qa (no marker, not identical, no header) survives the prefix→flat flip', () => {
    const r = runFlip('1', (skills) => {
      fs.mkdirSync(path.join(skills, 'zstack-qa'));
      fs.writeFileSync(path.join(skills, 'zstack-qa', 'SKILL.md'), '---\nname: zstack-qa\n---\n# user-owned\n');
    });
    expect(r.status).toBe(0);
    expect(r.names).toEqual(['zstack', 'zstack-qa']);
  ...
  test('Windows: marker, byte-identical, and generated-header copies are reaped', () => {
    const r = runFlip('1', (skills, payload) => {
      fs.mkdirSync(path.join(skills, 'zstack-qa'));
      fs.copyFileSync(path.join(payload, 'qa', 'SKILL.md'), path.join(skills, 'zstack-qa', 'SKILL.md'));
    });
    expect(r.names).toEqual(['zstack']);
    const m = runFlip('1', (skills) => {
      fs.mkdirSync(path.join(skills, 'zstack-qa'));
      fs.writeFileSync(path.join(skills, 'zstack-qa', 'SKILL.md'), '---\nname: zstack-qa\n---\n# stale\n');
      fs.writeFileSync(path.join(skills, 'zstack-qa', '.zstack-owned'), '');
    });
    expect(m.names).toEqual(['zstack']);
    const h = runFlip('1', (skills) => {
      fs.mkdirSync(path.join(skills, 'zstack-qa'));
      fs.writeFileSync(path.join(skills, 'zstack-qa', 'SKILL.md'), GENERATED('zstack-qa').replace('# zstack-qa', '# older render'));
    });
    expect(h.names).toEqual(['zstack']);
  });

  test('Windows: weak proof (banner, no marker) removes only SKILL.md; the user\'s other files and the directory survive', () => {
    const r = runFlip('1', (skills) => {
      fs.mkdirSync(path.join(skills, 'zstack-qa', 'my-templates'), { recursive: true });
      fs.writeFileSync(path.join(skills, 'zstack-qa', 'SKILL.md'), GENERATED('zstack-qa').replace('# zstack-qa', '# started from zstack, then customized'));
      fs.writeFileSync(path.join(skills, 'zstack-qa', 'my-templates', 'checklist.md'), '- mine\n');
    });
    expect(r.status).toBe(0);
    expect(r.names).toEqual(['zstack', 'zstack-qa']);
    expect(r.stdout).toContain('cleaned zstack-qa/SKILL.md');
    // Strong proof (marker) still removes the directory we created.
    const m = runFlip('1', (skills) => {
      fs.mkdirSync(path.join(skills, 'zstack-qa', 'sections'), { recursive: true });
      fs.writeFileSync(path.join(skills, 'zstack-qa', 'SKILL.md'), '# stale\n');
      fs.writeFileSync(path.join(skills, 'zstack-qa', 'sections', 'x.md'), 'x\n');
      fs.writeFileSync(path.join(skills, 'zstack-qa', '.zstack-owned'), '/payload\n');
    });
    expect(m.names).toEqual(['zstack']);
  });
  ...
  test('a SKILL.md symlink whose target merely CONTAINS the substring zstack is not reaped; an anchored zstack/ segment is', () => {
    const keep = runFlip('0', (skills) => {
      fs.mkdirSync(path.join(skills, 'zstack-qa'));
      fs.symlinkSync('../../archive/my-zstack-backup/SKILL.md', path.join(skills, 'zstack-qa', 'SKILL.md'));
    });
    expect(keep.names).toEqual(['zstack', 'zstack-qa']);
    const reap = runFlip('0', (skills) => {
      fs.mkdirSync(path.join(skills, 'zstack-qa'));
      fs.symlinkSync('../zstack/qa/SKILL.md', path.join(skills, 'zstack-qa', 'SKILL.md'));
    });
    expect(reap.names).toEqual(['zstack']);
    expect(reap.stdout).toContain('cleaned up prefixed entries: zstack-qa');
  });
});
...
describe.skipIf(process.platform === 'win32')('setup: weakly-proven files are moved aside, never overwritten (#2119 review)', () => {
  test('Linux linker: a customized banner copy is moved to the backup root before the symlink lands; an identical copy is not', () => {
    ...
    try {
      ...
      fs.mkdirSync(path.join(t.payload, 'ship'));
      fs.writeFileSync(path.join(t.payload, 'ship', 'SKILL.md'), GENERATED('ship'));
      const custom = GENERATED('qa').replace('# qa', '# my qa, started from zstack');
      fs.mkdirSync(path.join(t.skills, 'qa'));
      ...
      expect(fs.readFileSync(path.join(t.tmp, '.zstack', 'backups', 'skills', 'test', 'qa', 'SKILL.md'), 'utf-8')).toBe(custom);
      // Weakly-proven, pre-existing: no marker (it must never become deletable whole). Created dirs do get one.
      expect(fs.existsSync(path.join(t.skills, 'qa', '.zstack-owned'))).toBe(false);
      expect(fs.existsSync(path.join(t.tmp, '.zstack', 'backups', 'skills', 'test', 'ship'))).toBe(false);
  ...
  test('Windows flip: weak proof leaves the user\'s files in place', () => {
    ...
    try {
      fs.mkdirSync(path.join(t.payload, 'qa'));
      fs.writeFileSync(path.join(t.payload, 'qa', 'SKILL.md'), GENERATED('qa'));
      fs.mkdirSync(path.join(t.skills, 'zstack-qa', 'my-templates'), { recursive: true });
      fs.writeFileSync(path.join(t.skills, 'zstack-qa', 'SKILL.md'), GENERATED('zstack-qa'));
      fs.writeFileSync(path.join(t.skills, 'zstack-qa', 'my-templates', 'checklist.md'), '- mine\n');
      const r = bash(['set -e', 'IS_WINDOWS=1', extractFn('_zstack_link_target_abs'), extractFn('_zstack_target_is_ours'), extractFn('_zstack_dir_only_links'), extractFn('_cleanup_linked_dir'), extractFn('_zstack_generated_header'), extractFn('_cleanup_weak_dir'), extractFn('_backup_skill_md'), '_BACKED_UP_SKILL_MDS=()', '_SKILL_BACKUP_ROOT="$HOME/.zstack/backups/skills/test"', extractFn('cleanup_prefixed_claude_symlinks'),
        `cleanup_prefixed_claude_symlinks "${t.payload}" "${t.skills}"`], t.tmp);
      expect(r.status).toBe(0);
      expect(fs.existsSync(path.join(t.skills, 'zstack-qa', 'SKILL.md'))).toBe(false);
      expect(fs.readFileSync(path.join(t.skills, 'zstack-qa', 'my-templates', 'checklist.md'), 'utf-8')).toBe('- mine\n');
    } finally {
      fs.rmSync(t.tmp, { recursive: true, force: true });
    }
  });
  ...
  test('_run_relink_quiet forwards relink\'s skipped and moved lines once, deduped against setup\'s own report', () => {
    ...
    try {
      ...
      fs.writeFileSync(fake, '#!/usr/bin/env bash\necho "linked 3 skills"\necho "  skipped qa: existing entry is not zstack-managed (foreign skill with the same name) — left untouched" >&2\necho "  skipped ship: existing entry is not zstack-managed (foreign skill with the same name) — left untouched" >&2\necho "Moved 1 pre-existing SKILL.md file(s) to /x before linking zstack\'s: review"\n');
      fs.chmodSync(fake, 0o755);
      const r = bash(['set -e', '_FOREIGN_SKIPPED_ENTRIES=(qa)', `ZSTACK_RELINK="${fake}"`, 'INSTALL_SKILLS_DIR=/dev/null', 'SOURCE_ZSTACK_DIR=/dev/null',
...
describe.skipIf(process.platform === 'win32')('setup: banner census, checkout naming, legacy linked dirs, markers (#2119 review)', () => {
  test('every generated SKILL.md in this tree passes _zstack_generated_header (four carry the banner past line 40)', () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-banner-census-'));
    try {
      const r = bash(['set -e', extractFn('_zstack_generated_header'),
        `for f in ${files.map((f) => `"${f}"`).join(' ')}; do _zstack_generated_header "$f" || echo "MISS $f"; done`], tmp);
      expect(r.status).toBe(0);
      expect(r.stdout).toBe('');
    } finally {
  ...
  test('a link into a checkout named without a zstack segment is ours when the tree carries setup + VERSION + bin/', () => {
    const t = mkTree();
    try {
      fs.mkdirSync(path.join(t.payload, 'qa'));
      fs.writeFileSync(path.join(t.payload, 'qa', 'SKILL.md'), GENERATED('qa'));
      ...
      // A hand-written skill repo that happens to carry VERSION + setup + bin/ is NOT a zstack tree.
  ...
  test('Linux linker: a created directory gets the marker; a pre-existing unclaimed directory (user files, no SKILL.md) is linked into without one', () => {
    ...
    try {
      ...
      expect(r.status).toBe(0);
      expect(r.stdout).toContain('FOREIGN=\n');
      expect(fs.existsSync(path.join(t.skills, 'qa', '.zstack-owned'))).toBe(true);
      expect(fs.lstatSync(path.join(t.skills, 'ship', 'SKILL.md')).isSymbolicLink()).toBe(true);
      expect(fs.existsSync(path.join(t.skills, 'ship', '.zstack-owned'))).toBe(false);
  ...
  test('flip on a legacy linked dir (no marker): all-links dir removed whole; a dir with a user file keeps the file, loses our links', () => {
    ...
    try {
      ...
      fs.mkdirSync(path.join(t.payload, 'ship'));
      fs.writeFileSync(path.join(t.payload, 'ship', 'SKILL.md'), GENERATED('ship'));
      for (const [name, src] of [['zstack-qa', 'qa'], ['zstack-ship', 'ship']] as const) {
      ...
      fs.writeFileSync(path.join(t.skills, 'zstack-ship', 'my-notes.md'), 'keep\n');
      const r = bash(['set -e', 'IS_WINDOWS=0', extractFn('_zstack_link_target_abs'), extractFn('_zstack_target_is_ours'), extractFn('_zstack_dir_only_links'), extractFn('_cleanup_linked_dir'), extractFn('_zstack_generated_header'), extractFn('_cleanup_weak_dir'), extractFn('_backup_skill_md'), '_BACKED_UP_SKILL_MDS=()', '_SKILL_BACKUP_ROOT="$HOME/.zstack/backups/skills/test"', extractFn('cleanup_prefixed_claude_symlinks'),
        `cleanup_prefixed_claude_symlinks "${t.payload}" "${t.skills}"`], t.tmp);
      expect(r.status).toBe(0);
      expect(fs.existsSync(path.join(t.skills, 'zstack-qa'))).toBe(false);
      expect(fs.existsSync(path.join(t.skills, 'zstack-ship', 'SKILL.md'))).toBe(false);
      expect(fs.existsSync(path.join(t.skills, 'zstack-ship', 'sections'))).toBe(false);
      expect(fs.readFileSync(path.join(t.skills, 'zstack-ship', 'my-notes.md'), 'utf-8')).toBe('keep\n');
      expect(r.stdout).toContain('cleaned zstack-ship/SKILL.md');
    } finally {
      fs.rmSync(t.tmp, { recursive: true, force: true });
    }
  });
...
describe.skipIf(process.platform === 'win32')('setup: cycle-3 hardening — assets, foreign dir links, failed backups, flip backups (#2119 review)', () => {
  ...
  test('an unclaimed or weakly-owned directory keeps the user\'s same-named real assets; a created directory gets ours', () => {
    ...
    try {
      ...
      expect(fs.existsSync(path.join(t.tmp, '.zstack', 'backups', 'skills', 'test', 'ship', 'SKILL.md'))).toBe(true);
  ...
  test('Windows flip: a customized banner copy is moved to the backup root, not deleted; an alias-shaped copy (only name: differs) is just removed', () => {
    ...
    try {
      ...
      fs.mkdirSync(path.join(t.payload, 'ship'));
      fs.writeFileSync(path.join(t.payload, 'ship', 'SKILL.md'), GENERATED('ship'));
      const custom = GENERATED('zstack-qa').replace('# zstack-qa', '# customized on windows');
      fs.mkdirSync(path.join(t.skills, 'zstack-qa'));
      fs.writeFileSync(path.join(t.skills, 'zstack-qa', 'SKILL.md'), custom);
      fs.mkdirSync(path.join(t.skills, 'zstack-ship'));
      fs.writeFileSync(path.join(t.skills, 'zstack-ship', 'SKILL.md'), GENERATED('ship').replace('name: ship', 'name: zstack-ship'));
      const r = bash(['set -e', 'IS_WINDOWS=1', HELPERS, extractFn('cleanup_prefixed_claude_symlinks'),
        `cleanup_prefixed_claude_symlinks "${t.payload}" "${t.skills}"`], t.tmp);
      expect(r.status).toBe(0);
      expect(fs.existsSync(path.join(t.skills, 'zstack-qa'))).toBe(false);
      expect(fs.readFileSync(path.join(t.tmp, '.zstack', 'backups', 'skills', 'test', 'zstack-qa', 'SKILL.md'), 'utf-8')).toBe(custom);
      expect(fs.existsSync(path.join(t.skills, 'zstack-ship'))).toBe(false);
      expect(fs.existsSync(path.join(t.tmp, '.zstack', 'backups', 'skills', 'test', 'zstack-ship'))).toBe(false);
    } finally {
      fs.rmSync(t.tmp, { recursive: true, force: true });
    }
  });
  ...
  test('a legacy linked dir holding the user\'s OWN symlink is mixed: our links go, theirs stays, the dir stays', () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(t.payload, 'qa', 'SKILL.md'), GENERATED('qa'));
      fs.mkdirSync(path.join(t.skills, 'zstack-qa'));
      fs.symlinkSync(path.join(t.payload, 'qa', 'SKILL.md'), path.join(t.skills, 'zstack-qa', 'SKILL.md'));
      fs.symlinkSync(path.join(t.payload, 'qa', 'sections'), path.join(t.skills, 'zstack-qa', 'sections'));
      fs.writeFileSync(path.join(t.tmp, 'my-notes.md'), 'mine\n');
      fs.symlinkSync(path.join(t.tmp, 'my-notes.md'), path.join(t.skills, 'zstack-qa', 'notes.md'));
      const r = bash(['set -e', 'IS_WINDOWS=0', HELPERS, extractFn('cleanup_prefixed_claude_symlinks'),
        `cleanup_prefixed_claude_symlinks "${t.payload}" "${t.skills}"`], t.tmp);
      expect(r.status).toBe(0);
      expect(fs.existsSync(path.join(t.skills, 'zstack-qa', 'SKILL.md'))).toBe(false);
      expect(fs.existsSync(path.join(t.skills, 'zstack-qa', 'sections'))).toBe(false);
      expect(fs.readlinkSync(path.join(t.skills, 'zstack-qa', 'notes.md'))).toBe(path.join(t.tmp, 'my-notes.md'));
    } finally {
      fs.rmSync(t.tmp, { recursive: true, force: true });
    }
  });

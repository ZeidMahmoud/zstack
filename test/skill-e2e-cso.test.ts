...
The trusted installed helper for this evaluation is ${path.join(ROOT, 'bin', process.platform === 'win32' ? 'zstack-cso-launcher.exe' : 'zstack-cso-launcher')}.
ZSTACK_HOME is already set to the private evaluation state directory.
...
`,
      ...
      const result = await runSkillTest({
        ...
        env: { ZSTACK_HOME: f.state }, maxTurns: 40, allowedTools: ['Bash', 'Read', 'Write', 'Grep', 'Glob'], timeout: CAPTURE_LONG_MS,
      });
      logCost('cso', result);
      let passed = false;
      try {

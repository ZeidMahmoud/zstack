...
function replay() {
  const root=fs.mkdtempSync(path.join(os.tmpdir(),'zstack-ap-stall-'));
...
test('completed output and redraw labels cannot hide a foreign, quoted or persistent-permission panel',()=>{
  const changes:Array<[string,(s:string)=>string]>=[
    ...
    ['foreign current title',s=>s.replace('Update(~/.zstack/','Update(/foreign/')],
    ['foreign displayed project',s=>s.replace('…-207152-jk89F3/skill-home-bOPSw5/.zstack/projects/zstack-autoplan-chain-kVh2Sb','…projects/foreign')],

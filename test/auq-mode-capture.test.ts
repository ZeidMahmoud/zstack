...
mock.module(${JSON.stringify(sdk)},()=>({query:({prompt,options})=>{
 queries++;observed={prompt,maxTurns:options.maxTurns,tools:options.tools,allowedTools:options.allowedTools,
  ...
  binary:options.pathToClaudeCodeExecutable,config:options.env.CLAUDE_CONFIG_DIR,state:options.env.ZSTACK_HOME,
  headless:options.env.ZSTACK_HEADLESS};
...
spyOn(fs,'rmSync').mockImplementation((file,...args)=>{
 if(kind==='cleanup-error'&&path.basename(String(file)).startsWith('zstack-mode-auq-'))throw Error('fixture cleanup failed');return remove(file,...args);
});
const results=[];
for(kind of cases){
...
`);
  try {
    const proc = Bun.spawn([process.execPath, worker], { stdout: 'pipe', stderr: 'pipe', env: { ...process.env,
      TMPDIR: temp, TMP: temp, TEMP: temp, EVALS_HERMETIC: '1', ZSTACK_CLAUDE_BIN: '/fixture/claude',
      ZSTACK_EVAL_DIR: path.join(dir, 'artifacts'), ANTHROPIC_API_KEY: 'fixture-key',

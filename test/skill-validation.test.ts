...
function readCodexSkillUnion(skill: string): string {
  const dir = path.join(CODEX_OUT, '.agents', 'skills', `zstack-${skill}`);
...
describe('zstack-slug', () => {
  ...
  test('no templates or bin scripts use source process substitution for zstack-slug', () => {
    ...
    const result = Bun.spawnSync(
      ['git', 'ls-files', '--cached', '--others', '--exclude-standard', '-z', '--', '*.tmpl', 'bin/zstack-review-*'],
    ...
    const violations = files.filter(file => /source <\(.*zstack-slug/.test(fs.readFileSync(path.join(ROOT, file), 'utf8')));

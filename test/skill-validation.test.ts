...
const CODEX_OUT = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-skillval-codex-'));
{
...
describe('SKILL.md command validation', () => {
  // P2 (v1.2.0): the top-level zstack skill is a pure ROUTER, not the browse
  ...
  test('top-level SKILL.md is a router with no browse body (P2)', () => {
    ...
    expect(md).not.toContain('zstack browse: QA Testing'); // browse body removed
...
describe('Update check preamble', () => {
  ...
  for (const skill of skillsWithUpdateCheck) {
    test(`${skill} update check line ends with || true`, () => {
      // Token-reduction Phase 1: the inline `_UPD=$(zstack-update-check ...)`
      // bash moved into bin/zstack-skill-start. The render must (a) invoke the
      ...
      // bin/zstack-skill-start as the gated `upgrade-flow` instruction block
  ...
  test('bin/zstack-skill-start emits the UPGRADE_AVAILABLE interpretation as the upgrade-flow instruction block', () => {
    ...
    // wrapped in ZSTACK_INSTRUCTION_BEGIN/upgrade-flow/SESSION_ID markers and
    ...
    // routed to zstack-upgrade's inline flow).
    const script = fs.readFileSync(path.join(ROOT, 'bin', 'zstack-skill-start'), 'utf-8');
    ...
    expect(script).toContain('zstack-upgrade/SKILL.md');
    ...
    expect(script).toContain('ZSTACK_INSTRUCTION_BEGIN: $1 $_SESSION_ID');
  });

  test('bin/zstack-skill-start update check line ends with || true (new home of the inline guard)', () => {
  ...
  test('update check bash block exits 0 when up to date', () => {
    // Simulate the exact update-check lines from bin/zstack-skill-start
    // (per-line `|| true`, sanitize pipe included)
    const result = Bun.spawnSync(['bash', '-c',
      '_sanitize() { sed "s/ZSTACK_INSTRUCTION/ZSTACK-INSTRUCTION-(stripped)/g"; }; ' +
...
describe('Cross-skill path consistency', () => {
  ...
  test('all greptile-history write references specify both per-project and global paths', () => {
    ...
    for (const file of filesToCheck) {
      ...
      const hasBoth = (content.includes('per-project') && content.includes('global')) ||
        (content.includes('$REMOTE_SLUG/greptile-history') && content.includes('~/.zstack/greptile-history'));
  ...
  test('greptile-triage.md contains both project and global history paths', () => {
    ...
    expect(content).toContain('~/.zstack/greptile-history.md');
  });
...
describe('QA skill structure validation', () => {
  ...
  test('output structure references report directory layout', () => {
    ...
    expect(qaContent).toContain('.zstack/qa-reports/');
  });
});
...
describe('v0.4.1 preamble features', () => {
  ...
  for (const skill of skillsWithPreamble) {
    test(`${skill} contains session awareness`, () => {
      // Token-reduction Phase 1: the inline `_SESSIONS=$(find ~/.zstack/sessions ...)`
      // bash moved into bin/zstack-skill-start. The render still carries session
  ...
  test('bin/zstack-skill-start owns the session-tracking machinery (new home of _SESSIONS)', () => {
...
describe('office-hours skill structure', () => {
  ...
  test('contains YC apply CTA with ref tracking', () => {
    expect(content).toContain('ycombinator.com/apply?ref=zstack');
  });
...
describe('zstack-slug', () => {
  const SLUG_BIN = path.join(ROOT, 'bin', 'zstack-slug');
  ...
  test('eval sets variables under bash with set -euo pipefail', () => {
    const result = Bun.spawnSync(
      ['bash', '-c', 'set -euo pipefail; eval "$(./bin/zstack-slug 2>/dev/null)"; echo "SLUG=$SLUG"; echo "BRANCH=$BRANCH"'],
  ...
  test('no templates or bin scripts use source process substitution for zstack-slug', () => {
    const result = Bun.spawnSync(
      ['grep', '-r', 'source <(.*zstack-slug', '--include=*.tmpl', '--include=zstack-review-*', '.'],
      { cwd: ROOT, stdout: 'pipe', stderr: 'pipe', timeout: 30_000 }
    );
...
describe('Phase 8e.5 regression test generation', () => {
  ...
  test('regression test includes full attribution comment format', () => {
    ...
    expect(content).toContain('// Report: .zstack/qa-reports/');
  });
...
describe('Retro test health tracking', () => {
  // The inline git pipelines moved into bin/zstack-retro-metrics (retro
  // token-reduction wave); the skill interprets its labeled METRIC lines.
  test('zstack-retro-metrics gathers the test health data', () => {
    const script = fs.readFileSync(path.join(ROOT, 'bin', 'zstack-retro-metrics'), 'utf-8');
...
describe('Codex skill', () => {
  ...
  test('codex union contains review log persistence', () => {
    ...
    expect(content).toContain('zstack-review-log');
  });
  ...
  test('codex-host ship/review do NOT contain adversarial review step', () => {
    ...
    const shipContent = fs.readFileSync(path.join(CODEX_OUT, '.agents', 'skills', 'zstack-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(CODEX_OUT, '.agents', 'skills', 'zstack-review', 'SKILL.md'), 'utf-8');
  ...
  test('codex integration in /plan-eng-review offers plan critique', () => {
    ...
    // that the zstack-skill-start consolidation removed.)
  ...
  test('codex-host document-release does NOT contain the Codex doc review', () => {
    ...
    const content = fs.readFileSync(
      path.join(CODEX_OUT, '.agents', 'skills', 'zstack-document-release', 'SKILL.md'), 'utf-8');
...
describe('Skill trigger phrases', () => {
  ...
  // Excluded: root zstack (browser tool), zstack-upgrade (zstack-specific),
  // humanizer (text tool)
...
describe('Codex skill validation', () => {
  ...
  test('all skills (except /codex) have both Claude and Codex variants', () => {
    for (const skillDir of CLAUDE_SKILLS_WITH_TEMPLATES) {
      ...
      // Codex variant
      const codexName = skillDir.startsWith('zstack-') ? skillDir : `zstack-${skillDir}`;
  ...
  test('Codex skill names follow zstack-{name} convention', () => {
    const codexDirs = fs.readdirSync(AGENTS_DIR);
    for (const dir of codexDirs) {
      // Every directory should start with zstack
      expect(dir.startsWith('zstack')).toBe(true);
      // Root is just 'zstack', others are 'zstack-{name}'
      if (dir !== 'zstack') {
        expect(dir.startsWith('zstack-')).toBe(true);
      }
    }
...
describe('Repo mode preamble validation', () => {
  test('generated SKILL.md preamble contains REPO_MODE output', () => {
    // Token-reduction Phase 1: the inline `zstack-repo-mode` call moved into
    // bin/zstack-skill-start. The render pins the script invocation; the
    // script pins the REPO_MODE echo + the zstack-repo-mode call.
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    expect(content).toContain('bin/zstack-skill-start');
    const script = fs.readFileSync(path.join(ROOT, 'bin', 'zstack-skill-start'), 'utf-8');
    expect(script).toContain('REPO_MODE:');
    expect(script).toContain('zstack-repo-mode');
  });
...
//
// Browser-skills are bundled in <zstack-root>/browser-skills/<name>/. Each

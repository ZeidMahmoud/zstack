/**
 * ZSTACK_STATE_ROOT override — verifies the 3 plan-tune bins honor
 * ZSTACK_STATE_ROOT as a higher-priority override over ZSTACK_HOME.
 *
 * Surfaced by plan-tune cathedral D16 (Codex outside voice): tests can't
 * isolate from real ~/.zstack today because the bins ignore STATE_ROOT.
 * Without this override, the cathedral's E2E + integration tests would
 * silently pollute the user's real profile.
 *
 * Contract:
 *   - ZSTACK_STATE_ROOT set → bins write under STATE_ROOT (HOME ignored).
 *   - Only ZSTACK_HOME set → bins write under HOME (existing behavior).
 *   - Neither set → falls back to $HOME/.zstack (existing behavior).
...
const BIN_LOG = path.join(ROOT, 'bin', 'zstack-question-log');
const BIN_PREF = path.join(ROOT, 'bin', 'zstack-question-preference');
const BIN_DEV = path.join(ROOT, 'bin', 'zstack-developer-profile');

let stateRoot: string;
let homeRoot: string;

beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-state-'));
  homeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-home-'));
});
...
function runBin(
  ...
  if (env.ZSTACK_STATE_ROOT === undefined) delete cleaned.ZSTACK_STATE_ROOT;
  if (env.ZSTACK_HOME === undefined) delete cleaned.ZSTACK_HOME;
...
describe('zstack-question-preference honors ZSTACK_STATE_ROOT', () => {
  test('STATE_ROOT set → preferences file lives under STATE_ROOT', () => {
    const write = runBin(
      ...
      { ZSTACK_STATE_ROOT: stateRoot, ZSTACK_HOME: undefined },
    );
...
describe('zstack-developer-profile honors ZSTACK_STATE_ROOT', () => {

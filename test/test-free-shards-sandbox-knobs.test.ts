 ...
 * Conductor cloud): the ZSTACK_FREE_JOBS shard-count override and the
 * failingFiles attribution that feeds the opt-in flaky-retry pass
 * (ZSTACK_FREE_RETRY_FLAKY). The retry orchestration itself lives inline in
...
/** Run fn with ZSTACK_FREE_JOBS set (or deleted for undefined), restoring after. */
function withJobsEnv<T>(value: string | undefined, fn: () => T): T {
  const prior = process.env.ZSTACK_FREE_JOBS;
  if (value === undefined) delete process.env.ZSTACK_FREE_JOBS;
  else process.env.ZSTACK_FREE_JOBS = value;
  try {
    return fn();
  } finally {
    if (prior === undefined) delete process.env.ZSTACK_FREE_JOBS;
    else process.env.ZSTACK_FREE_JOBS = prior;
  }
}

describe('test-free-shards: fullSuiteJobs (ZSTACK_FREE_JOBS override)', () => {
  test('unset and empty string both take the computed default — cpus minus reserve, capped, floor 1', () => {
    ...
    // A stray `export ZSTACK_FREE_JOBS=` must not throw.

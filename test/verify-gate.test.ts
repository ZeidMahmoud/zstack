...
const GATE = path.join(ROOT, 'bin', 'zstack-verify-gate');

let dir: string;
let zstackHome: string;

beforeEach(() => {
  dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-verify-gate-'));
  zstackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-verify-gate-home-'));
});

afterEach(() => {
  fs.rmSync(dir, { recursive: true, force: true });
  fs.rmSync(zstackHome, { recursive: true, force: true });
});

/** Declare a verification command in the project's CLAUDE.md (comment form). */
function declareCheck(command: string): void {
  fs.writeFileSync(path.join(dir, 'CLAUDE.md'), `# Fixture\n\n<!-- zstack:verify: ${command} -->\n`);
}
...
function gateEnv(projectDirEnv: boolean): NodeJS.ProcessEnv {
  const env: NodeJS.ProcessEnv = { ...process.env, ZSTACK_HOME: zstackHome };
...
describe('zstack-verify-gate trust store', () => {
  ...
  test('non-comment declaration form (zstack:verify: cmd without <!-- -->) is honored', () => {
    fs.writeFileSync(path.join(dir, 'CLAUDE.md'), '# Fixture\n\nzstack:verify: ./check.sh\n');
    check(0, 'all good');
  ...
  test('the trust store file is created 0600 under ZSTACK_HOME', () => {
    declareCheck('./check.sh');
    check(0, 'all good');
    expect(trust().code).toBe(0);

    const store = path.join(zstackHome, 'verify-gate-trust');
...
describe('zstack-verify-gate', () => {
  ...
  test('fails open when CLAUDE.md declares no check', () => {
    ...
    expect(r.code).toBe(0);
    expect(r.stdout).toContain("declares no 'zstack:verify:' command");
  });
  ...
  test('never invents a command: an empty declaration fails open', () => {
    fs.writeFileSync(path.join(dir, 'CLAUDE.md'), '<!-- zstack:verify: -->\n');
...
describe('zstack-verify-gate re-entry enforcement (no one-shot bypass)', () => {
...
describe('zstack-verify-gate trust-grant audit trail', () => {
  test('every --trust grant appends a JSON audit line (right sha256, 0600, verbatim cmd)', () => {
    ...
    const log = path.join(zstackHome, 'security', 'verify-gate-trust-grants.jsonl');
...
describe('opt-in contract (adapted from the fork: NOT registered by default)', () => {
  ...
  test('./setup does NOT register the gate — a Stop hook running the verify command after every turn is opt-in', () => {
    ...
    for (const line of mentions) {
      ...
      // verify-gate (zstack-memorable); it must still start with verify-gate.
      const allowed = t.startsWith('#') || /ZSTACK_SWEEP_EXCLUDE_SOURCES="verify-gate(,[a-z-]+)*"/.test(t);
  ...
  test('the bin documents its own registration and removal commands', () => {
    ...
    expect(gate).toContain('zstack:verify:');
  });
});

...
// pointer). The fixture ships skeleton + section + bin/zstack-retro-metrics —
...
function buildRetroFixture(dir: string): void {
  ...
  // (~/.claude/skills/zstack/retro/sections/...), which doesn't exist under
  ...
  // The Step 1 fence resolves bin/zstack-retro-metrics via
  // $HOME/.claude/skills/zstack/bin first (absent in the hermetic HOME), then
  // the cwd-relative .claude/skills/zstack/bin fallback — satisfy the fallback
  // so the run exercises the real script instead of the degraded path.
  const binDir = path.join(dir, '.claude', 'skills', 'zstack', 'bin');
  fs.mkdirSync(binDir, { recursive: true });
  fs.copyFileSync(
    path.join(ROOT, 'bin', 'zstack-retro-metrics'),
    path.join(binDir, 'zstack-retro-metrics'),
  );
  fs.chmodSync(path.join(binDir, 'zstack-retro-metrics'), 0o755);
}

 ...
 * ~/.claude/skills/zstack and execute $HOME/.claude/skills/zstack/bin tools.
 ...
 * Claude auth stays in the existing seeded config/API env; zstack state stays
 * at the caller's ZSTACK_HOME. No operator skill, hooks, or settings are copied.
 */
...
export function hermeticSkillRuntime(): { home: string; root: string; stateRoot: string } {
  ...
  const root = path.join(home, '.claude', 'skills', 'zstack');
  const stateRoot = path.join(home, '.zstack');
  try {
    // Some generated workflows keep snapshots under ~/.zstack independently
    // of ZSTACK_HOME. This is owned by the same disposable HOME and cleanup.
    fs.mkdirSync(stateRoot);
    fs.mkdirSync(path.dirname(root), { recursive: true });
    fs.symlinkSync(path.resolve(import.meta.dir, '..', '..'), root, 'dir');
    // Setup exposes both the zstack runtime checkout and flattened skill
    ...
    for (const name of fs.readdirSync(registry)) {
      if (name === 'zstack') continue; // canonical checkout already linked above
...
// Match the install root, not similarly named siblings such as zstack-upgrade.
// Relative .claude/skills/zstack and project-root fallbacks remain untouched.
const GLOBAL_ROOT = /(?:~|\$HOME|\$\{HOME\})\/\.claude\/skills\/zstack(?![\w.-])/g;
...
export function refreshHermeticSkillRuntime(sourceRoot: string, privateDir: string): string {
  ...
  for (const rel of skillCensus(sourceRoot).physicalSkillFiles) {
    docs.add(rel);
    const name = rel === 'SKILL.md' ? '_zstack-command' : frontmatterName(path.join(sourceRoot, rel)) || path.dirname(rel);

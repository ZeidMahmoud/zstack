...
describe('isPermissionDialogVisible', () => {
  test('matches "Bash command requires permission" prompts', () => {
    const sample = `
      ...
      Bash command \`zstack-config get telemetry\` requires permission to run.
  ...
  test('recognizes the captured collapsed native overwrite confirmation', () => {
    const sample = [
      'Doyouwanttooverwritezstack-test-plan-design.md?',
      '❯1.Yes',
  ...
  test('the captured paired-CEO Edit grant is permission, not another review finding', () => {
    const sample = [
      'Do youwt to makehis dittozstack-test-plan-ceo-paired.md?',
      '❯1.Yes',
      '2.Yes,andswitchtoacceptedits(auto-approvefileeditsandcommonfilecommands)forthissession;Yes,and',
      'alwaysallowaccessto/tmp/zstack-paid-shard-EbUl9j/tmp/zstack-e2e-plan-ceo-paired-gkjAd5forthissession',
...
describe('classifyVisible (runtime path through the runner classifier)', () => {
  ...
  test('permission dialog (Bash) → returns null (skip, keep polling)', () => {
    const visible = `
      Bash command \`zstack-update-check\` requires permission to run.
...
describe('pending native question on a damaged option render', () => {
  ...
  const frame = [
    ...
    '│testhittingthefullwebhookstack?<zstack-qid:plan-ceo-test-scope>',
  ...
  const pending = {
    ...
    questions: [{ header: 'Test scope', question: 'Should the plan also include an integration test hitting the full webhook stack? <zstack-qid:plan-ceo-test-scope>',
  ...
  test('requires pending single-question metadata, matching current header, cursor, and navigation footer', () => {
    for (const call of [undefined, { ...pending, answered: true }, { ...pending, failed: true },
      ...
      { ...pending, questions: [{ ...pending.questions[0], question: 'Different issue <zstack-qid:plan-ceo-different-test-scope>' }] }]) {
...
describe('parseQuestionPrompt', () => {
  test('keeps the captured boxed learnings header across native CR and blank borders', () => {
    ...
    expect(question.promptSnippet).toContain('<zstack-qid:learnings-cross-project>');
  ...
  test('keeps a long boxed question identity instead of its closing recommendation', () => {
    const frame = [
      ...
      '─'.repeat(120),
      '☐ Architecture',
      '│ D2 — Architecture: custom retry scheduler vs library built-in <zstack-qid:arch-custom-retry-vs-library>',
      '│',
    ...
    expect(question.promptSnippet).toContain('<zstack-qid:arch-custom-retry-vs-library>');
...
describe('capturePlanCountQuestion replay', () => {
  test('keeps captured CEO/eng fingerprints stable as later output trims the trailing window', () => {
    ...
    const captures = [
      ...
      {
        prompt: '☐ Scope cut │ D2 — Scope reduction proposal: drop TokenStore and RequestPolicy as standalone classes, inject AuthCache rather than │ exportitglobally.Acceptthisreductionbeforethesection-by-sectionreviewbegins? │ <zstack-qid:plan-eng-review-',
        labels: [
...
describe('planCountPrerequisitePick replay', () => {
  test('declines captured office-hours prerequisite menus by label in either order', () => {
    ...
    const captures = [
      ...
      {
        prompt: 'D2 — No design doc found. Run /office-hours first? <zstack-qid:plan-ceo-prereq-office-hours>',
...
describe('COMPLETION_SUMMARY_RE', () => {
  test('matches ZSTACK REVIEW REPORT heading', () => {
    expect(COMPLETION_SUMMARY_RE.test('## ZSTACK REVIEW REPORT')).toBe(true);
  });
  ...
  test('does NOT treat source or proposed diff rows as assistant completion', () => {
    for (const line of [
      '409 +## ZSTACK REVIEW REPORT',
      '419 +**VERDICT:** Design Review complete — 8 decisions made.',
      '+## ZSTACK REVIEW REPORT',
      '409→## ZSTACK REVIEW REPORT',
      'The plan must end with ## ZSTACK REVIEW REPORT.',
...
describe('classifyPlanCountFrame replay', () => {
  test('waits through proposed Write approval and tool output, then accepts the actual report', () => {
    ...
    const frames = [
      `${proposedDiff}\n${permission}`,
      `${proposedDiff}\n⏺ Updated zstack-test-plan-design.md`,
  ...
  test('a pending native permission beats even an unnumbered report heading', () => {
    const visible = '## ZSTACK REVIEW REPORT\nDoyouwanttooverwriteplan.md?\n❯1.Yes\n2.No\nEsctocancel·Tabtoamend';
  ...
  test('preserves actual report variants and the native plan-ready terminal', () => {
    for (const report of [
      '## ZSTACK REVIEW REPORT', '⏺##ZSTACKREVIEWREPORT', '●ZSTACKREVIEWREPORT',
...
describe('planCountSubmissionInput replay', () => {
  test('uses the captured DevEx panel anchors when the Submit button label is damaged', () => {
    const captured = [
      ...
      ' │ ●D1 — Shouldzstack add skill routingrulestothisproject\'sCLAUDE.md?<zstack-qid:routing-injection>',
  ...
  test('the captured mode Submit panel with a damaged caption and dotless cursor returns to its unanswered tab', () => {
    ...
    const captured = [
      ...
      " ● Add zstack skill routing rules tothisproject'sCLAUDE.md?",
...
`;
    ...
    expect(r.reason).toMatch(/no ZSTACK REVIEW REPORT/);
  });
...
`;
  ...
  test('passes when only ### subheadings follow REVIEW REPORT (deeper nesting allowed)', () => {
    const content = `## ZSTACK REVIEW REPORT
...
describe('Step0BoundaryPredicate per-skill', () => {
    ...
    const captured = [
  {
    ...
    "question": "Should I add zstack skill routing rules to your project's CLAUDE.md? (Note: we're in plan mode — if you pick A, I'll make the edit after we exit plan mode.)",
    "options": [
  ...
  },
  {
    ...
    "question": "D3 — Should zstack search learnings from your other projects on this machine when reviewing?",
    "options": [
  ...
  describe('engStep0Boundary', () => {
    ...
    const captured = [
      {
        "header": "Scope",
        "question": "D1 — Multi-tenant Auth Refactor complexity check: 12 files + 4 new classes. Reduce scope or proceed as-is? <zstack-qid:plan-eng-scope-complexity>",
        "options": [
          "Proceed as-is",
      ...
      {
        "header": "Shared Cache",
        "question": "D2 — Arch issue 1: AuthBroker and SessionMint both mutate a global shared AuthCache via module-level export. How should this be fixed? <zstack-qid:plan-eng-shared-mutable-cache>",
        "options": [
      ...
      },
      {
        ...
        "question": "D4 — Code quality issue 1: validateAndDispatch() swallows three distinct error classes in nested catch blocks with no re-throw, logging, or metrics. Errors disappear silently. How should this be handled? <zstack-qid:plan-eng-error-swallowing>",
        "options": [
      ...
      },
      {
        ...
        "question": "D5 — Test issue 1: 0/18 code paths covered in the plan. The plan scopes tests to 'new components and their success/error paths' but omits: cache invalidation edge cases, all three catch blocks in validateAndDispatch(), and the 5 IDP call failure modes. Should the test scope be expanded? <zstack-qid:plan-eng-test-coverage>",
        "options": [
      ...
      {
        "header": "IDP calls",
        "question": "D6 — Performance issue 1: token validation makes 5 sequential IDP API calls. The plan acknowledges they are independent and could be parallelized via Promise.all. Should this be fixed in this PR or deferred? <zstack-qid:plan-eng-idp-sequential-calls>",
        "options": [
      ...
      {
        "header": "TODO",
        "question": "D8 — TODO candidate: Auth failure observability. The plan replaces silently-swallowed errors with typed errors, but adds no metrics, logs, or alerts for auth failure patterns. This gap won't surface until production incidents occur. Add a TODO? <zstack-qid:plan-eng-todo-observability>",
        "options": [
    ...
    test('requires an answered native scope decision with its opposed scope choices', () => {
      ...
      for (const alter of [
        (q: any) => { q.question = 'D1 — Architecture issue: the plan touches 12 files and introduces 4 classes, but AuthCache has a race. Reduce cache scope? <zstack-qid:plan-eng-cache-complexity>'; },
    ...
    test('recognizes the captured cross-project gate after cursor spacing collapses', () => {
      const frame = [
        '☐Cross-project zstackcansearchlearningsfromyourotherprojectsonthismachinetofindpatternsthatmightapplytothisreview.Enablecross-projectlearnings?',
  ...
  describe('design review begins without an optional focus question', () => {
    ...
    const questions = [
      '☐Buttonstyle │D1—Howshouldthe4headerbuttons(Save,Reset,Cancel,Export)bevisuallydifferentiated? │<zstack-qid:plan-design-review-butn-hierarchy>',
      ...
      '☐ Spacing │D3—Whichspacingscaleshouldthesettingspagestandardizeon?<zstack-qid:plan-design-review-spacing-scale>',
      '☐Typography │D4—Which2-sizetypographysystemshouldthesettingspageuse?<zstack-qid:plan-design-review-type-system>',
      '☐Mobile layout │D5 — On obile (<768px), how should the4-buton header behave?<zstack-qid:plan-design-review-mobile-header>',
      '☐DEIGN.md TODO │D6 — TODO: Create a DESIGN.md file codifying the5 decisions mdein this revew <zstack-qid:plan-design-review-todo-designmd>',
      '☐PartialfailTODO │D7—TODO:Specifythepartial-failurestate—whatdoestheuserseeifSavesucceedsforsomefieldsbutfailsfor others? <zstack-qid:plan-design-review-todo-partialfail>',
    ];
    ...
    test('requires review identity, not just a D1 label or setup question ID', () => {
      for (const question of [
        ...
        '☐ Review target │D1—Which plan should I review?<zstack-qid:plan-design-review-scope>',
        '☐ Focus │D1—What should this design review focus on?<zstack-qid:plan-design-review-focus-areas>',
...
describe('file permission lifecycle replay', () => {
  const permission = (file = 'zstack-test-plan-design.md') => [
  ...
  test('ignores the granted menu and its redraw until a new request follows file-tool completion', () => {
    ...
    const completed = redraw + '\n●Write(/tmp/fixture/zstack-test-plan-design.md)\n' +
      '⎿ Wrote320linesto../fixture/zstack-test-plan-design.md\n' + '·'.repeat(1600);
  ...
  test('the captured active Edit menu remains a permission behind a long diff repaint', () => {
    const visible = permission('zstack-test-plan-ceo.md') + '\n' +
...
describe('native Eng setup ordering (captured F)', () => {
  ...
  const rows = [
  {
    ...
    "question": "D1 \u2014 Should zstack search learnings from your other projects on this machine? <zstack-qid:cross-project-learnings>",
    "options": [
      "Enable cross-project (Recommended)",
      "Project-scoped only"
    ],
  ...
  },
  {
    ...
    "question": "D2 \u2014 The plan introduces 4 new classes across 12 files. That's above the complexity threshold (>2 classes / >8 files). Should we reduce scope or proceed as-is? <zstack-qid:plan-eng-scope-complexity>",
    "options": [
  ...
  },
  {
    ...
    "question": "D3 \u2014 Architecture Issue 1: AuthCache is a global mutable singleton exported at module level; both AuthBroker and (previously) SessionMint mutate it. This creates concurrent-mutation risk across tenant requests and makes the services untestable in isolation. <zstack-qid:plan-eng-arch-global-cache>",
    "options": [
  ...
  test('setup IDs plus opposed answered choices survive header and question-body variants', () => {
    for (const index of [0, 1]) {
      ...
      for (const header of ['Learnings scope', 'Scope complexity', 'Architecture', '']) {
        ...
        q.header = header;
        q.question = `Choose the setup scope. <zstack-qid:${id}>`;
        q.options.reverse();
  ...
  test('only successful answered setup metadata can exclude a call', () => {
    for (const index of [0, 1]) {
      ...
      for (const alter of [
        ...
        (call: any) => { call.questions[0].question = 'Review cache isolation. <zstack-qid:plan-eng-cache-complexity>'; },
  ...
  test('the optional predicate leaves other callers and substantive Eng qids unchanged', () => {
    ...
    fp.promptSnippet += ' <zstack-qid:plan-eng-review-global-cache>';
    expect(engStep0Boundary(fp)).toBe(true);
    expect(planCountQuestionPhase(fp, true, engStep0Boundary, engFirstReviewAUQ, engSetupAUQ).preReview).toBe(false);
...
describe('native Eng first packet and registry identities', () => {
  const scope = {
    header: 'Scope complexity',
    question: 'D1 — Choose the whole-plan scope. <zstack-qid:plan-eng-review-scope-reduce>',
    options: [{ label: 'Reduce: merge two classes (Recommended)' }, { label: 'Proceed as-is: four classes' }],
  };
  const finding = {
    header: 'Arch: shared state',
    question: 'D3 — Architecture Issue 1: AuthCache is a global mutable singleton exported at module level. <zstack-qid:plan-eng-arch-global-cache>',
  ...
  test('registry setup qids stay setup and require opposed scope actions', () => {
    ...
    const learnings = {
      header: 'Learnings scope', question: 'Choose local learning scope. <zstack-qid:preamble-cross-project-learnings>',
  ...
  test('only positive substantive native question identity starts review', () => {
    ...
    for (const qid of ['plan-eng-arch-focus', 'plan-eng-test-focus', 'plan-eng-quality-mode', 'plan-eng-perf-next-steps']) {
      const q = { ...finding, header: 'Review setup', question: `Choose which issue to review first. <zstack-qid:${qid}>` };
      expect(engFirstReviewAUQ(nativePlanCallFingerprint(callWith([q], { [q.question]: q.options[0]!.label }), 0, true))).toBe(false);
    }
    const testScope = { ...finding, header: 'Test scope', question: 'D3 — Test Issue: no coverage of the rewritten legacy flow is specified. <zstack-qid:plan-eng-test-scope>' };
    ...
    const q = { ...finding, header: 'Architecture', question: 'Choose a review focus. <zstack-qid:plan-eng-review-arch-finding>' };
    expect(engFirstReviewAUQ(nativePlanCallFingerprint(callWith([q], { [q.question]: q.options[0]!.label }), 0, true))).toBe(false);
  });
});
...
describe('native Eng setup semantics (captured G)', () => {
  ...
  {
    "header": "Cross-project",
    "question": "zstack can search learnings from your other projects on this machine to find patterns that might apply to this auth refactor review. This stays local \u2014 no data leaves your machine. Enable cross-project learnings? <zstack-qid:cross-project-learnings>",
    "options": [
      "Enable (recommended)",
      "Project-scoped only"
    ],
  ...
  {
    "header": "Scope",
    "question": "D1 \u2014 Scope challenge: this plan touches 12 files and introduces 4 new classes. Reduce scope or proceed as-is? <zstack-qid:plan-eng-scope-challenge>",
    "options": [
  ...
  {
    "header": "Code quality",
    "question": "D4 \u2014 Code quality issue 1: validateAndDispatch() is 60 lines with three nested try/catch blocks, each silently swallowing a different error class. Swallowed errors mean silent failures in production \u2014 a token validation error looks identical to a dispatch error from the outside. Fix approach? <zstack-qid:plan-eng-cq-error-swallowing>",
    "options": [
  ...
  {
    "header": "Tests",
    "question": "D5 \u2014 Test issue: all 13 new code paths in AuthCache and AuthBroker are untested (0% coverage planned). The plan mentions unit and integration coverage for success/error paths, but specifics are absent. Add explicit test requirements to the plan now? <zstack-qid:plan-eng-test-coverage>",
    "options": [
  ...
  {
    "header": "Performance",
    "question": "D6 \u2014 Performance issue: token validation makes 5 sequential IDP API calls. The plan notes they are independent and could be parallelized via Promise.all trivially. Fix in this PR or defer? <zstack-qid:plan-eng-perf-idp-parallelization>",
    "options": [
  ...
  {
    "header": "TODO: PR 2",
    "question": "D7 \u2014 TODO: capture PR 2 scope (SessionMint, TokenStore, RequestPolicy) in TODOS.md so it doesn\u2019t get lost after PR 1 ships. Add it? <zstack-qid:plan-eng-todo-pr2-scope>",
    "options": [
      "Add to TODOS.md (recommended)",
      "Skip \u2014 not valuable enough"
    ],
  ...
  },
  {
    ...
    "question": "D8 \u2014 TODO: IDP circuit breaker. Token validation makes 5 IDP calls (now parallelized). If the IDP is degraded, all 5 fail together \u2014 no retry, no fallback, no circuit breaker in the plan. Add a TODO to add circuit breaker / exponential retry logic around IDP calls? <zstack-qid:plan-eng-todo-idp-circuit-breaker>",
    "options": [
      "Add to TODOS.md (recommended)",
  ...
  test('whole-plan scope uses its premise and opposed actions, not a model-chosen qid or header', () => {
    for (const question of [
      ...
      'The plan introduces 4 new services. Reduce the scope or proceed as-is? <zstack-qid:plan-eng-unfamiliar-size-check>',
      'Complexity check: the plan spans 12 files. Reduce scope or proceed as-is? <zstack-qid:another-scope-name>',
    ]) {
  ...
  test('abbreviated enable requires explicit cross-project learnings and an opposed project-scoped action', () => {
    for (const question of [
      'Should zstack search learnings from your other projects on this machine?',
      'Enable cross-project learnings for this review? <zstack-qid:model-selected-learning-scope>',
    ]) {
    ...
    for (const question of ['Enable the new feature?', 'Choose setup. <zstack-qid:cross-project-learnings>']) {
...
describe('native question identity outranks permission wording', () => {
  const question = {
    header: 'File policy',
    question: 'D1 — Should we create a file that documents always allow access to the project? <zstack-qid:plan-eng-review-file-policy>',
    options: [{ label: 'Create it' }, { label: 'Keep current policy' }],
  };
...
describe('native file/class complexity gate classification', () => {
  const rows = [
  {
    ...
    "header": "Prerequisite",
    "question": "D1 — No design doc found for this branch. Run /office-hours first? <zstack-qid:plan-eng-review-office-hours-prereq>",
    "labels": [
  ...
  },
  {
    ...
    "header": "Scope",
    "question": "D2 — Complexity check triggered: 12 files, 4+ new classes. Reduce scope or proceed as-is? <zstack-qid:plan-eng-review-complexity-check>",
    "labels": [
      "Proceed as-is",
  ...
  },
  {
    ...
    "header": "Learnings",
    "question": "D3 — Cross-project learnings: search past sessions from other projects on this machine? <zstack-qid:plan-eng-review-cross-project-learnings>",
    "labels": [
      "Enable cross-project (Recommended)",
      "Project-scoped only"
    ],
  ...
  },
  {
    ...
    "header": "Architecture",
    "question": "D4 — Architecture A1: Two services mutate a shared AuthCache with no mutation serialization. How should this be resolved? <zstack-qid:plan-eng-review-arch-shared-mutable-cache>",
    "labels": [
  ...
  },
  {
    ...
    "header": "Architecture",
    "question": "D5 — Architecture A2: legacyAuthFlow() gets rewritten with no regression tests capturing prior behavior. Approach? <zstack-qid:plan-eng-review-arch-legacy-rewrite>",
    "labels": [
  ...
  },
  {
    ...
    "header": "Code Quality",
    "question": "D6 — Code Quality CQ1: validateAndDispatch() has 3 nested try/catch blocks that each swallow a different error class. Fix? <zstack-qid:plan-eng-review-cq-validate-dispatch>",
    "labels": [
  ...
  },
  {
    ...
    "header": "Code Quality",
    "question": "D7 — Code Quality CQ2: AuthCache shared via module-level export (implicit global). Switch to dependency injection? <zstack-qid:plan-eng-review-cq-module-export>",
    "labels": [
      "Dependency injection (Recommended)",
      "Keep module export"
    ],
  ...
  },
  {
    ...
    "header": "Tests",
    "question": "D8 — Tests T1: No described test for coordinator ordering under concurrent mutations. Add concurrency tests to the plan? <zstack-qid:plan-eng-review-test-concurrency>",
    "labels": [
  ...
  },
  {
    ...
    "header": "Tests",
    "question": "D9 — Tests T2: No E2E tests planned for core auth flows (cross-tenant validation, suspension, logout, revocation). Add them? <zstack-qid:plan-eng-review-test-e2e-auth>",
    "labels": [
  ...
  },
  {
    ...
    "header": "Performance",
    "question": "D10 — Performance P1: 5 sequential IDP calls per token validation. Parallelize or eliminate? <zstack-qid:plan-eng-review-perf-idp-calls>",
    "labels": [
 ...
 const calls = [
  {
    ...
    "questions": [
      {
        "question": "D1 — The plan's scope (12 files, 4 new classes) triggers the complexity smell check. Proceed as-is or reduce scope first? <zstack-qid:plan-eng-complexity-check>",
        "header": "Scope",
        "multiSelect": false,
        "options": [
          {
    ...
    "failed": false,
    "answers": {
      "D1 — The plan's scope (12 files, 4 new classes) triggers the complexity smell check. Proceed as-is or reduce scope first? <zstack-qid:plan-eng-complexity-check>": "Proceed as-is (recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D2 — Architecture: shared global mutable AuthCache between AuthBroker and SessionMint, with no serialization, in a multi-tenant system. <zstack-qid:plan-eng-arch-shared-cache>",
        "header": "Architecture",
        "multiSelect": false,
        "options": [
          {
    ...
    "failed": false,
    "answers": {
      "D2 — Architecture: shared global mutable AuthCache between AuthBroker and SessionMint, with no serialization, in a multi-tenant system. <zstack-qid:plan-eng-arch-shared-cache>": "Inject via constructor (recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D3 — Architecture: the plan describes 5 IDP API calls for token validation but says nothing about partial failure semantics. <zstack-qid:plan-eng-arch-idp-partial-failure>",
        "header": "Architecture",
        "multiSelect": false,
        "options": [
          {
    ...
    "failed": false,
    "answers": {
      "D3 — Architecture: the plan describes 5 IDP API calls for token validation but says nothing about partial failure semantics. <zstack-qid:plan-eng-arch-idp-partial-failure>": "Add explicit failure semantics to the plan (recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D4 — Architecture: TokenStore is listed as one of 4 new classes but the plan never describes its responsibility. The existing cache adapter already stores tokens keyed by tenant/issuer/audience/policy. <zstack-qid:plan-eng-arch-tokenstore-purpose>",
        "header": "Architecture",
        "multiSelect": false,
        "options": [
          {
    ...
    "failed": false,
    "answers": {
      "D4 — Architecture: TokenStore is listed as one of 4 new classes but the plan never describes its responsibility. The existing cache adapter already stores tokens keyed by tenant/issuer/audience/policy. <zstack-qid:plan-eng-arch-tokenstore-purpose>": "Define TokenStore's boundary in the plan (recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D5 — Code Quality: validateAndDispatch() is 60 lines with 3 nested try/catch blocks, each swallowing a different error class. <zstack-qid:plan-eng-cq-validate-dispatch>",
        "header": "Code Quality",
        "multiSelect": false,
        "options": [
          {
    ...
    "failed": false,
    "answers": {
      "D5 — Code Quality: validateAndDispatch() is 60 lines with 3 nested try/catch blocks, each swallowing a different error class. <zstack-qid:plan-eng-cq-validate-dispatch>": "Refactor as part of this PR (recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D6 — Tests: IDP partial failure (calls 1-4 of 5 succeed, call 5 fails) has no test coverage in the plan. We just added the fail-closed semantic in D3. <zstack-qid:plan-eng-test-idp-partial>",
        "header": "Tests",
        "multiSelect": false,
        "options": [
          {
    ...
    "failed": false,
    "answers": {
      "D6 — Tests: IDP partial failure (calls 1-4 of 5 succeed, call 5 fails) has no test coverage in the plan. We just added the fail-closed semantic in D3. <zstack-qid:plan-eng-test-idp-partial>": "Add to plan as required test (recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D8 — Performance: token validation makes 5 sequential IDP API calls that the plan itself identifies as trivially parallelizable. <zstack-qid:plan-eng-perf-idp-parallel>",
        "header": "Performance",
        "multiSelect": false,
        "options": [
          {
    ...
    "failed": false,
    "answers": {
      "D8 — Performance: token validation makes 5 sequential IDP API calls that the plan itself identifies as trivially parallelizable. <zstack-qid:plan-eng-perf-idp-parallel>": "Parallelize as part of this PR (recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D9 — TODOS: the plan has no mention of IDP circuit breaker or timeout per call. With 5 calls now running in parallel (D8 decision), an IDP outage generates 5 concurrent timeouts per request. <zstack-qid:plan-eng-todo-idp-circuit-breaker>",
        "header": "TODO",
        "multiSelect": false,
        "options": [
          {
    ...
    "failed": false,
    "answers": {
      "D9 — TODOS: the plan has no mention of IDP circuit breaker or timeout per call. With 5 calls now running in parallel (D8 decision), an IDP outage generates 5 concurrent timeouts per request. <zstack-qid:plan-eng-todo-idp-circuit-breaker>": "Add to TODOS.md (recommended)"
    },
    "unansweredQuestionIndices": [],
...
describe('explicit Step 0 complexity gate with size in native choices', () => {
  const calls = [
  {
    ...
    "questions": [
      {
        "question": "D1 — Add zstack skill routing rules to CLAUDE.md? <zstack-qid:routing-injection>",
    ...
    "failed": false,
    "answers": {
      "D1 — Add zstack skill routing rules to CLAUDE.md? <zstack-qid:routing-injection>": "Add routing rules (recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D2 — Should zstack search learnings from your other projects on this machine? <zstack-qid:cross-project-learnings>",
        "header": "Learnings scope",
        "multiSelect": false,
        "options": [
          {
    ...
    "failed": false,
    "answers": {
      "D2 — Should zstack search learnings from your other projects on this machine? <zstack-qid:cross-project-learnings>": "Enable cross-project learnings (recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D3 — Step 0 Scope Challenge: This plan triggers the complexity gate. Proceed at full scope or reduce? <zstack-qid:plan-eng-step0-complexity>",
        "header": "Step 0 scope",
        "multiSelect": false,
        "options": [
          {
    ...
    "failed": false,
    "answers": {
      "D3 — Step 0 Scope Challenge: This plan triggers the complexity gate. Proceed at full scope or reduce? <zstack-qid:plan-eng-step0-complexity>": "Proceed at full scope (recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D4 — Architecture A1: Global mutable AuthCache shared via module-level export, no mutation serialization. How should this be fixed? <zstack-qid:plan-eng-arch-global-cache>",
    ...
    "failed": false,
    "answers": {
      "D4 — Architecture A1: Global mutable AuthCache shared via module-level export, no mutation serialization. How should this be fixed? <zstack-qid:plan-eng-arch-global-cache>": "Inject AuthCache as constructor dependency (recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D5 — Architecture A2: Cache adapter failure posture is unstated. What should happen when the cache adapter is unavailable? <zstack-qid:plan-eng-arch-cache-failure>",
    ...
    "failed": false,
    "answers": {
      "D5 — Architecture A2: Cache adapter failure posture is unstated. What should happen when the cache adapter is unavailable? <zstack-qid:plan-eng-arch-cache-failure>": "Fail closed (recommended) — state this explicitly in the plan"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D6 — Code Quality Q1: validateAndDispatch() — 60 lines, 3 nested try/catch blocks each swallowing a different error class. How do you want to address this? <zstack-qid:plan-eng-cq-validate-dispatch>",
    ...
    "failed": false,
    "answers": {
      "D6 — Code Quality Q1: validateAndDispatch() — 60 lines, 3 nested try/catch blocks each swallowing a different error class. How do you want to address this? <zstack-qid:plan-eng-cq-validate-dispatch>": "Refactor — split function + typed error returns (recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D7 — Tests T1: The plan says success/error paths will be covered, but doesn't include a test for the fail-closed cache behavior (cache adapter unavailable → auth rejected). Add it? <zstack-qid:plan-eng-test-fail-closed>",
    ...
    "failed": false,
    "answers": {
      "D7 — Tests T1: The plan says success/error paths will be covered, but doesn't include a test for the fail-closed cache behavior (cache adapter unavailable → auth rejected). Add it? <zstack-qid:plan-eng-test-fail-closed>": "Add explicit fail-closed test (recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D8 — Tests T2: Concurrent mint + revoke for the same tenant has no planned test. AuthBroker and SessionMint both write to the shared AuthCache instance. Add a concurrency test? <zstack-qid:plan-eng-test-concurrent>",
    ...
    "failed": false,
    "answers": {
      "D8 — Tests T2: Concurrent mint + revoke for the same tenant has no planned test. AuthBroker and SessionMint both write to the shared AuthCache instance. Add a concurrency test? <zstack-qid:plan-eng-test-concurrent>": "Add concurrent access test (recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D9 — Performance P1: 5 sequential IDP calls during token validation — the plan identifies this as trivially parallelizable. Address it in this PR? <zstack-qid:plan-eng-perf-idp-calls>",
        "header": "Perf: IDP calls",
        "multiSelect": false,
        "options": [
          {
    ...
    "failed": false,
    "answers": {
      "D9 — Performance P1: 5 sequential IDP calls during token validation — the plan identifies this as trivially parallelizable. Address it in this PR? <zstack-qid:plan-eng-perf-idp-calls>": "Parallelize in this PR with Promise.all (recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D10 — TODO: Add p99 latency metric for IDP calls before/after Promise.all parallelization. Add to TODOS.md? <zstack-qid:plan-eng-todo-idp-metrics>",
    ...
    "failed": false,
    "answers": {
      "D10 — TODO: Add p99 latency metric for IDP calls before/after Promise.all parallelization. Add to TODOS.md? <zstack-qid:plan-eng-todo-idp-metrics>": "Add to TODOS.md (recommended)"
    },
    "unansweredQuestionIndices": [],

...
describe('runBin', () => {
  ...
  test('zstackHome sets both ZSTACK_HOME and ZSTACK_STATE_DIR (config precedence)', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'run-bin-'));
    try {
      const r = runBin('sh', ['-c', 'printf "%s|%s" "$ZSTACK_HOME" "$ZSTACK_STATE_DIR"'], { zstackHome: dir });

...
export function hasDisabledOutsideReview(output: string): boolean {
  const headings = [...output.matchAll(/^## ZSTACK REVIEW REPORT[ \t]*\r?$/gm)];
...
export async function captureSectionReads(opts: {
  ...
  // never mutate the operator's or another capture's zstack configuration.
  // Keep the model-facing config path relative to the fixture's working directory.
  const stateDir = opts.nativeReviewOnly
    ? fs.mkdtempSync(path.join(path.resolve(opts.planDir), '.zstack-section-state-')) : null;
  const nativeReviewRule = stateDir
    ? `\n- Read ${path.relative(path.resolve(opts.planDir), path.join(stateDir, 'config.yaml'))}, the isolated zstack configuration for this capture. It sets codex_reviews: disabled. Follow that documented control: skip the entire extra outside-review step, including its native fallback, and report outside coverage as disabled. Complete all native review sections and the full required report.`
    : '';
...
- After all required writes are complete, return a brief completion message and STOP. Do not reproduce the full report in the final response.`;
  ...
  try {
    ...
    result = await runSkillTest({
      ...
      ...(stateDir ? { env: { ZSTACK_HOME: stateDir, ZSTACK_STATE_ROOT: stateDir } } : {}),
    });
  } finally {
...
Ask the user through the AskUserQuestion tool and wait for their answer.`;
  ...
  try {
    ...
    ownedRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-mode-auq-'));
    const configDir = path.join(ownedRoot, '.claude'), stateDir = path.join(ownedRoot, 'zstack-home');
    ...
    try {
      const result = await runAgentSdkTest({
        ...
        env: { CLAUDE_CONFIG_DIR: configDir, ZSTACK_HOME: stateDir, ZSTACK_HEADLESS: '' },
    ...
    try {
      if (process.env.ZSTACK_EVAL_DIR || process.env.EVALS_RUN_ID || opts.runId) {
        ...
        const root = path.resolve(process.env.ZSTACK_EVAL_DIR || getProjectEvalDir(), 'native-auq', segment(runId));

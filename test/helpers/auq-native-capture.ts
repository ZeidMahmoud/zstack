...
export async function captureNativeFirstAuq(opts: NativeAuqCaptureOptions): Promise<NativeAuqCapture> {
  ...
  try {
    ...
    if (process.env.ZSTACK_EVAL_DIR || process.env.EVALS_RUN_ID || opts.runId) {
      ...
      artifactRoot = path.resolve(process.env.ZSTACK_EVAL_DIR || getProjectEvalDir(), 'native-auq', segment(runId));
    }
    ...
    ownedRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-native-auq-'));
    const configDir = path.join(ownedRoot, '.claude');
    const stateDir = path.join(ownedRoot, 'zstack-home');

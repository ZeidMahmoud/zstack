 ...
 * `ZSTACK_SIZE_BUDGET_OVERRIDE_REASON` — if the logger silently drops events,
...
// ZSTACK_HOME is scoped to this file's execution window (beforeAll/afterAll),
...
// the shard process (pinned by test/zstack-home-module-scope.test.ts).
...
describe('logBudgetOverride', () => {
  ...
  test('survives an unwritable audit path (logs warning, does not throw)', () => {
    // Point ZSTACK_HOME at a path inside a file (illegal directory location)
    const originalHome = process.env.ZSTACK_HOME;
    const bogusFile = path.join(TMP_HOME, 'not-a-dir.txt');
    fs.writeFileSync(bogusFile, 'just a file');
    process.env.ZSTACK_HOME = bogusFile;
    expect(() => logBudgetOverride({ scope: 'unwritable', reason: 'fs error path' })).not.toThrow();
    process.env.ZSTACK_HOME = originalHome;
  });
});

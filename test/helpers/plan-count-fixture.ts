...
export function ownedNativeReviewStateRoot(state: NativeReviewState, env: Record<string, string | undefined>): string {
  ...
  if (!owned || env.ZSTACK_HOME !== owned.root || env.ZSTACK_STATE_ROOT !== owned.root)
...
export function createNativeReviewState(opts: { preconfiguredReviewActor?: boolean } = {}): NativeReviewState {
  const sharedState = getHermeticDirs().zstackHome;
  const stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-native-review-state-'));
  ...
  try {
    ...
    state = { env: { ZSTACK_HOME: stateRoot, ZSTACK_STATE_ROOT: stateRoot }, cleanup };
...
export function createPlanCountFixture(prompt: string, opts: {
  ...
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-plan-count-'));

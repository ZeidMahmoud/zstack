...
function isCurrentLedgerNavigation(fp: AskUserQuestionFingerprint, plan: string,
  ...
  const ledger = section('Decision ledger'), tasks = section('Implementation Tasks'), lanes = section('Worktree parallelization strategy'), report = section('ZSTACK REVIEW REPORT');
  ...
  const maintenance = /(?:First two edits after exit|Post-exit|After exiting): ((?:append|add) (?:zstack )?routing rules to CLAUDE\.md) \((D[1-9]\d*)\) and ((?:create|write) TODOS\.md) \((D[1-9]\d*)\)\./i.exec(actions);
  if (maintenance) {
    ...
    if (!routing || !todo || routing.q.header !== 'Routing' || !/^(?:Add|Append) (?:zstack )?routing rules(?: to CLAUDE\.md)?$/i.test(routing.selected) ||
...
function isApprovedInvestigationRecap(fp: AskUserQuestionFingerprint, plan: string,
  ...
  const ledger = section(/^## Decision ledger$/i), tasks = section(/^## Implementation Tasks$/), lanes = section(/^## Worktree parallelization strategy$/i), report = section(/^## ZSTACK REVIEW REPORT$/);
...
function isApprovedMaintenanceRecap(fp: AskUserQuestionFingerprint, plan: string,
  ...
  const routing = actions.filter(a => /^(?:append|add) (?:the )?(?:zstack )?routing rules to CLAUDE\.md$/i.test(a));
  ...
  if (routingCalls.length !== 1 || routingCalls.filter(({q, selected}) => current(q.question) &&
      ...
      /^(?:Add|Append) (?:zstack )?routing rules to CLAUDE\.md$/.test(selected)).length !== 1) return false;
...
function isRecordedMaintenanceRecap(fp: AskUserQuestionFingerprint, plan: string,
  ...
  const report = section(/^## ZSTACK REVIEW REPORT$/), ledger = section(/^## Decision ledger$/), tasks = section(/^## Implementation Tasks$/), todos = section(/^## Accepted TODOs(?: \([^\n]*\))?$/);
...
function isPublishedPrerequisiteHandoff(fp: AskUserQuestionFingerprint, reviewedPlan: string): boolean {
  ...
  if (lines.length !== 7 || !/^D[1-9]\d* [—–-] Next step after this eng review\?$/.test(lines[0]!) ||
      !/^Project\/branch\/task: [\w.-]+ on [\w./-]+, reviewed plan written to zstack-test-plan-eng\.md\.$/.test(lines[1]!) ||
...
function isPublishedReadyNavigation(fp: AskUserQuestionFingerprint, reviewedPlan: string,
  ...
  const outsideCommand=/\bRun zstack-config set codex_reviews enabled and re-run \/plan-eng-review(?=[.;, ]|$)/;
  ...
  const tasks=section('Implementation Tasks'), lanes=section('Worktree parallelization strategy'), report=section('ZSTACK REVIEW REPORT');
...
function isPublishedTaskPauseNavigation(fp: AskUserQuestionFingerprint, plan: string,
  ...
  const ledger = section(/^## Decision ledger$/), tasks = section(/^## Implementation Tasks$/), graph = section(/^## Worktree parallelization strategy$/), report = section(/^## ZSTACK REVIEW REPORT$/);

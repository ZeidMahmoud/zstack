...
export function geminiProducerSystemSettings(contextFileName: string, helperLauncher: string) {
  if (!/^\.zstack-cso-context-[a-f0-9]{32}\.md$/.test(contextFileName)) throw new Error('INVALID_GEMINI_CONTEXT_FILENAME');
  const helper = csoProducerHelperLauncher({ prompt: '', workdir: '/', timeoutMs: 1, csoProducer: { stateDirectory: '/', sourceDirectory: '/source', helperLauncher, helperGeneration: path.join(path.dirname(helperLauncher), '.zstack-cso-generation'), providerCommand: { executable: '/provider', argsPrefix: [] } } });
...
export function prepareGeminiProducerState(stateDirectory: string, helperLauncher: string): GeminiProducerPaths {
  ...
  const contextFileName = `.zstack-cso-context-${randomBytes(16).toString('hex')}.md`;
...
export function geminiExecEnvironment(
  ...
  if (paths) {
    env.HOME = paths.home;
    env.ZSTACK_HOME = csoProducerHelperHome(opts)!;

...
export class ClaudeAdapter implements ProviderAdapter {
  ...
  async available(opts?: RunOpts): Promise<AvailabilityCheck> {
    // Binary on PATH (or ZSTACK_CLAUDE_BIN override). Routes through the shared
    ...
    if (!resolved) {
      return { ok: false, reason: 'claude CLI not found on PATH. Install from https://claude.ai/download or npm i -g @anthropic-ai/claude-code (or set ZSTACK_CLAUDE_BIN)' };
    }
  ...
  async run(opts: RunOpts): Promise<RunResult> {
    ...
    const resolved = producerCommand ? { command: producerCommand.executable, argsPrefix: producerCommand.argsPrefix } : resolveClaudeCommand();
    if (!resolved) {
      throw new Error('claude CLI not resolvable (set ZSTACK_CLAUDE_BIN or install)');
    }
    ...
    try {
      ...
      const out = execFileSync(resolved.command, args, {
        ...
        // Default ZSTACK_HEADLESS=1 so a benchmark run classifies as headless (an
...
export function claudeExecEnvironment(
  ...
  return {
    ...
    ...(paths ? { HOME: paths.home, ZSTACK_HOME: csoProducerHelperHome(opts)! } : {}),

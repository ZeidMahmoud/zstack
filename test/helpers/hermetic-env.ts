 ...
 * skills), ~/.zstack decision logs, and CONDUCTOR_-/CLAUDECODE-style env vars.
 ...
 *   │   AUTH_TOKEN                │
 *   │ ZSTACK_ANTHROPIC_API_KEY    │── promotedEnv() ─────► ANTHROPIC_API_KEY
 *   │ CONDUCTOR_*, CLAUDECODE,    │
 *   │ CLAUDE_*, ZSTACK_*, MCP_*,  │── dropped ───────────► ∅
 *   │ GBRAIN_*, GH_TOKEN, ...     │
 *   └─────────────────────────────┘
 *      + per-runner extraAllow (codex: OpenAI vars; gemini: Google vars)
 *      + CLAUDE_CONFIG_DIR=<runRoot>/.claude  ZSTACK_HOME=<runRoot>/zstack-home
 ...
 * (ZSTACK_HOME, CONDUCTOR_WORKSPACE_PATH, ZSTACK_HEADLESS opt-out) always
...
const DIR_PREFIX = 'zstack-hermetic-';
...
export function getHermeticDirs(): HermeticDirs {
  ...
  const zstackHome = path.join(runRoot, 'zstack-home');
  ...
  cachedDirs = { configDir, zstackHome, runRoot };
  return cachedDirs;
}
...
export function hermeticChildEnv(
  ...
  return buildHermeticEnv(
    process.env,
    { CLAUDE_CONFIG_DIR: dirs.configDir, ZSTACK_HOME: dirs.zstackHome },
    overrides,
    opts,
  );
}

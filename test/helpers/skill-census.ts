 ...
 *
 *   1. `connect-chrome/` is a directory SYMLINK to `open-zstack-browser/`.
 ...
 *      alias name `_zstack-command`, not as an authored skill.
 *
...
export interface SkillCensus {
   ...
   * plus the `_zstack-command` root alias when the root router exists.
...
export function skillCensus(root: string): SkillCensus {
  ...
  if (fs.existsSync(path.join(root, 'SKILL.md'))) {
    ...
    registrySet.add('_zstack-command');
  }

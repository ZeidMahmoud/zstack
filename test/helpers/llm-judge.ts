...
// scoped work. Override per run with ZSTACK_EVAL_MODEL_JUDGE; Haiku remains
...
export async function callJudge<T>(
  ...
  // ZSTACK_EVAL_MODEL_JUDGE > ZSTACK_EVAL_MODEL > sonnet default. The old
  // inline `ZSTACK_EVAL_MODEL_JUDGE || sonnet` silently ignored the global
  // ZSTACK_EVAL_MODEL override that every other eval call site honors.
...
Rubric:
- 5: Reason names a SPECIFIC TRADEOFF that distinguishes the chosen option from at least one alternative (e.g. "because hybrid ships V1 in zstack-only without blocking on cross-repo gbrain coordination", "because Postgres preserves ACID guarantees the workflow already depends on").
 ...
 * (ZSTACK_EVAL_MODEL_JUDGE is ignored). Cross-run comparability is the whole

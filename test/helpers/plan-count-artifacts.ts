...
export function createPlanCountSnapshotWriter(env: NodeJS.ProcessEnv = process.env):
  ...
  const runId = env.EVALS_RUN_ID || (env.ZSTACK_EVAL_DIR ? `local-${randomUUID()}` : undefined);
  return (input) => {
    if (!runId) return {};
    try {
      if (!artifactDir) {
        const segment = (text: string) => text.replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 120) || 'run';
        const root = path.resolve(env.ZSTACK_EVAL_DIR || getProjectEvalDir(), 'pty-count', segment(runId));

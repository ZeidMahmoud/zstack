...
export const CARVE_GUARDS: Record<string, CarveGuard> = {
  ...
  'plan-ceo-review': {
    ...
    minUnionBytes: 123_600, // token-reduction Phases 1-2 (v1.69.x branch): preamble bash -> bin/zstack-skill-start, onboarding -> gated emission; measured union 137,346
...
do not launch the downstream skill or open a browser.`,
  ...
  'document-release': {
    ...
    minUnionBytes: 56_700, // token-reduction Phases 1-2 (v1.69.x branch): preamble bash -> bin/zstack-skill-start, onboarding -> gated emission; measured union 63,018
  ...
  'design-consultation': {
    ...
    minUnionBytes: 65_000, // token-reduction Phases 1-2 (v1.69.x branch): preamble bash -> bin/zstack-skill-start, onboarding -> gated emission; measured union 72,252
  ...
  'land-and-deploy': {
    ...
    maxSizeRatio: 1.10, // + v1.81 Aside contract + zstack-browser fallback block; measured 1.077
  ...
  qa: {
    ...
    maxSizeRatio: 1.08, // + v1.81 Aside contract + zstack-browser fallback block; measured 1.063
  ...
  retro: {
    ...
    scenario:
      'Run the repo-scoped weekly retrospective for the last 7 days on this repo. There is no origin remote — proceed with the local branch per the guard disclosure rules. The zstack-retro-metrics script is not installed, so follow the degraded path (compute the metrics manually with git). Skip any AskUserQuestion calls — this is non-interactive. Produce the full narrative retrospective report.',
    staticInvariants: {
      mustStayInSkeleton: ['zstack-retro-metrics', '### Step 2: Compute Metrics', '### Step 13: Save Retro History'],

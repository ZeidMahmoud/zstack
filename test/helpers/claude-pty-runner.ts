...
export interface ClaudePtyOptions {
  ...
  /** Working directory. Default: process.cwd(). The repo cwd has the zstack
...
export interface ClaudePtySession {
  ...
  /** Owned HOME/.zstack created by the seeded launcher; absent for caller overrides. */
  ...
  /** The same validated root bound into the artifact hook, distinct from legacy HOME/.zstack. */
...
// ~/.zstack/analytics/pty-judge.jsonl for offline analysis.
// ────────────────────────────────────────────────────────────────────────────
...
function logPtyJudge(record: Record<string, unknown>): void {
  try {
    const dir = `${process.env.HOME}/.zstack/analytics`;
...
/**
 * Snapshot dump for postmortem debugging when ZSTACK_PTY_LOG=1.
 * Writes the last 4KB of visible TTY plus context to
 * ~/.zstack/analytics/pty-snapshots/<testName>-<elapsed>ms.txt.
 */
export function logPtySnapshot(visible: string, ctx: { testName: string; elapsedMs: number; tag?: string }): void {
  if (process.env.ZSTACK_PTY_LOG !== '1') return;
  try {
    const dir = `${process.env.HOME}/.zstack/analytics/pty-snapshots`;
    fs.mkdirSync(dir, { recursive: true });
...
export function matchesNativePlanQuestion(visible: string, call: NativePlanQuestionCall, planningDirectory?: string): boolean {
  ...
  const identity = question.question.match(/<zstack-qid:[^>]+>/i)?.[0] ?? question.question;
...
export const COMPLETION_SUMMARY_RE =
  /^[\t ]*(?:[⏺●][\t ]*)?(?:#{1,6}[\t ]*)?(?:ZSTACK[\t ]*REVIEW[\t ]*REPORT|Completion[\t ]*[Ss]ummary|Status:[\t ]*(?:clean|issues_open)|(?:\*\*)?VERDICT:)/m;
...
export function classifyPlanCountFrame(
  ...
  // such as "409 +## ZSTACK REVIEW REPORT". The anchored matcher also
...
/**
 * Result of asserting that a plan file ends with `## ZSTACK REVIEW REPORT`
...
/**
 * Assert that `## ZSTACK REVIEW REPORT` is the last `## ` heading in a plan
...
export function assertReviewReportAtBottom(
  ...
  const re = /^## ZSTACK REVIEW REPORT\s*$/m;
  const match = re.exec(content);
  if (!match) {
    return { ok: false, reason: 'no ZSTACK REVIEW REPORT section' };
  }
  ...
  if (trailingHeadings.length > 0) {
    return {
      ok: false,
      reason: 'trailing ## heading(s) after ZSTACK REVIEW REPORT',
      trailingHeadings,
    };
  }
  return { ok: true };
...
function hasCompletePlanReport(expectedPlanPath: string, minimumMtime: number, maximumMtime: number,
  ...
  try {
    ...
    const report = content.slice(content.indexOf('## ZSTACK REVIEW REPORT'));
...
function isCompletedDxHandoff(call: NativePlanQuestionCall): boolean {
  ...
  const ids = [...q.question.matchAll(/<zstack-qid:([^>]+)>/gi)];
  if ((q.question.match(/<zstack-qid/gi)?.length ?? 0) !== ids.length) return false;
  ...
  if (ids.length) {
    ...
    if (/<zstack-qid/i.test(q.question) ||
...
function manualDxTaskNavigation(call: NativePlanQuestionCall): boolean {
  ...
  if (/```|~~~|<zstack-qid/i.test(text) || (text.match(/\?/g)?.length ?? 0) !== 1 ||
...
function architecturalDxTaskNavigation(call: NativePlanQuestionCall): boolean {
  ...
  const match = /^D[1-9]\d* [—–-] Next steps: DX Review is complete \((?:10|[0-9])\/10 → (?:10|[0-9])\/10, ([1-9]\d*) P1 tasks, TTHW target achievable\)\. The ([1-9]\d*) fixes include architectural decisions \(demo CI exemption, arg order normalization\) that should go through an engineering gate\. What next\? <zstack-qid:plan-devex-next-steps>$/.exec(compact(q.question));
 ...
 * `## ZSTACK REVIEW REPORT` is the last `## ` section. Throws on
...
export function assertReportAtBottomIfPlanWritten(
  ...
  if (!verdict.ok) {
    const trailing = verdict.trailingHeadings?.length
      ...
      : '';
    throw new Error(
      `ZSTACK REVIEW REPORT contract violation in ${obs.planFile}: ${verdict.reason}${trailing}\n` +
        `--- evidence (last 2KB) ---\n${obs.evidence}`,
    );
  }
}
...
function nativeExplicitCeoFinding(fp: AskUserQuestionFingerprint, allowQuestionId = false): boolean {
  ...
  if (q.multiSelect || fp.options.length !== q.options.length ||
      ...
      (!allowQuestionId && /<zstack-qid/i.test(q.question)) ||
  ...
  if (allowQuestionId && ((q.question.match(/<zstack-qid/gi)?.length ?? 0) !== 1 ||
      !/<zstack-qid:\s*(?:plan-)?ceo-(?:review-)?[a-z0-9-]+\s*>/i.test(q.question))) return false;
  const title = q.question.split('\n')[0]!.replace(/\s*<zstack-qid:[^>]+>\s*$/i, '');
  ...
  if (allowQuestionId && sectionFindingIdentity) {
    ...
    const qid = /<zstack-qid:\s*plan-ceo-review-s([1-9]\d*)-[a-z0-9-]+\s*>/i.exec(q.question);
...
export const ceoFirstReviewAUQ: Step0BoundaryPredicate = (fp) =>
  nativeExplicitCeoFinding(fp) || (fp.nativeCall?.questions.some(q => {
    ...
    const id = /<zstack-qid:\s*(?:plan-)?ceo-(?:review-)?([a-z0-9-]+)/i.exec(q.question)?.[1];
...
function engWholePlanSetupAUQ(fp: AskUserQuestionFingerprint): boolean {
  ...
  const ids = [...q.question.matchAll(/<zstack-qid:([^>]+)>/gi)];
  if (ids.length !== 1 || (q.question.match(/<zstack-qid/gi)?.length ?? 0) !== 1 ||
      ids[0]![1] !== 'plan-eng-review-scope-challenge') return false;
  const body = q.question.replace(/\s*<zstack-qid:[^>]+>\s*$/i, '').trim().replace(/\s+/g, ' ');
...
export const engSetupAUQ: Step0BoundaryPredicate = (fp) => {
  ...
  return answered.length > 0 && answered.every(q => {
    ...
    const id = /<zstack-qid:\s*([a-z0-9-]+)\s*>/i.exec(q.question)?.[1]?.toLowerCase();
    const body = q.question.replace(/<zstack-qid:[^>]*>/gi, '').trim()
      .replace(/^D\s*\d+\s*[—–:-]\s*/i, '');
...
function engExplicitRepairAUQ(fp: AskUserQuestionFingerprint): boolean {
  ...
  const body = q.question.replace(/<zstack-qid:[^>]+>/i, '').trim().replace(/\s+/g, ' ');
...
function engDependencyBindingAUQ(fp: AskUserQuestionFingerprint): boolean {
  ...
  const body = normalize(q.question.replace(/\s*<zstack-qid:[^>]+>\s*$/i, ''));
...
function engNumberedFindingAUQ(fp: AskUserQuestionFingerprint): boolean {
  ...
  if (q.multiSelect || q.options.length < 2 || q.options.length > 4 ||
      ...
      /<zstack-qid/i.test(q.question)) return false;
...
export const engFirstReviewAUQ: Step0BoundaryPredicate = (fp) => {
  ...
  return call.questions.some(q => {
    ...
    const id = /<zstack-qid:\s*([a-z0-9-]+)\s*>/i.exec(q.question)?.[1] ?? '';
...
export const engStep0Boundary: Step0BoundaryPredicate = (fp) =>
  ...
  // tagged <zstack-qid:plan-eng-review-...> ({skill}-{slug} convention) —
  ...
  /zstack-qid:\s*(?:plan-)?eng-review-/i.test(fp.promptSnippet);
...
export const designFirstReviewAUQ: Step0BoundaryPredicate = (fp) => {
  ...
  const id = /<zstack-qid:\s*plan-design-review-([a-z0-9-]+)/i.exec(fp.promptSnippet)?.[1];
...
export async function launchClaudePty(
  ...
  if (opts.seedSkills && hermetic && !opts.env?.CLAUDE_CONFIG_DIR) {
    ...
    if (opts.env?.HOME === undefined) {
      ...
      // Runtime paths, registered skill assets, and ~/.zstack snapshots are
...
export async function runPlanSkillObservation(opts: {
  ...
  try {
    ...
    while (Date.now() < deadlineAt) {
      ...
      // visible buffer at each judge call when ZSTACK_PTY_LOG=1.
...
export interface PlanSkillCountObservation {
  /** Durable full raw/visible PTY output plus JSON observation, when EVALS_RUN_ID or ZSTACK_EVAL_DIR is set. */
  artifactDir?: string;
  artifactError?: string;
  outcome:
    | 'plan_ready'
 ...
 * extra outside review in its own zstack config: independent reviewers can
  ...
  try {
    session = await launchClaudePty({
      ...
      env: { ...opts.env, ...fixture.env,
        ...
        ...(opts.bindDesignBoardState ? { DESIGN_DAEMON_STATE_FILE: path.join(fixture.cwd, '.zstack', 'design.json') } : {}),
      },
      model: opts.model,
      seedSkills: true,
      observeScreen: true,
...
export async function runPlanSkillFloorCheck(opts: {
  ...
  try {
    ...
    while (Date.now() - start < timeoutMs) {
      ...
      // matches "ZSTACK REVIEW REPORT" anywhere in the buffer, including

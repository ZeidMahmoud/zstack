...
export function isDesignArtifactGeneration(fp: AskUserQuestionFingerprint): boolean {
  ...
  if (/^D\d+ StateTable$/.test(q.header) &&
      /^D\d+ — Add a state coverage table to the plan body for implementer reference\? <zstack-qid:plan-design-review-states-\d+>$/.test(question)) {
  ...
  if (/^D\d+ Storyboard$/.test(q.header) &&
      /^D\d+ — Add a user journey storyboard to the plan\? <zstack-qid:plan-design-review-journey-\d+>$/.test(question)) {

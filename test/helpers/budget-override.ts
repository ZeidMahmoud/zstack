 ...
 *
 * Records uses of ZSTACK_SIZE_BUDGET_OVERRIDE_REASON or
 * EVALS_BUDGET_OVERRIDE_REASON so a reviewer can see what was waived,
 * by whom, and why. Append-only JSONL at ~/.zstack/analytics/spend-overrides.jsonl.
 *
...
function getAuditPath(): string {
  const base = process.env.ZSTACK_HOME || path.join(os.homedir(), '.zstack');

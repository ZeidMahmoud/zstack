 ...
 *   #3 / D-CODEX(A) — ZSTACK_CARVE_SKILL=<name> runs only that skill's case, so
...
const only = process.env.ZSTACK_CARVE_SKILL?.trim();
    ...
    test(
      ...
      async () => {
        ...
        const planDir = setupSkillDir({
          ...
          sectionsFrom,
          fixtures,
          tmpPrefix: `zstack-${guard.skill}-secload-`,
        });
        ...
        const { readSections, reportProduced: completionMarked, reportWritten, exitReason, output } = await captureSectionReads({
          ...
          ...(guard.skill === 'plan-eng-review' ? {
            ...
            artifactCommands: `Bash may run only the canonical local review helpers \`${path.resolve(__dirname, '../../bin/zstack-review-log')}\` with the actual review JSON and \`${path.resolve(__dirname, '../../bin/zstack-review-read')}\`. Use these paths instead of their installed-root equivalents. Keep the inherited ZSTACK_HOME and ZSTACK_STATE_ROOT unchanged; they identify this capture's private state. The helpers' internal read-only git inspection is permitted. Do not run other shell commands, provider tools, git mutations, implementation, or tests. Keep the complete legacy QA/task artifacts labeled not persisted when their specified paths are outside this fixture, as the review's write policy requires.`,
          } : {}),

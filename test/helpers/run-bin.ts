...
export interface RunBinOptions {
  ...
  /**
   * Isolation shorthand: sets ZSTACK_HOME + ZSTACK_STATE_DIR (zstack-config
   * precedence is ZSTACK_HOME > ZSTACK_STATE_DIR > $HOME/.zstack, so both
   * must move to isolate from the operator's real ~/.zstack).
   */
  zstackHome?: string;
...
export function runBin(command: string, args: string[] = [], opts: RunBinOptions = {}): RunBinResult {
  ...
  if (opts.zstackHome !== undefined) {
    env.ZSTACK_HOME = opts.zstackHome;
    env.ZSTACK_STATE_DIR = opts.zstackHome;
  }

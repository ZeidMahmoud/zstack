...
export function seedDesignBoardActorProtocol(cwd: string): void {
  ...
  git(['add', 'CLAUDE.md']);
  git(['-c', 'user.name=Finding fixture', '-c', 'user.email=fixture@zstack.test', 'commit', '-m', 'Declare Design board fixture actor interface']);
...
export function createDesignReviewPicker({ cwd, deadlineAt }: { cwd: string; deadlineAt: number }): (question: NativeQuestion) => number {
  ...
  const stateFile = path.resolve(cwd, '.zstack/design.json');
  return question => {

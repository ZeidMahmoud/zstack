...
export function installOutsideReviewFixture(rendered: string, host: 'claude' | 'codex', repo: string, runtimeRoot: string): string {
  const source = host === 'claude' ? join(rendered, 'review') : join(rendered, '.agents', 'skills', 'zstack-review');
  const name = host === 'claude' ? 'review' : 'zstack-review';
  ...
  const content = (head + '\n' + section)
    .replaceAll('~/.claude/skills/zstack', runtimeRoot)
    .replaceAll('$HOME/.claude/skills/zstack', runtimeRoot)
    .replaceAll('${ZSTACK_BIN}', join(runtimeRoot, 'bin'))
    .replaceAll('$ZSTACK_BIN', join(runtimeRoot, 'bin'))
    .replaceAll('$ZSTACK_ROOT', runtimeRoot);

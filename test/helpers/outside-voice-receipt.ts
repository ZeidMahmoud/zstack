...
export function createOutsideReceiptRuntime(runtimeRoot: string, fixtureRoot: string, cwd: string) {
  ...
  for (const entry of fs.readdirSync(path.join(runtimeRoot, 'bin'))) if (entry !== 'zstack-claude-code')
    fs.symlinkSync(path.join(runtimeRoot, 'bin', entry), path.join(facade, 'bin', entry));
  const launcher = path.join(runtimeRoot, 'bin/zstack-claude-code');
  ...
  const shim = path.join(facade, 'bin/zstack-claude-code');

...
export async function createSetupGbrainSandbox(options: {
  ...
  try {
    ...
    const names = new Set([
      'claude', 'gbrain', 'zstack-gbrain-install',
      ...Array.from(originalFixture.matchAll(/~\/\.claude\/skills\/zstack\/bin\/([\w.-]+)/g), (m) => m[1]!),
    ]);
    ...
    for (const name of names) {
      ...
      const fake = ['claude', 'gbrain', 'zstack-gbrain-install'].includes(name);
    ...
    const env: Record<string, string> = {
      HOME: home, GBRAIN_HOME: home, ZSTACK_HOME: state, GBRAIN_MCP_TOKEN: token,
...
export async function runSetupGbrainAttempt(
  ...
  const queryProvider: QueryProvider = (input) => {
    ...
    configuration = {
      ...
      zstackHomeMatches: childEnv.ZSTACK_HOME === fixture.state,

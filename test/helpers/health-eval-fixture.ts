...
export function createHealthEvalFixture(dir: string, repoRoot: string): HealthEvalFixture {
  const zstackHome = path.join(dir, 'zstack-state');
  ...
  fs.copyFileSync(path.join(repoRoot, 'bin', 'zstack-slug'), path.join(dir, 'bin', 'zstack-slug'));
  fs.chmodSync(path.join(dir, 'bin', 'zstack-slug'), 0o755);
  ...
  const skill = extractSkillSections(path.join(repoRoot, 'health'), HEALTH_EVAL_SECTIONS)
    .replaceAll('~/.claude/skills/zstack/bin/zstack-slug', path.join(dir, 'bin', 'zstack-slug'))
    .replaceAll('~/.zstack', zstackHome);
  ...
  for (const project of ['partial', 'no-tools']) {
    ...
    const historyDir = path.join(zstackHome, 'projects', project);
...
`);
  ...
  return {
    dir, zstackHome, receipts, projectFiles: projectSnapshot(dir),
...
${zstackHome}/projects/<project>/health-history.jsonl; apply the skill's normal
history rules. ZSTACK_HOME already points at this isolated state directory.
...
export function healthReportingFailures(fixture: HealthEvalFixture): string[] {
  ...
  const history = read(path.join(fixture.zstackHome, 'projects', 'partial', 'health-history.jsonl'));
  ...
  check(read(path.join(fixture.zstackHome, 'projects', 'no-tools', 'health-history.jsonl')) === PRIOR_HISTORY,

...
const PROJECT_DIR = path.dirname(getProjectEvalDir()); // ~/.zstack/projects/$SLUG/
...
export async function runSkillTest(options: {
  ...
  const proc = spawn('claude', args, {
    ...
    // session context (CONDUCTOR_*, CLAUDECODE, ~/.claude config, ~/.zstack)
    // never reaches the child; EVALS_HERMETIC=0 restores the legacy env.
    // Default ZSTACK_HEADLESS=1 so eval/E2E runs classify as headless (BLOCK on an
  ...
  if (browseErrors.length > 0 || exitReason !== 'success') {
    try {
      const failureDir = runDir || path.join(workingDirectory, '.zstack', 'test-transcripts');

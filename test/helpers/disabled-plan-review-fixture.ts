...
export function installDisabledPlanReviewFixture(rendered: string, repo: string, runtimeRoot: string) {
  ...
  const instructions = generated
    .replaceAll('$HOME/.claude/skills/zstack', '$PWD/runtime')
    .replaceAll('~/.claude/skills/zstack', './runtime');
  const workflowPath = join(repo, 'OUTSIDE-PLAN.md');
  writeFileSync(workflowPath, instructions);

  const stateDir = join(repo, 'zstack-state');
  ...
  writeFileSync(join(spyDir, 'codex'), '#!/bin/sh\nprintf "codex invoked\\n" >> "$ZSTACK_DISABLED_CLI_LOG"\nexit 73\n', { mode: 0o755 });
  const env = {
    PATH: `${spyDir}${delimiter}${process.env.PATH ?? ''}`,
    CLAUDE_CONFIG_DIR: configDir,
    ZSTACK_HOME: stateDir,
    ZSTACK_STATE_ROOT: stateDir,
    ZSTACK_DISABLED_CLI_LOG: cliDispatchLog,
    ZSTACK_ACTIVE_HOST: 'claude',
    ZSTACK_PROJECT_SLUG: 'disabled-plan-fixture',
  };
  const config = spawnSync(join(runtimeRoot, 'bin/zstack-config'), ['set', 'codex_reviews', 'disabled'], {
  ...
  const logged = spawnSync(join(runtimeRoot, 'bin/zstack-review-log'), [JSON.stringify(prior)], {
    cwd: repo, env: { ...process.env, ...env }, encoding: 'utf8', timeout: 5_000,
  });
  if (logged.status !== 0) throw new Error(`Cannot seed historical review: ${logged.stderr}`);
  const slug = spawnSync(join(runtimeRoot, 'bin/zstack-slug'), [], {
    cwd: repo, env: { ...process.env, ...env }, encoding: 'utf8', timeout: 5_000,
  });
  ...
  const reviewLogPath = join(stateDir, 'projects', env.ZSTACK_PROJECT_SLUG, `${branch}-reviews.jsonl`);
...
export function disabledPlanReviewEvidence(result: {
  ...
  const outsideCommandMentions = toolCalls.filter((call: any) => call.name === 'Bash'
    && /\bcodex\s+(?:exec|review)\b|\bzstack-claude-code(?:['"])?\s+--/.test(String(call.input?.command ?? '')));
  const disabledPreflight = claudeOutsideExecutions(result.transcript).filter(call => call.succeeded
    && /zstack-config\b/.test(call.command) && /codex_reviews\b/.test(call.command)

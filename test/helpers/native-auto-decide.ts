...
function cliArgs(command: string, name: string): string[] | null {
  ...
  if (!words?.length || !/^(?:~\/|\$HOME\/|\.claude\/|\/)/.test(words[0]!) ||
      words[0]!.split('/').includes('..') || !words[0]!.endsWith(`/skills/zstack/bin/${name}`)) return null;
...
function preambleArgs(command: string): string[] | null {
  const direct = cliArgs(command, 'zstack-skill-start');
  if (direct) return direct;
  // The generated preamble's fixed fallback wrapper owns its _SS variable.
  const prefix = '_SS="$HOME/.claude/skills/zstack/bin/zstack-skill-start"\n[ -x "$_SS" ] || _SS=".claude/skills/zstack/bin/zstack-skill-start"\n"$_SS" ';
  const suffix = ' || echo "SKILL_START: unavailable — stale install; run ./setup or /zstack-upgrade (preamble degraded, continue the user\'s task)"';
...
function structuredModeDecision(transcript: PlanCountTranscript, tools: NativePublicToolEvent[],
  ...
  const starts = bash.flatMap(use => {
    ...
    const status = result.content.split('ZSTACK_INSTRUCTION_BEGIN:')[0]!;
  ...
  const checks = bash.flatMap(use => {
    ...
    const args = cliArgs(command, 'zstack-question-preference');
  ...
  const logs = bash.flatMap(use => {
    ...
    const args = cliArgs(command, 'zstack-question-log'); if (!args || args.length !== 1) return [];
...
export function findNativeAutoDecision(
  ...
  const uses = tools.filter(e => e.kind === 'use' && e.sessionId === opts.sessionId && e.name === 'Skill' &&
    [opts.skillName, `zstack:${opts.skillName}`].includes(String(e.input?.skill ?? '')) && timely(e.timestamp));

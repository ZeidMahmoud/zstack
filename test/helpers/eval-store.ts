 ...
 * ~/.zstack/projects/$SLUG/evals/{version}-{branch}-{tier}-{timestamp}.json,
...
const LEGACY_EVAL_DIR = path.join(os.homedir(), '.zstack-dev', 'evals');

/**
 * Detect project-scoped eval dir via zstack-slug.
 * Falls back to legacy ~/.zstack-dev/evals/ if slug detection fails.
 */
export function getProjectEvalDir(): string {
  try {
    // Try repo-local zstack-slug first, then global install
    const localSlug = spawnSync('bash', ['-c', '.claude/skills/zstack/bin/zstack-slug 2>/dev/null || ~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null'], {
    ...
    if (output) {
      ...
      if (slugMatch && slugMatch[1]) {
        const dir = path.join(os.homedir(), '.zstack', 'projects', slugMatch[1], 'evals');
...
/**
 * Lazy + memoized so importing this module never spawns the zstack-slug
 * subprocess. Callers that pass an explicit dir or set ZSTACK_EVAL_DIR
 ...
 * directory (the sharded paid runner's per-shard ZSTACK_EVAL_DIR layout),
 ...
 * `ZSTACK_BUDGET_RATIO` env var. New tests with no prior data are skipped.
 */
export function findBudgetRegressions(
  comparison: ComparisonResult,
  opts?: { ratioCap?: number; minPriorTools?: number; minPriorTurns?: number },
): BudgetRegression[] {
  const envRatio = Number(process.env.ZSTACK_BUDGET_RATIO);
...
export function assertNoBudgetRegression(
  ...
  const cap = opts?.ratioCap ?? (Number(process.env.ZSTACK_BUDGET_RATIO) || 2.0);
  ...
  throw new Error(
    ...
    lines.join('\n') +
    `\n(Override per run: ZSTACK_BUDGET_RATIO=<n>. ${comparison.before_file} vs ${comparison.after_file})`,
  );
}
...
// archaeology dig.
//
// ZSTACK_CLAUDE_CLI_VERSION short-circuits the spawn entirely: the paid
...
export function getClaudeCliVersion(): string {
  ...
  const fromEnv = process.env.ZSTACK_CLAUDE_CLI_VERSION;
  if (fromEnv) {
...
export class EvalCollector {
  ...
  constructor(tier: 'e2e' | 'llm-judge', evalDir?: string) {
    this.tier = tier;
    this.evalDir = evalDir || process.env.ZSTACK_EVAL_DIR || defaultEvalDir();

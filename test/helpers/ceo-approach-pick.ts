...
export function pickCeoRecommendedApproach(fp: AskUserQuestionFingerprint): number | null {
  ...
  if (ids.length !== 1 || (q.question.match(/<zstack-qid/gi)?.length ?? 0) !== 1) return null;
  ...
  const directQuestion = question.replace(/\s*<zstack-qid:[^>]+>\s*$/i, '').trim();

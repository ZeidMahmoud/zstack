...
function closedReviewNavigation(declaration: string, context: string): boolean {
  const question = declaration.replace(/<zstack-qid:[^>]+>/gi, '');
...
function metadataClosedReviewNavigation(declaration: string, context: string): boolean {
  const question = declaration.replace(/<zstack-qid:[^>]+>/gi, '').trim();
...
function sequencedReviewNavigation(fp: AskUserQuestionFingerprint): number | null {
  ...
  const grammar = [
    ...
    /^ELI10: zstack chains reviews\. The CEO review just settled scope and strategy\. The engineering review is the required gate before shipping: it checks architecture, test design, and code quality in detail\. skip_eng_review is false, so it is still required\. No UI scope was detected, so the design review does not apply here\.$/,
...
function manualHandoffIndex(fp: AskUserQuestionFingerprint): number | null {
  ...
  const ids = [...q.question.matchAll(/<zstack-qid:\s*([a-z0-9-]+)\s*>/gi)];
  if (ids.length > 1 || (q.question.match(/<zstack-qid/gi)?.length ?? 0) !== ids.length) return null;
  ...
  const questionText = declaration.replace(/<zstack-qid:[^>]+>/gi, '').trim();
...
function closedNextReviewExplanations(question: string, descriptions: string[]): boolean {
  ...
  const navigation = [
    ...
    /^zstack['’]s shipping gate is the eng review, which checks architecture and test design$/i,

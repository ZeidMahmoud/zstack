...
export function isDesignCountSetup(fp: AskUserQuestionFingerprint): boolean {
  ...
  if (q.multiSelect || !/^outside(?: design)? voices$/i.test(q.header.trim()) ||
      (q.question.match(/<zstack-qid:/g)?.length ?? 0) !== 1 ||
      !/<zstack-qid:(?:plan-design-review-outside-voices|outside-voices-design)>\s*$/.test(q.question) ||
  ...
  const remainder = q.question.slice(q.question.indexOf('?') + 1).replace(/<zstack-qid:[^>]+>\s*$/, '').trim();
...
function ordinaryDesignIssue(fp: AskUserQuestionFingerprint, scope: { primary: boolean; ownsPrimaryPremise?: boolean }): boolean {
  ...
  if (!(new RegExp(`^Issue ${issue[1]}(?:: [A-Za-z][A-Za-z0-9 _-]{0,39})?$`, 'i').test(q.header.trim()) || descriptivePrimaryHeader || ownedPrimaryHeader) ||
      /<zstack-qid:/i.test(q.question) || q.multiSelect ||
...
function designSystemChoiceIssue(fp: AskUserQuestionFingerprint): boolean {
  ...
  if (gapIssue && (() => {
    ...
    if (!q.header.trim() || (headerIssue && headerIssue[1] !== issueNumber) ||
        ...
        /<zstack-qid:/i.test(q.question) || q.multiSelect || q.options.length < 2 || q.options.length > 4 ||
        fp.options.length !== q.options.length || !fp.options.every((o, i) => o.index === i + 1 && o.label === q.options[i]!.label) ||
        !q.options.some(o => o.label === call.answers?.[q.question])) return false;
  ...
  if (!issue || q.header.trim() !== `Issue ${issue[1]}` || lines.length !== 7 ||
      ...
      /<zstack-qid:|```|^ELI10: (?:Example|Hypothetical|Quoted)\b/im.test(q.question) || q.multiSelect ||
...
function compactPrimaryDecision(fp: AskUserQuestionFingerprint): boolean {
  ...
  if (q.multiSelect || q.options.length !== 2 || new Set(q.options.map(o => o.label)).size !== 2 ||
      fp.options.length !== 2 || !fp.options.every((o, i) => o.index === i + 1 && o.label === q.options[i]!.label) ||
      !q.options.some(o => o.label === call.answers?.[q.question]) || /<zstack-qid:/i.test(q.question)) return false;
...
export function isDesignCountFirstReview(fp: AskUserQuestionFingerprint): boolean {
  ...
  return call.questions.some(q => {
    ...
    if (call.answered === true && call.failed === false && /^Pass\s*[1-7]\s*\([^)]*\)\s*[—–:]\s*Finding\s*[1-9]\d*:\s+\S/i.test(q.question.trim()) &&
        ...
        (q.question.match(/<zstack-qid/gi)?.length ?? 0) === 1 &&
        /\b(?:Apply|Add|Fix|Specify|Define|Restore)\b[^?\n]*\b(?:to|in) the plan\?\s*<zstack-qid:[^>]+>\s*$/i.test(q.question) &&
    ...
    if ((scoredPass || namedPass) && /^plan-design-review-[a-z0-9-]+$/i.test(id) &&
        ...
        /\bShould I (?:add|fix|specify|define|restore)\b[^?]+\?\s*<zstack-qid:[^>]+>\s*$/i.test(q.question) &&
...
function closedDesignGateRecap(tail: string, descriptions: string[]): boolean {
  const navigation = /\bWhat(?:['’]s)?\s+next\?\s*<zstack-qid:[a-z0-9-]+>\s*$/i.exec(tail);
...
function designHandoff(fp: AskUserQuestionFingerprint): { manualIndex: number | null } | null {
  ...
  const ids = [...q.question.matchAll(/<zstack-qid:\s*([a-z0-9-]+)\s*>/gi)].map(match => match[1]);
  if ((q.question.match(/<zstack-qid\b/gi) ?? []).length !== 1 || ids.length !== 1 || !/^plan-design-(?:review-)?next-steps?$/i.test(ids[0]!)) return null;
  ...
  if (!completed) return null;
  const requiredGateOffer = /^The required next gate is Eng(?:ineering)? Review\s*[—–-]\s*want me to run it now\?\s*<zstack-qid:[a-z0-9-]+>\s*$/i.test(declaration.slice(completed[0].length).trim());
  ...
  const requiredGateQuestion = requiredGateOffer || closedRecap || /^(?:\d+ implementation tasks ready\.\s*)?Eng(?:ineering)? Review is the required shipping gate\.\s*What next\?\s*<zstack-qid:[a-z0-9-]+>\s*$/i.test(declaration.slice(completed[0].length).trim());
  ...
  const describedRequiredGate = /^What['’]s\s+next\?\s*<zstack-qid:[a-z0-9-]+>\s*$/i.test(declaration.slice(completed[0].length).trim()) &&
  ...
  if (!requiredGateQuestion && !/\bWhat['’]s\s+next\?\s*<zstack-qid:[a-z0-9-]+>\s*$/i.test(declaration)) return null;

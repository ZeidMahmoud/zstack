 ...
 * (`~/.claude/skills/zstack/setup-gbrain/sections/*.md`) that do not exist in
...
export const SECTION_ANCHORS: Record<string, string> = {
  ...
  'transcript-gate.md': 'zstack-memory-ingest.ts --probe',
};
...
export function buildSetupGbrainFixture(
  ...
  if (options.helperBinDir) {
    ...
    full = full.replaceAll('~/.claude/skills/zstack/bin', quotedBin);
  }
  return full;
}

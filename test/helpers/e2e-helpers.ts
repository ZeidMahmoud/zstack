...
// NOTE: since zstack-skill-start honors ZSTACK_HOME (EOV7), hermetic children read the
// temp ZSTACK_HOME that hermetic-env.ts seeds (the canonical marker list lives there);
// this operator-HOME seeding only serves EVALS_HERMETIC=0 debug runs.
if (evalsEnabled) {
  const zstackDir = path.join(os.homedir(), '.zstack');
  fs.mkdirSync(zstackDir, { recursive: true });
  // Marker list kept at parity with hermetic-env.ts's child-ZSTACK_HOME seed

...
function clippedRoutingTitle(visible: string, question: AskUserQuestionFingerprint): string | null {
  ...
  if (!title || !/^(?:D\s*\d+\s*[—–:-]\s*)?Add\s+(?:zstack\s+)?skill\s+routing\s+rules\s+to\s+CLAUDE\.md\?\s*<zstack-qid:routing-injection>$/i.test(title)) return null;
  ...
  if (before.some(line => line.trim() && !/^ {0,3}[│┃](?:[\t ]|$)/.test(line)) ||
      ...
      (before.join('\n').match(/<zstack-qid/gi)?.length ?? 0) !== 1 ||
...
function routingSetupActions(question: AskUserQuestionFingerprint, allowTemporarySkip: boolean, knownSetupOffer = false) {
  ...
  const routingId = /<zstack-qid:routing-injection>/i.test(question.promptSnippet);
  const routingPremise = /zstack/i.test(prompt) && /CLAUDE\.md/i.test(prompt) && /skillroutingrules/i.test(prompt);
...
function contextualPacketSetup(question: NativePlanQuestion, pending: NativePlanQuestionCall) {
  ...
  const ids = [...question.question.matchAll(/<zstack-qid:[a-z0-9-]+>/gi)];
  const text = question.question.replace(/<zstack-qid:[a-z0-9-]+>/gi, '').trim();
  ...
  if (/^Routing(?: rules)?$/i.test(question.header.trim()) &&
      /^Add\s+(?:zstack\s+)?(?:skill\s+)?routing\s+rules\s+to\s+CLAUDE\.md\?$/i.test(offer) &&
      (!ids.length || ids.length === 1 && ids[0]![0].toLowerCase() === '<zstack-qid:routing-injection>')) {
...
function numberedPacketSetup(question: NativePlanQuestion, pending: NativePlanQuestionCall) {
  ...
  const ids = [...question.question.matchAll(/<zstack-qid:[a-z0-9-]+>/gi)];
  const text = compact(question.question.replace(/<zstack-qid:[a-z0-9-]+>/gi, ''))
    .replace(/^D[1-9]\d*\s*[—–:-]\s*/, '');
  const fp: AskUserQuestionFingerprint = { signature: '', observedAtMs: 0, preReview: true,
    promptSnippet: `${question.header} ${question.question}`,
    options: question.options.map((option, index) => ({ index: index + 1, label: option.label })) };
  const routing = routingSetupActions(fp, true);
  if (ids.length === 1 && ids[0]![0].toLowerCase() === '<zstack-qid:routing-injection>' &&
      /^Add\s+(?:zstack\s+)?skill\s+routing\s+rules\s+to\s+CLAUDE\.md\?$/i.test(text) &&
...
function setupPacketDecision(visible: string, seen: ReadonlySet<string>, pending: NativePlanQuestionCall): AutoplanSetupDecision {
  ...
  const policies = pending.questions.map(question => {
    ...
    const ids = [...question.question.matchAll(/<zstack-qid:[a-z0-9-]+>/gi)];
    if (ids.length > 1 || (question.question.match(/<zstack-qid/gi)?.length ?? 0) !== ids.length) return null;
    const offerText = question.question.replace(/<zstack-qid:[a-z0-9-]+>/gi, '').trim();
    const fp: AskUserQuestionFingerprint = { signature: '', observedAtMs: 0, preReview: true,
      promptSnippet: `${question.header} ${question.question}`,
      options: question.options.map((option, index) => ({ index: index + 1, label: option.label })) };
    const routing = routingSetupActions(fp, true);
    ...
    const routingOffer = /^(?:zstack\s+works\s+best\s+when\s+(?:your|this|the)\s+project['’]s\s+CLAUDE\.md\s+includes\s+skill\s+routing\s+rules\.\s*)?(?:(?:Should|Can)\s+(?:I|zstack)\s+|Would\s+you\s+like\s+(?:me|zstack)\s+to\s+)?Add\s+(?:(?:zstack\s+)?skill\s+routing\s+rules\s+to\s+(?:this\s+project['’]s\s+)?CLAUDE\.md|(?:skill\s+)?routing\s+rules(?:\s+to\s+CLAUDE\.md)?|them)(?:\s+now)?\?$/i.test(offerText);
...
export function autoplanSetupDecision(visible: string, seen: ReadonlySet<string>, pending?: NativePlanQuestionCall): AutoplanSetupDecision {
  ...
  if (prerequisitePrompt) {
    ...
    if (prerequisite === null) {
      ...
      const offer = /\brun\s*\/office-hours\s*(?:now|first)\s*\?(?:\s*<zstack-qid:[^>]+>)?\s*$/i.test(beforeOptions);

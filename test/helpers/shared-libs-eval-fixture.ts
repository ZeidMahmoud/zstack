...
export function createSharedLibsFixture(label: string): SharedLibsFixture {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `zstack-shared-${label}-`));
...
export function sharedReadOnlyViolations(toolCalls: Array<{ tool: string; input: any }>, requests: SourceRequest[] = []): string[] {
  ...
  for (const call of toolCalls) {
    ...
    if (/\bzstack-(?:review-read|wtree|skill-start|learnings-log)\b/.test(command)) violations.push('stateful zstack helper');
...
export function standaloneInstructions(f: SharedLibsFixture, codex = false): string {
  const source = codex ? path.join(SHARED_LIBS_ROOT, '.agents/skills/zstack-deslop-shared-libs') : path.join(SHARED_LIBS_ROOT, 'deslop-shared-libs');
...
The installed zstack helpers under ${SHARED_LIBS_ROOT}/bin and ${SHARED_LIBS_ROOT}/lib, plus the provider wrappers under ${f.bin}, are trusted harness infrastructure. Invoke their required interfaces; auditing their implementation or the fixture request logs is outside the target review. Still inspect target repository source, Git configuration and attributes, actual snapshot coverage, and prior/final persisted review records as the workflow requires.
...
- After final verification, combine successful --finish persistence and one complete, untruncated read-back through zstack-review-read in the same tool invocation. Read back only after persistence succeeds, inspect the full current record and binding, then return the final review summary in conversation.
...
export async function seedSkippedAdvisory(f: SharedLibsFixture): Promise<any> {
  ...
  const log = path.join(SHARED_LIBS_ROOT, 'bin/zstack-review-log');
  const env = { ...process.env, ...f.env, PATH: process.env.PATH, ZSTACK_HOME: f.state };
...
export function fixtureWorkingTree(f: SharedLibsFixture): string {
  return execFileSync(path.join(SHARED_LIBS_ROOT, 'bin/zstack-wtree'), [], {

...
function decisionGaps(q: NativePlanQuestion): DevexSeededGap[] {
  ...
  if (declaration || upgradeTransition || explainedSubject || evidenceJourney) {
    ...
    if (explainedSubject) {
      ...
      const marker = 'ZSTACK_OWNED_AUTH_LITERAL';
      ...
      if (explainedAuthentication) {
        const index = sentences.findIndex(sentence => /ZSTACK_OWNED_AUTH_LITERAL/.test(sentence));
        const fact = sentences[index] ?? '', preceding = sentences[index - 1] ?? '';
        const event = /^(?:(?:If|When) (.+),\s*)?(?:(?:the )?SDK (?:raises|throws)|they (?:get|receive)) ZSTACK_OWNED_AUTH_LITERAL(?:\s*\([\w./-]+\.md:\d+(?:[-–]\d+)?\))?\.?$/i.exec(fact);

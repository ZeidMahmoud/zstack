...
export function assertEngTerminalReport(plan: string): void {
  ...
  const heads = tokens.flatMap((token, i) => token.type === 'heading' && token.depth === 2 && token.text === 'ZSTACK REVIEW REPORT' ? [i] : []);
...
function recordedBatchingIssue(call: NativePlanQuestionCall, savedPlan: string): string | undefined {
  ...
  const end = tokens.findIndex((token, at) => at > start && token.type === 'heading' && /^ZSTACK REVIEW REPORT$/i.test(clean(token.text)));

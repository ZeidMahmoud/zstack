...
const CONTAMINATED: NodeJS.ProcessEnv = {
  ...
  ZSTACK_HOME: '/Users/op/.zstack',
  ZSTACK_HEADLESS_DEFAULT: 'x',
  MCP_TIMEOUT: '5000',
...
const HERMETIC_VARS = { CLAUDE_CONFIG_DIR: '/x/.claude', ZSTACK_HOME: '/x/zstack-home' };
...
describe('buildHermeticEnv allowlist', () => {
  ...
  test('drops session-context and operator-credential vars', () => {
    for (const k of [
      ...
      'CLAUDE_CODE_ENTRYPOINT', 'ZSTACK_HEADLESS_DEFAULT', 'MCP_TIMEOUT',
  ...
  test('redirects CLAUDE_CONFIG_DIR and ZSTACK_HOME to hermetic values', () => {
    expect(env.CLAUDE_CONFIG_DIR).toBe('/x/.claude');
    expect(env.ZSTACK_HOME).toBe('/x/zstack-home');
  });
  ...
  test('overrides merge last — per-test re-contamination is deliberate', () => {
    const e = buildHermeticEnv(CONTAMINATED, HERMETIC_VARS, {
      ...
      ZSTACK_HOME: '/tmp/test-home',
      ZSTACK_HEADLESS: '',
    });
    expect(e.CONDUCTOR_WORKSPACE_PATH).toBe('/tmp/test-ws');
    expect(e.ZSTACK_HOME).toBe('/tmp/test-home');
    expect(e.ZSTACK_HEADLESS).toBe('');
  });

  test('promotes ZSTACK_ANTHROPIC_API_KEY when canonical absent (shared shim fn)', () => {
    const base = { ...CONTAMINATED } as NodeJS.ProcessEnv;
    delete base.ANTHROPIC_API_KEY;
    base.ZSTACK_ANTHROPIC_API_KEY = 'sk-ant-promoted-9876543210';
    const e = buildHermeticEnv(base, HERMETIC_VARS);
    expect(e.ANTHROPIC_API_KEY).toBe('sk-ant-promoted-9876543210');
    expect(e.ZSTACK_ANTHROPIC_API_KEY).toBeUndefined(); // ZSTACK_* still dropped
  });
...
describe('EVALS_HERMETIC=0 escape hatch', () => {
  test('returns byte-identical legacy env, overrides still last', () => {
    ...
    const e = buildHermeticEnv(base, HERMETIC_VARS, { ZSTACK_HEADLESS: '1' });
    ...
    expect(e.ZSTACK_HOME).toBe('/Users/op/.zstack');
    expect(e.ZSTACK_HEADLESS).toBe('1');
    expect(e).toEqual({ ...(base as Record<string, string>), ZSTACK_HEADLESS: '1' });
  });
...
describe('gcStaleHermeticDirs', () => {
  test('removes dead-pid dirs, keeps live-pid and foreign dirs', () => {
    ...
    const dead = path.join(tmp, `zstack-hermetic-${deadPid}-abc`);
    const live = path.join(tmp, `zstack-hermetic-${process.pid}-abc`);
    const foreign = path.join(tmp, 'unrelated-dir');
    const malformed = path.join(tmp, 'zstack-hermetic-notapid-abc');
  ...
  test('keeps a fresh dead-pid dir (PID-reuse grace window)', () => {
    ...
    const freshDead = path.join(tmp, 'zstack-hermetic-99999999-xyz');
...
describe('hermeticChildEnv composition', () => {
  test('hermetic by default: redirects config dirs, drops contamination', () => {
    ...
    const e = hermeticChildEnv({ ZSTACK_HEADLESS: '1' });
    const dirs = getHermeticDirs();
    expect(e.CLAUDE_CONFIG_DIR).toBe(dirs.configDir);
    expect(e.ZSTACK_HOME).toBe(dirs.zstackHome);
    expect(e.ZSTACK_HEADLESS).toBe('1');
...
describe('split CEO artifact Read scope', () => {
  function fixture(check: (cwd: string, env: Record<string, string>) => void): void {
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-e2e-plan-ceo-split-overflow-'));
    try {
  ...
  test('grants only this fixture\'s generated markdown Read rules', () => {
    fixture((cwd, env) => {
      const scope = path.join(getHermeticDirs().zstackHome, 'projects', path.basename(cwd), 'ceo-plans');
  ...
  test('refuses operator/foreign homes and a project-slug override', () => {
    fixture((cwd, env) => {
      for (const home of [path.join(os.homedir(), '.zstack'), path.dirname(env.ZSTACK_HOME!), env.ZSTACK_HOME! + '-other']) {
        expect(() => hermeticCeoPlanReadArgs(cwd, { ...env, ZSTACK_HOME: home })).toThrow('private split fixture');
      }
      expect(() => hermeticCeoPlanReadArgs(cwd, { ...env, ZSTACK_PROJECT_SLUG: 'another-fixture' })).toThrow('private split fixture');
    });
  });
  ...
  test('refuses a remote-derived foreign project slug', () => {
    fixture((cwd, env) => {
      ...
      expect(fs.existsSync(path.join(env.ZSTACK_HOME!, 'projects', 'foreign-repo'))).toBe(false);
    });
  });

  test('refuses another fixture kind and nested or symlinked working directories', () => {
    fixture((cwd, env) => {
      const other = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-e2e-plan-ceo-finding-'));
  ...
  test('refuses a substituted scope or project without reading its target', () => {
    fixture((cwd, env) => {
      const project = path.join(env.ZSTACK_HOME!, 'projects', path.basename(cwd));
...
describe('Design artifact Read scope', () => {
  ...
  for (const prefix of ['zstack-e2e-plan-design-', 'design-ui-project-']) {
    test(`grants only generated PNG Read for ${prefix}`, () => fixture(prefix, (cwd, env) => {
      const scope = path.join(getHermeticDirs().zstackHome, 'projects', path.basename(cwd), 'designs');
  ...
  test('refuses operator/foreign roots, slug overrides and another fixture kind', () => {
    fixture('zstack-e2e-plan-design-', (cwd, env) => {
      for (const home of [path.join(os.homedir(), '.zstack'), path.dirname(env.ZSTACK_HOME!), env.ZSTACK_HOME! + '-other']) {
        expect(() => hermeticDesignReadArgs(cwd, { ...env, ZSTACK_HOME: home })).toThrow('private Design fixture');
      }
      expect(() => hermeticDesignReadArgs(cwd, { ...env, ZSTACK_PROJECT_SLUG: 'foreign' })).toThrow('private Design fixture');
    ...
    fixture('zstack-e2e-plan-ceo-split-overflow-', (cwd, env) => {

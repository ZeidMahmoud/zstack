...
export function createCodexEvalCollector(suite: string, evalDir?: string): EvalCollector {
  const root = evalDir || process.env.ZSTACK_EVAL_DIR || (defaultEvalDir ??= getProjectEvalDir());
...
export function validateCodexDiscovery(result: CodexResult): void {
  ...
  requireCondition(/review|zstack|skill/.test(result.output.toLowerCase()), 'Codex discovery did not reference the skill');
}
...
export function createCodexPlanFormatCapture(file: string, kind: 'kind' | 'coverage') {
  let observation: { type: 'zstack_plan_format_capture'; file_path: string; content: string; session_id: string | null } | undefined;
  return {
    reset(): void {
    ...
    validate(result: CodexResult): void {
      ...
      observation = { type: 'zstack_plan_format_capture', file_path: file, content: captured, session_id: result.sessionId };

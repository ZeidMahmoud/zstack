...
test('public xterm projection restores globals and reconstructs an exact current file dialog without a DOM', async () => {
  ...
  try {
    ...
    projection.feed('\x1b[2;1HDo you want to create zstack-test-plan-ceo.md?\r\n❯ 1. Yes\r\n  2. Yes, and switch to accept edits\r\n  3. No\x1b[?2026l');
    const snapshot = await projection.snapshot();
    expect(snapshot.cols).toBe(120);
    expect(snapshot.rows).toBe(40);
    expect(snapshot.lines[0].text).toBe('Create file /tmp/fixture/zstack-test-plan-ceo.md');
    expect(snapshot.text).toContain('Do you want to create zstack-test-plan-ceo.md?\n❯ 1. Yes');

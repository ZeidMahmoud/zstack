 ...
 * Both sites also honor ZSTACK_SKIP_ASIDE=1 (the library's and the skills'
 ...
 * zstack's own browser), so the two can never drift.
...
function runBash(lines: string[], env: Record<string, string> = {}): string {
  // ZSTACK_SKIP_ASIDE is read from the environment. Strip any inherited value
  // so the outcome is decided by the test, never by the operator's shell.
  const base: Record<string, string | undefined> = { ...process.env };
  delete base.ZSTACK_SKIP_ASIDE;
...
/** `skipAside` is the value ZSTACK_SKIP_ASIDE carries in the environment;
...
function siteEnv(opts: SiteOpts): Record<string, string> {
  return opts.skipAside === undefined ? {} : { ZSTACK_SKIP_ASIDE: opts.skipAside };
}
...
function runSummary(opts: SiteOpts): string {
  return runBash([
    ...
    `ASIDE_PRESENT=${opts.aside ? 1 : 0}`,
    COMMAND_SHADOW,
    'SOURCE_ZSTACK_DIR=/nonexistent-zstack-dir', // no telemetry binary → the event is skipped
    `_PW_FAIL_REASON=${JSON.stringify(opts.reason)}`,
    summaryReasonBlock(),
...
describe('setup: _browser_hint', () => {
  ...
  test('Aside present, bootstrap fine → Aside primary with the bundled fallback', () => {
    ...
    expect(out).toContain('browser: Aside (primary) — zstack browser is the fallback');
  });
  ...
  test('Aside present, bootstrap failed → Aside primary, fallback named unavailable with the reason', () => {
    ...
    expect(out).not.toContain('zstack browser is the fallback');
  });

  test('Aside absent, bootstrap fine → bundled browser is the fallback, Aside suggested', () => {
    const out = runHint({ aside: false, reason: '' });
    expect(out).toContain('browser: zstack browser (fallback). Install Aside for the primary path: aside.com (macOS 15+)');
  });
  ...
  test('Aside absent, bootstrap failed → no browser promised; reason and both remedies named', () => {
    ...
    expect(out).not.toContain('zstack browser (fallback)');
  });

  test('static pin: the hint honors the ZSTACK_SKIP_ASIDE opt-out before probing for Aside', () => {
    expect(extractFn('_browser_hint')).toContain('[ "${ZSTACK_SKIP_ASIDE:-}" != "1" ] && command -v aside');
  });

  test('ZSTACK_SKIP_ASIDE=1 with Aside on PATH, bootstrap fine → treated as Aside absent: the fallback line, never Aside (primary)', () => {
    const out = runHint({ aside: true, reason: '', skipAside: '1' });
    expect(out).toContain('browser: zstack browser (fallback). Install Aside for the primary path: aside.com (macOS 15+)');
    expect(out).not.toContain('Aside (primary)');
  });

  test('ZSTACK_SKIP_ASIDE=1 with Aside on PATH, bootstrap failed → none available; Aside is not promised', () => {
  ...
  test('only the literal 1 opts out: ZSTACK_SKIP_ASIDE=0 or empty keeps Aside primary', () => {
    for (const v of ['0', '']) {
...
describe('setup: _browser_hint treats ZSTACK_SKIP_PLAYWRIGHT as a request, not a failure', () => {
  test('Aside absent, bootstrap skipped by request → names the flag, does not say fix the bootstrap', () => {
    ...
    expect(out).toContain('browser: none available');
    expect(out).toContain('skipped by request (ZSTACK_SKIP_PLAYWRIGHT=1)');
...
describe('setup: Chromium bootstrap summary is Aside-aware', () => {
  ...
  test('ZSTACK_SKIP_ASIDE=1 with Aside on PATH → the Aside-absent wording: the skills need the bundled browser', () => {
  ...
  test('static pin: _PW_BROWSER_SKILLS is derived from _PW_ASIDE_SKILLS (plus /pair-agent) so the two lists cannot drift', () => {
    ...
    // /pair-agent always runs on zstack's own browser, so it belongs only to
    ...
    expect(block).toContain('[ "${ZSTACK_SKIP_ASIDE:-}" != "1" ] && command -v aside');
  });

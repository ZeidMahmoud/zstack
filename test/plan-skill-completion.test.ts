...
describe('owned assistant plan review completion', () => {
  ...
  test.each(['## ZSTACK REVIEW REPORT', '## Completion Summary', '**VERDICT: APPROVED**', 'Status: clean', '**DONE**', 'DONE_WITH_CONCERNS — remaining risks'])('completed assistant %s is a valid chat terminal', marker => {
  ...
  test('Write previews and tool results cannot complete a review', () => {
    const marker = '## ZSTACK REVIEW REPORT\nVERDICT: APPROVED';
  ...
  test('quoted, fenced, indented, and future-example markers cannot complete a review', () => {
    ...
    expect(readPlanSkillCompletion(config, sessionId, 'DONE ZSTACK REVIEW REPORT VERDICT: APPROVED')).toBeNull();
  });

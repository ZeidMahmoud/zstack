 ...
 *
 * Verifies the zstack-config resolve-user-slug subcommand walks the
 ...
 * Test isolation via ZSTACK_HOME and HOME env overrides.
 *
 * Gate-tier, free, ~50ms.
 */
...
const CONFIG_BIN = join(REPO_ROOT, 'bin', 'zstack-config');
...
beforeEach(() => {
  TMP_HOME = mkdtempSync(join(tmpdir(), 'zstack-user-slug-test-'));
...
describe('endpoint-hash subcommand', () => {
  test('returns deterministic 8-char hex or literal "local"', () => {
    const result = runConfig(['endpoint-hash'], { ZSTACK_HOME: TMP_HOME });
...
describe('resolve-user-slug fallback chain', () => {
  test('uses $USER when set (layer 2)', () => {
    const result = runConfig(['resolve-user-slug'], { ZSTACK_HOME: TMP_HOME, USER: 'alice-test' });
    expect(result.status).toBe(0);
  ...
  test('falls through past empty $USER to git email or anonymous', () => {
    const result = runConfig(['resolve-user-slug'], { ZSTACK_HOME: TMP_HOME, USER: '' });
    expect(result.status).toBe(0);
  ...
  test('persists resolution to user_slug_at_<endpoint-id> on first call', () => {
    runConfig(['resolve-user-slug'], { ZSTACK_HOME: TMP_HOME, USER: 'persisttest' });
  ...
  test('subsequent calls return same slug (stable across sessions)', () => {
    const first = runConfig(['resolve-user-slug'], { ZSTACK_HOME: TMP_HOME, USER: 'stabletest' });
    const second = runConfig(['resolve-user-slug'], { ZSTACK_HOME: TMP_HOME, USER: 'changed-after' });
...
describe('sha8_of portable hash (sha256sum → shasum fallback)', () => {
  ...
  // from bin/zstack-config (no drift-prone copy) and run it under /bin/sh
  ...
  function sha8FnSource(): string {
    ...
    if (!m) throw new Error('sha8_of() not found in bin/zstack-config');
    return m[0];
  }
  ...
  function makeShimDir(): string {
    const dir = mkdtempSync(join(tmpdir(), 'zstack-sha8-shim-'));
...
describe('endpoint-hash collision escalation (sha8 → sha16)', () => {
  ...
  test('emits sha8 normally, sha16 when a stored sha16-namespaced key exists', () => {
    ...
    const plain = runConfig(['endpoint-hash'], { ZSTACK_HOME: TMP_HOME });
    ...
    const escalated = runConfig(['endpoint-hash'], { ZSTACK_HOME: TMP_HOME });
...
describe('brain_trust_policy@<endpoint-id> namespace', () => {
  test('default value is "unset"', () => {
    const result = runConfig(['get', 'brain_trust_policy@deadbeef'], { ZSTACK_HOME: TMP_HOME });
    expect(result.status).toBe(0);
    expect(result.stdout).toBe('unset');
  });

  test('set + get roundtrip works', () => {
    const setResult = runConfig(['set', 'brain_trust_policy@deadbeef', 'personal'], { ZSTACK_HOME: TMP_HOME });
    expect(setResult.status).toBe(0);
    const getResult = runConfig(['get', 'brain_trust_policy@deadbeef'], { ZSTACK_HOME: TMP_HOME });
    expect(getResult.stdout).toBe('personal');
  });

  test('invalid value falls back to unset with warning', () => {
    const result = runConfig(['set', 'brain_trust_policy@deadbeef', 'invalid-value'], { ZSTACK_HOME: TMP_HOME });
    expect(result.status).toBe(0);
  ...
  test('shared value accepted', () => {
    runConfig(['set', 'brain_trust_policy@deadbeef', 'shared'], { ZSTACK_HOME: TMP_HOME });
    const getResult = runConfig(['get', 'brain_trust_policy@deadbeef'], { ZSTACK_HOME: TMP_HOME });
    expect(getResult.stdout).toBe('shared');
  });

  test('per-endpoint policies dont collide', () => {
    runConfig(['set', 'brain_trust_policy@aaaaaaaa', 'personal'], { ZSTACK_HOME: TMP_HOME });
    runConfig(['set', 'brain_trust_policy@bbbbbbbb', 'shared'], { ZSTACK_HOME: TMP_HOME });
    const a = runConfig(['get', 'brain_trust_policy@aaaaaaaa'], { ZSTACK_HOME: TMP_HOME });
    const b = runConfig(['get', 'brain_trust_policy@bbbbbbbb'], { ZSTACK_HOME: TMP_HOME });
...
describe('key validation', () => {
  test('rejects keys with disallowed characters', () => {
    const result = runConfig(['get', 'bad-key'], { ZSTACK_HOME: TMP_HOME });
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain('alphanumeric');
  });

  test('accepts plain alphanumeric/underscore keys', () => {
    const result = runConfig(['get', 'proactive'], { ZSTACK_HOME: TMP_HOME });
    expect(result.status).toBe(0);
  });

  test('accepts @<hex-hash> suffix on key', () => {
    const result = runConfig(['get', 'brain_trust_policy@abc123ff'], { ZSTACK_HOME: TMP_HOME });
    expect(result.status).toBe(0);
  });

  test('accepts @local suffix on key', () => {
    const result = runConfig(['get', 'brain_trust_policy@local'], { ZSTACK_HOME: TMP_HOME });
    expect(result.status).toBe(0);
    expect(result.stdout).toBe('unset');
  });
});

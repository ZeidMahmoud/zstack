...
const child = String.raw`
  ...
  switch (scenario) {
    ...
    case 'actual-hermetic-registry': {
      delete process.env.ANTHROPIC_API_KEY; delete process.env.ZSTACK_ANTHROPIC_API_KEY;
    ...
    case 'live-source-drift': {
      ...
      if (scenario === 'live-census-alias') {
        for (const name of ['_zstack-command', 'zstack']) {
        ...
        fs.mkdirSync(path.join(registry, 'zstack'));
        fs.symlinkSync(source, path.join(registry, 'zstack', 'SKILL.md'));

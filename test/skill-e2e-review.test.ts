...
describeIfSelected('Review design lite E2E', ['review-design-lite'], () => {
  ...
  beforeAll(() => {
    ...
    // installed zstack bin; point it at THIS checkout so the test is hermetic.
    fs.writeFileSync(
      path.join(designDir, 'review-design-checklist.md'),
      fs.readFileSync(path.join(ROOT, 'review', 'design-checklist.md'), 'utf-8').replaceAll('~/.claude/skills/zstack/bin', path.join(ROOT, 'bin')),
    );

...
describeIfSelected('Review Dashboard Via Attribution', ['review-dashboard-via'], () => {
  ...
  beforeAll(() => {
    ...
    // zstack-review-read reads from ~/.zstack/projects/$SLUG/$BRANCH-reviews.jsonl
    // For the test, we'll write a mock zstack-review-read script that returns our test data
    ...
    // Write a mock zstack-review-read that returns our test data
    const mockBinDir = path.join(dashDir, '.mock-bin');
    fs.mkdirSync(mockBinDir, { recursive: true });
    fs.writeFileSync(path.join(mockBinDir, 'zstack-review-read'), [
      '#!/usr/bin/env bash',
    ...
    fs.chmodSync(path.join(mockBinDir, 'zstack-review-read'), 0o755);
...
Instead of running ~/.claude/skills/zstack/bin/zstack-review-read, run this mock: ${mockBinDir}/zstack-review-read

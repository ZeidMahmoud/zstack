...
const use = (f: any) => f.tools.find((e: any) => e.kind === 'use' &&
  e.input?.command?.includes('zstack-skill-start'));
...
for (const [name, mutate] of Object.entries({
  'missing state and native log ACK': (f: any) => {
    delete f.options.stateEvidence;
    const log = f.tools.find((e: any) => e.kind === 'use' && e.input?.command?.includes('zstack-question-log'));
  ...
  'wrong preference and native check': (f: any) => {
    ...
    const check = f.tools.find((e: any) => e.kind === 'use' && e.input?.command?.includes('zstack-question-preference'));

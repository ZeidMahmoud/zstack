...
// ZSTACK_HAS_IOS_DEVICE=1; the signing/install/interaction smoke test has the
// separate, explicit ZSTACK_IOS_DEVICE_DEPLOY=1 opt-in.
//
// Runs only when:
...
//
// ZSTACK_IOS_DEVICE_DEPLOY=1 additionally generates the fixture Xcode project,
// signs it with ZSTACK_IOS_DEVELOPMENT_TEAM + ZSTACK_IOS_BUNDLE_ID, installs
...
const HAS_DEVICE = process.env.ZSTACK_HAS_IOS_DEVICE === '1';
const DEPLOY_TO_DEVICE = process.env.ZSTACK_IOS_DEVICE_DEPLOY === '1';
...
function requireDeployEnv(name: 'ZSTACK_IOS_DEVELOPMENT_TEAM' | 'ZSTACK_IOS_BUNDLE_ID'): string {
  const value = process.env[name]?.trim();
  if (!value) {
    throw new Error(`${name} is required when ZSTACK_IOS_DEVICE_DEPLOY=1`);
  }
  return value;
}
...
describe('ios device deployment (explicit opt-in)', () => {
  testIfDeploy('generates, signs, installs, launches, and drives the fixture through the daemon', async () => {
    const developmentTeam = requireDeployEnv('ZSTACK_IOS_DEVELOPMENT_TEAM');
    const bundleId = requireDeployEnv('ZSTACK_IOS_BUNDLE_ID');
    const devices = listDevices();
    ...
    if (!device) {
      ...
      throw new Error([
        'ZSTACK_IOS_DEVICE_DEPLOY=1 requires an available, paired iPhone; stale unavailable devices are never selected.',
        summary,
      ].join('\n'));
    }

    const workDir = mkdtempSync(join(tmpdir(), 'zstack-ios-device-deploy-'));
    ...
    try {
      ...
      runChecked(join(ROOT, 'bin/zstack-ios-qa-regen'), [

/**
 * zstack-settings-hook schema-aware surface (T3 plan-tune cathedral).
 *
...
const SETTINGS_HOOK = path.join(ROOT, 'bin', 'zstack-settings-hook');

let tmpDir: string;
let settingsFile: string;

beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-shsa-'));
...
function run(args: string[]): { stdout: string; stderr: string; exitCode: number } {
  try {
    const stdout = execSync([SETTINGS_HOOK, ...args].map((s) => `'${s}'`).join(' '), {
      env: { ...process.env, ZSTACK_SETTINGS_FILE: settingsFile },
      encoding: 'utf-8',
      timeout: 10000,
    });
...
describe('add-event', () => {
  test('registers a PreToolUse hook with matcher + source tag', () => {
    ...
    expect(s.hooks.PreToolUse[0]._zstack_source).toBe('plan-tune-cathedral');
  ...
  test('dedup includes command: same (event, matcher, command) with different source updates in place', () => {
    ...
    const s = settings();
    expect(s.hooks.PostToolUse).toHaveLength(1);
    expect(s.hooks.PostToolUse[0]._zstack_source).toBe('source-B');
  });
  ...
  test('dedup includes command: untagged entry with same command is updated not duplicated', () => {
    ...
    const s = settings();
    expect(s.hooks.PostToolUse).toHaveLength(1);
    expect(s.hooks.PostToolUse[0]._zstack_source).toBe('plan-tune-cathedral');
  });
...
describe('ensure-event collapses duplicate (event, source) entries', () => {
  test('two same-source entries from the old matcher-keyed dedup collapse to ONE updated entry', () => {
    ...
    // (event, _zstack_source) — the old dedup keyed on the matcher too, so a
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [
          { _zstack_source: 'plan-tune-cathedral', matcher: 'OldMatcherA', hooks: [{ type: 'command', command: '/old-a', timeout: 5 }] },
          { matcher: 'Bash', hooks: [{ type: 'command', command: '/user-own-hook' }] },
          { _zstack_source: 'plan-tune-cathedral', matcher: 'OldMatcherB', hooks: [{ type: 'command', command: '/old-b', timeout: 5 }] },
        ],
      },
    }, null, 2));
    ...
    const r = spawnSync('bash', [
      ...
      '--source', 'plan-tune-cathedral',
      '--timeout', '5',
    ], { env: { ...process.env, ZSTACK_SETTINGS_FILE: settingsFile }, encoding: 'utf-8', timeout: 15_000 });

    expect(r.status).toBe(0);
    // The collapse is reported on stderr, never silent.
    expect(r.stderr).toContain('collapsed 1 duplicate');
    const s = settings();
    const mine = s.hooks.PostToolUse.filter((e: any) => e._zstack_source === 'plan-tune-cathedral');
  ...
  test('no duplicates → no collapse message, single entry updated as before', () => {
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [
          { _zstack_source: 'plan-tune-cathedral', matcher: 'OldMatcher', hooks: [{ type: 'command', command: '/old', timeout: 5 }] },
        ],
      },
    }, null, 2));
...
// the fixed table) because Claude Code strips the _zstack_source key when it
...
/** run() with hermetic zstack-config state (prune-stale consults plan_tune_hooks). */
function runIso(args: string[], extraEnv: Record<string, string> = {}) {
  try {
    const stdout = execSync([SETTINGS_HOOK, ...args].map((s) => `'${s}'`).join(' '), {
      env: {
        ...process.env,
        ZSTACK_SETTINGS_FILE: settingsFile,
        ZSTACK_STATE_ROOT: tmpDir,
        ...extraEnv,
      },
      encoding: 'utf-8',
      timeout: 15000,
...
function mkCanon(base: string, name = 'canon'): string {
  ...
  const su = path.join(canon, 'bin', 'zstack-session-update');
...
describe('add-event: per-item identity re-point', () => {
  ...
  test('foreign path with a zstack basename is NOT claimed (wrong relpath suffix)', () => {
    const canon = mkCanon(tmpDir);
    fs.writeFileSync(settingsFile, JSON.stringify({
  ...
  test('mixed entry: only the zstack item (index > 0) is replaced; the user item survives', () => {
    const canon = mkCanon(tmpDir);
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
...
describe('legacy remove: per-item (regression)', () => {
  test('mixed SessionStart entry: user item survives in place, zstack item removed', () => {
    // REGRESSION pin: the pre-v1.67.2 legacy `remove` dropped the ENTIRE
    // entry when any item matched zstack-session-update, destroying a user's
      ...
      hooks: {
        SessionStart: [{
          hooks: [
            ...
            { type: 'command', command: '/old/install/bin/zstack-session-update' },
          ],
        }],
      },
    }, null, 2));
    const r = runIso(['remove', '/old/install/bin/zstack-session-update']);
    expect(r.exitCode).toBe(0);
    const s = settings();
...
describe('review-army hardening (specialist findings)', () => {
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        SessionStart: [
          ...
          { hooks: 'not-an-array' },
          { hooks: [] },
          { hooks: [{ type: 'command', command: '/x/bin/zstack-session-update' }] },
        ],
      },
    }, null, 2));
    runIso(['remove', '/x/bin/zstack-session-update']);
    const s = settings();
  ...
  test('add-event never tags a mixed entry (old-version ratchet guard)', () => {
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [{
          matcher: AUQ_MATCHER,
          _zstack_source: 'plan-tune-cathedral',
          hooks: [
            { type: 'command', command: '/Users/me/my-own-hook' },
    ...
    expect(e._zstack_source).toBeUndefined();
  });
  ...
  test('a 0600 settings.json keeps its mode across mutations (API keys stay private)', () => {
    ...
    runIso(['add-event', '--event', 'Stop', '--command', '/x/hosts/claude/hooks/timeline-stop-hook', '--source', 'zstack-timeline-stop']);
  ...
  test('a canonical root containing $ is escaped in the registered command', () => {
    ...
    expect(r2.stdout).toMatch(/removed 0 zstack hook entries \(repointed 0\)/);
    expect(fs.readFileSync(settingsFile, 'utf-8')).toBe(before);
  });

  test('backups rotate: at most 10 .bak files survive repeated mutations', () => {
    for (let i = 0; i < 13; i++) {
      runIso(['add-event', '--event', 'Stop', '--command', `/x/hosts/claude/hooks/timeline-stop-hook-${i}`, '--source', 'zstack-timeline-stop']);
    }
  ...
  test('tag-stripped verify-gate entry is table-owned: healed by --repoint, swept by --all', () => {
    ...
    const canon = mkCanon(tmpDir);
    const vg = path.join(canon, 'bin', 'zstack-verify-gate');
    fs.writeFileSync(vg, '#!/bin/sh\n');
    fs.chmodSync(vg, 0o755);
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: { Stop: [hookEntry('/dead/install/bin/zstack-verify-gate')] },   // tag STRIPPED
    }, null, 2));
    runIso(['prune-stale', '--repoint', canon]);
    let s = settings();
    expect(s.hooks.Stop[0].hooks[0].command).toBe(vg);
    expect(s.hooks.Stop[0]._zstack_source).toBe('verify-gate');
  ...
  test('a foreign entry that STARTED empty survives prune-stale untouched', () => {
    ...
    expect(r.stdout).toMatch(/removed 0 zstack hook entries \(repointed 0\)/);
    expect(fs.readFileSync(settingsFile, 'utf-8')).toBe(before);
  });
...
describe('remove-source: per-item', () => {
  test('mixed tagged entry: zstack item removed, user item survives, tag dropped', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [hookEntry(
    ...
    expect(s.hooks.PostToolUse[0]._zstack_source).toBeUndefined();
  });
});

describe('Memorable UserPromptSubmit hook ownership', () => {
  const source = 'zstack-memorable';
  const stale = '/old/worktree/hosts/claude/hooks/memorable-user-prompt-hook';
  const canonical = '/stable/zstack/hosts/claude/hooks/memorable-user-prompt-hook';
  ...
  test('ensure-event re-points only the wrapper in a mixed entry and preserves the foreign hook', () => {
    ...
    expect(entries[0]._zstack_source).toBeUndefined();
  });
...
describe('remove-source: identity-aware (tag OR table)', () => {
  // Claude Code strips _zstack_source when it rewrites settings.json. A
  ...
  const memo = '/stable/zstack/hosts/claude/hooks/memorable-user-prompt-hook';
  const foreign = '/Users/me/my-user-prompt-hook';
  ...
  test('removes an UNTAGGED single-item memorable entry by identity', () => {
    ...
    const r = run(['remove-source', '--source', 'zstack-memorable']);
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toMatch(/removed 1 /);
    expect(settings().hooks).toBeUndefined();
  });
  ...
  test('CRITICAL regression: identity is per source -- another source\'s tag-stripped item is never touched', () => {
    ...
    const s = settings();
    expect(s.hooks.Stop).toHaveLength(1);              // timeline (zstack-timeline-stop) untouched
    expect(s.hooks.PostToolUse).toBeUndefined();
  });

  test('a tagged entry of source A holding an item of source B keeps B\'s item and its tag (nothing of A inside)', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: { Stop: [{ _zstack_source: 'plan-tune-cathedral', hooks: [
  ...
  test('setup --no-team sweep: ZSTACK_SWEEP_EXCLUDE_SOURCES keeps verify-gate AND zstack-memorable (tagged or tag-stripped), sweeps timeline', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        Stop: [
          { _zstack_source: 'verify-gate', hooks: [{ type: 'command', command: '/x/bin/zstack-verify-gate' }] },
          { _zstack_source: 'zstack-timeline-stop', hooks: [{ type: 'command', command: '/x/hosts/claude/hooks/timeline-stop-hook' }] },
        ],
        UserPromptSubmit: [
          { _zstack_source: 'zstack-memorable', hooks: [{ type: 'command', command: memo }] },
          { hooks: [{ type: 'command', command: `bash "${memo}"` }] },   // tag stripped by Claude Code
        ],
      },
    }, null, 2));
    const r = runIso(['prune-stale', '--all'], { ZSTACK_SWEEP_EXCLUDE_SOURCES: 'verify-gate,zstack-memorable' });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toMatch(/removed 1 /);
    const s = settings();
  ...
  test('a tagged legacy stray (single item, no table row) is still removed', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: { UserPromptSubmit: [{ _zstack_source: 'zstack-memorable', hooks: [{ type: 'command', command: '/legacy/anything' }] }] },
    }, null, 2));
    const r = run(['remove-source', '--source', 'zstack-memorable']);
    expect(r.stdout).toMatch(/removed 1 /);
    expect(settings().hooks).toBeUndefined();
...
describe('remove-source: identity removal holds for EVERY KNOWN_HOOKS source (regression)', () => {
  ...
  const seedAll = () => fs.writeFileSync(settingsFile, JSON.stringify({
    hooks: {
      Stop: [
        ...
        { hooks: [{ type: 'command', command: '/x/bin/zstack-verify-gate' }] },
  ...
  for (const [source, own] of [
    ['zstack-timeline-stop', '/x/hosts/claude/hooks/timeline-stop-hook'],
    ['verify-gate', '/x/bin/zstack-verify-gate'],
    ['auq-error-fallback', '/x/hosts/claude/hooks/auq-error-fallback-hook'],
    ['zstack-session-update', '/x/bin/zstack-session-update'],
  ...
  test('a tagged entry holding only a command-less item, and a tagged multi-item entry with no table rows, are kept with their tags', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({ hooks: { UserPromptSubmit: [
      { _zstack_source: 'zstack-memorable', hooks: [{ type: 'command' }] },
      { _zstack_source: 'zstack-memorable', hooks: [{ type: 'command', command: '/a/foreign' }, { type: 'command', command: '/b/foreign' }] },
    ] } }, null, 2));
    const before = fs.readFileSync(settingsFile, 'utf-8');
    const r = run(['remove-source', '--source', 'zstack-memorable']);
    expect(r.stdout).toMatch(/removed 0 /);
...
describe('list-items: read-only identity view', () => {
  ...
  test('--owned-by prints only the table-identified item, as a JSON string literal, tag or no tag', () => {
    seed();
    const r = run(['list-items', '--event', 'UserPromptSubmit', '--owned-by', 'zstack-memorable']);
    expect(r.exitCode).toBe(0);
  ...
  test('an unknown flag exits 1; --owned-by combined with --command-regex intersects (a regex never widens a selection)', () => {
    ...
    const both = run(['list-items', '--event', 'UserPromptSubmit', '--owned-by', 'zstack-memorable', '--command-regex', 'memorable-user-prompt-hook$']);
    expect(both.stdout.trim().split('\n')).toEqual([JSON.stringify(memo)]);
    const none = run(['list-items', '--event', 'UserPromptSubmit', '--owned-by', 'zstack-memorable', '--command-regex', 'no-such-thing']);
...
describe('prune-stale', () => {
  test('prunes dead zstack items; keeps live zstack and dead non-zstack', () => {
    const canon = mkCanon(tmpDir);
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [
          hookEntry(`${canon}/hosts/claude/hooks/question-log-hook`, AUQ_MATCHER),   // live zstack
          hookEntry('/dead/wt/hosts/claude/hooks/auq-error-fallback-hook', AUQ_MATCHER), // dead zstack
          hookEntry('/dead/user/own-hook', AUQ_MATCHER),                              // dead NON-zstack
        ],
      },
    }, null, 2));
    const r = runIso(['prune-stale']);
    expect(r.stdout).toMatch(/removed 1 zstack hook entries/);
    const s = settings();
    expect(s.hooks.PostToolUse).toHaveLength(2);
  ...
  test('--repoint re-points dead AND live items, preserves bash prefix, restores tags', () => {
    ...
    expect(s.hooks.Stop[0]._zstack_source).toBe('zstack-timeline-stop');
  ...
  test('Windows backslash path is classified as zstack-owned and pruned when dead', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
  ...
  test('--all removes live untagged zstack items, spares user hooks and mixed-entry user items', () => {
    const canon = mkCanon(tmpDir);
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        PostToolUse: [hookEntry(`${canon}/hosts/claude/hooks/question-log-hook`, AUQ_MATCHER)],
        Stop: [hookEntry(
          `${canon}/hosts/claude/hooks/timeline-stop-hook`, undefined, 'zstack-timeline-stop',
    ...
    expect(s.hooks.Stop[0]._zstack_source).toBeUndefined();
  ...
  test('--all removes tagged single-item legacy strays (no table match)', () => {
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: { Stop: [hookEntry('/old/install/bin/zstack-verify-gate', undefined, 'zstack-verify-gate')] },
    }, null, 2));
    const r = runIso(['prune-stale', '--all']);
    expect(r.stdout).toMatch(/removed 1/);
    expect(settings().hooks).toBeUndefined();
  ...
  test('explicit plan_tune_hooks:no — dead plan-tune pruned, live plan-tune NOT re-pointed, Stop still re-pointed', () => {
    const canon = mkCanon(tmpDir);
    const live = mkCanon(tmpDir, 'live-worktree');
    execSync(`'${path.join(ROOT, 'bin', 'zstack-config')}' set plan_tune_hooks no`, {
      env: { ...process.env, ZSTACK_STATE_ROOT: tmpDir },
      timeout: 30_000,
    });
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
  ...
  test('incident facsimile: the exact live-damage shape heals to canonical', () => {
    ...
    fs.writeFileSync(settingsFile, JSON.stringify({
      hooks: {
        SessionStart: [hookEntry(`${canon}/bin/zstack-session-update`)],
        PostToolUse: [
        ...
        Stop: [
          ...
          hookEntry(dead2('timeline-stop-hook'), undefined, 'zstack-timeline-stop'),
        ],
        PreCompact: [hookEntry('/Users/me/my-own-hook')],
      },
    }, null, 2));
    ...
    for (const ev of ['SessionStart', 'PostToolUse', 'PreToolUse', 'Stop']) {
      for (const e of s.hooks[ev]) {
        expect(e._zstack_source).toBeDefined();
        for (const it of e.hooks) expect(it.command.startsWith(canon)).toBe(true);
      }
    }
    const postSources = s.hooks.PostToolUse.map((e: any) => e._zstack_source).sort();
...
describe('fail-closed parse (pre-existing data-loss fix)', () => {
  const MUTATORS: string[][] = [
    ['add', '/x/bin/zstack-session-update'],
    ['remove', '/x/bin/zstack-session-update'],
...
describe('mutation lock', () => {
  test('stale lock (old mtime) is taken over; mutation proceeds', () => {
    ...
    const r = runIso(['add-event', '--event', 'Stop', '--command', '/x/hosts/claude/hooks/timeline-stop-hook', '--source', 'zstack-timeline-stop']);
    expect(r.exitCode).toBe(0);
  ...
  test('fresh foreign lock: mutation skipped loudly (exit 5), file untouched', () => {
    ...
    const r = runIso(
      ['add-event', '--event', 'Stop', '--command', '/x', '--source', 's'],
      { ZSTACK_SETTINGS_LOCK_TIMEOUT_MS: '300' },
    );
  ...
  test('two concurrent add-events both land (lock serializes; file stays valid JSON)', () => {
    ...
    execSync(`sh -c "${a} & ${b} & wait"`, {
      env: { ...process.env, ZSTACK_SETTINGS_FILE: settingsFile, ZSTACK_STATE_ROOT: tmpDir },
      encoding: 'utf-8',
      timeout: 20000,
    });
...
describe('zstack-settings-hook adversarial hardening', () => {
  let tmpDir: string;
  let settingsFile: string;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-settings-adv-'));
    settingsFile = path.join(tmpDir, 'settings.json');
  });
  ...
  test('foreign hook whose basename collides with Object.prototype survives --all', () => {
    ...
    expect(r.exitCode).toBe(0);
    const s = advSettings();
    expect(s.hooks.Stop).toHaveLength(1);   // zstack entry swept...
    expect(s.hooks.Stop[0].hooks[0].command).toBe('/usr/local/bin/toString');  // ...foreign one kept
  });

  test('ZSTACK_SWEEP_EXCLUDE_SOURCES preserves verify-gate during an --all sweep', () => {
    ...
    const r = runAdv(['prune-stale', '--all'], { ZSTACK_SWEEP_EXCLUDE_SOURCES: 'verify-gate' });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toMatch(/removed 1 /);
    const s = advSettings();
    expect(s.hooks.Stop).toHaveLength(1);

...
import { prepareMethodology } from '../bin/zstack-autoplan-snapshot';
...
const owned: string[] = [];
function fixture() {
  const dir = mkdtempSync(join(tmpdir(), 'zstack-autoplan-init-')); owned.push(dir);
...
mock.module('node:fs', () => ({ ...real, mkdtempSync(prefix, options) {
  if (String(prefix).includes('.zstack-autoplan-restore-')) throw new Error('Injected restore staging failure');

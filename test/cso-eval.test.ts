...
function portableSkill(version: 'v2' | 'v3', sectionMarker: string): string {
  const manifest = JSON.stringify({ $schema: 'https://zstack.dev/schemas/section-manifest.json', skill: 'cso', version: 1, sections: [{ id: 'audit-phases', file: 'audit-phases.md', title: 'Audit phases', trigger: 'during audit phases' }] }, null, 2) + '\n';
...
describe('CSO matched producer orchestration', () => {
  ...
  const helperBundle = (directory = join(root(), 'helpers')) => {
    ...
    for (const name of ['cso-eval-producer', 'zstack-cso-launcher', 'zstack-cso-core', 'zstack-cso-watchdog']) {
      const path = join(directory, `${name}${suffix}`);
      writeFileSync(path, process.platform === 'win32' ? 'test executable\n' : '#!/bin/sh\nexit 0\n', { mode: 0o755 });
      if (process.platform !== 'win32') chmodSync(path, 0o755);
    }
    const core = join(directory, `zstack-cso-core${suffix}`);
    writeFileSync(join(directory, '.zstack-cso-generation'), `${sha256(readFileSync(core))}\n`, { mode: 0o644 });
    writeFileSync(join(directory, `provider-test${suffix}`), process.platform === 'win32' ? 'test provider\n' : '#!/bin/sh\nexit 0\n', { mode: 0o755 });
    return join(directory, `zstack-cso-launcher${suffix}`);
  };
  ...
  test('documents the complete five-artifact production installation', () => {
    ...
    expect(guide).toContain('bin/.zstack-cso-generation "$stage/.zstack-cso-generation"');
    expect(guide).toContain('/opt/zstack-cso-producer/.zstack-cso-generation');
    expect(guide).toContain('CSO_EVAL_PAID=1 /opt/zstack-cso-producer/cso-eval-producer run');
  ...
  test('binds only a complete adjacent executable helper bundle outside source and state', () => {
    ...
    expect(resolveProducerHelperBinding(source, state, launcher)).toEqual({
      ...
      launcher,
      core: join(dirname(launcher), `zstack-cso-core${suffix}`),
      watchdog: join(dirname(launcher), `zstack-cso-watchdog${suffix}`),
      generation: join(dirname(launcher), '.zstack-cso-generation'),
    });

    const missingGeneration = helperBundle();
    rmSync(join(dirname(missingGeneration), '.zstack-cso-generation'));
    ...
    writeFileSync(join(dirname(malformedGeneration), '.zstack-cso-generation'), 'f'.repeat(64));
    ...
    writeFileSync(join(dirname(mismatchedGeneration), '.zstack-cso-generation'), `${'f'.repeat(64)}\n`);
    ...
    rmSync(join(dirname(missingWatchdog), `zstack-cso-watchdog${suffix}`));
    ...
    if (process.platform !== 'win32') {
      chmodSync(join(dirname(nonExecutable), 'zstack-cso-core'), 0o600);
    ...
    if (process.platform !== 'win32') {
      rmSync(symlinked);
      symlinkSync(join(dirname(symlinked), 'zstack-cso-core'), symlinked);
    ...
    const containedLauncher = join(containedDirectory, `zstack-cso-launcher${suffix}`);
    for (const name of ['cso-eval-producer', 'zstack-cso-launcher', 'zstack-cso-core', 'zstack-cso-watchdog']) {
  ...
  test('production binding rejects a privileged or producer-writable binary and directory chain', () => {
    ...
    if (process.platform !== 'win32') {
      ...
      chmodSync(join(binaryDirectory, `zstack-cso-core${suffix}`), 0o555);
      chmodSync(join(binaryDirectory, `zstack-cso-watchdog${suffix}`), 0o555);
    ...
    if (process.platform !== 'win32') {
      for (const path of [writableDirectory, join(directory, `zstack-cso-core${suffix}`), join(directory, `zstack-cso-watchdog${suffix}`), adjacentProducer]) chmodSync(path, 0o555);
  ...
  test('requires explicit authorization, consumes labels before the model starts, and records measured receipts', async () => {
    ...
    const invalidLauncher = join(root(), 'missing', `zstack-cso-launcher${process.platform === 'win32' ? '.exe' : ''}`);
    ...
    const adapter = new FakeAdapter(opts => {
      ...
      expect(opts).toMatchObject({
        ...
        csoProducer: {
          ...
          helperGeneration: join(dirname(launcher), '.zstack-cso-generation'),
      ...
      const artifactRoot = join(process.env.ZSTACK_HOME!, 'security', 'cso', 'run-1');
      ...
      expect(process.env.ZSTACK_SESSION_KIND).toBe('spawned');
      expect(process.env.ZSTACK_HEADLESS).toBe('1');
    });
    ...
    for (const [host, family] of [['codex', 'gpt'], ['claude', 'claude'], ['gemini', 'gemini']] as const) {
      ...
      const adapter = new FakeAdapter(opts => {
        ...
        if (host === 'gemini') {
          ...
          expect(contextFileName).toMatch(/^\.zstack-cso-context-[a-f0-9]{32}\.md$/);
  ...
  test('withholds the receipt when the helper generation changes during a cell', async () => {
    ...
    const receiptPath = join(outputRoot, `${cell.id}.json`);
    const adapter = new FakeAdapter(opts => {
      const core = join(dirname(opts.csoProducer!.helperLauncher), `zstack-cso-core${process.platform === 'win32' ? '.exe' : ''}`);

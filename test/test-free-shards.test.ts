...
describe('test-free-shards: strict shard execution', () => {
  ...
  test('the log-file path is announced once at start and the PASS epilogue repeats it', async () => {
    ...
    const outcome = await runFreeShard(['pass'], 1, 1, { commandFor, quiet: true, log: (l) => lines.push(l) });
    expect(outcome.status).toBe('passed');
    const announced = lines.filter((l) => /^\[test:free\] full log: .+zstack-free-test-.+\.log$/.test(l));
  ...
  test('spawned shard gets throwaway TMPDIR but NEVER an injected ZSTACK_HOME', async () => {
    // ZSTACK_HOME injection was tried and reverted: one shared scratch home
    ...
    try {
      const script =
        ...
        + `  home: process.env.ZSTACK_HOME ?? null, tmp: process.env.TMPDIR,`
      ...
      // ZSTACK_HOME passes through untouched (whatever the parent had, incl. unset).
      expect(seen.home).toBe(process.env.ZSTACK_HOME ?? null);
      // TMPDIR is a per-shard throwaway, cleaned up once the shard finishes.
      expect(seen.tmp).toContain('zstack-free-shard-');
  ...
  test('concurrent shards isolate browser state and remove it on success or failure', async () => {
    ...
    try {
      ...
      for (const [index, shard] of seen.entries()) {
        ...
        expect(path.relative(ownedRoot, shard.state)).toBe(path.join('.zstack', 'browse.json'));
...
describe('test-free-shards: duration-aware packing (full-suite LPT)', () => {
  ...
  test('corrupt seed falls back to null (hash sharding), never throws', () => {
    ...
    const prev = process.env.ZSTACK_FREE_TEST_DURATIONS;
    process.env.ZSTACK_FREE_TEST_DURATIONS = seedPath;
    try {
      expect(loadFreeTestDurations()).toBeNull();
      // Missing file: silent null (fresh checkouts are normal).
      process.env.ZSTACK_FREE_TEST_DURATIONS = path.join(dir, 'missing.json');
      expect(loadFreeTestDurations()).toBeNull();
      ...
      if (prev === undefined) delete process.env.ZSTACK_FREE_TEST_DURATIONS;
      else process.env.ZSTACK_FREE_TEST_DURATIONS = prev;
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });

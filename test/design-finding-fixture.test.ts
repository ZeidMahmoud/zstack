...
`);
    ...
    const failure:Record<string,string>={below:'BAND FAIL (below floor)',above:'BAND FAIL (above ceiling)',
      'missing-report':'D19 FAIL: agent did not produce expected plan file','trailing-report':'trailing ## heading(s) after ZSTACK REVIEW REPORT',
...
for (const storage of ['configured', 'plugin', 'default']) test(`Design mockup setup honors ${storage} state storage`, async () => {
  ...
  try {
    fs.mkdirSync(path.join(home, '.claude/skills/zstack'), { recursive: true });
    fs.symlinkSync(path.join(ROOT, 'bin'), path.join(home, '.claude/skills/zstack/bin'), 'dir');
    fs.mkdirSync(cwd);
    await promisify(execFile)('git', ['init', '-q', cwd], { timeout: 5000 });
    const expected = storage === 'configured' ? configured : storage === 'plugin' ? plugin : path.join(home, '.zstack');
    const env = { PATH: process.env.PATH, HOME: home, USERPROFILE: '', TMPDIR: directory, TMP: directory,
      ...(storage === 'configured' ? { ZSTACK_HOME: configured, CLAUDE_PLUGIN_DATA: plugin, CLAUDE_PLUGIN_ROOT: '/plugins/zstack' } : {}),
      ...(storage === 'plugin' ? { CLAUDE_PLUGIN_DATA: plugin, CLAUDE_PLUGIN_ROOT: '/plugins/zstack' } : {}) };
    const sources = [
    ...
    const fakeDesign = path.join(home, '.claude/skills/zstack/design/dist/design');
    ...
    if (storage !== 'default') expect(fs.existsSync(path.join(home, '.zstack/projects'))).toBe(false);

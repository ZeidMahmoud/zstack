/**
 * #2673: zstack-config set must reject malformed cross_project_learnings.
 *
 * Empty get is the first-run prompt sentinel (pinned in
 * zstack-config-defaults.test.ts). Skills only enable on the literal "true".
...
const CONFIG = path.resolve(import.meta.dir, "..", "bin", "zstack-config");
...
function cfg(args: string[]): { code: number; out: string; err: string } {
  const r = spawnSync(CONFIG, args, {
    timeout: 30_000,
    encoding: "utf8",
    env: { ...process.env, ZSTACK_STATE_ROOT: stateRoot },
  });
...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zstack-config-xproj-"));
});

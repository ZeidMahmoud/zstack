/**
 * zstack-config default-table completeness (gate, free).
 *
 ...
 *      table. Add a `zstack-config get some_new_key` to a preamble without
...
const CONFIG_BIN = path.join(ROOT, 'bin', 'zstack-config');
const SELF = 'zstack-config-defaults.test.ts';

// Isolated state dir, so a value the developer happens to have set in their own
// ~/.zstack/config.yaml cannot mask a missing default.
const STATE = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-config-test-'));

function get(key: string): { out: string; code: number } {
  const r = spawnSync('bash', [CONFIG_BIN, 'get', key], {
    encoding: 'utf-8',
    timeout: 30_000,
    env: { ...process.env, ZSTACK_STATE_ROOT: STATE },
  });
...
/** Every `zstack-config get <key>` call site in the tree. */
function keysReadInTree(root = ROOT): string[] {
  const keys = new Set<string>();
  // [ \t]+ rather than \s+: \s crosses newlines and would pair a trailing
  // "zstack-config get" with the first word of the next line.
  const re = /zstack-config["']?[ \t]+get[ \t]+([a-zA-Z0-9_]+)/g;
  const stack = [root];
  while (stack.length) {
    const cur = stack.pop()!;
    ...
    for (const ent of entries) {
      ...
      if (!/\.(md|ts|sh)$|^zstack-[a-z-]+$/.test(ent.name)) continue;
      let text: string;
      try {
...
describe('zstack-config defaults (gate, free)', () => {
  test('workspace history does not add call sites to the source census', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-config-census-'));
    try {
      fs.mkdirSync(path.join(root, '.context', 'old-checkout'), { recursive: true });
      fs.writeFileSync(path.join(root, 'active.md'), 'zstack-config get question_tuning\n');
      fs.writeFileSync(path.join(root, '.context', 'old-checkout', 'old.md'), 'zstack-config get retired_workspace_key\n');
  ...
  test('an unknown key exits non-zero, so the caller fallback fires', () => {
    const r = get('definitely_not_a_zstack_key_9f3a');
    expect(r.code).not.toBe(0);
    expect(r.out).toBe('');
  });

  test('a known key whose default is intentionally empty still exits 0', () => {
    // repo_mode is in this class BY CONTRACT: zstack-repo-mode treats any
    ...
    // (caught live by test/zstack-repo-mode.test.ts during the wave).
...
describe('design_detector (auto|off, rejecting validator)', () => {
  ...
  test('set to an invalid value exits 1 and leaves the file unchanged', () => {
    ...
    const r = spawnSync('bash', [CONFIG_BIN, 'set', 'design_detector', 'maybe'], {
      encoding: 'utf-8', timeout: 30_000, env: { ...process.env, ZSTACK_STATE_ROOT: STATE },
    });
  ...
  test('list and defaults enumerate design_detector', () => {
    for (const verb of ['list', 'defaults']) {
      const r = spawnSync('bash', [CONFIG_BIN, verb], { encoding: 'utf-8', timeout: 30_000, env: { ...process.env, ZSTACK_STATE_ROOT: STATE } });
  ...
  test('set off / set auto round-trip', () => {
    spawnSync('bash', [CONFIG_BIN, 'set', 'design_detector', 'off'], { encoding: 'utf-8', timeout: 30_000, env: { ...process.env, ZSTACK_STATE_ROOT: STATE } });
    expect(get('design_detector').out).toBe('off');
    spawnSync('bash', [CONFIG_BIN, 'set', 'design_detector', 'auto'], { encoding: 'utf-8', timeout: 30_000, env: { ...process.env, ZSTACK_STATE_ROOT: STATE } });
    expect(get('design_detector').out).toBe('auto');
  });
});

describe('design_detector_install_prompted (true|false, rejecting validator)', () => {
  const env = { ...process.env, ZSTACK_STATE_ROOT: STATE };

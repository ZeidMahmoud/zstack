...
for(const [name,edit] of Object.entries({
 ...
 'missing report':(x:ReturnType<typeof e366Navigation>)=>{x.plan=x.plan.slice(0,x.plan.indexOf('## ZSTACK REVIEW REPORT'));},
...
for(const [name,edit] of Object.entries({
 ...
 'foreign resolved source':(x:ReturnType<typeof countNavigation>)=>{x.plan=x.plan.replace('/zstack-plan-count-VjWQw7/PLAN.md','/zstack-plan-count-VjWQw7/OTHER.md');},
 'missing report':(x:ReturnType<typeof countNavigation>)=>{x.plan=x.plan.slice(0,x.plan.indexOf('## ZSTACK REVIEW REPORT'));},
...
for(const [name,edit] of Object.entries({
 ...
 'missing report':(x:ReturnType<typeof retryNavigation>)=>{x.plan=x.plan.slice(0,x.plan.indexOf('## ZSTACK REVIEW REPORT'));},
...
check('missing report',false,x=>x.plan=x.plan.slice(0,x.plan.indexOf('## ZSTACK REVIEW REPORT')));
...
for(const [name,mutate] of Object.entries({
 ...
 'missing current report':(x:any)=>{x.plan=x.plan.slice(0,x.plan.indexOf('## ZSTACK REVIEW REPORT'));},
...
investigationCheck('historical quoted withdrawal cannot erase current owned approval',true,x=>{x.plan=x.plan.replace('## ZSTACK REVIEW REPORT','Earlier note: "R6 is withdrawn."\n\n## ZSTACK REVIEW REPORT');});
for(const [name,mutate] of Object.entries({
 ...
 'later current task withdrawal':(x:any)=>{x.plan=x.plan.replace('## ZSTACK REVIEW REPORT','T6 is withdrawn.\n\n## ZSTACK REVIEW REPORT');},
 'later current issue reopening':(x:any)=>{x.plan=x.plan.replace('## ZSTACK REVIEW REPORT','R6 is reopened.\n\n## ZSTACK REVIEW REPORT');},
...
for(const [name,mutate] of Object.entries({
 ...
 'current quoted issue withdrawal':(x:any)=>{x.plan=x.plan.replace('## ZSTACK REVIEW REPORT','R6 is "withdrawn".\n\n## ZSTACK REVIEW REPORT');},
...
for(const status of ['R6 is a blocker.','R6 is now blocking.','R6 is no longer optional.'])
 investigationCheck(`later report correction remains authoritative: ${status}`,false,x=>{x.plan=x.plan.replace('## ZSTACK REVIEW REPORT','Correction: '+status+'\n\n## ZSTACK REVIEW REPORT');});
...
investigationCheck('historical quoted blocker remains inert',true,x=>{x.plan=x.plan.replace('## ZSTACK REVIEW REPORT','Earlier note: "R6 is a blocker."\n\n## ZSTACK REVIEW REPORT');});
...
investigationCheck('an unrelated previously approved task stays approved',true,x=>{x.plan=x.plan.replace('## ZSTACK REVIEW REPORT','T2 implementation is approved.\n\n## ZSTACK REVIEW REPORT');});
...
ledgerCheckCf74('published routing approval withdrawal stays operative',false,x=>{x.plan=x.plan.replace('## ZSTACK REVIEW REPORT','D1 approval is revoked.\n\n## ZSTACK REVIEW REPORT');});

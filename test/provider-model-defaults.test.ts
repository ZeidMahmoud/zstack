...
const ENV_KEYS = ['PATH', 'ZSTACK_CLAUDE_BIN', 'ZSTACK_CLAUDE_BIN_ARGS',
  'ZSTACK_CODEX_MODEL', 'EVALS_MODEL', 'ZSTACK_EVAL_MODEL', 'ZSTACK_EVAL_MODEL_CAPTURE'];
let saved: Record<string, string | undefined>;
let workdir: string;

describe('CSO producer provider policies', () => {
  const suffix = process.platform === 'win32' ? '.exe' : '';
  const helperAt = (directory: string) => join(directory, `zstack-cso-launcher${suffix}`);
  ...
  const policy = (stateDirectory: string, sourceDirectory: string, helperLauncher: string) => ({
    stateDirectory, sourceDirectory, helperLauncher, helperGeneration: join(dirname(helperLauncher), '.zstack-cso-generation'), providerCommand: providerAt(dirname(helperLauncher)),
  });

  test('keeps exact Codex read-only defaults and gives the producer a custom least-privilege profile', () => {
    const providerWorkdir = join(tmpdir(), 'zstack-provider-work');
    ...
    for (const admitted of [
      ...
      join(dirname(helper), `zstack-cso-core${suffix}`), join(dirname(helper), `zstack-cso-watchdog${suffix}`),
      join(dirname(helper), '.zstack-cso-generation'), source,
  ...
  test('keeps exact Claude defaults and enables its producer-only noninteractive safe policy', () => {
    const work = join(tmpdir(), 'zstack-claude-work');
  ...
  test('keeps exact Gemini defaults and replaces deprecated yolo only for the producer', () => {
    const work = join(tmpdir(), 'zstack-gemini-work');
    const state = join(work, 'state');
    const source = join(work, 'source');
    const helper = helperAt(join(work, 'installed'));
    ...
    const contextFileName = `.zstack-cso-context-${'a'.repeat(32)}.md`;
  ...
  test('passes only selected provider auth plus safe execution inputs and strips Docker credentials', () => {
    const fixtureRoot = join(tmpdir(), 'zstack-provider-environment-policy');
    ...
    const sourceEnv = {
      ...
      ZSTACK_HOME: state, ZSTACK_SESSION_KIND: 'spawned', ZSTACK_HEADLESS: '1',
    ...
    const safe = { PATH: '/usr/bin', HOME: '/home/eval', LANG: 'C.UTF-8', LC_ALL: 'C.UTF-8', TZ: 'UTC', ZSTACK_HOME: state, ZSTACK_SESSION_KIND: 'spawned', ZSTACK_HEADLESS: '1' };
    ...
    expect(codexExecEnvironment(common, sourceEnv)).toEqual({
      ...safe, HOME: codexPaths.home, ZSTACK_HOME: join(state, 'cso-home'), CODEX_HOME: codexPaths.home, OPENAI_API_KEY: 'openai-auth',
    });
    const claudePaths = claudeProducerPaths(state);
    expect(claudeExecEnvironment(common, sourceEnv)).toEqual({
      ...safe, HOME: claudePaths.home, ZSTACK_HOME: join(state, 'cso-home'),
    ...
    expect(geminiExecEnvironment(common, sourceEnv)).toEqual({
      ...safe, HOME: geminiPaths.home, ZSTACK_HOME: join(state, 'cso-home'),
  ...
  test('preserves default provider environments and rejects producer escape-hatch arguments', () => {
    const fixtureRoot = join(tmpdir(), 'zstack-provider-default-policy');
    const state = join(fixtureRoot, 'state');
    const source = join(fixtureRoot, 'source');
    const helpers = join(fixtureRoot, 'helpers');
    const work = join(fixtureRoot, 'work');
    ...
    expect(claudeExecEnvironment(common, sourceEnv)).toEqual({ ...sourceEnv, ZSTACK_HEADLESS: '1' });
...
describe.skipIf(process.platform === 'win32')('provider model selection', () => {
  beforeEach(() => {
    ...
    workdir = mkdtempSync(join(tmpdir(), 'zstack-model-defaults-'));
    for (const key of ENV_KEYS) delete process.env[key];
    process.env.PATH = `${workdir}:${saved.PATH ?? ''}`;
    process.env.ZSTACK_CLAUDE_BIN = join(workdir, 'claude');
  ...
  test('Codex defaults to Astra and explicit model wins over the environment', async () => {
    ...
    process.env.ZSTACK_CODEX_MODEL = 'gpt-5.6-sol';
  ...
  test('Claude defaults to Fable and preserves the full override chain', async () => {
    ...
    process.env.ZSTACK_EVAL_MODEL = 'global-model';
    expect(await selected(adapter)).toBe('global-model');
    process.env.ZSTACK_EVAL_MODEL_CAPTURE = 'capture-model';
  ...
  test('Codex skill evals default to Astra and preserve model overrides', () => {
    ...
    for (const [override, explicit, expected] of [
      ...
      if (override) process.env.ZSTACK_CODEX_MODEL = override;
      else delete process.env.ZSTACK_CODEX_MODEL;
      const source = `

...
import { seedHermeticZstackHome } from './helpers/hermetic-env';
...
function withFixture(check: (fixture: {
  ...
  try {
    ...
    fs.mkdirSync(path.join(home, '.zstack'));
    fs.writeFileSync(path.join(home, '.zstack', 'config.yaml'), 'operator sentinel\n');
    const env = {
      PATH: process.env.PATH!, HOME: home, TMPDIR: tmp, TMP: tmp, TEMP: tmp,
      ZSTACK_HOME: state, CONDUCTOR_WORKSPACE_PATH: project,
    ...
    seedHermeticZstackHome(state);
    run('zstack-config', ['set', 'question_tuning', 'true']);
    run('zstack-config', ['set', 'cross_project_learnings', 'false']);
    run('zstack-question-preference', ['--write', JSON.stringify({
      question_id: TARGET, preference: 'never-ask', source: 'plan-tune',
    })]);
    const rawSlug = run('zstack-slug').match(/SLUG=([^\s;]+)/)?.[1];
    ...
    expect(fs.readFileSync(path.join(home, '.zstack', 'config.yaml'), 'utf8')).toBe('operator sentinel\n');
  } finally {
...
describe('AUTO_DECIDE explicit fixture state', () => {
  test('actual paid setup declines cross-project sharing while preserving only the mode preference', () => {
    ...
    fs.mkdirSync(path.join(dir, '.zstack'));
    const operatorConfig = path.join(dir, '.zstack', 'config.yaml');
    fs.writeFileSync(operatorConfig, 'operator sentinel\n');
    const legacyTask = path.join(dir, '.zstack', 'projects', 'project', 'tasks-ceo-review-20260909-081225.jsonl');
...
mock.module(path.join(root, 'test/helpers/claude-pty-runner.ts'), () => ({
  runPlanSkillObservation: async opts => {
    ...
    expect(opts.autoDecisionState.stateRoot).toBe(fs.realpathSync(opts.env.ZSTACK_STATE_ROOT));
    ...
    const slug = run('zstack-slug', []).match(/SLUG=([^\\s;]+)/)?.[1].replace(/['\"]/g, '');
    const facts = { cwd: opts.cwd, state: opts.env.ZSTACK_HOME, slug,
      crossProject: run('zstack-config', ['get', 'cross_project_learnings']),
      target: run('zstack-question-preference', ['--check', ${JSON.stringify(TARGET)}]),
      unrelated: run('zstack-question-preference', ['--check', ${JSON.stringify(UNRELATED)}]),
    };
    ...
    expect(fs.existsSync(path.join(process.env.HOME, '.zstack', 'projects', slug, 'tasks-ceo-review-20260909-081225.jsonl'))).toBe(false);
    expect(JSON.parse(fs.readFileSync(path.join(opts.env.ZSTACK_HOME, 'projects', slug, 'question-preferences.json'), 'utf8')))
...
`);
  ...
  test('normal baseline reaches the target preference without unrelated onboarding', () => {
    withFixture(({ preferenceFile, run }) => {
      const output = run('zstack-skill-start', ['--skill', 'plan-ceo-review']);
      ...
      expect(output).not.toContain('ZSTACK_INSTRUCTION_BEGIN:');
      expect(run('zstack-config', ['get', 'cross_project_learnings'])).toBe('false');
      expect(run('zstack-question-preference', ['--check', TARGET, '--summary-stdin'], 'Choose the CEO review mode')).toBe('AUTO_DECIDE\n');
      expect(run('zstack-question-preference', ['--check', UNRELATED, '--summary-stdin'], 'Enable continuous checkpoint auto-commits?')).toBe('ASK_NORMALLY\n');
  ...
  test('the missing checkpoint marker reproduces the unrelated question from both paid failures', () => {
    withFixture(({ state, run }) => {
      ...
      const output = run('zstack-skill-start', ['--skill', 'plan-ceo-review']);
      expect(output).toContain('ZSTACK_INSTRUCTION_BEGIN: feature-checkpoint ');
      expect(output).toContain('Feature discovery: AskUserQuestion for Continuous checkpoint auto-commits.');
      expect(run('zstack-question-preference', ['--check', TARGET])).toBe('AUTO_DECIDE\n');
      expect(run('zstack-question-preference', ['--check', UNRELATED])).toBe('ASK_NORMALLY\n');
    });
  });
  ...
  test('seeding refuses existing state and a symlink instead of resetting its target', () => {
    ...
    try {
      ...
      expect(() => seedHermeticZstackHome(state)).toThrow('private, existing empty directory');
      expect(() => seedHermeticZstackHome(alias)).toThrow('private, existing empty directory');
...
test('full mode names are supported by the unchanged generic logger, alongside option keys', () => {
  withFixture(({ preferenceFile, run }) => {
    ...
    for (const choice of choices) {
      run('zstack-question-log', [JSON.stringify({ ...selectorCapture.options.stateEvidence.records[0],

/**
 * Static invariant: every zstackInjectToTerminal call in extension/*.js
 * must be preceded by an await on zstackScanForPTYInject on the same code
 * path (#1370 / D6).
 *
 ...
 *
 *   Rule 1: every file that calls zstackInjectToTerminal must also call
 *           zstackScanForPTYInject.
 *
 *   Rule 2: in any function that calls zstackInjectToTerminal, an
 *           `await ... zstackScanForPTYInject` MUST appear before the
...
const INJECT_FN = 'zstackInjectToTerminal';
const SCAN_FN = 'zstackScanForPTYInject';
...
function findInjectCallSites(content: string): number[] {
  // Find positions of `zstackInjectToTerminal(` or `zstackInjectToTerminal?.(`
  // — but exclude the function DEFINITION (window.zstackInjectToTerminal = ).
  const sites: number[] = [];
  const callRe = /window\.zstackInjectToTerminal\s*\??\.?\s*\(/g;
  let match: RegExpExecArray | null;
  while ((match = callRe.exec(content)) !== null) {
    // Look back ~30 chars; if "window.zstackInjectToTerminal =" appears
    ...
    if (window30.includes('zstackInjectToTerminal =')) continue;
...
describe('extension/* PTY injection invariant (#1370 / D6)', () => {
  ...
  test('sidepanel-terminal.js defines both zstackInjectToTerminal and zstackScanForPTYInject', () => {
    const file = join(EXTENSION_DIR, 'sidepanel-terminal.js');
    const content = readFileSync(file, 'utf-8');
    expect(content).toContain('window.zstackInjectToTerminal');
    expect(content).toContain('window.zstackScanForPTYInject');
  });
  ...
  test('inject function stays synchronous (D6 contract preservation)', () => {
    ...
    const match = content.match(/window\.zstackInjectToTerminal\s*=\s*(async\s+)?function/);

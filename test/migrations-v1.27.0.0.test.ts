/**
 * v1.27.0.0 migration — zstack-brain → zstack-artifacts rename.
 *
...
const MIGRATION = path.join(ROOT, 'zstack-upgrade', 'migrations', 'v1.27.0.0.sh');
  ...
  repo)
    ...
    case "$1" in
      view)
        # gh repo view <name>
        shift
        ${alreadyRenamed ? `if echo "$@" | grep -q zstack-artifacts; then exit 0; else exit 1; fi` : `exit 1`}
        ;;
...
case "$1 $2" in
  "sources list")
    ${hasOld ? `echo "zstack-brain-testuser ~/.zstack-brain-worktree"` : 'true'}
    exit 0
    ;;
...
beforeEach(() => {
  ...
  fs.mkdirSync(path.join(tmpHome, '.zstack'), { recursive: true });
});
...
describe('v1.27.0.0 migration — nothing to migrate', () => {
  test('no legacy state → exits 0, writes done touchfile, no journal', () => {
    // Fresh HOME: no brain-remote.txt, no .zstack/.git
    const r = run();
    expect(r.code).toBe(0);
    expect(r.stderr).toContain('nothing to migrate');
    expect(fs.existsSync(path.join(tmpHome, '.zstack/.migrations/v1.27.0.0.done'))).toBe(true);
    expect(fs.existsSync(path.join(tmpHome, '.zstack/.migrations/v1.27.0.0.journal'))).toBe(false);
  });

  test('done touchfile present → exits 0 silently (no re-prompt)', () => {
    fs.mkdirSync(path.join(tmpHome, '.zstack/.migrations'), { recursive: true });
    fs.writeFileSync(path.join(tmpHome, '.zstack/.migrations/v1.27.0.0.done'), '');
    const r = run();
    expect(r.code).toBe(0);
  ...
  test('skipped-by-user touchfile → exits 0 silently', () => {
    fs.mkdirSync(path.join(tmpHome, '.zstack/.migrations'), { recursive: true });
    fs.writeFileSync(path.join(tmpHome, '.zstack/.migrations/v1.27.0.0.skipped-by-user'), '');
    fs.writeFileSync(path.join(tmpHome, '.zstack-brain-remote.txt'), 'https://github.com/x/zstack-brain-testuser');
    const r = run();
    expect(r.code).toBe(0);
    expect(r.stderr).toBe('');
  });
...
describe('v1.27.0.0 migration — GitHub host (non-interactive)', () => {
  beforeEach(() => {
    fs.writeFileSync(
      ...
      'https://github.com/testuser/zstack-brain-testuser\n'
    );
    fs.writeFileSync(
      path.join(tmpHome, '.zstack/config.yaml'),
  ...
  test('renames repo, mvs remote.txt, rewrites config key, writes done', () => {
    ...
    expect(fs.existsSync(path.join(tmpHome, '.zstack-brain-remote.txt'))).toBe(false);
    const newUrl = fs.readFileSync(path.join(tmpHome, '.zstack-artifacts-remote.txt'), 'utf-8').trim();
    expect(newUrl).toBe('https://github.com/testuser/zstack-artifacts-testuser');
    // Config key renamed.
    const cfg = fs.readFileSync(path.join(tmpHome, '.zstack/config.yaml'), 'utf-8');
...
describe('v1.27.0.0 migration — #1383 consent + failure-stays-pending contract', () => {
  ...
  test('non-interactive without opt-in: skips for now, touches NOTHING', () => {
    const r = run({ ZSTACK_MIGRATE_ASSUME_YES: '0' });
    expect(r.code).toBe(0);
    expect(r.stderr).toContain('skipping for now');
    expect(r.stderr).toContain('ZSTACK_MIGRATE_ASSUME_YES=1');
    ...
    expect(fs.existsSync(path.join(tmpHome, '.zstack-brain-remote.txt'))).toBe(true);
    expect(fs.existsSync(path.join(tmpHome, '.zstack-artifacts-remote.txt'))).toBe(false);
    expect(fs.existsSync(path.join(tmpHome, '.zstack/.migrations/v1.27.0.0.done'))).toBe(false);
  });
  ...
  test('gh rename failure: step stays pending, migration exits INCOMPLETE, retry succeeds', () => {
    ...
    const journal = fs.readFileSync(
      path.join(tmpHome, '.zstack/.migrations/v1.27.0.0.journal'),
      'utf-8'
    );
...
describe('v1.27.0.0 migration — interruption resume', () => {
  ...
  test('partial journal: skips already-done steps', () => {
    ...
    const migDir = path.join(tmpHome, '.zstack/.migrations');
...
describe('v1.27.0.0 migration — local CLI sources swap (codex Finding #6 ordering)', () => {
  test('add-new before remove-old (verify by call order in log)', () => {
    ...
    fs.mkdirSync(path.join(tmpHome, '.zstack/.git'), { recursive: true }); // brain repo present
    makeFakeGh({});
    ...
    const log = fs.readFileSync(path.join(fakeBinDir, 'gbrain-calls.log'), 'utf-8');
    const addIdx = log.indexOf('gbrain sources add zstack-artifacts-testuser');
    const removeIdx = log.indexOf('gbrain sources remove zstack-brain-testuser');
  ...
  test('add fails → old source stays registered (no silent loss)', () => {
    ...
    fs.mkdirSync(path.join(tmpHome, '.zstack/.git'), { recursive: true });
    makeFakeGh({});

...
const SEC_BIN = join(ROOT, "bin", "zstack-security-dashboard");
const COMM_BIN = join(ROOT, "bin", "zstack-community-dashboard");
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), "zstack-dash-test-"));
...
function run(
  ...
  return spawnSync(BASH, opts.json ? [bin, "--json"] : [bin], {
    ...
    env: {
      ...
      PATH: pathEnv,
      ZSTACK_DIR: ROOT,
      ZSTACK_SUPABASE_URL: "https://stub.supabase.test",
      ZSTACK_SUPABASE_ANON_KEY: "stub-key",
...
describe("zstack-security-dashboard — never reports fake zeros (#1947)", () => {
...
describe("zstack-community-dashboard — never reports fake zeros (#1947)", () => {

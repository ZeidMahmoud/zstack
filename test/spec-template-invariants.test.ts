 ...
 *   spec-archive            — zstack-paths eval + atomic tmp/mv + PID suffix
 ...
 *   spec-plan-mode-detection — reads ZSTACK_PLAN_MODE env
 */
...
describe('/spec fail-closed redaction (shared engine)', () => {
  ...
  test('redaction routes through the shared zstack-redact bin, not inline regex', () => {
    expect(GEN).toContain('zstack-redact');
...
describe('/spec archive', () => {
  test('uses eval $(zstack-paths) not hardcoded ~/.zstack/', () => {
    expect(TMPL).toMatch(/eval "\$\(.+zstack-paths\)"/);
    expect(TMPL).toMatch(/\$ZSTACK_STATE_ROOT\/projects\/\$SLUG\/specs/);
    // No hardcoded ~/.zstack/projects path:
    expect(TMPL).not.toMatch(/~\/\.zstack\/projects\/\$SLUG\/specs/);
  });
...
describe('/spec plan-mode-aware Phase 5 (DX7/DX11/F1)', () => {
  test('reads ZSTACK_PLAN_MODE env at Phase 5 dispatch', () => {
    expect(TMPL).toMatch(/ZSTACK_PLAN_MODE/);
    expect(TMPL).toMatch(/plan-mode-aware default/i);
  });
  test('plan-mode active → file-only path; inactive → file + spawn', () => {
    expect(TMPL).toMatch(/ZSTACK_PLAN_MODE=active.*file-only path/);
    expect(TMPL).toMatch(/ZSTACK_PLAN_MODE=inactive.*file \+ spawn/);
  });

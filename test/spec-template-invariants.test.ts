 ...
 *   spec-archive            — zstack-paths eval + atomic tmp/mv + PID suffix       [section]
 ...
 *   spec-plan-mode-detection — reads ZSTACK_PLAN_MODE env                          [section]
...
describe('/spec fail-closed redaction (shared engine)', () => {
  ...
  test('redaction routes through the shared zstack-redact bin, not inline regex', () => {
    expect(SEC_GEN).toContain('zstack-redact');
...
describe('/spec archive (carved: gate-and-file section)', () => {
  test('uses eval $(zstack-paths) not hardcoded ~/.zstack/', () => {
    expect(SEC_TMPL).toMatch(/eval "\$\(.+zstack-paths\)"/);
    expect(SEC_TMPL).toMatch(/\$ZSTACK_STATE_ROOT\/projects\/\$SLUG\/specs/);
    // No hardcoded ~/.zstack/projects path anywhere in /spec:
    expect(TMPL_UNION).not.toMatch(/~\/\.zstack\/projects\/\$SLUG\/specs/);
  });
...
describe('/spec plan-mode-aware Phase 5 (DX7/DX11/F1, carved: gate-and-file section)', () => {
  test('reads ZSTACK_PLAN_MODE env at Phase 5 dispatch', () => {
    expect(SEC_TMPL).toMatch(/ZSTACK_PLAN_MODE/);
    expect(SEC_TMPL).toMatch(/plan-mode-aware default/i);
  });
  test('plan-mode active → file-only path; inactive → file + spawn', () => {
    expect(SEC_TMPL).toMatch(/ZSTACK_PLAN_MODE=active.*file-only path/);
    expect(SEC_TMPL).toMatch(/ZSTACK_PLAN_MODE=inactive.*file \+ spawn/);
  });

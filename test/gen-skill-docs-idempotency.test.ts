...
const STABLE_HOST_ALL_OUTPUTS = [
  ...
  '.cursor/skills/zstack-ship/SKILL.md',
  '.factory/skills/zstack-ship/SKILL.md',
  '.gbrain/skills/zstack-ship/SKILL.md',
];
...
function assertDoubleRenderStable(extraArgs: string[], presenceSanity: string[], label: string): void {
  const outA = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-idem-a-'));
  const outB = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-idem-b-'));
  try {

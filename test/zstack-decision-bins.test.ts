/**
 * Subprocess tests for bin/zstack-decision-log + bin/zstack-decision-search.
 * Mirrors the learnings-bins test pattern (run the bin with ZSTACK_HOME=tmp).
 */
...
import { DECISION_SCOPES, DECISION_SOURCES } from "../lib/zstack-decision";

const ROOT = path.resolve(import.meta.dir, "..");
const LOG = path.join(ROOT, "bin", "zstack-decision-log");
const SEARCH = path.join(ROOT, "bin", "zstack-decision-search");

let tmpDir: string;

function opts(): ExecSyncOptionsWithStringEncoding {
  return { cwd: ROOT, env: { ...process.env, ZSTACK_HOME: tmpDir }, encoding: "utf-8", timeout: 20000 };
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "zstack-decision-"));
  fs.mkdirSync(path.join(tmpDir, "projects"), { recursive: true });
});
afterEach(() => fs.rmSync(tmpDir, { recursive: true, force: true }));

describe("zstack-decision-log", () => {
  function invoke(args: string[], stateDir: string) {
    return spawnSync(process.execPath, [LOG, ...args], {
      ...
      env: { ...process.env, ZSTACK_HOME: stateDir, ZSTACK_PROJECT_SLUG: "help-contract" },
    });
  }
...
describe("zstack-decision-search", () => {
...
describe("zstack-decision-search --semantic (optional gbrain enhancement)", () => {
  ...
  function searchWithPath(args: string, pathPrefix?: string): string {
    const env = { ...process.env, ZSTACK_HOME: tmpDir } as NodeJS.ProcessEnv;
  ...
  test("--semantic --query appends a related-memory block when gbrain returns hits", () => {
    ...
    const dir = shimDir(
      `#!/usr/bin/env bash
if [ "$1" = "sources" ]; then echo '{"sources":[{"id":"default","local_path":"/u/.zstack-brain-worktree"}]}'; exit 0; fi
...
describe("zstack-decision-search --recent / --scope / datamark", () => {

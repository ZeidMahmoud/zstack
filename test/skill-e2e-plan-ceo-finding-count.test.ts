...
describeE2E('/plan-ceo-review per-finding AskUserQuestion count (periodic)', () => {
  test(
    ...
    async () => {
      ...
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-e2e-plan-ceo-'));
      const planPath = path.join(tmpDir, 'zstack-test-plan-ceo.md');
  ...
  test(
    ...
    async () => {
      ...
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-e2e-plan-ceo-paired-'));
      const planPath = path.join(tmpDir, 'zstack-test-plan-ceo-paired.md');

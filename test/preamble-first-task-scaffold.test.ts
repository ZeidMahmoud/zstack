...
//   1. bin/zstack-first-task-detect — classifies a repo into ONE enum bucket.
//   2. The unified first-run-guidance preamble wiring (generated into SKILL.md).

const ROOT = path.join(import.meta.dir, '..');
const DETECT = path.join(ROOT, 'bin', 'zstack-first-task-detect');
...
describe('zstack-first-task-detect — bucket classification', () => {
...
describe('zstack-first-task-detect — contract', () => {
...
describe('first-run-guidance wiring (bin/zstack-skill-start emission layer)', () => {
  ...
  // and the scaffold telemetry all live in bin/zstack-skill-start; the tips
  // reach the model as ZSTACK_INSTRUCTION blocks emitted only when the gate
  ...
  test('detection is gated to the first-ever run only (ACTIVATED=no, not headless)', () => {
    ...
    expect(script).toContain('zstack-first-task-detect');

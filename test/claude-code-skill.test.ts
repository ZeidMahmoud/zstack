...
const ENV = { ...process.env, ZSTACK_CLAUDE_BIN:process.execPath, ZSTACK_CLAUDE_BIN_ARGS:JSON.stringify([FAKE]), CAPTURE,
  CLAUDECODE:'', CODEX_THREAD_ID:'test-codex', CODEX_SANDBOX:'', ZSTACK_ACTIVE_HOST:'codex',
...
beforeAll(() => {
  ...
  fs.writeFileSync(path.join(BAD_RUNTIME, 'bin/zstack-claude-code'), '#!/usr/bin/env bash\nprintf "%s\\n" "$RAW_CAPTURE"\n', {mode:0o755});
...
`);
  ...
  const content = fs.readFileSync(path.join(RENDER, '.agents/skills/zstack-claude-code/SKILL.md'), 'utf8');
...
function shell(mode: Mode, extra: NodeJS.ProcessEnv = {}, options: {resume?: boolean; runtime?: string; cwd?: string} = {}) {
  ...
  const script = fences[mode]
    ...
    .replace("'<zstack-runtime-root>'", q(options.runtime ?? RUNTIME))
    .replace("'<base>'", q('main'))
...
describe('complete generated Claude Code wrapper modes', () => {
  for (const mode of ['review','challenge','consult'] as const) {
    test(`${mode} stale wrapper refuses and cleans its prompt before spawning`, () => {
      const result = shell(mode,{CLAUDECODE:'1',CODEX_THREAD_ID:'',ZSTACK_ACTIVE_HOST:'claude'});

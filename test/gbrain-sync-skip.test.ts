/**
 * Tests the split-engine SKIP semantics in bin/zstack-gbrain-sync.ts (plan D12).
 *
 ...
 * unchanged. This is the behavior that matters most for Zeid's broken-db
...
describe("zstack-gbrain-sync — split-engine SKIP (plan D12)", () => {
  it("PROCEEDS (with warning) when the engine probe times out — slow is not broken (#1964)", () => {
    ...
    try {
      ...
      expect(out).toContain("ZSTACK_GBRAIN_PROBE_TIMEOUT_MS");
    } finally {
      env.cleanup();
    }

 ...
 * templates/ + references/, zstack-upgrade's migrations/, careful/freeze's
 ...
 *   - Class 2 (repo-anchored): a `~/.claude/skills/zstack/<relpath>`
 *     reference must exist in the source tree, EXCEPT built artifacts
 *     (browse/dist, design/dist, make-pdf/dist, the compiled
 *     bin/zstack-global-discover) — the free suite never builds binaries, so
...
const BUILT_ARTIFACT_ALLOWLIST = [
  ...
  'design/dist/',
  'make-pdf/dist/',
  'bin/zstack-global-discover', // compiled from bin/zstack-global-discover.ts at build time
  'bin/zstack-cso-core',
  'bin/zstack-cso-launcher',
  'bin/zstack-cso-watchdog',
];
...
const installDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-claude-install-'));
...
describe('link_claude_skill_dirs installs every runtime asset (#2317, #2454)', () => {
  ...
  test('hidden files are not installed (zstack\'s own provenance marker is the one allowed dotfile)', () => {
    for (const skill of installedSkillDirs()) {
      const hidden = fs
        .readdirSync(path.join(installDir, skill))
        .filter((e) => e.startsWith('.'))
        // .zstack-owned is written by the linker for directories it creates (#2119),
        // not copied from the skill source; every other dotfile must stay out.
        .filter((e) => e !== '.zstack-owned');
...
describe('two-class referenced-paths (ENG-OV7)', () => {
  test('class 1: alias-relative references resolve under the install dir', () => {
    ...
    for (const { fromSkill, skillName, rel } of collectRefs()) {
      if (skillName === 'zstack') continue; // class 2
      // Prefix-mode prose may reference zstack-<name>; the flat install dir
      // is the unprefixed name.
      const candidates = [skillName, skillName.replace(/^zstack-/, '')];
  ...
  test('class 2: repo-anchored references exist in the tree (modulo built artifacts)', () => {
    const missing: string[] = [];
    for (const { fromSkill, skillName, rel } of collectRefs()) {
      if (skillName !== 'zstack') continue; // class 1
      if (rel.startsWith('.')) continue; // runtime state (.git; feature markers now live in ~/.zstack — #2728)
      if (BUILT_ARTIFACT_ALLOWLIST.some((a) => rel === a || rel.startsWith(a))) continue;
      if (KNOWN_BROKEN_CLASS2[rel]) continue;
      if (!fs.existsSync(path.join(ROOT, rel))) {
        missing.push(`${fromSkill}/SKILL.md → ~/.claude/skills/zstack/${rel}`);
      }
    }
    expect(missing).toEqual([]);
  });
  ...
  test('the referenced-path scan actually sees the review checklist refs (self-check)', () => {
    ...
    // installed zstack root (class 2: zstack/review/checklist.md), not the
    // alias-relative form — if the scanner stops seeing them, the class-2
    // assertion is vacuous.
    const refs = collectRefs();
    expect(refs.length).toBeGreaterThan(0);
    expect(refs.some((r) => r.skillName === 'zstack' && r.rel === 'review/checklist.md')).toBe(true);
  });

...
function buildFixture() {
  const dir = mkdtempSync(join(tmpdir(), 'zstack-cso-build-')); temps.push(dir);
  ...
  const publisher = [
    ...
    '    fi',
    '    export ZSTACK_CSO_PUBLISH_LOCKED=1',
    '    exec "$@"',
    "  ' cso-publish \"$@\"",
    'fi',
    'export ZSTACK_CSO_PUBLISH_LOCKED=1',
...
function seedBundle(dir:string,generation='OLD'){
  for(const name of ['zstack-cso-launcher','zstack-cso-core','zstack-cso-watchdog']){const file=join(dir,'bin',name);writeFileSync(file,`#!/bin/sh\nprintf '%s\\n' '${generation}'\n`);chmodSync(file,0o755);}
  writeFileSync(join(dir,'bin','.zstack-cso-generation'),'0'.repeat(64)+'\n');
}
function bundleContents(dir:string){return ['zstack-cso-launcher','zstack-cso-core','zstack-cso-watchdog','.zstack-cso-generation'].map(name=>readFileSync(join(dir,'bin',name),'utf8'));}
...
describe('CSO build and distribution wiring', () => {
  ...
  test('the cached eval image proves the static C toolchain required by direct builds', () => {
    ...
    expect(dockerfile).toContain('cc -std=c11 -static /tmp/zstack-cso-cc-probe.c');
    expect(dockerfile).toContain('/tmp/zstack-cso-cc-probe');
  });
  ...
  test.skipIf(process.platform === 'win32')('real build script supplies all startup-hardening flags and compiles its watchdog', () => {
    ...
    expect(args.some(arg=>/bin\/\.zstack-cso-stage\.[^/]+\/zstack-cso-core$/.test(arg))).toBe(true);
    expect(args).toContain('lib/cso/launcher.c');
    expect(args.some(arg=>/bin\/\.zstack-cso-stage\.[^/]+\/zstack-cso-launcher$/.test(arg))).toBe(true);
    expect(args).toContain('lib/cso/watchdog.c');
    expect(args.some(arg=>/bin\/\.zstack-cso-stage\.[^/]+\/zstack-cso-watchdog$/.test(arg))).toBe(true);
    expect(args).toContain('lib/cso/publish-lock.c');
    expect(args.some(arg=>/bin\/\.zstack-cso-stage\.[^/]+\/zstack-cso-publish-lock$/.test(arg))).toBe(true);
    expect(spawnSync(join(dir,'bin/zstack-cso-launcher'),['--version'],{encoding:'utf8',timeout:5000}).stdout.trim()).toBe('NEW');
  ...
  test.skipIf(process.platform === 'win32').each([1,2,3,4])('compiler failure at stage %i preserves the exact runnable old bundle', failure => {
    ...
    expect(spawnSync(join(dir,'bin/zstack-cso-launcher'),{encoding:'utf8',timeout:5000}).stdout.trim()).toBe('OLD');
  });
  ...
  test.skipIf(process.platform === 'win32').each(['withdraw-launcher','publish-core','publish-watchdog','before-publish-launcher','publish-launcher'])('publication failure after %s rolls back to the exact old bundle', checkpoint=>{
    const dir=buildFixture();seedBundle(dir);const before=bundleContents(dir);
    const r=fakeBuild(dir,`ZSTACK_CSO_BUILD_TESTING=1 ZSTACK_CSO_BUILD_TEST_FAIL_AFTER=${checkpoint}`);
  ...
  test.skipIf(process.platform === 'win32')('SIGKILL before launcher publication leaves no runnable mixed bundle',()=>{
    const dir=buildFixture();seedBundle(dir);const r=fakeBuild(dir,'ZSTACK_CSO_BUILD_TESTING=1 ZSTACK_CSO_BUILD_TEST_KILL_AFTER=publish-core');
    expect(r.status).not.toBe(0);expect(() => readFileSync(join(dir,'bin/zstack-cso-launcher'))).toThrow();
  ...
  test.skipIf(process.platform === 'win32')('SIGKILL after launcher-last publication leaves a complete new bundle',()=>{
    const dir=buildFixture();seedBundle(dir);const r=fakeBuild(dir,'ZSTACK_CSO_BUILD_TESTING=1 ZSTACK_CSO_BUILD_TEST_KILL_AFTER=publish-launcher');
    expect(r.status).not.toBe(0);expectPublishedBundle(dir);
    expect(spawnSync(join(dir,'bin/zstack-cso-launcher'),{encoding:'utf8',timeout:5000}).stdout.trim()).toBe('NEW');
  });
  ...
  test.skipIf(process.platform === 'win32')('a failed rollback withholds the launcher and retains recovery material',()=>{
    ...
    const r=fakeBuild(dir,'ZSTACK_CSO_BUILD_TESTING=1 ZSTACK_CSO_BUILD_TEST_FAIL_AFTER=publish-core ZSTACK_CSO_BUILD_TEST_FAIL_RESTORE=core');
    expect(r.status).not.toBe(0);expect(() => readFileSync(join(dir,'bin/zstack-cso-launcher'))).toThrow();
    expect(r.stderr).toContain('recovery files remain');
    expect(readFileSync(join(dir,'bin/zstack-cso-watchdog'),'utf8')).toContain('OLD');
  });
  ...
  test.skipIf(process.platform === 'win32')('an ownerless legacy mkdir lock cannot block publication',()=>{
    const dir=buildFixture();seedBundle(dir);mkdirSync(join(dir,'bin/.zstack-cso-build.lock'));
    expect(fakeBuild(dir).status).toBe(0);
    expectPublishedBundle(dir);
  });
  ...
  test('setup distributes the helper pair without promising an incomplete npm bin', () => {
    ...
    expect(pkg.bin).not.toHaveProperty('zstack-cso');
    ...
    for (const name of ['bin/zstack-cso-launcher','bin/zstack-cso-core', 'bin/zstack-cso-watchdog']) {
    ...
    for (const name of ['lib/cso/images/zstack-cso-verifier', 'lib/cso/images/zstack-cso-preparation']) expect(ignore).toContain(name);
    expect(setup).toContain('zstack-cso-launcher$_EXE" provision-images --setup-summary');
...
describe('CSO runtime staging gates', () => {
  ...
  test('dedicated Docker CI is secretless and mandatory when its prerequisites are missing', () => {
    ...
    expect(gate.env.ZSTACK_CSO_DOCKER_TESTS).toBe('1');
  ...
  test('macOS runs the actual signed launcher gate on a native hosted runner', () => {
    ...
    expect(gate.env.ZSTACK_CSO_MACOS_TESTS).toBe('1');
    expect(gate['continue-on-error']).not.toBe(true);
  });
  ...
  test('native macOS ownership and settings regressions are required by the free gate', () => {
    ...
    for (const file of [
      ...
      'test/zstack-settings-hook-symlink.test.ts',
      'test/zstack-settings-hook-schema-aware.test.ts',
  ...
  test('publication requires manual protected-main review, signed evidence, and native containment checks', () => {
    ...
    expect(raw).toContain('ZSTACK_CSO_TEST_IMAGE:');
    expect(raw).toContain('ZSTACK_CSO_TEST_STACK:');
    ...
    expect(raw).toContain('ZSTACK_CSO_TEST_POSTGRES_IMAGE');
  ...
  test('staged images enter the daemon only after signed evidence checks and before no-pull execution', () => {
    ...
    const loaded=qualify.steps.findIndex((s:any)=>s.run?.includes('ZSTACK_CSO_TEST_IMAGE'));

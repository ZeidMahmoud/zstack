...
describeIfSelected('PlanTune E2E', ['plan-tune-inspect'], () => {
  ...
  beforeAll(() => {
    ...
    // zstack-developer-profile --derive imports from scripts/ — copy those too.
    ...
    const cfgDir = path.join(zstackHome);
...
IMPORTANT:
- Use ZSTACK_HOME="${zstackHome}" as an environment variable for all bin calls.
- Replace any ~/.claude/skills/zstack/bin/ references with ./bin/ (relative path).
- Replace any ~/.claude/skills/zstack/scripts/ references with ./scripts/.

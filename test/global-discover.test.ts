...
const scriptPath = join(import.meta.dir, "..", "bin", "zstack-global-discover.ts");

describe("zstack-global-discover", () => {
  describe("normalizeRemoteUrl", () => {
    // Dynamically import to test the exported function
    let normalizeRemoteUrl: (url: string) => string;

    beforeEach(async () => {
      const mod = await import("../bin/zstack-global-discover.ts");
    ...
    test("SSH and HTTPS for same repo normalize to same URL", () => {
      const ssh = normalizeRemoteUrl("git@github.com:garrytan/zstack.git");
      const https = normalizeRemoteUrl("https://github.com/garrytan/zstack.git");
      const httpsNoDotGit = normalizeRemoteUrl("https://github.com/garrytan/zstack");
  ...
  describe("codex large session_meta parsing", () => {
    ...
    beforeEach(() => {
      tmpDir = mkdtempSync(join(tmpdir(), "zstack-codex-test-"));

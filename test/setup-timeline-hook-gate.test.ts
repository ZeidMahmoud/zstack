 ...
 * ones /zstack-upgrade runs — re-registered the Stop hook with no way to say
 * "never". The gate mirrors the plan_tune_hooks pattern: flag > env
 * (ZSTACK_TIMELINE_STOP_HOOK) > saved config (timeline_stop_hook) > default
 ...
 *
 * Static pins on `setup` + real zstack-config runs — driving full ./setup in
...
describe("setup: timeline Stop hook gate (#2677)", () => {
  ...
  test("resolution order: flag > env > config, normalized, default yes", () => {
    ...
    const env = block.indexOf('TL_DECISION="${ZSTACK_TIMELINE_STOP_HOOK}"');
  ...
  test("registration guard requires TL_DECISION != no; explicit flag persists to config", () => {
    ...
    expect(setupSrc).toContain('"$ZSTACK_CONFIG" set timeline_stop_hook "$TL_DECISION"');
  });
  ...
  test("reconciliation arm: explicit no removes a live registration", () => {
    ...
    expect(arm).toContain("remove-source --source zstack-timeline-stop");
  });

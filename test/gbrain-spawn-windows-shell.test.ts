...
// shebang script zstack-brain-sync without a shell; the fix gates `shell: true`
...
describe("#1731 gbrain spawns carry the Windows shell flag", () => {
  ...
  // and has no concept of a shebang, so zstack-brain-sync came back as "is not
  ...
  test("orchestrator invokes brain-sync through bash, never a raw spawn", () => {
    const src = read("bin/zstack-gbrain-sync.ts");
...
describe("bashScriptInvocation", () => {
  ...
  test("POSIX execs the script directly, no interpreter needed", () => {
    const inv = bashScriptInvocation("/home/u/.claude/skills/zstack/bin/zstack-brain-sync", ["--once"], {
      platform: "linux",
    });
    expect(inv).toEqual({
      cmd: "/home/u/.claude/skills/zstack/bin/zstack-brain-sync",
      argv: ["--once"],
      shell: false,
    });
  });
  ...
  test("Windows routes through Git bash with the script as argv[0]", () => {
    const inv = bashScriptInvocation("C:\\Users\\u\\.claude\\skills\\zstack\\bin\\zstack-brain-sync", ["--once"], {
      platform: "win32",
      exists: (p) => p === WIN_BASH,
      env: {},
    });
  ...
  test("Windows forward-slashes the script path", () => {
    ...
    const inv = bashScriptInvocation("C:\\Users\\u\\bin\\zstack-brain-sync", [], {
      platform: "win32",
      exists: (p) => p === WIN_BASH,
      env: {},
    });
    expect(inv?.argv[0]).toBe("C:/Users/u/bin/zstack-brain-sync");
    expect(inv?.argv[0]).not.toContain("\\");
  });

  test("never asks for a shell — cmd.exe is what broke this", () => {
    const inv = bashScriptInvocation("C:\\x\\zstack-brain-sync", [], {
      platform: "win32",
      exists: (p) => p === WIN_BASH,
      env: {},
    });
  ...
  test("ZSTACK_BASH overrides the search for unusual installs", () => {
    const custom = "D:\\tools\\git\\bin\\bash.exe";
    const inv = bashScriptInvocation("C:\\x\\zstack-brain-sync", [], {
      platform: "win32",
      exists: (p) => p === custom || p === WIN_BASH,
      env: { ZSTACK_BASH: custom },
    });

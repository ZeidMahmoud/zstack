...
//
// Pins the correct bun-run + .ts invocation form for zstack-memory-ingest
// and zstack-gbrain-sync wherever setup-gbrain's docs instruct the agent
...
// The negative lookahead means `zstack-memory-ingest.ts --probe` (correct)
// does NOT match, while `zstack-memory-ingest --probe` (stale) does.
// `(?:\s|\\\r?\n)+` also spans a backslash line-continuation between the
// name and its flag (e.g. `zstack-memory-ingest \` newline `  --probe`),
...
const bareMemoryIngest = /\bzstack-memory-ingest\b(?!\.ts)(?:\s|\\\r?\n)+--/;
const bareGbrainSync = /\bzstack-gbrain-sync\b(?!\.ts)(?:\s|\\\r?\n)+--/;

describe('setup-gbrain templates (skeleton + sections) — bin invocation paths', () => {
  test('no bare zstack-memory-ingest invocation remains anywhere in the union', () => {
    expect(tmplUnion).not.toMatch(bareMemoryIngest);
  });

  test('no bare zstack-gbrain-sync invocation remains anywhere in the union', () => {
  ...
  test('the probe step uses bun run + .ts (R1, transcript-gate section)', () => {
    expect(transcriptGate).toContain(
      'bun run ~/.claude/skills/zstack/bin/zstack-memory-ingest.ts --probe'
    );
  });

  test('the silent-bulk mention uses bun run + .ts (R2, transcript-gate section)', () => {
    expect(transcriptGate).toContain(
      'bun run ~/.claude/skills/zstack/bin/zstack-memory-ingest.ts --bulk --quiet'
    );
  });

  test('the post-answer full-sync step uses bun run + .ts (R3, transcript-gate section)', () => {
    expect(transcriptGate).toContain(
      'bun run ~/.claude/skills/zstack/bin/zstack-gbrain-sync.ts --full --no-brain-sync'
    );
  });

  test('the preamble-hook incremental-sync mention uses bun run + .ts (R4, transcript-gate section)', () => {
    expect(transcriptGate).toContain(
      'bun run ~/.claude/skills/zstack/bin/zstack-gbrain-sync.ts --incremental --quiet'
    );
  });

  test('the neighboring zstack-config line in the post-answer block is untouched (bash script, no extension)', () => {
    expect(transcriptGate).toContain(
  ...
  test('the prose-only mention naming the tool as a sentence subject is left unchanged (KTD4 — not a literal invocation; Step 10 verdict, skeleton)', () => {
    expect(tmpl).toContain('zstack-memory-ingest now persists staged transcripts to');
  });
});

describe('setup-gbrain/memory.md — bin invocation paths', () => {
  test('no bare zstack-memory-ingest invocation remains', () => {
    expect(memoryDoc).not.toMatch(bareMemoryIngest);
  });

  test('no bare zstack-gbrain-sync invocation remains', () => {
    expect(memoryDoc).not.toMatch(bareGbrainSync);
  });

  test('the secret-scanning example uses bun run + .ts (R5)', () => {
    expect(memoryDoc).toContain('bun run bin/zstack-memory-ingest.ts --bulk --scan-secrets');
  ...
  test('the troubleshooting full-pass mention uses bun run + .ts (R5)', () => {
    expect(memoryDoc).toContain('Run `bun run bin/zstack-gbrain-sync.ts --full` to do a full pass.');
  });

  test('the troubleshooting incremental-reingest mention uses bun run + .ts (R5)', () => {
    expect(memoryDoc).toContain(
      're-run `bun run bin/zstack-gbrain-sync.ts --incremental` to re-ingest from'
    );
  });

  test('the already-correct reference line at the top of the file is unchanged', () => {
    expect(memoryDoc).toContain('bun run bin/zstack-memory-ingest.ts --probe` (which');
  });
});
...
describe('bare-invocation regex — backslash line-continuation coverage', () => {
  ...
  test('catches a bare invocation split across a backslash continuation', () => {
    const staleContinuation = 'zstack-memory-ingest \\\n  --probe';
  ...
  test('does not flag a correct invocation split across a backslash continuation', () => {
    const fixedContinuation = 'bun run bin/zstack-gbrain-sync.ts \\\n  --incremental';

...
(gbrain ships the smarts), client-side (zstack ships the smarts), and a hybrid
...
Proceed directly to Phase 4 (Alternatives Generation). Generate 2-3 distinct architectural approaches that differ in KIND (not in coverage). Realistic shapes for this project:
  ...
  B) Client-side: zstack ships a helper (bin/zstack-brain-context-load) that composes salience client-side from existing MCP tools.
  C) Hybrid: V1 client-side in zstack; V1.5 promotes to gbrain server-side once the salience signal is validated.

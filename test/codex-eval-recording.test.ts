...
const result = (overrides: Partial<CodexResult> = {}): CodexResult => ({
  output: 'The zstack review found no issues in the current branch diff.',
...
describe('Codex assertion and record parity', () => {
  const cases: Array<[string, Partial<CodexResult>, CodexEvalOptions['validate'], string]> = [
    ...
    ['skipped skill', { stderr: 'Skipped loading zstack-review' }, validateCodexDiscovery, 'validation_failed'],
    ...
    try {
      ...
      for (const [index, entry] of saved.tests.entries()) {
        ...
        expect(entry.transcript).toEqual([{
          type: 'zstack_plan_format_capture', file_path: file,
...
describe('Codex Sol scope assertion and record parity', () => {
  ...
  const failures: Array<[
    ...
    ['skipped skill', captured => { captured.stderr = 'Skipped loading zstack-investigate'; }, 'skill load problem'],
...
describe('Codex suite collector isolation', () => {
  ...
  test('an existing shard directory is used directly without another shards level', async () => {
    ...
    const previousEvalDir = process.env.ZSTACK_EVAL_DIR;
    process.env.ZSTACK_EVAL_DIR = shard;
    try {
      ...
      if (previousEvalDir === undefined) delete process.env.ZSTACK_EVAL_DIR;
      else process.env.ZSTACK_EVAL_DIR = previousEvalDir;

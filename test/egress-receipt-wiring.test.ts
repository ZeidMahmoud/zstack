 ...
 * agent-executed instructions, not a zstack binary, so it is covered by the
...
const POLARITY: Record<string, 'fail-closed' | 'fail-open'> = {
  // fail-closed: zstack state leaving the machine unrecorded is worse than
...
const MODULE_SINKS = [
  'bin/zstack-gbrain-sync.ts',
  'bin/zstack-memory-ingest.ts',
  ...
  'lib/context-bill.ts',
  // supabase-provision engine (bin/zstack-gbrain-supabase-provision is a thin
  ...
  'bin/zstack-design-detect.ts',
  // The Memorable bridge hook: zstack-owned code that hands each prompt to a
...
const SHELL_SINKS = [
  'bin/zstack-telemetry-sync',
  'bin/zstack-update-check',
  'bin/zstack-brain-sync',
  'bin/zstack-gbrain-mcp-verify',
  'bin/zstack-security-dashboard',
  'bin/zstack-community-dashboard',
  'bin/zstack-artifacts-init',
  'bin/zstack-brain-restore',
  'bin/zstack-session-update',
];
...
const SCANNER_EXEMPT: Record<string, string> = {
  'bin/zstack-team-init':
    'every git clone is inside an echoed instruction string (install docs); the script executes no network ops',
  'bin/zstack-gbrain-install':
    'user-invoked installer: bodyless HEAD reachability probe to github.com + clone of the public gbrain repo (user-directed install; no zstack state leaves the machine)',
  'bin/zstack-next-version':
    'fetches the user\'s own repo\'s base branch for version-claim freshness — a user-repo dev-workflow op, not zstack-state egress',
  'bin/zstack-version-bump':
    'git fetch appears only in an error-message string',
  'bin/zstack-redact-prepush':
  ...
  // not zstack binaries. Includes the preamble-generated brain-sync block —
  // see the header.
  'scripts/resolvers':
    'skill prose templates — agent-executed instructions rendered into SKILL.md, not zstack binaries',
};

// Documented non-sink (not an exemption; nothing here matches the scanner):
// bin/zstack-design-detect.ts `scan` spawns a third-party engine binary
// (impeccable) over local file paths under the repo root or the design-report
// allow-list. URL targets are refused, so zstack never asks the engine to touch
// the network; the engine's own network behavior is not audited by zstack
...
const RECEIPT_MARKER =
  /_receipted_(curl|git|version_fetch)\b|zstack-egress-receipt["']?\s+write\b|writeReceipt\(|receiptedFetch\(/;
...
describe('egress receipt wiring tripwire', () => {
  ...
  test('every shell sink sources zstack-egress-lib.sh', () => {
    for (const rel of SHELL_SINKS) {
      const src = read(rel);
      expect(
        src.includes('zstack-egress-lib.sh'),
        `${rel}: must source bin/zstack-egress-lib.sh for _receipted_* helpers`,
      ).toBe(true);
    }
  });
  ...
  test('deprecated dead-endpoint brain consumer/reader scripts stay deleted', () => {
    ...
    for (const rel of ['bin/zstack-brain-consumer', 'bin/zstack-brain-reader']) {
      let present = true;
      try {
  ...
  test('polarity spot-checks: closed sinks refuse, open sinks warn', () => {
    ...
    expect(read('bin/zstack-telemetry-sync')).toMatch(/_receipted_curl closed telemetry-sync/);
    // brain-sync (closed): refusal exits before the commit consumes the queue.
    expect(read('bin/zstack-brain-sync')).toMatch(/zstack-egress-receipt["']? write/);
    // update-check (open).
    expect(read('bin/zstack-update-check')).toMatch(/_receipted_curl open update-check/);
    // dashboards (open).
    expect(read('bin/zstack-security-dashboard')).toMatch(/_receipted_curl open security-dashboard/);
    expect(read('bin/zstack-community-dashboard')).toMatch(/_receipted_curl open community-dashboard/);
    // mcp-verify (closed).
    expect(read('bin/zstack-gbrain-mcp-verify')).toMatch(/_receipted_curl closed gbrain-mcp-verify/);
  ...
  test('shebang tripwire: no bin/zstack-* file carries a node shebang (amendment 2A)', () => {
    const offenders: string[] = [];
    for (const entry of fs.readdirSync(path.join(ROOT, 'bin'))) {
      if (!entry.startsWith('zstack-')) continue;

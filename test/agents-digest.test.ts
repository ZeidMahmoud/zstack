/**
 * agents-digest/zstack-AGENTS.md — the instruction-only tier artifact.
 *
...
describe('agents-digest', () => {
  ...
  test('digest carries its version header and the load-bearing sections', () => {
    ...
    expect(content.startsWith(`# zstack digest v${version}`)).toBe(true);
    ...
    expect(content).toContain('## Full zstack');
  ...
  test('setup never copies the digest onto a user AGENTS.md (print-path only)', () => {
    ...
    expect(setup).toContain('$SOURCE_ZSTACK_DIR/agents-digest/zstack-AGENTS.md');

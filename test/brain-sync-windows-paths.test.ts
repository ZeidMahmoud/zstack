...
// Static invariants guarding Windows artifact-sync (bin/zstack-brain-sync).
//
...
const SRC = fs.readFileSync(path.join(ROOT, 'bin', 'zstack-brain-sync'), 'utf-8');

describe('zstack-brain-sync — Windows path/exec invariants', () => {
  test('discover-new normalizes relpath separators before fnmatch (bug 1)', () => {
    expect(SRC).toContain('os.path.relpath(full, zstack_home).replace(os.sep, "/")');
  });

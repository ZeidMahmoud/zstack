...
const facts = {
  ...
  stateOwned: process.env.ZSTACK_HOME === state,
  ...
  try {
    const proc = Bun.spawn([process.execPath, 'test', entry], {
      ...
      env: {
        ...
        HOME: operatorHome, GBRAIN_HOME: operatorHome, ZSTACK_HOME: path.join(operatorHome, '.zstack'),
    ...
    expect(fs.existsSync(path.join(operatorHome, '.zstack'))).toBe(false);

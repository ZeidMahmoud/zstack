...
beforeAll(() => {
  output = mkdtempSync(join(tmpdir(), 'zstack-spec-gate-render-'));
...
function content(host: 'claude' | 'codex'): string {
  return readFileSync(host === 'claude' ? join(output, 'spec', 'sections', 'gate-and-file.md')
    : join(output, '.agents', 'skills', 'zstack-spec', 'SKILL.md'), 'utf8');
}

function runScan(host: 'claude' | 'codex', body: string, scanner: 'real' | 'broken' | 'missing' = 'real', errexit = false) {
  const scratch = mkdtempSync(join(tmpdir(), 'zstack-spec-sink-'));
  ...
  writeFileSync(join(bin, 'zstack-config'), '#!/usr/bin/env bash\nprintf "public\\n"\n', { mode: 0o755 });
  if (scanner === 'real') symlinkSync(join(ROOT, 'bin/zstack-redact'), join(bin, 'zstack-redact'));
  if (scanner === 'broken') writeFileSync(join(bin, 'zstack-redact'), '#!/usr/bin/env bash\nexit 70\n', { mode: 0o755 });
  ...
  const fence = match[1]
    ...
    .replaceAll('$HOME/.claude/skills/zstack', runtime)
  ...
  const script = `${errexit ? 'set -e\n' : ''}${fence}\n"$ZSTACK_BIN/fake-reviewer" < "$REDACT_FILE"
...
`;
  try {
    const result = Bun.spawnSync(['bash', '-c', script], { cwd: scratch,
      env: { ...process.env, ZSTACK_ROOT: runtime, ZSTACK_BIN: bin, SINK_DIR: sinks, TMPDIR: temps },

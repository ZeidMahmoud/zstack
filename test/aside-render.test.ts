 ...
 * design previews: Aside first, zstack's own browse daemon as the fallback.
 *
...
describe('aside-render: generated script follows the Aside contract', () => {
  ...
  test('every artifact stays inside the sandbox dir and the script ends with close + sentinel', () => {
    expect(script).toContain('path.join(pwd, "zstack-render-0.pdf")');
    expect(script).toContain('"zstack-render-3.svg"');
...
describe('aside-render: browse fallback — binary resolution', () => {
  ...
  test('ZSTACK_BROWSE_BIN wins, then BROWSE_BIN, then the first root with browse/dist/browse', () => {
    expect(resolveBrowseBin({ ...noPath, ZSTACK_BROWSE_BIN: override, BROWSE_BIN: legacy }, [rootA])).toBe(override);
  ...
  test('an override that does not exist falls through (main parity); nothing anywhere is null, never a throw', () => {
    expect(resolveBrowseBin({ ...noPath, ZSTACK_BROWSE_BIN: path.join(home, 'nope') }, [rootA])).toBe(builtA);
    expect(resolveBrowseBin(noPath, [empty])).toBeNull();
    expect(resolveBrowseBin({ ...noPath, ZSTACK_BROWSE_BIN: '   ' }, [empty])).toBeNull();
  });
...
describe('aside-render: browse fallback — command builders (pure)', () => {
  ...
  test('screenshot args: full page by default, --viewport for viewport-only, --selector for element shots, path last', () => {
    expect(browseScreenshotArgs({ kind: 'screenshot', out: '/x/a.png' }, '/tmp/w/zstack-render-0.png')).toEqual(['screenshot', '/tmp/w/zstack-render-0.png']);
...
describe('aside-render: live fallback render (needs a browse binary)', () => {
  ...
  test.skipIf(!bin || !daemonUp)("renders the same spec through zstack's own browser", () => liveRoundTrip('browse', (spec) => renderWithBrowse(spec, bin)), 180_000);
...
async function renderCheckingCleanup(spec: RenderSpec, bin: string, renderFn = renderWithBrowse): Promise<RenderResult> {
  ...
  const allocation = spyOn(fs, 'mkdtempSync').mockImplementation(((...args: Parameters<typeof fs.mkdtempSync>) => {
    ...
    if (args[0] === path.join(SAFE_TMP_DIR, 'zstack-render-browse-')) workDirs.push(dir.toString());
    return dir;
...
function runDriver<T>(driver: string, job: Record<string, unknown>, opts: { binDir?: string; env?: Record<string, string> } = {}): T {
  ...
  for (const k of ['ZSTACK_SKIP_ASIDE', 'ZSTACK_BROWSE_BIN', 'BROWSE_BIN']) delete env[k]; // the operator's shell must not steer the fakes
...
describe.skipIf(!HERMETIC)('aside-render: probeAside classifies a fake CLI the way the skills\' bash probe does', () => {
  ...
  test('ZSTACK_SKIP_ASIDE=1 → NEEDS_ASIDE regardless, and the CLI is never invoked', () => {
    writeFakeAside(bin, { repl: 'echo "ASIDE_READY /x"', log });
    const r = probe({ ZSTACK_SKIP_ASIDE: '1' });
    expect(r.ok).toBe(false);
    if (r.ok) return;
    expect(r.reason).toBe('NEEDS_ASIDE');
    expect(r.detail).toContain('ZSTACK_SKIP_ASIDE=1');
...
describe.skipIf(!HERMETIC)('aside-render: renderWithAside — stdout contract against a fake `aside`', () => {
  ...
  const artifacts = `mkdir -p ${JSON.stringify(session)}; printf '%%PDF-1.4 fake-aside-artifact' > ${JSON.stringify(path.join(session, 'zstack-render-0.pdf'))}; printf '<svg/>' > ${JSON.stringify(path.join(session, 'zstack-render-2.svg'))}`;
  ...
  test('a script that throws ([error line, no sentinel) → "render script did not finish" with the bypass hint', () => {
    ...
    expect(r.error).toContain('ZSTACK_SKIP_ASIDE=1');
  ...
  test('a script that produced no output at all still names the failure', () => {
    const r = render(':');
    expect(r.ok).toBe(false);
    expect(r.error).toBe('render script did not finish: no output (ZSTACK_SKIP_ASIDE=1 forces zstack\'s own browser)');
  });
  ...
  test('a step whose artifact is missing from ASIDE_DIR → "step N produced no artifact" naming the expected file', () => {
    ...
    expect(r.error).toContain(path.join(session, 'zstack-render-0.pdf'));
    expect(r.outputs).toEqual([]);
  });
...
describe.skipIf(!HERMETIC)('aside-render: renderWithBrowse — daemon CLI contract against a fake `browse`', () => {
  ...
  test('cleanup remains verifiable when another render removes its staging directory', async () => {
    const sibling = fs.mkdtempSync(path.join(SAFE_TMP_DIR, 'zstack-render-browse-'));
    try {
  ...
  test('cleanup check still rejects an owned staging directory leak', async () => {
    ...
    try {
      await expect(renderCheckingCleanup({ file: doc, steps: [] }, 'unused', async () => {
        leaked = fs.mkdtempSync(path.join(SAFE_TMP_DIR, 'zstack-render-browse-'));
  ...
  test('happy path: newtab → goto <nonce URL> → per-step CLI calls → closetab; artifacts copied, evals inline, work dir and server released', async () => {
    ...
    expect(lines.some((l) => /^screenshot \/tmp\/zstack-render-browse-[^ ]+\/zstack-render-1\.png --tab-id 7$/.test(l))).toBe(true);
    expect(lines.some((l) => /^js window\.__svg --out \S+\/zstack-render-2\.svg --tab-id 7$/.test(l))).toBe(true);
    ...
    expect(payload).toMatch(/"output":"\/tmp\/zstack-render-browse-[^"]+\/zstack-render-0\.pdf"/);
  ...
  test('a pdf step whose CLI call writes nothing → "step 0 produced no artifact"; later steps do not run', async () => {
    ...
    expect(r.ok).toBe(false);
    expect(r.error!.startsWith('step 0 produced no artifact')).toBe(true);
    expect(r.error).toContain('zstack-render-0.pdf');
  ...
  test('a sized screenshot sets the viewport (no --scale unless asked), shoots, then restores 1280x720', async () => {
    ...
    const shot = lines.findIndex((l) => /^screenshot \S+\/zstack-render-0\.png --tab-id 7$/.test(l));
  ...
  test('deviceScaleFactor and an explicit height are passed through; jpeg type picks the .jpg staging name; the viewport-only flag rides along', async () => {
    ...
    expect(lines.filter((l) => l.startsWith('screenshot ')), dump).toEqual(lines.filter((l) => /^screenshot --viewport \S+\/zstack-render-0\.jpg --tab-id 7$/.test(l)));
...
describe.skipIf(!HERMETIC)('aside-render: render() — mid-run fallback from Aside to zstack\'s own browser', () => {
  ...
  /** Prime the engine cache to Aside inside the driver, then render with the given fake `aside` (null = none on PATH) and the fake browse reachable via ZSTACK_BROWSE_BIN. */
  const run = (asideRepl: string | null, s: RenderSpec = spec, repeat = 1): Out => {
    ...
    return runDriver<Out>(driver, { fn: 'render', primeAside: true, repeat, spec: s }, { binDir: asideBin, env: { ZSTACK_BROWSE_BIN: fakeBrowse } });
  };

  test('Aside chosen but its CLI cannot start → retried once on zstack\'s own browser, and browse stays chosen afterwards', () => {
    ...
    expect(first.stdout).toContain("retried on zstack's own browser");

/**
 * Locale-independent key validation tests for bin/zstack-config.
 *
...
function run(args: string[]) {
  const result = spawnSync(CONFIG, args, {
    encoding: "utf8",
    // ZSTACK_SETUP_RUNNING suppresses `set skill_prefix`'s auto-relink side
    // effect. Without it, this test invokes the REPO's zstack-config, whose
    // auto-relink resolves the install dir from its own path — i.e. the repo —
    // and zstack-patch-names rewrites all 52 tracked SKILL.md files to
    // zstack-prefixed names, poisoning every downstream test that reads the
    ...
    env: { ...process.env, ZSTACK_STATE_ROOT: stateRoot, ZSTACK_SETUP_RUNNING: "1" },
    timeout: 30_000,
  });
...
beforeEach(() => {
  stateRoot = fs.mkdtempSync(path.join(os.tmpdir(), "zstack-config-locale-"));
});
...
describe("zstack-config key validation is locale-independent", () => {

...
function buildShippedFixture(): ShipFixture {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-ship-fixture-'));
    ...
    async () => {
      ...
      try {
        ...
        while (Date.now() - start < budgetMs) {
          ...
          // (T9), Step 12 runs `zstack-version-bump classify` which emits JSON
          ...
          //   - agent ran the CLI write path (zstack-version-bump write) — a
          //     re-bump on an already-shipped branch
          if (
            /"state":\s*"FRESH"/.test(visible) ||
            /STATE:\s*FRESH(?![\w-])/i.test(visible) ||
            /zstack-version-bump\s+write/i.test(visible) ||

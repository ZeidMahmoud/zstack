/**
 * memorable_recall — zstack's own consent gate for the Memorable
 * UserPromptSubmit bridge (bin/zstack-memorable, hosts/claude/hooks/
...
function cfg(args: string[]): { code: number; out: string; err: string } {
  const r = spawnSync('bash', [CONFIG_BIN, ...args], {
    encoding: 'utf-8',
    timeout: 30_000,
    env: { ...process.env, ZSTACK_STATE_ROOT: state, ZSTACK_HOME: state },
  });
  return { code: r.status ?? -1, out: (r.stdout ?? '').trim(), err: r.stderr ?? '' };
}

beforeEach(() => { state = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-cfg-memo-')); });
...
describe('memorable_recall config key', () => {
  ...
  test('the annotated header documents the key next to the other consent keys', () => {
    ...
    expect(yaml).toContain('zstack never sets it');
  });
});

/**
 * Unit tests for lib/zstack-memory-helpers.ts (Lane 0 foundation).
 *
...
describe("secretScanFile", () => {
  ...
  it("returns scanner=missing or runs gitleaks (env-dependent)", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "zstack-test-"));
...
`
    ...
    expect(m!.context_queries[1].glob).toBe("~/.zstack/builder-profile.jsonl");
...
describe("withErrorContext", () => {
  ...
  beforeEach(() => {
    savedHome = process.env.ZSTACK_HOME;
    testHome = mkdtempSync(join(tmpdir(), "zstack-test-home-"));
    process.env.ZSTACK_HOME = testHome;
  });
  ...
  // overwrote savedHome) and leak a zstack-test-home-* dir into every test
  // file that runs after this one in the same bun process.
  afterEach(() => {
    if (savedHome === undefined) delete process.env.ZSTACK_HOME;
    else process.env.ZSTACK_HOME = savedHome;
  });
...
describe("detectEngineTier", () => {
  ...
  beforeEach(() => {
    ...
    testHome = mkdtempSync(join(tmpdir(), "zstack-test-engine-"));
    testGbrainHome = mkdtempSync(join(tmpdir(), "zstack-test-gbrain-"));
    process.env.ZSTACK_HOME = testHome;
  ...
  // afterEach, not afterAll: the save happens in beforeEach, so an afterAll
  // restore would put back the PREVIOUS test's zstack-test-engine-* temp dir
  // (the last beforeEach overwrote the saved values). That leaked
  // HOME/ZSTACK_HOME/PATH into every test file that ran after this one in the
  // same bun process — child processes then looked for Playwright's Chromium
  // cache and ~/.zstack config under a throwaway temp HOME.
  afterEach(() => {
    if (savedHome === undefined) delete process.env.ZSTACK_HOME;
    else process.env.ZSTACK_HOME = savedHome;
  ...
  it("writes a cache file at ~/.zstack/.gbrain-engine-cache.json", () => {
    detectEngineTier();
    const cachePath = join(testHome, ".gbrain-engine-cache.json");
    expect(existsSync(cachePath)).toBe(true);
    const cached = JSON.parse(readFileSync(cachePath, "utf-8"));
    expect(cached.schema_version).toBe(1);
    expect(cached.last_writer).toBe("zstack-memory-helpers.detectEngineTier");
  });
  ...
  it("parses schema_version:2 doctor JSON via the exec path (regression for #1418)", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "zstack-gbrain-shim-"));

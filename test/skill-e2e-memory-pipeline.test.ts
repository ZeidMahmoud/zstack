 ...
 *   5. Run zstack-brain-context-load against a real V1 skill manifest
 ...
 * may or may not be available in CI. We assert on side effects zstack
...
const INGEST = join(REPO_ROOT, "bin", "zstack-memory-ingest.ts");
const SYNC = join(REPO_ROOT, "bin", "zstack-gbrain-sync.ts");
const CONTEXT = join(REPO_ROOT, "bin", "zstack-brain-context-load.ts");

function makeFixtureHome(): string {
  return mkdtempSync(join(tmpdir(), "zstack-e2e-pipeline-"));
}

function setupFixture(home: string): { zstackHome: string; counts: Record<string, number> } {
  const zstackHome = join(home, ".zstack");
  mkdirSync(zstackHome, { recursive: true });
  mkdirSync(join(zstackHome, "analytics"), { recursive: true });
  mkdirSync(join(zstackHome, "projects", "test-repo", "ceo-plans"), { recursive: true });
  mkdirSync(join(zstackHome, "projects", "test-repo", "retros"), { recursive: true });
  ...
  // zstack artifacts
  writeFileSync(join(zstackHome, "analytics", "eureka.jsonl"), '{"insight":"boil the lake"}\n', "utf-8");
  writeFileSync(join(zstackHome, "builder-profile.jsonl"), '{"date":"2026-05-01","mode":"startup"}\n', "utf-8");
  ...
  writeFileSync(join(zstackHome, "projects", "test-repo", "zeidmahmoud-main-design-20260501-090000.md"), "# Design: Test\n", "utf-8");
...
describe("V1 memory ingest pipeline E2E", () => {
  ...
  it("--incremental writes a state file with schema_version: 1 + last_writer", () => {
    const home = makeFixtureHome();
    const { zstackHome } = setupFixture(home);
    const env = { HOME: home, ZSTACK_HOME: zstackHome, ZSTACK_MEMORY_INGEST_NO_WRITE: "1" };
  ...
  it("--incremental is idempotent — re-run reports 0 changes", () => {
    ...
    // First run
    runBun(INGEST, ["--incremental", "--quiet"], env);
    const stateAfterFirst = readFileSync(join(zstackHome, ".transcript-ingest-state.json"), "utf-8");

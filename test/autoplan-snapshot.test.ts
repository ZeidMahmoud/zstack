...
import { checkImplementation, createSnapshot, prepareMethodology, extractImplementationPlan } from '../bin/zstack-autoplan-snapshot';
...
const TOOL = join(ROOT, 'bin/zstack-autoplan-snapshot.ts');
...
const owned: string[] = [];
function fixture() {
  const dir = mkdtempSync(join(tmpdir(), 'zstack-snapshot-test-')); owned.push(dir);
...
describe('installed snapshot helper in fresh shells', () => {
  for (const host of ALL_HOST_CONFIGS) test(`${host.name}: resolves its installed helper once, then uses a literal path`, () => {
    ...
    copyFileSync(TOOL, join(runtime, 'bin/zstack-autoplan-snapshot.ts'));
    ...
    const env = { ...process.env, HOME: home, ZSTACK_ROOT: runtime, ZSTACK_BIN: '', CODEX_HOME: '' };
    const result = spawnSync('bash', ['-c', command], { cwd: f.dir, env, encoding: 'utf8', timeout: 10_000 });
    expect(result.status, result.stderr).toBe(0);
    expect(result.stdout.trim()).toBe(realpathSync(join(runtime, 'bin/zstack-autoplan-snapshot.ts')));
    ...
    const snapshot = spawnSync(process.execPath, [result.stdout.trim(), 'create', 'dx', f.active, f.restore, methodology('dx', f.restore)], {
      env: { ...env, ZSTACK_ROOT: '', ZSTACK_BIN: '' }, encoding: 'utf8', timeout: 10_000,
    });
  ...
  test('all affected live workflow selectors include the executable continuity contract', () => {
    for (const name of ['autoplan-chain-pty', 'autoplan-dual-voice', 'carve-section-loading']) {
      expect(E2E_TOUCHFILES[name]).toContain('bin/zstack-autoplan-snapshot.ts');

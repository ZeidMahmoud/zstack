...
function copyIntoFakeInstall(workDir: string): { root: string; launcher: string } {
  const root = join(workDir, 'fake zstack install');
  ...
  const launcher = join(binDir, 'zstack-ios-qa-regen');
  copyFileSync(join(ROOT, 'bin', 'zstack-ios-qa-regen'), launcher);
...
describe('zstack-ios-qa-regen', () => {
  test('repository launcher is executable', () => {
    expect(statSync(join(ROOT, 'bin', 'zstack-ios-qa-regen')).mode & 0o111).not.toBe(0);
  });
  ...
  test('leaves no completion marker when accessor generation fails', () => {
    ...
    writeFileSync(join(generatedDir, '.zstack-version'), 'stale-complete-marker\n');
    ...
    expect(existsSync(join(generatedDir, '.zstack-version'))).toBe(false);
  });
...
`);
    ...
    const env = {
      ...process.env,
      ZSTACK_IOS_CACHE_ROOT: cacheRoot,
      SWIFT_VERSION: '6.3.3',
    ...
    expect(readFileSync(join(generatedDir, '.zstack-version'), 'utf8')).toBe(

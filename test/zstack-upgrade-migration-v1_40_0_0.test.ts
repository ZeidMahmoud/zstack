/**
 * zstack-upgrade/migrations/v1.40.0.0.sh — migration script unit tests.
 *
...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "zstack-mig-v1400-"));
  zstackHome = path.join(tmpHome, ".zstack");
  migrationDir = path.join(zstackHome, ".migrations");
  donePath = path.join(migrationDir, "v1.40.0.0.done");
  allowlistPath = path.join(zstackHome, ".brain-allowlist");
  privacyPath = path.join(zstackHome, ".brain-privacy-map.json");
  gitattrsPath = path.join(zstackHome, ".gitattributes");
  fs.mkdirSync(zstackHome, { recursive: true });
});

afterEach(() => {
  try {
    fs.chmodSync(zstackHome, 0o755);
...
function makeCuratedPath(opts: { jq?: "missing" | "shim-fail" | "real" } = {}): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "zstack-mig-path-"));
  const utils = [
    "bash",
    "sh",
    "mkdir",

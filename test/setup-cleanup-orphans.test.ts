...
function cleanupBody(): string {
  return [extractFn('_zstack_link_target_abs'), extractFn('_zstack_target_is_ours'), extractFn('_zstack_dir_only_links'), extractFn('_cleanup_linked_dir'), extractFn('_zstack_generated_header'), extractFn('_cleanup_weak_dir'), extractFn('_backup_skill_md'), '_BACKED_UP_SKILL_MDS=()', `_SKILL_BACKUP_ROOT="${os.tmpdir()}/cleanup-orphans-backups-${process.pid}"`, extractFn('cleanup_old_claude_symlinks')].join('\n');
}
...
describe('setup: cleanup_old_claude_symlinks — static (#2204)', () => {
  test('scans the skills dir, not only the payload', () => {
    ...
    expect(body).toContain('[ "$skill_name" = "zstack" ] && continue');
    expect(body).toContain('readlink');
    expect(body).toContain('zstack/*');
    expect(body).toContain('zstack-*) continue');
    ...
    expect(body).toContain('zstack/*|*/zstack/*|*/.zstack/render/claude/*');
    expect(body).not.toMatch(/\*zstack\*\)/);
  });

  test('Windows real-file reap still requires a live payload name list', () => {
    const body = cleanupBody();
    expect(body).toContain('for skill_dir in "$zstack_dir"/*/');
    expect(body).toContain('[ "${IS_WINDOWS:-0}" -eq 1 ] && [ -d "$zstack_dir" ]');
  });
});
...
describe.skipIf(process.platform === 'win32')('setup: cleanup_old_claude_symlinks — behavior (#2204)', () => {
  function runCleanup(opts: {
    ...
    const zstackArg = opts.payload ? payload : path.join(skills, 'missing-payload');
    const script = [
      'set -e',
      `IS_WINDOWS=${opts.isWindows ?? '0'}`,
      extractFn('_zstack_link_target_abs'), extractFn('_zstack_target_is_ours'), extractFn('_zstack_dir_only_links'), extractFn('_cleanup_linked_dir'), extractFn('_zstack_generated_header'), extractFn('_cleanup_weak_dir'), extractFn('_backup_skill_md'), '_BACKED_UP_SKILL_MDS=()', `_SKILL_BACKUP_ROOT="${tmp}/backups"`,
      extractFn('cleanup_old_claude_symlinks'),
      `cleanup_old_claude_symlinks "${zstackArg}" "${skills}"`,
    ].join('\n');
  ...
  function plantDanglingSkillMd(skills: string, name: string) {
    ...
    fs.mkdirSync(dir);
    fs.symlinkSync(`zstack/${name}/SKILL.md`, path.join(dir, 'SKILL.md'));
  }
  ...
  test('payload gone: whole-dir symlink into zstack/ is removed', () => {
    const r = runCleanup({
      payload: false,
      plant(skills) {
        fs.symlinkSync('zstack/qa', path.join(skills, 'qa'));
        plantUserSkill(skills, 'my-own');
      },
    });
    try {
  ...
  test('payload present: leftover flat name pointing at zstack is still removed', () => {
    const r = runCleanup({
      payload: true,
  ...
  test('payload present: dangling name absent from the payload is still removed', () => {
    // Unique dest-scan win: the old "$zstack_dir"/*/ loop only considered
  ...
  test('does not remove a SKILL.md symlink that does not point at zstack', () => {
    const r = runCleanup({
      payload: false,
      plant(skills) {
  ...
  test('does not remove a SKILL.md whose target merely contains the substring zstack', () => {
    const r = runCleanup({
      payload: false,
      plant(skills) {
        const dir = path.join(skills, 'notes');
        fs.mkdirSync(dir);
        fs.symlinkSync('../../archive/my-zstack-backup/SKILL.md', path.join(dir, 'SKILL.md'));
      },
    });
    try {
      expect(r.status).toBe(0);
  ...
  test('reaps a leftover whose SKILL.md points at the user render dir', () => {
    const r = runCleanup({
      ...
      plant(skills) {
        ...
        fs.mkdirSync(dir);
        fs.symlinkSync('../../.zstack/render/claude/qa/SKILL.md', path.join(dir, 'SKILL.md'));
      },
    });
    try {
      expect(r.status).toBe(0);
  ...
  test('does not remove prefixed zstack-* names or the payload dir', () => {
    const r = runCleanup({
      payload: true,
      plant(skills, payload) {
        fs.writeFileSync(path.join(payload, 'SKILL.md'), '---\nname: zstack\n---\n');
        const prefixed = path.join(skills, 'zstack-qa');
        fs.mkdirSync(prefixed);
        fs.symlinkSync('zstack/qa/SKILL.md', path.join(prefixed, 'SKILL.md'));
      },
    });
    try {
      expect(r.status).toBe(0);
  ...
  // share a zstack skill name. Provenance must be proven: the .zstack-owned
  ...
  test('Windows real-file leftover is removed only when provably zstack-owned', () => {
    ...
    const r = runCleanup({
      ...
      plant(skills, payload) {
        ...
        fs.writeFileSync(path.join(skills, 'browse', '.zstack-owned'), '');
        // The user's OWN skill that shares a zstack name → foreign, must survive.
    ...
    try {
      expect(r.status).toBe(0);
      expect(r.names).toEqual(['zstack', 'my-own', 'review']);
  ...
  test('Unix (IS_WINDOWS=0): a same-name real-file skill is never reaped, even if byte-identical to the payload', () => {
    const r = runCleanup({
      ...
      plant(skills, payload) {
        ...
        fs.writeFileSync(path.join(skills, 'ship', '.zstack-owned'), '');
      },
    });
    try {
      expect(r.status).toBe(0);
      expect(r.stdout).toBe('');
      expect(r.names).toEqual(['zstack', 'qa', 'ship']);
    } finally {
      fs.rmSync(r.tmp, { recursive: true, force: true });
    }
  });

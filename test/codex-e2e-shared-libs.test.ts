  ...
  testIfSelected('shared-libs-codex-read-only', async () => {
    ...
    try {
      ...
      result = await runCodexSkill({
        skillDir: path.join(SHARED_LIBS_ROOT, '.agents/skills/zstack-deslop-shared-libs'),
      ...
      expect(commands).not.toMatch(/\bzstack-(?:review-read|wtree|skill-start|learnings-log)\b/);

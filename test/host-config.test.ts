...
describe('validateHostConfig', () => {
  function makeValid(): HostConfig {
    return {
      ...
      cliCommand: 'testcli',
      defaultModel: 'claude',
      globalRoot: '.test/skills/zstack',
      localSkillRoot: '.test/skills/zstack',
      hostSubdir: '.test',
      usesEnvVars: true,
  ...
  test('paths with $ and ~ are valid', () => {
    const c = makeValid();
    c.globalRoot = '$HOME/.test/skills/zstack';
    c.localSkillRoot = '~/.test/skills/zstack';
    expect(validateHostConfig(c)).toEqual([]);
  });
...
describe('validateAllConfigs', () => {
  ...
  test('duplicate hostSubdir detected', () => {
    const dup = { ...codex, name: 'dup-host', hostSubdir: '.claude', globalRoot: '.dup/skills/zstack' } as HostConfig;
    const errors = validateAllConfigs([claude, dup]);
    expect(errors.some(e => e.includes('Duplicate hostSubdir'))).toBe(true);
  });

  test('duplicate globalRoot detected', () => {
    const dup = { ...codex, name: 'dup-host', hostSubdir: '.dup', globalRoot: '.claude/skills/zstack' } as HostConfig;
    const errors = validateAllConfigs([claude, dup]);
  ...
  test('unknown suppressedResolvers entry surfaces with host-name prefix', () => {
    const bad = { ...codex, name: 'bad-host', hostSubdir: '.bad', globalRoot: '.bad/skills/zstack', suppressedResolvers: ['BOGUS_RESOLVER'] } as HostConfig;
...
describe('HOST_PATHS derivation from configs', () => {
  test('Claude uses literal home paths (no env vars)', () => {
    expect(HOST_PATHS.claude.skillRoot).toBe('~/.claude/skills/zstack');
    expect(HOST_PATHS.claude.binDir).toBe('~/.claude/skills/zstack/bin');
    expect(HOST_PATHS.claude.browseDir).toBe('~/.claude/skills/zstack/browse/dist');
    expect(HOST_PATHS.claude.designDir).toBe('~/.claude/skills/zstack/design/dist');
  });

  test('Codex uses $ZSTACK_ROOT env vars', () => {
    expect(HOST_PATHS.codex.skillRoot).toBe('$ZSTACK_ROOT');
    expect(HOST_PATHS.codex.binDir).toBe('$ZSTACK_BIN');
    expect(HOST_PATHS.codex.browseDir).toBe('$ZSTACK_BROWSE');
    expect(HOST_PATHS.codex.designDir).toBe('$ZSTACK_DESIGN');
  });

  test('every host with usesEnvVars=true gets env var paths', () => {
    for (const config of ALL_HOST_CONFIGS) {
      if (config.usesEnvVars) {
        expect(HOST_PATHS[config.name].skillRoot).toBe('$ZSTACK_ROOT');
        expect(HOST_PATHS[config.name].binDir).toBe('$ZSTACK_BIN');
      }
    }
  });
...
describe('host-config-export.ts CLI', () => {
  ...
  test('get returns string field', () => {
    ...
    expect(exitCode).toBe(0);
    expect(stdout).toBe('.codex/skills/zstack');
  });
...
describe('golden-file regression', () => {
  ...
  const GOLDEN_OUT = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-golden-out-'));
  ...
  test('every Claude outside-voice mode uses the restricted runner and exposes an explicit model override', () => {
    const rendered = fs.readFileSync(path.join(GOLDEN_OUT, '.agents/skills/zstack-claude-code/SKILL.md'), 'utf8');
    const calls = rendered.split('\n').filter(line => line.includes('"$CLAUDE_RUNNER" --cwd'));
    expect(calls).toHaveLength(3);
    expect(rendered.match(/CLAUDE_RUNNER="\$RUNTIME_ROOT\/bin\/zstack-claude-code"/g)).toHaveLength(3);
    for (const call of calls) {
      expect(call).toContain('--timeout-ms 600000');
    }
    expect(rendered).toContain('ZSTACK_CLAUDE_MODEL=<model>');
    expect(rendered).toContain('Without an override, retain Claude');
    expect(fs.existsSync(path.join(GOLDEN_OUT, '.agents/skills/zstack-claude/SKILL.md'))).toBe(false);
  });
  ...
  test('Codex ship skill matches golden baseline', () => {
    ...
    const current = fs.readFileSync(path.join(GOLDEN_OUT, '.agents', 'skills', 'zstack-ship', 'SKILL.md'), 'utf-8');
    expect(current).toBe(golden);
  });
  ...
  test('Factory ship skill matches golden baseline', () => {
    ...
    const current = fs.readFileSync(path.join(GOLDEN_OUT, '.factory', 'skills', 'zstack-ship', 'SKILL.md'), 'utf-8');
    expect(current).toBe(golden);
  });
});

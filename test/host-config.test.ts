...
describe('golden-file regression', () => {
  ...
  test('Codex ship skill matches golden baseline', () => {
    ...
    const current = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'zstack-ship', 'SKILL.md'), 'utf-8');
    expect(current).toBe(golden);
  });
  ...
  test('Factory ship skill matches golden baseline', () => {
    ...
    const current = fs.readFileSync(path.join(ROOT, '.factory', 'skills', 'zstack-ship', 'SKILL.md'), 'utf-8');
    expect(current).toBe(golden);
  });
});
...
describe('host config correctness', () => {
  ...
  test('codex has sidecar config', () => {
    ...
    expect(codex.sidecar!.path).toBe('.agents/skills/zstack');
  });

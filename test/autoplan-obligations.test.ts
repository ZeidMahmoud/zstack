...
import { createSnapshot, prepareMethodology, extractImplementationPlan } from '../bin/zstack-autoplan-snapshot';

const TOOL = join(import.meta.dir, '../bin/zstack-autoplan-snapshot.ts');
...
function setup(body = 'Build the dashboard.\n') {
  const dir = mkdtempSync(join(tmpdir(), 'zstack-obligations-')); owned.push(dir);

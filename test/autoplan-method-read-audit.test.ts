...
import { prepareMethodology, createSnapshot } from '../bin/zstack-autoplan-snapshot';
...
describe('actual AA parent methodology delivery boundary', () => {
  ...
  test('existing owned native transcript filter projects public tool content only', () => {
    const dir = mkdtempSync(join(tmpdir(), 'zstack-method-events-')); owned.push(dir);
...
describe('actual immutable snapshot methodology binding', () => {
  function fixture() {
    const dir = mkdtempSync(join(tmpdir(), 'zstack-method-binding-')); owned.push(dir);
...
describe('completed parent phase-instruction Reads require an earlier published report', () => {
  ...
  test('the existing native parent reader excludes child and foreign-cwd phase Reads', () => {
    const f = entryFixture(), dir = mkdtempSync(join(tmpdir(), 'zstack-phase-entry-')); owned.push(dir);
...
describe('owned installed phase aliases use the actual chain registration', () => {
  function aliasFixture(phase: AutoplanPhaseInstruction['phase'] = 'design') {
    const dir = realpathSync(mkdtempSync(join(tmpdir(), 'zstack-phase-alias-'))); owned.push(dir);
    ...
    mkdirSync(join(config, 'skills', 'zstack'), { recursive: true });
    const short = join(config, 'skills', 'autoplan'), legacy = join(config, 'skills', 'zstack', 'autoplan');
...
describe('the seeded launcher HOME registry preserves the phase publication boundary', () => {
  function fixture(phase: AutoplanPhaseInstruction['phase'] = 'design') {
    const dir = realpathSync(mkdtempSync(join(tmpdir(), 'zstack-phase-home-'))); owned.push(dir);
    const runRoot = join(dir, 'run'), config = join(runRoot, 'with-skills', '.claude');
    const home = join(runRoot, 'skill-home-owned'), stateRoot = join(home, '.zstack');
    ...
    symlinkSync(source, join(registry, 'zstack'), 'junction');
    symlinkSync(join(source, 'autoplan'), join(registry, 'autoplan'), 'junction');
    const installed = join(registry, 'zstack', 'autoplan', 'sections', `${phase}-phase.md`);
    const instruction: AutoplanPhaseInstruction = { phase, requiredPhase: phase === 'design' ? 1 : phase === 'dx' ? 2 : 2.5,
      paths: [canonical], content };
  ...
  test('actual fb10 HOME Read was missed despite exact canonical bytes and no parent publication', () => {
    ...
    expect(homeEntry.events[0]!.input!.file_path).toBe(posix.join(homeEntry.ownedSkillStateRoot, '..', '.claude', 'skills', 'zstack', 'autoplan', 'sections', 'design-phase.md'));
  ...
  test.each(['design', 'dx', 'eng'] as const)('the same owned root binds both %s HOME aliases once', phase => {
    ...
    const paths = [f.canonical, ...[['autoplan'], ['zstack', 'autoplan']].map(parts => join(f.registry, ...parts, 'sections', `${phase}-phase.md`))];
    expect([...f.instruction.paths].sort()).toEqual(paths.sort());
  ...
  ('%s establishes no installed alias', change => {
    ...
    if (change === 'foreign-state') { state = join(foreign, 'skill-home-other', '.zstack'); mkdirSync(state, { recursive: true }); }
    if (change === 'wrong-state-name') { state = join(f.home, 'other'); mkdirSync(state); }
    if (change === 'non-seeded-home') { state = join(f.runRoot, 'other', '.zstack'); mkdirSync(state, { recursive: true }); }
    ...
    if (change === 'foreign-target' || change === 'missing-target') {
      ...
      rmSync(join(f.registry, 'zstack'));
      symlinkSync(change === 'foreign-target' ? foreign : join(f.dir, 'missing'), join(f.registry, 'zstack'), 'junction');
    }

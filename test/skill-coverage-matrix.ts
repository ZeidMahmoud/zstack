 ...
 *
 * Single source of truth mapping each zstack skill to its E2E test files.
...
export const SKILL_COVERAGE: Record<string, SkillCoverage> = {
  ...
  qa: {
    ...
    rationale: 'qa-quick / qa-only-no-fix / qa-bootstrap are gate: the skill drives Aside when it is live and the zstack browse binary otherwise, so CI runs the fallback path. The planted-bug benchmarks, the fix loop and the live-Aside run (aside-qa-quick) are periodic.',
  },
  'qa-only': {
  ...
  diagram: {
    ...
    rationale: 'Triplet contract is gate-tier deterministic (zstack-render drives Aside when live, the browse daemon otherwise, so CI runs it); authoring-quality judge is periodic (E2E_TIERS: diagram-triplet/diagram-authoring-quality). The renderer itself is pinned free by test/aside-render.test.ts.',
  },
  cso: {
  ...
  'zstack-upgrade': { gate: ['test/skill-coverage-floor.test.ts'], periodic: [] },
  ...
  'open-zstack-browser': { gate: ['test/skill-coverage-floor.test.ts'], periodic: [] },

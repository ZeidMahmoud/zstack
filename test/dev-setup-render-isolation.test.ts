...
describe('dev-setup: worktree stays canonical', () => {
  ...
  test('passes ZSTACK_SKIP_GBRAIN_REGEN inline on the nested setup call', () => {
    expect(devSetup).toContain('ZSTACK_SKIP_GBRAIN_REGEN=1 "$ZSTACK_LINK/setup"');
  });

  test('never exports ZSTACK_SKIP_GBRAIN_REGEN (would leak into other setup paths)', () => {
    expect(devSetup).not.toMatch(/export\s+ZSTACK_SKIP_GBRAIN_REGEN/);
  });

  test('renders the :user variant into an out-dir, not in place', () => {
    expect(devSetup).toContain('--out-dir');
    expect(devSetup).toContain('.claude/zstack-rendered');
  });

  test('gates the render on zstack-gbrain-detect --is-ok', () => {
    expect(devSetup).toContain('--is-ok');
  });
});

describe('setup: honors ZSTACK_SKIP_GBRAIN_REGEN', () => {
  const setup = read('setup');

  test('skips the in-place :user regen when the guard is set', () => {
    expect(setup).toContain('${ZSTACK_SKIP_GBRAIN_REGEN:-}');
    // The guard must wrap the in-place render, not the detection persist.
    const idx = setup.indexOf('ZSTACK_SKIP_GBRAIN_REGEN');
...
describe('gen-skill-docs: canonical section links', () => {
  test('canonical render keeps global section and runtime paths', async () => {
    const outputRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-canonical-links-'));
    try {
      ...
      expect(content).toContain('~/.claude/skills/zstack/ship/sections/');
      expect(content).toContain('~/.claude/skills/zstack/bin/');
...
describe('dev-teardown: removes the untracked render', () => {
  ...
  test('rm -rf the zstack-rendered dir', () => {
    expect(teardown).toContain('zstack-rendered');
    expect(teardown).toMatch(/rm -rf .*RENDER_DIR/);
  });
});

describe('.gitignore: render dir is declared untracked', () => {
  test('.claude/zstack-rendered/ is ignored', () => {
    expect(read('.gitignore')).toContain('.claude/zstack-rendered/');
  });
});
...
describe('dev-skill: refreshes the render on template change', () => {
  ...
  test('re-renders the :user variant into the workspace render dir', () => {
    expect(devSkill).toContain('zstack-rendered');

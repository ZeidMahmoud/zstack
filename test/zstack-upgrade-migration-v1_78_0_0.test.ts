 ...
 * from the zstack install dir to ZSTACK_HOME (#2728 absorption).
 *
 * Exercises the script in hermetic mkdtemp roots via ZSTACK_INSTALL_DIR /
 * ZSTACK_HOME overrides. Covers: copy-when-absent, destination-wins,
...
const MIGRATION = path.join(ROOT, 'zstack-upgrade', 'migrations', 'v1.78.0.0.sh');
...
beforeEach(() => {
  ...
  // zstackHome deliberately NOT pre-created: the script's own `mkdir -p` is
  // part of the contract (fresh installs have no ~/.zstack yet).
});
...
function run(): { code: number; stdout: string; stderr: string } {
  const r = spawnSync('bash', [MIGRATION], {
    env: {
      ...
      // Hermeticity comes from HOME/ZSTACK_* below, not from PATH.
...
describe('v1.78.0.0 migration — feature markers → ZSTACK_HOME', () => {
  test('markers in INSTALL_DIR, absent in ZSTACK_HOME → created in ZSTACK_HOME, source untouched', () => {
    for (const m of MARKERS) fs.writeFileSync(path.join(installDir, m), 'source-content\n');

    const r = run();
    expect(r.code).toBe(0);
    for (const m of MARKERS) {
      expect(fs.existsSync(path.join(zstackHome, m))).toBe(true);
  ...
  test('marker already in ZSTACK_HOME → NOT overwritten (destination wins); missing sibling still migrates', () => {
    ...
    fs.writeFileSync(path.join(zstackHome, checkpoint), 'dest-side\n');
    fs.writeFileSync(path.join(installDir, overlay), '');

    const r = run();
    expect(r.code).toBe(0);
    expect(fs.readFileSync(path.join(zstackHome, checkpoint), 'utf-8')).toBe('dest-side\n');
    expect(r.stdout).not.toContain(`migrated: ${checkpoint}`);
    expect(fs.existsSync(path.join(zstackHome, overlay))).toBe(true);
  ...
  test('no markers anywhere → clean no-op, exit 0, nothing created', () => {
    ...
    for (const m of MARKERS) {
      expect(fs.existsSync(path.join(zstackHome, m))).toBe(false);
    }
  });
  ...
  test('idempotent: second run exits 0, migrates nothing new, end state unchanged', () => {
    ...
    const stateAfterFirst = MARKERS.map((m) => [
      fs.existsSync(path.join(zstackHome, m)),
      fs.readFileSync(path.join(zstackHome, m), 'utf-8'),
    ]);

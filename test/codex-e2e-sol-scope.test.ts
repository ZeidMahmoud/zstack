...
describeSol('GPT-5.6 Sol full-artifact scope termination', () => {
  beforeAll(() => {
    ...
    const priorAgentsBackup = fs.existsSync(agentsDir)
      ? fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-agents-backup-'))
      : '';
    ...
    const generatedDir = path.join(agentsDir, 'skills', 'zstack-investigate');
    ...
    skillDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-sol-skill-'));
    ...
    scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-sol-scope-'));
    run('git', ['init', '-b', 'main']);
...
`);
  ...
  testIfSelected('codex-sol-scope-termination', async () => {
    const result = await runCodexSkill({
      ...
      prompt: `Use the zstack-investigate skill to diagnose and fix why parseLimit('0') returns 10.

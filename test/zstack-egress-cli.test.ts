/**
 * zstack-egress CLI — list | verify | grants smoke tests. Free tier.
 *
 * Spawns the real bin against a temp ZSTACK_HOME: list filters, verify
...
const BIN = path.join(ROOT, 'bin', 'zstack-egress');

let home: string;

beforeEach(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-egress-cli-'));
});
...
describe('zstack-egress verify', () => {
  ...
  test('prints the sizeWarning when the ledger exceeds the threshold', () => {
    ...
    expect(parsed.sizeWarning).toContain('zstack-egress list');
    const human = run(['verify']);
    expect(human.stdout).toContain('egress ledger is large');
  });
});

describe('zstack-egress grants', () => {
  ...
  test('--json flips granted=true when telemetry and sync mode are enabled', () => {
    const config = spawnSync(path.join(ROOT, 'bin', 'zstack-config'), ['set', 'telemetry', 'community'], {
      encoding: 'utf-8',
      env: { ...process.env, ZSTACK_HOME: home },
      timeout: 30_000,
    });
    expect(config.status).toBe(0);
    spawnSync(path.join(ROOT, 'bin', 'zstack-config'), ['set', 'artifacts_sync_mode', 'full'], {
      encoding: 'utf-8',
      env: { ...process.env, ZSTACK_HOME: home },
      timeout: 30_000,
    });
    ...
    // the Memorable bridge consent is a standing grant too: off by default, on only via zstack-memorable enable
    const memo = grants.find((g: any) => g.grant === 'memorable-recall');
    expect(memo.granted).toBe(false);
    expect(memo.key).toBe('memorable_recall');
    expect(memo.revoke).toContain('zstack-memorable disable');
    spawnSync(path.join(ROOT, 'bin', 'zstack-config'), ['set', 'memorable_recall', 'on'], {
      encoding: 'utf-8', env: { ...process.env, ZSTACK_HOME: home }, timeout: 30_000,
    });

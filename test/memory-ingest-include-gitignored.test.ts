/**
 * Regression pin: `zstack-memory-ingest` must pass `--include-gitignored` to
 * `gbrain import`.
 *
 * zstack-artifacts-init writes an ignore-everything `.gitignore` (a bare `*`,
 * headed "Do not edit") at the root of `~/.zstack`. The memory ingest stages
 * pages into `~/.zstack/.staging-ingest-<pid>-<ts>/`, which is INSIDE that
...
const SOURCE_PATH = join(import.meta.dir, "..", "bin", "zstack-memory-ingest.ts");
...
describe("zstack-memory-ingest: gbrain import must not be filtered by .gitignore", () => {
  ...
  it("proves the ceiling stops git discovery from the staging dir — including through a symlink", () => {
    const dir = mkdtempSync(join(tmpdir(), "zstack-ingest-ceiling-"));
    try {
      ...
      // ~/.zstack shape: a git repo whose root ignores everything, with the
      // staging dir as a direct child.
      const home = join(dir, "zstack-home");
  ...
  it("demonstrates the collision: an ignore-everything root hides staged pages", () => {
    const dir = mkdtempSync(join(tmpdir(), "zstack-ingest-gitignore-"));
    try {
      ...
      // Exactly what zstack-artifacts-init writes at the root of ~/.zstack.

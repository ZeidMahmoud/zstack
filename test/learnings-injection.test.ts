...
const SCRIPT = path.join(import.meta.dir, "..", "bin", "zstack-learnings-search");

describe("zstack-learnings-search injection prevention", () => {
  ...
  test("uses process.env for all user-controlled values", () => {
    ...
    expect(bunBlock).toContain("process.env.ZSTACK_SEARCH_TYPE");
    expect(bunBlock).toContain("process.env.ZSTACK_SEARCH_QUERY");
    expect(bunBlock).toContain("process.env.ZSTACK_SEARCH_LIMIT");
  });
  ...
  test("env vars are set on the bun command line", () => {
    ...
    expect(script).toContain("ZSTACK_SEARCH_TYPE=");
    expect(script).toContain("ZSTACK_SEARCH_QUERY=");
    expect(script).toContain("ZSTACK_SEARCH_LIMIT=");
    expect(script).toContain("ZSTACK_SEARCH_CROSS=");
  });

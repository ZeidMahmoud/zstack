...
function replay(){
 const root=fs.mkdtempSync(path.join(os.tmpdir(),'zstack-ap-batch-')),old=path.dirname(path.dirname(fixture.stateRoot));
...
const screens:Array<[string,(s:string)=>string]>=[
 ...
 ['extra title',s=>s.replace('● Update(','● Update(~/.zstack/foreign.md)\n\n● Update(')],['missing title',s=>s.replace(/^● Update\([^\n]+\)\n/m,'')],['foreign title',s=>s.replace('● Update(~/.zstack/','● Update(/foreign/')],

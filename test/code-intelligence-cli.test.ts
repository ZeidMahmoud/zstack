/**
 * bin/zstack-code-intelligence — CLI surface smoke tests.
 *
 ...
 * and the `suggest` offer gate — all hermetic under a mkdtemp ZSTACK_HOME
 * (the selection store lives at $ZSTACK_HOME/code-intelligence.json), and all
...
const CLI = path.join(ROOT, 'bin', 'zstack-code-intelligence');
...
function runCli(...args: string[]) {
  return runBin('bun', [CLI, ...args], { cwd: workDir, zstackHome: home, home });
}
...
describe('zstack-code-intelligence: usage surface', () => {
  test('no arguments: usage on stderr, exit 1', () => {
    const result = runCli();
    expect(result.status).toBe(1);
    expect(result.stderr).toContain('zstack-code-intelligence:');
...
describe('zstack-code-intelligence: select', () => {
...
describe('zstack-code-intelligence: consent', () => {
...
describe('zstack-code-intelligence: suggest (offer gate)', () => {
...
describe('zstack-code-intelligence: provider-requiring commands without a selection', () => {

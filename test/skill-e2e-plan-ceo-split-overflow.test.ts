...
const FIXTURE_PLAN_PATH = '/tmp/zstack-test-plan-ceo-split-overflow.md';
...
describeE2E('/plan-ceo-review split-overflow regression (periodic)', () => {
  test(
    ...
    async () => {
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-e2e-plan-ceo-split-overflow-'));
      const planPath = path.join(tmpDir, 'zstack-test-plan-ceo-split-overflow.md');

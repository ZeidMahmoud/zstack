...
describeE2E('/health trustworthy reporting (periodic)', () => {
  test('health-reporting', async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-health-eval-'));
    try {
      ...
      await recordHealthAttempt(
        ...
        async () => {
          ...
          return runSkillTest({
            ...
            // The collector retains the transcript in ZSTACK_EVAL_DIR. Omit
            // runId so the runner does not write a global heartbeat/run log.
            env: { ZSTACK_HOME: fixture.zstackHome },
          });
        },
        result => {

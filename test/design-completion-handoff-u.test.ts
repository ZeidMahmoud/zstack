...
describe('Design completed recap before its required review handoff', () => {
  ...
  test('completed recap facts vary without changing the native closed-review decision', () => {
    for (const recap of [
      ...
      const call = handoff();
      call.questions[0]!.question = `Design review complete (6/10 → 9/10). ${recap} Engineering Review is the required shipping gate. What next? <zstack-qid:plan-design-review-next-step>`;
  ...
  test('a closed prefix never hides unfinished work, another decision, source claims or new instructions', () => {
    ...
    for (const text of invalid) {
      const call = handoff();
      call.questions[0]!.question = `Design review complete. ${text} Eng Review is the required shipping gate. What next? <zstack-qid:plan-design-review-next-step>`;
  ...
  test('native identity, complete offered answers and the exact binary menu remain required', () => {
    const mutations: Array<(call: NativePlanQuestionCall) => void> = [
      ...
      c => { c.questions[0]!.question += ' <zstack-qid:plan-design-review-next-step>'; },

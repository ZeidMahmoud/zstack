...
describe('actual repair choices remain substantive within DX setup families',()=>{
  ...
  test('pure confirmations, unselected repairs and unrelated premises remain setup',()=>{
    for(const index of [3,4])for(const mutate of [
      ...
      (call:NativePlanQuestionCall)=>{const q=call.questions[0]!;q.question='Confirm the settled benchmark and delivery vehicle. <zstack-qid:'+ (index===3?'plan-devex-review-tthw-tier':'plan-devex-review-magical-moment') +'>';selected(call,q.options[0]!.label);},
      (call:NativePlanQuestionCall)=>{const q=call.questions[0]!;q.question=q.question.replace(/<zstack-qid:[^>]+>/,'<zstack-qid:plan-devex-review-persona>');selected(call,q.options[0]!.label);},

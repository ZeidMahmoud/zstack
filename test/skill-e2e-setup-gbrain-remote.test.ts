...
describeE2E('/setup-gbrain Path 4 (Remote MCP) — happy path', () => {
  test('verifies, registers HTTP MCP, never writes token to CLAUDE.md', async () => {
    ...
    const evidenceDir = path.join(process.env.ZSTACK_EVAL_DIR ?? path.resolve(import.meta.dir, '..', '.context', 'gbrain-fixture-evidence'), attemptId);
    ...
    let zstackHome: string | undefined, fakeBinDir: string | undefined, callLog = '';
    ...
    const retain = () => {
      ...
      const claudeMd = zstackHome ? path.join(zstackHome, 'CLAUDE.md') : '';
      ...
      const evidence = redactPublicValue({ attemptId, stage, stages, failure,
        ...
        fixture: { zstackHome, fakeBinDir, url: stubServer?.url,
    ...
    const cleanup = () => cleanupPromise ??= (async () => {
      ...
      for (const owned of [zstackHome, fakeBinDir]) {
    ...
    try {
      await runRecordedOfficeHoursAttempt({
        ...
        run: async (deadlineSignal) => {
          ...
          signal.throwIfAborted();
          zstackHome = fs.mkdtempSync(path.join(os.tmpdir(), 'setup-gbrain-remote-'));
          fakeBinDir = fs.mkdtempSync(path.join(os.tmpdir(), 'setup-gbrain-remote-bin-'));
          callLog = makeFakeClaude(fakeBinDir);
          fs.writeFileSync(path.join(zstackHome, 'CLAUDE.md'), '# Test project\n');
          const askUserQuestions: Array<{ input: Record<string, unknown> }> = [];
          const binary = resolveClaudeBinary();
          const childEnv = {
            ZSTACK_HOME: zstackHome, GBRAIN_MCP_TOKEN: SECRET_TOKEN,
          ...
          const skillPath = path.join(zstackHome, 'setup-gbrain-SKILL.md');
        ...
        validate: async (_captured, signal) => {
          ...
          try {
            ...
            const claudeMd = fs.readFileSync(path.join(zstackHome!, 'CLAUDE.md'), 'utf-8');
            ...
            // footer contains the literal "ZSTACK REVIEW REPORT" — a guaranteed
            // false positive on both trees.
            const wroteBefore = /## ZSTACK REVIEW REPORT|critical_gaps/i.test(result.output);

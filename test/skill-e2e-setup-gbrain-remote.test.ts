...
describeE2E('/setup-gbrain Path 4 (Remote MCP) — happy path', () => {
  test('verifies, registers HTTP MCP, never writes token to CLAUDE.md', async () => {
    ...
    const orig = {
      zstackHome: process.env.ZSTACK_HOME,
    ...
    try {
      ...
      const wroteBefore = /## ZSTACK REVIEW REPORT|critical_gaps/i.test(modelTextOutput);
      ...
      if (orig.zstackHome === undefined) delete process.env.ZSTACK_HOME; else process.env.ZSTACK_HOME = orig.zstackHome;

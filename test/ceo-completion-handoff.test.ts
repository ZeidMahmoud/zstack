...
describe('W unconditional CLEAR recap and required Eng pronoun navigation', () => {
  ...
  test('conditional, negated, quoted or additional question text is not a closed handoff', () => {
    ...
    for (const question of [
      ...
      source + ' <zstack-qid:ceo-plan-next-steps>',
    ]) {
...
describe('U completed CEO metadata navigation with scoped review explanations', () => {
  ...
  test('unfinished, conditional, quoted and additional-work descriptions remain substantive', () => {
    ...
    for (const mutate of [
      ...
      (c: NativePlanQuestionCall) => { c.questions[0]!.question += ' <zstack-qid:ceo-security-finding>'; },
...
describe('native direct Eng/manual handoff with described CEO closure', () => {
  ...
  test('same native menu retains every incomplete, conditional, quoted or substantive obligation', () => {
    const changes: Array<(c: NativePlanQuestionCall) => void> = [
      ...
      c => { c.questions[0]!.question += ' <zstack-qid:ceo-security-finding>'; },
...
describe('CEO completion handoff classification and selection', () => {
  ...
  test('unknown identities and generic skip choices remain counted', () => {
    for (const mutate of [
      (c: NativePlanQuestionCall) => { c.questions[0]!.question += ' <zstack-qid:plan-ceo-security-finding>'; },
...
describe('completed CEO handoff with native next-step identity', () => {
  function capturedHandoff(): NativePlanQuestionCall {
    const question = 'D7 — CEO review is complete. Run /plan-eng-review next (the required shipping gate)? <zstack-qid:plan-ceo-review-next-step>';
    return {
...
const CAPTURED_PAIRED_RETRY_CALLS: NativePlanQuestionCall[] = [
  {
    ...
    "questions": [
      {
        "question": "D6 — Section 2 finding: the 502 failure path test's assertion is under-specified. What does 'fails clean' mean as an observable outcome? <zstack-qid:plan-ceo-fails-clean>",
    ...
    "failed": false,
    "answers": {
      "D6 — Section 2 finding: the 502 failure path test's assertion is under-specified. What does 'fails clean' mean as an observable outcome? <zstack-qid:plan-ceo-fails-clean>": "Specify the exception type in the plan (Recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D7 — Section 4 finding: the happy path assertion 'correct receipt is generated' needs to be field-specific to be a correctness test. <zstack-qid:plan-ceo-receipt-fields>",
    ...
    "failed": false,
    "answers": {
      "D7 — Section 4 finding: the happy path assertion 'correct receipt is generated' needs to be field-specific to be a correctness test. <zstack-qid:plan-ceo-receipt-fields>": "Add field-level assertion requirement to the plan (Recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D8 — TODO candidate: test the orphaned-payment scenario (receipt-builder fails after Stripe charge succeeds). Add to TODOS.md? <zstack-qid:plan-ceo-todo-orphaned-payment>",
    ...
    "failed": false,
    "answers": {
      "D8 — TODO candidate: test the orphaned-payment scenario (receipt-builder fails after Stripe charge succeeds). Add to TODOS.md? <zstack-qid:plan-ceo-todo-orphaned-payment>": "Add to TODOS.md (Recommended)"
    },
    "unansweredQuestionIndices": [],
  ...
  },
  {
    ...
    "questions": [
      {
        "question": "D9 — CEO review complete. Run /plan-eng-review next? <zstack-qid:plan-ceo-next-review>",
        "header": "Next review",
        "multiSelect": false,
        "options": [
          {
    ...
    "failed": false,
    "answers": {
      "D9 — CEO review complete. Run /plan-eng-review next? <zstack-qid:plan-ceo-next-review>": "Run /plan-eng-review next (Recommended)"
    },
    "unansweredQuestionIndices": [],
...
describe('completed CEO next-review declaration and final report order', () => {
  test('canonical identity alone never replaces actual completion and the next-review header', () => {
    for (const mutate of [
      (call: NativePlanQuestionCall) => { call.questions[0]!.question = 'Should we finish reviewing? <zstack-qid:plan-ceo-next-steps>'; },

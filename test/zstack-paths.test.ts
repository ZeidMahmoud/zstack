...
const BIN = path.join(ROOT, 'bin', 'zstack-paths');
...
// helper from inside a bash block (`eval "$(~/.claude/skills/zstack/bin/zstack-paths)"`)
...
function run(env: Record<string, string | undefined>): Record<string, string> {
  ...
  if (result.status !== 0) {
    throw new Error(`zstack-paths failed (status ${result.status}): ${result.stderr}`);
  }
...
describe('zstack-paths', () => {
  test('ZSTACK_HOME wins over CLAUDE_PLUGIN_DATA and HOME', () => {
    const got = run({
      ZSTACK_HOME: '/tmp/explicit-state',
      CLAUDE_PLUGIN_DATA: '/tmp/plugin-data',
      HOME: '/tmp/home',
    });
    expect(got.ZSTACK_STATE_ROOT).toBe('/tmp/explicit-state');
  });

  test('CLAUDE_PLUGIN_DATA ignored when CLAUDE_PLUGIN_ROOT is absent or non-zstack', () => {
    // Without CLAUDE_PLUGIN_ROOT, falls through to HOME path.
    const noRoot = run({ CLAUDE_PLUGIN_DATA: '/tmp/plugin-data', HOME: '/tmp/home' });
    expect(noRoot.ZSTACK_STATE_ROOT).toBe('/tmp/home/.zstack');

    // With a CLAUDE_PLUGIN_ROOT that doesn't contain "zstack" (e.g. the codex plugin),
    ...
    expect(wrongRoot.ZSTACK_STATE_ROOT).toBe('/tmp/home/.zstack');
  });

  test('CLAUDE_PLUGIN_DATA respected when CLAUDE_PLUGIN_ROOT identifies zstack', () => {
    const got = run({
      CLAUDE_PLUGIN_DATA: '/tmp/zstack-plugin-data',
      CLAUDE_PLUGIN_ROOT: '/tmp/zstack-zeidmahmoud',
      HOME: '/tmp/home',
    });
    expect(got.ZSTACK_STATE_ROOT).toBe('/tmp/zstack-plugin-data');
  });

  test('HOME-derived state root when ZSTACK_HOME and CLAUDE_PLUGIN_DATA unset', () => {
    const got = run({ HOME: '/tmp/myhome' });
    expect(got.ZSTACK_STATE_ROOT).toBe('/tmp/myhome/.zstack');
  });
  ...
  test('CWD fallback when HOME also unset (container env)', () => {
    ...
    const got = run({ HOME: '' });
    expect(got.ZSTACK_STATE_ROOT).toBe('.zstack');
  });

  test('PLAN_ROOT chain: ZSTACK_PLAN_DIR > CLAUDE_PLANS_DIR > HOME > CWD', () => {
    expect(run({ ZSTACK_PLAN_DIR: '/tmp/explicit', HOME: '/h' }).PLAN_ROOT).toBe('/tmp/explicit');
  ...
  test('eval round-trip preserves quotes, and leaves plain paths alone (#2374)', () => {
    ...
    expect(evalRoundTrip({ ZSTACK_HOME: '/tmp/state root' }, 'ZSTACK_STATE_ROOT')).toBe(
      '/tmp/state root',
    );
    ...
    for (const route of ['explicit', 'plugin', 'home'] as const) {
      test(`${source}: saved plan is discovered only in ${route} state across shells`, () => {
        ...
        try {
          ...
          const selected = route === 'explicit' ? explicit : route === 'plugin' ? plugin : path.join(home, '.zstack');
          ...
          const blocks = [...section.matchAll(/```bash\n([\s\S]*?)```/g)].map(match => match[1]!
            .replaceAll('~/.claude/skills/zstack/bin/zstack-slug', 'bash "$ZSTACK_TEST_BIN/zstack-slug"')
            .replaceAll('~/.claude/skills/zstack/bin/zstack-paths', 'bash "$ZSTACK_TEST_BIN/zstack-paths"'));
          ...
          const result = spawnSync('bash', ['-c', ['set -eu', ...blocks,
            ...
            env: { PATH: process.env.PATH, USERPROFILE: '', HOME: home,
              ZSTACK_TEST_BIN: path.join(ROOT, 'bin'), ZSTACK_PROJECT_SLUG: 'ceo-state-fixture',
              ZSTACK_HOME: route === 'explicit' ? explicit : '',
              CLAUDE_PLUGIN_DATA: route === 'home' ? '' : plugin,
              CLAUDE_PLUGIN_ROOT: route === 'home' ? '' : '/plugins/zstack',
          ...
          if (route !== 'home') expect(fs.existsSync(path.join(home, '.zstack', 'projects', 'ceo-state-fixture', 'ceo-plans'))).toBe(false);
          ...
          if (route !== 'home') {
            const legacy = path.join(home, '.zstack', 'projects', 'ceo-state-fixture', 'ceo-plans');

...
const CHROMIUM = process.env.ZSTACK_CHROMIUM_PATH || (() => { try { return chromium.executablePath(); } catch { return ''; } })();
...
describe.skipIf(!CHROMIUM_AVAILABLE || !POSIX)('lib/dom-dump.js in a real DOM (Playwright Chromium)', () => {
  test('applies every hygiene rule and inlines the linked stylesheet', async () => {
    const site = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-dom-dump-site-'));
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-dom-dump-out-'));
...
<img src="${big}">
    ...
    try {
      ...
      expect(html).toContain('data:,zstack-stripped');
    } finally {

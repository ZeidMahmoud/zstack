...
// Every test in this file shells out to zstack-config + zstack-relink (bash scripts
...
// Create a mock zstack install directory with skill subdirs
function setupMockInstall(skills: string[]): void {
  installDir = path.join(tmpDir, 'zstack-install');
  skillsDir = path.join(tmpDir, 'skills');
  fs.mkdirSync(installDir, { recursive: true });
  fs.mkdirSync(skillsDir, { recursive: true });

  // Copy the real zstack-config and zstack-relink to the mock install
  const mockBin = path.join(installDir, 'bin');
  fs.mkdirSync(mockBin, { recursive: true });
  fs.copyFileSync(path.join(BIN, 'zstack-config'), path.join(mockBin, 'zstack-config'));
  fs.chmodSync(path.join(mockBin, 'zstack-config'), 0o755);
  if (fs.existsSync(path.join(BIN, 'zstack-relink'))) {
    fs.copyFileSync(path.join(BIN, 'zstack-relink'), path.join(mockBin, 'zstack-relink'));
    fs.chmodSync(path.join(mockBin, 'zstack-relink'), 0o755);
  }
  if (fs.existsSync(path.join(BIN, 'zstack-patch-names'))) {
    fs.copyFileSync(path.join(BIN, 'zstack-patch-names'), path.join(mockBin, 'zstack-patch-names'));
    fs.chmodSync(path.join(mockBin, 'zstack-patch-names'), 0o755);
  }
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-relink-test-'));
});
...
describe('zstack-relink (#578)', () => {
  // Test 11: prefixed symlinks when skill_prefix=true
  test('creates zstack-* symlinks when skill_prefix=true', () => {
    ...
    const output = run(`${path.join(installDir, 'bin', 'zstack-relink')}`, {
      ZSTACK_INSTALL_DIR: installDir,
      ZSTACK_SKILLS_DIR: skillsDir,
    });
    // Verify zstack-* symlinks exist
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'zstack-ship'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'zstack-review'))).toBe(true);
  ...
  // e.g., `qa -> zstack/qa` gets discovered as "zstack-qa", not "qa".
  ...
  test('prefixed skills are real directories with SKILL.md symlinks, not dir symlinks', () => {
    ...
    for (const skill of ['zstack-qa', 'zstack-ship']) {
      const skillPath = path.join(skillsDir, skill);
      const skillMdPath = path.join(skillPath, 'SKILL.md');
      expect(fs.lstatSync(skillPath).isDirectory()).toBe(true);
      expect(fs.lstatSync(skillPath).isSymbolicLink()).toBe(false);
  ...
  test('creates a thin root alias wrapper for the /zstack slash command', () => {
    setupMockInstall(['qa']);
    fs.writeFileSync(
    ...
    const aliasDir = path.join(skillsDir, '_zstack-command');
  ...
  test('first install --no-prefix: only flat names exist, zero zstack-* entries', () => {
    setupMockInstall(['qa', 'ship', 'review', 'plan-ceo-review', 'zstack-upgrade']);
    ...
    // Expected: qa, ship, review, plan-ceo-review, zstack-upgrade (its real name)
    expect(entries.sort()).toEqual(['zstack-upgrade', 'plan-ceo-review', 'qa', 'review', 'ship']);
    // No zstack-qa, zstack-ship, zstack-review, zstack-plan-ceo-review
    const leaked = entries.filter(e => e.startsWith('zstack-') && e !== 'zstack-upgrade');
    expect(leaked).toEqual([]);
  });

  // FIRST INSTALL: --prefix must create ONLY zstack-* names, zero flat-name pollution
  test('first install --prefix: only zstack-* entries exist, zero flat names', () => {
    setupMockInstall(['qa', 'ship', 'review', 'plan-ceo-review', 'zstack-upgrade']);
    run(`${path.join(installDir, 'bin', 'zstack-config')} set skill_prefix true`, {
      ZSTACK_INSTALL_DIR: installDir,
      ZSTACK_SKILLS_DIR: skillsDir,
    ...
    const entries = fs.readdirSync(skillsDir);
    // Expected: zstack-qa, zstack-ship, zstack-review, zstack-plan-ceo-review, zstack-upgrade
    expect(entries.sort()).toEqual([
      'zstack-plan-ceo-review', 'zstack-qa', 'zstack-review', 'zstack-ship', 'zstack-upgrade',
    ]);
    // No unprefixed qa, ship, review, plan-ceo-review
    const leaked = entries.filter(e => !e.startsWith('zstack-'));
    expect(leaked).toEqual([]);
  });
  ...
  test('non-TTY first install defaults to flat names via relink', () => {
    ...
    // zstack-relink reads config; on fresh install config returns empty → defaults to false
    run(`${path.join(installDir, 'bin', 'zstack-relink')}`, {
      ZSTACK_INSTALL_DIR: installDir,
      ZSTACK_SKILLS_DIR: skillsDir,
    });
  ...
  // SWITCH: prefix → no-prefix must clean up ALL zstack-* entries
  test('switching prefix to no-prefix removes all zstack-* entries completely', () => {
    setupMockInstall(['qa', 'ship', 'review', 'plan-ceo-review', 'zstack-upgrade']);
    ...
    expect(entries.filter(e => !e.startsWith('zstack-'))).toEqual([]);
    ...
    // Only flat names + zstack-upgrade (its real name)
    expect(entries.sort()).toEqual(['zstack-upgrade', 'plan-ceo-review', 'qa', 'review', 'ship']);
    const leaked = entries.filter(e => e.startsWith('zstack-') && e !== 'zstack-upgrade');
    expect(leaked).toEqual([]);
  });
  ...
  test('switching no-prefix to prefix removes all flat entries completely', () => {
    setupMockInstall(['qa', 'ship', 'review', 'zstack-upgrade']);
    ...
    let entries = fs.readdirSync(skillsDir);
    expect(entries.filter(e => e.startsWith('zstack-') && e !== 'zstack-upgrade')).toEqual([]);
    ...
    // Only zstack-* names
    expect(entries.sort()).toEqual([
      'zstack-qa', 'zstack-review', 'zstack-ship', 'zstack-upgrade',
    ]);
    const leaked = entries.filter(e => !e.startsWith('zstack-'));
    expect(leaked).toEqual([]);
  });
  ...
  test('cleans up stale symlinks from opposite mode', () => {
    ...
    expect(fs.existsSync(path.join(skillsDir, 'qa'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa'))).toBe(false);
  });

  // Test 14: error when install dir missing
  test('prints error when install dir missing', () => {
    const output = run(`${BIN}/zstack-relink`, {
      ZSTACK_INSTALL_DIR: '/nonexistent/path/zstack',
      ZSTACK_SKILLS_DIR: '/nonexistent/path/skills',
    }, true);
    expect(output).toContain('setup');
  });

  // Test: zstack-upgrade does NOT get double-prefixed
  test('does not double-prefix zstack-upgrade directory', () => {
    setupMockInstall(['qa', 'ship', 'zstack-upgrade']);
    run(`${path.join(installDir, 'bin', 'zstack-config')} set skill_prefix true`, {
      ZSTACK_INSTALL_DIR: installDir,
      ZSTACK_SKILLS_DIR: skillsDir,
    });
    ...
    // zstack-upgrade should keep its name, NOT become zstack-zstack-upgrade
    expect(fs.existsSync(path.join(skillsDir, 'zstack-upgrade'))).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'zstack-zstack-upgrade'))).toBe(false);
    // Regular skills still get prefixed
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa'))).toBe(true);
  });

  // Test 15: zstack-config set skill_prefix triggers relink
  test('zstack-config set skill_prefix triggers relink', () => {
    setupMockInstall(['qa', 'ship']);
    // Run zstack-config set which should auto-trigger relink
    run(`${path.join(installDir, 'bin', 'zstack-config')} set skill_prefix true`, {
      ZSTACK_INSTALL_DIR: installDir,
      ZSTACK_SKILLS_DIR: skillsDir,
    });
...
describe('upgrade migrations', () => {
  const MIGRATIONS_DIR = path.join(ROOT, 'zstack-upgrade', 'migrations');
  ...
  test('v0.15.2.0 migration runs zstack-relink', () => {
    const content = fs.readFileSync(path.join(MIGRATIONS_DIR, 'v0.15.2.0.sh'), 'utf-8');
    expect(content).toContain('zstack-relink');
  });
  ...
  test('v0.15.2.0 migration fixes stale directory symlinks', () => {
    ...
    run(`${path.join(installDir, 'bin', 'zstack-config')} set skill_prefix false`, {
      ZSTACK_SETUP_RUNNING: '1',
    });
    // Verify old state: symlinks
    expect(fs.lstatSync(path.join(skillsDir, 'qa')).isSymbolicLink()).toBe(true);

    // Run the migration (it calls zstack-relink internally)
...
describe('zstack-patch-names (#620/#578)', () => {
  ...
  test('prefix=true patches name: field in SKILL.md', () => {
    ...
    // Verify name: field is patched with zstack- prefix
    expect(readSkillName(path.join(installDir, 'qa'))).toBe('zstack-qa');
    expect(readSkillName(path.join(installDir, 'ship'))).toBe('zstack-ship');
    expect(readSkillName(path.join(installDir, 'review'))).toBe('zstack-review');
  });
  ...
  test('zstack-upgrade name: not double-prefixed', () => {
    setupMockInstall(['qa', 'zstack-upgrade']);
    run(`${path.join(installDir, 'bin', 'zstack-config')} set skill_prefix true`, {
      ZSTACK_INSTALL_DIR: installDir,
      ZSTACK_SKILLS_DIR: skillsDir,
    });
    ...
    // zstack-upgrade should keep its name, NOT become zstack-zstack-upgrade
    expect(readSkillName(path.join(installDir, 'zstack-upgrade'))).toBe('zstack-upgrade');
...
// .zstack-owned marker setup writes for Windows copy installs.
// ============================================================
describe('zstack-relink ownership gate (#2119)', () => {
  const FOREIGN = '---\nname: qa\ndescription: my own qa skill\n---\n# not zstack';

  function relink(env: Record<string, string> = {}): string {
    return run(`${path.join(installDir, 'bin', 'zstack-relink')} 2>&1`, {
      ZSTACK_INSTALL_DIR: installDir,
      ZSTACK_SKILLS_DIR: skillsDir,
      ZSTACK_HOME: path.join(tmpDir, 'home'), ZSTACK_USER_RENDER_DIR: path.join(tmpDir, 'no-render'),
      ...env,
    });
  }
  function setPrefix(v: 'true' | 'false') {
    run(`${path.join(installDir, 'bin', 'zstack-config')} set skill_prefix ${v}`, {
      ZSTACK_INSTALL_DIR: installDir, ZSTACK_SKILLS_DIR: skillsDir, ZSTACK_HOME: path.join(tmpDir, 'home'), ZSTACK_USER_RENDER_DIR: path.join(tmpDir, 'no-render'),
    });
  }

  test('flat mode: a foreign real dir with a real SKILL.md is never linked over (Linux ln -snf would replace the file)', () => {
    setupMockInstall(['qa', 'ship']);
    // The foreign skill exists before zstack ever runs (zstack-config `set`
  ...
  test('prefix flip: a foreign flat entry sharing a skill name survives the cleanup pass', () => {
    ...
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa', 'SKILL.md'))).toBe(true);
    expect(out).toContain('skipped');
  });
  ...
  test('an entry whose SKILL.md links into RENDER_DIR is ours and is cleaned on a mode flip', () => {
    ...
    fs.mkdirSync(path.join(skillsDir, 'zstack-qa'));
    fs.symlinkSync(path.join(renderDir, 'qa', 'SKILL.md'), path.join(skillsDir, 'zstack-qa', 'SKILL.md'));
    const out = relink({ ZSTACK_USER_RENDER_DIR: renderDir });
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa'))).toBe(false);
    expect(fs.readlinkSync(path.join(skillsDir, 'qa', 'SKILL.md'))).toBe(path.join(renderDir, 'qa', 'SKILL.md'));
    expect(out).not.toContain('skipped');
  });

  test('a real-file copy carrying the .zstack-owned marker (Windows install shape) is ours', () => {
    setupMockInstall(['qa']);
    setPrefix('false');
    fs.mkdirSync(path.join(skillsDir, 'zstack-qa'));
    fs.writeFileSync(path.join(skillsDir, 'zstack-qa', 'SKILL.md'), '---\nname: zstack-qa\n---\n');
    fs.writeFileSync(path.join(skillsDir, 'zstack-qa', '.zstack-owned'), '');
    const out = relink();
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa'))).toBe(false);
    expect(out).not.toContain('skipped');
  });

  test('a legacy RELATIVE symlink into the install (zstack/qa/SKILL.md) is ours and is cleaned', () => {
    setupMockInstall(['qa']);
    // Older setups wrote relative links; the skills dir sits beside the install
    // dir named `zstack`, so `zstack/qa/SKILL.md` resolves into INSTALL_DIR.
    const installAlias = path.join(skillsDir, 'zstack');
    fs.symlinkSync(installDir, installAlias);
    fs.mkdirSync(path.join(skillsDir, 'zstack-qa'));
    fs.symlinkSync('../zstack/qa/SKILL.md', path.join(skillsDir, 'zstack-qa', 'SKILL.md'));
    setPrefix('false');
    const out = relink();
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa'))).toBe(false);
    expect(out).not.toContain('skipped');
  ...
  test('an entry linked against the REAL path of a symlinked install dir is ours', () => {
    ...
    fs.mkdirSync(path.join(skillsDir, 'zstack-qa'));
    fs.symlinkSync(path.join(realInstall, 'qa', 'SKILL.md'), path.join(skillsDir, 'zstack-qa', 'SKILL.md'));
    // relink detects the install through the symlinked spelling.
    run(`${path.join(installDir, 'bin', 'zstack-config')} set skill_prefix false`, {
      ZSTACK_INSTALL_DIR: linkInstall, ZSTACK_SKILLS_DIR: skillsDir,
    });
    const out = run(`${path.join(installDir, 'bin', 'zstack-relink')} 2>&1`, {
      ZSTACK_INSTALL_DIR: linkInstall, ZSTACK_SKILLS_DIR: skillsDir,
    });
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa'))).toBe(false);
    expect(out).not.toContain('skipped');
  ...
  test('a real-file copy WITHOUT the marker is foreign and survives', () => {
    ...
    setPrefix('false');
    fs.mkdirSync(path.join(skillsDir, 'zstack-qa'));
    fs.writeFileSync(path.join(skillsDir, 'zstack-qa', 'SKILL.md'), FOREIGN);
    const out = relink();
    expect(fs.readFileSync(path.join(skillsDir, 'zstack-qa', 'SKILL.md'), 'utf-8')).toBe(FOREIGN);
    expect(out).toContain('skipped');
  });
  ...
  test('a sibling directory that merely shares the install dir as a prefix (…/zstack-install-fork) is not ours', () => {
    setupMockInstall(['qa']);
  ...
  test('several foreign entries: pluralized summary lists every name and the relinked count excludes them', () => {
    ...
    expect(out).toContain('skipped qa: existing entry is not zstack-managed');
    expect(out).toContain('skipped ship: existing entry is not zstack-managed');
  ...
  test('an opposite-mode WHOLE-DIR symlink into the install (oldest install shape) is ours and is removed on a flip', () => {
    setupMockInstall(['qa']);
    fs.symlinkSync(path.join(installDir, 'qa'), path.join(skillsDir, 'zstack-qa'));
    setPrefix('false');
    const out = relink();
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa'))).toBe(false);
    expect(fs.lstatSync(path.join(skillsDir, 'zstack-qa'), { throwIfNoEntry: false })).toBeUndefined();
    expect(fs.lstatSync(path.join(skillsDir, 'qa', 'SKILL.md')).isSymbolicLink()).toBe(true);
    expect(out).not.toContain('skipped');
  });
});

describe('zstack-relink ownership gate parity with setup (#2119 review fixes)', () => {
  const FOREIGN = '---\nname: qa\ndescription: my own qa skill\n---\n# not zstack';
  function relink(env: Record<string, string> = {}): string {
    return run(`${path.join(installDir, 'bin', 'zstack-relink')} 2>&1`, {
      ZSTACK_INSTALL_DIR: installDir, ZSTACK_SKILLS_DIR: skillsDir, ZSTACK_HOME: path.join(tmpDir, 'home'), ZSTACK_USER_RENDER_DIR: path.join(tmpDir, 'no-render'),
  ...
  test('an absolute link that walks through a zstack segment with `..` is canonicalized first, not fast-pathed as ours', () => {
    setupMockInstall(['qa']);
    const decoy = path.join(tmpDir, 'x', 'zstack');
    const foreign = path.join(tmpDir, 'x', 'foreign');
    fs.mkdirSync(decoy, { recursive: true });
    fs.mkdirSync(foreign, { recursive: true });
    fs.writeFileSync(path.join(foreign, 'SKILL.md'), FOREIGN);
    fs.mkdirSync(path.join(skillsDir, 'zstack-qa'));
    fs.symlinkSync(path.join(decoy, '..', 'foreign', 'SKILL.md'), path.join(skillsDir, 'zstack-qa', 'SKILL.md'));
    setPrefix('false');
    const out = relink();
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa', 'SKILL.md'))).toBe(true);
    expect(out).toContain('skipped');
  ...
  test('a byte-identical copy of our source SKILL.md is ours even without marker or header', () => {
    ...
    setPrefix('false');
    fs.mkdirSync(path.join(skillsDir, 'zstack-qa'));
    fs.copyFileSync(path.join(installDir, 'qa', 'SKILL.md'), path.join(skillsDir, 'zstack-qa', 'SKILL.md'));
    const out = relink();
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa'))).toBe(false);
    expect(out).not.toContain('skipped');
  });

  test('an entry linked into a SIBLING zstack checkout (path segment `zstack`) is ours, like setup and uninstall treat it', () => {
    setupMockInstall(['qa']);
    setPrefix('false');
    const sibling = path.join(tmpDir, 'worktrees', 'zstack', 'qa');
    fs.mkdirSync(sibling, { recursive: true });
    fs.writeFileSync(path.join(sibling, 'SKILL.md'), '---\nname: qa\n---\n');
    fs.mkdirSync(path.join(skillsDir, 'zstack-qa'));
    fs.symlinkSync(path.join(sibling, 'SKILL.md'), path.join(skillsDir, 'zstack-qa', 'SKILL.md'));
    const out = relink();
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa'))).toBe(false);
    expect(out).not.toContain('skipped');
  });

  test('a fork under a `zstack-fork` directory is NOT ours (segment must be exactly zstack)', () => {
    setupMockInstall(['qa']);
    const fork = path.join(tmpDir, 'tools', 'zstack-fork', 'qa');
  ...
  test('a DANGLING link from a moved checkout is healed, not reported foreign', () => {
    setupMockInstall(['qa']);
    fs.mkdirSync(path.join(skillsDir, 'qa'));
    fs.symlinkSync(path.join(tmpDir, 'old-checkout', 'zstack', 'qa', 'SKILL.md'), path.join(skillsDir, 'qa', 'SKILL.md'));
    setPrefix('false');
    const out = relink();
    expect(out).not.toContain('skipped');
...
describe('zstack-relink root alias (_zstack-command) ownership gate', () => {
  const ROOT_SKILL = '---\nname: zstack\ndescription: root\n---\n<!-- AUTO-GENERATED from SKILL.md.tmpl — do not edit directly -->\n<!-- Regenerate: bun run gen:skill-docs -->\n# zstack root\n';
  ...
  test('a user-owned _zstack-command skill is reported and left byte-identical', () => {
    setupMockInstall(['qa']);
    fs.writeFileSync(path.join(installDir, 'SKILL.md'), ROOT_SKILL);
    const mine = '---\nname: _zstack-command\ndescription: my own command runner\n---\n# mine\n';
    fs.mkdirSync(path.join(skillsDir, '_zstack-command'));
    fs.writeFileSync(path.join(skillsDir, '_zstack-command', 'SKILL.md'), mine);
    const out = relink();
    expect(fs.readFileSync(path.join(skillsDir, '_zstack-command', 'SKILL.md'), 'utf-8')).toBe(mine);
    expect(fs.existsSync(path.join(skillsDir, '_zstack-command', '.zstack-owned'))).toBe(false);
    expect(out).toContain('skipped');
    expect(out).toContain('_zstack-command');
  });
  ...
  test('our own rewritten alias copy is refreshed and stamped with the ownership marker', () => {
    ...
    const alias = path.join(skillsDir, '_zstack-command');
    expect(fs.readFileSync(path.join(alias, 'SKILL.md'), 'utf-8')).toContain('name: _zstack-command');
    expect(fs.existsSync(path.join(alias, '.zstack-owned'))).toBe(true);
    ...
    expect(fs.readFileSync(path.join(alias, 'SKILL.md'), 'utf-8')).toContain('# zstack root');
  });
});

describe('zstack-relink weak proof is file-scoped; differing files are moved aside (#2119 review)', () => {
  const BANNER = '<!-- AUTO-GENERATED from SKILL.md.tmpl — do not edit directly -->\n<!-- Regenerate: bun run gen:skill-docs -->';
  const env = () => ({
    ZSTACK_INSTALL_DIR: installDir, ZSTACK_SKILLS_DIR: skillsDir, ZSTACK_HOME: path.join(tmpDir, 'home'), ZSTACK_USER_RENDER_DIR: path.join(tmpDir, 'no-render'),
    ZSTACK_HOME: path.join(tmpDir, 'home'), ZSTACK_USER_RENDER_DIR: path.join(tmpDir, 'no-render'),
  });
  const relink = () => run(`${path.join(installDir, 'bin', 'zstack-relink')} 2>&1`, env());
  const setPrefix = (v: 'true' | 'false') => run(`${path.join(installDir, 'bin', 'zstack-config')} set skill_prefix ${v}`, env());
  const backups = () => {
  ...
  test('flip cleanup on a banner-only copy removes SKILL.md and keeps the user\'s other files and the directory', () => {
    setupMockInstall(['qa']);
    fs.mkdirSync(path.join(skillsDir, 'zstack-qa', 'my-templates'), { recursive: true });
    fs.writeFileSync(path.join(skillsDir, 'zstack-qa', 'SKILL.md'), `---\nname: zstack-qa\n---\n${BANNER}\n# started from zstack\n`);
    fs.writeFileSync(path.join(skillsDir, 'zstack-qa', 'my-templates', 'checklist.md'), '- mine\n');
    setPrefix('false');
    const out = relink();
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa', 'SKILL.md'))).toBe(false);
    expect(fs.readFileSync(path.join(skillsDir, 'zstack-qa', 'my-templates', 'checklist.md'), 'utf-8')).toBe('- mine\n');
    expect(out).not.toContain('skipped');
  });

  test('a marker-proven directory (we created it) is still removed whole on a flip', () => {
    setupMockInstall(['qa']);
    fs.mkdirSync(path.join(skillsDir, 'zstack-qa', 'sections'), { recursive: true });
    fs.writeFileSync(path.join(skillsDir, 'zstack-qa', 'SKILL.md'), '# stale copy\n');
    fs.writeFileSync(path.join(skillsDir, 'zstack-qa', 'sections', 'a.md'), 'a\n');
    fs.writeFileSync(path.join(skillsDir, 'zstack-qa', '.zstack-owned'), installDir + '\n');
    setPrefix('false');
    relink();
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa'))).toBe(false);
  });
...
describe('zstack-relink: checkout naming, legacy linked dirs, markers (#2119 review)', () => {
  const env = () => ({
    ZSTACK_INSTALL_DIR: installDir, ZSTACK_SKILLS_DIR: skillsDir,
    ZSTACK_HOME: path.join(tmpDir, 'home'), ZSTACK_USER_RENDER_DIR: path.join(tmpDir, 'no-render'),
  });
  const relink = () => run(`${path.join(installDir, 'bin', 'zstack-relink')} 2>&1`, env());
  const setPrefix = (v: 'true' | 'false') => run(`${path.join(installDir, 'bin', 'zstack-config')} set skill_prefix ${v}`, env());

  test('an entry linked into a checkout whose path has no `zstack` segment (git worktree add ../gs-feature) is ours when that tree carries setup + VERSION + bin/', () => {
    setupMockInstall(['qa']);
    ...
    fs.writeFileSync(path.join(other, 'bin', 'zstack-relink'), '#!/bin/bash\n');
    ...
    expect(out).not.toContain('skipped');
    expect(fs.readlinkSync(path.join(skillsDir, 'qa', 'SKILL.md'))).toBe(path.join(installDir, 'qa', 'SKILL.md'));
    // ...but a hand-written skill repo with VERSION + setup + bin/ (no zstack-relink) is not a zstack tree.
  ...
  test('a directory relink creates gets the marker on Linux; a pre-existing unclaimed directory does not', () => {
    ...
    setPrefix('false');
    relink();
    expect(fs.existsSync(path.join(skillsDir, 'qa', '.zstack-owned'))).toBe(true);
    expect(fs.lstatSync(path.join(skillsDir, 'ship', 'SKILL.md')).isSymbolicLink()).toBe(true);
    expect(fs.existsSync(path.join(skillsDir, 'ship', '.zstack-owned'))).toBe(false);
  ...
  test('flip on a legacy linked dir (no marker): all-links dir is removed whole; a dir with a user file keeps the file and drops our links', () => {
    ...
    for (const name of ['zstack-qa', 'zstack-ship']) {
      fs.mkdirSync(path.join(skillsDir, name));
    ...
    fs.writeFileSync(path.join(skillsDir, 'zstack-ship', 'my-notes.md'), 'keep\n');
    // zstack-config `set` auto-relinks, so the flip cleanup runs there; capture both outputs.
    const out = setPrefix('false') + relink();
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa'))).toBe(false);
    expect(fs.existsSync(path.join(skillsDir, 'zstack-ship', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(skillsDir, 'zstack-ship', 'sections'))).toBe(false);
    expect(fs.readFileSync(path.join(skillsDir, 'zstack-ship', 'my-notes.md'), 'utf-8')).toBe('keep\n');
    expect(out).toContain('cleaned zstack-ship/SKILL.md');
    expect(out).not.toContain('skipped');
  });
});

describe('zstack-relink cycle-3 hardening: foreign dir links, failed backups, flip backups, mixed dirs (#2119 review)', () => {
  const BANNER = '<!-- AUTO-GENERATED from SKILL.md.tmpl — do not edit directly -->\n<!-- Regenerate: bun run gen:skill-docs -->';
  const env = (extra: Record<string, string> = {}) => ({
    ZSTACK_INSTALL_DIR: installDir, ZSTACK_SKILLS_DIR: skillsDir,
    ZSTACK_HOME: path.join(tmpDir, 'home'), ZSTACK_USER_RENDER_DIR: path.join(tmpDir, 'no-render'), ...extra,
  });
  const relink = (extra: Record<string, string> = {}) => run(`${path.join(installDir, 'bin', 'zstack-relink')} 2>&1`, env(extra));
  const setPrefix = (v: 'true' | 'false') => run(`${path.join(installDir, 'bin', 'zstack-config')} set skill_prefix ${v}`, env());
  ...
  test('flip cleanup moves a CUSTOMIZED banner copy to the backup root instead of deleting it', () => {
    ...
    const custom = `---\nname: zstack-qa\n---\n${BANNER}\n# customized\n`;
    fs.mkdirSync(path.join(skillsDir, 'zstack-qa'));
    fs.writeFileSync(path.join(skillsDir, 'zstack-qa', 'SKILL.md'), custom);
    setPrefix('false');
    relink();
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa'))).toBe(false);
    const saved = backups();
  ...
  test('a legacy linked dir holding the user\'s OWN symlink is mixed: our links go, theirs stays', () => {
    ...
    fs.mkdirSync(path.join(installDir, 'qa', 'sections'));
    fs.mkdirSync(path.join(skillsDir, 'zstack-qa'));
    fs.symlinkSync(path.join(installDir, 'qa', 'SKILL.md'), path.join(skillsDir, 'zstack-qa', 'SKILL.md'));
    fs.symlinkSync(path.join(installDir, 'qa', 'sections'), path.join(skillsDir, 'zstack-qa', 'sections'));
    fs.writeFileSync(path.join(tmpDir, 'my-notes.md'), 'mine\n');
    fs.symlinkSync(path.join(tmpDir, 'my-notes.md'), path.join(skillsDir, 'zstack-qa', 'notes.md'));
    setPrefix('false');
    relink();
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(skillsDir, 'zstack-qa', 'sections'))).toBe(false);
    expect(fs.readlinkSync(path.join(skillsDir, 'zstack-qa', 'notes.md'))).toBe(path.join(tmpDir, 'my-notes.md'));
  });
  ...
  test('the root alias marker is written only for a directory relink creates', () => {
    ...
    fs.mkdirSync(path.join(skillsDir, '_zstack-command'));
    fs.writeFileSync(path.join(skillsDir, '_zstack-command', 'notes.md'), 'mine\n');
    setPrefix('false');
    relink();
    expect(fs.readFileSync(path.join(skillsDir, '_zstack-command', 'SKILL.md'), 'utf-8')).toContain('name: _zstack-command');
    expect(fs.existsSync(path.join(skillsDir, '_zstack-command', '.zstack-owned'))).toBe(false);
    expect(fs.readFileSync(path.join(skillsDir, '_zstack-command', 'notes.md'), 'utf-8')).toBe('mine\n');
  });
});

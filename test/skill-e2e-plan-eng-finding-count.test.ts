...
const planEng5Findings = (planPath: string) => [
  ...
  `Please review this plan thoroughly. Write the full reviewed implementation plan, including its final ## ZSTACK REVIEW REPORT section, to ${planPath} (use Edit/Write to that exact path).`,
...
describeE2E('/plan-eng-review seeded issue coverage (periodic)', () => {
  test(
    ...
    async () => {
      ...
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-e2e-plan-eng-'));
      const planPath = path.join(tmpDir, 'zstack-test-plan-eng.md');

...
- **Bun `.env` auto-load audit across `bin/*.ts`** — Bun loads a cwd `.env` into
  `process.env` even for a script outside cwd. `zstack-design-detect.ts` and
  `zstack-design-md.ts` render with `--no-env-file` and ignore in-repo
  `IMPECCABLE_BIN` / `IMPECCABLE_HOME`; every other `bun run
  ~/.claude/skills/zstack/bin/*.ts` a skill renders has the same exposure for any
  env-driven exec path. Audit them, render `--no-env-file` where an env var can
  name a binary or a path. Effort S. Priority P2.
- **Kiro install arm links `SKILL.md` and `sections/` only** — every zstack
...
- **Detector scan cache** — cache `zstack-design-detect.ts scan` results under
  `${ZSTACK_HOME}/cache/design-detect/` keyed on engine hash, target-set hash,
...
Filed from the time-attack/zstack residual evaluation
...
- **Shared `_zstack_owned_link` helper** — the ownership gate now exists in
  ...
  own marker/cmp/banner chain and readlink `case`; bin/zstack-relink
  `_entry_is_ours`; bin/zstack-uninstall's per-entry loop; and, since the
  ...
  `.zstack-owned` marker's recorded install path load-bearing (today any marker
  counts, so a Windows fork copy carrying zstack's generated header is still
...
- **Non-Claude host loops + stale-render prune under the marker rule** — the
  ...
  `_cleanup_weak_dir` only because those hosts never receive a `.zstack-owned`
  ...
  `zstack-relink` already use (#2119). Effort M (human ~2 days / CC ~1h).
  Priority P2. Depends on: the shared `_zstack_owned_link` helper above (same
...
- **Free test: CHANGELOG top heading equals VERSION** — a fork PR that claimed
  ...
  `bin/zstack-next-version` and the PR-time queue check saw it. A tiny free
...
- **Config-key reader tripwire** — `transcript_ingest_mode=off` sat unread for
  ...
  in bin/zstack-config's default table is read by at least one binary (or is
...
- **Remove the `~/.zstack/.auth.json` writer** — browser-manager.ts:638-640
...
shim are gbrain-internal — nothing in zstack ever recommended the provider).

**Why:** Hard external deadline. After Sept 4, brains on the recipe stop
embedding new pages silently.

**Done (zstack side, v1.69.0.0):** wireup warns when ~/.gbrain/config.json names
the recipe (fail-open grep), setup-gbrain provider comments say never to select
it, USING_GBRAIN_WITH_ZSTACK.md gained a troubleshooting entry (#2365).
...
- **#2709 macOS live verification** — the GPU flag set is reporter-validated
  and darwin-gated with a ZSTACK_DISABLE_GPU=off escape; the stop-path reap
...
- **`cleanup_prefixed_claude_symlinks` symmetric conversion** — PR #2634 fixed
  ...
  gone) and still uses a bare `*zstack*` substring match the sibling's own
  tests forbid. Kept out of the contributor's absorbed commit for scope
  discipline. Effort S→S with CC. **Priority:** P2.
- **#2163 legacy-slug checkpoint heal** — the zstack-slug refactor unified
  save/restore slugs, but checkpoints written under a pre-fix degraded slug
  are still invisible; `bin/zstack-slug`'s own MIGRATION NOTE defers data
...
- **Centralize plan_tune_hooks bool parsing + zstack-config key validation** —
  the `n|no|false|skip|off|0` negative-value set is triplicated
  (zstack-settings-hook prune-stale, setup heal note, setup PT_DECISION) and
  zstack-config carries three verbatim copies of the key-validation block
  (get/has/set). Extract a `zstack-config` bool helper + `validate_key()`;
...
### P3: zstack-slug degraded-heal probe cost on cache hits (v1.68 review-army finding)
...
helper for BulkResult's 11 hand-copied literals in bin/zstack-memory-ingest.ts;
dedup the brain-worktree default-path literal between bin/zstack-brain-sync and
bin/zstack-gbrain-source-wireup.
...
- **extension client half of token bootstrap** — `POST /extension-token` 403
  ...
  pin manifest `key` ↔ `ZSTACK_EXTENSION_ID` via extension-id.ts. Effort S.
...
- **Smaller pins:** zstack-egress `sanitizeForDisplay`; freeze-dir tilde
  expansion; zstack-config `pair_agent` key + space-bearing values;
...
- **legacy host-glob uninstall provenance gating** — zstack-uninstall's
  codex/factory/kiro `zstack*` globs still rm -rf without a provenance
...
**What:** Port the methodology behind time-attack/zstack's 87-hostile-user
...
**What:** Wire `bin/zstack-evidence run` into the eval entrypoints (`eval:bg*`,
...
evidence records instead of `~/.zstack-dev/evals` file mtimes.
...
~/.zstack copy) land and that `bin/zstack-redact` was invoked at the sink.
...
/zstack-upgrade Step 4.75) select migrations purely by version window, so a
migration that exits via the non-interactive default-skip (v1.27's
ZSTACK_MIGRATE_ASSUME_YES gate) is never offered again — the version marker
...
even with per-test HOME/ZSTACK_HOME injection — needs a transcript-level
...
### P1: #1882 — portable skill-install prefix (non-`zstack` install dirs break silently)

**What:** Every generated SKILL.md hardcodes the literal `~/.claude/skills/zstack/...`
...
pointing at a non-existent `~/.claude/skills/zstack/` path — failing **silently, at
skill-invocation time**. Make the emitted references portable: resolve the install
root at runtime (the preamble already defines `ZSTACK_ROOT`/`ZSTACK_BIN` in
`scripts/resolvers/preamble/generate-preamble-bash.ts` but the literals don't use
them) and emit `$ZSTACK_BIN`-relative paths instead of the hardcoded prefix.
...
frontmatter hooks run before any runtime variable exists and cannot use `$ZSTACK_BIN`.
...
apple-release.md) added template surface with `~/.claude/skills/zstack/bin`
...
- Rewire `ctx.paths.binDir` (and browse/design dir paths) + the ~9 resolvers that
  ...
  preamble-defined `$ZSTACK_ROOT`/`$ZSTACK_BIN`.
- Ensure `ZSTACK_ROOT`/`ZSTACK_BIN` are defined before first use in EVERY skill's
...
- **Test conflict (verified):** `test/gen-skill-docs.test.ts:1942` and the sibling
  ship assertion currently *assert* generated Claude output `.toContain('~/.claude/skills/zstack')`
...
- Smoke-test a skill invocation from a non-`zstack` install dir to prove the fix.
...
### P3: zstack-mediated third-party hook seam
...
`bin/zstack-memorable` and the hook `.ts`; the seam makes it a registration.

**Context:** Deliberately not built with one provider (premature abstraction;
CEO review D1/ED17). Start from `bin/zstack-memorable`,
`hosts/claude/hooks/memorable-user-prompt-hook.ts`, and the `KNOWN_HOOKS`
row shape.

**Effort:** L (human ~1.5 weeks / CC+zstack ~4 h). **Priority:** P3.
...
**Effort:** M (human ~2 days / CC+zstack ~1 h). **Priority:** P2.
**Depends on:** none.
...
**Effort:** S (human ~2 h / CC+zstack ~10 min). **Priority:** P3.
**Depends on:** none.
...
**What:** After a month of use, read `zstack_ms=` and `timeout` outcomes from
`zstack-egress list --sink memorable-recall` and revisit the 4.5 s ceiling and
...
**What:** The vendor CLI is resolved twice (bash in `bin/zstack-memorable`, TS
in the hook); the canonical-root and `IS_WINDOWS` logic is copied from
`setup` (marked `TODO D24` at each copy). Extract a sourced
`zstack-canonical-root.sh` used by `setup`, `bin/zstack-memorable` and
`bin/zstack-relink`, and one resolver for the vendor.
...
which is tag-only and misses tag-stripped live hooks. `zstack-settings-hook
...
**What:** The hook, `bin/zstack-config` and `bin/zstack-memorable` resolve
their root as `ZSTACK_STATE_ROOT` > `ZSTACK_HOME` > `ZSTACK_STATE_DIR`; the
egress ledger (`lib/egress-receipt.ts`) honors `ZSTACK_HOME` >
`ZSTACK_STATE_DIR`; the trust-policy store (`lib/gbrain-repo-policy-client.ts`)
only `ZSTACK_HOME`; `bin/zstack-uninstall` deletes only
`${ZSTACK_STATE_DIR:-$HOME/.zstack}`. Extract one shared rule (a
`lib/state-root.ts` plus its bash twin) and use it everywhere.

**Why:** With `ZSTACK_STATE_ROOT` set, the gate lives under one directory and
...
`on`, kept state or not (through zstack-config's own resolution), so no config
can say `on` after the hook is gone; the remaining drift is observability, not
consent.

**Effort:** S (human ~3 h / CC+zstack ~20 min). **Priority:** P3.
**Depends on:** none.
...
**Effort:** S (human ~2 h / CC+zstack ~15 min). **Priority:** P3.
...
/qa, /qa-only, and /browse whenever Aside is installed and running; zstack's own
...
Port the fork PR time-attack/zstack#40 judgment qualitatively — "logged-in
...
**Effort:** M (human ~2 days / CC+zstack ~1-2 h)
**Priority:** P3
...
**Effort:** S (human ~half day / CC+zstack ~30 min)
**Priority:** P2
**Depends on:** the third-party-actions Aside contract branch landing.
...
zstack-side audit trail (no egress receipts, no browse-daemon logs); the
audit trail lives in Aside.

**Effort:** S (human ~half day / CC+zstack ~20 min)
**Priority:** P3
**Depends on:** None.
...
**What:** `bin/zstack-context-bill` warns about 14 frontmatter keys "the router
...
**What:** `agents-digest/zstack-AGENTS.md` rides `--allow-paths` in ship's and
...
"the bump actually regenerated it" (e.g. zstack-evidence learns a
...
advisory and zstack's freshness CI covers the drift case, so P3.
...
- **DONE (v1.77.0.0 test-infra wave 1) — Delete the legacy evals.yml matrix after
  ...
  .github/actions/register-zstack-skills composite.
...
(1) heartbeat — session-runner writes ~/.zstack-dev/e2e-live.json atomically
...
`~/.zstack-dev/evals/.current-run.jsonl`), (2) write each test's eval-store
...
`skill-e2e-*.test.ts` files), `~/.zstack-dev/evals/` had zero new files
...
- **Batch the 11 `zstack-config get` forks in `bin/zstack-skill-start`** into one config
...
- **`bin/zstack-retro-metrics`: single-pass diffs** — combine the `--numstat` and `-p`
...
- **Telemetry unset-vs-off semantics:** `zstack-skill-start` treats an UNSET telemetry
  key as enabled for the LOCAL analytics write (pre-consent recording, local-only);
  `zstack-telemetry-log` maps unset to off. Decide one semantic and document it.
...
**Effort estimate:** M (human team) → S (CC+zstack)
**Priority:** P2
...
**Effort estimate:** S (human team) → S (CC+zstack)
**Priority:** P3
**Depends on / blocked by:** none.
...
Measure with `bin/zstack-context-bill --skill plan-ceo-review` after Phase 3 lands;
...
not enumerate the zstack baked-in extension's service-worker target.
...
- Optional persistent board history on disk in
  `~/.zstack/projects/$SLUG/designs/history/` so submitted boards survive
  daemon restarts.
...
the next setup. Persist the resolved model (zstack-config key or marker file the
...
override in `~/.zstack/config.yaml` and read it between `--explicit` and the
TOML lookup.
...
**Pros:** Single source of truth for "what zstack tears down". Trivial
...
alone (after zstack's prepare phase, which is now <10s after dropping per-file
gitleaks). On 501 files it took 10s. The scaling is worse than linear and the
bottleneck is inside gbrain, not the zstack orchestrator.
...
path. Not on the zstack critical path; zstack's architecture is already correct.

**Context:** Verified on real corpus 2026-05-10. zstack-side prepare with
...
`bin/zstack-memory-ingest.ts:ingestPass` reaching the `runGbrainImport` call
quickly, then the child process taking the bulk of the wall time.

**Depends on:** None — zstack's batch-ingest architecture (D1-D8 in
...
**What:** The current `~/.zstack/projects/<slug>/learnings.jsonl` storage works (append-only, tolerant parser, idle compactor) but Codex outside-voice (T5) flagged JSONL as "the wrong primitive" for multi-writer canonical state: lost-update on rewrite, partial-line corruption on crash, no transactions. v1.8.0.0 hardened JSONL with flock + O_APPEND but the right long-term primitive is SQLite (which Bun has built in via `bun:sqlite`).
...
**Cons:** Migration touches every consumer of `learnings.jsonl` — `/learn` scripts (`zstack-learnings-log`, `zstack-learnings-search`), domain-skills.ts read/write, gbrain-sync (which currently treats it as a flat file). Old `learnings.jsonl` files in the wild need a one-shot migration script.
...
### P2: Bump gbrain install-pin in lockstep with zstack memory-feature releases (#1305 part 2)

**What:** `bin/zstack-gbrain-install` pins gbrain to commit `08b3698` (v0.18.2). When zstack ships features that depend on newer gbrain ops or schema (e.g. v1.26.0 manifests + `code-def`/`code-refs`/`reindex-code`), the pin doesn't move with it. Fresh `/setup-gbrain` installs an old gbrain that fails `gbrain doctor` schema_version checks (24 vs latest 32+) until the user manually upgrades.

**Why:** Filed in #1305 alongside the `put_page` CLI bug. Out of scope for the v1.26.5.0 fix wave (separate release-coordination concern: which gbrain version we install vs. how we call it). The install-pin should either (a) auto-bump whenever zstack releases features that need newer gbrain, or (b) detect a stale pin during preamble and either auto-upgrade gbrain or print a one-line FIX hint.

**Pros:** Closes the "fresh-install paper-cut" path. New users land on a healthy schema. Reduces support noise on `/setup-gbrain` flows. Makes the zstack/gbrain release contract visible.

**Cons:** Adds release-cadence coupling between zstack and gbrain. Needs a policy: pin = "minimum version that still works" vs "latest known good." If gbrain ships a breaking change to `put` shape and zstack doesn't update the pin, fresh installs break in a new way.

**Context:** Issue #1305 part 1 (the `put_page` CLI verb bug) was handled in v1.26.5.0. Part 2 (this TODO) is the install-pin staleness. Pin lives in `bin/zstack-gbrain-install` near the top as a constant. Easiest minimal fix: ship the pin as a tracked release artifact (e.g. write it from `package.json` at build time) and add a doctor-style preamble check.
...
**What:** v1.26.5.0's `deriveCodeSourceId` drops the host segment to fit gbrain's 32-char source-id budget. This means `github.com/acme/foo` and `gitlab.com/acme/foo` collapse to the same `zstack-code-acme-foo`. `ensureSourceRegisteredSync()` in `bin/zstack-gbrain-sync.ts:323` will silently re-register the source when `local_path` differs, evicting one side.
...
## P3: Honor env vars in zstack-config (so QUESTION_TUNING/EXPLAIN_LEVEL actually isolate tests)

**What:** `zstack-config get <key>` reads `~/.zstack/config.yaml`. `runPlanSkillObservation` plumbs `env: { QUESTION_TUNING: 'false', EXPLAIN_LEVEL: 'default' }` through to the spawned `claude` process — but the skill preamble bash uses `zstack-config get question_tuning`, which never looks at env. The env passthrough is theater on current code.
...
**Pros:** Makes the gate test hermetic across machines. The env wiring is already in place — only `zstack-config` needs to read env first, fall back to YAML.
**Cons:** Touches the zstack-config binary across all 3 platforms (linux/darwin/windows). Cross-binary refactor.

**Context:** Captured from v1.21.1.0 adversarial review. Documented honestly in the test docstring as a known limitation.

**Priority:** P3.
**Effort:** S. Single-file edit to `bin/zstack-config` (~10 LOC for env-first lookup).

---

## P3: Path-confusion hardening on SANCTIONED_WRITE_SUBSTRINGS

**What:** `runPlanSkillObservation`'s silent-write detector uses substring matching on a few sanctioned paths (`.zstack/`, `CHANGELOG.md`, `TODOS.md`, etc). A write to `node_modules/some-pkg/CHANGELOG.md` or `src/foo/.zstack/leak.ts` is currently sanctioned because the substring matches anywhere in the path.

**Why:** Defensive — no current bug exploits this, but a malicious skill or fixture could write to a path that happens to contain `.zstack/` or `CHANGELOG.md` and slip past silent-write detection.

**Pros:** Hardens the harness against future skill misbehavior. Aligns substring rules with their intent.
**Cons:** Need to anchor against absolute prefixes (`os.homedir() + '/.zstack/'`, worktree root) which makes the test less portable across machines.
...
**Context:** Relevant files — `scripts/question-registry.ts` (typed question catalog), `scripts/resolvers/question-tuning.ts` (preference classification), `bin/zstack-question-log` (event log), `bin/zstack-question-preference` (read/write preferences), `test/helpers/agent-sdk-runner.ts` (canUseTool harness). Existing question-log already captures fire events; the gap is declaring expected counts and auditing against them.

**Effort:** L (human: ~1-2 weeks / CC+zstack: ~2-3 hours for design doc + first-pass implementation).
...
**Context:** Source of the lane data model is `plan-eng-review/SKILL.md.tmpl:240-249` (the "Parallelization Strategy" output with Lane A/B/C dependency tables and conflict flags). Deferred from the v0.18.5.0 rename PR so the rename could land as a tight, low-risk fix. Saved files currently live at `~/.zstack/projects/$SLUG/checkpoints/YYYYMMDD-HHMMSS-<title>.md` with YAML frontmatter (branch, timestamp, etc.). The lane feature would add a `lane:` field to frontmatter and a `--lane` filter to both skills.
...
`~/.zstack/developer-profile.json` on preamble and adapts skill-specific
...
**Pros:** zstack feels personal. Every skill adapts to the user's steering
...
right reads of `~/.zstack/developer-profile.json` and the right decision
...
**Pros:** Killer delight feature. Social surface for zstack. Concrete, specific
...
via `zstack-config set blind_spot_coach false`.
...
**Why:** Biggest personality moment in zstack. The "one-word thing that makes
...
the right trigger shape. Dedup marker per PR in `~/.zstack/.landed-celebrated-*`.
...
zstack feels like it's reading your mind.
...
**What:** TestSavantAI was trained on direct-injection text, wrong distribution for browser-agent attacks (measured 15% recall). Take BERT-base, fine-tune on BrowseSafe-Bench (3,680 cases) + Qualifire prompt-injection-benchmark (5k) + xxz224 (3.7k) combined, ship in ~/.zstack/models/ as replacement L4 classifier.
...
**What:** Flip `ZSTACK_SECURITY_ENSEMBLE=deberta` from opt-in to default. Adds a 3rd ML vote; 2-of-3 agreement rule should reduce FPs while catching attacks that only DeBERTa sees.
...
**What:** Every Allow/Block click is labeled data. Log (suspected_text hash, layer scores, user decision, ts) to ~/.zstack/security/feedback.jsonl. Aggregate via community-pulse when `telemetry: community`. Periodically retrain the classifier on aggregate feedback.
...
#### ~~Attack telemetry via zstack-telemetry-log (P1)~~ — SHIPPED
...
(community-pulse edge function security aggregation) + 756875a7 (bin/zstack-
security-dashboard). Users can now run `zstack-security-dashboard` to see
...
Web UI at zstack.gg/dashboard/security is still open — that's a separate
...
`ZSTACK_SECURITY_ENSEMBLE=deberta` — sidebar-agent warmup downloads the 721MB
model to ~/.zstack/models/deberta-v3-injection/ on first run. combineVerdict
...
**Context:** Google shipped Chrome DevTools MCP in Chrome 146+ (June 2025). It provides screenshots, console messages, performance traces, Lighthouse audits, and full page interaction through the user's real browser. zstack should use it for real-session access while keeping Playwright for headless CI/testing workflows.
...
`$B state save/load` ships in v0.12.1.0. V1 saves cookies + URLs only (not localStorage, which breaks on load-before-navigate). Files at `.zstack/browse-states/{name}.json` with 0o600 permissions. Load replaces session (closes all pages first). Name sanitized to `[a-zA-Z0-9_-]`.
...
`$B connect` launches Playwright's bundled Chromium in headed mode with the zstack Chrome extension auto-loaded. `$B handoff` now produces the same result (extension + side panel). Sidebar chat gated behind `--chat` flag.
...
**What:** Publish the zstack browse Chrome extension to Chrome Web Store for easier install.
...
**What:** A PreToolUse hook (settings.json) that forces or verifies `run_in_background: false` on Agent tool calls made inside zstack workflows, making the #497/#2440 bug class structurally impossible on Claude Code instead of prose-pinned.
...
**Context:** Third recurrence of the class (#497 → #2440 → /ship Step 18 stranding). The hook must scope to zstack skill sessions (never break legitimate background Agent use elsewhere), is Claude-host only (other hosts get nothing from it), and mirrors the existing question-preference PreToolUse hook wiring in `bin/zstack-settings-hook*`. Filed from the v1.79.0.0 CEO plan review (approach C, deliberately split out for bake time).
...
**Context:** Redesigns the #2733 JSON contract (files_updated/commit_sha/pushed/documentation_section/decisions), so it needs its own PR with bake time. Start from `ship/sections/pr-body.md.tmpl` Step 18 and `document-release/SKILL.md.tmpl`'s spawned contract; decide whether the mode is a dispatch-prompt parameter or a `ZSTACK_DOC_RELEASE_MODE` env the preamble echoes.
...
**What:** Append structured JSON entry to `.zstack/ship-log.json` at end of every /ship run (version, date, branch, PR URL, review findings, Greptile stats, todos completed, test results).
...
**Effort:** M
**Priority:** P2
**Depends on:** /setup-zstack-upload
...
### /setup-zstack-upload skill (S3 bucket)
...
### zstack-upload helper

**What:** `browse/bin/zstack-upload` — upload file to S3, return public URL.
...
**Context:** Reads `~/.zstack-dev/evals/*.json`. ~200 lines HTML + chart.js via Bun HTTP server.
...
**What:** `zstack-open-url` helper script — detect platform, use `open` (macOS) or `xdg-open` (Linux).

**Why:** The first-time Completeness Principle intro uses macOS `open` to launch the essay. If zstack ever supports Linux, this silently fails.
...
**Why:** Cross-team design discovery at scale. Local `~/.zstack/projects/$SLUG/` keyword-grep discovery works for same-machine users now, but Supabase sync makes it work across the whole team. Duplicate ideas surface, everyone sees what's been explored.

**Context:** /office-hours writes design docs to `~/.zstack/projects/$SLUG/`. The team store already syncs test plans, retro snapshots, QA reports. Design docs follow the same pattern — just add a sync adapter.
...
**What:** Capture auto-chosen decisions from spawned sessions (OPENCLAW_SESSION or ZSTACK_SESSION_KIND=spawned) into `zstack-question-log` so `/plan-tune` learning sees them.
...
**Context:** #2733 made spawned sessions reachable from Claude Code subagents (every Conductor-hosted /ship now produces one). The subagent reports auto-chosen decisions in the JSON contract's `decisions` array (user-visible in the ship console), but nothing writes them to `~/.zstack/` question analytics. Start from the spawned-session instruction block in `bin/zstack-skill-start` — add a "log each auto-chosen decision with bin/zstack-question-log" sentence and a `source` value distinguishing auto-chosen from human-answered so tuning never trains on machine picks as if a human made them.

**Effort:** S
**Priority:** P3
**Depends on:** #2733 fix (ZSTACK_SESSION_KIND=spawned marker) landing.
...
marks the subagent `ZSTACK_SESSION_KIND=spawned` so its interactive gates
...
**What:** Create a placeholder resolver in gen-skill-docs.ts encoding the zstack voice guide (friendly, user-forward, lead with benefits). Inject into /ship Step 5, /document-release Step 5, and reference from CLAUDE.md.
...
`bin/zstack-diff-scope` shipped — categorizes diff into SCOPE_FRONTEND, SCOPE_BACKEND, SCOPE_PROMPTS, SCOPE_TESTS, SCOPE_DOCS, SCOPE_CONFIG. Used by design-review-lite to skip when no frontend files changed. Dashboard integration for conditional row display is a follow-up.
...
**Effort:** S
**Priority:** P3
**Depends on:** zstack-diff-scope (shipped)
...
**What:** Track how often Claude chooses the complete option vs shortcut across zstack sessions. Aggregate into a dashboard showing completeness trend over time.
...
5. Debug session persistence (~/.zstack/investigate-sessions/ — save investigation for reuse)
...
**What:** Add ~10 lines of prose to the preamble telling the agent to re-read zstack artifacts (CEO plans, design reviews, eng reviews, checkpoints) after compaction or context degradation.
...
**What:** Append one-line JSONL entry to `~/.zstack/projects/$SLUG/timeline.jsonl` after every skill run (timestamp, skill, branch, outcome). `/retro` renders the timeline.
...
**What:** When a new zstack session starts on a branch with recent checkpoints or plans, the preamble prints a one-line summary: "Last session: implemented JWT auth, 3/5 tasks done." Agent knows where you left off before reading any files.
...
**What:** Manual skill to snapshot current working state: what's being done and why, files being edited, decisions made (and rationale), what's done vs. remaining, critical types/signatures. Saved to `~/.zstack/projects/$SLUG/checkpoints/<timestamp>.md`.
...
**What:** Write `docs/designs/SESSION_INTELLIGENCE.md` describing the architectural vision: zstack as the persistent brain that survives Claude's ephemeral context. Every skill writes to `~/.zstack/projects/$SLUG/`, preamble re-reads, `/retro` rolls up.
...
**What:** Skill that runs type-check, lint, test suite, and dead code scan, then reports a composite 0-10 health score with breakdown by category. Tracks over time in `~/.zstack/health/<project-slug>/` for trend detection. Optionally integrates CodeScene MCP for deeper complexity/cohesion/coupling analysis.
...
**What:** Expose zstack's browse binary and key workflows as an MCP server that Factory Droid connects to natively. Factory users would run /mcp, add the zstack server, and get browse, QA, and review capabilities as Factory tools.

**Why:** Factory already supports 40+ MCP servers in its registry. Getting zstack's browse binary listed there is a distribution play. Nobody else has a real compiled browser binary as an MCP tool. This is the thing that makes zstack uniquely valuable on Factory Droid.
...
**What:** Factory has "custom droids" (subagents with tool restrictions, model selection, autonomy levels). Could ship `zstack-qa.md` droid configs alongside skills that restrict tools to read-only + execute for safety.

**Why:** Deeper Factory integration. Droid configs give Factory users tighter control over what zstack skills can do.
...
**What:** Maintain a Chromium fork where anti-bot stealth, ZStack Browser branding, and native sidebar support live in the source code, not as runtime monkey-patches.

**Why:** The CDP patches are brittle. They break on every Playwright upgrade and target compiled JS with fragile string matching. A proper fork means: (1) stealth is permanent, not patched, (2) branding is native (no plist hacking at launch), (3) native sidebar replaces the extension (Phase 4 of V0 roadmap), (4) custom protocols (zstack://) for internal pages. Companies like Brave, Arc, and Vivaldi maintain Chromium forks with small teams. With CC, the rebase-on-upstream maintenance could be largely automated.

**Context:** Trigger criteria from V0 design doc: fork when extension side panel becomes the bottleneck, when anti-bot patches need to live deeper than CDP, or when native UI integration (sidebar, status bar) can't be done via extension. The Chromium build takes ~4 hours on a 32-core machine and produces ~50GB of build artifacts. CI would need dedicated build infra. See `docs/designs/ZSTACK_BROWSER_V0.md` Phase 5 for full analysis.
...
LIVE options exceeds a ZSTACK_DETACH_TIMEOUT env exported by zstack-detach.
**Where:** scripts/test-paid-shards.ts; bin/zstack-detach.
**Effort:** S (human ~2h, CC ~10min).

### P3: Eval store records the effective judge/capture model per run

**What:** Model defaults moved (capture Opus→Sonnet) and ZSTACK_EVAL_MODEL_JUDGE
...
### P3: DI-refactor zstack-gbrain-detect-mcp-mode test (~40s spawn cost, absorbed but real)
...
**Where:** test/zstack-gbrain-detect-mcp-mode.test.ts.
**Effort:** S (human ~2h, CC ~15min).
...
**What:** A Codex-native skill (`.agents/skills/zstack-claude/SKILL.md`) that runs `claude -p` to get an independent second opinion from Claude — the reverse of what `/codex` does today from Claude Code.
...
**Context:** The `/codex` skill template (`codex/SKILL.md.tmpl`) shows the pattern — it wraps `codex exec` with JSONL parsing, timeout handling, and structured output. The reverse skill would wrap `claude -p` with similar infrastructure. Would be generated into `.agents/skills/zstack-claude/` by `gen-skill-docs --host codex`.
...
### ✅ DONE (v1.69.0.0, zstack side): ZeroEntropy sunset detect + advisory
...
comments say never to select the legacy recipe; USING_GBRAIN_WITH_ZSTACK.md
...
on every AskUserQuestion/session stop. Fixed for ALL zstack hooks, not just
Stop: canonical-only registration via `_hook_command_path`, a KNOWN_HOOKS
identity table in `zstack-settings-hook` (survives Claude Code stripping
`_zstack_source` tags), a `prune-stale [--repoint|--all]` self-healer that
...
~/.zstack-dev/logs/free-suite-main-check.log (3 masked fails, exit 0).
...
- `test/brain-sync.test.ts` ZSTACK_HOME isolation fixed on main in v1.13.0.0.
...
- **Fixed the `bearer-token-json` regression in `bin/zstack-brain-sync`** — the value charset `[A-Za-z0-9_./+=-]{16,}` didn't permit spaces, so auth headers with the standard `Bearer <token>` form (literal space after the scheme name) slipped past the scanner. Added an optional `(Bearer |Basic |Token )?` prefix to the pattern. Validated against 5 positive cases (including the regression fixture) + 3 negative cases (short tokens, non-secret keys, random JSON). The 7-pattern secret scanner now passes all fixtures including bearer-json.
- **Added `test/zstack-brain-init-gh-mock.test.ts`** — 8 tests exercising the `gh` CLI auto-create path that previously had zero coverage. Stubs `gh` on PATH to record every call, asserts `gh repo create --private --description "..." --source <ZSTACK_HOME>` fires with the computed `zstack-brain-<user>` default name. Covers: happy path, fall-through-to-`gh repo view` when create hits already-exists, user-provided-URL-bypasses-gh, gh-not-on-path prompts for URL, gh-not-authed prompts for URL, idempotent `--remote` re-runs, conflicting-remote rejection.
...
- **Registered `brain-privacy-gate` in `test/helpers/touchfiles.ts`** (periodic tier) with dependency tracking on `scripts/resolvers/preamble/generate-brain-sync-block.ts`, `bin/zstack-brain-sync`, `bin/zstack-brain-init`, `bin/zstack-config`, and the Agent SDK runner. Diff-based selection will re-run the E2E whenever any of those change.
...
- Config CLI (`bin/zstack-config`), auto-upgrade via `~/.zstack/config.yaml`, 12h cache TTL, exponential snooze backoff (24h→48h→1wk), "never ask again" option, vendored copy sync on upgrade
**Completed:** v0.3.8
...
### P2: /zstack-reflect nightly synthesis skill (E2)

**What:** Scheduled skill that reads weekly `zstack/skill-run` + takes +
`get_recent_salience` and synthesizes a `zstack/insight` page surfaced at
...
real `zstack/skill-run` data to accumulate before designing the reflection
layer against real patterns instead of imagined ones.

**Effort:** L (human ~1-2 days, CC ~4-6h)

**Depends on:** Phase 0 (zstack/skill-run page type from v1.48.0.0) +
~6 weeks of accumulated data

### P3: Cross-machine brain-cache sync (E3)

**What:** Push compressed digests through the zstack-brain-sync git pipeline
...
### P3: /zstack-onboarding dedicated skill (E4)

**What:** Guided 5-minute setup skill for new zstack installs: walks user
through reading CLAUDE.md + README + recent commits to build `zstack/product`
...
`~/.zstack/analytics/section-reads.jsonl` and surface drift via
...
points at the zstack-root install path (`scripts/resolvers/sections.ts` builds it
...
**What:** Cache rendered diagram SVG/PNG in `~/.zstack/cache/diagram-render/`,
...
**What:** Rotate `~/.zstack/security/egress.jsonl` at a size threshold (match
...
`zstack-egress verify` can walk across generations.
...
`ZSTACK_EXTENSION_ID`; launch paths in `browse/src/browser-manager.ts` (~358,
...
- **P2 — telemetry-sync HTTP-status outcome is dead code.** `_ZSTACK_EGRESS_LAST_RECEIPT`
  is set inside a command-substitution subshell in `bin/zstack-telemetry-sync`, so the
...
- **P3 — DRY/robustness polish:** one shared `_zstack_egress_host_of` helper for the
...
- **P3 — test-coverage gaps the audit named:** `PAID_TEST_GLOBS` ↔ `package.json`
  `test:gate` parity test; `ZSTACK_EXTENSION_ID` ↔ `manifest.json` key derivation parity
  test (`browse/scripts/extension-id.ts`); a runner test asserting each shard child gets
  its own `ZSTACK_EVAL_DIR` under `shards/<slug>`; receipt-refusal branch tests for
...
`<zstack-qid:plan-design-review-scope-gate>` marker present), but the test's
...
`~/.zstack-dev/eval-runs/pdwu-verify-*.log`. **Effort:** M (human ~half day / CC ~30min).
...
- Playwright bootstrap abort/timeout absorbs (PRs 2233/2359, issues
  ...
  bounded (ZSTACK_PLAYWRIGHT_INSTALL_TIMEOUT, default 600s), lock contention
...
Every zstack skill that touches a web page drives the Aside AI browser first
(`scripts/resolvers/aside.ts` is the contract; `lib/aside-render.ts` /
`bin/zstack-render.ts` render local HTML through it; `{{ASIDE_RESEARCH}}` runs
web research through it). zstack's own browser engine — the `browse` daemon,
ZStack Browser headed mode, cookie import, `/pair-agent`, browser-skills /
...
`ASIDE_DIR=`, `ZSTACK_STEP_OK`), update the table in the same commit and add a
...
zstack's own CSP `<meta>` into the print template (`default-src 'none';
...
and zstack's is not; alternatively keep make-pdf on the bundled engine by
default.
...
through them. `zstack-render` now fences its own `EVAL`/`PAGE_ERRORS` lines and
...
- **Probe cost:** `probeAside()` runs two blocking spawns per process and the
  ...
  `ZSTACK_HOME` and lower the repl probe timeout on the code path. Effort S.

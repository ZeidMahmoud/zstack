...
```bash
~/.claude/skills/zstack/bin/zstack-evidence check --label tests --expect-cmd '<the project test command>' --max-age 24 --allow-paths CHANGELOG.md,VERSION,package.json,agents-digest/zstack-AGENTS.md
```
...
```bash
~/.claude/skills/zstack/bin/zstack-evidence run --label tests -- 'bun test 2>&1'
```
...
```bash
~/.claude/skills/zstack/bin/zstack-issue-guard pr-body
```

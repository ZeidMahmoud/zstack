...
zstack is MIT. The files below contain material derived from Apache-2.0 works,
rewritten in zstack's voice and modified. Rewriting is not an exemption: the
...
- `lib/design-catalog.ts`: rule ids and names from `crates/live/assets/antipatterns.json`; the prose is zstack's.
...
Not distributed: `bin/zstack-design-detect.ts` invokes an impeccable engine the
user installed. zstack does not ship or mirror that engine and never runs impeccable's
installer or launcher. The one download zstack can make is the engine binary
...
the egress ledger first. zstack does not audit the engine's network behavior; the
wrapper refuses URL targets so zstack never asks it to touch the network.

## DESIGN.md specification — Copyright Google LLC — Apache License 2.0

https://github.com/google-labs-code/design.md

`lib/design-md.ts`, `bin/zstack-design-md.ts`, and the Phase 6 template in

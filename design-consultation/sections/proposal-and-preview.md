...
_DESIGN_DIR="$ZSTACK_STATE_ROOT/projects/$SLUG/designs/design-system-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
Use real token values, no placeholders; omit invented `components` entries. Outside plan mode, after writing DESIGN.md, require `bun --no-env-file run ~/.claude/skills/zstack/bin/zstack-design-md.ts check DESIGN.md` to print `DESIGN_MD_FORMAT: spec`.

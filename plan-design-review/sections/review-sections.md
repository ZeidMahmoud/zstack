...
is no longer in context, Read `~/.claude/skills/zstack/plan-design-review/SKILL.md`
...
reconciliation from the Section self-check in `~/.claude/skills/zstack/plan-design-review/SKILL.md` (Read it if no longer in context). Export only agreed implementation work; retain unapproved remedies as pending findings.
...
```bash
~/.claude/skills/zstack/bin/zstack-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
...
```bash
~/.claude/skills/zstack/bin/zstack-learnings-log '{"skill":"plan-design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null)" 2>/dev/null || true
  ~/.claude/skills/zstack/bin/zstack-brain-cache invalidate brand --project "$SLUG" 2>/dev/null || true
```

...
preamble-tier: 3
version: 2.0.0
description: Designer's eye plan review — interactive, like CEO and Eng review. (zstack)
allowed-tools:
  - Read
  - Edit
  - Grep
...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"plan-design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "plan-design-review" --outcome OUTCOME \
...
### The zstack designer — YOUR PRIMARY TOOL

Use the **zstack designer** by default to turn design briefs into real visual mockups.
...
eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "brand"
  ~/.claude/skills/zstack/bin/zstack-brain-cache get brand --project "$SLUG" 2>/dev/null || printf '_(no brand digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
...
zstack designer is available (`DESIGN_READY` was printed during setup), **generate
mockups immediately.** Do not ask permission. This is the default behavior.

Tell the user: "Generating visual mockups with the zstack designer. This is how we
...
`$ZSTACK_STATE_ROOT/projects/$SLUG/designs/` (the configured state directory).
Mockups are design artifacts that inform the plan, not code changes. The zstack
...
- `mkdir -p "$ZSTACK_STATE_ROOT/projects/$SLUG/designs/..."`
...
**If `DESIGN_NOT_AVAILABLE`:** Tell the user: "The zstack designer isn't set up yet. Run `$D setup` to enable visual mockups. Proceeding with text-only review, but you're missing the best part." Then proceed to review passes with text-based review.
...
eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null)"
eval "$(~/.claude/skills/zstack/bin/zstack-paths)"
_DESIGN_DIR="$ZSTACK_STATE_ROOT/projects/$SLUG/designs/ideal-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D="$_ROOT/.claude/skills/zstack/design/dist/design"
[ -x "$D" ] || D=~/.claude/skills/zstack/design/dist/design
...
> **STOP.** Before running the 7 design passes, required outputs, and review report (only after Step 0 scope is agreed), Read `~/.claude/skills/zstack/plan-design-review/sections/review-sections.md` and execute it

 ...
 * Conductor's ZSTACK_-prefixed forms are present. Promote the ZSTACK_ form to
...
/**
 * Pure form: returns a copy of `base` with each ZSTACK_-prefixed key promoted
...
export function promotedEnv(base: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const key of PROMOTED_KEYS) {
    if (!out[key] && out[`ZSTACK_${key}`]) {
      out[key] = out[`ZSTACK_${key}`];
    }
  }
  return out;
}

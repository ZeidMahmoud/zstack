 ...
 * zstack's own headless browser as the fallback.
 *
 ...
 * closed app), the same RenderSpec runs through the `browse` daemon (zstack's
 ...
 *      `ZSTACK_RENDER_OK` sentinel on stdout. A `[error` line means failure.
 *
...
export const RENDER_SENTINEL = 'ZSTACK_RENDER_OK';
...
/** The page-number footer shared by make-pdf, zstack-render and the browse `pdf` command. */
...
export function probeAside(timeoutMs = 30_000): AsideProbe {
  if (process.env.ZSTACK_SKIP_ASIDE === '1') {
    return { ok: false, reason: 'NEEDS_ASIDE', detail: 'ZSTACK_SKIP_ASIDE=1 — Aside skipped by request' };
  }
...
export function buildRenderScript(url: string, spec: RenderSpec): string {
  ...
  L.push(`console.log("PAGE_ERRORS=" + JSON.stringify(await pg.evaluate(() => window.__zstackErrs || [])));`);
...
async function runProc(cmd: string, args: string[], timeoutMs: number): Promise<{ code: number | null; stdout: string; stderr: string; error?: string }> {
  ...
  // loop alive and a CLI with no explicit process.exit (zstack-render) would sit
...
async function asideRender(spec: RenderSpec): Promise<RenderResult> {
  ...
  try {
    ...
    if (!stdout.split('\n').some((l) => l.trim() === RENDER_SENTINEL)) {
      ...
      return { ok: false, outputs: [], evals, stdout, error: `render script did not finish: ${errLine || 'no output'} (ZSTACK_SKIP_ASIDE=1 forces zstack's own browser)` };
    }
...
export function renderTmpDir(): string {
  const dir = path.join(os.tmpdir(), 'zstack-render');
  ...
  return ours() ? dir : fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-render-'));
}

// ─── Render: browse (zstack's own headless browser, the fallback) ────────────
...
const BROWSE_ROOTS = [
  ...
  path.join(os.homedir(), '.claude/skills/zstack'),
];
...
/**
 * Locate zstack's own browse binary: $ZSTACK_BROWSE_BIN → $BROWSE_BIN →
...
export function resolveBrowseBin(env: NodeJS.ProcessEnv = process.env, roots: string[] = BROWSE_ROOTS): string | null {
  ...
  const override = (env.ZSTACK_BROWSE_BIN ?? env.BROWSE_BIN ?? '').trim().replace(/^"(.*)"$/, '$1');
  if (override) {
...
export async function renderWithBrowse(spec: RenderSpec, bin: string | null = resolveBrowseBin()): Promise<RenderResult> {
  ...
  try {
    work = fs.mkdtempSync(path.join(SAFE_TMP_DIR, 'zstack-render-browse-'));
    srv = serveDir(root);
    ...
    const errs = await bestEffortJs('JSON.stringify(window.__zstackErrs || [])', 'PAGE_ERRORS');
...
export const NO_BROWSER_HELP = "open the Aside app (macOS 15+, aside.com), or run ./setup in the zstack repo to build zstack's own headless browser (or point ZSTACK_BROWSE_BIN at a browse binary)";
...
/** Aside when it answers, else zstack's own browser, else neither. Cached per process (the Aside probe is a round-trip). */
 ...
 * zstack's own browser when that is built, and the choice sticks for the rest
...
export async function render(spec: RenderSpec): Promise<RenderResult> {
  ...
  if (c.engine === 'aside') {
    ...
    // Retry on zstack's own browser when Aside could not START (spawn error,
    ...
    if (!r.ok && /^aside repl did not run: (?!timed out)|_sendToTarget|openTab is not defined/.test(r.error ?? '')) {
      ...
      if (bin) {
        ...
        return { ...fb, stdout: `[aside unavailable mid-run: ${r.error}] retried on zstack's own browser\n${fb.stdout}` };
      }
    }
    return r;
  }

...
int wmain(int argc, wchar_t **argv) {
  if (argc < 4) { fputs("zstack-cso: publish lock requires a directory and command\n", stderr); return 69; }
  wchar_t directory[CSO_COMMAND_CAP];
  DWORD length = GetFullPathNameW(argv[1], CSO_COMMAND_CAP, directory, NULL);
  if (!length || length >= (DWORD)CSO_COMMAND_CAP) { fputs("zstack-cso: publication directory is invalid\n", stderr); return 69; }
  wchar_t gate_path[CSO_COMMAND_CAP];
  if (!joined_path(gate_path, CSO_COMMAND_CAP, directory, L".zstack-cso-generation.lock")) {
    fputs("zstack-cso: generation lock path is too long\n", stderr); return 69;
  }
  ...
  for (int i = 2; i < argc; i++)
    if ((i > 2 && !append(command, &used, L' ')) || !argument(command, &used, argv[i])) {
      fputs("zstack-cso: publication command is too large\n", stderr); return 69;
    }
  if (!SetEnvironmentVariableW(L"ZSTACK_CSO_PUBLISH_LOCKED", L"1")) return 69;
  ...
  if (!CreateProcessW(argv[2], command, NULL, NULL, TRUE, CREATE_SUSPENDED, NULL, NULL, &startup, &child)) {
    fwprintf(stderr, L"zstack-cso: publication command could not start (Windows error %lu)\n", (unsigned long)GetLastError()); return 69;
  }
...
int main(int argc, char **argv) {
  if (argc < 4 || argv[2][0] != '/') {
    fputs("zstack-cso: publish lock requires a directory and absolute command\n", stderr); return 69;
  }
  int directory = open(argv[1], O_RDONLY | O_DIRECTORY | O_NOFOLLOW);
  struct stat state;
  if (directory < 0 || fstat(directory, &state) != 0 || !S_ISDIR(state.st_mode)) {
    fputs("zstack-cso: publication directory is unavailable\n", stderr); return 69;
  }
  if (flock(directory, LOCK_EX | LOCK_NB) != 0) {
    if (errno == EWOULDBLOCK || errno == EAGAIN) return 73;
    fputs("zstack-cso: publication lock is unavailable\n", stderr); return 69;
  }
  if (setenv("ZSTACK_CSO_PUBLISH_LOCKED", "1", 1) != 0) return 69;
  execv(argv[2], &argv[2]);
  fputs("zstack-cso: publication command could not start\n", stderr);
  return 69;
}
#endif

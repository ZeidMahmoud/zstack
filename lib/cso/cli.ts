...
const GENERATION_LOCK_FD=(()=>{
  ...
  if(process.platform==='win32'){
    if(process.env.ZSTACK_CSO_GENERATION_GUARD!=='inherited-windows-generation-handle-v3')throw new CsoError('ISOLATION_FAILED','Direct use of the internal CSO payload is unsupported; invoke zstack-cso-launcher');
    delete process.env.ZSTACK_CSO_GENERATION_GUARD;return undefined;
  }
  const raw=process.env.ZSTACK_CSO_GENERATION_LOCK_FD;if(raw===undefined||!/^(?:[3-9]|[1-9][0-9]+)$/.test(raw))throw new CsoError('ISOLATION_FAILED','Direct use of the internal CSO payload is unsupported; invoke zstack-cso-launcher');const fd=Number(raw);let stat:fs.Stats;try{stat=fs.fstatSync(fd);}catch{throw new CsoError('ISOLATION_FAILED','The launcher publication lease was not preserved by the runtime');}const install=fs.statSync(dirname(process.execPath));if(!stat.isDirectory()||stat.dev!==install.dev||stat.ino!==install.ino)throw new CsoError('ISOLATION_FAILED','The launcher publication lease does not name the helper installation directory');delete process.env.ZSTACK_CSO_GENERATION_LOCK_FD;return fd;
})();
void GENERATION_LOCK_FD;
const HELP = `zstack-cso ${VERSION} (helper ABI ${ABI})
Usage:
  zstack-cso start --repo PATH [--comprehensive] [--diff] [--base REF] [--budget SECONDS] [--offline] [--infra|--code|--skills|--supply-chain|--owasp|--scope DOMAIN]
  zstack-cso doctor --repo PATH
  zstack-cso provision-images [--setup-summary] [--per-image-seconds 5..300]
  zstack-cso resume RUN
  zstack-cso inspect RUN
  zstack-cso read RUN PATH_OR_HANDLE
  zstack-cso history RUN [PATH_OR_HANDLE]
  zstack-cso submit RUN SUBMISSION.json
  zstack-cso scan RUN gitleaks|osv|semgrep|zizmor|trivy|schemathesis [REQUEST.json]
  zstack-cso scanner-outcome RUN ARTIFACT_ID
  zstack-cso import-sarif RUN REPORT.sarif
  zstack-cso record-review RUN REQUEST.json --producer ID
  zstack-cso test-plan RUN node|bun|python|rails
  zstack-cso runtime-plan RUN node|bun|python|rails --port PORT
  zstack-cso verify RUN REQUEST.json
  zstack-cso patch-hash REQUEST.json
  zstack-cso finish RUN
  zstack-cso replay BUNDLE_ID [--source MATCHING_SOURCE]
  zstack-cso recheck FINDING --repo PATH [--run RUN]
  zstack-cso import-v2 REPORT.json
  zstack-cso inspect-v2 IMPORT_ID
  zstack-cso schema
...
function callerPath(value:string):string {
  ...
  const cwd=process.env.ZSTACK_CSO_CALLER_CWD;
  if(!cwd||!isAbsolute(cwd))throw new CsoError('INVALID_ARGUMENT','Relative paths require the trusted zstack-cso launcher');
...
async function scanner(args:string[],sarif=false){
  ...
  return await withLock(dir,async()=>{
    ...
    if(sarif){
      ...
      record=await executeScanner({id,runId:report.runId,runDir:dir,manifest:readJson(join(dir,'snapshot.json')),policy:report.policy,executionDeadline:Date.parse(report.deadline)-60_000,platform:platform(),request,watchdogPath:join(dirname(process.execPath),'zstack-cso-watchdog')});
    }
...
function watchdog():string{const p=join(dirname(process.execPath),process.platform==='win32'?'zstack-cso-watchdog.exe':'zstack-cso-watchdog');if(!fs.existsSync(p))throw new CsoError('ISOLATION_FAILED','Trusted detached watchdog is missing');return p;}

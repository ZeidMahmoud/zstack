...
#endif
#ifndef ZSTACK_CSO_GIT_PATH
#error ZSTACK_CSO_GIT_PATH must bind the launcher to setup's resolved git.exe
#endif

static int fail(const char *message) {
  fprintf(stderr, "zstack-cso: %s\n", message);
  return 69;
}
...
int wmain(int argc, wchar_t **argv) {
  ...
  *slash = L'\0';
  ...
  if (!joined_path(gate_path, CSO_PATH_CAP, module, L".zstack-cso-generation.lock"))
  ...
  if (generation_gate == INVALID_HANDLE_VALUE || !GetFileInformationByHandle(generation_gate, &gate_info) ||
    ...
    return fail("installation generation lock is missing or invalid; run zstack setup/build");
  if (!joined_path(core, CSO_PATH_CAP, module, L"zstack-cso-core.exe"))
  ...
  if (pinned_core == INVALID_HANDLE_VALUE || !GetFileInformationByHandle(pinned_core, &core_info) ||
    ...
    return fail("trusted compiled helper is missing or invalid; run zstack setup/build");
  wchar_t generation_path[CSO_PATH_CAP];
  if (!joined_path(generation_path, CSO_PATH_CAP, module, L".zstack-cso-generation"))
  ...
  if (generation == INVALID_HANDLE_VALUE || !GetFileInformationByHandle(generation, &generation_info) ||
    ...
    return fail("generation manifest is missing or invalid; run zstack setup/build");
  CloseHandle(generation); actual[65] = '\0';
  if (actual[64] != '\n' || strncmp(actual, ZSTACK_CSO_CORE_SHA256, 64) != 0)
    return fail("launcher and compiled helper generations do not match; run zstack setup/build");
  char core_sha256[65];
  if (!sha256_handle(pinned_core, core_sha256) ||
      strcmp(core_sha256, ZSTACK_CSO_CORE_SHA256) != 0)
    return fail("compiled helper digest does not match its launcher; run zstack setup/build");
  ...
  if (!environment_entry(environment, &used, L"ZSTACK_CSO_CALLER_CWD", caller_cwd) ||
      !environment_entry(environment, &used, L"ZSTACK_CSO_GENERATION_GUARD", L"inherited-windows-generation-handle-v3") ||
      !environment_entry(environment, &used, L"ZSTACK_CSO_TRUSTED_GIT", ZSTACK_CSO_GIT_PATH) ||
      !inherited_entry(environment, &used, L"ZSTACK_HOME") ||
  ...
  if (wcslen(ZSTACK_CSO_GIT_PATH) >= CSO_ENV_VALUE_CAP) return fail("trusted Git path is too large");
  wcscpy(git_path, ZSTACK_CSO_GIT_PATH); wcscpy(git_directory, git_path);

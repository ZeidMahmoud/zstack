...
export function createPreparedExport(sourceRoot: string, exportRoot: string, maxBytes = MAX_EXPANDED, maxEntries = MAX_FILES): PreparedExportManifest {
  ...
  if (!Number.isSafeInteger(maxBytes) || maxBytes <= 0 || maxBytes > MAX_EXPANDED ||
    ...
    output === root || !output.startsWith(`${root}${sep}`) || !/^\.zstack-cso-export-[a-f0-9]{24}$/.test(output.slice(root.length + 1)))
...
async function download(raw: string, hosts: string[], destination: string, max: number): Promise<string> {
  ...
  for (let redirects = 0; redirects <= 3; redirects++) {
    const response = await fetch(current, { redirect: 'manual', proxy: 'http://127.0.0.1:18443', headers: { 'user-agent': `zstack-cso-preparation/${VERSION}`, accept: 'application/octet-stream' } } as any);
...
async function manifest(policyPath: string, archiveRoot: string, outputPath: string): Promise<void> {
  const outputMatch = outputPath.match(/^\/archives\/(\.zstack-cso-acquisition-export-[a-f0-9]{24})\/artifacts\.json$/);
...
async function seedBunCache(archives: NonNullable<Policy['archives']>): Promise<void> {
  const cacheRoot = '/work/.cso-bun-cache', seedRoot = '/tmp/zstack-cso-bun-seed';
  ...
  try {
    ...
    for (const pkg of packages) {
      ...
      const manifest = { name: `zstack-cso-seed-${pkg.inputIndex}`, private: true, dependencies: { [pkg.name]: pkg.version } };
...
async function seed(policyPath: string): Promise<void> {
  ...
  fs.writeFileSync('/work/.zstack-cso-preparation-ready', `${policy.planHash}\n`, { mode: 0o400, flag: 'wx' });
  await new Promise<void>(() => {});
}

function ready(): void {
  const stat = fs.lstatSync('/work/.zstack-cso-preparation-ready');
  if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1 || stat.size !== 65) die('offline preparation is not ready');
}

function finalize(): void {
  for (const path of ['/work/.cso-home', '/work/.cso-npm-cache', '/work/.cso-bun-cache', '/work/.cso-uv-cache', '/work/.zstack-cso-public-requirements.txt'])
    fs.rmSync(path, { recursive: true, force: true });
  fs.rmSync('/work/.zstack-cso-preparation-ready', { force: true });
  for (const path of ['/work/.cso-home', '/work/.cso-npm-cache', '/work/.cso-bun-cache', '/work/.cso-uv-cache', '/work/.zstack-cso-public-requirements.txt', '/work/.zstack-cso-preparation-ready'])
...
async function main(): Promise<void> {
  ...
  if (command === 'export-prepared' && args.length === 3 && args[0] === '/work' && /^\/work\/\.zstack-cso-export-[a-f0-9]{24}$/.test(args[1]) && /^\d+$/.test(args[2])) {

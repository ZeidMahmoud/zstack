...
static int cleanup_label(const char *run_dir,const char *docker,const char *endpoint,const char *label){
  char output[4096],filter[160];if(snprintf(output,sizeof output,"%s/watchdog.labels.%ld",run_dir,(long)getpid())>=(int)sizeof output||snprintf(filter,sizeof filter,"label=com.zstack.cso.run=%s",label)>=(int)sizeof filter)return -1;

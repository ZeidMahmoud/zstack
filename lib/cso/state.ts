...
export function stateRoot(env: Record<string,string|undefined> = process.env): string {
  // Mirrors bin/zstack-paths. Security artifacts are intentionally outside every sync allowlist.
  const userHome=env.HOME||(process.platform==='win32'?env.USERPROFILE:'');
  return resolve(env.ZSTACK_HOME || (env.CLAUDE_PLUGIN_ROOT?.toLowerCase().includes('zstack') ? env.CLAUDE_PLUGIN_DATA : '') || join(userHome || '.', '.zstack'));
}

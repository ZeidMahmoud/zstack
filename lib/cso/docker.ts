...
export class DockerGroup {
  ...
  private async launchWatchdog(explicit?:string):Promise<void>{
    const watchdog=explicit??join(dirname(process.execPath),'zstack-cso-watchdog');
  ...
  async createContainer(spec:ContainerSpec,joinAnchor=true):Promise<string>{
    ...
    const args=['create','--pull=never','--label',`com.zstack.cso.run=${this.runId}`,'--label',`com.zstack.cso.role=${spec.role}`,'--log-driver=none',
  ...
  async copyPreparedExport(id:string,containerPath:string,destination:string):Promise<void>{
    ...
    if(!/^\/work\/\.zstack-cso-export-[a-f0-9]{24}$/.test(containerPath))throw new CsoError('INVALID_SCHEMA','Prepared export path is outside the fixed helper contract');
  ...
  async copyAcquisitionExport(id:string,containerPath:string,destination:string):Promise<void>{
    ...
    if(!/^\/archives\/\.zstack-cso-acquisition-export-[a-f0-9]{24}$/.test(containerPath))throw new CsoError('INVALID_SCHEMA','Acquisition export path is outside the fixed helper contract');
  ...
  async cleanup():Promise<void>{
    let failure:unknown;try{
      const labeled=await this.docker(['ps','--all','--quiet','--no-trunc','--filter',`label=com.zstack.cso.run=${this.runId}`],128*1024),targets=new Set(this.ids.map(item=>item.id));
      ...
      const remaining=await this.docker(['ps','--all','--quiet','--no-trunc','--filter',`label=com.zstack.cso.run=${this.runId}`],8192);if(remaining)throw new CsoError('ISOLATION_FAILED','Run-owned Docker resources remain after cleanup');

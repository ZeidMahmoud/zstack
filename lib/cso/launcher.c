...
#ifndef ZSTACK_CSO_CORE_SHA256
#error ZSTACK_CSO_CORE_SHA256 must bind the launcher to its compiled core
#endif
...
int main(int argc,char **argv){
  ...
  if(!getcwd(caller_cwd,sizeof caller_cwd)){
    fputs("zstack-cso: caller working directory unavailable\n",stderr);return 69;
  }
  if(executable_path(located,sizeof located,argc?argv[0]:"")!=0||!realpath(located,self)){
    fputs("zstack-cso: launcher path unavailable\n",stderr);return 69;
  }
  char *slash=strrchr(self,'/');if(!slash){fputs("zstack-cso: invalid launcher path\n",stderr);return 69;}
  *slash=0;if(!self[0]){self[0]='/';self[1]=0;}
#ifdef ZSTACK_CSO_TESTING
  if(test_pause(argc,argv,"__cso-test-pause-before-generation-lock")<0){fputs("zstack-cso: test generation pause failed\n",stderr);return 69;}
#endif
  int generation_lock=open(self,O_RDONLY|O_DIRECTORY|O_NOFOLLOW);
  if(generation_lock<0){fputs("zstack-cso: installation lock unavailable\n",stderr);return 69;}
  while(flock(generation_lock,LOCK_SH)!=0)if(errno!=EINTR){fputs("zstack-cso: installation lock unavailable\n",stderr);return 69;}
  char core[PATH_MAX];if(snprintf(core,sizeof core,"%s%szstack-cso-core",self,strcmp(self,"/")?"/":"")>=(int)sizeof core){fputs("zstack-cso: core path too long\n",stderr);return 69;}
  int core_fd=openat(generation_lock,"zstack-cso-core",O_RDONLY|O_NOFOLLOW);struct stat core_state,st;
  if(core_fd<0||fstat(core_fd,&core_state)||!S_ISREG(core_state.st_mode)||core_state.st_nlink!=1||(core_state.st_mode&0111)==0){fputs("zstack-cso: trusted compiled helper is missing; run zstack setup/build\n",stderr);return 69;}
  int generation_fd=openat(generation_lock,".zstack-cso-generation",O_RDONLY|O_NOFOLLOW);char actual[66];size_t manifest_used=0;
  if(generation_fd<0||fstat(generation_fd,&st)||!S_ISREG(st.st_mode)||st.st_nlink!=1||st.st_size!=65){fputs("zstack-cso: generation manifest is missing or invalid; run zstack setup/build\n",stderr);return 69;}
  while(manifest_used<65){ssize_t count=read(generation_fd,actual+manifest_used,65-manifest_used);if(count>0){manifest_used+=(size_t)count;continue;}if(count<0&&errno==EINTR)continue;fputs("zstack-cso: generation manifest is missing or invalid; run zstack setup/build\n",stderr);return 69;}
  close(generation_fd);actual[65]='\0';
  if(actual[64]!='\n'||strncmp(actual,ZSTACK_CSO_CORE_SHA256,64)!=0){fputs("zstack-cso: launcher and compiled helper generations do not match; run zstack setup/build\n",stderr);return 69;}
  char core_sha256[65];if(sha256_file(core_fd,core_sha256)!=0||strcmp(core_sha256,ZSTACK_CSO_CORE_SHA256)!=0){fputs("zstack-cso: compiled helper digest does not match its launcher; run zstack setup/build\n",stderr);return 69;}
#ifdef ZSTACK_CSO_TESTING
  if(test_pause(argc,argv,"__cso-test-pause-after-core-verification")<0){fputs("zstack-cso: test core pause failed\n",stderr);return 69;}
#endif
  ...
  const char *names[]={"HOME","ZSTACK_HOME","CLAUDE_PLUGIN_DATA","CLAUDE_PLUGIN_ROOT","DOCKER_HOST","DOCKER_CONTEXT","DOCKER_CONFIG"};
  for(size_t i=0;i<sizeof names/sizeof names[0];i++){envp[e]=entry(names[i],"");if(!envp[e++]){fputs("zstack-cso: environment input is too large\n",stderr);return 69;}}
  envp[e]=fixed_entry("ZSTACK_CSO_CALLER_CWD",caller_cwd);if(!envp[e++]){fputs("zstack-cso: caller working directory is too large\n",stderr);return 69;}
  char lock_value[32];snprintf(lock_value,sizeof lock_value,"%d",generation_lock);
  envp[e]=fixed_entry("ZSTACK_CSO_GENERATION_LOCK_FD",lock_value);if(!envp[e++])return 69;
  envp[e]=NULL;
  ...
  if(chdir(self)!=0){fputs("zstack-cso: trusted working directory unavailable\n",stderr);return 69;}
#ifdef __linux__
  ...
  struct stat current;
  if(fstatat(generation_lock,"zstack-cso-core",&current,AT_SYMLINK_NOFOLLOW)!=0||!S_ISREG(current.st_mode)||current.st_dev!=core_state.st_dev||current.st_ino!=core_state.st_ino||current.st_nlink!=1){fputs("zstack-cso: compiled helper changed before execution; run zstack setup/build\n",stderr);return 69;}
  execve(core,child,envp);
#endif
  fprintf(stderr,"zstack-cso: trusted compiled helper could not start (%d)\n",errno);return 69;
}

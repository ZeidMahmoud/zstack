 ...
 * bin/zstack-gbrain-supabase-provision (thin bun-shebang entry).
 *
 ...
 * mirroring `_receipted_curl closed` in bin/zstack-egress-lib.sh.
 *
 * Exit codes:
 *   0 — success
...
const PROG = 'zstack-gbrain-supabase-provision';
 ...
 * is not reproduced.
 */
export const HELP_TEXT = `zstack-gbrain-supabase-provision — Supabase Management API wrapper for
 ...
 * and refusal message as _receipted_curl closed in zstack-egress-lib.sh).
 */
  ...
  for (;;) {
    ...
    try {
      const receipt = writeReceipt({
        ...
        consent: 'user ran zstack-gbrain-supabase-provision',
      });
      receiptId = receipt.id;
    } catch (error) {
      // Refused — the send never happens. Same problem/cause/fix contract as
      // _zstack_egress_refusal in zstack-egress-lib.sh, then exit 8.
      const home = resolveEgressHome(ctx.env);
      const cause = `EGRESS_RECEIPT_FAILED: ${(error as Error)?.message ?? error}`.replace(/\n/g, ' ');
      ctx.stderr(
        `zstack: supabase-provision NOT sent — the egress receipt could not be written (${cause}). ` +
          `Fix: chmod -R u+w ${path.join(home, 'security')} (or check ZSTACK_HOME). ` +
          `What this is: zstack records everything it ATTEMPTS to send off-machine; see zstack-egress.\n`,
      );
...
async function cmdPoolerUrl(ctx: Ctx, args: string[]): Promise<void> {
  ...
  // Override with ZSTACK_SUPABASE_TRUST_API_PORT=1 if a future API version
  // starts returning a working transaction port and this rewrite is wrong.
  if ((ctx.env.ZSTACK_SUPABASE_TRUST_API_PORT ?? '0') !== '1' && poolMode === 'transaction' && dbPort === '6543') {
    ctx.stderr(
      'pooler-url: API returned transaction pooler (port 6543); shared pooler for new projects listens on session port 5432 — rewriting (set ZSTACK_SUPABASE_TRUST_API_PORT=1 to disable)\n',
    );
    dbPort = '5432';
    poolMode = 'session';
  }
...
export async function runProvision(argv: string[], options: ProvisionOptions = {}): Promise<number> {
  ...
  try {
    switch (cmd) {
      ...
      case undefined:
      case '':
        die(ctx, 'usage: zstack-gbrain-supabase-provision {list-orgs|create|wait|pooler-url|list-orphans|delete-project|--help}');
        break;
      default:

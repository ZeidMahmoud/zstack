 ...
 * zstack-initiated off-machine send (`~/.zstack/security/egress.jsonl`, 0600).
 *
...
export interface WriteReceiptOptions {
  /** zstack home; resolved from env when omitted */
  home?: string;
  /** env for home resolution (tests) */
  env?: Env;
  /** which zstack component is sending */
  sink: string;
...
/**
 * Same resolution order as the rest of zstack (shell sinks, selection code):
 * ZSTACK_HOME, legacy ZSTACK_STATE_DIR, then $HOME/.zstack.
 */
export function resolveEgressHome(env: Env = process.env): string {
  const configured = env.ZSTACK_HOME || env.ZSTACK_STATE_DIR;
  if (configured) return path.resolve(configured);
  return path.join(env.HOME || os.homedir(), '.zstack');
}
 ...
 * zstack ATTEMPTS to send off-machine), how to inspect it, and that
 * trimming arrives with rotation.
 */
export function ledgerSizeWarning(ledger: string, size: number): string {
  const mb = (size / (1024 * 1024)).toFixed(1);
  return (
    `zstack: egress ledger is large (${mb}MB): ${ledger}. ` +
    `This file records what zstack ATTEMPTS to send off-machine (content-free receipts, for auditing). ` +
    `Inspect it with 'zstack-egress list'. Trimming arrives with ledger rotation (TODO); until then it only grows.`
  );
}

...
// `m_frontend` arm of bin/zstack-diff-scope (the bash source of truth for
// SCOPE_FRONTEND); test/frontend-scope.test.ts pins the two against the same
// sample paths so they cannot drift. bin/zstack-design-detect.ts uses this to
...
/** Repo-relative path (forward slashes) → is it a frontend file per zstack-diff-scope? */
export function isFrontendPath(relPath: string): boolean {
  const rel = relPath.replace(/\\/g, '/').replace(/^\.\//, '');
  const base = rel.slice(rel.lastIndexOf('/') + 1);
  const dot = base.lastIndexOf('.');
  const ext = dot >= 0 ? base.slice(dot) : ''; // case-sensitive, exactly like zstack-diff-scope's globs

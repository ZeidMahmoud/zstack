...
const OLD = 'zstack-claude';
const NEXT = 'zstack-claude-code';
const BANNER = '<!-- AUTO-GENERATED from';
const RUNTIME_FILES = ['bin/zstack-claude-code', 'lib/claude-code.ts', 'lib/claude-code-windows-job.ts', 'lib/claude-bin.ts', 'lib/outside-review-result.ts'];
...
function owned(entry: string, root: string): boolean {
  ...
  return generated(path.join(entry, 'SKILL.md')) || fs.existsSync(path.join(entry, '.zstack-owned'));
}
...
function retire(entry: string, root: string, oldSources: string[]): void {
  ...
  fs.rmSync(path.join(entry, '.zstack-owned'), { force: true });
...
export function migrateClaudeCodeSkills(opts: RenameOptions): { migrated: number; pending: string[] } {
  ...
  const render = opts.render ?? ((host: string, output: string) => {
    ...
    if (host === 'codex') {
      let model = env.ZSTACK_CODEX_GENERATION_MODEL;
      if (!model) {
  ...
  for (const target of candidates) {
    ...
    try {
      ...
      const runtime = path.join(target.dir, 'zstack');
      if (exists(runtime) && (
      ...
      )) throw new Error(`runtime root is not zstack-managed: ${runtime}`);
      ...
      temporary = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-claude-rename-'));
      ...
      if (!generated(path.join(skill, 'SKILL.md')) || !/^name:\s*(?:zstack-)?claude-code\s*$/m.test(content)) {
      ...
      if (!/^name:\s*(?:zstack-)?claude-code\s*$/m.test(fs.readFileSync(path.join(next, 'SKILL.md'), 'utf8'))) {
      ...
      for (const name of fs.readdirSync(renderedSkills)) {
        ...
        if (!owned(installed, root) && !(name === 'zstack' && !fs.existsSync(path.join(installed, 'SKILL.md')))) continue;
      ...
      if (fs.realpathSync(runtime) !== root) {
        for (const [name, alias] of [['zstack-office-hours', 'office-hours'], ['zstack-upgrade', 'zstack-upgrade']]) {
          const rendered = path.join(renderedSkills, name);

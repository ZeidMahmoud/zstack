/**
 * staging-guard — fail-closed ownership proof for zstack ingest staging dirs.
 *
 * Fixes #1802. The /sync-gbrain memory stage stages prepared pages to a
 * throwaway dir under ~/.zstack and `rm -rf`s it when done. The resume path
 ...
 * proved it owned. This module is the single definition of "a path zstack is
 ...
 *   2. Structural    — canonical path is a DIRECT child of $ZSTACK_HOME named
 ...
 *                      git working tree.
 *   4. Minted by us  — a `.zstack-staging` marker file (written by
 ...
 * mitigation at zstack's own rm -rf boundary; see the companion gbrain issue.
 */
...
export const STAGING_MARKER = ".zstack-staging";
...
/**
 * Prove (fail-closed) that `dir` is a zstack-owned ingest staging directory
 ...
 * @param zstackHome  Resolved $ZSTACK_HOME (injected for testability).
 */
export function checkOwnedStagingDir(dir: string, zstackHome: string): StagingVerdict {
  ...
  if (dirname(canon) !== home) {
    return { ok: false, reason: `not a direct child of ZSTACK_HOME (${home})` };
  }
...
export function isOwnedStagingDir(dir: string, zstackHome: string): boolean {
  return checkOwnedStagingDir(dir, zstackHome).ok;
}

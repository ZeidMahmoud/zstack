...
`bin/zstack-render.ts` (the CLI the /diagram skill runs) — in the Aside browser
when it is running, otherwise in zstack's own headless browser (the browse
...
| `__mountForScreenshot(svg, px)` | taint-proof fallback: mounts SVG at `#raster-stage` for `zstack-render --screenshot out.png --selector '#raster-stage'`. |
| `__probeImage(src)` | data URI/URL → `{width, height}` JSON. |
| `__bundleInfo` | `{ name, deps }` — pinned dependency versions baked at build. |

Readiness: poll until `#status` text is `ready` (or `zstack-render ... --wait-selector '#done'`).

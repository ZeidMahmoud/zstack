/**
 * zstack-memory-helpers — shared helpers for the V1 memory ingest + retrieval pipeline.
 *
 * Imported by:
 *   - bin/zstack-memory-ingest.ts (Lane A)
 *   - bin/zstack-gbrain-sync.ts   (Lane B)
 *   - bin/zstack-brain-context-load.ts (Lane C)
 ...
 *   ED1 — engine-tier cache lives in ~/.zstack/.gbrain-engine-cache.json (60s TTL)
 *
 ...
 *
 * Examples:
 *   https://github.com/garrytan/zstack.git → github.com/garrytan/zstack
 *   git@github.com:garrytan/zstack.git     → github.com/garrytan/zstack
...
function gitleaksAvailable(): boolean {
  ...
  try {
    ...
    process.stderr.write(
      "[zstack-memory-helpers] gitleaks not in PATH; secret scanning disabled. " +
 ...
 * Replaces the inadequate regex scanner in bin/zstack-brain-sync (which only
...
function zstackHome(): string {
  return process.env.ZSTACK_HOME || join(homedir(), ".zstack");
}

function engineCachePath(): string {
  return join(zstackHome(), ".gbrain-engine-cache.json");
}

function errorLogPath(): string {
  return join(zstackHome(), ".gbrain-errors.jsonl");
}

/**
 * Detect which gbrain engine is active (PGLite vs Supabase) and cache the
 * answer for 60s in ~/.zstack/.gbrain-engine-cache.json. Caching avoids
...
export function detectEngineTier(): EngineDetect {
  ...
  try {
    ...
    writeFileSync(
      engineCachePath(),
      JSON.stringify({ ...fresh, last_writer: "zstack-memory-helpers.detectEngineTier" }, null, 2),
      "utf-8"
    );
  } catch {
...
// Best-effort JSONL append to ~/.zstack/.gbrain-errors.jsonl. Never throws.
 ...
 * runtime read path used by zstack-brain-context-load; it tolerates extra
...
const ERROR_LOG_PATH = join(zstackHome(), ".gbrain-errors.jsonl");

/**
 * Wrap an op with structured error logging. Logs success/failure + duration
 * to ~/.zstack/.gbrain-errors.jsonl for forensic debugging. Replaces ad-hoc

 ...
 *
 * gbrain (the separate CLI zstack shells out to) can rm-rf a user's working tree
 * during an autopilot race (its own bug, upstream gbrain #1526). zstack can't fix
...
export function gbrainHome(env: NodeJS.ProcessEnv = process.env): string {
  ...
  // gbrain appends `.gbrain` itself, so zstack must too.

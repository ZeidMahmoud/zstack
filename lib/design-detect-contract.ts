...
// (bin/zstack-design-detect.ts) or the DESIGN.md tool (bin/zstack-design-md.ts)
...
//   scan  ──► stdout:  one JSON document (--format zstack) or engine bytes (--format raw)
...
//   any   ──► exit 3 + DESIGN_DETECT_INTERNAL_ERROR: a zstack bug, never retried
...
export const SENTINEL = {
  ...
  /** printed by zstack-design-md check / convert */
...
/**
 * Checksums zstack pins for the engine versions it has tested, per platform:
 ...
 * linux-x64 hash also matches the engine zstack's fixtures were captured with.
...
export const DETECT_LIMITS = {
  /** default engine wall clock; ZSTACK_DESIGN_DETECT_TIMEOUT_MS overrides */
  timeoutMs: 120_000,
...
/**
 * One pass over every shape the agent reads as zstack's own voice: the two fence

/**
 * bin-context — tiny shared helpers for non-interactive zstack bins that need the
 * project slug, current branch, and argv flags. Extracted from the decision bins
 * (zstack-decision-log / zstack-decision-search) so the slug/branch/flag plumbing
...
/** Keep the slug inside the [a-zA-Z0-9._-] alphabet zstack-slug promises (`tr -cd`). */
 ...
 * `C:\Users\j\foo` → `/c/Users/j/foo`. zstack-slug keys its cache on THAT form
 * (`tr '/' '_'`), so a native lookup must reproduce it exactly or it misses the very
 * entry zstack-slug wrote and silently re-derives instead of staying consistent.
...
// Marker tiers mirror bin/zstack-slug's `_outermost_project_root` exactly.
...
/**
 * Native port of bin/zstack-slug's `_outermost_project_root`: walk UP
...
/**
 * Native port of bin/zstack-slug's `_outermost_remote_repo` (step 1a): walk UP
...
/**
 * Native port of bin/zstack-slug's resolution order, used when that script cannot be
 ...
 * test/bin-context-windows-slug.test.ts against test/zstack-slug-cwd-walk-up.test.ts
 * and test/zstack-slug-parity.test.ts):
 *   0. $ZSTACK_PROJECT_SLUG env override — wins over everything, never cached.
 ...
 *      like `url = ..` must never escape ~/.zstack/projects/<slug>.
 *   4. Project root's basename; else basename(cwd) for plain non-project folders.
 */
export function slugFromEnvironment(zstackHome?: string, cwd: string = process.cwd()): string {
  const home = zstackHome || process.env.ZSTACK_HOME || join(homedir(), ".zstack");
  ...
  const envSlug = sanitizeSlug((process.env.ZSTACK_PROJECT_SLUG || "").trim());
  if (envSlug) return envSlug;

  // 1. outermost project root along the cwd ancestor chain (may be "").
  const projectRoot = outermostProjectRoot(cwd);

  // Lazy, memoized remote discovery (mirrors zstack-slug's _resolve_remote):
  ...
  // 3. canonical remote-derived slug from the outermost remote-bearing repo.
  //    Parse mirrors bin/zstack-slug step 2 exactly (byte-parity with
  ...
  if (!slug) {
    ...
    if (url) {
      ...
      // Dot-only / degenerate guard (mirrors bin/zstack-slug): a hostile
      // origin like `url = ..` yields "." or ".." here, which would file
      // state OUTSIDE ~/.zstack/projects/. Reject and let the basename
  ...
  // 5. cache it, as zstack-slug does — atomic, self-healing (only rewrites when
 ...
 *  PATHEXT match for an explicit path), so zstack-slug spawns ENOENT there. */
export const NEEDS_NATIVE_SLUG_ON_WINDOWS = process.platform === "win32";

/**
 * Resolve the project slug via the `zstack-slug` helper (parses `SLUG=...`).
 *
 ...
 * ~/.zstack/projects/unknown/ — one bucket shared by every project on the machine —

 ...
 *
 * Shared between bin/zstack-gbrain-detect (preamble probe on every skill start)
 * and bin/zstack-gbrain-sync.ts (orchestrator SKIP-when-not-ok semantics).
 ...
 *   - Cache: 60s TTL at ~/.zstack/.gbrain-local-status-cache.json, keyed on
 ...
 * Timeout → probe exceeded ZSTACK_GBRAIN_PROBE_TIMEOUT_MS (default 15s) with no
...
interface CacheEntry {
  // Local-cache schema version, controlled by zstack. Not to be confused
  // with `gbrain doctor --json` output schema_version (gbrain v0.25+ emits
  // schema_version: 2). Doctor-output parsing lives in
  // lib/zstack-memory-helpers.ts:freshDetectEngineTier and accepts both
...
/**
 * Effective probe timeout. `ZSTACK_GBRAIN_PROBE_TIMEOUT_MS` overrides the
...
export function probeTimeoutMs(env?: NodeJS.ProcessEnv): number {
  const raw = (env ?? process.env).ZSTACK_GBRAIN_PROBE_TIMEOUT_MS;
...
/** Cache path computed fresh on each call so tests can mutate ZSTACK_HOME per case. */
export function cacheFilePath(): string {
  return join(
    process.env.ZSTACK_HOME || join(userHome(), ".zstack"),
 ...
 * resolution in bin/zstack-skill-start (the single bash copy of the #2499
...
export function hasRemoteOnlyGbrainMcp(
  ...
  // Same classification rules as zstack-gbrain-detect's detectMcpMode tier 3,
...
// `ZSTACK_GBRAIN_VERSION_PROBE_TIMEOUT_MS` overrides for tests (same
// precedent as ZSTACK_GBRAIN_PROBE_TIMEOUT_MS on the sources probe).
...
function versionProbeTimeoutMs(env?: NodeJS.ProcessEnv): number {
  const raw = (env ?? process.env).ZSTACK_GBRAIN_VERSION_PROBE_TIMEOUT_MS;

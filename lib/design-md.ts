...
// from scripts/; bin/zstack-design-md.ts and design/src/memory.ts do the file work.
//
...
//        ──► convertLegacy  ──► zstack's pre-spec DESIGN.md (Product Context, Aesthetic Direction,
...
//        ──► upsertSection  ──► body-only splice on the parsed doc (files zstack writes from scratch)
...
//   spec files:   line 1 `---`, line 2 `# zstack: design-md-format=spec` (a YAML comment, so
//                 parsers that require `---` on line 1 keep working)
//   legacy files: line 1 `<!-- zstack: design-md-format=legacy-keep -->`
...
export const FORMAT_MARKER_PREFIX = 'zstack: design-md-format=';
...
/** `<!-- zstack: design-md-format=... -->` on line 1 (legacy files) */
const LEGACY_MARKER_RE = new RegExp('^<!--[ \\t]*' + MARKER_RE_BODY + '[ \\t]*-->\\n?');
/** `# zstack: design-md-format=...` as a YAML comment inside the front matter (spec files) */
...
/** Headings that mark zstack's pre-spec file by themselves (either one is enough evidence of a legacy shape). */
...
/** Headings that identify zstack's pre-spec DESIGN.md. */
...
export function isLegacyZstackFormat(doc: DesignMdDoc): boolean {
...
export function detectFormat(doc: DesignMdDoc | null): { format: DesignMdFormat; code: FormatCode; reason?: string } {
  ...
  if (isLegacyZstackFormat(doc)) return { format: 'legacy', code: 'legacy' };
  ...
  return { format: 'unknown', code: 'no-shape', reason: 'no front matter and no zstack legacy headings' };
}
 ...
 * that edits a file the user owns should use; renderDesignMd is for files zstack
...
/**
 * Convert zstack's pre-spec DESIGN.md into the open format. Product Context and

/**
 * zstack context-bill — token bill-of-materials for an installed zstack skills tree.
 *
 ...
 * envelope subtracted. Regenerate with `zstack-context-bill <tree> --exact
...
function totalMd(dir: string, tokensOf: TokensOf): { bytes: number; tokens: number } {
  ...
  for (const p of walkMd(dir)) {
    // A skill dir that CONTAINS other skill dirs (the zstack root skill wraps
 ...
 * installed ~/.claude/skills tree contains flat skill dirs PLUS a full zstack
 * repo checkout (`zstack/`, with .git). Its nested SKILL.md files are the
...
const USAGE =
  "Usage:\n" +
  "  zstack-context-bill [TREE] [--json] [--skill <name>]\n" +
  "  zstack-context-bill --diff <treeA> <treeB> [--json]\n" +
  "  zstack-context-bill [TREE] --budget <budget.json> [--json]\n" +
  "\n" +
  ...
  "                       zstack-egress); if it cannot be written, the run falls\n" +

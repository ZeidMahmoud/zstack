...
export function claudeCodeArgs(options: Pick<ClaudeCodeOptions, 'access' | 'resume'>, command: ClaudeCommand, env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  // An explicit zstack override wins; otherwise leave native CLI configuration
  // and ANTHROPIC_MODEL intact instead of replacing the user's selected model.
  if (env.ZSTACK_CLAUDE_MODEL) args.push('--model', env.ZSTACK_CLAUDE_MODEL);
...
export async function runClaudeCode(options: ClaudeCodeOptions): Promise<ClaudeCodeResult> {
  ...
  if (!command) return failure('not-found', 'Claude Code CLI not found. Install Claude Code or set ZSTACK_CLAUDE_BIN, then retry.', true);
  ...
  try {
    ...
    for (let i = 0; i < argv.length; i += 2) {
      if (!['--cwd', '--access', '--timeout-ms', '--resume'].includes(argv[i]) || values.has(argv[i]) || argv[i + 1] === undefined) {
        throw new Error('Usage: zstack-claude-code --cwd <repo> --access none|read-only --timeout-ms <n> [--resume <session-id>] (prompt on stdin)');
      }

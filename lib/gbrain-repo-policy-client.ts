 ...
 * trust store (bin/zstack-gbrain-repo-policy, a bash CLI that owns URL
 ...
 * veto; bin/zstack-gbrain-sync.ts code-import gate) each spawnSync'd the script
...
/** Absolute path of the policy store for this env (ZSTACK_HOME-aware). */
export function repoPolicyStorePath(env: NodeJS.ProcessEnv = process.env): string {
  const home = env.ZSTACK_HOME || join(env.HOME || homedir(), ".zstack");
...
const POLICY_SCRIPT = join(import.meta.dir, "..", "bin", "zstack-gbrain-repo-policy");

/**
 * Trust tier for a remote URL, via `zstack-gbrain-repo-policy get <url>`.
 *
...
export function repoPolicyTier(url: string | null, env: NodeJS.ProcessEnv = process.env): RepoPolicyResult {
  ...
  const res = spawnSync(cmd, args, {
    ...
    // process.env mutations (e.g. tests redirecting ZSTACK_HOME).
...
/**
 * Bulk trust-tier lookup via `zstack-gbrain-repo-policy get --batch` — ONE

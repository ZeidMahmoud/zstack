...
export function captureReviewStart(skill: string, env = process.env): string {
  if (!DIFF_REVIEWS.has(skill) || !env.ZSTACK_STAMP_WTREE || !env.ZSTACK_REVIEW_REPO) {
    throw new Error('cannot capture a diff review without a working-tree fingerprint');
  }
  const dir = join(env.ZSTACK_REVIEW_DIR!, '.review-starts');
  mkdirSync(dir, { recursive: true, mode: 0o700 });
  const token = crypto.randomUUID();
  writeFileSync(join(dir, `${token}.json`), JSON.stringify({
    skill, repo: env.ZSTACK_REVIEW_REPO, branch: env.ZSTACK_REVIEW_BRANCH,
    wtree: env.ZSTACK_STAMP_WTREE, started_at: new Date().toISOString(),
  }), { mode: 0o600, flag: 'wx' });
  return token;
}
...
export function bindReview(rec: Record<string, any>, token: string, env = process.env): Record<string, any> {
  ...
  if (env.ZSTACK_STAMP_COMMIT_FULL) rec.commit_full = env.ZSTACK_STAMP_COMMIT_FULL;
  if (env.ZSTACK_STAMP_TREE) rec.tree = env.ZSTACK_STAMP_TREE;
  if (env.ZSTACK_STAMP_DIRTY) rec.dirty = env.ZSTACK_STAMP_DIRTY === 'true';
  if (!DIFF_REVIEWS.has(rec.skill)) {
    if (env.ZSTACK_STAMP_WTREE) rec.wtree = env.ZSTACK_STAMP_WTREE;
    return rec;
  }

  let start;
  if (/^[0-9a-f-]{36}$/.test(token)) {
    const file = join(env.ZSTACK_REVIEW_DIR!, '.review-starts', `${token}.json`);
    try {
      const saved = readFileSync(file, 'utf8');
      unlinkSync(file);
      const parsed = JSON.parse(saved);
      if (parsed.skill === rec.skill && parsed.repo === env.ZSTACK_REVIEW_REPO &&
          parsed.branch === env.ZSTACK_REVIEW_BRANCH && parsed.wtree) start = parsed;
    } catch (error: any) {
      if (error.code !== 'ENOENT') console.error(`zstack-review-log: cannot consume review start: ${error.message}`);
    }
  }
  const end = env.ZSTACK_STAMP_WTREE;

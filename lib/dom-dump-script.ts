...
//     <style data-zstack-dom-css> in <head>, and removes each inlined <link>
...
export const DOM_DUMP_SCRIPT = String.raw`() => {
  ...
  liveLinks.forEach((link, i) => {
    ...
    try {
      ...
      inlined.push("/* zstack-dom-dump: " + (sheet.href || "link") + " */\n" + text);
  ...
  const cleanCss = (t) => t.replace(dataUrl, "url(data:,zstack-stripped)").replace(cssQuery, "url($1$2$1)");
  if (inlined.length) {
    const style = document.createElement("style");
    style.setAttribute("data-zstack-dom-css", "");
  ...
  for (const el of Array.from(root.querySelectorAll("style"))) {
    if (el.getAttribute("data-zstack-dom-css") === null && el.textContent) el.textContent = cleanCss(el.textContent);
  }
  ...
  for (const el of Array.from(root.querySelectorAll("*"))) {
    for (const attr of Array.from(el.attributes)) {
      ...
      else if (value.indexOf("data:") === 0 && value.length > 1024) el.setAttribute(name, "data:,zstack-stripped");
    }
  }
...
export const DOM_DUMP_STYLE_ATTR = 'data-zstack-dom-css';
/** Prefix of the trailing HTML comment the dump script appends. */
export const DOM_DUMP_NOTE_PREFIX = 'zstack-dom-dump:';

// lib/design-catalog.ts — zstack's design anti-pattern vocabulary, typed.
...
//     ├─ bin/zstack-design-detect.ts             normalizes engine findings by impeccableId
...
// id the detector cannot emit. Everything else is a zstack-only tell that the
...
export interface DesignSlopEntry {
  ...
  name: string;
  /** the doctrine line, zstack voice (cat 9, checklist, consultation bullets) */
  prose: string;
...
export const DESIGN_SLOP_CATALOG: DesignSlopEntry[] = [
  ...
  },
  {
    ...
    handoff: 'layout', source: 'zstack', legacyBlacklist: true,
  },
  {
    ...
    handoff: 'quieter', source: 'zstack', legacyBlacklist: true,
  },
  {
  ...
  },
  {
    ...
    handoff: 'polish', source: 'zstack', legacyBlacklist: true,
  },
  {
  ...
  },
  {
    ...
    handoff: 'clarify', source: 'zstack', legacyBlacklist: true,
  },
  {
  ...
  },
  {
    ...
    handoff: 'typeset', source: 'zstack', legacyBlacklist: true,
  },
  ...
  // ── zstack-only slop tells: the LLM pass judges these; no detector id, so no brackets. ──
  {
    ...
    handoff: 'colorize', source: 'zstack',
  },
  {
...
/** zstack-only slop tells (no detector rule), minus the legacy lines: the LLM pass is the detector. */

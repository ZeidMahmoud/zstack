...
Review, search, prune, and export what zstack
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"learn","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.zstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/zstack/bin/zstack-timeline-log '{"skill":"learn","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"learn","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
see what zstack has learned across sessions on this project, search for relevant
...
```bash
eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null)"
~/.claude/skills/zstack/bin/zstack-learnings-search --limit 20 2>/dev/null || echo "No learnings yet."
```
...
zstack will automatically capture patterns, pitfalls, and insights it discovers."
...
```bash
eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null)"
~/.claude/skills/zstack/bin/zstack-learnings-search --query "USER_QUERY" --limit 20 2>/dev/null || echo "No matches."
```
...
```bash
eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null)"
~/.claude/skills/zstack/bin/zstack-learnings-search --limit 100 2>/dev/null
```
...
```bash
eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null)"
~/.claude/skills/zstack/bin/zstack-learnings-search --limit 50 2>/dev/null
```
...
LEARN_FILE="$ZSTACK_STATE_ROOT/projects/$SLUG/learnings.jsonl"
...
```bash
~/.claude/skills/zstack/bin/zstack-learnings-log '{"skill":"learn","type":"TYPE","key":"KEY","insight":"INSIGHT","confidence":N,"source":"user-stated","files":["FILE1"]}'
```

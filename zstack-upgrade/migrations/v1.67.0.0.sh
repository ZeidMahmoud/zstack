...
# (~/.claude/skills/zstack), rewriting ~16 TRACKED SKILL.md files. The
# checkout stayed permanently dirty, and every /zstack-upgrade `git stash`
...
# version. v1.67 renders to an untracked out-dir (~/.zstack/render/claude)
...
#
# Affected: global-git installs that ever ran ./setup or `zstack-config
# gbrain-refresh` with gbrain detected, before v1.67.0.0.
#
# Idempotent: a clean checkout is a no-op. Non-fatal throughout.
set -u

INSTALL_DIR="${ZSTACK_INSTALL_DIR:-$HOME/.claude/skills/zstack}"

# Only operate on a real (non-symlink) git checkout that looks like zstack.
...
if [ "$_RESTORED" -gt 0 ]; then
  ...
  echo "  Brain-aware blocks now render to ~/.zstack/render/claude — the checkout stays clean from here on."
fi
if [ "$_LEFT" -gt 0 ]; then

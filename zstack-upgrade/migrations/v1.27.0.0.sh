#!/usr/bin/env bash
# Migration: v1.27.0.0 — rename zstack-brain-* → zstack-artifacts-*
#
# Phase C of the v1.27.0.0 plan. Hard-rename, no compat shim. Steps:
#   1. gh_repo_renamed       — gh/glab repo rename zstack-brain-$USER →
#                              zstack-artifacts-$USER (skipped on user opt-out)
#   2. remote_txt_renamed    — mv ~/.zstack-brain-remote.txt → artifacts-remote.txt
#   3. config_key_renamed    — rewrite gbrain_sync_mode → artifacts_sync_mode
#                              in ~/.zstack/config.yaml
...
#
# Interruption-safe via journal at ~/.zstack/.migrations/v1.27.0.0.journal:
# each step writes its name on success; re-entry resumes from the next un-done
# step. Done touchfile at ~/.zstack/.migrations/v1.27.0.0.done.
#
...
# Configuration
# ---------------------------------------------------------------------------
ZSTACK_HOME="${HOME}/.zstack"
SKILLS_DIR="${HOME}/.claude/skills"
BIN_DIR="${SKILLS_DIR}/zstack/bin"
CONFIG_BIN="${BIN_DIR}/zstack-config"
URL_BIN="${BIN_DIR}/zstack-artifacts-url"

MIGRATION_DIR="${ZSTACK_HOME}/.migrations"
...
RERUN_CMD="ZSTACK_MIGRATE_ASSUME_YES=1 bash ${SELF_PATH}"

USER_NAME="${USER:-$(whoami 2>/dev/null || echo unknown)}"
OLD_REPO_NAME="zstack-brain-${USER_NAME}"
NEW_REPO_NAME="zstack-artifacts-${USER_NAME}"
OLD_REMOTE_TXT="${HOME}/.zstack-brain-remote.txt"
NEW_REMOTE_TXT="${HOME}/.zstack-artifacts-remote.txt"
...
#   - presence of ~/.zstack-brain-remote.txt (legacy file)
#   - presence of ~/.zstack/.git (brain-init ever ran)
HAS_LEGACY_STATE=0
[ -f "$OLD_REMOTE_TXT" ] && HAS_LEGACY_STATE=1
[ -d "$ZSTACK_HOME/.git" ] && HAS_LEGACY_STATE=1

# If nothing to migrate, finalize silently.
if [ "$HAS_LEGACY_STATE" = "0" ]; then
  echo "  [v1.27.0.0] no legacy zstack-brain state detected — nothing to migrate." >&2
  touch "$DONE"
...
if [ ! -f "$JOURNAL" ]; then
  ...
  [v1.27.0.0] zstack-brain has been renamed to zstack-artifacts.
  ...
  This migration will:
    ...
    2. mv ~/.zstack-brain-remote.txt → ~/.zstack-artifacts-remote.txt
    3. Rename gbrain_sync_mode → artifacts_sync_mode in ~/.zstack/config.yaml
...
EOF
  if [ -t 0 ]; then
    ...
    # proceeds unattended.
    if [ "${ZSTACK_MIGRATE_ASSUME_YES:-0}" = "1" ]; then
      echo "  (non-interactive: proceeding — ZSTACK_MIGRATE_ASSUME_YES=1)" >&2
    else
...
# ---------------------------------------------------------------------------
# Step 2: rename ~/.zstack-brain-remote.txt → ~/.zstack-artifacts-remote.txt
# ---------------------------------------------------------------------------
if ! journal_done "remote_txt_renamed"; then
  echo "  [v1.27.0.0] step 2: rename ~/.zstack-brain-remote.txt → ~/.zstack-artifacts-remote.txt" >&2
  if [ -f "$OLD_REMOTE_TXT" ] && [ ! -f "$NEW_REMOTE_TXT" ]; then
    # Update the URL inside if the rename happened on the host: replace
    # zstack-brain-$USER with zstack-artifacts-$USER in the URL.
...
if ! journal_done "claude_md_block_rewritten"; then
  ...
  # Look in cwd's CLAUDE.md (where /setup-gbrain wrote it) and ~/.zstack/CLAUDE.md
  ...
  for CMD in "$PWD/CLAUDE.md" "$ZSTACK_HOME/CLAUDE.md"; do
    [ -f "$CMD" ] || continue
...
EOF
  ...
  elif command -v gbrain >/dev/null 2>&1 && [ -d "$ZSTACK_HOME/.git" ]; then
    ...
    WORKTREE="${ZSTACK_BRAIN_WORKTREE:-$HOME/.zstack-brain-worktree}"
    ...
    echo "    gbrain CLI not available or no ~/.zstack/.git — skipping" >&2
    mark_done "sources_swapped"
  fi
fi

...
if [ -f "$INSTALL_DIR/bin/zstack-migrate-claude-code" ] && command -v bun >/dev/null 2>&1; then
  bun "$INSTALL_DIR/bin/zstack-migrate-claude-code" --install-dir "$INSTALL_DIR" || true
fi
exit 0

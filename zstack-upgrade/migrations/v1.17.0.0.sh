...
#
# Pre-1.17.0.0 /setup-gbrain wrote ~/.zstack/consumers.json with a placeholder
...
#   - No ~/.zstack/.git (brain-init never ran)
...
# CLI is a benign skip rather than blocking the rest of /zstack-upgrade.
set -euo pipefail
...
WIREUP_BIN="${BIN_DIR}/zstack-gbrain-source-wireup"

# Skip if user never opted into brain-sync.
SYNC_MODE=""
if [ -x "$CONFIG_BIN" ]; then
  # Trim whitespace defensively: zstack-config can emit trailing newlines,

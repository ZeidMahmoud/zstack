...
# Info.plist ("Google Chrome for Testing" → "ZStack Browser") and overwrote
...
#
# Idempotent: detection is content-based (plist contains "ZStack Browser");
...
if [ -d "${PW_CACHE}" ]; then
  ...
  while IFS= read -r plist; do
    if grep -q "ZStack Browser" "${plist}" 2>/dev/null; then
...
if [ "${REMOVED}" = "1" ]; then
  ...
  # headed use the same bundle). Run from the zstack install root so bunx

...
#
# Why a migration: zstack-artifacts-init regenerates these files but also
# does `git commit + push` on ~/.zstack/, which would clobber user state on
...
if [ -f "${PRIVACY}" ]; then
  if command -v jq >/dev/null 2>&1; then
    ...
    echo "  [v1.38.1.0] WARN: jq not found; skipping privacy-map repair. Install jq and re-run zstack-upgrade, or run zstack-artifacts-init manually." >&2
  fi
fi
...
# patches ship into their federated artifacts repo (next zstack-brain-sync

...
name: zstack-upgrade
version: 1.1.0
description: Upgrade zstack to the latest version.
triggers:
  - upgrade zstack
  - update zstack version
  - get latest zstack
allowed-tools:
  - Bash
  - Read
...
runs the upgrade, and shows what's new. Use when asked to "upgrade zstack",
"update zstack", or "get latest version".

Voice triggers (speech-to-text aliases): "upgrade the tools", "update the tools", "gee stack upgrade", "g stack upgrade".

# /zstack-upgrade

Upgrade zstack to the latest version and show what's new.
...
```bash
_AUTO=""
[ "${ZSTACK_AUTO_UPGRADE:-}" = "1" ] && _AUTO="true"
[ -z "$_AUTO" ] && _AUTO=$(~/.claude/skills/zstack/bin/zstack-config get auto_upgrade 2>/dev/null || true)
echo "AUTO_UPGRADE=$_AUTO"
```

**If `AUTO_UPGRADE=true` or `AUTO_UPGRADE=1`:** Skip AskUserQuestion. Log "Auto-upgrading zstack v{old} → v{new}..." and proceed directly to Step 2. On setup failure, follow Step 4's install-specific recovery: vendored installs restore their backup; git installs stop with the pre-upgrade commit recorded, without a destructive reset. Never claim restoration unless it actually succeeded.

**Otherwise**, use AskUserQuestion:
- Question: "zstack **v{new}** is available (you're on v{old}). Upgrade now?"
...
```bash
~/.claude/skills/zstack/bin/zstack-config set auto_upgrade true
```
Tell user: "Auto-upgrade enabled. Future updates will install automatically." Then proceed to Step 2.

**If "Not now":** Write snooze state with escalating backoff (first snooze = 24h, second = 48h, third+ = 1 week), then continue with the current skill. Do not mention the upgrade again.
```bash
_SNOOZE_FILE="$HOME/.zstack/update-snoozed"
_REMOTE_VER="{new}"
_CUR_LEVEL=0
...
Tell user the snooze duration: "Next reminder in 24h" (or 48h or 1 week, depending on level). Tip: "Set `auto_upgrade: true` in `~/.zstack/config.yaml` for automatic upgrades."

**If "Never ask again":**
```bash
~/.claude/skills/zstack/bin/zstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.claude/skills/zstack/bin/zstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "$HOME/.claude/skills/zstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.claude/skills/zstack"
elif [ -d "$HOME/.zstack/repos/zstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.zstack/repos/zstack"
elif [ -d ".claude/skills/zstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".claude/skills/zstack"
elif [ -d ".agents/skills/zstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agents/skills/zstack"
elif [ -d ".claude/skills/zstack" ]; then
  INSTALL_TYPE="vendored"
  INSTALL_DIR=".claude/skills/zstack"
elif [ -d "$HOME/.claude/skills/zstack" ]; then
  INSTALL_TYPE="vendored-global"
  INSTALL_DIR="$HOME/.claude/skills/zstack"
else
  echo "ERROR: zstack not found"
  exit 1
fi
...
# ~/.zstack/render), so discarding is lossless.
...
git clone --depth 1 https://github.com/zeidmahmoud/zstack.git "$TMP_DIR/zstack" || { echo "ERROR: clone failed — aborting upgrade (install untouched)." >&2; rm -rf "$TMP_DIR"; exit 1; }
mv "$INSTALL_DIR" "$INSTALL_DIR.bak" || { rm -rf "$TMP_DIR"; exit 1; }
if mv "$TMP_DIR/zstack" "$INSTALL_DIR"; then
...
LOCAL_ZSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.claude/skills/zstack" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/.claude/skills/zstack" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_ZSTACK="$_ROOT/.claude/skills/zstack"
  fi
fi
_TEAM_MODE=$(~/.claude/skills/zstack/bin/zstack-config get team_mode 2>/dev/null || echo "false")
echo "LOCAL_ZSTACK=$LOCAL_ZSTACK"
echo "TEAM_MODE=$_TEAM_MODE"
```

**If `LOCAL_ZSTACK` is non-empty AND `TEAM_MODE` is `true`:** Remove the vendored copy. Team mode uses the global install as the single source of truth.

```bash
cd "$_ROOT"
git rm -r --cached .claude/skills/zstack/ 2>/dev/null || true
if ! grep -qF '.claude/skills/zstack/' .gitignore 2>/dev/null; then
  echo '.claude/skills/zstack/' >> .gitignore
fi
rm -rf "$LOCAL_ZSTACK"
```
Tell user: "Removed vendored copy at `$LOCAL_ZSTACK` (team mode active — global install is the source of truth). Commit the `.gitignore` change when ready."

**If `LOCAL_ZSTACK` is non-empty AND `TEAM_MODE` is NOT `true`:** Update it by copying from the freshly-upgraded primary install (same approach as README vendored install):
```bash
[ -e "$LOCAL_ZSTACK.bak" ] && { echo "ERROR: stale vendored backup; inspect it before retrying." >&2; exit 1; }
mv "$LOCAL_ZSTACK" "$LOCAL_ZSTACK.bak" || exit 1
if cp -Rf "$INSTALL_DIR" "$LOCAL_ZSTACK" && rm -rf "$LOCAL_ZSTACK/.git" && (cd "$LOCAL_ZSTACK" && ./setup); then
  rm -rf "$LOCAL_ZSTACK.bak"
  echo "LOCAL_SYNC_OK"
else
  rm -rf "$LOCAL_ZSTACK"
  mv "$LOCAL_ZSTACK.bak" "$LOCAL_ZSTACK" || { echo "ERROR: restore failed; backup retained." >&2; exit 1; }
  echo "ERROR: sync failed; previous vendored copy restored." >&2
  exit 1
fi
```
Only on `LOCAL_SYNC_OK`, tell user: "Also updated vendored copy at `$LOCAL_ZSTACK` — commit `.claude/skills/zstack/` when you're ready." Otherwise stop and report the recovery outcome; do not continue migrations or announce success.
...
```bash
MIGRATIONS_DIR="$INSTALL_DIR/zstack-upgrade/migrations"
if [ -d "$MIGRATIONS_DIR" ]; then
  for migration in $(find "$MIGRATIONS_DIR" -maxdepth 1 -name 'v*.sh' -type f 2>/dev/null | sort -V); do
    ...
    if [ "$OLD_VERSION" != "unknown" ] && [ "$(printf '%s\n%s' "$OLD_VERSION" "$m_ver" | sort -V | head -1)" = "$OLD_VERSION" ] && [ "$OLD_VERSION" != "$m_ver" ]; then
      ...
      # ZSTACK_INSTALL_DIR: migrations that clean the INSTALL (not just
      # ~/.zstack state) default to ~/.claude/skills/zstack when unset —
      # a repo-local install would silently no-op without this.
      ZSTACK_INSTALL_DIR="$INSTALL_DIR" bash "$migration" || echo "  Warning: migration $m_ver had errors (non-fatal)"
    fi
  done
fi
```

Migrations are idempotent bash scripts in `zstack-upgrade/migrations/`. Each is named
...
_STATE_FILE="${BROWSE_STATE_FILE:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)/.zstack/browse.json}"
...
```bash
mkdir -p ~/.zstack
echo "$OLD_VERSION" > ~/.zstack/just-upgraded-from
rm -f ~/.zstack/last-update-check
rm -f ~/.zstack/update-snoozed
```
...
Format:
```
zstack v{new} — upgraded from v{old}!
...
When invoked directly as `/zstack-upgrade` (not from a preamble):

1. Force a fresh update check (bypass cache):
```bash
~/.claude/skills/zstack/bin/zstack-update-check --force 2>/dev/null || \
.claude/skills/zstack/bin/zstack-update-check --force 2>/dev/null || true
```
...
Run the Step 2 bash block above to detect the primary install type and directory (`INSTALL_TYPE` and `INSTALL_DIR`). Then run the Step 4.5 detection bash block above to check for a local vendored copy (`LOCAL_ZSTACK`) and team mode status (`TEAM_MODE`).

**If `LOCAL_ZSTACK` is empty** (no local vendored copy): tell the user "You're already on the latest version (v{version})."

**If `LOCAL_ZSTACK` is non-empty AND `TEAM_MODE` is `true`:** Remove the vendored copy using the Step 4.5 team-mode removal bash block above. Tell user: "Global v{version} is up to date. Removed stale vendored copy (team mode active). Commit the `.gitignore` change when ready."

**If `LOCAL_ZSTACK` is non-empty AND `TEAM_MODE` is NOT `true`**, compare versions:
```bash
PRIMARY_VER=$(cat "$INSTALL_DIR/VERSION" 2>/dev/null || echo "unknown")
LOCAL_VER=$(cat "$LOCAL_ZSTACK/VERSION" 2>/dev/null || echo "unknown")
echo "PRIMARY=$PRIMARY_VER LOCAL=$LOCAL_VER"
```

**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `.claude/skills/zstack/` when you're ready."

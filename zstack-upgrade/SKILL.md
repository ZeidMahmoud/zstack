...
**If `AUTO_UPGRADE=true` or `AUTO_UPGRADE=1`:** Skip AskUserQuestion. Log "Auto-upgrading zstack v{old} → v{new}..." and proceed directly to Step 2. If `./setup` fails during auto-upgrade, restore from backup (`.bak` directory) and warn the user: "Auto-upgrade failed — restored previous version. Run `/zstack-upgrade` manually to retry."
...
```bash
mv "$LOCAL_ZSTACK" "$LOCAL_ZSTACK.bak"
cp -Rf "$INSTALL_DIR" "$LOCAL_ZSTACK"
rm -rf "$LOCAL_ZSTACK/.git"
cd "$LOCAL_ZSTACK" && ./setup
rm -rf "$LOCAL_ZSTACK.bak"
```
Tell user: "Also updated vendored copy at `$LOCAL_ZSTACK` — commit `.claude/skills/zstack/` when you're ready."

If `./setup` fails, restore from backup and warn the user:
```bash
rm -rf "$LOCAL_ZSTACK"
mv "$LOCAL_ZSTACK.bak" "$LOCAL_ZSTACK"
```
Tell user: "Sync failed — restored previous version at `$LOCAL_ZSTACK`. Run `/zstack-upgrade` manually to retry."

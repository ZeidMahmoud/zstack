...
name: cso
version: 3.0.0
description: "Security audit: supported static findings; qualified profiles add reproduction and repair candidates. (zstack)"
allowed-tools:
...
Find exploitable defects. State attacker, boundary, impact, and challenge. Static assessment remains available without runtime or scanner profiles. Qualified comprehensive profiles add reproduction and repair candidates without changing the branch. Trusted `zstack-cso` owns execution, persistence, and proof labels.

**Private startup.** Skip shared startup, learning, checkpoint, and telemetry. Resolve `bin/zstack-cso-launcher` (or `.exe`) and sections from the trusted installed zstack distribution. Never use the repository, PATH, Bun, or Node as a bypass. If unavailable, report **not assessed** with the install prerequisite; run no repository tooling.
...
Use `zstack-cso schema` for JSON inputs and `--help` for the installed contract. Below, `zstack-cso` means the trusted absolute launcher:

```text
zstack-cso start --repo <repo> [scope/diff/base/budget/offline flags] [--comprehensive]
zstack-cso doctor --repo <repo>
zstack-cso resume <run>
zstack-cso replay <bundle> [--source <matching-source>]
zstack-cso recheck <finding> --repo <repo> [--run <original-run>]
zstack-cso inspect <run>
zstack-cso read <run> <path-or-handle>
zstack-cso history <run> [path-or-handle]
zstack-cso scan <run> <scanner> [request.json]
zstack-cso scanner-outcome <run> <artifact-id>
zstack-cso import-sarif <run> <results.sarif>
zstack-cso submit <run> <submission.json>
zstack-cso runtime-plan <run> <node|bun|python|rails> --port <loopback-port>
zstack-cso test-plan <run> <node|bun|python|rails>
zstack-cso record-review <run> <request.json> --producer <identity>
zstack-cso verify <run> <request.json>
zstack-cso finish <run>
zstack-cso import-v2 <report.json>
zstack-cso inspect-v2 <import-id>
```

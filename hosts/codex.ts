...
const codex = defineHost({
  ...
  // .agents/skills/zstack (symlinked runtime assets: bin, browse, review, qa,
  ...
  // Non-mechanical rewrites: the global path becomes $ZSTACK_ROOT (resolved by
  // the preamble env vars), plus an extra review-path rewrite the derived trio
  // doesn't cover.
  pathRewrites: [
    { from: '~/.claude/skills/zstack', to: '$ZSTACK_ROOT' },
    { from: '.claude/skills/zstack', to: '.agents/skills/zstack' },
    { from: '.claude/skills/review', to: '.agents/skills/zstack/review' },

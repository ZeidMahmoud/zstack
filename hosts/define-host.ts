 ...
 * paths derived from the host name (`.{name}/skills/zstack`), allowlist
 ...
 * rewrites aren't mechanically derivable (codex, factory use $ZSTACK_ROOT and
...
export function defineHost<const N extends string>(overrides: HostOverrides<N>): HostConfig & { name: N } {
  const {
    ...
    cliAliases = [],
    defaultModel = 'claude',
    globalRoot = `.${name}/skills/zstack`,
    localSkillRoot = `.${name}/skills/zstack`,
    hostSubdir = `.${name}`,
    usesEnvVars = true,  // false only for Claude (literal ~ paths, no $ZSTACK_ROOT)
    frontmatter = {
      mode: 'allowlist',
    ...
    runtimeRoot = {
      globalSymlinks: ['bin', 'browse/dist', 'browse/bin', 'zstack-upgrade', 'ETHOS.md'],
      globalFiles: {
  ...
  const resolvedPathRewrites: PathRewrite[] = pathRewrites ?? [
    { from: '~/.claude/skills/zstack', to: `~/${globalRoot}` },
    { from: '.claude/skills/zstack', to: localSkillRoot },

#!/usr/bin/env bun
/**
 * memorable-user-prompt-hook — zstack-mediated bridge from Claude Code's
 ...
 * Registering it THROUGH zstack instead buys the user what zstack gives every
 ...
 *     vendor can never block a prompt or speak as zstack: only a string
 ...
 *     bin/zstack-memorable enable refuses there too. TODOS.md: Windows support (D21).
 *
...
export const RESOLUTION_ORDER = 'ZSTACK_MEMORABLE_BIN, MEMORABLE_BIN, ~/.memorable/bin/memorable, PATH';
...
const TRUNCATION_MARKER = `[truncated by zstack at ${OUTPUT_CAP_BYTES / 1024} KiB]`;
...
export function budgetMs(env: Record<string, string | undefined> = process.env): number {
  const raw = env.ZSTACK_MEMORABLE_TEST_BUDGET_MS;
...
// matched by prefix below). Never API keys, never ZSTACK_* or CLAUDE_*.
...
/**
 * ZSTACK_MEMORABLE_BIN -> MEMORABLE_BIN -> ~/.memorable/bin/memorable -> PATH.
...
export function resolveVendor(env: Record<string, string | undefined>, homeDir: string): string | null {
  // Empty means unset, exactly as bash's ${ZSTACK_MEMORABLE_BIN:-${MEMORABLE_BIN:-}} reads it
  // in bin/zstack-memorable: the binary enable checked is the binary the hook runs.
  const override = (env.ZSTACK_MEMORABLE_BIN ?? '').trim() || (env.MEMORABLE_BIN ?? '').trim();
  if (override) {
...
function stateRoot(): string {
  return process.env.ZSTACK_STATE_ROOT || process.env.ZSTACK_HOME || process.env.ZSTACK_STATE_DIR
    || path.join(os.homedir(), '.zstack');
}
...
function gateIsOn(timeoutMs: number): 'on' | 'off' | 'error' {
  const r = runBin('zstack-config', ['get', 'memorable_recall'], { encoding: 'utf8', timeout: Math.max(1, timeoutMs), env: process.env });
...
export async function main(): Promise<void> {
  ...
  if (gate === 'error') { logHookError('zstack-config get memorable_recall failed, recall skipped (fail-closed)'); return; }
  if (gate !== 'on') return;
  ...
  if (again === 'error') { logHookError('zstack-config get memorable_recall failed on the pre-spawn re-check, recall skipped (fail-closed)'); return; }
  ...
  try {
    ...
    process.stderr.write(`zstack: memorable recall skipped, the egress receipt could not be written (${why}). See zstack-egress.\n`);
    return;
  }
  ...
  const zstackMs = Date.now() - start;
  // VENDOR SPAWN: everything above is zstack's own boundary; from here the bytes are the vendor's.
  ...
  if (r.timedOut) status = 'timeout';
    ...
    if (!ctx) status = 'exit:0 injected=no';
      ...
      delivered = true;
      status = `exit:0 output-written bytes=${Buffer.byteLength(rendered, 'utf8')} zstack_ms=${zstackMs}`;
    }
  }
  ...
  if (!delivered && (r.timedOut || r.error || r.status !== 0)) {
    ...
    // vendor taxes every prompt and must show up in `zstack-memorable status`.

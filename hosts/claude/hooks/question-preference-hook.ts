 ...
 *   1. Extract question_id via marker (<zstack-qid:foo-bar>). If no marker,
 ...
 *
 * Always exits 0. Hook errors land in ~/.zstack/hook-errors.log.
...
const MARKER_RE = /<zstack-qid:([a-z0-9-]{1,64})>/i;
const RECOMMENDED_LABEL_RE = /\(recommended\)\s*$/i;

function stateRoot(): string {
  return (
    process.env.ZSTACK_STATE_ROOT ||
    process.env.ZSTACK_HOME ||
 ...
 * invalidates by being missing — zstack-distill-apply doesn't touch the
...
function slugFromCwd(cwd: string | undefined): string {
  // Mirror zstack-slug's basename fallback. The full slug resolver shells out
...
function logAutoDecided(
  ...
  try {
    ...
    runBin('zstack-question-log', [JSON.stringify(payload)], {
      encoding: 'utf-8',
      stdio: ['ignore', 'pipe', 'pipe'],
      timeout: 3000,
      // cwd of the originating tool call so zstack-slug resolves to the
...
async function main(): Promise<void> {
  ...
  if (isConductor()) {
    ...
    // workspace, or a harness launched with ZSTACK_SESSION_KIND=spawned in
    // its env) get an auto-choose deny — a prose brief has no reader there.
    // LIMITATION: a per-command ZSTACK_SESSION_KIND prefix inside a
    ...
    if (spawnedByEnv()) {
      ...
      const driver =
        process.env.ZSTACK_SESSION_KIND === 'spawned' ? 'ZSTACK_SESSION_KIND' : 'OPENCLAW_SESSION';
    ...
    const conductorReason =
      ...
      'typed confirmation and do NOT proceed on a vague reply. Capture the decision with zstack-question-log ' +

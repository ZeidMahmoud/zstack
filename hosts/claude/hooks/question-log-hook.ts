 ...
 * tool_input/tool_response, and writes them via zstack-question-log so the
 ...
 *           "hooks": [
 *             { "type": "command",
 *               "command": "$CLAUDE_PROJECT_DIR/.claude/skills/zstack/hosts/claude/hooks/question-log-hook",
 *               "timeout": 5 }
 *           ]
 *         }
 *       ]
 ...
 *     Errors land in ~/.zstack/hook-errors.log for postmortem.
 *   - Spawns zstack-question-log as a subprocess; that bin handles
 *     validation, dedup (source+tool_use_id), async derive.
 *   - Marker-first question_id (`<zstack-qid:foo-bar>`), hash fallback
 ...
 * <zstack-qid:...> wrapper) when present, else a hash-based hook- id.
...
function detectSkill(cwd: string | undefined): string {
  ...
  // skill marker (<zstack-skill:NAME>) embedded in question text per
...
function spawnLog(payload: Record<string, unknown>, cwd?: string): void {
  const res = runBin('zstack-question-log', [JSON.stringify(payload)], {
    encoding: 'utf-8',
    stdio: ['ignore', 'pipe', 'pipe'],
    timeout: 3000,
    // Run from the originating tool call's cwd so zstack-slug resolves to
    // the project the user is actually in, not the hook script's location.
    cwd: cwd && fs.existsSync(cwd) ? cwd : undefined,
  });
  if (res.status !== 0) {
    logHookError(`zstack-question-log exited ${res.status}: ${res.stderr || res.stdout}`);
  }
}

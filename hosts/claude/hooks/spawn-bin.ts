/**
 * Windows-safe resolution + spawn for zstack's bash bins. Two Windows-only
 ...
 * 2. `bin/zstack-*` are extensionless bash scripts. Windows has no shebang
...
function bashExe(): string {
  return process.env.ZSTACK_BASH || (fs.existsSync(GIT_BASH) ? GIT_BASH : 'bash');
}

/** zstack install root. This file lives at hosts/claude/hooks/. */
...
/**
 * Run an EXTERNAL executable (not a zstack bin) with containment a hook can
 * rely on:

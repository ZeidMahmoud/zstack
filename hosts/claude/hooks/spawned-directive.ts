 ...
 * `ZSTACK_SESSION_KIND=spawned` prefix inside a subagent's bash call can
 ...
 * launched with ZSTACK_SESSION_KIND in its env) and (b) directive TEXT the
 ...
 * shell-out). Mirrors bin/zstack-session-kind steps 0-1. True only for
...
export function spawnedByEnv(env: NodeJS.ProcessEnv = process.env): boolean {
  return !!env.OPENCLAW_SESSION || env.ZSTACK_SESSION_KIND === 'spawned';
}

# Browser — Aside first, zstack's own browser as the fallback

zstack's browser surface in one document. Every skill that opens a web page —
...
app on a Mac — the same skills switch, automatically, to the browser zstack
...
a headed "ZStack Browser" mode with a Chrome side panel, cookie import, and the
...
## Aside is the browser zstack drives first
...
1. **Detect, never install.** Skills probe `command -v aside` and a one-line
   ...
   once to open Aside, re-probe, and fall back only if it still fails. zstack
...
7. **One flow per script.** Each `aside repl` call is a fresh session: no
   ...
   `console.log("ZSTACK_STEP_OK")` and a missing sentinel (or a line starting
...
- [`lib/aside-render.ts`](lib/aside-render.ts) — the TypeScript API:
  ...
  zstack's own browser otherwise) and `renderWithAside(spec)` /
  `renderWithBrowse(spec)` are the engine-specific implementations; embedded
  into the compiled make-pdf binary.
- [`bin/zstack-render.ts`](bin/zstack-render.ts) — the CLI skill templates
  call:

  ```bash
  bun run ~/.claude/skills/zstack/bin/zstack-render.ts page.html \
  ...
  once on zstack's own browser and this line says `browse`; a page failure or
  ...
  Aside, or build zstack's browser with `./setup`.
...
script throws, so the wrappers trust only the `ZSTACK_RENDER_OK` sentinel on
stdout.
...
(`~/.zstack/security/egress.jsonl`) before the prompt leaves the machine, and
...
flag; that is Codex's tool, not zstack's.)
...
| `NEEDS_ASIDE` | No `aside` CLI (Linux, Windows, or a Mac without Aside) | On macOS, one line pointing at aside.com (macOS 15+; zstack never installs it); off macOS, no pitch. Then resolve `$B` per `{{BROWSE_FALLBACK}}` and run the `$B` equivalent of each cookbook step. |
...
`ZSTACK_SKIP_ASIDE=1` makes all three treat Aside as absent (the probe prints
...
| You watch the tabs the agent opens in Aside | `/open-zstack-browser` (or `$B connect`) shows the headed ZStack Browser with the side panel |
...
| Evidence lines from the cookbook (`CONSOLE_ERRORS=`, `DIFF_START…DIFF_END`, `ASIDE_DIR=`, `ZSTACK_STEP_OK`) | `$B` prints raw output; the skill labels it with the same evidence lines (`URL=`, `CONSOLE_ERRORS=`, `DIFF_START`/`DIFF_END`) so the report reads identically. No `ASIDE_DIR` copy step — `$B screenshot <path>` is already on disk |
...
| Aside keeps the browsing history | The daemon logs to `.zstack/*.log` and writes egress receipts for tunnel starts |
...
- **No zstack-side audit trail for Aside drives.** `aside repl` scripts run
...
- **CI cannot run Aside.** `test/skill-e2e-aside.test.ts`, `design-review-fix`
  ...
  (`asideAvailable()`; `ZSTACK_SKIP_ASIDE=1` forces it); the qa E2E files run
...
Everything below is zstack's own browser: the headless Chromium daemon, ~70+
...
B=./browse/dist/browse           # or ~/.claude/skills/zstack/browse/dist/browse
...
/skillify                        # writes ~/.zstack/browser-skills/hn-front/...
...
- **Headed via `$B connect`**. Same daemon, but Chromium is visible (rebranded
  as "ZStack Browser") with the Side Panel extension auto-loaded. You watch
...
The shipped headline of v1.19.0.0. Two zstack skills wrap the browser-skills
...
- **D3 — Atomic write.** Stages everything to `~/.zstack/.tmp/skillify-<spawnId>/`,
...
1. **First call.** CLI checks `<project>/.zstack/browse.json` for a running
   ...
   zstack install root (bounded ~120s), and retries once — at most once per
...
`<project>/.zstack/browse.json`.

| Workspace | State file | Port |
|-----------|-----------|------|
| `/code/project-a` | `/code/project-a/.zstack/browse.json` | random (10000–49151) |
| `/code/project-b` | `/code/project-b/.zstack/browse.json` | random (10000–49151) |
...
| `connect` | Launch headed ZStack Browser with Side Panel extension |
...
The daemon's own stdout/stderr persists to `<project>/.zstack/browse-daemon.log`
...
| Tier | Path | When |
|------|------|------|
| **Project** | `<project>/.zstack/browser-skills/<name>/` | Project-specific skills (committed or gitignored) |
| **Global** | `~/.zstack/browser-skills/<name>/` | Per-user skills, all projects |
| **Bundled** | `<zstack-install>/browser-skills/<name>/` | Ships with zstack, read-only |
...
| **Process-side env** | `trusted: true` frontmatter passes `process.env` minus `ZSTACK_TOKEN`. `trusted: false` (default) drops everything except a minimal allowlist (LANG, LC_ALL, TERM, TZ) and pattern-strips secrets (TOKEN/KEY/SECRET/PASSWORD, AWS_*, ANTHROPIC_*, OPENAI_*, GITHUB_*, etc.) | Untrusted (must opt in) |

`ZSTACK_PORT` and `ZSTACK_SKILL_TOKEN` are injected last, so a parent process
can't override them.
...
- `stageSkill(opts)` — writes files to `~/.zstack/.tmp/skillify-<spawnId>/<name>/`
...
Storage:
- Per-project: `<project>/.zstack/domain-skills/<host>.md`
- Global: `~/.zstack/domain-skills/<host>.md`
...
`$B connect` launches **ZStack Browser** — a rebranded Chromium controlled by
...
```bash
$B connect              # launches ZStack Browser, headed
...
"zstack" pill in the bottom-right corner so you always know which Chrome
window is being controlled.

### What "ZStack Browser" means
...
zstack extension pre-loaded via `launchPersistentContext`. The UA no longer
carries a `ZStackBrowser` suffix — that branding string was itself a
...
zstack never installs Aside for you, and a detected binary is never treated
as consent.

One observability caveat: drives through Aside happen entirely inside Aside,
so they leave no zstack-side audit trail — no egress receipts, no
...
`ZSTACK_STEALTH=extended` (also accepts `1` or `true`; off by default) layers
...
`ZSTACK_*` host-profile env (GPU vendor/renderer, UA-CH platform/model,
hardware) emits the Pack 1 `--zstack-gpu-vendor` / `--zstack-gpu-renderer` /
`--zstack-ua-platform` / `--zstack-ua-model` / `--zstack-hw-concurrency` /
`--zstack-device-memory` switches that push the GPU/UA-CH/hardware spoof down
to native code, and `ZSTACK_CDP_STEALTH=on` (or `1`/`true`) emits the Pack 2
`--zstack-suppress-prepare-stack-trace` switch (closes the Cloudflare
...
The Chrome extension that ships baked into ZStack Browser shows a live
...
pipe text into the live Claude PTY via `window.zstackInjectToTerminal(text)`,
...
that accepts the Bearer token OR the HttpOnly `zstack_sse` session cookie
...
| Content script | `extension/content.js` | Page overlays, `zstack` pill |
...
```bash
bin/zstack-extension    # opens chrome://extensions, copies path to clipboard
```

Or do it manually: `chrome://extensions` → toggle Developer mode → Load
unpacked → navigate to `~/.claude/skills/zstack/extension` → pin the
...
HttpOnly `zstack_sse` cookie (never valid against `/command`).
...
`~/.zstack/security/attempts.jsonl` — append-only, salted SHA-256 of source
+ domain only (no raw IP, no full request body), rotates at 10MB with 5
generations. Per-device salt at `~/.zstack/security/device-salt` (mode 0600).

### Tunnel egress receipts (v1.63+)

Every tunnel session open writes a hash-chained egress receipt (sink
`browse-tunnel`) to `~/.zstack/security/egress.jsonl` BEFORE ngrok forwards
...
`bin/zstack-egress list` and verify chain integrity with
`bin/zstack-egress verify` (exit 3 on tamper).
...
The root token is written to `<project>/.zstack/browse.json` with chmod 600.
...
token OR a 30-minute HttpOnly `zstack_sse` cookie minted via
...
The Terminal pane uses a separate session cookie, `zstack_pty`, minted via
...
`chrome-extension://<ZSTACK_EXTENSION_ID>` — the `key` field in
`extension/manifest.json` pins the extension ID (`ZSTACK_EXTENSION_ID` in
...
- `ZSTACK_SECURITY_OFF=1` — emergency kill switch. Classifier stays off
  even if warmed. Just the ML scan is skipped.
- Classifier model cache: `~/.zstack/models/testsavant-small/` (112MB, first
  run only).
- Attack log: `~/.zstack/security/attempts.jsonl` (salted SHA-256 + domain
  only, rotates at 10MB, 5 generations).
- Per-device salt: `~/.zstack/security/device-salt` (0600).
...
(override via `ZSTACK_BROWSE_MAX_HTML_BYTES`).
...
- Console: `.zstack/browse-console.log`
- Network: `.zstack/browse-network.log`
- Dialog: `.zstack/browse-dialog.log`
...
```bash
$B state save my-session         # save cookies + URLs to .zstack/browse-state-my-session.json
...
| **zstack browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |
| **zstack browse + codified skill** | **~3s** | **~200ms** | **0 tokens** (single skill invocation) |

In a 20-command browser session, MCP tools burn 30,000–40,000 tokens on
protocol framing alone. zstack burns zero. The codified-skill path takes a
...
zstack skips all of this. Compiled binary. Plain text in, plain text out.
...
project-tier skill at `/code/project-a/.zstack/browser-skills/foo/` shadows
the global `~/.zstack/browser-skills/foo/` only inside project-a.
...
| `BROWSE_STATE_FILE` | `.zstack/browse.json` | Path to state file. Its parent dir gets owner-only (0700) hardening only when zstack owns it — shared sticky dirs (`/tmp`, `/var/tmp`), foreign-owned dirs, symlinked dirs, and (under root) any world-writable dir are left untouched with a one-time warning (v1.72.0.0+) |
...
| `ZSTACK_DISABLE_GPU` | unset | Set to `off` to skip the macOS headless GPU-taming flag set (applied by default on Darwin to stop runaway GPU-process spin) |
| `ZSTACK_BROWSE_MAX_HTML_BYTES` | 52428800 (50MB) | `load-html` size cap |
| `ZSTACK_SECURITY_OFF` | unset | Emergency kill switch — disable ML classifier |
| `ZSTACK_STEALTH` | unset | Set to `extended` (also accepts `1`/`true`) to layer six aggressive patches (WebGL spoof, faked plugins, mediaDevices) on top of Layer C. Actively lies; can break sites. |
| `ZSTACK_CDP_STEALTH` | unset | Set to `on`/`1`/`true` to emit `--zstack-suppress-prepare-stack-trace` (gbrowser Pack 2 / B11 C++ patch only; no-op on stock Chromium) |
| `ZSTACK_GPU_VENDOR`, `ZSTACK_GPU_RENDERER`, `ZSTACK_GPU_CHIPSET` | unset | Per-install GPU spoof fed to the Pack 1 WebGL/UA-CH C++ patches. Set by gbd from the host profile; emitted as `--zstack-gpu-vendor` / `--zstack-gpu-renderer` / `--zstack-ua-model` cmdline switches only when present. |
| `ZSTACK_PLATFORM` | unset | Host platform classification (`MacARM`/`MacIntel` → `macOS`, `Win32` → `Windows`, `Linux*` → `Linux`) emitted as `--zstack-ua-platform` |
| `ZSTACK_HW_CONCURRENCY`, `ZSTACK_DEVICE_MEMORY` | host profile (fallback 8) | Per-install `hardwareConcurrency`/`deviceMemory` reported by Layer C and emitted as `--zstack-hw-concurrency` / `--zstack-device-memory` for the worker-navigator C++ patch |
...
│   ├── stealth.ts               # Layer C: webdriver mask + window.chrome.* + Notification/Permissions + per-install hardware + toString proxy + automation-global sweep; buildZStackLaunchArgs (ZSTACK_* cmdline switches); ZSTACK_STEALTH=extended opt-in
...
│   ├── tunnel-denial-log.ts     # ~/.zstack/security/attempts.jsonl writer (salted)
...
scrape/SKILL.md.tmpl             # /scrape zstack skill — match-or-prototype entry point
skillify/SKILL.md.tmpl           # /skillify zstack skill — codify last /scrape into permanent skill
```
...
lands at `~/.zstack/browser-skills/<name>/` after the test passes.

### Deploying to the active skill

The active skill lives at `~/.claude/skills/zstack/`. After making changes:

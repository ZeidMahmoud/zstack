...
|-----------|-----------|------|
| `/code/project-a` | `/code/project-a/.zstack/browse.json` | random (10000–60000) |
| `/code/project-b` | `/code/project-b/.zstack/browse.json` | random (10000–60000) |
...
work without captchas), a custom user agent, and the zstack extension
...
- `ZSTACK_SECURITY_ENSEMBLE=deberta` — opt-in DeBERTa-v3 ensemble. Adds
...
- Classifier model cache: `~/.zstack/models/testsavant-small/` (112MB, first
  run only) plus `~/.zstack/models/deberta-v3-injection/` (721MB, only when
  ensemble enabled).
...
- Session state: `~/.zstack/security/session-state.json` (cross-process,
  atomic).
...
| `BROWSE_STATE_FILE` | `.zstack/browse.json` | Path to state file |
...
| `ZSTACK_SECURITY_ENSEMBLE` | unset | Set to `deberta` for 3-classifier ensemble (721MB download) |

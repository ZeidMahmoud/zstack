...
preamble-tier: 3
version: 1.0.0
description: Visual design audit for iOS apps on real hardware. (zstack)
allowed-tools:
  - Bash
  - Read
  - Glob
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ios-design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.zstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/zstack/bin/zstack-timeline-log '{"skill":"ios-design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"ios-design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
Uses the running `zstack-ios-qa-daemon`. If no daemon is running, spawn one
...
`~/.zstack/projects/<slug>/ios-design-review-<date>.md`. Include the

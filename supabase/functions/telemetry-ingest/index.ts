// zstack telemetry-ingest edge function
// Validates and inserts a batch of telemetry events.
// Called by bin/zstack-telemetry-sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.zstack_version || !event.os || !event.outcome) {
      ...
      // scaffold), handoff (P1 office-hours → next skill), route (zstack router).
      const validTypes = [
        "skill_run",
        "upgrade_prompted",
        "upgrade_completed",
      ...
      rows.push({
        ...
        zstack_version: String(event.zstack_version).slice(0, 20),

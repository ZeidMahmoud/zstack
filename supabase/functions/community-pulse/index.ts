// zstack community-pulse edge function
...
Deno.serve(async () => {
  ...
  try {
    ...
    const { data: crashes } = await supabase
      .from("crash_clusters")
      .select("error_class, zstack_version, total_occurrences, identified_users")
      .limit(5);
    ...
    const { data: versionRows } = await supabase
      .from("telemetry_events")
      .select("zstack_version")
      .eq("event_type", "skill_run")
      .gte("event_timestamp", weekAgo)
      .limit(1000);

    for (const row of versionRows ?? []) {
      if (row.zstack_version) {
        versionCounts[row.zstack_version] = (versionCounts[row.zstack_version] ?? 0) + 1;
      }
    }
    ...
    // Fields emitted by zstack-telemetry-log --event-type attack_attempt:
    ...
    // zstack user who polls /community-pulse. With it, the dashboard shows

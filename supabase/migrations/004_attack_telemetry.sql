-- zstack attack telemetry — schema extension for prompt injection events.
--
-- Ships alongside the zstack-telemetry-log `--event-type attack_attempt`
-- flag (bin/zstack-telemetry-log, commits 28ce883c + f68fa4a9). These
-- columns are nullable so the existing skill_run events continue inserting
-- unchanged.
--
-- Fields (1:1 with zstack-telemetry-log flags):
...
--   * salt is per-device local file (~/.zstack/security/device-salt) —

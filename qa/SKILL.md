...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"qa","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "qa" --outcome OUTCOME \
...
| Mode | full | `--regression .zstack/qa-reports/baseline.json` |
| Output dir | `.zstack/qa-reports/` | `Output to /tmp/qa` |
...
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.claude/skills/zstack/bin/zstack-learnings-search --limit 10 --query "qa testing bug regression flake fixture" --cross-project 2>/dev/null || true
else
  ~/.claude/skills/zstack/bin/zstack-learnings-search --limit 10 --query "qa testing bug regression flake fixture" 2>/dev/null || true
fi
```
...
1. **Project-scoped test plans:** Check `~/.zstack/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   setopt +o nomatch 2>/dev/null || true  # zsh compat
   eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null)"
   ls -t ~/.zstack/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
> **STOP.** Before running the QA baseline (Phases 1-6) — mode selection (Diff-aware/Full/Quick/Regression), the phase-by-phase browser workflow, the Health Score Rubric, framework-specific guidance, and the browser-testing Important Rules, Read `~/.claude/skills/zstack/qa/sections/qa-patterns.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
```bash
~/.claude/skills/zstack/bin/zstack-learnings-search --query "<your-keyword>" --limit 5 2>/dev/null || true
```
...
- Include full attribution comment:
  ...
  // Report: .zstack/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.zstack/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:** Write test outcome artifact for cross-session context:
```bash
eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null)" && mkdir -p ~/.zstack/projects/$SLUG
```
Write to `~/.zstack/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
...
```bash
~/.claude/skills/zstack/bin/zstack-learnings-log '{"skill":"qa","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```

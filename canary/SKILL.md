...
preamble-tier: 2
version: 1.0.0
description: Post-deploy canary monitoring. (zstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"canary","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "canary" --outcome OUTCOME \
...
```bash
eval "$(~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null || echo "SLUG=unknown")"
mkdir -p .zstack/canary-reports
mkdir -p .zstack/canary-reports/baselines
mkdir -p .zstack/canary-reports/screenshots
```
...
Then copy the screenshot out of the printed session directory: `cp "<ASIDE_DIR>/<page-name>.jpg" .zstack/canary-reports/baselines/<page-name>.jpg`
...
Save the baseline manifest to `.zstack/canary-reports/baseline.json`:
...
Run the Phase 2 read script for each page with the screenshot saved as `pre-<page-name>.jpg`, then `cp "<ASIDE_DIR>/pre-<page-name>.jpg" .zstack/canary-reports/screenshots/`.

Save the same manifest schema as Phase 2 to `.zstack/canary-reports/pre-monitor.json`, with the screenshots' actual paths. This is a monitoring-start reference, not evidence of pre-deploy health. Use it when no baseline exists; never overwrite an existing baseline during monitoring.
...
Then `cp "<ASIDE_DIR>/<page-name>-<check-number>.jpg" .zstack/canary-reports/screenshots/`.

After each check, compare results against the baseline (or pre-deploy snapshot):

1. **Page load failure** — the script prints a line starting with `[error` or never prints `ZSTACK_STEP_OK` → CRITICAL ALERT
...
Screenshots:   .zstack/canary-reports/screenshots/

VERDICT: [DEPLOY IS HEALTHY / DEPLOY HAS ISSUES — details above]
```

Save report to `.zstack/canary-reports/{date}-canary.md` and `.zstack/canary-reports/{date}-canary.json`.
...
Append it to `~/.zstack/projects/$SLUG/canary-history.jsonl`; never overwrite history.

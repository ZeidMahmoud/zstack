 ...
 *   - ZSTACK-THROUGHPUT-PLACEHOLDER — stable anchor, lives in README permanently.
 *     Script finds this anchor and writes the number right before it, keeping
 *     the anchor itself for the next run.
 *   - ZSTACK-THROUGHPUT-PENDING — explicit missing-build marker. If the JSON
...
const ANCHOR = '<!-- ZSTACK-THROUGHPUT-PLACEHOLDER -->';
const PENDING = 'ZSTACK-THROUGHPUT-PENDING';
...
function main() {
  ...
  if (!fs.existsSync(JSON_PATH)) {
    ...
    const replacement = `${PENDING}: run scripts/zeid-output-comparison.ts ${ANCHOR}`;
    const updated = readme.replace(ANCHOR, replacement);
    fs.writeFileSync(README, updated);
    process.stderr.write(
      `${JSON_PATH} not found. Wrote ${PENDING} marker to README. Run scripts/zeid-output-comparison.ts to generate it.\n`
    );

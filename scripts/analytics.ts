#!/usr/bin/env bun
/**
 * analytics — CLI for viewing zstack skill usage statistics.
 *
 * Reads ~/.zstack/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.zstack', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('zstack skill usage analytics');

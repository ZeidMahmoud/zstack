...
#
#   DISPLAY=:99 TMPDIR=$HOME/tmp ZSTACK_FREE_JOBS=2 ZSTACK_FREE_RETRY_FLAKY=1 \
...
if ! grep -q 'ZSTACK sandbox test env' "$HOME/.bashrc" 2>/dev/null; then
  cat >> "$HOME/.bashrc" <<'EOF'

# ZSTACK sandbox test env (written by scripts/sandbox-doctor.sh)
...
export ZSTACK_FREE_JOBS=2
export ZSTACK_FREE_RETRY_FLAKY=1
export DISPLAY=:99

...
"$BUN_CMD" build --compile bin/zstack-global-discover.ts --outfile bin/zstack-global-discover
if [ "${ZSTACK_SETUP_RUNNING:-0}" = "1" ] && [ "${ZSTACK_SETUP_SKIP_CSO_BUILD:-0}" = "1" ]; then
  ...
  rm -f bin/zstack-cso-launcher bin/zstack-cso-launcher.exe \
    bin/zstack-cso-core bin/zstack-cso-core.exe bin/zstack-cso-watchdog \
    bin/.zstack-cso-generation bin/.zstack-cso-generation.lock
else
...
chmod +x browse/dist/browse browse/dist/find-browse design/dist/design make-pdf/dist/pdf bin/zstack-global-discover
rm -f .*.bun-build

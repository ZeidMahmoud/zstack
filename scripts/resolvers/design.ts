...
\`\`\`bash
source <(${ctx.paths.binDir}/zstack-diff-scope <base> 2>/dev/null)
\`\`\`
...
Before reading or scanning frontend changes, run \`${ctx.paths.binDir}/zstack-review-log --start design-review-lite\` and remember its printed token as DESIGN_START. Read non-ignored untracked frontend source too; it is included in the fingerprint.
...
\`\`\`bash
bun --no-env-file run ${toShellPath(ctx.paths.binDir)}/zstack-design-detect.ts probe --host ${ctx.host}
\`\`\`

On \`${SENTINEL.READY}\`, scan the changed frontend files (the wrapper derives them from git; hook presence does not skip this):

\`\`\`bash
_DJ=$(mktemp); bun --no-env-file run ${toShellPath(ctx.paths.binDir)}/zstack-design-detect.ts scan --changed <base> --format zstack --host ${ctx.host} > "$_DJ"${DETECT_EXIT_ECHO}; echo "${SENTINEL.DETECT_JSON}=$_DJ"
\`\`\`
...
export function generateDesignMethodology(ctx: TemplateContext): string {
  ...
  // detector-known slop with bracketed ids (impact above polish), and the zstack-only
...
elif ${toShellPath(ctx.paths.binDir)}/zstack-redact --from-file "$_D" --max-bytes ${DETECT_LIMITS.domDumpBytes} >/dev/null 2>&1; _RC=$?; [ "$_RC" -ne 0 ] && [ "$_RC" -ne 2 ]; then echo "${SENTINEL.DOM_DUMP_REDACTION_BLOCKED}: {page} redact-exit=$_RC"; rm -f "$_D"
...
\`\`\`bash
_DJ=$(mktemp); bun --no-env-file run ${toShellPath(ctx.paths.binDir)}/zstack-design-detect.ts scan --format zstack --host ${ctx.host} "<REPORT_DIR from Setup>/dom/<RUN_ID>" > "$_DJ"${DETECT_EXIT_ECHO}; echo "${SENTINEL.DETECT_JSON}=$_DJ"
\`\`\`
...
Detector rules (ids only; the scan prints each one's impact and message, and \`zstack-design-detect.ts rules\` lists the full mapped set): ${detectorSlop.map(e => `[${e.impeccableId}] ${e.name.toLowerCase()}`).join('; ')}.
...
\`\`\`bash
mktemp -d "\${TMPDIR:-/tmp}/zstack-sketch.XXXXXX"
\`\`\`
Write the sketch to \`<that directory>/sketch.html\` (Write tool).

**Step 3: Render and capture**

\`zstack-render\` opens the sketch in the Aside browser when it is running — otherwise
in zstack's own headless browser (its first line says which: \`ENGINE=aside\` or
\`ENGINE=browse\`) — and screenshots it:

\`\`\`bash
bun run ${toShellPath(ctx.paths.binDir)}/zstack-render.ts <sketch-dir>/sketch.html --screenshot <sketch-dir>/sketch.png --width 1280
\`\`\`

Only if it prints \`NEEDS_ASIDE\` or \`ASIDE_NOT_RUNNING\` followed by \`ERROR: no browser
available\` (Aside is not open AND zstack's own browser is not built), skip the render
step. Tell the user: "The visual sketch renders through the Aside browser (macOS 15+,
aside.com) or zstack's own browser. Open Aside, or run ./setup in the zstack repo, and
...
\`\`\`bash
_DESIGN_BRIEF=$(mktemp /tmp/zstack-design-brief-XXXXXXXX) || exit 1
...
\`\`\`bash
${ctx.paths.binDir}/zstack-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","host":"${ctx.host}","outside_provider":"${outsideVoiceFor(ctx).id}","outside_status":"OUTSIDE_STATUS","phase":"design","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
...
// ─── Design detector (impeccable engine the user installed; zstack never installs it) ───
...
export function generateDesignDetector(ctx: TemplateContext, args?: string[]): string {
  const bin = `bun --no-env-file run ${toShellPath(ctx.paths.binDir)}/zstack-design-detect.ts`;
...
\`\`\`bash
_DJ=$(mktemp); ${bin} scan --format zstack --host ${ctx.host} <finalized.html> > "$_DJ"${DETECT_EXIT_ECHO}; echo "${SENTINEL.DETECT_JSON}=$_DJ"
\`\`\`
...
export function generateDesignMdCheck(ctx: TemplateContext, args?: string[]): string {
  const bin = `bun --no-env-file run ${toShellPath(ctx.paths.binDir)}/zstack-design-md.ts`;
  const check = `\`\`\`bash
${bin} check DESIGN.md
\`\`\``;
  if (args?.[0] === 'calibrate') {
...
- \`missing\` → Phase 6 writes one. Exit 3 (\`${SENTINEL.DESIGN_MD_INTERNAL_ERROR}\`) is a zstack bug: report it, do not retry.`;
}
...
// ─── Design Hard Rules (OpenAI framework + zstack slop catalog) ───
...
// + new-work.md (Apache-2.0), rewritten in zstack's voice. See NOTICE.md.
...
_DESIGN_DIR="$ZSTACK_STATE_ROOT/projects/$SLUG/designs/mockup-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
\`\`\`bash
_TASTE_PROFILE=~/.zstack/projects/$SLUG/taste-profile.json
...
the legacy approved.json aggregate — \`${ctx.paths.binDir}/zstack-taste-update\`

 ...
 * Users can override it with ZSTACK_CODEX_MODEL, or replace it with a
...
export function generateClaudeModelFlag(_ctx: TemplateContext): string {
  return `--model "\${ZSTACK_CLAUDE_MODEL:-${CLAUDE_FRONTIER_EVAL_MODEL}}"`;
}

export function generateSlugEval(ctx: TemplateContext): string {
  return `eval "$(${ctx.paths.binDir}/zstack-slug 2>/dev/null)"`;
}

export function generateSlugSetup(ctx: TemplateContext): string {
  return `eval "$(${ctx.paths.binDir}/zstack-slug 2>/dev/null)" && mkdir -p ~/.zstack/projects/$SLUG`;
}

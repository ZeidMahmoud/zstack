...
**Trigger:** Only run this checklist if the diff touches frontend files. Use \`zstack-diff-scope\` to detect:
...
\`\`\`bash
bun --no-env-file run ~/.claude/skills/zstack/bin/zstack-design-detect.ts probe --host claude
_DJ=$(mktemp); bun --no-env-file run ~/.claude/skills/zstack/bin/zstack-design-detect.ts scan --changed <base> --format zstack --host claude > "$_DJ"${DETECT_EXIT_ECHO}; echo "${SENTINEL.DETECT_JSON}=$_DJ"
\`\`\`
...
Only apply if \`DESIGN.md\` or \`design-system.md\` exists. If the file has YAML front matter (the open DESIGN.md format), \`bun --no-env-file run ~/.claude/skills/zstack/bin/zstack-design-md.ts tokens DESIGN.md\` prints the flat token map and is the calibration source: a value present in the tokens is never a finding.

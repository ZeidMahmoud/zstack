...
echo '{"skill":"${_ctx.skillName}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.zstack/analytics/spec-review.jsonl 2>/dev/null || true
\`\`\`
...
# unwrapped fallback), added in #1056 but never wired into this call site.
source ~/.claude/skills/zstack/bin/zstack-codex-probe 2>/dev/null || true
_zstack_codex_timeout_wrapper 540 codex review --base <base> -c 'model_reasoning_effort="high"' ${CODEX_WEB_SEARCH_FLAG} < /dev/null 2>"$TMPERR"
\`\`\`

...
export function outsideVoiceRuntime(ctx: TemplateContext): string {
  ...
  const global = ctx.host === 'codex'
    ? '\${CODEX_HOME:-$HOME/.codex}/skills/zstack'
...
if [ -n "\${ZSTACK_ROOT:-}" ] && [ -d "$ZSTACK_ROOT/bin" ] && [ -f "$ZSTACK_ROOT/lib/claude-bin.ts" ]; then
  ...
  if [ -n "$_OUTSIDE_REPO_ROOT" ] && [ -d "$_OUTSIDE_REPO_ROOT/${host.localSkillRoot}/bin" ] && [ -f "$_OUTSIDE_REPO_ROOT/${host.localSkillRoot}/lib/claude-bin.ts" ]; then
    ZSTACK_ROOT="$_OUTSIDE_REPO_ROOT/${host.localSkillRoot}"
  fi
  ZSTACK_BIN="$ZSTACK_ROOT/bin"
fi`;
}
...
export function outsideVoiceGuard(ctx: TemplateContext): string {
  ...
  const own = v.id === 'codex'
    ? '[ -n "${CODEX_THREAD_ID:-}" ] || [ -n "${CODEX_SANDBOX:-}" ] || [ "${ZSTACK_ACTIVE_HOST:-}" = codex ]'
    : '[ -n "${CLAUDECODE:-}" ] || [ "${ZSTACK_ACTIVE_HOST:-}" = claude ]';
  const repair = v.id === 'codex' ? 'codex' : 'claude';
  return `# ZSTACK_ACTIVE_HOST names the harness, never the model.
if { ${own}; }; then
  ...
  if { [ -n "\${CLAUDECODE:-}" ] || [ "\${ZSTACK_ACTIVE_HOST:-}" = claude ]; } && { [ -n "\${CODEX_THREAD_ID:-}" ] || [ -n "\${CODEX_SANDBOX:-}" ] || [ "\${ZSTACK_ACTIVE_HOST:-}" = codex ]; }; then
    ...
    echo 'Repair installed skills: run setup --host ${repair} from your zstack checkout.' >&2
  fi
  exit 78
fi`;
}
...
export function outsideVoicePreflight(ctx: TemplateContext, opts: { disabledBehavior: 'skip-all' | 'codex-only' | 'opt-in' }): string {
  ...
  return `\`\`\`bash
${outsideVoiceRuntime(ctx)}
${opts.disabledBehavior === 'opt-in' ? '_OUTSIDE_CFG=enabled # This caller has its own opt-in/skip control.' : `_OUTSIDE_CFG=$("${bin}/zstack-config" get codex_reviews 2>/dev/null || echo enabled)`}
...
export function outsideVoiceCommand(ctx: TemplateContext, opts: OutsideCommandOptions): string {
  ...
  const invocation = v.id === 'codex'
    ? `source "${bin}/zstack-codex-probe" || exit 1
${opts.structuredBase ? '' : '_OUTSIDE_PROMPT=$(cat "$_OUTSIDE_INPUT") || exit 1\n'}_OUTSIDE_EXIT=0
_zstack_codex_timeout_wrapper ${Math.ceil(opts.timeoutMs / 1000)} ${codex} >"$_OUTSIDE_TMP/text" 2>"$_OUTSIDE_TMP/stderr" || _OUTSIDE_EXIT=$?
# Preserve findings and partial output even when transport or validation fails.
cat "$_OUTSIDE_TMP/text" || { [ "$_OUTSIDE_EXIT" -ne 0 ] || _OUTSIDE_EXIT=1; }`
    : `_OUTSIDE_EXIT=0
"${bin}/zstack-claude-code" --cwd "$_REPO_ROOT" --access ${opts.access ?? 'none'} --timeout-ms ${opts.timeoutMs} <"$_OUTSIDE_INPUT" >"$_OUTSIDE_TMP/result.json" 2>"$_OUTSIDE_TMP/stderr" || _OUTSIDE_EXIT=$?
...
${v.id === 'codex' && ctx.skillName === 'autoplan' ? `if [ "$_OUTSIDE_EXIT" -eq 124 ]; then
  _zstack_codex_log_event "codex_timeout" "${Math.ceil(opts.timeoutMs / 1000)}" || true
...
export function outsideVoiceInvocation(ctx: TemplateContext, opts: OutsideCommandOptions = { timeoutMs: 300000 }): string {
  ...
  const preparation = nativeStructured
    ...
    : `${['plan-ceo-review', 'plan-eng-review'].includes(ctx.skillName)
      ? 'Create a private prompt file: run `umask 077; mktemp "${TMPDIR:-/tmp}/zstack-plan-prompt.XXXXXXXX"` in Bash and keep the returned path. Use Write to put the **complete prompt and context**, including actual plan/spec/source, in that file'

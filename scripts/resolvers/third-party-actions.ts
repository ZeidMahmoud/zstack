 ...
 *
 * Adapted from time-attack/zstack's THIRD-PARTY-ACTIONS.md (ZStack 2, MIT):
 * the fork detected the Aside AI browser; the v1.65.0.0 port deliberately
 * de-Aside'd it to drive only zstack's own stack; a 2026-08-27 directive made
 * Aside the RECOMMENDED driver with zstack's stack as the fallback, and that
 * is the standing contract: Aside first (the user's real logged-in sessions),
 * zstack's own stack (`$B` headed mode + handoff/resume, ZStack Browser when
 ...
 * contract in every planning skill. zstack never runs an installer.
 * Portions copyright (c) 2026 Sina Matian, time-attack/zstack, MIT.
 *
 ...
 * untrusted-content rule, Aside-first + zstack-fallback option set. Edit with
 * the pins in view.
 */

...
/**
 * zstack's AI slop anti-patterns — shared between DESIGN_METHODOLOGY and DESIGN_HARD_RULES.
 *
 ...
 * it. Deliberate: zstack wants deterministic cached search for review
...
/**
 * Default model for zstack-owned Codex invocations.
 *
 * Conductor's current Codex CLI default may lag the frontier model exposed to
 * agents, so zstack pins its own default and lets users override it per shell
 * with ZSTACK_CODEX_MODEL or per invocation with an explicit `-c model=...`.
 * The -c form is accepted by both `codex exec` and `codex review`.
 */
export const CODEX_FRONTIER_MODEL = 'gpt-6-astra';
export const CODEX_MODEL_CONFIG_FLAG = `-c "model=\\"\${ZSTACK_CODEX_MODEL:-${CODEX_FRONTIER_MODEL}}\\""`;
// Native review prefers review_model over model when the user has pinned it.
export const CODEX_REVIEW_MODEL_CONFIG_FLAG = `${CODEX_MODEL_CONFIG_FLAG} -c "review_model=\\"\${ZSTACK_CODEX_MODEL:-${CODEX_FRONTIER_MODEL}}\\""`;
 ...
 *   2. sources `zstack-codex-probe`,
 *   3. runs `command -v codex` (literal — keeps the e2e substring assertion),
 *      then `_zstack_codex_auth_probe`, then `_zstack_codex_version_check`,
 *   4. logs the relevant `_zstack_codex_log_event` for each non-ready outcome,
 ...
 * literal `~/.claude/skills/zstack` is correct here and becomes `$ZSTACK_ROOT`
...
export function codexPreflight(opts: { modeVar?: string; disabledBehavior: 'skip-all' | 'codex-only' }): string {
  ...
  const disabledLine = opts.disabledBehavior === 'codex-only'
    ...
    : 'Skip this section entirely; do NOT fall back to a Claude subagent — disabled means no extra review step. Print: "Codex review skipped (codex_reviews disabled). Re-enable: `zstack-config set codex_reviews enabled`."';
  return `\`\`\`bash
...
# (observed: one /review = 15M tokens). A stale own-harness artifact must stop.
  ...
  ${m}="not_installed"; _zstack_codex_log_event "codex_cli_missing" 2>/dev/null || true
elif ! _zstack_codex_auth_probe >/dev/null 2>&1; then
  ${m}="not_authed"; _zstack_codex_log_event "codex_auth_failed" 2>/dev/null || true
else
  ...
  if [ "$_CODEX_MP" -eq 2 ]; then
    ...
    ${m}="ready"; _zstack_codex_version_check 2>/dev/null || true
  fi
fi
echo "CODEX_MODE: $${m}"
\`\`\`

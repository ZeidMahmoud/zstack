 ...
 *
 * Learnings are stored per-project at ~/.zstack/projects/{slug}/learnings.jsonl.
 ...
 * by zstack-learnings-search ("latest winner" per key+type).
 *
 * Cross-project discovery is opt-in. The resolver asks the user once via
 * AskUserQuestion and persists the preference via zstack-config.
 */
...
// queries hand-written in zstack are safe, but the resolver API must defend against
...
export function generateLearningsSearch(ctx: TemplateContext, args?: string[]): string {
  ...
  if (getHostConfig(ctx.host).learningsMode === 'basic') {
    ...
    // uses $ZSTACK_BIN (all basic hosts are env-var hosts)
    return `## Prior Learnings

Search for relevant learnings from previous sessions on this project:

\`\`\`bash
$ZSTACK_BIN/zstack-learnings-search --limit 10${queryFlag} 2>/dev/null || true
\`\`\`
...
\`\`\`bash
_CROSS_PROJ=$(${ctx.paths.binDir}/zstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ${ctx.paths.binDir}/zstack-learnings-search --limit 10${queryFlag} --cross-project 2>/dev/null || true
else
  ${ctx.paths.binDir}/zstack-learnings-search --limit 10${queryFlag} 2>/dev/null || true
fi
\`\`\`
...
If A: run \`${ctx.paths.binDir}/zstack-config set cross_project_learnings true\`
If B: run \`${ctx.paths.binDir}/zstack-config set cross_project_learnings false\`
...
export function generateLearningsLog(ctx: TemplateContext): string {
  const binDir = ctx.paths.binDir; // env-var hosts already resolve to $ZSTACK_BIN via types.ts
...
\`\`\`bash
${binDir}/zstack-learnings-log '{"skill":"${ctx.skillName}","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
\`\`\`

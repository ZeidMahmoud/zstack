 ...
 * under ~/.zstack/projects/<slug>/, falling back to newest project-scoped
 ...
 * ~/.zstack, and the committed copy is what teammates see — but a stale old
 ...
 * the prerequisite-skill re-check in review.ts (ZStack 2 fork-port wave,
 * time-attack/zstack). Drift between copies meant plan reviews could
 ...
 */
export const DESIGN_DOC_DISCOVERY_BLOCK = `_LOCALDOC=$(ls -t ~/.zstack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)

 ...
 *   1. If ~/.zstack-artifacts-remote.txt (or legacy ~/.zstack-brain-remote.txt
 *      during the v1.27.0.0 migration window) exists AND ~/.zstack/.git is
 *      missing, surface a restore-available hint (does NOT auto-run restore).
 *   2. If sync is on, run `zstack-brain-sync --once` (drain + push). The
 ...
 * to `zstack-brain-sync --discover-new` + `--once`.
 */
...
_BRAIN_SYNC_BIN="${ctx.paths.binDir}/zstack-brain-sync"
_BRAIN_CONFIG_BIN="${ctx.paths.binDir}/zstack-config"
...
${isBrainHost ? `If output shows \`ARTIFACTS_SYNC: artifacts repo detected\`, offer \`zstack-brain-restore\` via AskUserQuestion; otherwise continue.` : ''}
...
\`\`\`bash
"${ctx.paths.binDir}/zstack-brain-sync" --discover-new 2>/dev/null || true
"${ctx.paths.binDir}/zstack-brain-sync" --once 2>/dev/null || true
\`\`\`
`;
}

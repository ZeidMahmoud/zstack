 ...
 * zstack skill that opens a web page. {{ASIDE_COOKBOOK}} — the verified script
 ...
 *
 * Aside first, zstack's own browser as fallback. The Aside AI browser
 ...
 * through the same app via lib/aside-render.ts and bin/zstack-render.ts.
 * When Aside is not installed or not running (Linux, Windows, a closed app),
 * {{BROWSE_FALLBACK}} (scripts/resolvers/browse.ts) takes over with zstack's
...
/**
 * `aside exec "<prompt>"` sends zstack-composed text to Aside's agent — an
...
export function asideExecPrelude(ctx: TemplateContext): string {
  ...
  const binDir = ctx?.paths?.binDir ? toShellPath(ctx.paths.binDir) : '$HOME/.claude/skills/zstack/bin';
  return `_EG="${binDir}/zstack-egress-lib.sh"; [ -r "$_EG" ] && . "$_EG"; _aside_exec() { if command -v _zstack_egress_run >/dev/null 2>&1; then _zstack_egress_run open aside-agent aside.com aside-exec "user invoked this skill" --no-payload aside exec "$@"; else aside exec "$@"; fi; }`;
}

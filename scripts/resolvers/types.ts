 ...
 * `~/.claude/skills/zstack/bin`. Bash only performs tilde expansion when the
 ...
 *
 * Env-var hosts already use `$ZSTACK_BIN`, which expands correctly when
 * quoted, so they pass through untouched.
 *
 * Use this ONLY where the path lands inside double quotes. Unquoted
 * interpolations (`${ctx.paths.binDir}/zstack-slug`) expand fine as-is and are
 ...
 * Non-Claude hosts use $ZSTACK_ROOT env vars (set by preamble).
 */
 ...
 * Claude-style dirs are `~`-rooted (e.g. `~/.claude/skills/zstack/browse/dist`)
 * and expand via `$HOME`; env-var hosts already carry an absolute `$ZSTACK_*`

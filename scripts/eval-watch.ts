...
// writes ~/.zstack-dev/e2e-live.json regardless of project ("heartbeat stays
// global"). The PARTIAL file is per-project: EvalCollector writes it into
// getProjectEvalDir() (or ZSTACK_EVAL_DIR), so watching the legacy global
// path missed it whenever slug detection succeeded — i.e. the normal case.
const HEARTBEAT_PATH = path.join(ZSTACK_DEV_DIR, 'e2e-live.json');
const EVAL_DIR = process.env.ZSTACK_EVAL_DIR || getProjectEvalDir();
...
export function renderDashboard(heartbeat: HeartbeatData | null, partial: PartialData | null): string {
  ...
  if (heartbeat?.runId) {
    const logPath = path.join(ZSTACK_DEV_DIR, 'e2e-runs', heartbeat.runId, 'progress.log');

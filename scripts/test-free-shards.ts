 ...
 *
 * Adapted from the McGluut/zstack fork's test-free-shards.ts (190 LOC). The
...
export const KNOWN_WINDOWS_INCOMPATIBLE: Array<{ file: string; reason: string }> = [
  ...
  {
    file: 'test/zstack-decision-semantic.test.ts',
...
const KNOWN_WINDOWS_SAFE: Array<{ file: string; reason: string }> = [
  ...
  },
  {
    ...
    // path.join(ROOT, 'bin', 'zstack-uninstall') constant, but the script is
 ...
 *
 * ZSTACK_FREE_JOBS overrides the computed count (the free runner's analogue
...
export function fullSuiteJobs(): number {
  const raw = process.env.ZSTACK_FREE_JOBS;
  if (raw !== undefined && raw !== '') {
    ...
    if (!/^\d+$/.test(raw.trim()) || Number.parseInt(raw, 10) <= 0) {
      throw new Error(`ZSTACK_FREE_JOBS must be a positive integer, got: ${raw}`);
    }
...
// timestamp). ZSTACK_FREE_TEST_DURATIONS overrides the path for experiments.
...
export function loadFreeTestDurations(rootDir = ROOT): Record<string, number> | null {
  const file = process.env.ZSTACK_FREE_TEST_DURATIONS
...
// ZSTACK_FLAKE_LEDGER at $RUNNER_TEMP and uploads it as an artifact every
...
export function flakeLedgerPath(env: NodeJS.ProcessEnv = process.env): string {
  if (env.ZSTACK_FLAKE_LEDGER) return env.ZSTACK_FLAKE_LEDGER;
  ...
  // series (review finding). CI always sets ZSTACK_FLAKE_LEDGER explicitly.
  try {
    const slug = spawnSync('bash', ['-c', '~/.claude/skills/zstack/bin/zstack-slug 2>/dev/null'], { stdio: 'pipe', timeout: 3000 })
      .stdout?.toString().match(/^SLUG=(.+)$/m)?.[1];
    if (slug) {
      const dir = path.join(os.homedir(), '.zstack', 'projects', slug);
      fs.mkdirSync(dir, { recursive: true });
      return path.join(dir, 'flake-ledger.jsonl');
    }
  } catch { /* fall through */ }
  return path.join(os.tmpdir(), 'zstack-flake-ledger.jsonl');
}
...
export interface FreeShardOutcome {
   ...
   * Feeds the opt-in flaky retry pass (ZSTACK_FREE_RETRY_FLAKY) — empty on
 ...
 * Deliberately NOT ZSTACK_HOME: injecting one shared scratch home for a whole
 ...
 * ZSTACK_HOME isolation mkdtemp their own per test — the repo convention —
...
export async function runFreeShard(
  ...
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-free-shard-'));
  const childTmp = path.join(stateDir, 'tmp');
  fs.mkdirSync(childTmp);
  env.TMPDIR = childTmp;
  env.TEMP = childTmp;
  env.TMP = childTmp;
  // CLI renders otherwise attach to the repo's shared .zstack/browse.json,
  ...
  env.BROWSE_STATE_FILE = path.join(stateDir, '.zstack', 'browse.json');
  ...
  // the SHARED ~/.zstack/chromium-profile, and two concurrent shards on one
...
function nextDefaultLogPath(): string {
  ...
  logPathSequence += 1;
  return path.join(os.tmpdir(), `zstack-free-test-${stamp}-${process.pid}-${logPathSequence}.log`);
}
...
async function recordFreeTestDurations(files: string[], jobs: number): Promise<number> {
  ...
  const target = process.env.ZSTACK_FREE_TEST_DURATIONS ?? path.join(ROOT, FREE_TEST_DURATIONS_FILE);
  const payload = {
    version: 1,
...
async function retryFailedFreeFiles(
  ...
  // Opt-in flaky retry (ZSTACK_FREE_RETRY_FLAKY=1): when every failure is an
  ...
  if (
    worst !== 0
    && process.env.ZSTACK_FREE_RETRY_FLAKY === '1'
    && !isTerminationRequested()

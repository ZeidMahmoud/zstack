#!/bin/bash
# Build ZStack Browser.app — macOS application bundle
#
...
#
# Output: dist/ZStack Browser.app and dist/ZStack-Browser.dmg
#
# Usage:
...
APP_NAME="ZStack Browser"
BUNDLE_ID="com.zstack.browser"
...
# Launcher script
cp "$ROOT/scripts/app/zstack-browser" "$APP_DIR/Contents/MacOS/zstack-browser"
chmod +x "$APP_DIR/Contents/MacOS/zstack-browser"
...
# Patch the bundled Chromium's Info.plist so macOS shows "ZStack Browser"
...
if [ -f "$CHROMIUM_PLIST" ]; then
  ...
  # Replace Chromium's icon with ours so the Dock shows the ZStack icon
...
echo "    Contents/MacOS/zstack-browser     (launcher)"
...
DMG_PATH="$BUILD_DIR/ZStack-Browser.dmg"
echo ""
echo "  Creating DMG..."
rm -f "$DMG_PATH"

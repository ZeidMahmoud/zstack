 ...
 *   3. No per-shard env / eval dir. Each shard needs its own ZSTACK_EVAL_DIR
...
export function selectPaidTestFiles(files: string[], tier: PaidTier, rootDir = ROOT, env: NodeJS.ProcessEnv = process.env): TierSelection {
  ...
  const carveSkill = tier === 'periodic' ? env.ZSTACK_CARVE_SKILL?.trim() : undefined;
  ...
  if (carveSkill && files.some(file => carveWrapper(file)) && !files.some(file => carveWrapper(file) === carveSkill)) {
    throw new Error(`ZSTACK_CARVE_SKILL=${carveSkill} has no generic section-loading wrapper`);
  }
  ...
  for (const file of files) {
    ...
    if (carveSkill && skill && skill !== carveSkill) {
      excluded.push({ file, reason: `ZSTACK_CARVE_SKILL=${carveSkill} selects another section-loading case` });
      continue;
    }
...
export function prProfileTestNamePattern(file: string, selection: PaidCaseSelection): string {
  const labels: Record<string, string> = {
    'plan-review-report': '/plan-eng-review writes ZSTACK REVIEW REPORT to plan file',
...
export interface RunShardsOptions {
  ...
  rootDir?: string;
  env?: NodeJS.ProcessEnv;
  /** When set, each shard child gets ZSTACK_EVAL_DIR=<evalDirBase>/shards/<slug>/. */
  evalDirBase?: string;
...
function nextShardLogPath(files: string[], logDir: string): string {
  ...
  shardLogSequence += 1;
  return path.join(logDir, `zstack-paid-shard-${shardSlug(files)}-${stamp}-${process.pid}-${shardLogSequence}.log`);
}
...
export async function runPaidShard(
  ...
  if (options.evalDirBase) {
    env.ZSTACK_EVAL_DIR = path.join(options.evalDirBase, 'shards', shardSlug(files));
  }
  ...
  if (!env.ZSTACK_CLAUDE_CLI_VERSION) {
    env.ZSTACK_CLAUDE_CLI_VERSION = getClaudeCliVersion();
  }
  ...
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zstack-paid-shard-'));
...
async function main(): Promise<number> {
  ...
  if (options.planPath || options.sliceIndex !== null) {
    ...
    const evalDirBase = process.env.ZSTACK_EVAL_DIR || getProjectEvalDir();
  ...
  const runSummary = await runPaidShards(runnable, {
    ...
    evalDirBase: process.env.ZSTACK_EVAL_DIR || getProjectEvalDir(),
  });

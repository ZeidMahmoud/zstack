...
CSO_FINAL_CORE="$CSO_BUILD_ROOT/bin/zstack-cso-core$CSO_EXE"
CSO_FINAL_LAUNCHER="$CSO_BUILD_ROOT/bin/zstack-cso-launcher$CSO_EXE"
CSO_FINAL_WATCHDOG="$CSO_BUILD_ROOT/bin/zstack-cso-watchdog"
CSO_FINAL_GENERATION="$CSO_BUILD_ROOT/bin/.zstack-cso-generation"
CSO_STAGE=""
CSO_PUBLISHING=0
CSO_COMMITTED=0
CSO_OLD_COHERENT=0
...
cso_checkpoint() {
  [ "${ZSTACK_CSO_BUILD_TESTING:-0}" = 1 ] || return 0
  if [ "${ZSTACK_CSO_BUILD_TEST_FAIL_AFTER:-}" = "$1" ]; then
    echo "Injected CSO build failure after $1" >&2
    exit 86
  fi
  if [ "${ZSTACK_CSO_BUILD_TEST_KILL_AFTER:-}" = "$1" ]; then
    kill -KILL "$$"
  fi
}
...
cso_restore_previous() {
  ...
  if [ "$CSO_OLD_CORE_MOVED" -eq 1 ]; then
    if [ "${ZSTACK_CSO_BUILD_TESTING:-0}" = 1 ] && [ "${ZSTACK_CSO_BUILD_TEST_FAIL_RESTORE:-}" = core ];then rollback_ok=0
...
cso_publish_locked() {
  [ "${ZSTACK_CSO_PUBLISH_LOCKED:-}" = 1 ] || { echo 'CSO publication requires the native publisher lock.' >&2;return 73; }
  [ "$#" -eq 1 ] && [ -d "$1" ] && [ ! -L "$1" ] || { echo 'CSO publication stage is invalid.' >&2;return 69; }
  CSO_STAGE="$(cd "$1" && pwd -P)"
  case "$CSO_STAGE" in "$CSO_BUILD_ROOT"/bin/.zstack-cso-stage.*) ;; *) echo 'CSO publication stage escaped the trusted bin directory.' >&2;return 69;;esac
  CSO_STAGE_CORE="$CSO_STAGE/zstack-cso-core$CSO_EXE"
  CSO_STAGE_LAUNCHER="$CSO_STAGE/zstack-cso-launcher$CSO_EXE"
  CSO_STAGE_WATCHDOG="$CSO_STAGE/zstack-cso-watchdog"
  CSO_STAGE_GENERATION="$CSO_STAGE/.zstack-cso-generation"
...
CSO_STAGE="$(mktemp -d "$CSO_BUILD_ROOT/bin/.zstack-cso-stage.XXXXXX")"
CSO_STAGE_CORE="$CSO_STAGE/zstack-cso-core$CSO_EXE"
CSO_STAGE_LAUNCHER="$CSO_STAGE/zstack-cso-launcher$CSO_EXE"
CSO_STAGE_WATCHDOG="$CSO_STAGE/zstack-cso-watchdog"
CSO_STAGE_GENERATION="$CSO_STAGE/.zstack-cso-generation"
CSO_STAGE_LOCKER="$CSO_STAGE/zstack-cso-publish-lock$CSO_EXE"
mkdir -m 700 "$CSO_STAGE/previous"
if [ -n "$CSO_EXE" ];then : > "$CSO_STAGE/.zstack-cso-generation.lock";fi
...
if [ -n "$CSO_EXE" ];then
  ...
  "$CSO_CC" -std=c11 -D_POSIX_C_SOURCE=200809L -O2 -Wall -Wextra $CSO_LAUNCHER_FLAGS \
    "-DZSTACK_CSO_CORE_SHA256=\"$CSO_CORE_SHA256\"" lib/cso/launcher.c -o "$CSO_STAGE_LAUNCHER"

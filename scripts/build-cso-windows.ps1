...
if (-not $CheckOnly) {
    ...
    function Resolve-StagedOutput([string]$Value, [string]$ExpectedName) {
        ...
        if ([System.IO.Path]::GetFileName($resolved) -cne $ExpectedName -or
            [System.IO.Path]::GetFileName($parent) -notlike '.zstack-cso-stage.*' -or
    ...
    $stagedOutput = Resolve-StagedOutput $OutputPath 'zstack-cso-launcher.exe'
    $stagedLockOutput = Resolve-StagedOutput $LockOutputPath 'zstack-cso-publish-lock.exe'
}
...
$temporary = Join-Path ([System.IO.Path]::GetTempPath()) ('zstack-cso-msvc-' + [guid]::NewGuid().ToString('N'))
...
try {
    if ($CheckOnly) {
        ...
        Set-Content -LiteralPath $binding -Encoding Ascii -Value "#define ZSTACK_CSO_CORE_SHA256 `"$CoreSha256`"`n#define ZSTACK_CSO_GIT_PATH L`"$gitLiteral`""
        $builds = @(

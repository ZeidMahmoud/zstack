 ...
 *   - bin/zstack-brain-cache (drives TTL + write-back invalidation)
...
export interface BrainCacheEntity {
  /** Filename inside ~/.zstack/{,projects/<slug>/}brain-cache/ */
  file: string;
   ...
   *   - 'skill-run-write'   — any skill that writes a zstack/skill-run page
 ...
 * `zstack-config set salience_allowlist '<comma-separated-prefixes>'`.
 */
...
/**
 * Retention policy: zstack/skill-run pages auto-archive after this many days.
 ...
 */
export const ZSTACK_SCHEMA_PACK_NAME = 'zstack-core';
export const ZSTACK_SCHEMA_PACK_VERSION = '1.0.0';
 ...
 * and persisted via `zstack-config set user_slug_at_<endpoint-hash> <slug>`.

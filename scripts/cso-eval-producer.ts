...
export function resolveProducerHelperBinding(
  ...
  const launcher = testLauncherPath ?? join(dirname(process.execPath), `zstack-cso-launcher${executableSuffix}`);
  if (!isAbsolute(launcher) || resolve(launcher) !== launcher || basename(launcher) !== `zstack-cso-launcher${executableSuffix}`) {
    throw new Error('INVALID_PRODUCER_HELPER');
  }
  ...
  const binding = {
    ...
    launcher,
    core: join(directory, `zstack-cso-core${executableSuffix}`),
    watchdog: join(directory, `zstack-cso-watchdog${executableSuffix}`),
    generation: join(directory, '.zstack-cso-generation'),
  };
  try {
...
export async function runProducerCell(inputPath: string, receiptPath: string, options: {
  ...
  const previousHome = process.env.ZSTACK_HOME;
  const previousSessionKind = process.env.ZSTACK_SESSION_KIND;
  const previousHeadless = process.env.ZSTACK_HEADLESS;
  process.env.ZSTACK_HOME = helperHome;
  process.env.ZSTACK_SESSION_KIND = 'spawned';
  process.env.ZSTACK_HEADLESS = '1';
  ...
  try {
    ...
    try {
      ...
      if (previousHome === undefined) delete process.env.ZSTACK_HOME; else process.env.ZSTACK_HOME = previousHome;
      if (previousSessionKind === undefined) delete process.env.ZSTACK_SESSION_KIND; else process.env.ZSTACK_SESSION_KIND = previousSessionKind;
      if (previousHeadless === undefined) delete process.env.ZSTACK_HEADLESS; else process.env.ZSTACK_HEADLESS = previousHeadless;
    }
  }

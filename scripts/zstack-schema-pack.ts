/**
 * zstack-core@1.0.0 schema pack (T1 / Phase 0).
 *
 * Defines the 7 typed page kinds zstack writes into a personal gbrain:
 *   zstack/user-profile, zstack/product, zstack/goal, zstack/developer-persona,
 *   zstack/brand, zstack/competitive-intel, zstack/skill-run
 *
 * Plus the typed take kind zstack writes for Phase 2 calibration:
 *   zstack/take (kind=bet, holder=<user>, with expected_resolution_date)
 *
 ...
 * `mcp__gbrain__schema_graph` to render the zstack subgraph correctly.
 */

import {
  ZSTACK_SCHEMA_PACK_NAME,
  ZSTACK_SCHEMA_PACK_VERSION,
...
export interface SchemaTypeDefinition {
  /** Page type slug, e.g. `zstack/product`. */
  type: string;
...
const USER_PROFILE: SchemaTypeDefinition = {
  type: 'zstack/user-profile',
  description:
    'Cross-project profile of the zstack user: tone/conviction patterns, ' +
  ...
  fields: [
    { name: 'type', type: 'string', required: true, description: 'zstack/user-profile' },
    { name: 'slug', type: 'string', required: true, description: 'zstack/user-profile/<user-slug>' },
  ...
  emits_links: [
    { verb: 'has_calibration', target_type: 'zstack/take' },
  ],
};
...
const PRODUCT: SchemaTypeDefinition = {
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'zstack/product' },
    { name: 'slug', type: 'string', required: true, description: 'zstack/product/<project-slug>' },
  ...
  emits_links: [
    { verb: 'targets', target_type: 'zstack/goal' },
    { verb: 'observed_by', target_type: 'zstack/developer-persona' },
    { verb: 'has_brand', target_type: 'zstack/brand' },
    { verb: 'competes_with', target_type: 'zstack/competitive-intel' },
    { verb: 'history', target_type: 'zstack/skill-run' },
  ],
};
...
const GOAL: SchemaTypeDefinition = {
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'zstack/goal' },
    { name: 'slug', type: 'string', required: true, description: 'zstack/goal/<project-slug>/<goal-id>' },
  ...
  emits_links: [
    { verb: 'belongs_to', target_type: 'zstack/product' },
  ],
};

const DEVELOPER_PERSONA: SchemaTypeDefinition = {
  type: 'zstack/developer-persona',
  description:
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'zstack/developer-persona' },
    { name: 'slug', type: 'string', required: true, description: 'zstack/developer-persona/<project-slug>' },
...
const BRAND: SchemaTypeDefinition = {
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'zstack/brand' },
    { name: 'slug', type: 'string', required: true, description: 'zstack/brand/<project-slug>' },
...
const COMPETITIVE_INTEL: SchemaTypeDefinition = {
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'zstack/competitive-intel' },
    { name: 'slug', type: 'string', required: true, description: 'zstack/competitive-intel/<project-slug>' },
...
const SKILL_RUN: SchemaTypeDefinition = {
  type: 'zstack/skill-run',
  description:
    'Every zstack skill invocation that produces output writes one of these on completion. ' +
    'Time-series log of decisions, modes, mode-selected, outcomes. Powers /retro ' +
    'and (deferred) /zstack-reflect. Auto-archives to summary-only after 90 days.',
  retention: 'archive-after-90d',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'zstack/skill-run' },
    { name: 'slug', type: 'string', required: true, description: 'zstack/skill-run/<project>/<skill>/<timestamp>' },
  ...
  emits_links: [
    { verb: 'related_to', target_type: 'zstack/product' },
    { verb: 'related_to', target_type: 'zstack/goal' },
    { verb: 'writes_bet', target_type: 'zstack/take' },
  ],
};
...
const TAKE: SchemaTypeDefinition = {
  ...
  retention: 'never-archive',
  fields: [
    { name: 'type', type: 'string', required: true, description: 'zstack/take' },
    { name: 'slug', type: 'string', required: true, description: 'zstack/take/<project>/<date>/<id>' },
  ...
  emits_links: [
    { verb: 'belongs_to', target_type: 'zstack/user-profile' },
    { verb: 'origin', target_type: 'zstack/skill-run' },
  ],
};
...
export const ZSTACK_CORE_SCHEMA_PACK: SchemaPackJSON = {
  name: ZSTACK_SCHEMA_PACK_NAME,
  version: ZSTACK_SCHEMA_PACK_VERSION,
  page_types: [
    USER_PROFILE,
    PRODUCT,
    GOAL,
...
export function getSchemaPackMutationPayload(): {
  ...
  return {
    schema_pack: ZSTACK_CORE_SCHEMA_PACK,
...
export function getSchemaPackTypeNames(): ReadonlyArray<string> {
  return ZSTACK_CORE_SCHEMA_PACK.page_types.map((t) => t.type);
}
...
export function getRetentionPolicy(pageType: string): SchemaTypeDefinition['retention'] {
  const def = ZSTACK_CORE_SCHEMA_PACK.page_types.find((t) => t.type === pageType);

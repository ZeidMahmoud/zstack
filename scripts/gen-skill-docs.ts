...
function loadGbrainOverride(): { detected: boolean } {
  ...
  try {
    ...
    // "ok", matching zstack-gbrain-detect --is-ok.
...
// zstack/llms.txt) into a separate (untracked) directory instead of writing
...
// section-base path (`~/.claude/skills/zstack/<skill>/sections/`) inside
...
// (bin/zstack-config gbrain-refresh, setup — the #2569 pattern) must pass the
 ...
 * docs/ references keep pointing at `~/.claude/skills/zstack` (the global
...
function rewriteSectionBase(content: string): string {
  ...
  return content.replace(
    /~\/\.claude\/skills\/zstack\/([^\s)`"'*]+\/sections\/)/g,
...
for (const currentHost of hostsToRun) {
  ...
  try {
    ...
    // Generate the OpenClaw orchestrator-injection docs (zstack-lite / zstack-full /
    // zstack-plan CLAUDE.md snippets). Sources live in openclaw/templates/ —
    ...
    if (currentHost === 'openclaw' && !DRY_RUN) {
      ...
      for (const variant of ['lite', 'full', 'plan'] as const) {
        const fileName = `zstack-${variant}-CLAUDE.md`;
  ...
  try {
    ...
    if (fs.existsSync(configPath)) {
      ...
      if (/^skill_prefix:\s*true/m.test(config)) {
        console.log('\nNote: skill_prefix is true. Run zstack-relink to re-apply name: patches (it patches both the install and any active gbrain render).');
      }
    }
  } catch { /* non-fatal */ }
}
  ...
  void (async () => {
    try {
      const result = await writeLlmsTxt(
        ...
        OUT_DIR ? { outputPath: path.join(OUT_DIR, 'zstack', 'llms.txt') } : {},
      );
    ...
    // Regenerate agents-digest/zstack-AGENTS.md — the instruction-only tier
    ...
    try {
      ...
      console.log(`[gen-agents-digest] agents-digest/zstack-AGENTS.md: ${digest.bytes} bytes (budget ${DIGEST_BYTE_BUDGET})`);
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);

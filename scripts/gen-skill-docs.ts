...
// When --respect-detection is passed, read ~/.zstack/gbrain-detection.json
...
// bin/zstack-gbrain-detect and persisted by `zstack-config gbrain-refresh`
// or by ./setup.
//
...
function loadGbrainOverride(): { detected: boolean } {
  ...
  const stateDir = process.env.ZSTACK_HOME || path.join(process.env.HOME || '', '.zstack');
...
function externalSkillName(skillDir: string, frontmatterName?: string): string {
  // Root skill (skillDir === '' or '.') always maps to 'zstack' regardless of frontmatter
  if (skillDir === '.' || skillDir === '') return 'zstack';
  ...
  // Don't double-prefix: zstack-upgrade → zstack-upgrade (not zstack-zstack-upgrade)
  if (baseName.startsWith('zstack-')) return baseName;
  return `zstack-${baseName}`;
}
...
// "(zstack)" tag. This pile is the always-loaded catalog surface — every
// session pays for the full text. The catalog trim splits the description
// into a one-line catalog entry (lead sentence + "(zstack)") that stays in
...
export function splitCatalogDescription(description: string): CatalogParts {
  ...
  const hasZstackTag = /\(zstack\)/.test(working);
  if (hasZstackTag) working = working.replace(/\(zstack\)/, '').trim();
  ...
  return { lead, routingProse, voiceLine, hasZstackTag };
}
...
export function buildTrimmedDescription(parts: CatalogParts): string {
  ...
  const suffix = parts.hasZstackTag ? ' (zstack)' : '';
  return `${lead}${suffix}`;
}
 ...
 *  - shorten frontmatter `description:` to lead + (zstack)
...
export function applyCatalogTrim(content: string, skillName: string): { content: string; parts: CatalogParts } | null {
  ...
  // If lead + (zstack) is already most of the text, no trim needed.
  ...
  try {
    ...
    for (const tmplPath of findTemplates()) {
      ...
      if (catalogParts) {
        ...
        // name (e.g. "seville-v3" in a Conductor worktree, "zstack" on CI).
        ...
        // root SKILL.md.tmpl declares `name: zstack` explicitly. For all other
...
`;
      fs.writeFileSync(path.join(openclawDir, 'zstack-lite-CLAUDE.md'), zstackLite);
      console.log('GENERATED: openclaw/zstack-lite-CLAUDE.md');

      const zstackFull = `# zstack-full Pipeline
...
`;
      fs.writeFileSync(path.join(openclawDir, 'zstack-full-CLAUDE.md'), zstackFull);
      console.log('GENERATED: openclaw/zstack-full-CLAUDE.md');

      const zstackPlan = `# zstack-plan: Full Review Gauntlet
...
5. Report back to the orchestrator:
   ...
   - Recommended next step (usually: spawn a new session with zstack-full to implement)

Do not implement anything. This is planning only.
The orchestrator will persist the plan link to its own memory/knowledge store.
`;
      fs.writeFileSync(path.join(openclawDir, 'zstack-plan-CLAUDE.md'), zstackPlan);
      console.log('GENERATED: openclaw/zstack-plan-CLAUDE.md');
    }
    ...
    if (currentHost === 'claude' && CATALOG_MODE === 'trim' && Object.keys(proactiveAggregate).length > 0 && !DRY_RUN) {
      ...
      const payload = {
        $schema: 'https://zstack.dev/schemas/proactive-suggestions.json',
        catalog_mode: 'trim',
...
if (!DRY_RUN) {
  try {
    const configPath = path.join(process.env.HOME || '', '.zstack', 'config.yaml');
    if (fs.existsSync(configPath)) {
      const config = fs.readFileSync(configPath, 'utf-8');
      if (/^skill_prefix:\s*true/m.test(config)) {
        console.log('\nNote: skill_prefix is true. Run zstack-relink to re-apply name: patches.');
      }
    }
  } catch { /* non-fatal */ }
}

// Regenerate zstack/llms.txt — single-file capability index for AI agents.
  ...
  void (async () => {
    try {
      ...
      if (result.warnings.length > 0) {
        ...
        console.log(`[gen-llms-txt] zstack/llms.txt: ${result.skills.length} skills, ${result.browseCommands.length} browse commands`);
      }
    } catch (err) {

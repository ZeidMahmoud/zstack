...
export async function runGeneration(settings: GenerationOptions = {}): Promise<GenerationResult> {
    ...
    try {
      ...
      if (!settings.dryRun && host !== 'claude') {
        ...
        for (const entry of entries) {
          if (entry.isSymbolicLink() || !entry.isDirectory() || !entry.name.startsWith('zstack-') || renderedNames.has(entry.name)) continue;
          // Keep the old render usable until setup has migrated installed copies/links.
          if (entry.name === 'zstack-claude' && process.env.ZSTACK_DEFER_CLAUDE_RENAME_PRUNE === '1') continue;
          let generated = false;
          try {
  ...
  try {
    ...
    emit(path.join(options.outputRoot, 'zstack', 'llms.txt'), index.content, 'index');
...
export async function main(args = process.argv.slice(2)): Promise<number> {
  try {
    ...
    if (!settings.dryRun) {
      try {
        const config = fs.readFileSync(path.join(process.env.HOME || '', '.zstack', 'config.yaml'), 'utf-8');

 ...
 *      comes from --email CLI flags (repeatable), the ZSTACK_AUTHOR_EMAILS env
 ...
 * Run: bun run scripts/zeid-output-comparison.ts [--repo-root <path>] [--email <addr>...]
 *      ZSTACK_AUTHOR_EMAILS=a@x.com,b@y.com bun run scripts/zeid-output-comparison.ts
...
function resolveAuthorEmails(argv: string[]): string[] {
  ...
  const envVar = process.env.ZSTACK_AUTHOR_EMAILS;
  ...
  process.stderr.write(
    ...
    'set ZSTACK_AUTHOR_EMAILS=a@x.com,b@y.com, or configure git user.email.\n'
  );
  process.exit(1);
}
...
function main() {
  ...
  const output: Output = {
    ...
    caveats_global: [
      ...
      'Authorship attribution relies on commit email matching. Supply historical aliases via --email flags or ZSTACK_AUTHOR_EMAILS.',
    ],
    version: 1,
  };

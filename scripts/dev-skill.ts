...
function regenerateAndValidate() {
  ...
  // (.claude/zstack-rendered), refresh it too so live template edits reflect at
  // this workspace's runtime. Only runs when the render dir already exists — we
  // never create it during plain template dev.
  const RENDER_DIR = path.join(ROOT, '.claude', 'zstack-rendered');
  if (fs.existsSync(RENDER_DIR)) {
    try {
      ...
      console.log('  [render] refreshed .claude/zstack-rendered (brain-aware workspace copy)');
    } catch (err: any) {

#!/usr/bin/env bun
/**
 * Generate zstack/llms.txt — a single discoverable index of every zstack
 ...
 *
 * Output: zstack/llms.txt at repo root.
 *
...
const OUTPUT = path.join(ROOT, 'zstack', 'llms.txt');
 ...
 * the zstack convention; we collapse those into a single paragraph.
 */
 ...
 * in a full zstack checkout; absent on minimal installs. Returns {} if the
...
export async function generateLlmsTxt(opts: GenerateOptions = {}): Promise<GenerateResult> {
  ...
  lines.push('# zstack');
  lines.push('');
  lines.push("> zstack is Garry's Stack: AI coding skills + a fast headless browser binary + a design CLI. This file indexes every capability so agents can discover and invoke them without crawling individual SKILL.md files.");
  lines.push('');
  ...
  lines.push('## More');
  lines.push('');
  lines.push('- Repository: https://github.com/ZeidMahmoud/zstack');
  ...
  void (async () => {
    ...
    if (dryRun) {
      ...
      if (existing !== result.content) {
        console.error('[gen-llms-txt] OUT OF DATE — run `bun run gen:skill-docs` to regenerate zstack/llms.txt');
        process.exit(1);
      }

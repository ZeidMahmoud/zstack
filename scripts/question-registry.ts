/**
 * Question Registry — typed schema for AskUserQuestion invocations across zstack.
 *
 * Purpose
 * -------
 ...
 *
 * Not every AskUserQuestion in zstack needs a registry entry right away. Skills
...
export interface QuestionDef {
  ...
  id: string;
  /** Skill that owns this question (must match a zstack skill directory name) */
  skill: string;
...
export const QUESTIONS = {
  ...
  // -----------------------------------------------------------------------
  // /zstack-upgrade — version upgrade
  // -----------------------------------------------------------------------
  'zstack-upgrade-inline': {
    id: 'zstack-upgrade-inline',
    skill: 'zstack-upgrade',
    category: 'approval',
    door_type: 'two-way',
    options: ['yes-upgrade', 'always-auto', 'not-now', 'never-ask'],
    description: "Upgrade zstack now? (Also: always auto-upgrade, snooze, or disable the prompt.)",
  },
  ...
  'preamble-telemetry-consent': {
    ...
    description: "Share usage data with zstack? community (recommended) / anonymous / off",
  },
  'preamble-proactive-behavior': {
    ...
    door_type: 'two-way',
    options: ['on', 'off'],
    description: "Let zstack proactively suggest skills based on conversation context?",
  },
  'preamble-routing-injection': {
    ...
    description: "Add zstack skill routing rules to CLAUDE.md?",
  },
  'preamble-vendored-migration': {
    ...
    description: "This repo has vendored zstack (deprecated) — migrate to team mode?",
  },

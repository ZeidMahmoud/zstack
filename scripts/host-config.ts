...
export interface HostConfig {
  ...
  // --- Path Configuration ---
  /** Global install path relative to $HOME (e.g., '.config/opencode/skills/zstack'). */
  globalRoot: string;
  /** Project-local skill path relative to repo root (e.g., '.opencode/skills/zstack'). */
  localSkillRoot: string;
  /** Gitignored directory under repo root for generated docs (e.g., '.opencode'). */
  hostSubdir: string;
  /** Whether preamble generates $ZSTACK_ROOT env vars (true for non-Claude hosts). */
  usesEnvVars: boolean;
  ...
  /** Optional repo-local sidecar config (e.g., Codex uses .agents/skills/zstack). */
  sidecar?: {
    /** Sidecar path relative to repo root (e.g., '.agents/skills/zstack'). */
    path: string;
  ...
  // --- Install Behavior ---
  install: {
    /** Whether zstack-config skill_prefix applies (Claude only). */
    prefixable: boolean;

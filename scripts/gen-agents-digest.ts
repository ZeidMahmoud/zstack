#!/usr/bin/env bun
/**
 * Generate agents-digest/zstack-AGENTS.md — the instruction-only tier.
 *
 ...
 * overwrite a user's own AGENTS.md (a zstack-AGENTS.md is auto-read by no
 ...
 * per-host rule-text sprawl zstack's generator model exists to avoid). If a
...
export const DIGEST_RELPATH = 'agents-digest/zstack-AGENTS.md';
...
export function generateAgentsDigest(opts?: { root?: string }): { content: string; bytes: number } {
  ...
  const content = `# zstack digest v${version} — regenerate/re-copy after upgrading zstack

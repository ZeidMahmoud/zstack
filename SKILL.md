...
preamble-tier: 1
version: 1.1.0
description: Fast headless browser for QA testing and site dogfooding. (zstack)
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"zstack","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.zstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/zstack/bin/zstack-timeline-log '{"skill":"zstack","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
# zstack browse: QA Testing & Dogfooding
...
`$B` is the browse binary (resolved from `$_ROOT/.claude/skills/zstack/browse/dist/browse` or `~/.claude/skills/zstack/browse/dist/browse`).

...
import { extractImplementationPlan, checkPhaseImplementation, acceptedBlocks } from '../../bin/zstack-autoplan-snapshot';
...
function initArguments(command: unknown, root: string): string[] | undefined {
  ...
  if (!args.every(ownPath) || fs.realpathSync(args[0]!) !== path.join(root, 'bin', 'zstack-autoplan-snapshot.ts')) return;
  return args.slice(1);
}

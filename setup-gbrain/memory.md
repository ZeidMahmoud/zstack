# zstack memory ingest — what it does, what stays local, what you can do with it
...
| Eureka log | `eureka` | `~/.zstack/analytics/eureka.jsonl` | Medium — your insights, often non-secret |
| Project learnings | `learning` | `~/.zstack/projects/<slug>/learnings.jsonl` | Medium |
| Project timeline | `timeline` | `~/.zstack/projects/<slug>/timeline.jsonl` | Low |
| CEO plans | `ceo-plan` | `~/.zstack/projects/<slug>/ceo-plans/*.md` | Medium |
| Design docs | `design-doc` | `~/.zstack/projects/<slug>/*-design-*.md` | Medium |
| Retros | `retro` | `~/.zstack/projects/<slug>/retros/*.md` | Medium |
| Builder profile | `builder-profile-entry` | `~/.zstack/builder-profile.jsonl` | Low |

## What stays local

- **State files** (`~/.zstack/.gbrain-sync-state.json`,
  `~/.zstack/.transcript-ingest-state.json`,
  `~/.zstack/.gbrain-engine-cache.json`,
  `~/.zstack/.gbrain-errors.jsonl`) are local-only per ED1 (state file
...
(`~/.zstack/gbrain-repo-policy.json`, managed by
`zstack-gbrain-repo-policy`). Each transcript's git remote is checked
...
- **Corrupted or unreadable store** — ingestion aborts before any writes
  ...
  `zstack-gbrain-repo-policy list`; re-run `/setup-gbrain` if it's corrupt.
...
The cross-machine secret boundary is `zstack-brain-sync` (the git push
...
```bash
bun run bin/zstack-memory-ingest.ts --bulk --scan-secrets
# or
ZSTACK_MEMORY_INGEST_SCAN_SECRETS=1 bun run bin/zstack-memory-ingest.ts --bulk
```
...
was skipped via `bun run bin/zstack-memory-ingest.ts --probe` (which
...
- **Supabase configured:** code + transcripts go to Supabase Storage
  ...
  brain-linked git repo via `zstack-brain-sync`.
...
- **Bulk-delete by criteria** (V1.0.1 follow-up — `zstack-transcript-prune`
  helper). For V1.0, use `gbrain delete_page <slug>` per-page or write
  a small loop over `gbrain list_pages` output.

- **Disable entirely:**
  ```bash
  zstack-config set transcript_ingest_mode off
  zstack-config set gbrain_context_load off  # also disables retrieval
  ```

## How the agent uses it

At every zstack skill start, the preamble runs
`zstack-brain-context-load` which:
...
- **Salience block is empty** — your transcripts may not be ingested
  yet. Run `bun run bin/zstack-gbrain-sync.ts --full` to do a full pass.
...
- **A page has stale or wrong content** — `gbrain delete_page <slug>`,
  then re-run `bun run bin/zstack-gbrain-sync.ts --incremental` to re-ingest from
  source if the source file is still on disk and unchanged.

## Privacy + audit

- Every `secretScanFile` finding is logged to stderr at ingest time.
- Every gbrain put/delete is logged to `~/.zstack/.gbrain-errors.jsonl`
  with `{ts, op, duration_ms, outcome}` for forensic tracing.
- `~/.zstack/.gbrain-engine-cache.json` shows which storage tier is
...
1. Verifies the URL + token via `zstack-gbrain-mcp-verify`. Three failure
...
3. Skips local install, local doctor, transcript ingest, and federated
   ...
   doesn't install.
4. Optionally provisions a `zstack-artifacts-$USER` private repo on
...
`zstack-artifacts-init` always prints the `gbrain sources add` command
...
(See `zstack/setup-gbrain/SKILL.md.tmpl` for the full Path 4 flow plus
the gbrain enhancement requests around scoped tokens that would let
zstack auto-rotate in V2.)

...
preamble-tier: 2
version: 1.0.0
description: "Set up gbrain for this coding agent: install the CLI, initialize a local PGLite or Supabase brain, register MCP, capture per-remote trust policy. (zstack)"
triggers:
  - setup gbrain
  - install gbrain
  - connect gbrain
...
```bash
~/.claude/skills/zstack/bin/zstack-gbrain-detect
```
...
```bash
~/.claude/skills/zstack/bin/zstack-gbrain-install
```
...
```bash
current_tier=$(~/.claude/skills/zstack/bin/zstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.claude/skills/zstack/bin/zstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
Separate AskUserQuestion: "Also sync your zstack artifacts (CEO plans,
...
`zstack-artifacts-$USER` (private), and writes the canonical HTTPS URL to
`~/.zstack-artifacts-remote.txt`. Pass `--url-form-supported` from Step 4c's
verify output (Path 4) or `false` (Paths 1/2/3 — local mode doesn't probe):

```bash
URL_FORM=${URL_FORM_SUPPORTED:-false}
~/.claude/skills/zstack/bin/zstack-artifacts-init --url-form-supported "$URL_FORM"
~/.claude/skills/zstack/bin/zstack-config set artifacts_sync_mode artifacts-only
# or "full" if user picked yes-full
```

`zstack-artifacts-init` always prints a "Send this to your brain admin" block
...
In remote mode, the local `zstack-gbrain-source-wireup` helper does NOT run
...
any gbrain client. The helper creates a `git worktree` of `~/.zstack/`,
...
try:
    ...
    pass
")
~/.claude/skills/zstack/bin/zstack-gbrain-source-wireup --strict \
  ${GBRAIN_URL:+--database-url "$GBRAIN_URL"}
```

`--strict` exits non-zero on missing prereqs (gbrain not installed, < 0.18.0,
or no `~/.zstack/.git` yet) so the user sees the failure rather than silently
...
this machine's transcripts indexed, they pull from your `zstack-artifacts-$USER`
...
The brain trust policy controls whether zstack auto-pushes `~/.zstack/`
...
> shared/team brain?
>
> Personal: zstack auto-pushes ~/.zstack/ artifacts (CEO plans, design
...
>
> Shared/team: read-only by default. zstack reads context but prompts
...
```bash
~/.claude/skills/zstack/bin/zstack-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(~/.claude/skills/zstack/bin/zstack-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  ~/.claude/skills/zstack/bin/zstack-config set artifacts_sync_mode full
...
```bash
~/.claude/skills/zstack/bin/zstack-gbrain-detect 2>/dev/null || true
~/.claude/skills/zstack/bin/zstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.claude/skills/zstack/bin/zstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
[ -f ~/.zstack/.gbrain-sync-state.json ] && cat ~/.zstack/.gbrain-sync-state.json || echo "{}"
```
...
gbrain status: GREEN  (mode: remote-http)
  ...
  Artifacts repo .. OK   {zstack_artifacts_remote URL}
...
- If user picked B (No): `N/A declined at Step 4d` — `zstack-config set local_code_index_offered true` to silence future migration notices.

The **Transcripts** row changed in v1.34.0.0: in remote-http mode,
zstack-memory-ingest now persists staged transcripts to
`~/.zstack/transcripts/run-<pid>-<ts>/` and zstack-brain-sync pushes them
...
- **Concurrent-run lock.** At skill start, create the parent before acquiring
  ...
  ```bash
  if ! mkdir -p ~/.zstack; then
    echo "ERROR: Cannot create setup-gbrain lock parent ~/.zstack." >&2
    exit 1
  fi
  if ! mkdir ~/.zstack/.setup-gbrain.lock.d; then
    if [ -d ~/.zstack/.setup-gbrain.lock.d ]; then
      echo "Another /setup-gbrain instance is running. Wait for it, or remove ~/.zstack/.setup-gbrain.lock.d only if you are sure it is stale." >&2
    else

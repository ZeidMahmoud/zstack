...
- **Concurrent-run lock.** At skill start, `mkdir ~/.zstack/.setup-gbrain.lock.d`
  (atomic). If the mkdir fails, abort with: "Another `/setup-gbrain` instance
  is running. Wait for it, or `rm -rf ~/.zstack/.setup-gbrain.lock.d` if

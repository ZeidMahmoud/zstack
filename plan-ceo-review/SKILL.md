...
gbrain:
  ...
  context_queries:
    - id: prior-ceo-plans
      kind: filesystem
      glob: "{zstack_state_root}/projects/{repo_slug}/ceo-plans/*.md"
      sort: mtime_desc
      limit: 5
...
`~/.zstack/projects/`, metrics = `~/.zstack/analytics/`; log helpers choose theirs.
...
3. Resolve that recommendation. When `QUESTION_TUNING: true`, first check
   ...
   wins. When `QUESTION_TUNING: true`, include `<zstack-qid:plan-ceo-review-mode>`.
...
CEO_PLANS="$ZSTACK_STATE_ROOT/projects/$SLUG/ceo-plans"
mkdir -p "$CEO_PLANS"
...
```bash
mkdir -p ~/.zstack/analytics || exit 1
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.zstack/analytics/spec-review.jsonl || exit 1
```
...
```
Save the sequence, feasibility blockers and pending choices in the plan, with human-team and CC + zstack effort.

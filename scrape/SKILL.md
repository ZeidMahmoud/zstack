...
preamble-tier: 1
version: 2.0.0
description: Pull data from a web page through the Aside browser — your real, already signed-in sessions. (zstack)
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "scrape" --outcome OUTCOME \
...
**On the zstack-browser fallback, the browser-skills runtime applies.** Before
...
A script whose output has no `ZSTACK_STEP_OK` (or a line starting with
...
```bash
~/.claude/skills/zstack/bin/zstack-learnings-log '{"skill":"scrape","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```

...
REDACT_BODY_EOF
if REDACT_JSON=$("$HOME/.claude/skills/zstack/bin/zstack-redact" --from-file "$REDACT_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json); then REDACT_CODE=0; else REDACT_CODE=$?; fi
case "$REDACT_CODE" in
...
_OUTSIDE_EXIT=0
_zstack_codex_timeout_wrapper 120 codex exec "$_OUTSIDE_PROMPT" -C "$_REPO_ROOT" -s read-only -c "model=\"${ZSTACK_CODEX_MODEL:-gpt-6-astra}\"" -c 'model_reasoning_effort="medium"' -c 'web_search="cached"' < /dev/null >"$_OUTSIDE_TMP/text" 2>"$_OUTSIDE_TMP/stderr" || _OUTSIDE_EXIT=$?
...
bun "$HOME/.claude/skills/zstack/lib/outside-review-result.ts" spec "$_OUTSIDE_TMP/text" || exit 1
...
Read `ZSTACK_PLAN_MODE` from the environment (emitted by the preamble bash at
...
~/.claude/skills/zstack/bin/zstack-decision-log '{"decision":"Spec filed #ISSUE_NUMBER: TITLE","rationale":"APPROACH","scope":"issue","issue":"ISSUE_NUMBER","source":"skill","confidence":7}' 2>/dev/null || true
```

...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === 'getToken') {
    if (sender.tab) {
      console.warn('[zstack] Rejected getToken from content script context');
  ...
  if (msg.type === 'sidebar-command') {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      fetch(`${base}/sidebar-command`, {
        ...
        .then(r => {
          if (!r.ok) {
            console.error(`[zstack bg] sidebar-command failed: ${r.status} ${r.statusText}`);
        ...
        .catch(err => {
          console.error('[zstack bg] sidebar-command error:', err.message);

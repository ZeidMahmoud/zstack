...
document.getElementById('chat-cookies-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    console.error('[zstack sidebar] Failed to open cookie picker:', err.message);
  }
});
...
async function ensureSseSessionCookie() {
  ...
  try {
    ...
    console.warn('[zstack sidebar] Failed to mint SSE session cookie:', err && err.message);
    return false;
  }
}
...
async function connectSSE() {
  ...
  eventSource.addEventListener('activity', (e) => {
    try { addEntry(JSON.parse(e.data)); } catch (err) {
      console.error('[zstack sidebar] Failed to parse activity event:', err.message);
    }
  });
  ...
  eventSource.addEventListener('gap', (e) => {
    try {
      ...
      console.error('[zstack sidebar] Failed to parse gap event:', err.message);
    }
  });
}
...
async function pollMemoryOnce() {
  ...
  try {
    ...
    try { evaluateMemToast(snapshot); } catch (err) {
      console.debug('[zstack sidebar] mem-toast evaluation failed:', err && err.message);
    }
    ...
    if (elapsed > MEM_POLL_SLOW_THRESHOLD_MS) {
      console.debug('[zstack sidebar] /memory poll slow/failed:', elapsed, 'ms', err && err.message);
    }
...
async function loadSnoozeState() {
  ...
  try {
    ...
    console.debug('[zstack sidebar] mem-toast snooze load failed:', err && err.message);
  }
}
...
async function saveSnoozeState() {
  ...
  try {
    ...
    console.debug('[zstack sidebar] mem-toast snooze save failed:', err && err.message);
  }
}
  ...
  closeBtn.onclick = async () => {
    ...
    for (const id of ids) {
      try {
        ...
        console.warn('[zstack sidebar] mem-toast closetab failed:', id, err && err.message);
      }
    }
  };
}
  ...
  if (snooze) {
    snooze.addEventListener('click', async () => {
      ...
      try {
        ...
        console.debug('[zstack sidebar] mem-toast snooze fetch failed:', err && err.message);
      }
      dismissMemToast();
    });
  }
...
async function fetchRefs() {
  ...
  try {
    ...
    console.error('[zstack sidebar] Failed to fetch refs:', err.message);
  }
}
...
inspectorSendBtn.addEventListener('click', async () => {
  ...
  // zstackScanForPTYInject is async; zstackInjectToTerminal stays sync.
  const verdict = await window.zstackScanForPTYInject?.(message + '\n', 'inspector-send');
  if (verdict?.verdict === 'BLOCK') {
    console.warn('[zstack sidebar] Inspector send BLOCKED by /pty-inject-scan:', verdict.reasons);
    return;
  }
  ...
  const ok = window.zstackInjectToTerminal?.(message + '\n');
  if (!ok) {
    console.warn('[zstack sidebar] Inspector send needs an active Terminal session.');
  }
});
...
async function runCleanup(...buttons) {
  ...
  // confirms every call site goes through zstackScanForPTYInject first.
  const verdict = await window.zstackScanForPTYInject?.(cleanupPrompt + '\n', 'cleanup-button');
  if (verdict?.verdict === 'BLOCK') {
    console.warn('[zstack sidebar] Cleanup BLOCKED by /pty-inject-scan:', verdict.reasons);
  ...
  const sent = window.zstackInjectToTerminal?.(cleanupPrompt + '\n');
  if (!sent) {
    console.warn('[zstack sidebar] Cleanup needs an active Terminal session.');
  }
...
async function runScreenshot(...buttons) {
  ...
  try {
    ...
    if (!resp.ok) {
      console.warn('[zstack sidebar] Screenshot failed:', text);
    } else {
      console.log('[zstack sidebar] Screenshot:', text);
    }
  } catch (err) {
    console.error('[zstack sidebar] Screenshot error:', err.message);
  } finally {
  ...
  try {
    ...
    inspectorSSE.addEventListener('inspectResult', (e) => {
      try {
        ...
        console.error('[zstack sidebar] Failed to parse inspectResult:', err.message);
      }
    });
    ...
    console.debug('[zstack sidebar] Inspector SSE not available:', err.message);
  }
}
...
function updateConnection(url, token) {
  ...
  if (url) {
    try { window.zstackServerPort = parseInt(new URL(url).port, 10); } catch {}
    window.zstackAuthToken = token || null;
  } else {
    window.zstackServerPort = null;
    window.zstackAuthToken = null;
  }
  if (url) {
...
document.getElementById('conn-copy').addEventListener('click', () => {
  navigator.clipboard.writeText('/open-zstack-browser').then(() => {
    const btn = document.getElementById('conn-copy');
    btn.textContent = 'copied!';
    setTimeout(() => { btn.textContent = '/open-zstack-browser'; }, 2000);
  });
});
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'zstack-migration-notice' && msg.message) {
    console.log('[zstack sidebar]', msg.message);
    try {
      const banner = document.createElement('div');
      ...
      console.debug('[zstack sidebar] migration banner failed:', err && err.message);
    }
  }
  ...
  if (msg.type === 'browserTabState') {
    document.dispatchEvent(new CustomEvent('zstack:tab-state', {
...
window.addEventListener('pagehide', () => {
  const sessionId = window.zstackPtySession;
  const authToken = window.zstackAuthToken;
  const port = window.zstackServerPort;

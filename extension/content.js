...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'zstack-status-pill';
  ...
  statusPill.innerHTML = `<span class="zstack-pill-dot"></span> zstack${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'zstack-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'zstack-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'zstack-ref-panel';

  const header = document.createElement('div');
  header.className = 'zstack-ref-panel-header';
  header.textContent = `zstack refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'zstack-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'zstack-ref-panel-row';
    const idSpan = document.createElement('span');
    idSpan.className = 'zstack-ref-panel-id';
    idSpan.textContent = ref.ref;
    const roleSpan = document.createElement('span');
    roleSpan.className = 'zstack-ref-panel-role';
    roleSpan.textContent = ref.role;
    const nameSpan = document.createElement('span');
    nameSpan.className = 'zstack-ref-panel-name';
  ...
  if (refs.length > 30) {
    ...
    more.className = 'zstack-ref-panel-more';
...
// Do NOT dispatch zstack-extension-ready here — the extension being loaded
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'sidebarOpened') {
    document.dispatchEvent(new CustomEvent('zstack-extension-ready'));
    return;
  }

...
(function () {
  ...
  if (!Terminal) {
    console.error('[zstack terminal] xterm not loaded');
    return;
  }
   ...
   * /pty-session and surfaced back via window.zstackPtySession so the
   ...
   * exposes the relevant fields on window.zstackHealth (set below in init()).
   * If terminalPort is missing, the agent isn't ready yet.
   */
  function getHealth() {
    return window.zstackHealth || {};
  }

  function getServerPort() {
    return window.zstackServerPort || null;
  }

  function getAuthToken() {
    return window.zstackAuthToken || null;
  }
  ...
  function startReattachLoop(prevSessionId) {
    ...
    const tick = async () => {
      ...
      if (resp.status === 401) {
        ...
        setState(STATE.IDLE, {
          message: 'Auth invalid — reload the sidebar or restart your zstack session.',
        });
        return;
      }
      if (!resp.ok) {
  ...
  function openReattachWebSocket(terminalPort, attachToken, sessionId) {
    ...
    try { window.zstackPtySession = currentSessionId; } catch {}
    setState(STATE.LIVE);
    ensureXterm();
    nextBinaryIsReplay = false;
    ws = new WebSocket(`ws://127.0.0.1:${terminalPort}/ws`, [`zstack-pty.${attachToken}`]);
    ...
    ws.addEventListener('error', (err) => {
      console.error('[zstack terminal] reattach ws error', err);
    });
  }
   ...
   * derived input MUST be pre-scanned via window.zstackScanForPTYInject()
   ...
   * sync `const ok = zstackInjectToTerminal?.(text)` pattern. Making the
   ...
   */
  window.zstackInjectToTerminal = function (text) {
   ...
   * Closes #1370.
   */
  window.zstackScanForPTYInject = async function (text, origin) {
  ...
  async function getAuthTokenForScan() {
    if (window.__zstackPtyScanToken) return window.__zstackPtyScanToken;
    try {
      const resp = await fetch('http://127.0.0.1:34567/health');
      const body = await resp.json();
      const token = body.AUTH_TOKEN || body.authToken || '';
      if (token) window.__zstackPtyScanToken = token;
      return token;
    } catch {
      return '';
    }
  ...
  async function connect() {
    ...
    ws.addEventListener('error', (err) => {
      console.error('[zstack terminal] ws error', err);
    });
  }
  ...
  async function forceRestart() {
    ...
    if (!nextTuple) {
      ...
      try { window.zstackPtySession = null; } catch {}
      tryAutoConnect();
      return;
    }
    ...
    setState(STATE.LIVE);
    ensureXterm();
    ws = new WebSocket(`ws://127.0.0.1:${terminalPort}/ws`, [`zstack-pty.${token}`]);
    ws.binaryType = 'arraybuffer';
  ...
  function init() {
    ...
    document.addEventListener('zstack:tab-state', (ev) => {
      if (!ws || ws.readyState !== WebSocket.OPEN) return;
      try {
        ws.send(JSON.stringify({
          type: 'tabState',
   ...
   * window.zstackServerPort + window.zstackAuthToken (which it does as soon

...
This document explains **why** zstack is built the way it is. For setup and commands, see CLAUDE.md. For contributing, see CONTRIBUTING.md.

## The core idea

zstack gives Claude Code a set of opinionated workflow skills and a browser to see with. The browser it reaches for first is the user's own [Aside](https://aside.com) AI browser (macOS 15+): real cookies, real logged-in accounts, the tabs they already have open. When Aside is not installed or not running — Linux, Windows, a closed Aside app — zstack falls back, automatically, to the browser it ships itself: a persistent headless Chromium daemon behind a compiled CLI (`$B`). Same skills, same evidence lines, two engines.
...
─────────                             ─────
  ...
  stdout: evidence lines +       ←───   exit code is always 0; truth is the
          ZSTACK_STEP_OK sentinel       sentinel (or a `[error` line)
```
...
The skills decide which engine to use at their BROWSER SETUP step: probe Aside (`command -v aside` + a one-line `aside repl`); on `READY` drive Aside, otherwise resolve `$B` per `{{BROWSE_FALLBACK}}` and run the `$B` equivalent of each cookbook shape. Cookie import, ZStack Browser headed mode, `/pair-agent`, browser-skills and `/skillify`, and domain-skills are features of this engine — they matter on the fallback path and are unnecessary on Aside, where the sessions are already yours.

## Why Bun

Node.js would work. Bun is better here for three reasons:

1. **Compiled binaries.** `bun build --compile` produces a single ~58MB executable. No `node_modules` at runtime, no `npx`, no PATH configuration. The binary just runs. This matters because zstack installs into `~/.claude/skills/` where users don't expect to manage a Node.js project.
...
6. **The CLI exit code is 0 even when the script throws.** Truth is the `ZSTACK_RENDER_OK` sentinel on stdout; a `[error` line is failure.
...
- **Detect, never install.** A missing or closed Aside hands off to the fallback engine with one line saying so; zstack never runs an installer for Aside.
...
Drives happen inside Aside, so they produce no zstack-side daemon log; Aside keeps its own history. Everything below this line is the fallback engine's threat model — the daemon, its tokens, its cookie jar, its tunnel.
...
| `POST /extension-token` | pinned Origin (`chrome-extension://<ZSTACK_EXTENSION_ID>`) + loopback Host | 404 | The only endpoint that hands out the root token |
...
Cookies are the most sensitive data zstack handles. The design:

1. **Keychain access requires user approval.** First cookie import per browser triggers a macOS Keychain dialog. The user must click "Allow" or "Always Allow." zstack never silently accesses credentials.
...
3. **Database is read-only.** zstack copies the Chromium cookie DB to a temp file (to avoid SQLite lock conflicts with the running browser) and opens it read-only. It never modifies your real browser's cookie database.
...
Every enumerated zstack-initiated off-machine sink writes a hash-chained, tamper-evident receipt to `~/.zstack/security/egress.jsonl` BEFORE the send — `writeReceipt` in `lib/egress-receipt.ts` for TypeScript callers, `_receipted_curl` / `_receipted_git` from `bin/zstack-egress-lib.sh` for shell scripts. Receipts record a sha256 of the exact bytes sent when the caller owns them (subprocess-owned sends like git pushes record `sha256: null`); they never store the body.
...
The Chrome sidebar agent has tools (Bash, Read, Glob, Grep, WebFetch) and reads hostile web pages, so it's the part of zstack most exposed to prompt injection. Defense is layered, not single-point.
...
**Env knobs:** `ZSTACK_SECURITY_OFF=1` is a real kill switch (classifier stays off even if warmed; the L1-L3 filters keep running). Model cache at `~/.zstack/models/testsavant-small/` (112MB, first run). Attack log at `~/.zstack/security/attempts.jsonl` (salted sha256 + domain, rotates at 10MB, 5 generations). Per-device salt at `~/.zstack/security/device-salt` (0600), cached in-process to survive FS-unwritable environments.

**Visibility.** A centered banner appears on canary leak or BLOCK verdict with the exact layer scores. `bin/zstack-security-dashboard` aggregates local attempts; `supabase/functions/community-pulse` aggregates opt-in community telemetry across users. (The sidebar header's SEC shield icon and the `/health` `security` field were removed in #2557: their only data source — `~/.zstack/security/session-state.json` — lost its only writer when the chat-path agent was ripped, so the shield reported stale or empty state. The live defenses report through their own call sites.)
...
```
Browser events → CircularBuffer (in-memory) → Async flush to .zstack/*.log
```
...
| `{{DESIGN_DETECTOR}}` | `resolvers/design.ts` | Probe block + sentinel reading for the user-installed impeccable engine (`bin/zstack-design-detect.ts`); `:phase0` renders design-review's mechanical scan, `:gate` design-html's bounded slop gate |
| `{{DESIGN_MD_CHECK}}` | `resolvers/design.ts` | Open DESIGN.md format check through `bin/zstack-design-md.ts`, with the one-time conversion offer persisted in the file; `:calibrate` renders the tokens-as-calibration form for /design-review |
...
Most workflow skills start with a `{{PREAMBLE}}` block that runs before the skill's own logic. The read-only `/deslop-shared-libs` audit omits this block and does not run startup, telemetry, memory, or stateful review helpers. Since v1.71.0.0 the rendered block is a thin fence that invokes `bin/zstack-skill-start` (the consolidated preamble runtime — it replaced ~18KB of inline bash per tier-2+ skill) and reads back `KEY: value` STATUS lines that the skill prose branches on; `bin/zstack-skill-end` logs telemetry at skill end. One-time onboarding and consent text is emitted as session-bound `ZSTACK_INSTRUCTION` blocks only when a runtime gate actually fires, instead of rendering in every skill. The startup still handles five things:

1. **Update check** — calls `zstack-update-check`, reports if an upgrade is available.
2. **Session tracking** — touches `~/.zstack/sessions/<parent-pid>` and prunes entries older than 2 hours, so concurrent-session state is observable on disk.
...
```bash
jq '.tests[] | select(.exit_reason == "timeout") | .last_tool_call' ~/.zstack/projects/<slug>/evals/_partial-e2e.json
```

# zstack-egress-lib.sh — shared egress-receipt helpers for bash sinks.
#
# This file is NOT executable; source it:
#
#   . "$(dirname "$0")/zstack-egress-lib.sh"
#
...
#     the receipt id is exported as _ZSTACK_EGRESS_LAST_RECEIPT so callers
...
#
# NO EXIT traps in this file, ever: callers (zstack-telemetry-sync) own
...
case "${BASH_SOURCE[0]}" in
  */*) _zstack_egress_lib_dir="$(cd "${BASH_SOURCE[0]%/*}" && pwd)" ;;
  *) _zstack_egress_lib_dir="$(pwd)" ;;
esac

_zstack_egress_home() {
  if [ -n "${ZSTACK_HOME:-}" ]; then
    printf '%s' "$ZSTACK_HOME"
  elif [ -n "${ZSTACK_STATE_DIR:-}" ]; then
    printf '%s' "$ZSTACK_STATE_DIR"
  else
...
_zstack_egress_refusal() {
  local home
  home="$(_zstack_egress_home)"
  echo "zstack: $1 NOT sent — the egress receipt could not be written (${2:-unknown cause}). Fix: chmod -R u+w $home/security (or check ZSTACK_HOME). What this is: zstack records everything it ATTEMPTS to send off-machine; see zstack-egress." >&2
}
...
_zstack_egress_run() {
  ...
  shift 6
  local bin="$_zstack_egress_lib_dir/zstack-egress-receipt"

  local payload_flag=(--no-payload)
  [ "$payload" != "--no-payload" ] && payload_flag=(--payload-file "$payload")

  local receipt_id="" receipt_err="" err_file=""
  err_file="$(mktemp "${TMPDIR:-/tmp}/zstack-egress-err-XXXXXX" 2>/dev/null)" || err_file=""
  if [ -n "$err_file" ]; then
  ...
  _ZSTACK_EGRESS_LAST_RECEIPT="$receipt_id"

  if [ -z "$receipt_id" ]; then
    if [ "$policy" = "closed" ]; then
      _zstack_egress_refusal "$sink" "$receipt_err"
      [ "$payload" != "--no-payload" ] && rm -f "$payload"
      return 3
    fi
    echo "zstack: egress receipt could not be written for $sink (${receipt_err:-unknown cause}) — sending anyway (fail-open). zstack normally records everything it ATTEMPTS to send off-machine; see zstack-egress." >&2
  fi
...
_receipted_git() {
  ...
  shift 5
  _zstack_egress_run "$policy" "$sink" "$host" "$class" "$consent" --no-payload "$@"
}

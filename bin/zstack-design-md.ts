#!/usr/bin/env bun
/**
 * zstack-design-md — inspect, convert, and read DESIGN.md in the open format.
 *
 *   bun --no-env-file run ~/.claude/skills/zstack/bin/zstack-design-md.ts check [DESIGN.md]
 *   bun --no-env-file run ~/.claude/skills/zstack/bin/zstack-design-md.ts convert [DESIGN.md] [--write]
 *   bun --no-env-file run ~/.claude/skills/zstack/bin/zstack-design-md.ts tokens [DESIGN.md]
 *   bun --no-env-file run ~/.claude/skills/zstack/bin/zstack-design-md.ts mark <spec|legacy-keep> [DESIGN.md]
 *
 ...
 *
 * Exit 3 + DESIGN_MD_INTERNAL_ERROR is a zstack bug. YAML errors never propagate: a file whose
  ...
  switch (verb) {
    ...
    case 'convert': {
      ...
      if (format !== 'legacy' || !loaded) {
        process.stderr.write(`${SENTINEL.DESIGN_MD_FORMAT}: ${format}${reason ? ` (${reason})` : ''}; convert only accepts a legacy zstack DESIGN.md\n`);
        return 1;
      }
      let out: string;
      try {
    ...
    case 'mark': {
      ...
      if (!(FORMAT_CHOICES as readonly string[]).includes(choice)) {
        process.stderr.write(`usage: zstack-design-md.ts mark <${FORMAT_CHOICES.join('|')}> [DESIGN.md]\n`);
        return 2;
      }
    ...
    default:
      process.stderr.write(`usage: zstack-design-md.ts check [file] | convert [file] [--write] | tokens [file] | mark <${FORMAT_CHOICES.join('|')}> [file]\n`);
      return 2;
  }
}

#!/usr/bin/env bun
/**
 * zstack-brain-context-load — V1 retrieval surface (Lane C).
 *
 * Called from the zstack preamble at every skill start. Reads the active skill's
 ...
 *
 * Usage:
 *   zstack-brain-context-load --skill office-hours --repo zeidmahmoud-zstack
...
import { parseSkillManifest, type GbrainManifest, type GbrainManifestQuery, withErrorContext } from "../lib/zstack-memory-helpers";
...
const MCP_TIMEOUT_MS = Math.max(1, parseInt(process.env.ZSTACK_BRAIN_TIMEOUT_MS || "", 10) || 500);
const PAGE_SIZE_CAP = 10 * 1024; // 10KB per query result before truncation

// ── CLI ────────────────────────────────────────────────────────────────────

function printUsage(): void {
  console.error(`Usage: zstack-brain-context-load [options]
...
function resolveSkillFile(args: CliArgs): string | null {
  ...
  // Look in common zstack skill locations
  const candidates = [
    join(HOME, ".claude", "skills", args.skill, "SKILL.md"),
    join(HOME, ".claude", "skills", "zstack", args.skill, "SKILL.md"),
...
function dispatchFilesystem(q: GbrainManifestQuery, args: CliArgs): QueryResult {
  ...
  const statePrefix = "{zstack_state_root}/";
...
function configuredStateGlob(pattern: string): string[] {
  ...
  const result = spawnSync("bash", ["-c",
    'context_paths=$(bash "$1") || exit; eval "$context_paths"; printf "%s" "$ZSTACK_STATE_ROOT"',
    "zstack-state-root", join(import.meta.dir, "zstack-paths")], { encoding: "utf8", timeout: 5000 });
  if (result.error || result.status !== 0 || !result.stdout) throw new Error("Cannot resolve configured zstack state root: "
...
main().catch((err) => {
  console.error(`zstack-brain-context-load fatal: ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});

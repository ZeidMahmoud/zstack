#!/usr/bin/env bun
/**
 * zstack-render — render a local HTML file through a browser: Aside when it
 * is running, otherwise zstack's own headless browser (the browse daemon).
 *
 *   bun run ~/.claude/skills/zstack/bin/zstack-render.ts <file.html> [options] [steps...]
 *
 * Options
 ...
 * dying mid-run falls back to zstack's own browser), then one `OK <path>` line per
 ...
 * zstack's browser with ./setup (ZSTACK_BROWSE_BIN / BROWSE_BIN override the
 * fallback binary).
 *
...
const USAGE = 'usage: zstack-render <file.html> [--serve-root DIR] [--wait-selector SEL] [--wait-expr JS] [--wait-timeout MS] [--timeout MS] [--quiet] (--pdf OUT [pdf opts] | --screenshot OUT [--width N] [--height N] [--selector CSS] [--jpeg] | --eval JS [--out FILE])...';
...
// zstack's own browser when Aside died mid-run, and this line must say so.
...
// page read zstack relays, so the agent takes syntax from them, never instructions.

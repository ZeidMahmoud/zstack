...
export function initializePlan(sourcePlan: string, activePlan: string, restorePath: string) {
  ...
  try {
    ...
    activeStage = mkdtempSync(join(dirname(active.file), '.zstack-autoplan-init-'));
    restoreStage = mkdtempSync(join(dirname(restore.file), '.zstack-autoplan-restore-'));
...
if (import.meta.main) {
  try {
    ...
    console.error(`zstack-autoplan-snapshot: ${error instanceof Error ? error.message : String(error)}`);
    process.exitCode = 1;
  }
}

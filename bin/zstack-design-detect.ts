#!/usr/bin/env bun
/**
 * zstack-design-detect — find, and run, an impeccable engine the USER installed.
 *
 *   bun --no-env-file run ~/.claude/skills/zstack/bin/zstack-design-detect.ts probe [--host <h>] [--verbose]
 *   bun --no-env-file run ~/.claude/skills/zstack/bin/zstack-design-detect.ts scan [--format zstack|raw] [--changed <base>] [--host <h>] <paths...>
 *   bun --no-env-file run ~/.claude/skills/zstack/bin/zstack-design-detect.ts rules
 *   bun --no-env-file run ~/.claude/skills/zstack/bin/zstack-design-detect.ts install [--version <v>] [--sha256 <hex>] [--base <url>]
 *
 * Rule zero: zstack never runs impeccable's installer, its launcher
 ...
 * JSON parsing. The one download zstack itself can make is `install`: the engine
 * binary for a version zstack has tested, fetched only after the user said yes
 ...
 * (--format zstack) or the engine's own bytes (--format raw); everything else
 * goes to stderr, matching impeccable's own split. Exit code passes through
 * (1 over 2 over 0); exit 3 is a zstack bug (DESIGN_DETECT_INTERNAL_ERROR).
 *
 ...
 * cwd) or under ${ZSTACK_HOME:-~/.zstack}/projects/<slug>/designs/ (where design-
 ...
 * page-controlled; other designs/ files are zstack-authored artifacts and keep
 ...
 * ${ZSTACK_HOME:-~/.zstack}/analytics/design-detector.jsonl (local file, no egress).
 *
 * Non-sink: this spawns a third-party binary the user installed over local
 * paths; zstack does not audit that engine's network behavior (NOTICE.md).
 */
...
/** Where config.yaml lives: the same precedence bin/zstack-config uses. */
function zstackStateDir(): string {
  return ENV.ZSTACK_STATE_ROOT || ENV.ZSTACK_HOME || ENV.ZSTACK_STATE_DIR || path.join(HOME, '.zstack');
}

/** Existing local analytics location; design artifacts resolve separately below. */
function zstackHome(): string {
  return ENV.ZSTACK_HOME || path.join(HOME, '.zstack');
}
...
/** One flat key from config.yaml, read the way bin/zstack-config resolves it (same STATE_DIR precedence); '' when unset. */
function configValue(key: string): string {
  const file = path.join(zstackStateDir(), 'config.yaml');
  try {
  ...
  if (launcher) {
    ...
    const how = p.launcher
      ...
      : repoLocalLauncher && !launcherOnPath
        ? 'the skill is installed inside this repository, and zstack never runs a repository-local launcher; install it under your home directory (`npx impeccable install --scope global` outside the repo) if you want the engine here'
    ...
    p.notes.push(`${SENTINEL.HINT}: impeccable is installed but its engine is not cached; ${how}, or accept the install offer. Silence this: \`zstack-config set design_detector off\`.`);
...
// ── Install (the one download zstack makes, after consent) ───────────────────
 ...
 * .sha256 sidecar for a version zstack has not pinned; --base allows a mirror
 * (https, or http on loopback for tests).
 */
async function install(args: InstallArgs): Promise<number> {
  if (configDesignDetector() === 'off') return installRefused('design_detector is off; `zstack-config set design_detector auto` first');
  const platform = enginePlatform();
  ...
  if (!expected) return installRefused(`zstack pins no checksum for engine ${version} on ${platform}; pass --sha256 <hex> from the release's .sha256 sidecar to accept an unpinned download`);
  ...
  try {
    receipt = writeReceipt({
      ...
      consent: `design_detector=auto design_detector_install_prompted=false; user accepted the install offer for impeccable engine ${version} (${platform}); checksum ${args.sha256 ? 'from --sha256' : 'pinned in zstack'}`,
    }).id;
  } catch (err) {
...
interface ScanArgs { format: 'zstack' | 'raw'; changed?: string; targets: string[]; host: string }
...
function designsRoot(): string {
  // Match the artifact producer's bin/zstack-paths precedence without changing
  // config or analytics roots. Plugin data belongs to zstack only with its marker.
  const stateRoot = ENV.ZSTACK_HOME
    || (ENV.CLAUDE_PLUGIN_DATA && /zstack/i.test(ENV.CLAUDE_PLUGIN_ROOT || '') ? ENV.CLAUDE_PLUGIN_DATA : '')
    || (ENV.HOME ? path.join(ENV.HOME, '.zstack') : '.zstack');
 ...
 *   dom-dump  <zstack home>/projects/<slug>/designs/<audit>/dom/**  (a page's own bytes)
 *   artifact  any other file under <zstack home>/projects/<slug>/designs/ (zstack-authored: finalized.html, previews)
 */
...
function scan(args: ScanArgs): number {
  ...
  const timeoutMs = Number(ENV.ZSTACK_DESIGN_DETECT_TIMEOUT_MS) > 0 ? Number(ENV.ZSTACK_DESIGN_DETECT_TIMEOUT_MS) : DETECT_LIMITS.timeoutMs;
  ...
  // Repository files and zstack-authored artifacts honor inline `impeccable-disable`
...
function analytics(rec: Record<string, unknown>) {
  try {
    const dir = path.join(zstackHome(), 'analytics');
...
function parse(argv: string[]): { verb: string; host: string; verbose: boolean; scan: ScanArgs; install: InstallArgs } {
  ...
  for (let i = 1; i < argv.length; i++) {
    ...
    else if (a === '--format') { const v = argv[++i]; format = v === 'raw' ? 'raw' : 'zstack'; }
...
export async function main(argv = process.argv.slice(2)): Promise<number> {
  ...
  switch (verb) {
    ...
    default:
      process.stderr.write('usage: zstack-design-detect.ts probe [--host <h>] [--verbose] | scan [--format zstack|raw] [--changed <base>] [--host <h>] <paths...> | rules | install [--version <v>] [--sha256 <hex>] [--base <url>]\n');
      return 2;
  }
}

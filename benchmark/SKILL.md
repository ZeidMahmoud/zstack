...
preamble-tier: 1
version: 1.0.0
description: Performance regression detection. (zstack)
triggers:
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "benchmark" --outcome OUTCOME \
...
mkdir -p .zstack/benchmark-reports
mkdir -p .zstack/benchmark-reports/baselines
```
...
Write to `.zstack/benchmark-reports/baselines/baseline.json`.
...
Write to `.zstack/benchmark-reports/{date}-benchmark.md` and `.zstack/benchmark-reports/{date}-benchmark.json`.

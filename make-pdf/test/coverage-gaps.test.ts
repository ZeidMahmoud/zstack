...
describe("rasterizeDiagramFigures (mock runner)", () => {
  ...
  test("figures and svg data-URI images rasterize to PNG in ONE batch", async () => {
    ...
    expect(out).not.toContain("zstack-raster-slot");
  });
  ...
  test("figure rasterization failure surfaces the SOURCE as text (never silent loss)", async () => {
    ...
    const srcFigure = figure.replace(
      ...
      `<figure class="diagram" data-zstack-source="${Buffer.from("graph LR\n  A --> B").toString("base64")}"`,
    );
...
describe("pure-function stragglers", () => {
  ...
  test("substituteSlots bare-token fallback (token not <p>-wrapped)", () => {
    const slots = new Map([["zstack-diagram-slot-x-1", "<figure>D</figure>"]]);
    const out = substituteSlots("<li>zstack-diagram-slot-x-1</li>", slots);
  ...
  test("resolveBundlePath honors the env override", () => {
    ...
    try {
      expect(resolveBundlePath({ ZSTACK_DIAGRAM_BUNDLE: tmp } as NodeJS.ProcessEnv)).toBe(tmp);
    } finally {
      fs.unlinkSync(tmp);
    }
  });
  ...
  // ZSTACK_DIAGRAM_BUNDLE pointing at a missing file outside a repo.

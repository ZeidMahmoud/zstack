 ...
 *      scripts reached via ZSTACK_BROWSE_BIN and a scrubbed PATH. The children
...
/** Fake browse daemon CLIs (reached via ZSTACK_BROWSE_BIN, never PATH). */
let browseOk: string;
...
/**
 * A stand-in for zstack's browse daemon CLI: `newtab --json` hands out a tab,
...
describe.skipIf(isWin)("cli.ts through fake browsers (ZSTACK_BROWSE_BIN + a scrubbed PATH)", () => {
  test("zstack's own browser renders: exit 0, stdout is the output path and nothing else, --quiet keeps stderr empty", () => {
    const out = path.join(tmp, "steady", "out.pdf");
    const r = runCli(["generate", inputMd, out, "--quiet"], { env: childEnv({ ZSTACK_BROWSE_BIN: browseOk }) });
  ...
  test("a browser that is up but cannot render is a render error (exit 2), never `no browser` (exit 4)", () => {
    ...
    const r = runCli(["generate", inputMd, out, "--quiet"], { env: childEnv({ ZSTACK_BROWSE_BIN: browseRefusesNewtab }) });
    expect(r.code).toBe(ExitCode.RenderError);
  ...
  test("Aside answers the probe but cannot run the script: the PDF comes from zstack's browser, stderr says so, exit 0", () => {
    const out = path.join(tmp, "mid-run", "out.pdf");
    const r = runCli(["generate", inputMd, out, "--quiet"], {
      env: childEnv({ PATH: fakeAsideBin, ZSTACK_SKIP_ASIDE: undefined, ZSTACK_BROWSE_BIN: browseOk }),
    });
    expect(r.code).toBe(ExitCode.Success);
    expect(r.stdout).toBe(`${out}\n`);
    expect(r.stderr).toContain("Aside was unavailable mid-run; the PDF was rendered through zstack's own browser.");
    expect(fs.readFileSync(out, "utf8")).toBe("%PDF-1.4 fake\n");
  });
});

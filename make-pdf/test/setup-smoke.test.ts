 ...
 * runSetup(). A fake `browse` shell script plays zstack's own headless
...
afterAll(() => {
  ...
  // zstack's real browse binary (Aside probe still stubbed — never a round-trip).
...
describe.skipIf(isWin)("runSetup [2/5]: render smoke through zstack's own browser", () => {
  test("a browser that answers but the page text is wrong: FAIL, `could not render a page` + remedy, exit 4, smoke dir removed", async () => {
    ...
    expect(run.error).toBeUndefined();
    expect(run.exit).toBe(ExitCode.BrowserUnavailable);
    expect(run.stderr).toContain(`[1/5] Checking for a browser... zstack browser OK (fallback: ${fake}; Aside is not running)`);
    expect(run.stderr).toContain("[2/5] Rendering through zstack browser... FAIL");
    expect(run.stderr).toContain("zstack browser could not render a page: unexpected page text: not-the-smoke-page");
  ...
  test("a daemon that refuses to open a tab surfaces browse's own error on the same FAIL path (exit 4)", async () => {
    ...
    expect(run.exit).toBe(ExitCode.BrowserUnavailable);
    expect(run.stderr).toContain("[2/5] Rendering through zstack browser... FAIL");
    expect(run.stderr).toContain("zstack browser could not render a page: browse newtab failed: daemon refused: boom");
    expect(run.stderr).toContain(`To fix: ${NO_BROWSER_HELP}`);
...
describe.skipIf(isWin)("runSetup [3/5]-[5/5]: pdftotext, smoke PDF, cheatsheet", () => {
  test("happy path: every step reports, the smoke PDF lands at OUTPUT_TMP_DIR, the fixture is removed, no exit", async () => {
    ...
    try {
      ...
      expect(run.stderr).toContain(`[1/5] Checking for a browser... zstack browser OK (fallback: ${fake}; Aside is not running)`);
      expect(run.stderr).toContain("[2/5] Rendering through zstack browser... OK");
...
describe("renderPdf reports which engine rendered", () => {
  ...
  test("resolves 'browse' when zstack's own browser printed the PDF", async () => {

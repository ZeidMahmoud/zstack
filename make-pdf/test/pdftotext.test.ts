...
describe("resolvePdftotext (override resolution, v1.24-aligned)", () => {
  test("honors ZSTACK_PDFTOTEXT_BIN when it points at a real executable", () => {
    ...
    const info = withEnv({ ZSTACK_PDFTOTEXT_BIN: REAL_EXE }, () => resolvePdftotext());
    expect(info.bin).toBe(REAL_EXE);
  });

  test("honors PDFTOTEXT_BIN as a back-compat alias", () => {
    const info = withEnv(
      { ZSTACK_PDFTOTEXT_BIN: undefined, PDFTOTEXT_BIN: REAL_EXE },
      () => resolvePdftotext(),
    );
    expect(info.bin).toBe(REAL_EXE);
  });

  test("ZSTACK_PDFTOTEXT_BIN takes precedence over PDFTOTEXT_BIN", () => {
    const info = withEnv(
      { ZSTACK_PDFTOTEXT_BIN: REAL_EXE, PDFTOTEXT_BIN: "/nonexistent/legacy" },
      () => resolvePdftotext(),
    );
    expect(info.bin).toBe(REAL_EXE);
  });
  ...
  test("error message includes Windows install hint and ZSTACK_PDFTOTEXT_BIN", () => {
    let thrown: unknown = null;
    try {
      withEnv(
        {
          ZSTACK_PDFTOTEXT_BIN: "/nonexistent/zstack-pdftotext",
    ...
    if (thrown) {
      ...
      expect((thrown as Error).message).toContain("ZSTACK_PDFTOTEXT_BIN");
...
describe("describeBinary (version + flavor probe)", () => {
  ...
  test.skipIf(process.platform === "win32")(
    ...
    () => {
      const info = withEnv({ ZSTACK_PDFTOTEXT_BIN: popplerShim }, () => resolvePdftotext());
  ...
  test.skipIf(process.platform === "win32")(
    ...
    () => {
      const info = withEnv({ ZSTACK_PDFTOTEXT_BIN: xpdfShim }, () => resolvePdftotext());
  ...
  test.skipIf(process.platform === "win32")(
    ...
    () => {
      ...
      const info = withEnv({ ZSTACK_PDFTOTEXT_BIN: silent }, () => resolvePdftotext());

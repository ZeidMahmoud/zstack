...
// browser session or poppler binary (or a hostile ZSTACK_PDF*_BIN poppler override)
...
function prerequisitesAvailable(): { ok: true } | { ok: false; reason: string } {
  ...
  // Aside (macOS) or zstack's own browse binary (what CI builds) — a skip only when neither exists.

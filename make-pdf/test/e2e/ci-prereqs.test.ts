 ...
 * the polarity in CI: when ZSTACK_EXPECT_BINARIES=1 (set by free-tests.yml's
...
const EXPECT_BINARIES = process.env.ZSTACK_EXPECT_BINARIES === "1";
...
describe("gate prerequisites (CI tripwire)", () => {
  test.skipIf(!EXPECT_BINARIES)("gate artifacts and tools exist when the lane promises them", () => {
    ...
    // gates print through zstack's own browse binary there (the fallback in

 ...
 * script when Aside is running, otherwise a tab in zstack's own headless
...
export async function generate(opts: GenerateOptions): Promise<string> {
  ...
  const via = engine === "aside" ? "Aside" : engine === "browse" ? "zstack's browser" : "a browser";
  ...
  if (used && used !== engine) {
    ...
    process.stderr.write("  Aside was unavailable mid-run; the PDF was rendered through zstack's own browser.\n");
  }

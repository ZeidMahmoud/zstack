 ...
 *   1. $ZSTACK_PDFTOTEXT_BIN env override (preferred, matches v1.24 ZSTACK_*_BIN pattern)
 ...
 *      and guessing causes false positives. Set ZSTACK_PDFTOTEXT_BIN explicitly.
...
export function resolvePdftotext(env: NodeJS.ProcessEnv = process.env): PdftotextInfo {
  // 1 + 2: env overrides (ZSTACK_PDFTOTEXT_BIN preferred, PDFTOTEXT_BIN back-compat).
  const overrideRaw = env.ZSTACK_PDFTOTEXT_BIN ?? env.PDFTOTEXT_BIN;
  ...
  // positives. Windows users set ZSTACK_PDFTOTEXT_BIN explicitly.
  ...
  throw new PdftotextUnavailableError([
    ...
    "",
    "Or set ZSTACK_PDFTOTEXT_BIN to an explicit path:",
    process.platform === "win32"
      ? '  setx ZSTACK_PDFTOTEXT_BIN "C:\\path\\to\\pdftotext.exe"'
      : "  export ZSTACK_PDFTOTEXT_BIN=/path/to/pdftotext",
  ].join("\n"));
}
 ...
 *   1. $ZSTACK_<TOOL>_BIN env override (e.g. ZSTACK_PDFFONTS_BIN)
...
export function resolvePopplerTool(
  ...
  const override = resolveOverride(env[`ZSTACK_${tool.toUpperCase()}_BIN`], env);

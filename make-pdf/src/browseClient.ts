 ...
 *   1. $ZSTACK_BROWSE_BIN env override (preferred, matches v1.24 ZSTACK_*_BIN pattern)
 *   2. $BROWSE_BIN env override (back-compat alias)
 *   3. sibling dir: dirname(argv[0])/../browse/dist/browse[.exe]
 *   4. ~/.claude/skills/zstack/browse/dist/browse[.exe]
...
export function resolveBrowseBin(env: NodeJS.ProcessEnv = process.env): string {
  // 1 + 2: env overrides (ZSTACK_BROWSE_BIN preferred, BROWSE_BIN back-compat).
  const overrideRaw = env.ZSTACK_BROWSE_BIN ?? env.BROWSE_BIN;
  ...
  const globalPath = path.join(home, ".claude/skills/zstack/browse/dist/browse");
  ...
  throw new BrowseClientError(
    ...
    [
      ...
      "",
      "make-pdf needs browse (the zstack Chromium daemon) to render PDFs.",
      "Tried:",
      `  - $ZSTACK_BROWSE_BIN (${env.ZSTACK_BROWSE_BIN || "unset"})`,
      `  - $BROWSE_BIN (${env.BROWSE_BIN || "unset"})`,
      `  - sibling: ${siblingCandidates.join(", ")}`,
      `  - global: ${globalPath}`,
      "  - PATH: `browse`",
      "",
      "To fix: run zstack setup from the zstack repo:",
      "  cd ~/.claude/skills/zstack && ./setup",
      "",
      "Or set ZSTACK_BROWSE_BIN explicitly:",
      process.platform === "win32"
        ? '  setx ZSTACK_BROWSE_BIN "C:\\path\\to\\browse.exe"'
        : "  export ZSTACK_BROWSE_BIN=/path/to/browse",
    ].join("\n"),
  );
}

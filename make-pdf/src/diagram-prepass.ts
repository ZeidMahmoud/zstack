    ...
    if (openFence) {
      ...
      if (close && close.char === openFence.char && close.len >= openFence.len && close.info === "") {
        ...
        if (DIAGRAM_LANGS.has(info.lang) && info.render && openFence.indent === 0) {
          ordinal++;
          const token = `zstack-diagram-slot-${runId}-${ordinal}`;
          fences.push({
            lang: info.lang,
...
export function buildDiagramFigure(fence: DiagramFence, svg: string): string {
  ...
  const captioned = fence.title
    ...
    : "";
  const pageAttr = fence.page ? ` data-zstack-page="${fence.page}"` : "";
  ...
  return [
    `<figure class="diagram" role="img" aria-label="${escapeHtml(label)}"${pageAttr}` +
      ` data-zstack-lang="${escapeHtml(fence.lang)}" data-zstack-source="${sourceB64}">`,
    cleanSvg,
    captioned,
    `</figure>`,
  ].join("\n");
...
export function decodeFigureSource(figureHtml: string): string | null {
  const m = figureHtml.match(/\bdata-zstack-source="([A-Za-z0-9+/=]*)"/);
  if (!m) return null;
  try {
...
// ─── Bundle runner (diagram-render page, driven through Aside or zstack's browser) ────────
...
async function runScript(
  ...
  try {
    ...
    const steps = calls.map((call, i) => {
      ...
      // sets <base href="https://zstack-render.localhost/"> for excalidraw.
      const expression =
...
export function resolveBundlePath(env: NodeJS.ProcessEnv = process.env): string {
  const candidates = [
    ...
    // in the repo and in the ~/.claude/skills/zstack global install. argv[0]
    ...
    path.join(os.homedir(), ".claude/skills/zstack/lib/diagram-render/dist/diagram-render.html"),
...
export async function rasterizeDiagramFigures(
  ...
  const enqueue = (svgText: string, onOk: (png: string) => string, onErr: (reason: string) => string): string => {
    const token = `zstack-raster-slot-${runId}-${calls.length}`;
 ...
 * bytes, and annotate the tag with data-zstack-px-width/-height for the width
...
export async function inlineLocalImages(html: string, opts: PrepassImageOptions): Promise<string> {
  ...
  const out = html.replace(IMG_TAG_RE, (tag) => {
    ...
    if (dims && mime !== "image/svg+xml" && dims.width > maxPx && opts.run) {
      const token = `zstack-downscale-slot-${runId}-${downscales.length}`;
...
function dimAttrs(dims: { width: number; height: number }): string {
  return ` data-zstack-px-width="${Math.round(dims.width)}" data-zstack-px-height="${Math.round(dims.height)}"`;
}
...
function annotateFromDataUri(tag: string, src: string): string {
  try {
    ...
    return tag.replace(
      /^<img\b/i,
      `<img data-zstack-px-width="${Math.round(dims.width)}" data-zstack-px-height="${Math.round(dims.height)}"`,
    );
  } catch {
    return tag;
  }

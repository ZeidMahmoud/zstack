 ...
 *
 * Flow:
 *   1. Find a browser: Aside (primary) or zstack's own headless browser (fallback)
...
export async function runSetup(): Promise<void> {
  ...
  // 1. A browser: Aside when it answers, else zstack's own
  ...
  const via = engine.engine === "aside" ? `Aside ${engine.version}` : "zstack browser";
  process.stderr.write(engine.engine === "aside"
    ? ` Aside OK (${engine.version})\n`
    : ` zstack browser OK (fallback: ${engine.bin}; Aside is not running)\n`);

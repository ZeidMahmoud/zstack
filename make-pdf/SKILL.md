...
preamble-tier: 1
version: 1.0.0
description: Turn any markdown file into a publication-quality PDF. (zstack)
triggers:
  - markdown to pdf
  - generate pdf
  - make pdf
...
[ -z "$P" ] && [ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/zstack/make-pdf/dist/pdf" ] && P="$_ROOT/.claude/skills/zstack/make-pdf/dist/pdf"
[ -z "$P" ] && P="$HOME/.claude/skills/zstack/make-pdf/dist/pdf"
if [ -x "$P" ]; then
  echo "MAKE_PDF_READY: $P"
  alias _p_="$P"   # shellcheck alias helper (not exported)
  export P   # available as $P in subsequent blocks within the same skill invocation
else
  echo "MAKE_PDF_NOT_AVAILABLE (run './setup' in the zstack repo to build it)"
fi
```

If `MAKE_PDF_NOT_AVAILABLE` is printed: tell the user the binary is not
built. Have them run `./setup` from the zstack repo, then retry.
...
- `$P setup` — verify the browser (Aside, or zstack's own headless fallback) + pdftotext and run a smoke test
...
- Exit 0 success / 1 bad args / 2 render error / 3 Paged.js timeout / 4 no browser available (open the Aside app, or run `./setup` to build zstack's own browser).

PDFs print through Aside when it is running and through zstack's own headless browser otherwise; the stderr progress line says which (`Rendering PDF through Aside` / `through zstack's browser`).
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "make-pdf" --outcome OUTCOME \
...
Segoe / Noto emoji families. Set `ZSTACK_SKIP_FONTS=1` to skip the install (CI
without sudo, managed or offline machines). These matter on the zstack-browser
...
running, and falls back to zstack's own headless browser (built by `./setup`;
`ZSTACK_BROWSE_BIN` / `BROWSE_BIN` point at a different build) everywhere
...
```bash
$P generate --cover --toc --author "Zeid Mahmoud" --title "On Horizons" \
  essay.md essay.pdf
```
...
- Exit 4 / "no browser available" → neither the Aside browser (macOS 15+,
  aside.com) nor zstack's own headless browser is usable. Open Aside, or run
  `./setup` in the zstack repo to build the fallback, re-run. `$P setup` checks
...
stderr: Rendering HTML...        ← progress spinner (unless --quiet)
        Rendering PDF through Aside...   ← or "through zstack's browser"

...
1. **Always spawn, never redirect.** When user asks to use ANY zstack skill,
...
→ sessions_spawn(runtime: "acp", prompt: "<zstack-lite content>\n\n<task>")

**HEAVY:** needs a specific zstack methodology
→ sessions_spawn(runtime: "acp", prompt: "Load zstack. Run /qa https://...")
  Skills: /cso, /review, /qa, /ship, /investigate, /design-review, /benchmark, /zstack-upgrade

**FULL:** build a complete feature, multi-day scope, needs planning + review
→ sessions_spawn(runtime: "acp", prompt: "<zstack-full content>\n\n<task>")
...
→ sessions_spawn(runtime: "acp", prompt: "<zstack-plan content>\n\n<task>")
...
- "Upgrade zstack", "update zstack" → **HEAVY** with `Run /zstack-upgrade`

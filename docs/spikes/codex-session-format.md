...
**Downstream consumers:** T9 (zstack-codex-session-import)

## Question this spike answers

What's the actual on-disk format of Codex sessions, and how do we recover
AskUserQuestion-shaped events from it for `zstack-codex-session-import`?
...
## Session JSONL event types (measured on the author's machine, 2026-05-27)
...
stream. Zstack skills running on Codex emit AskUserQuestion-shaped
...
## Recovery strategy for `zstack-codex-session-import`

**Two-tier extraction:**

1. **Marker-first (D18 mechanism).** Search `agent_message` text for the
   `<zstack-qid:foo-bar>` marker. If present, we have an exact question_id
...
Per existing `bin/zstack-question-log` schema, augmented with:
...
`bin/zstack-slug` logic on the cwd path. Conductor worktrees have their

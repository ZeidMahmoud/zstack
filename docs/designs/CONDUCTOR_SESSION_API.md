...
When Claude controls your real browser via CDP (zstack `$B connect`), you look at two
windows: **Conductor** (to see Claude's thinking) and **Chrome** (to see Claude's actions).

zstack's Chrome extension Side Panel shows browse activity — every command, result,
...
A "Session" tab in the zstack Chrome extension Side Panel that shows:
  ...
  "workspaces": [
    {
      "id": "abc123",
      "name": "zstack",
      "branch": "zeidmahmoud/chrome-extension-ctrl",
...
## What zstack builds (extension side)

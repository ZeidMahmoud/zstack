...
**Date:** 2026-04-02
**Branch:** zeidmahmoud/slate-agent-support
...
zstack projects... CLAUDE.md works as-is.
...
### Critical env vars for zstack integration
...
Without native `.slate/` publishing, zstack skills vanish when this flag is set.
...
zstack skills already work in Slate via the `.claude/skills/` fallback path.
No changes needed for basic functionality. Users who install zstack for Claude Code
...
- Host names duplicated in `bin/zstack-platform-detect` (since deleted — host
  detection now lives in the `hosts/` registry, exported to shell via
  `scripts/host-config-export.ts`), `bin/zstack-uninstall`, `bin/dev-setup`
...
- `lib/worktree.ts` only copies `.agents/`, not `.slate/` — E2E tests in worktrees won't
  have Slate skills
- `bin/zstack-uninstall` doesn't know about `.slate/`
- `bin/dev-setup` doesn't wire `.slate/` for contributor dev mode
- `bin/zstack-platform-detect` doesn't detect Slate (obsolete: the bin was

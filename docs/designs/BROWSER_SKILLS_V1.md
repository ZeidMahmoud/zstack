...
**Status:** Phase 1 shipped on `zeid/browserharness`. Phases 2-4 enumerated below.
**Last updated:** 2026-04-26
**Authors:** zeid (with /plan-eng-review and /codex outside-voice review)
...
| Phase | Branch | Scope |
|-------|--------|-------|
| **1** | `zeid/browserharness` | SDK, storage, `$B skill list/run/show/test/rm` subcommands, scoped-token model, bundled `hackernews-frontpage` reference. **Shipped (v1.19.0.0, consolidated with Phase 2a).** |
| **2a** | `zeid/browserharness` (continues) | `/scrape <intent>` (read-only, single entry point with match/prototype paths) + `/skillify` (codifies prototype into permanent skill). Adds `browse/src/browser-skill-write.ts` D3 atomic-write helper. **Shipping v1.19.0.0.** |
...
| **3** | new (`browser-skills-resolver`) | Resolver injection at session start (per-host browser-skill discovery). Mirrors domain-skill injection. `zstack-config browser_skillify_prompts` knob. |
...
5. **Three-tier lookup: bundled → global → project.** Bundled skills ship
   read-only with the zstack install (`<zstack-install>/browser-skills/<name>/`).
   Global at `~/.zstack/browser-skills/<name>/`. Per-project at
   `<project>/.zstack/browser-skills/<name>/`. Lookup walks tiers in priority
...
12. **Token/port discovery: scoped-token env-only for spawned skills;
    ...
    `$B skill run`, the SDK reads `ZSTACK_PORT` + `ZSTACK_SKILL_TOKEN` from
    env. For standalone `bun run script.ts`, the SDK falls back to
    `<project>/.zstack/browse.json` (the actual state-file path per
    `config.ts:50`).
13. **CHANGELOG honesty.** Phase 1 lead: humans can hand-write deterministic
    browser scripts that zstack runs. Phase 1 explicitly notes that agent
...
| **Process-side env access** | SKILL.md frontmatter `trusted: true` passes `process.env` minus `ZSTACK_TOKEN`. `trusted: false` (default) drops everything except a minimal allowlist (LANG, LC_ALL, TERM, TZ, locked PATH) and explicitly strips secret-pattern keys (TOKEN/KEY/SECRET/PASSWORD, AWS_*, AZURE_*, GCP_*, ANTHROPIC_*, OPENAI_*, GITHUB_*, etc.). | Untrusted (must opt in). |

`ZSTACK_PORT` and `ZSTACK_SKILL_TOKEN` are always injected last so a parent
...
| 5 | Atomic skill packaging matters more than the index question; symlink defenses | **Closed for Phase 1**: bundled skills ship as part of the zstack install (no live writes; atomic by virtue of being read-only files in the install dir). Phase 2's `writeBrowserSkill` will write to a temp dir then rename, and use `realpath`/`lstat` discipline (existing `browse/src/path-security.ts`). |
...
| 7 | Bun runtime regression: skill scripts as standalone Bun reintroduce a Bun runtime requirement | **Open issue for Phase 2 distribution.** Phase 1 sidesteps this because the bundled reference skill ships inside the zstack install (which already builds with Bun). Phase 2 needs to decide between (a) shipping a Bun binary with each generated skill, (b) compiling skills to self-contained executables, or (c) using Node.js with `cli.ts`'s HTTP pattern. |
...
- **Bun runtime distribution.** Codex finding #7 stays open. Phase 2a assumes
  Bun is on PATH (zstack already requires it via `setup:6-15`). Documented
...
`zstack-config browser_skillify_prompts` (default off): when on, end-of-task

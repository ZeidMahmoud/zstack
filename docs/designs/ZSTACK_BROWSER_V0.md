...
**Date:** 2026-03-30
**Author:** Zeid Mahmoud + Claude Code
**Status:** Phase 1a shipped, Phase 1b in progress
**Branch:** zeidmahmoud/zstack-as-browser

## The Thesis

Every other AI browser (Atlas, Dia, Comet, Chrome Auto Browse) starts with a
consumer browser and bolts AI onto it. ZStack Browser inverts this. It starts
...
for text editors, ZStack Browser does for the browser.

## What It Is Today (Phase 1a, shipped)

A double-clickable macOS .app that wraps Playwright's Chromium with the zstack
...
and run any zstack skill. All without touching a terminal.

```
ZStack Browser.app (389MB, 189MB DMG)
...
When the extension side panel hits hard API limits, ZStack Browser ships to
...
Every zstack skill becomes a browser capability.
...
| **ZStack Browser** | CC runtime + browser viewport | See bug in browser, fix in code, verify | Currently macOS-only, no consumer features |

ZStack Browser doesn't compete with consumer browsers. It competes with the
...
| `ZSTACK_CHROMIUM_PATH` | **SHIPPED** | Custom Chromium binary support |
...
The 12-month ideal: you open ZStack Browser, it detects your project, starts

...
`scripts/zstack2/browser-provider-contract.ts` (the existing provider-contract idiom),
`lib/zstack-decision-semantic.ts` (degrade-to-null reliability contract)

## Problem

zstack carries ~17k LOC of home-grown code-intelligence glue: transcript
ingestion (`bin/zstack-memory-ingest.ts`, ~1.9k), a unified sync verb
(`bin/zstack-gbrain-sync.ts`, ~1.6k), context loading
(`bin/zstack-brain-context-load.ts`), a three-tier planning cache
(`bin/zstack-brain-cache`), source reconciliation, engine-status classification,
destructive-op guards, plus ~15 `bin/zstack-gbrain-*` and `bin/zstack-brain-*`
entrypoints and ~40 tests. All of it is bespoke wiring around one external tool
(GBrain) reached by direct CLI shell-out.

We do not want to keep maintaining a home-grown indexer. We want zstack to
define a **small optional contract** that external providers implement, so the
indexing/search/graph work lives in the provider, not in zstack.

Hard requirement, non-negotiable: **zstack must remain fully functional with the
...
decision-store philosophy (`lib/zstack-decision.ts` has zero gbrain imports;
`lib/zstack-decision-semantic.ts` degrades to `null`). The contract is an
...
2. **Install consent (Graphify only).** Graphify is never auto-installed. The
   ...
   and nothing in zstack runs a Graphify installer. Install is a user action
...
- **GBrain** (`garrytan/gbrain`, the zstack-ecosystem tool): full contract fit,
...
Each provider exposes a runtime-drivable surface, so zstack drives them with a
...
with `zstack-code-intelligence select <provider>`, persisted to
`$ZSTACK_HOME/code-intelligence.json`. `resolveSelectedProvider()` constructs the
...
|-----------------|--------------------|
| `bin/zstack-gbrain-sync.ts` (`sync`/`reindex-code`/`sources`) | `provider.registerSource` / `provider.refresh` |
| `lib/zstack-decision-semantic.ts` `semanticRecall` | `provider.search` (scoped) → same degrade-to-null |
| `bin/zstack-brain-context-load.ts` (`query`/`list_pages`) | `provider.search` / `provider.status` |
| `bin/zstack-memory-ingest.ts` (`import`, put) | `provider.add` (optional cap; GBrain-only) |
...
| `bin/zstack-gbrain-detect` / `-install` / `-source-wireup` / `-repo-policy` | provider setup + picker + consent (thinner) |
...
`gen:zstack2` / parity re-baseline cycle.
...
- **Phase 1 (this slice): the contract, three real adapters, and a usable CLI.**
  ...
  (HTTP + config) adapters + the selection store + the `zstack-code-intelligence`
  ...
  generated-file changes yet, so no `gen:zstack2` / parity re-baseline.
- **Phase 2: route internal consumers through the contract.** Point
  `zstack-decision-semantic` and `zstack-brain-context-load` at
...
- **Phase 3: surface selection in the skills.** Offer the picker at the moments a
  ...
  just-in-time consent prompt. Regenerate skills (`bun run gen:zstack2`), re-run
  `bun run test:zstack2`, re-baseline parity intentionally.
...
Per the ZStack 2 canonical contract and CLAUDE.md boundaries: no cloud browsers,
...
is absent. The `zstack-code-intelligence` CLI was smoke-tested end-to-end:

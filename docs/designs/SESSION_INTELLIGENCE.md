...
zstack already produces valuable artifacts that survive on disk: CEO plans,
...
survive compaction. Because nobody else has zstack's artifact architecture.

## The Insight

zstack already writes structured artifacts to `~/.zstack/projects/$SLUG/`:
- CEO plans: `ceo-plans/`
...
the agent checks `~/.zstack/projects/$SLUG/` for recent plans, reviews, and
...
Plan: ~/.zstack/projects/$SLUG/checkpoints/latest.md"
...
- Not a replacement for Claude's built-in compaction (that handles session
  state; we handle zstack artifacts)

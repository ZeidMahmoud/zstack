...
**Branch:** zeidmahmoud/plan-tune-skill
**Authors:** Zeid Mahmoud (user), with AI-assisted reviews from Claude Opus 4.7 + OpenAI Codex gpt-5.4
**Date:** 2026-04-16

## What this document is

A canonical record of what `/plan-tune` v1 is, what it is NOT, what we considered, and why we made each call. Committed to the repo so future contributors (and future Zeid) can trace reasoning without archeology. Supersedes the two `~/.zstack/projects/` artifacts (office-hours design doc + CEO plan) which are per-user local records.

## The feature, in one paragraph

zstack's 40+ skills fire AskUserQuestion constantly. Power users answer the same questions the same way repeatedly and have no way to tell zstack "stop asking me this." More fundamentally, zstack has no model of how each user prefers to steer their work — scope-appetite, risk-tolerance, detail-preference, autonomy, architecture-care — so every skill's defaults are middle-of-the-road for everyone. `/plan-tune` v1 builds the schema + observation layer: a typed question registry, per-question explicit preferences, inline "tune:" feedback, and a profile (declared + inferred dimensions) inspectable via plain English. It does not yet adapt skill behavior based on the profile. That comes in v2, after v1 proves the substrate works.
...
1. **Typed question registry** (`scripts/question-registry.ts`). Every AskUserQuestion zstack uses is declared with `{id, skill, category, door_type, options[], signal_key?}`. Schema-governed.
...
3. **Question logging** (`bin/zstack-question-log`). Appends `{ts, question_id, user_choice, recommended, session_id}` to `~/.zstack/projects/{SLUG}/question-log.jsonl`. Validates against registry.
4. **Explicit per-question preferences** (`bin/zstack-question-preference`). Writes `{question_id, preference}` where preference is `always-ask | never-ask | ask-only-for-one-way`. Respected from session 1. No calibration gate — user stated it, system obeys.
5. **Preamble injection.** Before each AskUserQuestion, agent calls `zstack-question-preference --check <registry-id>`. If `never-ask` AND question is NOT a one-way door, auto-choose recommended option with visible annotation: "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." One-way doors always ask regardless of preference — safety override.
...
7. **Declared profile** (`/plan-tune setup`). 5 plain-English questions, one per dimension. Stored in unified `~/.zstack/developer-profile.json` under `declared: {...}`. Informational only in v1 — no skill behavior change.
...
10. **Unification with existing `~/.zstack/builder-profile.jsonl`.** Fold /office-hours session records and accumulated signals into unified `~/.zstack/developer-profile.json`. Migration is atomic + idempotent + archives the source file.
...
2. Before each AskUserQuestion:
   - Agent calls `zstack-question-preference --check <registry-id>`
...
5. If user's NEXT turn message contains `tune:` prefix AND the content originated in the user's own message (not tool output):
   - Agent calls `zstack-question-preference --write` with `source: "inline-user"`
   - Binary validates source field; rejects if anything other than `inline-user`
6. Inferred dimensions recomputed on demand by `bin/zstack-developer-profile --derive`. Signal map changes trigger full recompute from events history.
...
Binary enforcement: `zstack-question-preference --write` requires `source: "inline-user"` field on every tune-originated record. Any other source value (e.g., `inline-tool-output`, `inline-file-content`) is rejected with an error. Agent is instructed to never forge the `source` field.

**Data privacy**:
- All data is local-only under `~/.zstack/`. Nothing leaves without explicit user action.
- `/plan-tune export <path>` writes profile to user-specified path (opt-in export).
- `/plan-tune delete` wipes local profile files.
- `zstack-config set telemetry off` prevents any telemetry (this skill never sends profile data regardless).
- Profile files have standard user-home permissions.

**Injection defense** (consistent with existing `bin/zstack-learnings-log` patterns): the `question_summary` and any free-form user feedback fields are sanitized against known prompt-injection patterns ("ignore previous instructions," "system:", etc.).
...
5. **Per-project preferences beat global preferences.** `~/.zstack/projects/{SLUG}/question-preferences.json` wins over any future global preference file. Global profile (`~/.zstack/developer-profile.json`) is a starting point for diversity across projects.

## Why event-sourced + dual-track

**Why event-sourced for the inferred profile**:
- Signal map can change between zstack versions. Recompute from events, no data migration needed.
...
### Binaries
- `bin/zstack-question-log` — append log record, validate against registry.
- `bin/zstack-question-preference` — read/write/check/clear explicit preferences.
- `bin/zstack-developer-profile` — supersedes `bin/zstack-builder-profile`. Subcommands: `--read` (legacy compat), `--derive`, `--gap`, `--profile`.
...
- `bin/zstack-builder-profile` — legacy shim delegates to `bin/zstack-developer-profile --read`.
...
- Cons: Requires auditing every existing AskUserQuestion in zstack — substantial up-front work.
...
- Pros: Consistent with zstack's local-first ethos.
- Cons: No aggregate view of onboarding time.

**Chosen: local-only.** If we need TTHW data later, we add it as a zstack-wide telemetry event behind existing opt-in, not a skill-specific one.
...
- Migration from `~/.zstack/builder-profile.jsonl` preserves 100% of sessions + signals_accumulated. Regression test with 7-session fixture.
...
- Dogfood: Zeid uses `/plan-tune` for 2+ weeks. Reports back whether:
...
1. Audit every `AskUserQuestion` invocation in every zstack SKILL.md.tmpl. Build initial `scripts/question-registry.ts` with IDs, categories, door_types, options. This is the foundation; everything else sits on it.
...
6. `bin/zstack-question-log` — validates against registry, rejects unknown IDs.
7. `bin/zstack-question-preference` — all subcommands + tests.
8. `bin/zstack-developer-profile` — `--read` (legacy), `--derive`, `--gap`, `--profile`.
...
This plan was developed through an iterative AI-collaboration loop over ~6 hours of planning. The author (Zeid Mahmoud) directed every scope decision; AI voices (Claude Opus 4.7 and OpenAI Codex gpt-5.4) challenged and refined the plan. Without Codex's outside voice, a much larger and less-defensible plan would have shipped. The value of cross-model review on high-stakes architectural changes is real and measurable.

...
**Branch:** zeidmahmoud/extension-prompt-injection-defense
**Date:** 2026-03-28
**CEO Plan:** ~/.zstack/projects/zeidmahmoud-zstack/ceo-plans/2026-03-28-sidebar-prompt-injection-defense.md

## The Problem

The zstack Chrome extension sidebar gives Claude bash access to control the browser.
...
```typescript
// Public API -- any zstack component can call these
...
```json
// ~/.zstack/security/attempts.jsonl
{
...
```
AskUserQuestion:
  "zstack just blocked a prompt injection attempt from {domain}. These detections
   are rare and valuable for improving defenses for all zstack users. Can we
...
2. Cache to ~/.zstack/models/browsesafe-bench/ (not re-downloaded in CI)
...
- zstack becomes the only open source tool with native-speed prompt injection detection.
...
**Limitation:** macOS-only (Linux would need OpenBLAS FFI). But zstack already
...
- [ ] Implement `loadModel()` with download-on-first-use to ~/.zstack/models/

...
zstack does not install it, bundle it, or depend on it. What zstack adds is a
**bridge**: Memorable's `UserPromptSubmit` hook registered through zstack's own
hook manager, wrapped in the guarantees zstack gives every other off-machine
...
- An explicit zstack-side consent key, `memorable_recall`, off by default and
  listed by `zstack-egress grants` with its revoke command.
- A receipt for every prompt handed to the vendor binary, before the hand-off
  (`zstack-egress list --sink memorable-recall`). No receipt, no hand-off.
...
- A trust envelope and an 8 KiB cap on whatever comes back, and the vendor can
  never block a prompt or speak as zstack.
- The vendor runs in an allowlisted environment (no API keys from your
  ...
  in the vendor's own behaviour, not something zstack can prevent.
- Registration at the stable install, healing on every `./setup`, survival of
  `./setup --no-team`, removal by `zstack-uninstall`, and an off switch that
...
- Not a zstack feature with zstack's guarantees past the process boundary.
  Everything the `memorable` binary does after zstack hands it a prompt belongs
...
|---|---|---|
| `memorable_recall` (zstack) | `zstack-memorable enable` / `disable` | whether zstack's hook hands prompts to the vendor binary at all |
...
zstack never runs the vendor's consent commands and never reads their state.
`zstack-memorable enable` prints them so you can run or inspect them yourself.
Turning the bridge off turns off zstack's hand-off; it does not change what
Memorable is allowed to do with what it already has.

## What zstack hands over, and what it can attest

| Command | What zstack hands to the vendor binary |
|---|---|
| `zstack-memorable status`, `enable`, `disable` | Nothing. They check that the binary exists; they never execute it. |
...
recipient zstack actually ran: `local:<path to the memorable executable>`. It
...
`ZSTACK_*`, no `CLAUDE_*` reaches it. Its stderr is kept out of
...
unknown to zstack (Memorable states: its embed API, on a local recall miss).
...
means zstack wrote enveloped context on its stdout for Claude Code to inject;
whether Claude used it is not something a hook can know.

## What zstack tests, and what is Memorable's claim

zstack tests the bridge in `test/zstack-memorable.test.ts`,
`test/memorable-user-prompt-hook.test.ts` and
`test/zstack-settings-hook-schema-aware.test.ts`, against a fake vendor: every
...
```bash
npm i -g memorable-cli          # the CLI, from npm, not from zstack
memorable login && memorable enable   # the vendor's account and the vendor's consent; only you can do these
zstack-memorable enable         # zstack's gate + the hook, at the stable install
```
...
To hand it to zstack instead, delete that entry from `~/.claude/settings.json`
and run `enable` again. Memorable has no command that removes its own hook.

## Turning it off

```bash
zstack-memorable disable
```

The gate goes off first, so the very next prompt is off even before the entry
is gone; a prompt already past the gate completes. Then zstack's entry is
...
The entry also comes out with `zstack-uninstall` (named in its summary) and
...
only ever hands prompts to the local `memorable` binary. zstack's `/setup-gbrain`
and `/sync-gbrain` are unrelated to it.

## Troubleshooting

- **The hook never fires.** `zstack-memorable status` should show the gate
  `on` and "registered by zstack". If it shows a mismatch line, follow it.
  ...
  during `enable` can drop the entry after zstack printed `registered`; the
...
- **Nothing is ever recalled.** The hand-offs are happening if
  `zstack-egress list --sink memorable-recall` shows receipts with
...
- **Outcomes say `timeout`.** The vendor took longer than the budget allowed
  (roughly 4 seconds after zstack's own work). That is usually network.
- **`hook-errors.log` names a refusal.** `refused:redaction-high` means a
  ...
  `~/.zstack/security` is not writable.
- **`enable` says the stable install predates this bridge.** Run `./setup`
  (or `/zstack-upgrade`) so the hook registered at `~/.claude/skills/zstack`
  is the code that will run.

## Under the hood, accurately

`bin/zstack-memorable` is the front door. It resolves the vendor CLI from
`ZSTACK_MEMORABLE_BIN`, then `MEMORABLE_BIN`, then `~/.memorable/bin/memorable`,
...
Registration goes through `zstack-settings-hook ensure-event` with a 5 s
...
- `zstack-settings-hook list-items --event UserPromptSubmit --owned-by
  zstack-memorable` shows it whether or not the tag survived;
  `list-sources` shows only tagged entries, so it may not.
- `prune-stale --repoint` (run by every `./setup`) heals a stale path.
- `./setup --no-team` excludes it from its sweep; `zstack-uninstall` removes it.
- `zstack-settings-hook rollback` is a whole-file restore of the last

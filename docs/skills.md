...
Detailed guides for every zstack skill — philosophy, workflow, and examples.
...
| [`/browse`](#browse) | **QA Engineer** | Give the agent eyes. Drives your Aside browser first — real sessions, real clicks, real screenshots — through deterministic `aside repl` scripts, and falls back to zstack's own Chromium (~100ms per command) when Aside isn't there. |
| [`/setup-browser-cookies`](#setup-browser-cookies) | **Session Manager** | Fallback-browser skill: import cookies from your real browser (Chrome, Arc, Brave, Edge) into zstack's headless session to test authenticated pages. Unnecessary on Aside, which already has your sessions. |
...
| [`/learn`](#learn) | **Memory** | Manage what zstack learned across sessions. Review, search, prune, and export project-specific patterns and preferences. |
...
| [`/pair-agent`](#browse) | **Remote Agent Bridge** | Pair a remote AI agent (OpenClaw, Codex, Cursor, Hermes) with zstack's own browser. Scoped tunnel, locked allowlist, session token. Fallback-browser skill; agents driving Aside open their own tabs. |
...
| [`/open-zstack-browser`](#open-zstack-browser) | **ZStack Browser** | Launch zstack's own browser headed, with sidebar, anti-bot stealth, auto model routing, cookie import, and Claude Code integration. The visible face of the fallback engine; with Aside open you watch the agent's tabs there. |
...
| [`/make-pdf`](#make-pdf) | **PDF Generator** | Turn any markdown file into a publication-quality PDF. Proper margins, page numbers, cover pages, clickable TOC. Mermaid/excalidraw fences render as vector diagrams; `--to html\|docx` for other formats. Prints through your Aside browser (macOS 15+), or zstack's bundled browser when Aside is absent. |
| [`/diagram`](#diagram) | **Diagram Maker** | English in, diagram out: mermaid source + editable `.excalidraw` (open it on excalidraw.com, hand-drawn style) + rendered SVG/PNG. Fully offline, rendered through your Aside browser (macOS 15+) or zstack's bundled browser when Aside is absent. |
...
| [`/ios-sync`](#ios-sync) | **iOS Bridge Resync** | Regenerate accessors and Swift templates against the latest upstream zstack. Run when you add new `@Observable` classes or upgrade zstack. |
...
Both modes end with a design doc written to `~/.zstack/projects/` — and that doc feeds directly into `/plan-ceo-review` and `/plan-eng-review`. The full lifecycle is now: `office-hours → plan → implement → review → QA → ship → retro`.
...
Visions and decisions are persisted to `~/.zstack/projects/` so they survive beyond the conversation. Exceptional visions can be promoted to `docs/designs/` in your repo for the team.
...
Eng Review is the only required gate (disable with `zstack-config set skip_eng_review true`). CEO and Design are informational — recommended for product and UI changes respectively.

Diff reviews use the `review_freshness` grade computed by `zstack-review-read`, shared by `/ship` and `/land-and-deploy`. CURRENT requires a clean, reviewer-reported completed and converged pass whose captured start and finish fingerprints match the current working-tree content. Tracked edits and non-ignored untracked source both count; an identical commit hash or zero commits since review is not a fallback.
...
When `/plan-eng-review` finishes the test review section, it writes a test plan artifact to `~/.zstack/projects/`. When you later run `/qa`, it picks up that test plan automatically — your engineering review feeds directly into QA testing with no manual copy-paste.
...
Claude: [Runs full 80-item visual audit on the live site]
        ...
        [Report with before/after screenshots saved to .zstack/design-reports/]
```
...
6. The approved variant saves to `~/.zstack/projects/$SLUG/designs/` with an `approved.json`
...
Claude: Approved Variant A. Saved to ~/.zstack/projects/myapp/designs/
...
Claude: Saved to ~/.zstack/projects/myapp/designs/hero-20260330/finalized.html
```
...
Claude: [Explores 12 pages, fills 3 forms, tests 2 flows]
        ...
        [Full report with screenshots saved to .zstack/qa-reports/]
```
...
Claude: [aside repl: console hook → openTab → goto /signup → snapshot
        ...
        > cp "$ASIDE_DIR/issue-001-result.jpg" .zstack/qa-reports/screenshots/
        > Read .zstack/qa-reports/screenshots/issue-001-result.jpg
...
Aside is macOS 15+ and zstack never installs it. Full contract and cookbook pointer: [BROWSER.md](../BROWSER.md).

### When Aside isn't there

Linux, Windows, or a Mac with Aside closed: `/browse` says so once and switches to zstack's own browser for the run — a compiled binary that talks to a persistent Chromium daemon built on [Playwright](https://playwright.dev/). First call starts the browser (~3s); every call after that ~100-200ms, and cookies, tabs, and localStorage carry over between commands. The same skills produce the same evidence; the features that only make sense when the browser is zstack's rather than yours live here: `/setup-browser-cookies` to import your sessions, `/open-zstack-browser` to watch it headed, `/skillify` to codify a `/scrape`, `/pair-agent` to share it with another agent.
...
Before `/qa` or `/browse` can test authenticated pages on zstack's own browser, they need cookies. Instead of manually logging in through the headless browser every time, `/setup-browser-cookies` imports your real sessions directly from your daily browser.
...
zstack learns from every session. Patterns, pitfalls, preferences, architectural decisions... they accumulate in `~/.zstack/projects/$SLUG/learnings.jsonl`. Each learning has a confidence score, source attribution, and the files it references.
...
This is my **co-presence mode** — for the fallback browser. With Aside open, you already watch the agent's tabs in Aside; this skill is how you watch it when the browser is zstack's own.

Without Aside, `/browse` runs headless by default. You don't see what the agent sees. `/open-zstack-browser` changes that. It launches ZStack Browser (rebranded Chromium with anti-bot stealth) controlled by Playwright, with the sidebar extension auto-loaded. You watch every action in real time.

The sidebar chat is a Claude instance that controls the browser. It auto-routes to the right model: Sonnet for navigation and actions (click, goto, fill, screenshot), Opus for reading and analysis (summarize, find bugs, describe). One-click cookie import from the sidebar footer. The browser stays alive as long as the window is open... no idle timeout in headed mode. The menu bar says "ZStack Browser" instead of "Chrome for Testing."
...
```
You:   /open-zstack-browser

Claude: Launched ZStack Browser with sidebar extension.
...
`/codex` brings OpenAI Codex CLI to review the same diff independently. It is available on every harness except Codex itself. External harnesses install it as `/zstack-codex`. Compare its findings with the native review to distinguish corroborated findings from issues only one reviewer caught.

zstack-owned Codex calls default to `gpt-6-astra`, including resumed consult
sessions. Set `ZSTACK_CODEX_MODEL=<model>` to change the default, or name a
...
On Codex hosts, the Claude outside-voice skill is `zstack-claude-code`. Its
review, challenge, and consult calls preserve Claude's configured model.
`ZSTACK_CLAUDE_MODEL=<model>` supplies an explicit override, including resumed
...
Claude Code provides the outside reviewer when zstack runs in Codex. Other non-Claude harnesses also expose this skill for explicit requests; Claude Code itself omits it. External harnesses install it as `/zstack-claude-code`.
...
Keep zstack current with one command. It detects your install type (global at `~/.claude/skills/zstack` vs vendored in your project at `.claude/skills/zstack`), runs the upgrade, syncs both copies if you have dual installs, and shows you what changed.
...
Set `auto_upgrade: true` in `~/.zstack/config.yaml` to skip the prompt entirely — zstack upgrades silently at the start of each session when a new version is available.
...
Install Greptile on your GitHub repo at [greptile.com](https://greptile.com) — it takes about 30 seconds. Once it's reviewing your PRs, zstack picks up its comments automatically. No additional configuration.
...
zstack solves this. `/review` and `/ship` are now Greptile-aware. They read Greptile's comments, classify each one, and take action:
...
Every false positive you confirm gets saved to `~/.zstack/greptile-history.md`. Future runs auto-skip known FP patterns for your codebase. And `/retro` tracks Greptile's batting average over time — so you can see whether the signal-to-noise ratio is improving.
...
Run after upgrading zstack or adding new `@Observable` classes. Detects what's installed, runs gen-accessors against the latest upstream templates, refreshes any changed Swift files, verifies the app rebuilds. Cache-key invalidation handles Swift version changes, generator git rev changes, and source changes.

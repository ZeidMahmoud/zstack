# zstack x OpenClaw Integration

zstack integrates with OpenClaw as a methodology source, not a ported codebase.
OpenClaw's ACP runtime spawns Claude Code sessions natively. zstack provides the
...
```
  ...
  calendar, memory, EA                     methodology + planning
       ...
       │   investigate, retro                   │
       │                                        ├── Generates zstack-lite
       ├── sessions_spawn(runtime: "acp")       │   (planning discipline)
       │       │                                │
       │       └── Claude Code                  ├── Generates zstack-full
       │           └── zstack installed at      │   (complete pipeline)
...
OpenClaw decides at spawn time which tier of zstack support to use:

| Tier | When | Prompt prefix |
|------|------|---------------|
| **Simple** | One-file edits, typos, config changes | No zstack context injected |
| **Medium** | Multi-file features, refactors | zstack-lite CLAUDE.md appended |
| **Heavy** | Specific zstack skill needed | "Load zstack. Run /X" |
| **Full** | Complete features, objectives, projects | zstack-full pipeline appended |
| **Plan** | "Help me plan a Claude Code project" | zstack-plan pipeline appended |
...
The complete ready-to-paste section lives in `openclaw/agents-zstack-section.md`.
Copy it into your OpenClaw AGENTS.md.

Key behavioral rules (these go ABOVE the dispatch tiers):

1. **Always spawn, never redirect.** When the user asks to use ANY zstack skill,
...
zstack-lite/full as a new section. Do not replace the repo's existing instructions.

## What zstack generates for OpenClaw

All artifacts live in the `openclaw/` directory and are generated by
`bun run gen:skill-docs --host openclaw`:

### zstack-lite (Medium tier)
`openclaw/zstack-lite-CLAUDE.md` — ~15 lines of planning discipline:
...
### zstack-full (Full tier)
`openclaw/zstack-full-CLAUDE.md` — chains existing zstack skills:
...
### zstack-plan (Plan tier)
`openclaw/zstack-plan-CLAUDE.md` — full review gauntlet, no implementation:
...
- `zstack-openclaw-office-hours` — Product interrogation (6 forcing questions)
- `zstack-openclaw-ceo-review` — Strategic challenge (10-section review, 4 modes)
- `zstack-openclaw-investigate` — Operational debugging (4-phase methodology)
- `zstack-openclaw-retro` — Operational retrospective (weekly review)
...
environment variable should be set. zstack detects this and adjusts:
...
### Explicit override: ZSTACK_SESSION_KIND

`ZSTACK_SESSION_KIND=spawned` is the explicit per-command marker for the same
classification, outranking every ambient env marker (including
`OPENCLAW_SESSION` and `ZSTACK_HEADLESS`). It exists because Claude Code
...
```bash
ZSTACK_SESSION_KIND=spawned "$_SS" --skill "document-release" ...
```

zstack itself uses this: `/ship` Step 18 dispatches the `/document-release`
...
already has `ZSTACK_HEADLESS`, and letting an env var force `interactive`
...
`ZSTACK_SESSION_KIND=spawned` for a real human's session and silently flip
...
For OpenClaw users: tell your OpenClaw agent "install zstack for openclaw."

The agent should:
1. Install zstack-lite CLAUDE.md into its coding session templates
2. Install the 4 native methodology skills
3. Add dispatch routing to AGENTS.md
4. Verify with a test spawn

For zstack developers: `./setup --host openclaw` outputs this documentation.
...
- No SOUL.md from zstack (OpenClaw has its own)

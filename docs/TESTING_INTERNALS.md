...
allowlist-scrubbed env (operator `CONDUCTOR_*`, `CLAUDE_*`, `ZSTACK_*`,
...
MCP servers / skills), a temp `ZSTACK_HOME`, and `--strict-mcp-config`.
...
(`CONDUCTOR_WORKSPACE_PATH`, per-test `ZSTACK_HOME`) keeps working. The
...
Setting `EVALS_RUN_ID` or `ZSTACK_EVAL_DIR` retains these snapshots; an output
...
`workflowCompleted: false`. With `ZSTACK_EVAL_DIR`, `EVALS_RUN_ID`, or an explicit
...
--verbose`). Results are persisted to `~/.zstack/projects/<slug>/evals/` (legacy
fallback `~/.zstack-dev/evals/`) with auto-comparison
...
browser first (`scripts/resolvers/aside.ts`) and falls back to zstack's own
...
set `ZSTACK_SKIP_ASIDE=1` to force the skip locally (which also exercises the
...
HTML through `lib/aside-render.ts` / `bin/zstack-render.ts`, which render in
...
kill, engine choice and the mid-run fallback). `test/zstack-render-cli.test.ts`
does the same for `bin/zstack-render.ts` with `ZSTACK_SKIP_ASIDE=1` and
`ZSTACK_BROWSE_BIN` pointed at a fake daemon that logs every argv line. A green
...
was skipped (`ZSTACK_SKIP_PLAYWRIGHT=1`) or failed, these gates run and fail
...
processes × `EVALS_CONCURRENCY` within-shard, per-shard `ZSTACK_EVAL_DIR`,
...
`ZSTACK_FLAKE_LEDGER`, the same env var the CI free lane sets before
...
the runner parent and handed to shard children as `ZSTACK_CLAUDE_CLI_VERSION`
...
across Aside present/absent, bootstrap ok/failed/skipped, `ZSTACK_SKIP_ASIDE`),
...
`test/relink.test.ts` shells out to a copy of the real `bin/zstack-relink`
against a temp `ZSTACK_INSTALL_DIR` / `ZSTACK_SKILLS_DIR`, and
...
`ZSTACK_HOME` state-root parity against `bin/zstack-paths`).
...
`ZSTACK_FREE_JOBS` overrides the shard count in either direction (2 is the measured sweet spot — one
serial mega-shard and 6-way sharding both saturate the per-process syscall
supervisor), and `ZSTACK_FREE_RETRY_FLAKY=1` re-runs attributed failures once
...
(`ZSTACK_FLAKE_LEDGER`) — a flaky pass never reds the lane, but it never
disappears either.

...
`ZSTACK_EXTENSION_ID` in `browse/src/server.ts`, derivation reproducible via
...
(`ZSTACK_SECURITY_ENSEMBLE=deberta`) existed until the chat-path agent that
...
- Attack log: `~/.zstack/security/attempts.jsonl` — written by
...
(`~/.zstack/security/session-state.json`), `getStatus()`, the `/health`

...
unavailable, it builds the rest of zstack, removes any stale CSO helpers, and
...
`bin/zstack-cso-launcher` is the setup-built direct command; the compiled core
...
launcher, compiled core, watchdog, and hidden `.zstack-cso-generation` manifest
must be built and installed together for the current host. Windows also requires
the empty `.zstack-cso-generation.lock` file from that same publication. Windows
...
`ZSTACK_CSO_IMAGE_PULL_TIMEOUT_SECONDS=120` when registry speed requires a
...
egress. `zstack-cso doctor --repo PATH` never downloads; it reports a qualified
...
`cso-docker-integration` CI job sets `ZSTACK_CSO_DOCKER_TESTS=1`; unavailable Docker
or missing containment prerequisites fail that job. Run it locally with a
working local daemon:

```bash
bun run build:cso
ZSTACK_CSO_DOCKER_TESTS=1 bun run test:cso:docker
```

When the staged-image workflow supplies `ZSTACK_CSO_TEST_IMAGE`, platform,
...
process; they do not claim published zstack images, passed containment, or

...
loaded into a browser by `bin/zstack-render.ts`, which serves the bundle from
...
zstack's own bundled browser, so it renders offline on every platform (PNGs
...
source is preserved base64-encoded in a `data-zstack-source` attribute on the
...
- **"diagram-render bundle not found"** → run `bun run build:diagram-render`
  in the zstack repo, or re-run `./setup`.

...
|---|---|
| Any host + `zstack-config gbrain-refresh` reports `gbrain_local_status: "ok"` | Compressed brain-aware blocks render. Agent reads this doc on-demand when it actually saves. ~250 token overhead per planning skill. |
...
`bin/zstack-gbrain-sync.ts` for code-lookup resolvers; zstack treats the
...
and writeback. Set via `zstack-config set
...
**Only extract real names** — actual person names (e.g. "Zeid Mahmoud") and
...
- **Any other non-zero exit**: treat as transient failure. Do not retry
  ...
  `zstack-config gbrain-refresh` if they suspect gbrain itself is
  misconfigured.
...
gbrain sources list                    # confirm zstack-brain-<user> source
...
endpoint depending on the user's `~/.gbrain/config.json`. **zstack
...
2. `zstack-config get brain_trust_policy@<endpoint-hash>` must be
   `personal` for auto-write. Run `zstack-config endpoint-hash` to get
...
3. If trust policy is `personal` and `gbrain doctor` is clean but the
   page still isn't there, file an issue against gbrain — zstack's
...
- **`.gbrain-source` write semantics**: zstack treats the documented

...
- zstack installed (`./setup` complete; `zstack-ios-qa-regen` and `zstack-ios-qa-daemon` must be on PATH).
...
`DebugBridgeGenerated/.zstack-version`. It excludes generated output from its
...
If `ZSTACK_IOS_TARGET_UDID` is unset, the daemon picks the best paired,
available iPhone.

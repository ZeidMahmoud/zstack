...
`zstack-cso scan RUN SCANNER [REQUEST.json]` collects candidate evidence through
...
with `zstack-cso import-sarif RUN REPORT.sarif` remain available.
The returned immutable artifact can be read later with
`zstack-cso scanner-outcome RUN ARTIFACT_ID`; callers never need to open private
...
`ZSTACK_CSO_IMAGE_PULL_TIMEOUT_SECONDS=120` selects a longer allowance (accepted
...
requires comprehensive mode and an `api` object; `zstack-cso schema` describes

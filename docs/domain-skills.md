...
> **Scope:** domain-skills are a feature of zstack's own browser engine (`$B`), the **fallback** that runs when the Aside browser is absent (Linux, Windows, a closed Aside app). Skills driving Aside record per-project learnings through `/learn` instead. See [BROWSER.md](../BROWSER.md).
...
This is zstack's borrow from [browser-use/browser-harness](https://github.com/browser-use/browser-harness).
zstack copies the per-site-notes pattern, NOT the self-modifying-runtime
...
on every zstack project I work on"), explicitly run
...
- **Per-project**: `~/.zstack/projects/<slug>/learnings.jsonl` — same JSONL
  file the `/learn` skill uses. Domain skills are `type:"domain"` rows.
- **Global**: `~/.zstack/global-domain-skills.jsonl` — only `state:"global"`
  rows.
...
following events are written to `~/.zstack/analytics/browse-telemetry.jsonl`:
...
`zstack-config set telemetry off` or `ZSTACK_TELEMETRY_OFF=1`.

...
```
zstack/
├── browse/          # /browse skill + zstack's own headless browser engine (Playwright) — the fallback when Aside is absent
...
│   ├── test-free-shards.ts  # Strict parallel free-suite runner (ZSTACK_FREE_JOBS, opt-in flaky retry)
...
│   ├── zstack-render-cli.test.ts # Tier 1: bin/zstack-render.ts argv guards + output contract against a fake daemon
│   ├── skill-validation.test.ts  # Tier 1: static validation (free, <1s)
│   ├── gen-skill-docs.test.ts    # Tier 1: generator quality (free, <1s)
│   ├── setup-*.test.ts, relink.test.ts, hook-scripts.test.ts  # Tier 1: setup linker ownership, retired-skill prune, browser hint, rebuild check + Chromium bootstrap (anchor-sliced from setup), zstack-relink, PreToolUse hooks (free)
...
├── careful/         # /careful skill; bin/check-careful.sh (PreToolUse destructive-command hook) + bin/hook-extract.sh (shared hook helpers: payload extraction, deny JSON, zstack_hook_state_root)
...
├── zstack-upgrade/  # /zstack-upgrade skill + migrations/ (run after ./setup during an upgrade)
├── bin/             # CLI utilities (zstack-render.ts = render a local HTML file through Aside or the engine, zstack-design-detect.ts = probe/scan through a user-installed impeccable engine; zstack-design-md.ts = open DESIGN.md check/convert/tokens/mark; zstack-repo-mode, zstack-slug, zstack-config, zstack-wtree, zstack-evidence, zstack-issue-guard, zstack-relink, zstack-memorable, etc.)
...
├── diagram/         # /diagram skill (mermaid → SVG/PNG/.excalidraw through bin/zstack-render.ts + lib/diagram-render)
├── design/          # Design binary CLI (GPT Image API)
│   ├── src/         # CLI + commands (generate, variants, compare, serve, etc.)
│   ├── test/        # Integration tests
│   └── dist/        # Compiled binary
├── agents-digest/   # Committed 2KB instruction-tier rules digest (zstack-AGENTS.md) for rules-reading hosts

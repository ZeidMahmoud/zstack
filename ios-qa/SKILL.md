...
preamble-tier: 3
version: 1.0.0
description: Live-device iOS QA for SwiftUI apps. (zstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ios-qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.zstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/zstack/bin/zstack-timeline-log '{"skill":"ios-qa","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"ios-qa","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
If `~/.zstack/ios-qa-session.json` exists and the device is still connected,
...
```bash
SESSION="$HOME/.zstack/ios-qa-session.json"
...
2. Run `swift run --package-path $ZSTACK_HOME/ios-qa/scripts/gen-accessors-tool gen-accessors --input <source-dir>`.
...
5. Spawn the Mac-side daemon (on-demand) — `zstack-ios-qa-daemon`. Daemon
   acquires an exclusive flock on `~/.zstack/ios-qa-daemon.pid`. If another
...
`~/.zstack/security/ios-qa-audit.jsonl`.
...
Owner mints via `zstack-ios-qa-mint --remote <identity> --capability <tier>`
...
| `403 identity_not_allowed` from `/auth/mint` | identity missing from allowlist | Run `zstack-ios-qa-mint --remote <identity>` on the Mac |

...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"ios-qa","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "ios-qa" --outcome OUTCOME \
...
1. Generate the canonical local bridge package, typed accessors, and installed
   ...
   ```bash
   ~/.claude/skills/zstack/bin/zstack-ios-qa-regen \
...
6. Spawn the Mac-side daemon (on-demand) — `zstack-ios-qa-daemon`. Daemon

// CLI tests for zstack-ios-qa-mint. Invokes the bash launcher end-to-end
...
// so the user's real ~/.zstack/ios-qa-allowlist.json is untouched.
...
const MINT_BIN = join(ROOT, 'bin', 'zstack-ios-qa-mint');
const DAEMON_BIN = join(ROOT, 'bin', 'zstack-ios-qa-daemon');

function runMint(args: string[]) {
  return spawnSync(MINT_BIN, args, { stdio: 'pipe', encoding: 'utf-8' });
}

describe('bin/zstack-ios-qa-mint launcher', () => {
  let tmpDir: string;
  let listPath: string;
  ...
  test('--help prints usage without touching allowlist', () => {
    ...
    expect(r.stdout).toContain('zstack-ios-qa-mint');
...
describe('bin/zstack-ios-qa-daemon launcher', () => {

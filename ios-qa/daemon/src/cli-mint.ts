...
//
// Invoked from bin/zstack-ios-qa-mint.
...
function printHelp() {
  const help = `zstack-ios-qa-mint — manage the tailnet allowlist for remote iOS QA agents

USAGE
  zstack-ios-qa-mint grant  --remote <identity> [--capability <tier>] [--ttl <seconds>] [--note <text>]
  zstack-ios-qa-mint revoke --remote <identity>
...
EXAMPLES
  ...
  zstack-ios-qa-mint grant --remote 'tag:ci' --capability mutate --ttl 86400 --note 'nightly run'
  zstack-ios-qa-mint revoke --remote 'alice@example.com'
  zstack-ios-qa-mint list

The allowlist lives at ~/.zstack/ios-qa-allowlist.json (mode 0600). The daemon's
...
if (import.meta.main) {
  main().catch((err) => {
    process.stderr.write(`zstack-ios-qa-mint: ${(err as Error).message}\n`);
    process.exit(1);
  });
}

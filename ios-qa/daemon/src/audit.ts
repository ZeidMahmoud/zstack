...
export function defaultAuditPath(): string {
  return process.env.ZSTACK_IOS_AUDIT_PATH
    ?? join(homedir(), '.zstack', 'security', 'ios-qa-audit.jsonl');
}

export function defaultAttemptsPath(): string {
  return process.env.ZSTACK_IOS_ATTEMPTS_PATH
    ?? join(homedir(), '.zstack', 'security', 'attempts.jsonl');
}
...
async function loadDeviceSalt(): Promise<string> {
  ...
  const path = join(homedir(), '.zstack', 'security', 'device-salt');
  try {

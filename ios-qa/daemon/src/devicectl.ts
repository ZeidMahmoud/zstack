...
export async function getDeviceTunnelIPv6(
  ...
  // ".coredevice.local" suffix. Apple normalizes "Zeid's Durendal" to
  // "Zeids-Durendal.coredevice.local".
 ...
 * from `tmp/zstack-ios-qa.token` after the StateServer starts.
 */
export function copyFileFromAppContainer(opts: {
  ...
  const spawn = opts.spawn ?? defaultSpawn;
  const dir = mkdtempSync(join(tmpdir(), 'zstack-ios-copy-'));

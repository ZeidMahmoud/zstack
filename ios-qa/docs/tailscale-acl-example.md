...
- **Auth:** Tailscale identity validation via the local `tailscaled` socket
  ...
  `~/.zstack/ios-qa-allowlist.json` is the single source of truth for who can
  do what.
...
zstack-ios-qa-mint --remote ci@example.com --capability mutate --ttl 1h
...
row to `~/.zstack/security/ios-qa-audit.jsonl`:
...
allowlisted, rate limit hit) write to `~/.zstack/security/attempts.jsonl`.
...
| `403 identity_not_allowed` | identity missing from allowlist | Owner mint: `zstack-ios-qa-mint --remote <identity>` |

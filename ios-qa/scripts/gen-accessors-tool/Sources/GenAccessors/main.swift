...
// ~/.zstack/cache/gen-accessors/<hash>/StateAccessor.swift.
...
struct GenAccessors {
    static func main() async {
        ...
        let cacheDir = getEnv("ZSTACK_IOS_CACHE_ROOT")
            ?? ("~/.zstack/cache/gen-accessors" as NSString).expandingTildeInPath
    ...
    static func render(specs: [AccessorSpec], buildId: String, accessorHash: String) -> String {
        ...
        if !specs.isEmpty {
            ...
            out += "private enum _ZStackDebugBridgeSnapshotJSON {\n"
        ...
        for spec in specs {
            ...
            for (index, field) in spec.fields.enumerated() {
                ...
                out += "                }\n"
                out += "                guard let restored\(index): \(typeText) = _ZStackDebugBridgeSnapshotJSON.decode(raw\(index), as: \(typeText).self) else {\n"
            ...
            for (name, typeText) in spec.fields {
                ...
                out += "            write: { value in\n"
                out += "                guard let typed: \(typeText) = _ZStackDebugBridgeSnapshotJSON.decode(value, as: \(typeText).self) else { return false }\n"

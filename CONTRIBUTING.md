...
Thanks for wanting to make zstack better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

zstack skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/zstack/` (your global install). But when you're developing zstack itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.
...
zstack automatically learns from failures. At the end of every skill session, the agent
reflects on what went wrong (CLI errors, wrong approaches, project quirks) and logs
operational learnings to `~/.zstack/projects/{slug}/learnings.jsonl`. Future sessions
surface these learnings automatically, so zstack gets smarter on your codebase over time.

No setup needed. Learnings are logged automatically. View them with `/learn`.

### The contributor workflow

1. **Use zstack normally** — operational learnings are captured automatically
2. **Check your learnings:** `/learn` or `ls ~/.zstack/projects/*/learnings.jsonl`
3. **Fork and clone zstack** (if you haven't already)
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where zstack annoyed you)
   ln -sfn /path/to/your/zstack-fork .claude/skills/zstack
   cd .claude/skills/zstack && bun install && bun run build && ./setup
   ```
   Setup creates per-skill directories with SKILL.md symlinks inside (`qa/SKILL.md -> zstack/qa/SKILL.md`),
...
6. **Test by actually using zstack** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix zstack while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ zstack sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all skills.

## Working on zstack inside the zstack repo

When you're editing zstack skills and want to test them by actually using zstack
...
```
zstack/                          <- your working tree
├── .claude/skills/              <- created by dev-setup (gitignored)
│   ├── zstack -> ../../         <- symlink back to repo root
│   ├── review/                  <- real directory (short name, default)
│   │   └── SKILL.md -> zstack/review/SKILL.md
│   ├── ship/                    <- or zstack-review/, zstack-ship/ if --prefix
│   │   └── SKILL.md -> zstack/ship/SKILL.md
│   └── ...                      <- one directory per skill
├── review/
│   └── SKILL.md                 <- edit this, test with /review
├── ship/
│   └── SKILL.md
├── browse/                      <- /browse skill + zstack's own browser engine (the fallback)
...
├── bin/
│   └── zstack-render.ts         <- the CLI skills call to render a local HTML file
└── ...
```
...
checklists). Alias skills (`_zstack-command`, `connect-chrome`) install as
...
under `zstack/`. Names depend on your prefix setting (`~/.zstack/config.yaml`).
Short names (`/review`, `/ship`) are the default. Run `./setup --prefix` if you
prefer namespaced names (`/zstack-review`, `/zstack-ship`).
...
If gbrain is installed and usable (`bin/zstack-gbrain-detect --is-ok` exits 0),
...
into `.claude/zstack-rendered/` (gitignored, per-workspace). It then repoints the
...
passes `ZSTACK_SKIP_GBRAIN_REGEN=1` inline to the nested `./setup` (so it never
dirties tracked source) and runs `gen:skill-docs:user --out-dir .claude/zstack-rendered`,
...
sessions, run `zstack-config gbrain-refresh`, which renders them to a user render
dir (`${ZSTACK_USER_RENDER_DIR:-~/.zstack/render/claude}`, swapped in only on a
successful render) and repoints the installed skills at it via `zstack-relink` —
the global install checkout stays git-clean, and the refresh is guarded so it
never touches a symlinked or non-zstack directory.

## Testing & evals

Codex evals and the GPT benchmark adapter default to `gpt-6-astra`:
explicit model > `ZSTACK_CODEX_MODEL` > default. Claude capture and judge
defaults are `claude-fable-5-1`, resolved through `lib/eval-model.ts`:

- Claude session, PTY, and Agent SDK eval runners and the Claude benchmark adapter: explicit model > `EVALS_MODEL` > `ZSTACK_EVAL_MODEL_CAPTURE` > `ZSTACK_EVAL_MODEL` > default.
- Shared judge calls (including benchmark quality scoring): explicit model > `ZSTACK_EVAL_MODEL_JUDGE` > `ZSTACK_EVAL_MODEL` > default. `EVALS_MODEL` applies to capture runners, not judges.
...
`ZSTACK_FREE_JOBS=<n>` overrides the shard count (digits only, loud on garbage),
and `ZSTACK_FREE_RETRY_FLAKY=1` opts into one serial retry pass for
...
seeded `CLAUDE_CONFIG_DIR`, a temp `ZSTACK_HOME`, and `--strict-mcp-config`. Your
operator `~/.claude` config, MCP servers (gbrain, Conductor), skills, `~/.zstack`
...
When E2E tests run, they produce machine-readable artifacts in `~/.zstack-dev/`:
...
wrap the eval command in `bin/zstack-detach`: a fresh session that escapes a
turn-boundary SIGTERM, a `caffeinate` wrapper that blocks idle-sleep, a machine-wide
`zstack-evals` lock so concurrent worktrees serialize instead of saturating the
model API, a run-scoped log under `~/.zstack-dev/eval-runs/`, a per-tier watchdog,
and a guaranteed `### zstack-detach EXIT=<code> ###` sentinel so a poller never
...
eval dir (`ZSTACK_EVAL_DIR=<evalDir>/shards/<slug>/`), and an aggregate that
...
Artifacts are never cleaned up — they accumulate in `~/.zstack-dev/` for post-mortem debugging and trend analysis.

### Tier 3: LLM-as-judge (~$0.15/run)

Uses `claude-fable-5-1` by default to score generated SKILL.md docs on three dimensions.
Override the judge model per run with `ZSTACK_EVAL_MODEL_JUDGE`:
...
- **Quality gate** (`.github/workflows/quality-gate.yml`, every PR and push) — scans the diff's added lines for credentials using zstack's own redact engine (`.github/scripts/gate-secret-scan.mjs`). HIGH findings fail the job; MEDIUM findings surface as an advisory count. Fails closed if the scan can't produce a report. Also gates critical dependency advisories and runs ShellCheck on the setup/build boundaries.
...
cards, og-images, PDFs) calls `bin/zstack-render.ts` from its template
...
`test/aside-render.test.ts`, and in `test/zstack-render-cli.test.ts` when it
...
page-number footer make-pdf, `zstack-render`, and the browse `pdf` command share).

## Jargon list (V1 writing style)

zstack's Writing Style section (injected into every tier-≥2 skill's preamble)
...
zstack generates SKILL.md files for 10 hosts from one set of `.tmpl` templates.
...
|--------|---------------------------|
| Output directory | `{skill}/SKILL.md` vs `.agents/skills/zstack-{skill}/SKILL.md` |
| Frontmatter | Full (name, description, hooks, version) vs minimal (name + description) |
| Paths | `~/.claude/skills/zstack` vs `$ZSTACK_ROOT` |
...
|------|--------|-------------|
| `setup` | `bin/dev-setup` | Copies `.env` from main worktree, installs deps, symlinks skills, runs `./setup` non-interactively, and (if gbrain is installed) renders brain-aware blocks into `.claude/zstack-rendered/` without dirtying tracked source |
| `archive` | `bin/dev-teardown` | Removes skill symlinks, the `.claude/zstack-rendered/` render, and cleans up `.claude/` directory |

When Conductor creates a new workspace, `bin/dev-setup` runs automatically. It detects the main worktree (via `git worktree list`), copies your `.env` so API keys carry over, and sets up dev mode — no manual steps needed.

`bin/dev-setup` runs `./setup` fully non-interactively (it passes `--plan-tune-hooks=prompt` and closes stdin), so a forwarded Conductor TTY can never hang on a hidden setup prompt. It also never installs the plan-tune Claude Code hooks, which means a throwaway workspace can't rewrite your global `~/.claude/settings.json` to point at an ephemeral worktree path. To install the plan-tune hooks deliberately, run `./setup --plan-tune-hooks` outside dev-setup (or `zstack-config set plan_tune_hooks yes`). The explicit flag counts as an explicit decision: setup's Conductor auto-opt-in for AskUserQuestion hooks fires only on the true silent fall-through (no flag, no `ZSTACK_PLAN_TUNE_HOOKS` env var, no `plan_tune_hooks` key literally present in config, checked via `zstack-config has`), so it can never override dev-setup into installing hooks. One stated repair exception: setup's heal-first pass (`zstack-settings-hook prune-stale --repoint`) may prune dead zstack hook entries and re-point existing ones at the stable `~/.claude/skills/zstack` install. That is strictly convergent repair, never a new registration, and registration itself is canonical-only, so an ephemeral tree path can never be baked into settings.json.

**First-time setup:** Put your `ANTHROPIC_API_KEY` in `.env` in the main repo (see `.env.example`). Every Conductor workspace inherits it automatically.

**`ZSTACK_*` env prefix (Conductor-injected keys).** Conductor explicitly strips `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` from every workspace's process env. The `.env` copy path doesn't restore them either — the strip happens after env inheritance. Users who want paid evals, `/sync-gbrain` embeddings, or `claude-agent-sdk` calls to work in a Conductor workspace must set `ZSTACK_ANTHROPIC_API_KEY` and `ZSTACK_OPENAI_API_KEY` in Conductor's workspace env config; Conductor passes those through untouched. On the zstack side, TS entry points import `lib/conductor-env-shim.ts` as a side effect, which promotes `ZSTACK_FOO_API_KEY` to `FOO_API_KEY` when the canonical name is empty. If you add a new TS entry point that hits a paid API, add `import "../lib/conductor-env-shim";` to the top of the file. Today the shim is imported from `bin/zstack-gbrain-sync.ts`, `bin/zstack-model-benchmark`, `scripts/preflight-agent-sdk.ts`, and `test/helpers/e2e-helpers.ts`.
...
- **Dev mode shadows your global install.** Project-local skills take priority over `~/.claude/skills/zstack`. `bin/dev-teardown` restores the global one.
...
- **Never delete or link over a skill entry `setup` cannot prove is zstack's.** Every destructive site in `setup` (the linker, the alias installer, both prefix-flip cleanups) and in `bin/zstack-relink` goes through the ownership helpers (`_claude_entry_is_ours` / `_claude_entry_owned_strongly` in `setup`, `_entry_is_ours` / `_entry_owned_strongly` in relink). The retired-skill prune (`_prune_stale_generated`) applies the same strong/weak split through its own gate: a real host directory is a candidate only when its SKILL.md carries the generated banner (`_owned_for_windows_refresh`), a host symlink is removed only when it resolves into zstack (`_zstack_target_is_ours`), a bannered real directory is cleaned through `_cleanup_weak_dir`, and a symlink inside the render tree is never followed. A symlink into zstack or the `.zstack-owned` marker proves the whole directory; a byte-identical or generated-banner SKILL.md proves only that file, and a differing one is moved to `~/.zstack/backups/skills/<ts>/` first. `test/setup-link-ownership.test.ts`, `test/setup-cleanup-orphans.test.ts`, `test/setup-prune-stale-generated.test.ts`, and `test/relink.test.ts` pin it. The rule is duplicated in the two scripts until the shared helper filed in TODOS.md lands: change both.
- **`./setup` never fails on Chromium.** The Playwright bootstrap (section `# 2` of `setup`) is best-effort and bounded: every failure becomes a reason code (`skipped`, `chromium-install`, `chromium-install-timeout`, `chromium-install-locked`, `windows-no-node`, `windows-node-modules`, `post-install-launch`) printed in the final summary alongside the browser-dependent skills, and skill registration always runs. `ZSTACK_PLAYWRIGHT_INSTALL_TIMEOUT=<seconds>` (default 600) bounds the download; `ZSTACK_SKIP_PLAYWRIGHT=1` skips it, the right knob for a no-browser box or a setup-only test loop. `ZSTACK_SKIP_ASIDE=1` makes the browser summary (like the skills' probe and the renderer) treat Aside as absent, so the summary never promises a fallback the bootstrap did not deliver (`test/setup-browser-hint.test.ts`). Anything you add after the bootstrap must stay independent of the browser. `test/setup-playwright-best-effort.test.ts` pins the block.

## Testing your changes in a real project

**This is the recommended way to develop zstack.** Symlink your zstack checkout
...
```bash
# In your core project (not the zstack repo)
ln -sfn /path/to/your/zstack-checkout .claude/skills/zstack
```

### Step 2: Run setup to create per-skill symlinks

The `zstack` symlink alone isn't enough. Claude Code discovers skills through
individual top-level directories (`qa/SKILL.md`, `ship/SKILL.md`, etc.), not through
the `zstack/` directory itself. Run `./setup` to create them:

```bash
cd .claude/skills/zstack && bun install && bun run build && ./setup
```

Setup will ask whether you want short names (`/qa`) or namespaced (`/zstack-qa`).
Your choice is saved to `~/.zstack/config.yaml` and remembered for future runs.
...
Remove the project-local symlink. Claude Code falls back to `~/.claude/skills/zstack/`:

```bash
rm .claude/skills/zstack
```

The per-skill directories (`qa/`, `ship/`, etc.) contain SKILL.md symlinks that point
to `zstack/...`, so they'll resolve to the global install automatically.

### Switching prefix mode

If you installed zstack with one prefix setting and want to switch:

```bash
cd .claude/skills/zstack && ./setup --no-prefix   # switch to /qa, /ship
cd .claude/skills/zstack && ./setup --prefix       # switch to /zstack-qa, /zstack-ship
```

Setup cleans up the old symlinks automatically. No manual cleanup needed. Only
entries zstack created are removed: a skill of your own that shares a name (a
...
- Renamed or moved config keys in `~/.zstack/config.yaml`
- Need to delete orphaned files from a previous version
- Changed the format of `~/.zstack/` state files
...
1. Create `zstack-upgrade/migrations/v{VERSION}.sh` where `{VERSION}` matches
   the VERSION file for the release that needs the fix.
2. Make it executable: `chmod +x zstack-upgrade/migrations/v{VERSION}.sh`
...
"$SCRIPT_DIR/bin/zstack-relink" 2>/dev/null || true
```

### How it runs

During `/zstack-upgrade`, after `./setup` completes (Step 4.75), the upgrade
skill scans `zstack-upgrade/migrations/` and runs every `v*.sh` script whose
...
verifies that all migration scripts in `zstack-upgrade/migrations/` are

...
preamble-tier: 4
version: 1.0.0
description: Report-only QA testing. (zstack)
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
~/.claude/skills/zstack/bin/zstack-question-log '{"skill":"qa-only","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/zstack/bin/zstack-skill-end --skill "qa-only" --outcome OUTCOME \
...
| Mode | full | `--quick`, `--regression .zstack/qa-reports/baseline.json` |
...
2. **Identify affected pages/routes** from the changed files:
   ...
   - API endpoints → call them with the session's own cookies from one `aside repl` script:
     ...
     await closeTab(pg); console.log("ZSTACK_STEP_OK");
     '
     ```
...
```bash
~/.claude/skills/zstack/bin/zstack-learnings-log '{"skill":"qa-only","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
